...
class DefaultForm
{
    ...
    function __construct(
        ...
        $form .= ">\n";
        $form .=
            $this->generate_hidden_field('my_post_key', $mybb->post_code) .
            "\n";
        if ($return == false) {
            echo $form;
        } else {
     ...
     */
    function generate_text_box($name, $value = '', $options = [])
    {
        $input =
     ...
     */
    function generate_numeric_field($name, $value = 0, $options = [])
    {
        if (is_numeric($value)) {
            $value = (float) $value;
        } else {
     ...
     */
    function generate_password_box($name, $value = '', $options = [])
    {
        $input =
     ...
     */
    function generate_file_upload_box($name, $options = [])
    {
     ...
     */
    function generate_text_area($name, $value = '', $options = [])
    {
        $textarea = '<textarea';
        if (!empty($name)) {
            $textarea .= " name=\"{$name}\"";
     ...
     */
    function generate_check_box($name, $value = '', $label = '', $options = [])
    {
        $input = '<label';
        if (isset($options['id'])) {
            $input .= " for=\"{$options['id']}\"";
        ...
        if (
            ...
            ($options['checked'] === true || $options['checked'] == 1)
        ) {
            $input .= " checked=\"checked\"";
        }
    ...
    function generate_select_box(
        ...
        foreach ($option_list as $value => $option) {
            $select_add = '';
            if (
                (!is_array($selected) || !empty($selected)) &&
                ((is_array($selected) &&
     ...
     */
    function generate_forum_select(
        $name,
        $selected,
        $options = [],
        $is_first = 1
        ...
        if (!is_array($fselectcache)) {
            ...
            foreach ($forum_cache as $fid => $forum) {
                $fselectcache[$forum['pid']][$forum['disporder']][
                    $forum['fid']
                ] = $forum;
            }
        }
            ...
            foreach ($fselectcache[$pid] as $main) {
                foreach ($main as $forum) {
                    if ($forum['fid'] != '0' && $forum['linkto'] == '') {
                        ...
                        $selectoptions .=
                            "<option value=\"{$forum['fid']}\"{$style}{$select_add}>" .
                            $sep .
                            htmlspecialchars_uni(strip_tags($forum['name'])) .
                            "</option>\n";

                        if ($forum_cache[$forum['fid']]) {
                            $options['depth'] += 5;
                            $options['pid'] = $forum['fid'];
                            $this->generate_forum_select(
                                $forum['fid'],
                                $selected,
                                $options,
                                0
     ...
     * @return string The built select box.
     */
    function generate_group_select($name, $selected = [], $options = [])
    {
        global $cache;
        ...
        foreach ($groups_cache as $group) {
            ...
            $select .=
                "<option value=\"{$group['gid']}\"{$selected_add}>" .
     ...
     * @return string The built select box.
     */
    function generate_prefix_select($name, $selected = [], $options = [])
    {
        global $cache;
        $select = "<select name=\"{$name}\"";
        if (isset($options['multiple'])) {
        ...
        foreach ($prefix_cache as $prefix) {
            ...
            $select .=
                "<option value=\"{$prefix['pid']}\"{$selected_add}>" .
                htmlspecialchars_uni($prefix['prefix']) .
                '</option>';
        }
        $select .= '</select>';
        return $select;
     ...
     */
    function generate_submit_button($value, $options = [])
    {
        $input =
     ...
     */
    function generate_reset_button($value, $options = [])
    {
        $input =
    ...
    function generate_yes_no_radio(
        ...
        $yes = $this->generate_radio_button(
            $name,
            $yes_value,
            $lang->yes,
            $yes_options
        );
        $no = $this->generate_radio_button(
            $name,
            $no_value,
            $lang->no,
            $no_options
     ...
     */
    function generate_on_off_radio(
        $name,
        $value = 1,
        $int = true,
        $on_options = [],
        ...
        $on = $this->generate_radio_button(
            $name,
            $on_value,
            $lang->on,
            $on_options
        );
        $off = $this->generate_radio_button(
            $name,
            $off_value,
            $lang->off,
            $off_options
    ...
    function generate_date_select($name, $day = 0, $month = 0, $year = 0)
    {
        ...
        $built =
            $this->generate_select_box($name . '_day', $days, (int) $day, [
                'id' => $name . '_day',
            ]) . ' &nbsp; ';
        $built .=
            $this->generate_select_box(
                $name . '_month',
                $months,
                (int) $month,
                ['id' => $name . '_month']
            ) . ' &nbsp; ';
        $built .= $this->generate_numeric_field($name . '_year', $year, [
            'id' => $name . '_year',
            'style' => 'width: 100px;',
            'min' => 0,
        ]);
...
class DefaultFormContainer
{
     ...
     */
    function output_row_header($title, $extra = [])
    {
     ...
     */
    function output_cell($data, $options = [])
    {
    ...
    function end($return = false)
    {
        ...
        if ($return == true) {
            return $this->_container->output(
                $this->_title,
                1,
                "general form_container {$this->extra_class}",
                true
            );
        } else {
            echo $this->_container->output(
                $this->_title,
                1,
                "general form_container {$this->extra_class}",
                false
