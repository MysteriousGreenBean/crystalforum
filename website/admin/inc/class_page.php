...
class DefaultPage
{
    ...
    function output_header($title = '')
    {
        ...
        echo "	<link rel=\"stylesheet\" href=\"styles/" .
            $this->style .
            "/main.css?ver=1813\" type=\"text/css\" />\n";
        echo "	<link rel=\"stylesheet\" href=\"styles/" .
            $this->style .
            "/modal.css?ver=1813\" type=\"text/css\" />\n";

        // Load stylesheet for this module if it has one
        if (
            file_exists(
                MYBB_ADMIN_DIR .
                    "styles/{$this->style}/{$this->active_module}.css"
            )
        ) {
...
//]]>
        ...
        echo "           <div id=\"inner\">\n";
        if (
            isset($admin_session['data']['flash_message']) &&
            $admin_session['data']['flash_message']
        ) {
    ...
    function output_footer($quit = true)
    {
        ...
        if (my_strpos(getenv('REQUEST_URI'), '?')) {
            ...
            $debuglink =
                htmlspecialchars_uni(getenv('REQUEST_URI')) . '?debug=1#footer';
        }
        ...
        echo "	<br style=\"clear: both;\" />";
        echo "	</div>\n";
        echo "<div id=\"footer\"><p class=\"generation\">" .
            $lang->sprintf(
                $lang->generated_in,
                $totaltime,
                $debuglink,
            ...
            "</p><p class=\"powered\">Powered By <a href=\"https://mybb.com/\" target=\"_blank\" rel=\"noopener\">MyBB</a>, &copy; 2002-" .
            COPY_YEAR .
            " <a href=\"https://mybb.com/\" target=\"_blank\" rel=\"noopener\">MyBB Group</a>.</p></div>\n";
        if ($mybb->debug_mode) {
            echo $db->explain;
        }
        echo "</div>\n";
    ...
    function show_login($message = '', $class = 'success')
    {
        ...
        if (isset($lang->login_field_width)) {
            $login_label_width =
                " style=\"width: " .
                ((int) $lang->login_field_width + 100) .
                "px;\"";
            $login_container_width =
                " style=\"width: " .
                (410 + ((int) $lang->login_field_width)) .
                "px;\"";
        }
...
EOF;
    ...
    function show_lockedout()
    {
        ...
        $lockedout_message = $lang->sprintf(
            $lang->error_mybb_admin_lockedout_message,
            $allowed_attempts
        );
...
EOF;
     ...
     */
    function add_menu_item($title, $id, $link, $order = 10, $submenu = [])
    {
        $this->_menu[$order][] = [
            'title' => $title,
            'id' => $id,
     ...
     */
    function output_nav_tabs($tabs = [], $active = '')
    {
        global $plugins;
    ...
    function output_confirm_action($url, $message = '', $title = '')
    {
        ...
        echo $form->generate_submit_button($lang->yes, [
            'class' => 'button_yes',
        ]);
            ...
            if (is_array($smiliecache)) {
                ...
                foreach ($smiliecache as $smilie) {
                    ...
                    $find = str_replace(
                        ['\\', '"'],
                        ['\\\\', '\"'],
                        htmlspecialchars_uni($finds[0])
                    );
                    $image = str_replace(
                        ['\\', '"'],
                        ['\\\\', '\"'],
                        htmlspecialchars_uni($smilie['image'])
                    );
        ...
        foreach ($items as $item) {
            if (
                !check_admin_permissions(
                    ['module' => $run_module, 'action' => $item['id']],
                    false
                )
            ) {
                continue;
