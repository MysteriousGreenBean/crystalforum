...
class DefaultTable
{
     ...
     */
    function construct_cell($data, $extra = [])
    {
        $this->_cells[] = ['data' => $data, 'extra' => $extra];
    }
    ...
    function construct_row($extra = [])
    {
        ...
        foreach ($this->_cells as $key => $cell) {
            ...
            if (
                isset($cell['extra']['colspan']) &&
                $cell['extra']['colspan'] > 1
            ) {
                $cells .= " colspan=\"" . $cell['extra']['colspan'] . "\"";
            }
            if (
                isset($cell['extra']['rowspan']) &&
                $cell['extra']['rowspan'] > 1
            ) {
     ...
     */
    function construct_header($data, $extra = [])
    {
        $this->_headers[] = ['data' => $data, 'extra' => $extra];
    }
    ...
    function construct_html(
        ...
        if ($this->_headers) {
            ...
            foreach ($this->_headers as $key => $data) {
                ...
                if (
                    isset($data['extra']['colspan']) &&
                    $data['extra']['colspan'] > 1
                ) {
        ...
        foreach ($this->_rows as $key => $table_row) {
            ...
            if ($table_row['extra']['class']) {
                $table .=
                    " class=\"" . trim($table_row['extra']['class']) . "\"";
            }
            $table .= ">\n";
        ...
        $table .= "</table>\n";
        // Clean up
        $this->_cells = $this->_rows = $this->_headers = [];
        if ($border == 1) {
            $table .= '</div>';
        }
        return $table;
