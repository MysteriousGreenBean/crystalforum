...
function make_parent_list($fid, $navsep = ',')
{
    ...
    if (!$pforumcache) {
        $query = $db->simple_select('forums', 'name, fid, pid', '', [
    ...
    foreach ($pforumcache[$fid] as $key => $forum) {
        if ($fid == $forum['fid']) {
            if (!empty($pforumcache[$forum['pid']])) {
                $navigation =
                    make_parent_list($forum['pid'], $navsep) . $navigation;
            }
...
function save_quick_perms($fid)
{
    ...
    while ($usergroup = $db->fetch_array($query)) {
        $query2 = $db->simple_select(
            'forumpermissions',
            $db->escape_string(implode(',', array_keys($permission_fields))),
            "fid='{$fid}' AND gid='{$usergroup['gid']}'",
            ['limit' => 1]
        );
        ...
        if (!$existing_permissions) {
            $query2 = $db->simple_select(
                'usergroups',
                $db->escape_string(
                    implode(',', array_keys($usergroup_permission_fields))
                ),
                "gid='{$usergroup['gid']}'",
                ['limit' => 1]
            );
        ...
        if (empty($inherit[$usergroup['gid']])) {
            ...
            foreach ($permission_fields as $field => $value) {
                ...
                $insertquery[$db->escape_string($field)] = isset(
                    $existing_permissions[$field]
                )
                    ? (int) $existing_permissions[$field]
                    : 0;
            }
...
function check_admin_permissions($action, $error = true)
{
    ...
    if (function_exists($action['module'] . '_admin_permissions')) {
        ...
        if (
            ...
            empty(
                $mybb->admin['permissions'][$action['module']][
                    $action['action']
                ]
            )
        ) {
            if ($error) {
                ...
                $page->add_breadcrumb_item(
                    $lang->access_denied,
                    'index.php?module=home-index'
                );
                $page->output_error(
                    "<b>{$lang->access_denied}</b><ul><li style=\"list-style-type: none;\">{$lang->access_denied_desc}</li></ul>"
                );
                $page->output_footer();
                exit();
            } else {
...
function get_admin_permissions($get_uid = 0, $get_gid = 0)
{
    ...
    if ($get_gid && !$get_uid) {
        ...
        $query = $db->simple_select(
            'adminoptions',
            'permissions',
            "(uid='-{$get_gid}' OR uid='0') AND permissions != ''",
            $options
        );
        ...
        $query = $db->simple_select(
            'adminoptions',
            'permissions, uid',
            "(uid='{$uid}'{$group_sql}) AND permissions != ''",
            $options
        );
...
function change_admin_permission($tab, $page = '', $default = 1)
{
    ...
    while ($adminoption = $db->fetch_array($query)) {
        $adminoption['permissions'] = my_unserialize(
            $adminoption['permissions']
        );
        ...
        $db->update_query(
            ...
            [
                'permissions' => $db->escape_string(
                    my_serialize($adminoption['permissions'])
                ),
            ],
            "uid='{$adminoption['uid']}'"
        );
...
function login_attempt_check_acp($uid = 0, $return_num = false)
{
    ...
    if (
        $mybb->settings['maxloginattempts'] > 0 &&
        $attempts['loginattempts'] >= $mybb->settings['maxloginattempts']
    ) {
...
function check_template($template)
{
    ...
    if (
        preg_match(
            '#\$config\[(([\'|"]database[\'|"])|([^\'"].*?))\]\[(([\'|"](database|hostname|password|table_prefix|username)[\'|"])|([^\'"].*?))\]#i',
            $template
        ) !== 0
    ) {
        return true;
    ...
    $allowed = preg_replace(
        '~\\{\\$+[a-zA-Z_][a-zA-Z_0-9]*((?:-\\>|\\:\\:)\\$*[a-zA-Z_][a-zA-Z_0-9]*|\\[\s*\\$*([\'"]?)[a-zA-Z_ 0-9 ]+\\2\\]\s*)*\\}~',
        '',
        $template
    );
    ...
    if (!$db->num_rows($query)) {
        ...
        if (!$db->num_rows($query)) {
            ...
            while ($post = $db->fetch_array($query)) {
                ...
                if (
                    !in_array($post['thread'], $thread_list) &&
                    !in_array($post['thread'], $delete_thread_list)
                ) {
...
<div class="confirm_action">
	...
	<p class="buttons">
		{$form->generate_submit_button($lang->proceed, [
        'class' => 'button_yes',
        'id' => 'proceed_button',
    ])}
	</p>
