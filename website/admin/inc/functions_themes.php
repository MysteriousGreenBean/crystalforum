 ...
 */
function import_theme_xml($xml, $options = [])
{
    global $mybb, $db;
    ...
    if ($css_120) {
        ...
        $theme['stylesheets']['stylesheet'][0]['attributes'] = [
            'name' => 'global.css',
    ...
    if (
        !empty($theme['templates']['template']) &&
        empty($options['no_templates'])
    ) {
        ...
        if (!empty($options['templateset'])) {
            ...
            $sid = $db->insert_query('templatesets', [
                'title' => $db->escape_string($name) . ' Templates',
            ]);
        }
        ...
        foreach ($templates as $template) {
            $templatecache[] = [
                ...
                'version' => $db->escape_string(
                    $template['attributes']['version']
                ),
                'dateline' => TIME_NOW,
            ];
        }
    ...
    if (
        !empty($theme['stylesheets']['stylesheet']) &&
        empty($options['no_stylesheets'])
    ) {
        ...
        if ($db->num_rows($query)) {
            $inherited_stylesheets = my_unserialize(
                $db->fetch_field($query, 'stylesheets')
            );

            if (
                isset($inherited_stylesheets['inherited']) &&
                is_array($inherited_stylesheets['inherited'])
            ) {
                $loop = 1;
                foreach (
                    $inherited_stylesheets['inherited']
                    as $action => $stylesheets
                ) {
        ...
        foreach ($theme['stylesheets']['stylesheet'] as $stylesheet) {
            $stylesheet['attributes']['name'] = my_substr(
            ...
            $properties['disporder'][$stylesheet['attributes']['name']] =
                $stylesheet['attributes']['disporder'];
            ...
            $new_stylesheet = [
                ...
                'lastmodified' =>
                    (int) $stylesheet['attributes']['lastmodified'],
        ...
        $updated_theme = [
            'stylesheets' => $db->escape_string(
                my_serialize($theme_stylesheets)
            ),
        ];
 ...
 */
function parse_theme_variables($string, $variables = [])
{
    $find = [];
    $replace = [];
...
function cache_stylesheet($tid, $filename, $stylesheet)
{
    ...
    elseif (!is_dir(MYBB_ROOT . $theme_directory)) {
        if (!@mkdir(MYBB_ROOT . $theme_directory, 0755, true)) {
            $theme_directory = 'cache/themes';
            $filename = $tid . '_' . $filename;
        } else {
...
function resync_stylesheet($stylesheet)
{
    ...
    if (
        ...
        strpos($stylesheet['cachefile'], 'css.php') !== false
    ) {
        return false;
    }
    ...
    if (
        !file_exists(
            MYBB_ROOT .
                "cache/themes/theme{$stylesheet['tid']}/{$stylesheet['name']}"
        ) &&
        !file_exists(
            MYBB_ROOT .
                "cache/themes/{$stylesheet['tid']}_{$stylesheet['name']}"
        )
    ) {
        if (
            cache_stylesheet(
...
function build_new_theme($name, $properties = null, $parent = 1)
{
                ...
                foreach ($parent_properties as $property => $value) {
                    ...
                    if (!empty($parent_properties['inherited'][$property])) {
                        $properties['inherited'][$property] =
                            $parent_properties['inherited'][$property];
                    } else {
            ...
            foreach ($parent_stylesheets as $location => $value) {
                ...
                foreach ($value as $action => $sheets) {
                    foreach ($sheets as $stylesheet) {
                        ...
                        if (
                            !empty(
                                $parent_stylesheets['inherited'][
                                    $inherited_check
                                ][$stylesheet]
                            )
                        ) {
                            $stylesheets['inherited'][$inherited_check][
                                $stylesheet
                            ] =
                                $parent_stylesheets['inherited'][
                                    $inherited_check
    ...
    if (!$inherited_properties) {
        ...
        $properties['logo'] = parse_theme_variables(
            $properties['logo'],
            $theme_vars
        );
    }
    ...
    if (!empty($stylesheets)) {
        $updated_theme['stylesheets'] = $db->escape_string(
            my_serialize($stylesheets)
        );
    }
    $updated_theme['properties'] = $db->escape_string(
        my_serialize($properties)
    );
...
function css_to_array($css)
{
    ...
    for ($i = 0; $i < $total; $i++) {
        ...
        if (isset($description_match[count($description_match) - 1][0])) {
            $description = trim(
                $description_match[count($description_match) - 1][0]
            );
        }
...
function copy_stylesheet_to_theme($stylesheet, $tid)
{
    ...
    foreach ($stylesheet as $key => $value) {
        if (!is_numeric($key)) {
            $new_stylesheet[$db->escape_string($key)] = $db->escape_string(
                $value
            );
        }
    }
 ...
 * @return bool
 */
function update_theme_stylesheet_list(
    $tid,
    $theme = false,
    $update_disporders = true
) {
    ...
    foreach ($stylesheets as $name => $stylesheet) {
        ...
        foreach ($parent_list as $theme_id) {
            if (
                ...
                if (
                    file_exists(
                        $cdnpath .
                            "cache/themes/theme{$theme_id}/{$stylesheet['name']}"
                    ) &&
                    filemtime(
                        $cdnpath .
                            "cache/themes/theme{$theme_id}/{$stylesheet['name']}"
        ...
        if (is_object($plugins)) {
            $plugins->run_hooks(
                'update_theme_stylesheet_list_set_css_url',
                $css_url
            );
        }
        ...
        foreach ($attachedto as $attached_file) {
            ...
            foreach ($attached_actions as $action) {
                ...
                if (!empty($stylesheet['inherited'])) {
                    $theme_stylesheets['inherited'][
                        "{$attached_file}_{$action}"
                    ][$css_url] = $stylesheet['inherited'];
                }
            }
        }
    }
    ...
    if ($update_disporders) {
        ...
        foreach ($stylesheets as $stylesheet) {
            ...
            if (
                $properties['disporder'][$stylesheet['name']] > $max_disporder
            ) {
                $max_disporder = $properties['disporder'][$stylesheet['name']];
            }

            $orders[$stylesheet['name']] =
                $properties['disporder'][$stylesheet['name']];
        }
...
function build_theme_list($parent = 0, $depth = 0)
{
    ...
    if (!is_array($theme_cache)) {
        $themes = cache_themes();
        $query = $db->simple_select('users', 'style, COUNT(uid) AS users', '', [
            'group_by' => 'style',
        ]);
        while ($user_themes = $db->fetch_array($query)) {
            ...
            if (
                isset($themes[$user_themes['style']]['users']) &&
                $themes[$user_themes['style']]['users'] > 0
            ) {
                $themes[$user_themes['style']]['users'] +=
                    (int) $user_themes['users'];
            } else {
                $themes[$user_themes['style']]['users'] =
                    (int) $user_themes['users'];
            }
    ...
    foreach ($theme_cache[$parent] as $theme) {
        ...
        if ($theme['tid'] > 1) {
            ...
            $theme['name'] =
                "<a href=\"index.php?module=style-themes&amp;action=edit&amp;tid={$theme['tid']}\">" .
            ...
            if ($theme_cache['num_themes'] > 2) {
                $popup->add_item(
                    $lang->delete_theme,
                    "index.php?module=style-themes&amp;action=delete&amp;tid={$theme['tid']}&amp;my_post_key={$mybb->post_code}",
                    "return AdminCP.deleteConfirmation(this, '{$lang->confirm_theme_deletion}')"
                );
            }
            ...
            if ($theme['def'] != 1) {
                $popup->add_item(
                    $lang->set_as_default,
                    "index.php?module=style-themes&amp;action=set_default&amp;tid={$theme['tid']}&amp;my_post_key={$mybb->post_code}"
                );
            ...
            $popup->add_item(
                $lang->force_on_users,
                "index.php?module=style-themes&amp;action=force&amp;tid={$theme['tid']}&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_theme_forced}')"
            );
            $set_default = "<div class=\"float_right\">{$set_default}</div>";
        }
        $popup->add_item(
            $lang->export_theme,
            "index.php?module=style-themes&amp;action=export&amp;tid={$theme['tid']}"
        );
        $popup->add_item(
        ...
        $table->construct_cell(
            "{$set_default}<div style=\"margin-left: {$padding}px;\"><strong>{$theme['name']}</strong></div>"
        );
        $table->construct_cell(my_number_format($theme['users']), [
            'class' => 'align_center',
        ]);
...
function build_theme_array($ignoretid = null, $parent = 0, $depth = 0)
{
    ...
    if (
        !isset($theme_cache[$parent]) ||
        !is_array($theme_cache[$parent]) ||
        $ignoretid === $parent
    ) {
        return null;
    }
...
function fetch_theme_stylesheets($theme)
{
        ...
        foreach ($action_stylesheet as $action => $style) {
            foreach ($style as $stylesheet2) {
                ...
                if (
                    ...
                    $stylesheets[$stylesheet2]['inherited'] =
                        $file_stylesheets['inherited'][$file . '_' . $action];
                    foreach (
                        $file_stylesheets['inherited'][$file . '_' . $action]
                        as $value
    ...
    foreach ($stylesheets as $file => $stylesheet2) {
        if (
            isset($stylesheet2['inherited']) &&
            is_array($stylesheet2['inherited'])
        ) {
...
function upgrade_css_120_to_140($css)
{
    ...
    foreach ($parsed_css as $class_id => $array) {
        $parsed_css[$class_id]['values'] = str_replace(
            '#eea8a1',
            '#ffdde0',
            $array['values']
        );
    ...
    $to_add = [
        md5('.trow_selected td') => [
            'class_name' => '.trow_selected td',
            'values' => 'background: #FFFBD9;',
        ],
        md5('blockquote') => [
        ...
        md5('blockquote cite') => [
            'class_name' => 'blockquote cite',
            'values' =>
                "font-weight: bold;\n\tborder-bottom: 1px solid #ccc;\n\tfont-style: normal;\n\tdisplay: block;\n\tmargin: 4px 0;",
        ],
        md5('blockquote cite span') => [
            'class_name' => 'blockquote cite span',
            'values' => "float: right;\n\tfont-weight: normal;",
        ],
        md5('.codeblock') => [
        ...
        md5('.codeblock .title') => [
            'class_name' => '.codeblock .title',
            'values' =>
                "border-bottom: 1px solid #ccc;\n\tfont-weight: bold;\n\tmargin: 4px 0;",
        ],
        md5('.codeblock code') => [
            'class_name' => '.codeblock code',
            'values' =>
                "overflow: auto;\n\theight: auto;\n\tmax-height: 200px;\n\tdisplay: block;\n\tfont-family: Monaco, Consolas, Courier, monospace;\n\tfont-size: 13px;",
        ],
        md5('.subject_new') => [
            'class_name' => '.subject_new',
            'values' => 'font-weight: bold;',
        ],
        md5('.highlight') => [
            'class_name' => '.highlight',
            'values' => "background: #FFFFCC;\n\tpadding: 3px;",
        ],
        md5('.pm_alert') => [
            'class_name' => '.pm_alert',
            'values' =>
        ...
        md5('.high_warning') => [
            'class_name' => '.high_warning',
            'values' => 'color: #CC0000;',
        ],
        md5('.moderate_warning') => [
            'class_name' => '.moderate_warning',
            'values' => 'color: #F3611B;',
        ],
        md5('.low_warning') => [
            'class_name' => '.low_warning',
            'values' => 'color: #AE5700;',
        ],
        md5('div.error') => [
            'class_name' => 'div.error',
            'values' =>
        ...
        md5('div.error p') => [
            'class_name' => 'div.error p',
            'values' => "margin: 0;\n\tcolor: #000;\n\tfont-weight: normal;",
        ],
        md5('div.error p em') => [
            'class_name' => 'div.error p em',
            'values' => "font-style: normal;\n\tfont-weight: bold;\n\tpadding-left: 24px;\n\tdisplay: block;\n\tcolor: #C00;\n\tbackground: url({$mybb->settings['bburl']}/images/error.png) no-repeat 0;",
        ],
        md5('div.error.ul') => [
            'class_name' => 'div.error.ul',
            'values' => 'margin-left: 24px;',
        ],
        md5('.online') => [
            'class_name' => '.online',
            'values' => 'color: #15A018;',
        ...
        md5('.tfoot .pagination, .tcat .pagination') => [
            'class_name' => '.tfoot .pagination, .tcat .pagination',
            'values' => 'padding-top: 0;',
        ],
        md5('.pagination .pages') => [
            'class_name' => '.pagination .pages',
            'values' => 'font-weight: bold;',
        ],
        md5('.pagination .pagination_current, .pagination a') => [
            'class_name' => '.pagination .pagination_current, .pagination a',
            'values' => "padding: 2px 6px;\n\tmargin-bottom: 3px;",
        ],
        md5('.pagination a') => [
            'class_name' => '.pagination a',
            'values' => 'border: 1px solid #81A2C4;',
        ],
        md5('.pagination .pagination_current') => [
            'class_name' => '.pagination .pagination_current',
            'values' =>
                "background: #F5F5F5;\n\tborder: 1px solid #81A2C4;\n\tfont-weight: bold;",
        ],
        md5('.pagination a:hover') => [
            'class_name' => '.pagination a:hover',
            'values' => "background: #F5F5F5;\n\ttext-decoration: none;",
        ],
        md5('.thread_legend, .thread_legend dd') => [
            'class_name' => '.thread_legend, .thread_legend dd',
            'values' => "margin: 0;\n\tpadding: 0;",
        ],
        md5('.thread_legend dd') => [
            'class_name' => '.thread_legend dd',
            'values' => "padding-bottom: 4px;\n\tmargin-right: 15px;",
        ],
        md5('.thread_legend img') => [
            'class_name' => '.thread_legend img',
            'values' => "margin-right: 4px;\n\tvertical-align: bottom;",
        ],
        md5('.forum_legend, .forum_legend dt, .forum_legend dd') => [
            'class_name' => '.forum_legend, .forum_legend dt, .forum_legend dd',
            'values' => "margin: 0;\n\tpadding: 0;",
        ],
        md5('.forum_legend dd') => [
            'class_name' => '.forum_legend dd',
            'values' => "float: left;\n\tmargin-right: 10px;",
        ],
        md5('.forum_legend dt') => [
            'class_name' => '.forum_legend dt',
            'values' => "margin-right: 10px;\n\tfloat: left;",
        ],
        md5('.success_message') => [
        ...
        md5('.post_content') => [
            'class_name' => '.post_content',
            'values' => 'padding: 5px 10px;',
        ],
        md5('.invalid_field') => [
            'class_name' => '.invalid_field',
            'values' => "border: 1px solid #f30;\n\tcolor: #f30;",
        ],
        md5('.valid_field') => [
            'class_name' => '.valid_field',
            'values' => 'border: 1px solid #0c0;',
        ],
        md5('.validation_error') => [
            'class_name' => '.validation_error',
            'values' =>
                "background: url(images/invalid.png) no-repeat center left;\n\tcolor: #f30;\n\tmargin: 5px 0;\n\tpadding: 5px;\n\tfont-weight: bold;\n\tfont-size: 11px;\n\tpadding-left: 22px;",
        ],
        md5('.validation_success') => [
            'class_name' => '.validation_success',
            'values' =>
                "background: url(images/valid.png) no-repeat center left;\n\tcolor: #00b200;\n\tmargin: 5px 0;\n\tpadding: 5px;\n\tfont-weight: bold;\n\tfont-size: 11px;\n\tpadding-left: 22px;",
        ],
        md5('.validation_loading') => [
            'class_name' => '.validation_loading',
            'values' =>
