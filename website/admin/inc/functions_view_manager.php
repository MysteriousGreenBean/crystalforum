    ...
    if ($mybb->input['do'] == 'add') {
        if ($mybb->request_method == 'post') {
            ...
            if ($mybb->input['fields_js']) {
                $mybb->input['fields'] = explode(
                    ',',
                    $mybb->input['fields_js']
                );
            }
            if (
                !isset($mybb->input['fields']) ||
                !is_array($mybb->input['fields']) ||
                count($mybb->input['fields']) <= 0
            ) {
            ...
            if (empty($errors)) {
                $new_view = [
                    ...
                    'type' => $type,
                    'visibility' => $mybb->get_input(
                        'visibility',
                        MyBB::INPUT_INT
                    ),
                    'fields' => $db->escape_string(
                        my_serialize($mybb->input['fields'])
                    ),
                    'conditions' => $db->escape_string(
                        my_serialize($mybb->input['conditions'])
                    ),
                    'custom_profile_fields' => $db->escape_string(
                        my_serialize($mybb->input['profile_fields'])
                    ),
                    'sortby' => $db->escape_string($mybb->input['sortby']),
                    'sortorder' => $db->escape_string(
                        $mybb->input['sortorder']
                    ),
                    'perpage' => $mybb->get_input('perpage', MyBB::INPUT_INT),
                    'view_type' => $db->escape_string(
                        $mybb->input['view_type']
                    ),
                ];
            ...
            $mybb->input = array_merge($mybb->input, ['perpage' => 20]);
        }

        // Write in our JS based field selector
        $page->extra_header .=
            "<script src=\"jscripts/view_manager.js\" type=\"text/javascript\"></script>\n";
        ...
        $visibility_options = [
            $form->generate_radio_button(
                'visibility',
                '1',
                "<strong>{$lang->private}</strong> - {$lang->private_desc}",
                ['checked' => $visibility_private_checked]
            ),
            $form->generate_radio_button(
                'visibility',
                '2',
                "<strong>{$lang->public}</strong> - {$lang->public_desc}",
                ['checked' => $visibility_public_checked]
            ),
        ];
        $form_container->output_row(
            $lang->visibility,
            '',
            implode('<br />', $visibility_options)
        );

        $form_container->output_row(
            $lang->set_as_default_view,
            '',
            $form->generate_yes_no_radio(
                'isdefault',
                $mybb->get_input('isdefault'),
                ['yes' => 1, 'no' => 0]
            )
        );
...
//]]>
        ...
        $field_select .=
            ...
            $form->generate_select_box(
                'fields[]',
                $field_options,
                $mybb->get_input('fields'),
                ['id' => 'fields', 'multiple' => true]
            ) .
            "</noscript>\n";

        $form_container = new FormContainer($lang->fields_to_show);
        $form_container->output_row(
            $lang->fields_to_show_desc,
            '',
            $field_select
        );
        $form_container->end();
        ...
        $form = new Form(
            $base_url .
                "&amp;action=views&amp;do=edit&amp;vid={$admin_view['vid']}",
            'post'
        );
        ...
        if (!empty($errors)) {
            ...
            $admin_view['conditions'] = my_unserialize(
                $admin_view['conditions']
            );
            $admin_view['fields'] = my_unserialize($admin_view['fields']);
            $admin_view['profile_fields'] = my_unserialize(
                $admin_view['custom_profile_fields']
            );
        ...
        $form_container->output_row(
            $lang->results_per_page,
            '',
            $form->generate_numeric_field('perpage', $mybb->input['perpage'], [
                'id' => 'perpage',
                'min' => 1,
            ]),
            'perpage'
        ...
        if ($type == 'user') {
            $form_container->output_row(
                ...
                $form->generate_radio_button(
                    ...
                    [
                        'checked' =>
                            $mybb->input['view_type'] != 'card' ? true : false,
                    ]
                ) .
                    '<br />' .
                    $form->generate_radio_button(
...
//]]></script>\n";
        ...
        if ($mybb->request_method == 'post') {
            ...
            $page->output_confirm_action(
                $base_url .
                    "&amp;action=views&amp;do=delete&amp;vid={$admin_view['vid']}",
                $lang->confirm_view_deletion
            );
        }
    }

    // Export views
    elseif ($mybb->input['do'] == 'export') {
        ...
        while ($admin_view = $db->fetch_array($query)) {
            ...
            $admin_view['title'] = str_replace(
                ']]>',
                ']]]]><![CDATA[>',
                $admin_view['title']
            );
            $admin_view['sortby'] = str_replace(
                ']]>',
                ']]]]><![CDATA[>',
                $admin_view['sortby']
            );
            $admin_view['sortorder'] = str_replace(
                ']]>',
                ']]]]><![CDATA[>',
                $admin_view['sortorder']
            );
            $admin_view['view_type'] = str_replace(
                ']]>',
                ']]]]><![CDATA[>',
                $admin_view['view_type']
            );
    ...
    // Generate a listing of all current views
        ...
        while ($view = $db->fetch_array($query)) {
            ...
            $table->construct_cell(
                "<div class=\"float_right\"><img src=\"styles/{$page->style}/images/icons/{$view_type}.png\" title=\"" .
                    $lang->sprintf($lang->this_is_a_view, $view_type) .
                    "\" alt=\"{$view_type}\" /></div><div class=\"{$default_class}\"><strong><a href=\"{$base_url}&amp;action=views&amp;do=edit&amp;vid={$view['vid']}\" >{$view['title']}</a></strong>{$default_add}{$created}</div>"
            );

            $popup = new PopupMenu("view_{$view['vid']}", $lang->options);
            $popup->add_item(
                $lang->edit_view,
                "{$base_url}&amp;action=views&amp;do=edit&amp;vid={$view['vid']}"
            );
            if ($view['vid'] != $default_view) {
                $popup->add_item(
                    $lang->set_as_default,
                    "{$base_url}&amp;action=views&amp;do=set_default&amp;vid={$view['vid']}"
                );
            }
            ...
            if ($views > 1 && $view['vid'] != 1) {
                $popup->add_item(
                    $lang->delete_view,
                    "{$base_url}&amp;action=views&amp;do=delete&amp;vid={$view['vid']}&amp;my_post_key={$mybb->post_code}",
                    "return AdminCP.deleteConfirmation(this, '{$lang->confirm_view_deletion}')"
                );
            }
...
function set_default_view($type, $vid)
{
    ...
    $updated_admin = ['defaultviews' => $db->escape_string($default_views)];
