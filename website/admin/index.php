...
if (
    !isset($config['admin_dir']) ||
    !file_exists(MYBB_ROOT . $config['admin_dir'] . '/inc/class_page.php')
) {
...
$mybb->settings['cookiepath'] =
    substr($loc, 0, strrpos($loc, "/{$config['admin_dir']}/")) .
    "/{$config['admin_dir']}/";
...
if (!isset($cp_style)) {
    if (
        !empty($mybb->settings['cpstyle']) &&
        file_exists(
            MYBB_ADMIN_DIR .
                '/styles/' .
                $mybb->settings['cpstyle'] .
                '/main.css'
        )
    ) {
...
if ($mybb->input['action'] == 'unlock') {
    ...
    if ($mybb->input['token'] && $user['uid']) {
        $query = $db->simple_select(
            ...
            "uid='" .
                (int) $user['uid'] .
                "' AND code='" .
                $db->escape_string($mybb->input['token']) .
                "' AND type='l'"
        );
        ...
        if ($db->fetch_field($query, 'num') > 0) {
            ...
            $db->update_query(
                'adminoptions',
                ['loginlockoutexpiry' => 0, 'loginattempts' => 0],
                "uid='" . (int) $user['uid'] . "'"
            );
    ...
    if (isset($mybb->cookies['adminsid'])) {
        ...
        $query = $db->simple_select(
            'adminsessions',
            'sid',
            "sid='" . $db->escape_string($mybb->cookies['adminsid']) . "'"
        );
    ...
    switch ($mybb->settings['username_method']) {
        case 0: // Username only
            $login_lang_string = $lang->sprintf(
                $login_lang_string,
                $lang->login_username
            );
            break;
        ...
        default:
            $login_lang_string = $lang->sprintf(
                $login_lang_string,
                $lang->login_username_and_password
            );
            break;
    }
    ...
    // Validate PIN first
    if (
        !empty($config['secret_pin']) &&
        (empty($mybb->input['pin']) ||
            $mybb->input['pin'] != $config['secret_pin'])
    ) {
        $login_user = get_user_by_username($mybb->input['username'], [
        ...
        if ($loginattempts !== false && $loginattempts['loginattempts'] > 0) {
            ...
            if ($loginattempts['loginlockoutexpiry'] == 0) {
                $db->update_query(
                    ...
                    [
                        'loginlockoutexpiry' =>
                            TIME_NOW +
                            (int) $mybb->settings['loginattemptstimeout'] * 60,
                    ],
                    "uid='" . (int) $login_user['uid'] . "'"
                );
            }
            ...
            if (
                ...
                $mybb->settings['maxloginattempts']
            ) {
                $db->delete_query(
                    'awaitingactivation',
                    "uid='" . (int) $login_user['uid'] . "' AND type='l'"
                );
                $lockout_array = [
                    'uid' => $login_user['uid'],
                    'dateline' => TIME_NOW,
                ...
                $subject = $lang->sprintf(
                    $lang->locked_out_subject,
                    $mybb->settings['bbname']
                );
                $message = $lang->sprintf(
                    $lang->locked_out_message,
                    htmlspecialchars_uni($mybb->input['username']),
                    $mybb->settings['bbname'],
                    $mybb->settings['maxloginattempts'],
                    $mybb->settings['bburl'],
    ...
    if (!empty($mybb->user['uid'])) {
        ...
        if (!empty($mybb->input['module'])) {
            ...
            $query_string =
                '?module=' . htmlspecialchars_uni($mybb->input['module']);
    ...
    // Otherwise, check admin session
        ...
        if (empty($admin_session) || !$admin_session['sid']) {
            ...
            if (
                !$mybb->user['uid'] ||
                $mybb->user['loginkey'] !== $admin_session['loginkey']
            ) {
                unset($mybb->user);
            } else {
                ...
                if ($admin_session['lastactive'] < TIME_NOW - 7200) {
                    ...
                    $db->delete_query(
                        'adminsessions',
                        "sid='" .
                            $db->escape_string($mybb->cookies['adminsid']) .
                            "'"
                    );
                    unset($mybb->user);
                }
                ...
                elseif (
                    ...
                    $exploded_admin_ip = explode(
                        '.',
                        my_inet_ntop($admin_session['ip'])
                    );
                    $matches = 0;
                    $valid_ip = false;
...
if (!isset($mybb->user['usergroup'])) {
    ...
    $mybbgroups =
        $mybb->user['usergroup'] . ',' . $mybb->user['additionalgroups'];
}
...
if (
    (empty($mybb->usergroup['cancp']) && !$is_super_admin) ||
    !$mybb->user['uid']
) {
    $uid = 0;
    if (isset($mybb->user['uid'])) {
...
if (!empty($mybb->user['uid'])) {
    ...
    if (
        empty($admin_options['authsecret']) ||
        ...
        if (
            !empty($admin_options['cpstyle']) &&
            file_exists(
                MYBB_ADMIN_DIR . "/styles/{$admin_options['cpstyle']}/main.css"
            )
        ) {
    ...
    if ($admin_session['sid']) {
        $db->update_query(
            ...
            "sid='" . $db->escape_string($admin_session['sid']) . "'"
        );
    }
...
if ($mybb->input['do'] == 'do_2fa' && $mybb->request_method == 'post') {
    ...
    if (!empty($codes) && in_array($mybb->get_input('code'), $codes)) {
        $recovery = true;
        $ncodes = array_diff($codes, [$mybb->input['code']]); // Removes our current code from the codes array
        $db->update_query(
            'adminoptions',
            ['recovery_codes' => $db->escape_string(my_serialize($ncodes))],
            "uid='{$mybb->user['uid']}'"
        );
    ...
    $test = $auth->verifyCode(
        $admin_options['authsecret'],
        $mybb->get_input('code')
    );
...
while (($module = readdir($dir)) !== false) {
    if (
        ...
        if (function_exists($module . '_admin_permissions')) {
            if (
                isset($mybb->admin['permissions'][$module]) ||
                $is_super_admin == true
            ) {
                $has_permission = true;
            }
