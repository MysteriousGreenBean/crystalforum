...
$page->add_breadcrumb_item(
    $lang->attachment_types,
    'index.php?module=config-attachment_types'
);
...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        if (
            !trim($mybb->input['mimetype']) &&
            !trim($mybb->input['extension'])
        ) {
        ...
        if (!$errors) {
            ...
            if (substr($mybb->input['extension'], 0, 1) == '.') {
                $mybb->input['extension'] = substr(
                    $mybb->input['extension'],
                    1
                );
            }
    ...
    $page->output_header(
        $lang->attachment_types . ' - ' . $lang->add_new_attachment_type
    );
    ...
    $form = new Form(
        'index.php?module=config-attachment_types&amp;action=add',
        'post',
        'add'
    );
    ...
    if ($errors) {
        switch ($mybb->input['groups']) {
            case 'all':
                ...
                break;
            case 'custom':
                $mybb->input['groups'] = implode(
                    ',',
                    (array) $mybb->input['select']['groups']
                );
                break;
            default:
                $mybb->input['groups'] = '';
        ...
        switch ($mybb->input['forums']) {
            case 'all':
                ...
                break;
            case 'custom':
                $mybb->input['forums'] = implode(
                    ',',
                    (array) $mybb->input['select']['forums']
                );
                break;
            default:
                $mybb->input['forums'] = '';
    ...
    if ($upload_max_filesize || $post_max_size) {
        ...
        if ($upload_max_filesize) {
            $limit_string .=
                '<br />' .
                $lang->sprintf(
                    $lang->limit_upload_max_filesize,
                    $upload_max_filesize
                );
        }
        if ($post_max_size) {
            $limit_string .=
                '<br />' .
                $lang->sprintf($lang->limit_post_max_size, $post_max_size);
        }
    }
		...
		<dd style=\"margin-top: 4px;\" id=\"groups_forums_groups_custom\" class=\"groups_forums_groups\">
			<table cellpadding=\"4\">
				<tr>
					...
					<td>" .
        $form->generate_group_select('select[groups][]', $selected_values, [
            'id' => 'groups',
            'multiple' => true,
            'size' => 5,
        ]) .
		...
		<dd style=\"margin-top: 4px;\" id=\"forums_forums_groups_custom\" class=\"forums_forums_groups\">
			<table cellpadding=\"4\">
				<tr>
					...
					<td>" .
        $form->generate_forum_select('select[forums][]', $selected_values, [
            'id' => 'forums',
            'multiple' => true,
            'size' => 5,
        ]) .
    ...
    $mybb->input['forcedownload'] = $mybb->get_input(
        'forcedownload',
        MyBB::INPUT_INT
    );
    ...
    $form_container->output_row(
        $lang->file_extension . ' <em>*</em>',
        $lang->file_extension_desc,
        $form->generate_text_box('extension', $mybb->get_input('extension'), [
            'id' => 'extension',
        ]),
        'extension'
    );
    $form_container->output_row(
        $lang->mime_type . ' <em>*</em>',
        $lang->mime_type_desc,
    ...
    $form_container->output_row(
        $lang->maximum_file_size,
        $lang->maximum_file_size_desc . $limit_string,
        $form->generate_numeric_field('maxsize', $mybb->get_input('maxsize'), [
            'id' => 'maxsize',
            'min' => 0,
        ]),
        'maxsize'
    ...
    $form_container->output_row(
        $lang->attachment_icon,
        $lang->attachment_icon_desc,
        $form->generate_text_box('icon', $mybb->get_input('icon'), [
            'id' => 'icon',
        ]),
        'icon'
    );
    $form_container->output_row(
        $lang->enabled,
        '',
        $form->generate_yes_no_radio('enabled', $mybb->get_input('enabled')),
        'enabled'
    );
    $form_container->output_row(
        $lang->forcedownload,
        $lang->forcedownload_desc,
        $form->generate_yes_no_radio(
            'forcedownload',
            $mybb->get_input('forcedownload')
        ),
        'forcedownload'
    );
    $form_container->output_row(
        $lang->available_to_groups,
        '',
        $groups_select_code,
        '',
        [],
    ...
    $form_container->output_row(
        $lang->avatar_file,
        $lang->avatar_file_desc,
        $form->generate_yes_no_radio(
            'avatarfile',
            $mybb->get_input('avatarfile')
        ),
        'avatarfile'
    );
...
if ($mybb->input['action'] == 'edit') {
    ...
    $page->output_header(
        $lang->attachment_types . ' - ' . $lang->edit_attachment_type
    );
    ...
    $form = new Form(
        "index.php?module=config-attachment_types&amp;action=edit&amp;atid={$attachment_type['atid']}",
        'post',
        'add'
    );
    ...
    $form_container->output_row(
        $lang->file_extension . ' <em>*</em>',
        $lang->file_extension_desc,
        $form->generate_text_box('extension', $mybb->input['extension'], [
            'id' => 'extension',
        ]),
        'extension'
    );
    $form_container->output_row(
        $lang->mime_type . ' <em>*</em>',
        $lang->mime_type_desc,
        $form->generate_text_box('mimetype', $mybb->input['mimetype'], [
            'id' => 'mimetype',
        ]),
        'mimetype'
    );
    $form_container->output_row(
        $lang->maximum_file_size,
        $lang->maximum_file_size_desc . $limit_string,
        $form->generate_numeric_field('maxsize', $mybb->input['maxsize'], [
            'id' => 'maxsize',
            'min' => 0,
        ]),
        'maxsize'
    ...
    $form_container->output_row(
        $lang->attachment_icon,
        $lang->attachment_icon_desc,
        $form->generate_text_box('icon', $mybb->input['icon'], [
            'id' => 'icon',
        ]),
        'icon'
    );
    $form_container->output_row(
        $lang->enabled,
        '',
        $form->generate_yes_no_radio('enabled', $mybb->input['enabled']),
        'enabled'
    );
    $form_container->output_row(
        $lang->forcedownload,
    ...
    $form_container->output_row(
        $lang->avatar_file,
        $lang->avatar_file_desc,
        $form->generate_yes_no_radio('avatarfile', $mybb->input['avatarfile']),
        'avatarfile'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-attachment_types&amp;action=delete&amp;atid={$attachment_type['atid']}",
            $lang->confirm_attachment_type_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    $table = new Table();
    $table->construct_header($lang->extension, ['colspan' => 2]);
    $table->construct_header($lang->mime_type);
    $table->construct_header($lang->alt_enabled, ['class' => 'align_center']);
    $table->construct_header($lang->maximum_size, ['class' => 'align_center']);
    ...
    while ($attachment_type = $db->fetch_array($query)) {
        ...
        $attachment_type['icon'] = htmlspecialchars_uni(
            str_replace('{theme}', 'images', $attachment_type['icon'])
        );
        ...
        if (
            ...
            $attachment_type['icon'] == 'images/attachtypes/'
        ) {
            $attachment_type['icon'] = '&nbsp;';
        } else {
            $attachment_type['name'] = htmlspecialchars_uni(
                $attachment_type['name']
            );
            $attachment_type[
                'icon'
            ] = "<img src=\"{$image}\" title=\"{$attachment_type['name']}\" alt=\"\" />";
        }
        ...
        $attachment_type['extension'] = htmlspecialchars_uni(
            $attachment_type['extension']
        );
        ...
        $table->construct_cell(
            htmlspecialchars_uni($attachment_type['mimetype'])
        );
        $table->construct_cell(
        ...
        $table->construct_cell(
            get_friendly_size($attachment_type['maxsize'] * 1024),
            ['class' => 'align_center']
        );

        $popup = new PopupMenu(
            "attachment_type_{$attachment_type['atid']}",
            $lang->options
        );
        $popup->add_item(
            $lang->edit,
        ...
        $popup->add_item(
            $phrase,
            "index.php?module=config-attachment_types&amp;action=toggle_status&amp;atid={$attachment_type['atid']}&amp;my_post_key={$mybb->post_code}"
        );
        $popup->add_item(
            $lang->delete,
            "index.php?module=config-attachment_types&amp;action=delete&amp;atid={$attachment_type['atid']}&amp;my_post_key={$mybb->post_code}",
            "return AdminCP.deleteConfirmation(this, '{$lang->confirm_attachment_type_deletion}')"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_attachment_types, ['colspan' => 6]);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            'index.php?module=config-attachment_types&amp;page={page}'
        );
