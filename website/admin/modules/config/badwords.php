...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            $query = $db->simple_select(
                ...
                'bid',
                "badword = '" .
                    $db->escape_string($mybb->input['badword']) .
                    "'"
            );
    ...
    $form = new Form(
        'index.php?module=config-badwords&amp;action=add',
        'post',
        'add'
    );
    ...
    $form_container->output_row(
        $lang->bad_word . ' <em>*</em>',
        $lang->bad_word_desc,
        $form->generate_text_box('badword', $mybb->get_input('badword'), [
            'id' => 'badword',
        ]),
        'badword'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_yes_no_radio(
            'regex',
            $mybb->get_input('regex', MyBB::INPUT_INT),
            ['id' => 'regex']
        ),
        'regex'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-badwords&action=delete&bid={$badword['bid']}",
            $lang->confirm_bad_word_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        "index.php?module=config-badwords&amp;action=edit&amp;bid={$badword['bid']}",
        'post'
    );
    ...
    $form_container->output_row(
        $lang->bad_word . ' <em>*</em>',
        $lang->bad_word_desc,
        $form->generate_text_box('badword', $badword_data['badword'], [
            'id' => 'badword',
        ]),
        'badword'
    );
    $form_container->output_row(
        $lang->replacement,
        $lang->replacement_desc,
        $form->generate_text_box('replacement', $badword_data['replacement'], [
            'id' => 'replacement',
        ]),
        'replacement'
    );
    $form_container->output_row(
        $lang->regex,
        $lang->regex_desc,
        $form->generate_yes_no_radio('regex', (int) $badword_data['regex'], [
            'id' => 'regex',
        ]),
        'regex'
    );
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->replacement, ['width' => '50%']);
    $table->construct_header($lang->regex, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    ...
    while ($badword = $db->fetch_array($query)) {
        ...
        $table->construct_cell($regex, ['class' => 'align_center']);
        $table->construct_cell(
            "<a href=\"index.php?module=config-badwords&amp;action=edit&amp;bid={$badword['bid']}\">{$lang->edit}</a>",
            ['class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-badwords&amp;action=delete&amp;bid={$badword['bid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_bad_word_deletion}');\">{$lang->delete}</a>",
            ['class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_bad_words, ['colspan' => 4]);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            'index.php?module=config-badwords&amp;page={page}'
        );
