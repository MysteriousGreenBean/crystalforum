...
if ($mybb->input['action'] == 'add' && $mybb->request_method == 'post') {
    ...
    $query = $db->simple_select(
        ...
        'fid',
        "filter = '" .
            $db->escape_string($mybb->input['filter']) .
            "' AND type = '" .
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-banning&amp;action=delete&amp;fid={$filter['fid']}",
            $lang->confirm_ban_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    $form = new Form(
        'index.php?module=config-banning&amp;action=add',
        'post',
        'add'
    );
    ...
    if ($mybb->input['type'] == 'usernames') {
    ...
    } elseif ($mybb->input['type'] == 'emails') {
        $form_container = new FormContainer(
            $lang->add_disallowed_email_address
        );
        $form_container->output_row(
            $lang->email_address . ' <em>*</em>',
            $lang->email_address_desc,
            $form->generate_text_box('filter', $mybb->input['filter'], [
                'id' => 'filter',
            ]),
            'filter'
        ...
        $buttons[] = $form->generate_submit_button(
            $lang->disallow_email_address
        );
    } else {
        $form_container = new FormContainer($lang->ban_an_ip_address);
        $form_container->output_row(
            $lang->ip_address . ' <em>*</em>',
            $lang->ip_address_desc,
            $form->generate_text_box('filter', $mybb->input['filter'], [
                'id' => 'filter',
            ]),
            'filter'
    ...
    if ($mybb->input['type'] == 'usernames') {
        ...
        $table->construct_header($lang->date_disallowed, [
            'class' => 'align_center',
            'width' => 200,
        ]);
        $table->construct_header($lang->last_attempted_use, [
            'class' => 'align_center',
            'width' => 200,
        ]);
    } elseif ($mybb->input['type'] == 'emails') {
        ...
        $table->construct_header($lang->ban_date, [
            'class' => 'align_center',
            'width' => 200,
        ]);
        $table->construct_header($lang->last_access, [
            'class' => 'align_center',
            'width' => 200,
        ]);
    }
    ...
    $query = $db->simple_select('banfilters', '*', "type='{$type}'", [
        'limit_start' => $start,
        'limit' => 20,
        'order_by' => 'filter',
        'order_dir' => 'asc',
    ...
    while ($filter = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=config-banning&amp;action=delete&amp;fid={$filter['fid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_ban_deletion}');\"><img src=\"styles/{$page->style}/images/icons/delete.png\" title=\"{$lang->delete}\" alt=\"{$lang->delete}\" /></a>",
            ['class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_bans, ['colspan' => 4]);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            "index.php?module=config-banning&amp;type={$mybb->get_input(
                'type'
            )}&amp;page={page}"
        );
