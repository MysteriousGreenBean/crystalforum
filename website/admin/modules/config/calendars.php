...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            $calendar = [
                ...
                'startofweek' => $mybb->get_input(
                    'startofweek',
                    MyBB::INPUT_INT
                ),
                'eventlimit' => $mybb->get_input('eventlimit', MyBB::INPUT_INT),
                'showbirthdays' => $mybb->get_input(
                    'showbirthdays',
                    MyBB::INPUT_INT
                ),
    ...
    $form = new Form(
        'index.php?module=config-calendars&amp;action=add',
        'post'
    );
    ...
    $form_container->output_row(
        $lang->event_limit,
        $lang->event_limit_desc,
        $form->generate_numeric_field(
            'eventlimit',
            $mybb->get_input('eventlimit'),
            ['id' => 'eventlimit', 'min' => 0]
        ),
        'eventlimit'
    );
    $form_container->output_row(
        $lang->show_birthdays,
        $lang->show_birthdays_desc,
        $form->generate_yes_no_radio(
            'showbirthdays',
            $mybb->get_input('showbirthdays'),
            true
        )
    );
    $form_container->output_row(
        $lang->moderate_events,
        $lang->moderate_events_desc,
        $form->generate_yes_no_radio(
            'moderation',
            $mybb->get_input('moderation'),
            true
        )
    );
    $form_container->output_row(
        $lang->allow_html,
        '',
        $form->generate_yes_no_radio('allowhtml', $mybb->get_input('allowhtml'))
    );
    $form_container->output_row(
        $lang->allow_mycode,
        '',
...
if ($mybb->input['action'] == 'permissions') {
    ...
    if ($mybb->request_method == 'post') {
        foreach (array_keys($usergroups) as $group_id) {
            ...
            $db->delete_query(
                'calendarpermissions',
                "cid='{$calendar['cid']}' AND gid='" . (int) $group_id . "'"
            );
    ...
    $page->add_breadcrumb_item(
        $calendar['name'],
        "index.php?module=config-calendars&amp;action=edit&amp;cid={$calendar['cid']}"
    );
    $page->add_breadcrumb_item($lang->permissions);
    $page->output_header($lang->calendars . ' - ' . $lang->edit_permissions);

    $form = new Form(
        'index.php?module=config-calendars&amp;action=permissions',
        'post'
    );
    ...
    $table->construct_header($lang->permissions_view, [
        'class' => 'align_center',
        'width' => '10%',
    ]);
    $table->construct_header($lang->permissions_post_events, [
        'class' => 'align_center',
        'width' => '10%',
    ]);
    $table->construct_header($lang->permissions_bypass_moderation, [
        'class' => 'align_center',
        'width' => '10%',
    ]);
    $table->construct_header($lang->permissions_moderator, [
        'class' => 'align_center',
        'width' => '10%',
    ]);
    $table->construct_header($lang->permissions_all, [
        'class' => 'align_center',
        'width' => '10%',
    ]);
    ...
    foreach ($usergroups as $usergroup) {
        ...
        $table->construct_cell(
            "<strong>{$usergroup['title']}</strong><br /><small style=\"vertical-align: middle;\">" .
                ...
                " <label for=\"default_permissions_{$usergroup['gid']}\">{$lang->permissions_use_group_default}</label></small>"
        );
        $table->construct_cell(
            $form->generate_check_box(
                "permissions[{$usergroup['gid']}][canviewcalendar]",
                1,
                '',
                [
                    'id' => "permissions_{$usergroup['gid']}_canviewcalendar",
                    'checked' => $perms_checked['canviewcalendar'],
        ...
        $table->construct_cell(
            $form->generate_check_box(
                "permissions[{$usergroup['gid']}][canaddevents]",
                1,
                '',
                [
                    'id' => "permissions_{$usergroup['gid']}_canaddevents",
                    'checked' => $perms_checked['canaddevents'],
                    'onclick' => $reset_default,
                ]
            ),
            ['class' => 'align_center']
        ...
        $table->construct_cell(
            $form->generate_check_box(
                "permissions[{$usergroup['gid']}][canbypasseventmod]",
                1,
                '',
                [
                    'id' => "permissions_{$usergroup['gid']}_canbypasseventmod",
                    'checked' => $perms_checked['canbypasseventmod'],
                    'onclick' => $reset_default,
                ]
            ),
            ['class' => 'align_center']
        ...
        $table->construct_cell(
            $form->generate_check_box(
                "permissions[{$usergroup['gid']}][canmoderateevents]",
                1,
                '',
                [
                    'id' => "permissions_{$usergroup['gid']}_canmoderateevents",
                    'checked' => $perms_checked['canmoderateevents'],
                    'onclick' => $reset_default,
                ]
            ),
            ['class' => 'align_center']
        ...
        $table->construct_cell(
            $form->generate_check_box(
                "permissions[{$usergroup['gid']}][all]",
                1,
                '',
                [
                    'id' => "permissions_{$usergroup['gid']}_all",
                    'checked' => $all_checked,
                    'onclick' => $all_check,
                ]
            ),
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        'index.php?module=config-calendars&amp;action=edit',
        'post'
    );
    ...
    $form_container->output_row(
        $lang->allow_html,
        '',
        $form->generate_yes_no_radio('allowhtml', $mybb->input['allowhtml'])
    );
    $form_container->output_row(
        $lang->allow_mycode,
        '',
        $form->generate_yes_no_radio('allowmycode', $mybb->input['allowmycode'])
    );
    $form_container->output_row(
        $lang->allow_img,
        '',
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-calendars&amp;action=delete&amp;cid={$calendar['cid']}",
            $lang->confirm_calendar_deletion
        );
    }
}

if (
    $mybb->input['action'] == 'update_order' &&
...
if (!$mybb->input['action']) {
    ...
    $form = new Form(
        'index.php?module=config-calendars&amp;action=update_order',
        'post'
    );
    $table = new Table();
    ...
    while ($calendar = $db->fetch_array($query)) {
        $calendar['name'] = htmlspecialchars_uni($calendar['name']);
        $table->construct_cell(
            "<a href=\"index.php?module=config-calendars&amp;action=edit&amp;cid={$calendar['cid']}\"><strong>{$calendar['name']}</strong></a>"
        );
        $table->construct_cell(
            $form->generate_numeric_field(
                "disporder[{$calendar['cid']}]",
                $calendar['disporder'],
                [
                    'id' => 'disporder',
                    'style' => 'width: 80%',
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=config-calendars&amp;action=edit&amp;cid={$calendar['cid']}\">{$lang->edit}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-calendars&amp;action=permissions&amp;cid={$calendar['cid']}\">{$lang->permissions}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-calendars&amp;action=delete&amp;cid={$calendar['cid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_calendar_deletion}')\">{$lang->delete}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    $no_results = false;
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_calendars, ['colspan' => 5]);
        $table->construct_row();
        $no_results = true;
    }
