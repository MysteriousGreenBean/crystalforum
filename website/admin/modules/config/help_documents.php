...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->input['type'] == 'section') {
        ...
        if ($mybb->request_method == 'post') {
            ...
            if (!is_array($errors)) {
                $sql_array = [
                    ...
                    'usetranslation' => $mybb->get_input(
                        'usetranslation',
                        MyBB::INPUT_INT
                    ),
                    'enabled' => $mybb->get_input('enabled', MyBB::INPUT_INT),
                    'disporder' => $mybb->get_input(
                        'disporder',
                        MyBB::INPUT_INT
                    ),
                ];
        ...
        $page->output_header(
            $lang->help_documents . ' - ' . $lang->add_new_section
        );
        ...
        $sub_tabs['add_help_document'] = [
            ...
            'link' =>
                'index.php?module=config-help_documents&amp;action=add&amp;type=document',
        ];

        $sub_tabs['add_help_section'] = [
            'title' => $lang->add_new_section,
            'link' =>
                'index.php?module=config-help_documents&amp;action=add&amp;type=section',
    ...
    // Add page
        ...
        $page->output_header(
            $lang->help_documents . ' - ' . $lang->add_new_document
        );
...
if ($mybb->input['action'] == 'edit') {
    ...
    if (isset($mybb->input['sid']) && !$mybb->get_input('hid')) {
        ...
        $page->output_header(
            $lang->help_documents . ' - ' . $lang->edit_section
        );

        $sub_tabs['edit_help_section'] = [
            'title' => $lang->edit_section,
            'link' =>
                'index.php?module=config-help_documents&amp;action=edit&amp;sid=' .
                $section['sid'],
        ...
        $form = new Form(
            'index.php?module=config-help_documents&amp;action=edit',
            'post',
            'edit'
        );
        ...
        $form_container = new FormContainer(
            $lang->edit_section . " ({$lang->id} " . $section['sid'] . ')'
        );
        $form_container->output_row(
            $lang->title . ' <em>*</em>',
            '',
    ...
    // Edit document
        ...
        $page->output_header(
            $lang->help_documents . ' - ' . $lang->edit_document
        );

        $sub_tabs['edit_help_document'] = [
            'title' => $lang->edit_document,
            'link' =>
                'index.php?module=config-help_documents&amp;action=edit&amp;hid=' .
                $doc['hid'],
        ...
        $form_container = new FormContainer(
            $lang->edit_document . " ({$lang->id} " . $doc['hid'] . ')'
        );
        ...
        $form_container->output_row(
            $lang->section . ' <em>*</em>',
            '',
            $form->generate_select_box('sid', $sections, $mybb->input['sid']),
            'sid'
        );
        $form_container->output_row(
            $lang->title . ' <em>*</em>',
        ...
        $form_container->output_row(
            $lang->document . ' <em>*</em>',
            '',
            $form->generate_text_area('document', $mybb->input['document'], [
                'id' => 'document',
            ]),
            'document'
        );
    ...
    // Show form for deletion
        ...
        if (isset($mybb->input['sid'])) {
            ...
            $page->output_confirm_action(
                "index.php?module=config-help_documents&amp;action=delete&amp;sid={$sid}",
                $lang->confirm_section_deletion
            );
        }
        // Document
        else {
            $hid = $mybb->get_input('hid', MyBB::INPUT_INT);
            $page->output_confirm_action(
                "index.php?module=config-help_documents&amp;action=delete&amp;hid={$hid}",
                $lang->confirm_document_deletion
            );
        }
    }
}
...
if (!$mybb->input['action']) {
    ...
    $query = $db->simple_select('helpsections', '*', '', [
        'order_by' => 'disporder',
    ]);
    while ($section = $db->fetch_array($query)) {
        $table->construct_cell(
            "<div><strong><a href=\"index.php?module=config-help_documents&amp;action=edit&amp;sid={$section['sid']}\">{$section['name']}</a></strong><br /><small>{$section['description']}</small></div>"
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-help_documents&amp;action=edit&amp;sid={$section['sid']}\">{$lang->edit}</a>",
            ['class' => 'align_center', 'width' => '60']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-help_documents&amp;action=delete&amp;sid={$section['sid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_section_deletion}')\">{$lang->delete}</a>",
            ['class' => 'align_center', 'width' => '90']
        );
        $table->construct_row();
        ...
        while ($doc = $db->fetch_array($query2)) {
            $table->construct_cell(
                "<div style=\"padding-left: 40px;\"><div><strong><a href=\"index.php?module=config-help_documents&amp;action=edit&amp;hid={$doc['hid']}\">{$doc['name']}</a></strong><br /><small>{$doc['description']}</small></div></div>"
            );
            $table->construct_cell(
                "<a href=\"index.php?module=config-help_documents&amp;action=edit&amp;hid={$doc['hid']}\">{$lang->edit}</a>",
                ['class' => 'align_center', 'width' => '60']
            );
            $table->construct_cell(
                "<a href=\"index.php?module=config-help_documents&amp;action=delete&amp;hid={$doc['hid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_document_deletion}')\">{$lang->delete}</a>",
                ['class' => 'align_center', 'width' => '90']
            );
            $table->construct_row();
        }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_help_documents, ['colspan' => 3]);
        $table->construct_row();
    }
