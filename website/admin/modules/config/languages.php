...
$page->add_breadcrumb_item(
    $lang->languages,
    'index.php?module=config-languages'
);
...
\$langinfo['charset'] = \"{$newlanginfo['charset']}\";\n";
        ...
        if ($file = fopen($file, 'w')) {
            ...
            admin_redirect(
                'index.php?module=config-languages&action=edit&lang=' .
                    htmlspecialchars_uni($editlang) .
                    '&editwith=' .
                    htmlspecialchars_uni($editlang)
            );
        } else {
    ...
    $page->add_breadcrumb_item(
        preg_replace(
            '<\?|\?>',
            '<span>?</span>',
            htmlspecialchars_uni($languages[$editlang])
        ),
        'index.php?module=config-languages&amp;action=edit&amp;lang=' .
            htmlspecialchars_uni($editlang)
    );
    ...
    $form = new Form(
        'index.php?module=config-languages&amp;action=edit_properties',
        'post',
        'editset'
    );
    ...
    $form_container->output_row(
        $lang->friendly_name . ' <em>*</em>',
        '',
        $form->generate_text_box('info[name]', $mybb->input['info']['name'], [
            'id' => 'name',
        ]),
        'name'
    );
    $form_container->output_row(
        $lang->language_in_html . ' <em>*</em>',
        '',
        $form->generate_text_box(
            'info[htmllang]',
            $mybb->input['info']['htmllang'],
            ['id' => 'htmllang']
        ),
        'htmllang'
    );
    ...
    $form_container->output_row(
        $lang->rtl . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio('info[rtl]', $mybb->input['info']['rtl'], [
            'id' => 'rtl',
        ]),
        'rtl'
    );
    ...
    $buttons[] = $form->generate_submit_button($lang->save_language_file, [
        'disabled' => $no_write,
    ]);
...
if ($mybb->input['action'] == 'quick_phrases') {
    ...
    $page->add_breadcrumb_item(
        ...
        'index.php?module=config-languages&amp;action=quick_edit&amp;lang=' .
            htmlspecialchars_uni($editlang)
    );
    ...
    $quick_phrases = [
        ...
        'messages.lang.php' => [
            ...
            'error_nopermission_guest_2' =>
                $lang->quickphrases_error_nopermission_guest_2,
            'error_nopermission_guest_3' =>
                $lang->quickphrases_error_nopermission_guest_3,
            'error_nopermission_guest_4' =>
                $lang->quickphrases_error_nopermission_guest_4,
        ],
    ];
            ...
            foreach ($quick_phrases as $file => $phrases) {
                ...
                if ($fp = @fopen($folder . $file, 'w')) {
                    ...
                    $lang_file_credits .=
                        ...
                        "\n";
                    $lang_file_credits .=
                        '// Language pack translators website : ' .
                        preg_replace(
                            "#<\?|\?>|\r|\n#i",
                            ' ',
                            $langinfo['website']
    ...
    $sub_tabs['quick_phrases'] = [
        ...
        'link' =>
            'index.php?module=config-languages&amp;action=quick_phrases&amp;lang=' .
            $editlang,
    ...
    foreach ($quick_phrases as $file => $phrases) {
        if (
            (file_exists($folder . $file) && !is_writable($folder . $file)) ||
            !is_writable($folder)
        ) {
            $no_write = 1;
        }
    ...
    foreach ($quick_phrases as $file => $phrases) {
        ...
        foreach ($phrases as $phrase => $description) {
            ...
            $form_container->output_row(
                $description,
                $phrase,
                $form->generate_text_area("edit[$phrase]", $value, [
                    'id' => 'lang_' . $phrase,
                    'rows' => 2,
                    'class' => "langeditor_textarea_edit {$quickphrases_dir_class}",
                ]),
                'lang_' . $phrase,
                ['width' => '50%']
            );
...
if ($mybb->input['action'] == 'edit') {
    ...
    if (
        $editwithfolder &&
        (!file_exists($editwithfolder) || !file_exists($editwithfolder))
    ) {
    ...
    if (isset($mybb->input['file'])) {
        ...
        if ($mybb->request_method == 'post') {
            ...
            foreach ($valid_keys as $key => $value) {
                $contents_wfile .=
                    ...
                    $key .
                    "'] = " .
                    var_export($mybb->input['edit'][$key], true) .
                    ";\n";
            }
            ...
            if ($fp = @fopen($editfile, 'w')) {
                ...
                log_admin_action(
                    $editlang,
                    $editfile,
                    $mybb->get_input('inadmin', MyBB::INPUT_INT)
                );
        ...
        $form = new Form(
            'index.php?module=config-languages&amp;action=edit',
            'post',
            'edit'
        );
        ...
        $no_write = null;
        if (
            (file_exists($editfile) && !is_writable($editfile)) ||
            !is_writable($folder)
        ) {
            $no_write = 1;
            $page->output_alert($lang->alert_note_cannot_write);
        ...
        if ($editwithfile && $withvars) {
            ...
            $form_container->output_row_header(
                preg_replace(
                    '<\?|\?>',
                    '<span>?</span>',
                    htmlspecialchars_uni($languages[$editwith])
                )
            );
            $form_container->output_row_header(
                preg_replace(
            ...
            foreach ($withvars as $key => $value) {
                if (my_strtolower($langinfo['charset']) == 'utf-8') {
                    $withvars[$key] = preg_replace_callback(
                        '#%u([0-9A-F]{1,4})#i',
                        'encode_language_string_utf8',
                        $withvars[$key]
                    );
                    $editvars[$key] = preg_replace_callback(
                        '#%u([0-9A-F]{1,4})#i',
                        'encode_language_string_utf8',
                        $editvars[$key]
                    );
                ...
                $withvars_value_cbvCount = preg_match_all(
                    "/{[ \t]*\d+[ \t]*}/",
                    $withvars[$key],
                    $matches
                );
                $editvars_value_cbvCount = preg_match_all(
                    "/{[ \t]*\d+[ \t]*}/",
                    $editvars[$key],
                    $matches
                );
                ...
                if (
                    ($withvars[$key] && !$editvars[$key]) ||
                    $withvars_value_cbvCount != $editvars_value_cbvCount
                ) {
                ...
                $form_container->output_row(
                    $key,
                    '',
                    $form->generate_text_area('', $withvars[$key], [
                        'readonly' => true,
                        'rows' => 2,
                        'class' => "langeditor_textarea_editwith {$editwith_dir_class}",
                    ]),
                    '',
                    ['width' => '50%', 'skip_construct' => true]
                );
                $form_container->output_row(
                    $key,
                    '',
                    $form->generate_text_area("edit[$key]", $editvars[$key], [
                        'id' => 'lang_' . $key,
                        'rows' => 2,
                        'class' => "langeditor_textarea_edit {$textarea_issue_class} {$editlang_dir_class}",
                    ]),
                    'lang_' . $key,
                    ['width' => '50%']
                );
            ...
            $present_in_edit_vars_only = (array) array_diff_key(
                $editvars,
                $withvars
            );
                    ...
                    $form_container->output_row(
                        ...
                        $form->generate_text_area(
                            ...
                            [
                                'id' => 'lang_' . $key,
                                'rows' => 2,
                                'class' => "langeditor_textarea_edit {$editlang_dir_class}",
                            ]
                        ),
                        'lang_' . $key,
                        ['width' => '50%']
            ...
            foreach ($editvars as $key => $value) {
                ...
                $form_container->output_row(
                    $key,
                    '',
                    $form->generate_text_area("edit[$key]", $value, [
                        'id' => 'lang_' . $key,
                        'rows' => 2,
                        'class' => "langeditor_textarea_edit {$editlang_dir_class}",
                    ]),
        ...
        if ($editwithfolder) {
            ...
            $table->construct_header($lang->phrases, [
                'class' => 'align_center',
                'width' => 100,
            ]);
            $table->construct_header(
            ...
            $table->construct_header($lang->issues, [
                'class' => 'align_center',
                'width' => 100,
            ]);
        ...
        if ($editwithfolder) {
            ...
            foreach ($files_left as $key => $file) {
                ...
                $icon_issues =
                    "<span class='langeditor_ok' title='" .
                    $lang->issues_ok .
                    "'></span>";
                if (count($editvars_left) > 0) {
                    $icon_issues =
                        "<span class='langeditor_warning' title='" .
                        $lang->issues_warning .
                        "'></span>";
                }

                $table->construct_cell(htmlspecialchars_uni($file), [
                    'class' => 'langeditor_editwithfile',
                ]);
                $table->construct_cell(count($editvars_left), [
                    'class' => 'langeditor_phrases',
                ]);
                $table->construct_cell('', ['class' => 'langeditor_editfile']);
                $table->construct_cell($icon_issues, [
                    'class' => 'langeditor_issues',
                ]);
                $table->construct_cell(
                    "<a href=\"index.php?module=config-languages&amp;action=edit&amp;lang=" .
                        htmlspecialchars_uni($editlang) .
                        '&amp;editwith=' .
                        htmlspecialchars_uni($editwith) .
                        '&amp;file=' .
                        htmlspecialchars_uni($file) .
                        "\">{$lang->edit}</a>",
                    ['class' => 'langeditor_edit']
                );
                $table->construct_row();
            }
            foreach ($files_right as $key => $file) {
                ...
                if (count($editvars_right) > 0) {
                    $icon_issues =
                        "<span class='langeditor_nothingtocompare' title='" .
                        $lang->issues_nothingtocompare .
                        "'></span>";
                }
            ...
            foreach ($files_both as $key => $file) {
                ...
                foreach (
                    $editvars_left
                    as $editvars_left_key => $editvars_left_value
                ) {
                    // Count {x} in left and right variable
                    $editvars_left_value_cbvCount = preg_match_all(
                        "/{[ \t]*\d+[ \t]*}/",
                        $editvars_left_value,
                        $matches
                    );
                    $editvars_right_value_cbvCount = preg_match_all(
                        "/{[ \t]*\d+[ \t]*}/",
                        $editvars_right[$editvars_left_key],
                        $matches
                    );
                    ...
                    if (
                        ($editvars_left_value &&
                            !$editvars_right[$editvars_left_key]) ||
                        $editvars_left_value_cbvCount !=
                            $editvars_right_value_cbvCount
                    ) {
                        $icon_issues =
                            "<span class='langeditor_warning' title='" .
                            $lang->issues_warning .
            ...
            foreach ($filenames as $key => $file) {
                ...
                $table->construct_cell(count($editvars_count), [
                    'class' => 'langeditor_phrases',
                ]);
                $table->construct_cell(
                    "<a href=\"index.php?module=config-languages&amp;action=edit&amp;lang=" .
        ...
        if ($table->num_rows() == 0) {
            $table->construct_cell($lang->no_language_files_front_end, [
                'colspan' => $edit_colspan,
            ]);
            $table->construct_row();
        }
        ...
        if ($langinfo['admin'] != 0) {
            ...
            if ($table->num_rows() == 0) {
                $table->construct_cell($lang->no_language_files_admin_cp, [
                    'colspan' => $edit_colspan,
                ]);
                $table->construct_row();
            }
...
if (!$mybb->input['action']) {
    ...
    $sub_tabs['find_language'] = [
        ...
        'link' =>
            'https://community.mybb.com/mods.php?action=browse&category=19',
        'link_target' => '_blank',
        'link_rel' => 'noopener',
    ];
    ...
    $table->construct_header($lang->version, [
        'class' => 'align_center',
        'width' => 100,
    ]);
    $table->construct_header($lang->controls, [
    ...
    foreach ($languages as $key1 => $langname1) {
        $langselectlangs[$key1] = $lang->sprintf(
            $lang->edit_with,
            preg_replace(
                '<\?|\?>',
                '<span>?</span>',
                htmlspecialchars_uni($langname1)
            )
        );
    }
    ...
    foreach ($languages as $key => $langname) {
        ...
        if (!empty($langinfo['website'])) {
            $author =
                "<a href=\"" .
                htmlspecialchars_uni($langinfo['website']) .
                "\" target=\"_blank\" rel=\"noopener\">" .
                htmlspecialchars_uni($langinfo['author']) .
                '</a>';
        } else {
        ...
        $table->construct_cell(
            "<span class='langeditor_info_name'>" .
                preg_replace(
                    '<\?|\?>',
                    '<span>?</span>',
                    htmlspecialchars_uni($langinfo['name'])
                ) .
                "</span><br /><span class='langeditor_info_author'>" .
                $author .
                '</span>'
        );
        $table->construct_cell(htmlspecialchars_uni($langinfo['version']), [
            'class' => 'align_center',
        ]);

        $popup = new PopupMenu(
            'language_' . htmlspecialchars_uni($key),
            $lang->options
        );
        $popup->add_item(
            $lang->edit_language_variables,
            'index.php?module=config-languages&amp;action=edit&amp;lang=' .
            ...
            if ($key != $key1) {
                $popup->add_item(
                    ...
                    'index.php?module=config-languages&amp;action=edit&amp;lang=' .
                        htmlspecialchars_uni($key) .
                        '&amp;editwith=' .
                        htmlspecialchars_uni($key1)
                );
            }
        }
        $popup->add_item(
            $lang->edit_properties,
            'index.php?module=config-languages&amp;action=edit_properties&amp;lang=' .
                htmlspecialchars_uni($key)
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_language, ['colspan' => 3]);
        $table->construct_row();
    }
