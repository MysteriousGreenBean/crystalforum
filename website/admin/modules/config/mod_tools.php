...
$page->add_breadcrumb_item(
    $lang->mod_tools,
    'index.php?module=config-mod_tools'
);
...
if ($mybb->input['action'] == 'delete_post_tool') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-mod_tools&amp;action=post_tools&amp;tid={$type['tid']}",
            $lang->confirm_post_tool_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'delete_thread_tool') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-mod_tools&amp;action=delete_thread_tool&amp;tid={$tool['tid']}",
            $lang->confirm_thread_tool_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'post_tools') {
    ...
    $sub_tabs['add_thread_tool'] = [
        ...
        'link' =>
            'index.php?module=config-mod_tools&amp;action=add_thread_tool',
    ];
    $sub_tabs['post_tools'] = [
    ...
    $query = $db->simple_select(
        'modtools',
        'tid, name, description, type',
        "type='p'",
        ['order_by' => 'name']
    );
    while ($tool = $db->fetch_array($query)) {
        $table->construct_cell(
            "<a href=\"index.php?module=config-mod_tools&amp;action=edit_post_tool&amp;tid={$tool['tid']}\"><strong>" .
                htmlspecialchars_uni($tool['name']) .
                '</strong></a><br /><small>' .
                htmlspecialchars_uni($tool['description']) .
                '</small>'
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-mod_tools&amp;action=edit_post_tool&amp;tid={$tool['tid']}\">{$lang->edit}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-mod_tools&amp;action=delete_post_tool&amp;tid={$tool['tid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_post_tool_deletion}')\">{$lang->delete}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_post_tools, ['colspan' => 3]);
        $table->construct_row();
    }
...
if ($mybb->input['action'] == 'edit_thread_tool') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            ...
            $mybb->input['approvethread'] != 'approve' &&
            $mybb->input['approvethread'] != 'unapprove' &&
            $mybb->input['approvethread'] != 'toggle'
        ) {
            $mybb->input['approvethread'] = '';
        }
        ...
        if (
            ...
            $mybb->input['softdeletethread'] != 'softdelete' &&
            $mybb->input['softdeletethread'] != 'restore' &&
            $mybb->input['softdeletethread'] != 'toggle'
        ) {
            $mybb->input['softdeletethread'] = '';
        }
        ...
        if (
            ...
            $mybb->input['openthread'] != 'open' &&
            $mybb->input['openthread'] != 'close' &&
            $mybb->input['openthread'] != 'toggle'
        ) {
            $mybb->input['openthread'] = '';
        }
        ...
        if (
            ...
            $mybb->input['stickthread'] != 'stick' &&
            $mybb->input['stickthread'] != 'unstick' &&
            $mybb->input['stickthread'] != 'toggle'
        ) {
            $mybb->input['stickthread'] = '';
        }
        ...
        if ($mybb->input['move_type'] == 2) {
            ...
            if (!$mybb->input['move_1_forum']) {
                ...
                $query = $db->simple_select(
                    ...
                    'type',
                    "fid = '" .
                        $mybb->get_input('move_1_forum', MyBB::INPUT_INT) .
                        "'"
                );
        ...
        if ($mybb->input['copy_type'] == 2) {
            ...
            if (!$mybb->input['copy_1_forum']) {
                ...
                $query = $db->simple_select(
                    ...
                    'type',
                    "fid = '" .
                        $mybb->get_input('copy_1_forum', MyBB::INPUT_INT) .
                        "'"
                );
                if ($db->fetch_field($query, 'type') == 'c') {
                    $errors[] = $lang->error_forum_is_category;
        ...
        if (!$errors) {
            $thread_options = [
                'confirmation' => $mybb->get_input(
                    'confirmation',
                    MyBB::INPUT_INT
                ),
                'deletethread' => $mybb->get_input(
                    'deletethread',
                    MyBB::INPUT_INT
                ),
                'mergethreads' => $mybb->get_input(
                    'mergethreads',
                    MyBB::INPUT_INT
                ),
                'deletepoll' => $mybb->get_input('deletepoll', MyBB::INPUT_INT),
                'removeredirects' => $mybb->get_input(
                    'removeredirects',
                    MyBB::INPUT_INT
                ),
                'removesubscriptions' => $mybb->get_input(
                    'removesubscriptions',
                    MyBB::INPUT_INT
                ),
                'recountrebuild' => $mybb->get_input(
                    'recountrebuild',
                    MyBB::INPUT_INT
                ),
                ...
                'movethread' => $mybb->get_input(
                    'move_1_forum',
                    MyBB::INPUT_INT
                ),
                'movethreadredirect' => $mybb->get_input(
                    'move_2_redirect',
                    MyBB::INPUT_INT
                ),
                'movethreadredirectexpire' => $mybb->get_input(
                    'move_3_redirecttime',
                    MyBB::INPUT_INT
                ),
                'copythread' => $mybb->get_input(
                    'copy_1_forum',
                    MyBB::INPUT_INT
                ),
            ...
            $plugins->run_hooks(
                'admin_config_mod_tools_edit_thread_tool_options',
                $args
            );

            $update_tool['type'] = 't';
            $update_tool['threadoptions'] = $db->escape_string(
                my_serialize($thread_options)
            );
            $update_tool['name'] = $db->escape_string($mybb->input['title']);
            $update_tool['description'] = $db->escape_string(
    ...
    $form = new Form(
        'index.php?module=config-mod_tools&amp;action=edit_thread_tool',
        'post'
    );
    ...
    if ($errors) {
        ...
        $mybb->input['move_3_redirecttime'] =
            $thread_options['movethreadredirectexpire'];
        ...
        $mybb->input['removesubscriptions'] =
            $thread_options['removesubscriptions'];
    ...
    $form_container->output_row(
        $lang->available_in_forums . ' <em>*</em>',
        '',
        $actions
    );
    ...
    $form_container->output_row(
        $lang->available_to_groups . ' <em>*</em>',
        '',
        $actions
    );
    $form_container->output_row(
        $lang->show_confirmation . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'confirmation',
            $mybb->input['confirmation'],
            ['style' => 'width: 2em;']
        )
    );
    $form_container->end();
    ...
    $form_container->output_row(
        $lang->stick_unstick_thread . ' <em>*</em>',
        '',
        $form->generate_select_box(
            'stickthread',
            $stick_unstick,
    ...
    $form_container->output_row(
        $lang->move_thread . ' <em>*</em>',
        $lang->move_thread_desc,
        $actions
    );
    ...
    $form_container->output_row(
        $lang->softdelete_restore_thread . ' <em>*</em>',
        '',
        $form->generate_select_box(
            'softdeletethread',
            $softdelete_restore,
    ...
    $form_container->output_row(
        $lang->delete_thread . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'deletethread',
            $mybb->input['deletethread'],
            ['style' => 'width: 2em;']
        )
    );
    $form_container->output_row(
        $lang->merge_thread . ' <em>*</em>',
        $lang->merge_thread_desc,
        $form->generate_yes_no_radio(
            'mergethreads',
    ...
    $form_container->output_row(
        $lang->delete_poll . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio('deletepoll', $mybb->input['deletepoll'], [
            'style' => 'width: 2em;',
        ])
    );
    $form_container->output_row(
        $lang->delete_redirects . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'removeredirects',
            $mybb->input['removeredirects'],
            ['style' => 'width: 2em;']
        )
    );
    $form_container->output_row(
        $lang->remove_subscriptions . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'removesubscriptions',
            $mybb->input['removesubscriptions'],
    ...
    $form_container->output_row(
        $lang->recount_rebuild . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'recountrebuild',
            $mybb->input['recountrebuild'],
            ['style' => 'width: 2em;']
        )
    );
    ...
    if (!empty($threadprefixes)) {
        ...
        $form_container->output_row(
            $lang->apply_thread_prefix . ' <em>*</em>',
            '',
            $form->generate_select_box(
                'threadprefix',
                $thread_prefixes,
    ...
    $form_container->output_row(
        $lang->subject_message_replacements,
        $lang->subject_message_replacements_desc
    );
    $form_container->output_row(
        $lang->new_subject . ' <em>*</em>',
        $lang->new_subject_desc,
        $form->generate_text_box('newsubject', $mybb->input['newsubject'], [
            'id' => 'newsubject',
        ])
    );
    $form_container->end();
    ...
    $form_container->output_row(
        $lang->add_new_reply,
        $lang->add_new_reply_desc,
        $form->generate_text_area('newreply', $mybb->input['newreply'], [
            'id' => 'newreply',
        ]),
        'newreply'
    );
    $form_container->output_row(
        $lang->reply_subject,
        $lang->reply_subject_desc,
        $form->generate_text_box(
            'newreplysubject',
            $mybb->input['newreplysubject'],
            ['id' => 'newreplysubject']
        ),
        'newreplysubject'
    );
    ...
    $form_container->output_row(
        $lang->private_message_message,
        $lang->private_message_message_desc,
        $form->generate_text_area('pm_message', $mybb->input['pm_message'], [
            'id' => 'pm_message',
        ]),
        'pm_message'
    );
    $form_container->output_row(
        $lang->private_message_subject,
        $lang->private_message_subject_desc,
        $form->generate_text_box('pm_subject', $mybb->input['pm_subject'], [
            'id' => 'pm_subject',
        ]),
        'pm_subject'
    );
...
if ($mybb->input['action'] == 'add_thread_tool') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            ...
            $plugins->run_hooks(
                'admin_config_mod_tools_add_thread_tool_options',
                $args
            );

            $new_tool['type'] = 't';
            $new_tool['threadoptions'] = $db->escape_string(
                my_serialize($thread_options)
            );
            $new_tool['name'] = $db->escape_string($mybb->input['title']);
            $new_tool['description'] = $db->escape_string(
                $mybb->input['description']
            );
            $new_tool['forums'] = '';
            $new_tool['groups'] = '';
            ...
            if ($mybb->get_input('threadprefix', MyBB::INPUT_INT) >= 0) {
                $thread_options['threadprefix'] = $mybb->get_input(
                    'threadprefix',
                    MyBB::INPUT_INT
                );
            }
...
if ($mybb->input['action'] == 'edit_post_tool') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            ...
            $mybb->input['approveposts'] != 'approve' &&
            $mybb->input['approveposts'] != 'unapprove' &&
            $mybb->input['approveposts'] != 'toggle'
        ) {
            $mybb->input['approveposts'] = '';
        }
        ...
        if (
            ...
            $mybb->input['softdeleteposts'] != 'approve' &&
            $mybb->input['softdeleteposts'] != 'unapprove' &&
            $mybb->input['softdeleteposts'] != 'toggle'
        ) {
            $mybb->input['softdeleteposts'] = '';
        }
        ...
        if (!$errors) {
            ...
            if (
                stripos($mybb->input['splitpostsnewsubject'], '{subject}') ===
                false
            ) {
                $mybb->input['splitpostsnewsubject'] =
                    '{subject}' . $mybb->input['splitpostsnewsubject'];
            }

            $post_options = [
                'deleteposts' => $mybb->get_input(
                    'deleteposts',
                    MyBB::INPUT_INT
                ),
            ...
            $plugins->run_hooks(
                'admin_config_mod_tools_edit_post_tool_options',
                $args
            );
            ...
            $update_tool['postoptions'] = $db->escape_string(
                my_serialize($post_options)
            );
            $update_tool['name'] = $db->escape_string($mybb->input['title']);
            $update_tool['description'] = $db->escape_string(
                $mybb->input['description']
            ...
            flash_message($lang->success_mod_tool_updated, 'success');
            admin_redirect(
                'index.php?module=config-mod_tools&action=post_tools'
            );
        }
    }
    ...
    $form = new Form(
        'index.php?module=config-mod_tools&amp;action=edit_post_tool',
        'post'
    );
    echo $form->generate_hidden_field('tid', $mybb->input['tid']);
    ...
    if ($errors) {
        ...
        $mybb->input['splitpostsnewsubject'] =
            $post_options['splitpostsnewsubject'];
        $mybb->input['splitpostsaddreply'] =
            $post_options['splitpostsaddreply'];
        $mybb->input['splitpostsreplysubject'] =
            $post_options['splitpostsreplysubject'];
    }
    ...
    $form_container->output_row(
        $lang->softdelete_restore_posts . ' <em>*</em>',
        '',
        $form->generate_select_box(
            'softdeleteposts',
            $softdelete_restore,
    ...
    $form_container->output_row(
        $lang->delete_posts . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio('deleteposts', $mybb->input['deleteposts'])
    );
    $form_container->output_row(
        $lang->merge_posts . ' <em>*</em>',
        $lang->merge_posts_desc,
        $form->generate_yes_no_radio('mergeposts', $mybb->input['mergeposts'])
    );
    $form_container->output_row(
        $lang->approve_unapprove_posts . ' <em>*</em>',
        '',
        $form->generate_select_box(
            'approveposts',
            $approve_unapprove,
    ...
    $form_container->output_row(
        $lang->split_posts2 . ' <em>*</em>',
        '',
        $form->generate_forum_select('splitposts', $mybb->input['splitposts'])
    );
    $form_container->output_row(
        $lang->close_split_thread . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'splitpostsclose',
            $mybb->input['splitpostsclose']
    ...
    $form_container->output_row(
        $lang->unapprove_split_thread . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'splitpostsunapprove',
            $mybb->input['splitpostsunapprove']
    ...
    if (!empty($splitthreadprefix)) {
        ...
        $form_container->output_row(
            $lang->split_thread_prefix . ' <em>*</em>',
            '',
            $form->generate_select_box(
                'splitthreadprefix',
                $split_thread_prefixes,
                [$mybb->get_input('splitthreadprefix', MyBB::INPUT_INT)],
                ['id' => 'splitthreadprefix']
            ),
            'splitthreadprefix'
        );
    }
    ...
    $form_container->output_row(
        $lang->split_thread_subject,
        $lang->split_thread_subject_desc,
        $form->generate_text_box(
            'splitpostsnewsubject',
            $mybb->input['splitpostsnewsubject'],
            ['id' => 'splitpostsnewsubject ']
        ),
        'newreplysubject'
    );
    $form_container->output_row(
        $lang->add_new_split_reply,
        $lang->add_new_split_reply_desc,
        $form->generate_text_area(
            'splitpostsaddreply',
            $mybb->input['splitpostsaddreply'],
            ['id' => 'splitpostsaddreply']
        ),
        'splitpostsaddreply'
    );
    $form_container->output_row(
        $lang->split_reply_subject,
        $lang->split_reply_subject_desc,
        $form->generate_text_box(
            'splitpostsreplysubject',
            $mybb->input['splitpostsreplysubject'],
            ['id' => 'splitpostsreplysubject']
        ),
        'splitpostsreplysubject'
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'add_post_tool') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            $mybb->input['softdeleteposts'] != '' &&
            $mybb->input['softdeleteposts'] != 'softdelete' &&
            $mybb->input['softdeleteposts'] != 'restore' &&
            $mybb->input['softdeleteposts'] != 'toggle'
        ) {
            $mybb->input['softdeleteposts'] = '';
        }
        ...
        if (!$errors) {
            ...
            $plugins->run_hooks(
                'admin_config_mod_tools_add_post_tool_options',
                $args
            );
            ...
            $new_tool['postoptions'] = $db->escape_string(
                my_serialize($post_options)
            );
            $new_tool['name'] = $db->escape_string($mybb->input['title']);
            $new_tool['description'] = $db->escape_string(
    ...
    $form = new Form(
        'index.php?module=config-mod_tools&amp;action=add_post_tool',
        'post'
    );
    ...
    $form_container->output_row(
        $lang->private_message_subject,
        $lang->private_message_subject_desc,
        $form->generate_text_box('pm_subject', $mybb->get_input('pm_subject'), [
            'id' => 'pm_subject',
        ]),
        'pm_subject'
    );
...
if (!$mybb->input['action']) {
    ...
    while ($tool = $db->fetch_array($query)) {
        $table->construct_cell(
            "<a href=\"index.php?module=config-mod_tools&amp;action=edit_thread_tool&amp;tid={$tool['tid']}\"><strong>" .
                htmlspecialchars_uni($tool['name']) .
                '</strong></a><br /><small>' .
                htmlspecialchars_uni($tool['description']) .
                '</small>'
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=config-mod_tools&amp;action=edit_thread_tool&amp;tid={$tool['tid']}\">{$lang->edit}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-mod_tools&amp;action=delete_thread_tool&amp;tid={$tool['tid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_thread_tool_deletion}')\">{$lang->delete}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_thread_tools, ['colspan' => 3]);
        $table->construct_row();
    }
