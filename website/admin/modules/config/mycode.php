...
if (
    $mybb->input['action'] == 'xmlhttp_test_mycode' &&
    ...
    $sandbox = test_regex(
        $mybb->input['regex'],
        $mybb->input['replacement'],
        $mybb->input['test_value']
    );
...
if ($mybb->input['action'] == 'add') {
    ...
    $form = new Form(
        'index.php?module=config-mycode&amp;action=add',
        'post',
        'add'
    );
    ...
    $form_container->output_row(
        $lang->regular_expression . ' <em>*</em>',
        $lang->regular_expression_desc .
            '<br /><strong>' .
            $lang->example .
            '</strong> \[b\](.*?)\[/b\]',
        $form->generate_text_area('regex', $mybb->get_input('regex'), [
            'id' => 'regex',
        ]),
        'regex'
    );
    $form_container->output_row(
        $lang->replacement . ' <em>*</em>',
        $lang->replacement_desc .
            '<br /><strong>' .
            $lang->example .
            '</strong> &lt;strong&gt;$1&lt;/strong&gt;',
        $form->generate_text_area(
            'replacement',
            $mybb->get_input('replacement'),
            ['id' => 'replacement']
        ),
        'replacement'
    );
    ...
    $form_container->output_row(
        $lang->parse_order,
        $lang->parse_order_desc,
        $form->generate_numeric_field(
            'parseorder',
            $mybb->get_input('parseorder'),
            ['id' => 'parseorder', 'min' => 0]
        ),
        'parseorder'
    );
    ...
    $form_container->output_row(
        $lang->test_value,
        $lang->test_value_desc,
        $form->generate_text_area(
            'test_value',
            $mybb->get_input('test_value'),
            ['id' => 'test_value']
        ) .
            '<br />' .
            $form->generate_submit_button($lang->test, [
                'id' => 'test',
                'name' => 'test',
            ]),
        'test_value'
    ...
    $form_container->output_row(
        $lang->result_html,
        $lang->result_html_desc,
        $form->generate_text_area(
            'result_html',
            isset($sandbox['html']) ? $sandbox['html'] : null,
            ['id' => 'result_html', 'disabled' => 1]
        ),
        'result_html'
    );
    $form_container->output_row(
        $lang->result_actual,
        $lang->result_actual_desc,
        "<div id=\"result_actual\">{$sandbox_actual}</div>"
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        'index.php?module=config-mycode&amp;action=edit',
        'post',
        'edit'
    );
    ...
    $form_container->output_row(
        ...
        $lang->regular_expression_desc .
            ...
            $lang->example .
            '</strong> \[b\](.*?)\[/b\]',
        $form->generate_text_area('regex', $mybb->input['regex'], [
            'id' => 'regex',
        ]),
        'regex'
    );
    $form_container->output_row(
        ...
        $lang->replacement_desc .
            ...
            $lang->example .
            '</strong> &lt;strong&gt;$1&lt;/strong&gt;',
        $form->generate_text_area('replacement', $mybb->input['replacement'], [
            'id' => 'replacement',
        ]),
        'replacement'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-mycode&amp;action=delete&amp;cid={$mycode['cid']}",
            $lang->confirm_mycode_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    while ($mycode = $db->fetch_array($query)) {
        ...
        if ($mycode['description']) {
            $mycode['description'] =
                '<small>' .
                htmlspecialchars_uni($mycode['description']) .
                '</small>';
        }

        $table->construct_cell(
            "<div>{$icon}<strong><a href=\"index.php?module=config-mycode&amp;action=edit&amp;cid={$mycode['cid']}\">" .
                htmlspecialchars_uni($mycode['title']) .
                "</a></strong><br />{$mycode['description']}</div>"
        );

        $popup = new PopupMenu("mycode_{$mycode['cid']}", $lang->options);
        $popup->add_item(
            $lang->edit_mycode,
            "index.php?module=config-mycode&amp;action=edit&amp;cid={$mycode['cid']}"
        );
        $popup->add_item(
            $phrase,
            "index.php?module=config-mycode&amp;action=toggle_status&amp;cid={$mycode['cid']}&amp;my_post_key={$mybb->post_code}"
        );
        $popup->add_item(
            $lang->delete_mycode,
            "index.php?module=config-mycode&amp;action=delete&amp;cid={$mycode['cid']}&amp;my_post_key={$mybb->post_code}",
            "return AdminCP.deleteConfirmation(this, '{$lang->confirm_mycode_deletion}')"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_mycode, ['colspan' => 2]);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            'index.php?module=config-mycode&amp;page={page}'
        );
...
{
    $array = [];
    $array['actual'] = @preg_replace(
        '#' . str_replace("\x0", '', $regex) . '#si',
        $replacement,
        $test
    );
 ...
 */
function check_existing_regex($regex = '', $current = [])
{
    global $cache;
