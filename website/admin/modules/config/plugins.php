...
if ($mybb->input['action'] == 'browse') {
    ...
    $contents = fetch_remote_file(
        "https://community.mybb.com/xmlbrowse.php?api=2&type=plugins&version={$search_version}{$keywords}{$url_page}"
    );
    ...
    if (!empty($tree['results']['result'])) {
        ...
        foreach ($tree['results']['result'] as $result) {
            ...
            $result['version']['value'] = htmlspecialchars_uni(
            ...
            $table->construct_cell($result['version']['value'], [
                'class' => 'align_center',
            ]);
            $table->construct_cell(
    ...
    $search = new Form(
        'index.php?module=config-plugins&amp;action=browse',
        'post',
        'search_form'
    );
    ...
    if ($mybb->request_method == 'post') {
        $table->output(
            "<span style=\"float: right;\"><small><a href=\"https://community.mybb.com/mods.php?action=browse&category=plugins\" target=\"_blank\" rel=\"noopener\">{$lang->browse_all_plugins}</a></small></span>" .
        ...
        $table->output(
            "<span style=\"float: right;\"><small><a href=\"https://community.mybb.com/mods.php?action=browse&category=plugins\" target=\"_blank\" rel=\"noopener\">{$lang->browse_all_plugins}</a></small></span>" .
                $lang->sprintf(
                    $lang->recommended_plugins_for_mybb,
                    $mybb->version
                )
        );
    }
    ...
    if (!$no_results) {
        echo '<br />' .
            draw_admin_pagination(
                ...
                "index.php?module=config-plugins&amp;action=browse{$keywords}&amp;page={page}"
            );
    }
...
if ($mybb->input['action'] == 'check') {
    ...
    if ($plugins_list) {
        ...
        foreach ($plugins_list as $plugin_file) {
            ...
            $plugininfo = $infofunc();
            $plugininfo['guid'] = isset($plugininfo['guid'])
                ? trim($plugininfo['guid'])
                : null;
            $plugininfo['codename'] = isset($plugininfo['codename'])
                ? trim($plugininfo['codename'])
                : null;

            if ($plugininfo['codename'] != '') {
                $info[] = $plugininfo['codename'];
                $names[$plugininfo['codename']] = [
                    'name' => $plugininfo['name'],
                    'version' => $plugininfo['version'],
                ];
            } elseif ($plugininfo['guid'] != '') {
                $info[] = $plugininfo['guid'];
                $names[$plugininfo['guid']] = [
                    'name' => $plugininfo['name'],
                    'version' => $plugininfo['version'],
                ];
            }
    ...
    foreach ($tree['plugins']['plugin'] as $plugin) {
        $compare_by = array_key_exists('codename', $plugin['attributes'])
            ? 'codename'
            : 'guid';
        ...
        if (
            version_compare(
                $names[$plugin['attributes'][$compare_by]]['version'],
                $plugin['version']['value'],
                '<'
            )
        ) {
            $plugin['download_url']['value'] = htmlspecialchars_uni(
                $plugin['download_url']['value']
            );
            $plugin['version']['value'] = htmlspecialchars_uni(
                $plugin['version']['value']
            );

            if (isset($plugin['vulnerable']['value'])) {
                $plugin['vulnerable']['value'] = htmlspecialchars_uni(
            ...
            if ($is_vulnerable) {
                ...
                $table->construct_cell(
                    "<strong>{$names[$plugin['attributes'][$compare_by]]['name']}</strong>"
                );
            }
            $table->construct_cell(
                "{$names[$plugin['attributes'][$compare_by]]['version']}",
            ...
            $table->construct_cell(
                "<strong><span style=\"color: #C00\">{$plugin['version']['value']}</span></strong>",
                ['class' => 'align_center']
            );
            if ($is_vulnerable) {
                $table->construct_cell(
                    "<a href=\"index.php?module=config-plugins\"><b>{$lang->deactivate}</b></a>",
                    ['class' => 'align_center', 'width' => 150]
                );
            } else {
                $table->construct_cell(
                    "<strong><a href=\"https://community.mybb.com/{$plugin['download_url']['value']}\" target=\"_blank\" rel=\"noopener\">{$lang->download}</a></strong>",
                    ['class' => 'align_center']
                );
            }
            $table->construct_row();
...
if (
    ...
    $active_plugins = isset($plugins_cache['active'])
        ? $plugins_cache['active']
        : [];
    ...
    if ($mybb->input['action'] == 'activate') {
        ...
        if ($plugins->is_compatible($codename) == false) {
            flash_message(
                $lang->sprintf($lang->plugin_incompatible, $mybb->version),
                'error'
            );
            admin_redirect('index.php?module=config-plugins');
        }
        ...
        $executed[] = 'activate';
    } elseif ($mybb->input['action'] == 'deactivate') {
        ...
        if (
            $mybb->get_input('uninstall') == 1 &&
            function_exists("{$codename}_uninstall")
        ) {
...
if (!$mybb->input['action']) {
    ...
    if (!empty($plugins_list)) {
        ...
        if (empty($a_plugins)) {
            $table->construct_cell($lang->no_active_plugins, ['colspan' => 3]);
            $table->construct_row();
        } else {
        ...
        if (empty($i_plugins)) {
            $table->construct_cell($lang->no_inactive_plugins, [
                'colspan' => 3,
            ]);
            $table->construct_row();
        } else {
    ...
    foreach ($plugin_list as $plugininfo) {
        ...
        if ($installed == false) {
            if ($compatibility_warning) {
                $table->construct_cell("{$compatibility_warning}", [
                    'class' => 'align_center',
                    'colspan' => 2,
                ]);
            } else {
                $table->construct_cell(
                    "<a href=\"index.php?module=config-plugins&amp;action=activate&amp;plugin={$plugininfo['codename']}&amp;my_post_key={$mybb->post_code}\">{$lang->install_and_activate}</a>",
        ...
        elseif ($plugininfo['is_active']) {
            $table->construct_cell(
                "<a href=\"index.php?module=config-plugins&amp;action=deactivate&amp;plugin={$plugininfo['codename']}&amp;my_post_key={$mybb->post_code}\">{$lang->deactivate}</a>",
                ['class' => 'align_center', 'width' => 150]
            );
            if ($uninstall_button) {
                $table->construct_cell(
                    "<a href=\"index.php?module=config-plugins&amp;action=deactivate&amp;uninstall=1&amp;plugin={$plugininfo['codename']}&amp;my_post_key={$mybb->post_code}\">{$lang->uninstall}</a>",
                    ['class' => 'align_center', 'width' => 150]
                );
            } else {
                $table->construct_cell('&nbsp;', [
        ...
        elseif ($installed == true) {
            if ($compatibility_warning && !$uninstall_button) {
                ...
                $table->construct_cell(
                    "<a href=\"index.php?module=config-plugins&amp;action=activate&amp;plugin={$plugininfo['codename']}&amp;my_post_key={$mybb->post_code}\">{$lang->activate}</a>",
                    ['class' => 'align_center', 'width' => 150]
                );
                if ($uninstall_button) {
                    $table->construct_cell(
