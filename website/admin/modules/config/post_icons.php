...
if ($mybb->input['action'] == 'add') {
    ...
    $form = new Form(
        'index.php?module=config-post_icons&amp;action=add',
        'post',
        'add'
    );
    ...
    if ($mybb->request_method == 'post') {
        if ($mybb->input['step'] == 1) {
            ...
            if (!$errors) {
                ...
                $page->output_header(
                    $lang->post_icons . ' - ' . $lang->add_multiple_post_icons
                );
                ...
                $form_container->output_row_header($lang->add, [
                    'class' => 'align_center',
                    'width' => '5%',
                ]);
                ...
                foreach ($icons as $key => $file) {
                    ...
                    $form_container->output_cell(
                        $form->generate_text_box("name[{$file}]", $name, [
                            'id' => 'name',
                            'style' => 'width: 98%',
                        ])
                    );
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        'index.php?module=config-post_icons&amp;action=edit',
        'post',
        'edit'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-post_icons&amp;action=delete&amp;iid={$icon['iid']}",
            $lang->confirm_post_icon_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->name, ['width' => '70%']);
    ...
    while ($icon = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<img src=\"" . htmlspecialchars_uni($image) . "\" alt=\"\" />",
            ['class' => 'align_center']
        );
        $table->construct_cell(htmlspecialchars_uni($icon['name']));

        $table->construct_cell(
            "<a href=\"index.php?module=config-post_icons&amp;action=edit&amp;iid={$icon['iid']}\">{$lang->edit}</a>",
            ['class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-post_icons&amp;action=delete&amp;iid={$icon['iid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_post_icon_deletion}')\">{$lang->delete}</a>",
            ['class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_post_icons, ['colspan' => 4]);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            'index.php?module=config-post_icons&amp;page={page}'
        );
