...
if ($mybb->input['action'] == 'add') {
        ...
        if (!$errors) {
            ...
            foreach (['viewableby', 'editableby'] as $key) {
                if ($mybb->input[$key] == 'all') {
                    ...
                    if (
                        isset($mybb->input['select'][$key]) &&
                        is_array($mybb->input['select'][$key])
                    ) {
                        ...
                        $mybb->input[$key] = implode(
                            ',',
                            (array) $mybb->input['select'][$key]
                        );
                    } else {
                        $mybb->input[$key] = '';
                    }
            ...
            $db->write_query(
                'ALTER TABLE ' . TABLE_PREFIX . "userfields ADD fid{$fid} TEXT"
            );
    ...
    $page->output_header(
        $lang->custom_profile_fields . ' - ' . $lang->add_new_profile_field
    );
    ...
    $form = new Form(
        'index.php?module=config-profile_fields&amp;action=add',
        'post',
        'add'
    );
    ...
    if ($errors) {
        switch ($mybb->input['viewableby']) {
            case 'all':
                ...
                break;
            case 'custom':
                $mybb->input['viewableby'] = implode(
                    ',',
                    (array) $mybb->input['select']['viewableby']
                );
                break;
            default:
        ...
        switch ($mybb->input['editableby']) {
            case 'all':
                ...
                break;
            case 'custom':
                $mybb->input['editableby'] = implode(
                    ',',
                    (array) $mybb->input['select']['editableby']
                );
                break;
            default:
    ...
    $form_container->output_row(
        $lang->field_type . ' <em>*</em>',
        $lang->field_type_desc,
    ...
    $form_container->output_row(
        $lang->field_regex,
        $lang->field_regex_desc,
        $form->generate_text_box('regex', $mybb->get_input('regex'), [
            'id' => 'regex',
        ]),
        'regex',
        [],
    ...
    $form_container->output_row(
        $lang->maximum_length,
        $lang->maximum_length_desc,
        $form->generate_numeric_field(
            'maxlength',
            $mybb->get_input('maxlength'),
            ['id' => 'maxlength', 'min' => 0]
        ),
        'maxlength',
        [],
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field('length', $mybb->get_input('length'), [
            'id' => 'length',
            'min' => 0,
        ]),
        'length',
    ...
    $form_container->output_row(
        $lang->selectable_options,
        $lang->selectable_options_desc,
        $form->generate_text_area('options', $mybb->get_input('options'), [
            'id' => 'options',
        ]),
        'options',
        [],
    ...
    $form_container->output_row(
        $lang->min_posts_enabled,
        $lang->min_posts_enabled_desc,
    ...
    $form_container->output_row(
        $lang->display_order . ' <em>*</em>',
        $lang->display_order_desc,
        $form->generate_numeric_field(
            'disporder',
    ...
    $form_container->output_row(
        $lang->show_on_registration . ' <em>*</em>',
        $lang->show_on_registration_desc,
        $form->generate_yes_no_radio(
            'registration',
            $mybb->get_input('registration')
        )
    );
    $form_container->output_row(
        $lang->display_on_profile . ' <em>*</em>',
        $lang->display_on_profile_desc,
        $form->generate_yes_no_radio('profile', $mybb->get_input('profile'))
    );
    $form_container->output_row(
        $lang->display_on_postbit . ' <em>*</em>',
        $lang->display_on_postbit_desc,
        $form->generate_yes_no_radio('postbit', $mybb->get_input('postbit'))
    );
		...
		<dd style=\"margin-top: 4px;\" id=\"viewableby_forums_groups_custom\" class=\"viewableby_forums_groups\">
			<table cellpadding=\"4\">
				<tr>
					...
					<td>" .
        $form->generate_group_select('select[viewableby][]', $selected_values, [
            'id' => 'viewableby',
            'multiple' => true,
            'size' => 5,
        ]) .
		...
		<dd style=\"margin-top: 4px;\" id=\"editableby_forums_groups_custom\" class=\"editableby_forums_groups\">
			<table cellpadding=\"4\">
				<tr>
					<td valign=\"top\"><small>{$lang->groups_colon}</small></td>
					<td>" .
        $form->generate_group_select('select[editableby][]', $selected_values, [
            'id' => 'editableby',
            'multiple' => true,
            'size' => 5,
        ]) .
    ...
    $parser_options = [
        $form->generate_check_box('allowhtml', 1, $lang->parse_allowhtml, [
            'checked' => $mybb->get_input('allowhtml'),
            'id' => 'allowhtml',
        ]),
        $form->generate_check_box('allowmycode', 1, $lang->parse_allowmycode, [
            'checked' => $mybb->get_input('allowmycode'),
            'id' => 'allowmycode',
        ]),
        $form->generate_check_box(
            'allowsmilies',
            1,
            $lang->parse_allowsmilies,
            [
                'checked' => $mybb->get_input('allowsmilies'),
                'id' => 'allowsmilies',
            ]
        ),
        $form->generate_check_box(
            'allowimgcode',
            1,
            $lang->parse_allowimgcode,
            [
                'checked' => $mybb->get_input('allowimgcode'),
                'id' => 'allowimgcode',
            ]
        ),
        $form->generate_check_box(
            'allowvideocode',
            1,
            $lang->parse_allowvideocode,
            [
                'checked' => $mybb->get_input('allowvideocode'),
                'id' => 'allowvideocode',
            ]
        ),
    ];
    $form_container->output_row(
        $lang->parser_options,
        '',
        implode('<br />', $parser_options),
        '',
        [],
        ['id' => 'row_parser_options']
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'edit') {
    ...
    $page->output_header(
        $lang->custom_profile_fields . ' - ' . $lang->edit_profile_field
    );
    ...
    $form = new Form(
        'index.php?module=config-profile_fields&amp;action=edit',
        'post',
        'edit'
    );
    ...
    $form_container->output_row(
        $lang->field_regex,
        $lang->field_regex_desc,
        $form->generate_text_box('regex', $mybb->input['regex'], [
            'id' => 'regex',
        ]),
        'regex',
        [],
    ...
    $form_container->output_row(
        $lang->maximum_length,
        $lang->maximum_length_desc,
        $form->generate_numeric_field('maxlength', $mybb->input['maxlength'], [
            'id' => 'maxlength',
            'min' => 0,
        ]),
        'maxlength',
    ...
    $form_container->output_row(
        $lang->field_length,
        $lang->field_length_desc,
        $form->generate_numeric_field('length', $mybb->input['length'], [
            'id' => 'length',
            'min' => 0,
        ]),
        'length',
    ...
    $form_container->output_row(
        $lang->selectable_options,
        $lang->selectable_options_desc,
        $form->generate_text_area('options', $mybb->input['options'], [
            'id' => 'options',
        ]),
        'options',
        [],
    ...
    $form_container->output_row(
        $lang->min_posts_enabled,
        $lang->min_posts_enabled_desc,
        $form->generate_numeric_field('postnum', $mybb->input['postnum'], [
            'id' => 'postnum',
            'min' => 0,
        ]),
        'postnum'
    ...
    $form_container->output_row(
        $lang->display_order . ' <em>*</em>',
        $lang->display_order_desc,
        $form->generate_numeric_field('disporder', $mybb->input['disporder'], [
            'id' => 'disporder',
            'min' => 0,
        ]),
        'disporder'
    ...
    $form_container->output_row(
        $lang->required . ' <em>*</em>',
        $lang->required_desc,
        $form->generate_yes_no_radio('required', $mybb->input['required'])
    );
    $form_container->output_row(
        $lang->show_on_registration . ' <em>*</em>',
        $lang->show_on_registration_desc,
        $form->generate_yes_no_radio(
            'registration',
            $mybb->input['registration']
        )
    );
    $form_container->output_row(
        $lang->display_on_profile . ' <em>*</em>',
        $lang->display_on_profile_desc,
        $form->generate_yes_no_radio('profile', $mybb->input['profile'])
    );
    $form_container->output_row(
        $lang->display_on_postbit . ' <em>*</em>',
        $lang->display_on_postbit_desc,
        $form->generate_yes_no_radio('postbit', $mybb->input['postbit'])
    );
    ...
    $parser_options = [
        $form->generate_check_box('allowhtml', 1, $lang->parse_allowhtml, [
            'checked' => $mybb->input['allowhtml'],
            'id' => 'allowhtml',
        ]),
        $form->generate_check_box('allowmycode', 1, $lang->parse_allowmycode, [
            'checked' => $mybb->input['allowmycode'],
            'id' => 'allowmycode',
        ]),
        $form->generate_check_box(
            'allowsmilies',
            1,
            $lang->parse_allowsmilies,
            ['checked' => $mybb->input['allowsmilies'], 'id' => 'allowsmilies']
        ),
        $form->generate_check_box(
            'allowimgcode',
            1,
            $lang->parse_allowimgcode,
            ['checked' => $mybb->input['allowimgcode'], 'id' => 'allowimgcode']
        ),
        $form->generate_check_box(
            'allowvideocode',
            1,
            $lang->parse_allowvideocode,
            [
                'checked' => $mybb->input['allowvideocode'],
                'id' => 'allowvideocode',
            ]
        ),
    ];
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $db->write_query(
            'ALTER TABLE ' .
                TABLE_PREFIX .
                "userfields DROP fid{$profile_field['fid']}"
        );
        ...
        $page->output_confirm_action(
            "index.php?module=config-profile_fields&amp;action=delete&amp;fid={$profile_field['fid']}",
            $lang->confirm_profile_field_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->required, ['class' => 'align_center']);
    $table->construct_header($lang->registration, ['class' => 'align_center']);
    $table->construct_header($lang->editable, ['class' => 'align_center']);
    $table->construct_header($lang->profile, ['class' => 'align_center']);
    $table->construct_header($lang->postbit, ['class' => 'align_center']);
    ...
    while ($field = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<strong><a href=\"index.php?module=config-profile_fields&amp;action=edit&amp;fid={$field['fid']}\">" .
        ...
        $table->construct_cell($required, [
            'class' => 'align_center',
            'width' => '10%',
        ]);
        $table->construct_cell($registration, [
            'class' => 'align_center',
            'width' => '10%',
        ]);
        $table->construct_cell($editable, [
            'class' => 'align_center',
            'width' => '10%',
        ]);
        ...
        $popup->add_item(
            $lang->edit_field,
            "index.php?module=config-profile_fields&amp;action=edit&amp;fid={$field['fid']}"
        );
        $popup->add_item(
            $lang->delete_field,
            "index.php?module=config-profile_fields&amp;action=delete&amp;fid={$field['fid']}&amp;my_post_key={$mybb->post_code}",
            "return AdminCP.deleteConfirmation(this, '{$lang->confirm_profile_field_deletion}')"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_profile_fields, ['colspan' => 7]);
        $table->construct_row();
    }
