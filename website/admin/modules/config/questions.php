...
if ($mybb->input['action'] == 'add') {
    ...
    $page->output_header(
        $lang->security_questions . ' - ' . $lang->add_new_question
    );
    ...
    if (isset($mybb->input['preview']) && !$errors) {
        ...
        $table->construct_cell(
            $parser->parse_message($mybb->input['question'], $parser_options)
        );
        $table->construct_row();
    ...
    $form = new Form(
        'index.php?module=config-questions&amp;action=add',
        'post',
        'add'
    );
    ...
    $form_container->output_row(
        $lang->question . ' <em>*</em>',
        $lang->question_desc,
        $form->generate_text_area('question', $mybb->get_input('question'), [
            'id' => 'question',
        ]),
        'question'
    );
    $form_container->output_row(
        $lang->answers . ' <em>*</em>',
        $lang->answers_desc,
        $form->generate_text_area('answer', $mybb->get_input('answer'), [
            'id' => 'answer',
        ]),
        'answer'
    );
    $form_container->output_row(
        $lang->active . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio('active', $mybb->input['active'])
    );
    $form_container->end();

    $buttons[] = $form->generate_submit_button($lang->save_question);
    $buttons[] = $form->generate_submit_button($lang->preview_question, [
        'name' => 'preview',
    ]);
...
if ($mybb->input['action'] == 'edit') {
    ...
    $page->output_header(
        $lang->security_questions . ' - ' . $lang->edit_question
    );
    ...
    $form = new Form(
        "index.php?module=config-questions&amp;action=edit&amp;qid={$question['qid']}",
        'post',
        'add'
    );
    ...
    if (
        (isset($mybb->input['preview']) || $show_preview === true) &&
        !$errors
    ) {
        $table = new Table();
    ...
    $form_container->output_row(
        $lang->question . ' <em>*</em>',
        $lang->question_desc,
        $form->generate_text_area('question', $mybb->input['question'], [
            'id' => 'question',
        ]),
        'question'
    );
    $form_container->output_row(
        $lang->answers . ' <em>*</em>',
        $lang->answers_desc,
        $form->generate_text_area('answer', $mybb->input['answer'], [
            'id' => 'answer',
        ]),
        'answer'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-questions&amp;action=delete&amp;qid={$question['qid']}",
            $lang->confirm_question_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->answers, ['width' => '35%']);
    $table->construct_header($lang->shown, [
        'width' => '5%',
        'class' => 'align_center',
    ]);
    $table->construct_header($lang->correct, [
        'width' => '5%',
        'class' => 'align_center',
    ]);
    $table->construct_header($lang->incorrect, [
        'width' => '5%',
        'class' => 'align_center',
    ]);
    ...
    $query = $db->simple_select('questions', '*', '', [
        'limit_start' => $start,
        'limit' => 20,
        'order_by' => 'question',
    ]);
    while ($questions = $db->fetch_array($query)) {
        ...
        $table->construct_cell($questions['shown'], [
            'class' => 'align_center',
        ]);
        $table->construct_cell($questions['correct'], [
            'class' => 'align_center',
        ]);
        $table->construct_cell($questions['incorrect'], [
            'class' => 'align_center',
        ]);
        $popup = new PopupMenu("questions_{$questions['qid']}", $lang->options);
        $popup->add_item(
            $lang->edit_question,
            "index.php?module=config-questions&amp;action=edit&amp;qid={$questions['qid']}"
        );
        $popup->add_item(
            $lang->preview_question,
            "index.php?module=config-questions&amp;action=edit&amp;qid={$questions['qid']}&amp;preview_list"
        );
        if ($questions['active'] == 1) {
            $popup->add_item(
                $lang->disable_question,
                "index.php?module=config-questions&amp;action=disable&amp;qid={$questions['qid']}&amp;my_post_key={$mybb->post_code}"
            );
        } else {
            $popup->add_item(
                $lang->enable_question,
                "index.php?module=config-questions&amp;action=enable&amp;qid={$questions['qid']}&amp;my_post_key={$mybb->post_code}"
            );
        }
        $popup->add_item(
            $lang->delete_question,
            "index.php?module=config-questions&amp;action=delete&amp;qid={$questions['qid']}&amp;my_post_key={$mybb->post_code}",
            "return AdminCP.deleteConfirmation(this, '{$lang->confirm_question_deletion}')"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_security_questions, ['colspan' => 6]);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            'index.php?module=config-questions&amp;page={page}'
        );
