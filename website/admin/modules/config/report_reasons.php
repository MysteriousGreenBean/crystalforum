...
if ($mybb->input['action'] == 'add') {
    ...
    $form = new Form(
        'index.php?module=config-report_reasons&amp;action=add',
        'post',
        'add'
    );
    ...
    $form_container->output_row(
        $lang->reason_title . ' <em>*</em>',
        $lang->reason_title_desc,
    ...
    $form_container->output_row(
        $lang->applies_to . ' <em>*</em>',
        '',
        generate_content_select()
    );
    $form_container->output_row(
        $lang->requires_extra . ' <em>*</em>',
        $lang->requires_extra_desc,
        $form->generate_yes_no_radio('extra', $mybb->input['extra'])
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        "index.php?module=config-report_reasons&amp;action=edit&amp;rid={$reason['rid']}",
        'post',
        'add'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-report_reasons&amp;action=delete&amp;rid={$reason['rid']}",
            $lang->confirm_reason_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    $form_container->output_row_header($lang->applies_to, ['width' => '35%']);
    $form_container->output_row_header($lang->extra_comment, [
        'width' => '10%',
        'class' => 'align_center',
    ]);
    ...
    while ($reasons = $db->fetch_array($query)) {
        ...
        $form_container->output_cell(
            htmlspecialchars_uni($reasons['appliesto'])
        );
        $form_container->output_cell("<div>{$icon}</div>", [
            'class' => 'align_center',
        ]);
        $form_container->output_cell(
            "<input type=\"text\" name=\"disporder[{$reasons['rid']}]\" value=\"{$reasons['disporder']}\" class=\"text_input align_center\" style=\"width: 80%;\" />",
            ['class' => 'align_center']
        );
        $popup = new PopupMenu("reasons_{$reasons['rid']}", $lang->options);
        $popup->add_item(
            $lang->edit_reason,
            "index.php?module=config-report_reasons&amp;action=edit&amp;rid={$reasons['rid']}"
        );
        $popup->add_item(
            $lang->delete_reason,
            "index.php?module=config-report_reasons&amp;action=delete&amp;rid={$reasons['rid']}&amp;my_post_key={$mybb->post_code}",
            "return AdminCP.deleteConfirmation(this, '{$lang->confirm_reason_deletion}')"
        );
    ...
    if ($form_container->num_rows() == 0) {
        $form_container->output_cell($lang->no_report_reasons, [
            'colspan' => 5,
        ]);
        $form_container->construct_row();
    }
					...
					<dd style=\"margin-top: 4px;\" id=\"appliesto_forums_groups_custom\" class=\"appliesto_forums_groups\">
						<table cellpadding=\"4\">
							<tr>
								...
								<td>" .
        implode('<br />', generate_content_choices()) .
        "</td>
							</tr>
						</table>
					</dd>
...
function generate_content_choices()
{
    ...
    foreach ($content_types as $content) {
        $key = "report_content_{$content}";
        $content_choices[] = $form->generate_check_box(
            "appliesto_{$content}",
            1,
            $lang->$key,
            [
                'id' => "appliesto_{$content}",
                'checked' => $mybb->get_input("appliesto_{$content}"),
            ]
        );
    }
