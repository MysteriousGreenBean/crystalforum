...
$page->add_breadcrumb_item(
    $lang->board_settings,
    'index.php?module=config-settings'
);
...
if ($mybb->input['action'] == 'addgroup') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if ($db->num_rows($query) > 0) {
            ...
            $errors[] = $lang->sprintf(
                $lang->error_duplicate_group_name,
                $dup_group_title
            );
        }
    ...
    $page->output_header(
        $lang->board_settings . ' - ' . $lang->add_new_setting_group
    );
    ...
    $form = new Form(
        'index.php?module=config-settings&amp;action=addgroup',
        'post',
        'add'
    );
...
if ($mybb->input['action'] == 'editgroup') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $query = $db->simple_select(
            ...
            'title',
            "name='" .
                $db->escape_string($mybb->input['name']) .
                "' AND gid != '{$group['gid']}'"
        );
        if ($db->num_rows($query) > 0) {
            $dup_group_title = $db->fetch_field($query, 'title');
    ...
    $page->output_header(
        $lang->board_settings . ' - ' . $lang->edit_setting_group
    );
    ...
    $form = new Form(
        'index.php?module=config-settings&amp;action=editgroup',
        'post',
        'editgroup'
    );
    ...
    $form_container->output_row(
        $lang->title . ' <em>*</em>',
        '',
        $form->generate_text_box('title', $group_data['title'], [
            'id' => 'title',
        ]),
        'title'
    );
    $form_container->output_row(
        $lang->description,
        '',
        $form->generate_text_area('description', $group_data['description'], [
            'id' => 'description',
        ]),
        'description'
    );
    $form_container->output_row(
        $lang->display_order,
        '',
        $form->generate_numeric_field('disporder', $group_data['disporder'], [
            'id' => 'disporder',
            'min' => 0,
        ]),
        'disporder'
    ...
    $form_container->output_row(
        $lang->name . ' <em>*</em>',
        $lang->group_name_desc,
        $form->generate_text_box('name', $group_data['name'], ['id' => 'name']),
        'name'
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'deletegroup') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-settings&amp;action=deletegroup&amp;gid={$group['gid']}",
            $lang->confirm_setting_group_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            !ctype_alnum($mybb->input['type']) ||
            strtolower($mybb->input['type']) == 'php'
        ) {
            $mybb->input['type'] = '';
        }
    ...
    $page->output_header(
        $lang->board_settings . ' - ' . $lang->add_new_setting
    );
    ...
    $form = new Form(
        'index.php?module=config-settings&amp;action=add',
        'post',
        'add'
    );
    ...
    while ($group = $db->fetch_array($query)) {
        ...
        if (!empty($lang->$group_lang_var)) {
            $options[$group['gid']] = htmlspecialchars_uni(
                $lang->$group_lang_var
            );
        } else {
    ...
    $form_container->output_row(
        $lang->group . ' <em>*</em>',
        '',
        $form->generate_select_box('gid', $options, $mybb->get_input('gid'), [
            'id' => 'gid',
        ]),
        'gid'
    );
    ...
    $form_container->output_row(
        $lang->extra,
        $lang->extra_desc,
        $form->generate_text_area('extra', $mybb->get_input('extra'), [
            'id' => 'extra',
        ]),
        'extra',
        [],
    ...
    $form_container->output_row(
        $lang->value,
        '',
        $form->generate_text_area('value', $mybb->get_input('value'), [
            'id' => 'value',
        ]),
        'value'
    );
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        'index.php?module=config-settings&amp;action=edit',
        'post',
        'edit'
    );
    ...
    $form_container->output_row(
        $lang->title . ' <em>*</em>',
        '',
        $form->generate_text_box('title', $setting_data['title'], [
            'id' => 'title',
        ]),
        'title'
    );
    $form_container->output_row(
        $lang->description,
        '',
        $form->generate_text_area('description', $setting_data['description'], [
            'id' => 'description',
        ]),
        'description'
    );
    ...
    $form_container->output_row(
        $lang->group . ' <em>*</em>',
        '',
        $form->generate_select_box('gid', $options, $setting_data['gid'], [
            'id' => 'gid',
        ]),
        'gid'
    );
    $form_container->output_row(
        $lang->display_order,
        '',
        $form->generate_numeric_field('disporder', $setting_data['disporder'], [
            'id' => 'disporder',
            'min' => 0,
        ]),
        'disporder'
    ...
    $form_container->output_row(
        $lang->name . ' <em>*</em>',
        $lang->name_desc,
        $form->generate_text_box('name', $setting_data['name'], [
            'id' => 'name',
        ]),
        'name'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_text_area(
            'extra',
            !empty($setting_data['extra']) ? $setting_data['extra'] : null,
            ['id' => 'extra']
        ),
        'extra',
        [],
    ...
    $form_container->output_row(
        $lang->value,
        '',
        $form->generate_text_area('value', $setting_data['value'], [
            'id' => 'value',
        ]),
        'value'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-settings&amp;action=delete&amp;sid={$setting['sid']}",
            $lang->confirm_setting_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'manage') {
    ...
    $page->output_header(
        $lang->board_settings . ' - ' . $lang->modify_existing_settings
    );
    ...
    $query = $db->simple_select(
        'settings',
        'sid, name, title, disporder, gid, isdefault',
        '',
    ...
    while ($group = $db->fetch_array($query)) {
        ...
        $table->construct_cell("<strong>{$group_title}</strong>", [
            'id' => "group{$group['gid']}",
        ]);
        $table->construct_cell(
            $form->generate_numeric_field(
                "group_disporder[{$group['gid']}]",
                $group['disporder'],
                [
        ...
        if ($group['isdefault'] != 1) {
            ...
            $popup->add_item(
                $lang->edit_setting_group,
                "index.php?module=config-settings&amp;action=editgroup&amp;gid={$group['gid']}"
            );
            $popup->add_item(
                $lang->delete_setting_group,
                "index.php?module=config-settings&amp;action=deletegroup&amp;gid={$group['gid']}&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_setting_group_deletion}')"
            );
        ...
        if (
            isset($settings_cache[$group['gid']]) &&
            is_array($settings_cache[$group['gid']])
        ) {
            foreach ($settings_cache[$group['gid']] as $setting) {
                $setting_lang_var = "setting_{$setting['name']}";
                if (!empty($lang->$setting_lang_var)) {
                    $setting_title = htmlspecialchars_uni(
                        $lang->$setting_lang_var
                    );
                } else {
                    $setting_title = htmlspecialchars_uni($setting['title']);
                }
                $table->construct_cell($setting_title, [
                    'style' => 'padding-left: 40px;',
                ]);
                $table->construct_cell(
                    $form->generate_numeric_field(
                        "setting_disporder[{$setting['sid']}]",
                        $setting['disporder'],
                        [
                            'style' => 'width: 80%',
                            'class' => 'align_center',
                ...
                if ($group['isdefault'] != 1 || $setting['isdefault'] != 1) {
                    $popup = new PopupMenu(
                        "setting_{$setting['sid']}",
                        $lang->options
                    );
                    $popup->add_item(
                        $lang->edit_setting,
                        "index.php?module=config-settings&amp;action=edit&amp;sid={$setting['sid']}"
                    );
                    $popup->add_item(
                        $lang->delete_setting,
                        "index.php?module=config-settings&amp;action=delete&amp;sid={$setting['sid']}&amp;my_post_key={$mybb->post_code}",
                        "return AdminCP.deleteConfirmation(this, '{$lang->confirm_setting_deletion}')"
                    );
                    $table->construct_cell($popup->fetch(), [
                        'class' => 'align_center',
                    ]);
...
if ($mybb->input['action'] == 'change') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $is_current_hiddencaptcha_wrong = in_array(
            $mybb->settings['hiddencaptchaimagefield'],
            $disallowed_fields
        );
        if (
            (isset($mybb->input['upsetting']['hiddencaptchaimagefield']) &&
                in_array(
                    $mybb->input['upsetting']['hiddencaptchaimagefield'],
                    $disallowed_fields
                )) ||
            ...
            $lang->success_settings_updated .= $lang->sprintf(
                $lang->success_settings_updated_hiddencaptchaimage,
                htmlspecialchars_uni(
                    $mybb->input['upsetting']['hiddencaptchaimagefield']
                ),
                htmlspecialchars_uni($wrong_value)
            );
        }
        ...
        $dimfields = [
            8 => ['postmaxavatarsize'],
            10 => ['useravatardims', 'maxavatardims'],
            13 => ['memberlistmaxavatarsize'],
        ];
            ...
            foreach ($dimfields[$gid] as $field) {
                if (isset($mybb->input['upsetting'][$field])) {
                    if (
                        preg_match(
                            '/\b\d+[|x]{1}\d+\b/i',
                            $mybb->input['upsetting'][$field]
                        ) ||
                        ($field == 'maxavatardims' &&
                            trim($mybb->input['upsetting'][$field]) == '')
                    ) {
                        // If pipe (|) is used normalize to 'x'
                        $mybb->input['upsetting'][$field] = str_replace(
                            '|',
                            'x',
                            my_strtolower($mybb->input['upsetting'][$field])
                        );
                    } else {
                        flash_message(
                            $lang->sprintf(
                                $lang->error_format_dimension,
                                $lang->{'error_field_' . $field}
                            ),
                            'error'
                        );
                        admin_redirect(
                            'index.php?module=config-settings&action=change&gid=' .
                                $gid
                        );
                    }
                }
        ...
        if ($gid == 9) {
            if (
                isset(
                    $mybb->input['upsetting']['minnamelength'],
                    $mybb->input['upsetting']['maxnamelength']
                ) &&
                $mybb->input['upsetting']['minnamelength'] > 0 &&
                $mybb->input['upsetting']['maxnamelength'] > 0 &&
                $mybb->input['upsetting']['minnamelength'] >
                    $mybb->input['upsetting']['maxnamelength']
            ) {
            ...
            if (
                isset(
                    $mybb->input['upsetting']['minpasswordlength'],
                    $mybb->input['upsetting']['maxpasswordlength']
                ) &&
                $mybb->input['upsetting']['minpasswordlength'] > 0 &&
                $mybb->input['upsetting']['maxpasswordlength'] > 0 &&
                $mybb->input['upsetting']['minpasswordlength'] >
                    $mybb->input['upsetting']['maxpasswordlength']
            ) {
            ...
            if (
                isset(
                    $mybb->input['upsetting']['minpasswordlength'],
                    $mybb->input['upsetting']['requirecomplexpasswords']
                ) &&
                $mybb->input['upsetting']['requirecomplexpasswords'] &&
                $mybb->input['upsetting']['minpasswordlength'] < 3
            ) {
                flash_message(
                    $lang->error_field_minpasswordlength_complex,
                    'error'
                );
                admin_redirect(
                    'index.php?module=config-settings&action=change&gid=' . $gid
        ...
        if (
            ...
            if (
                in_array($captchaimage, [
                    captcha::NOCAPTCHA_RECAPTCHA,
                    captcha::RECAPTCHA_INVISIBLE,
                ]) &&
                (!$recaptchaprivatekey || !$recaptchapublickey)
            ) {
                $set_captcha_image = true;
            } elseif (
                in_array($captchaimage, [captcha::RECAPTCHA_V3]) &&
                (!$recaptchaprivatekey ||
                    !$recaptchapublickey ||
                    !$recaptchascore)
            ) {
                ...
                (!$hcaptchaprivatekey || !$hcaptchapublickey)
            ) {
                $set_captcha_image = true;
            }
        }
        ...
        if (
            ...
            in_array($captchaimage, [captcha::RECAPTCHA_V3]) &&
            (!$recaptchaprivatekey || !$recaptchapublickey || !$recaptchascore)
        ) {
            $set_captcha_image = true;
        } elseif (
            in_array($captchaimage, [
        ...
        if ($set_captcha_image) {
            ...
            $lang->success_settings_updated .=
                $lang->success_settings_updated_captchaimage;
        }

        // If using fulltext then enforce minimum word length given by database
        if (
            isset($mybb->input['upsetting']['minsearchword']) &&
            $mybb->input['upsetting']['minsearchword'] > 0 &&
            $mybb->input['upsetting']['searchtype'] == 'fulltext' &&
            $db->supports_fulltext_boolean('posts') &&
            $db->supports_fulltext('threads')
        ) {
            ...
            if (
                ...
                $mybb->input['upsetting']['minsearchword'] = $min_length;
                $lang->success_settings_updated .=
                    $lang->success_settings_updated_minsearchword;
            }
        }

        // Get settings which optionscode is a forum/group select, checkbox or numeric
        // We cannot rely on user input to decide this
        $checkbox_settings = $forum_group_select = $prefix_select = [];
        $query = $db->simple_select(
            'settings',
            'name, optionscode',
            "optionscode IN('forumselect', 'groupselect', 'prefixselect') OR optionscode LIKE 'checkbox%' OR optionscode LIKE 'numeric%'"
        );
        ...
        while ($multisetting = $db->fetch_array($query)) {
            ...
            if (substr($multisetting['optionscode'], 0, 8) == 'checkbox') {
                ...
                if (
                    empty($mybb->input['upsetting'][$multisetting['name']]) &&
                    isset($mybb->input["isvisible_{$multisetting['name']}"])
                ) {
                ...
                if (isset($mybb->input['upsetting'][$multisetting['name']])) {
                    ...
                    for ($i = 1; $i < count($type); $i++) {
                        ...
                        if (in_array($opt[0], ['min', 'max', 'step'])) {
        ...
        if (
            isset($mybb->input['upsetting']['adminemail']) &&
            !validate_email_format($mybb->input['upsetting']['adminemail'])
        ) {
            unset($mybb->input['upsetting']['adminemail']);
            $lang->success_settings_updated .=
                $lang->error_admin_email_settings_empty;
        }

        // Administrator is changing the login method.
        if (
            isset($mybb->input['upsetting']['username_method']) &&
            (int) $mybb->input['upsetting']['username_method'] > 0
        ) {
            if ((int) $mybb->settings['allowmultipleemails'] == 1) {
                $mybb->input['upsetting']['username_method'] = 0;
                $lang->success_settings_updated .=
                    $lang->success_settings_updated_username_method_conflict;
            } else {
                $query = $db->simple_select('users', 'email', "email != ''", [
                    'group_by' => 'email HAVING COUNT(email)>1',
                ]);
                if ($db->num_rows($query)) {
                    $mybb->input['upsetting']['username_method'] = 0;
                    $lang->success_settings_updated .=
                        $lang->success_settings_updated_username_method;
                }
            }
        }
        ...
        if (
            isset(
                $mybb->input['upsetting']['username_method'],
                $mybb->input['upsetting']['allowmultipleemails']
            )
        ) {
            // Administrator is changing registration email allowance
            if (
                (int) $mybb->settings['username_method'] > 0 &&
                (int) $mybb->input['upsetting']['allowmultipleemails'] !== 0
            ) {
                $mybb->input['upsetting']['allowmultipleemails'] = 0;
                $lang->success_settings_updated .=
                    $lang->success_settings_updated_allowmultipleemails;
            }

            // Reset conflict silently, if by chance
            if (
                (int) $mybb->settings['username_method'] > 0 &&
                (int) $mybb->settings['allowmultipleemails'] == 1
            ) {
                $mybb->input['upsetting']['allowmultipleemails'] = 0;
            }
        }
        ...
        $fields = ['uploadspath', 'cdnpath', 'avataruploadpath'];
        ...
        $dynamic_include_directories_realpath = array_map(
            'realpath',
            $dynamic_include_directories
        );
        ...
        foreach ($fields as $field) {
            if (isset($mybb->input['upsetting'][$field])) {
                ...
                $realpath = realpath(
                    mk_path_abs($mybb->input['upsetting'][$field])
                );
                ...
                foreach (
                    $dynamic_include_directories_realpath
                    as $forbidden_realpath
                ) {
                    if (
                        $realpath === $forbidden_realpath ||
                        strpos(
                            $realpath,
                            $forbidden_realpath . DIRECTORY_SEPARATOR
                        ) === 0
                    ) {
                        unset($mybb->input['upsetting'][$field]);
                        continue 2;
        ...
        if (is_array($mybb->input['upsetting'])) {
            foreach ($mybb->input['upsetting'] as $name => $value) {
                if (
                    $forum_group_select &&
                    in_array($name, $forum_group_select)
                ) {
                    if ($value == 'all') {
                        $value = -1;
                    } elseif ($value == 'custom') {
                        if (
                            isset($mybb->input['select'][$name]) &&
                            is_array($mybb->input['select'][$name])
                        ) {
                    ...
                    $checkbox_settings &&
                    in_array($name, $checkbox_settings)
                ) {
                    $value = '';
        ...
        if (
            ...
            $mybb->settings['searchtype'] !=
                $mybb->input['upsetting']['searchtype'] &&
            $mybb->input['upsetting']['searchtype'] == 'fulltext'
        ) {
            if (
                !$db->is_fulltext('posts') &&
                $db->supports_fulltext_boolean('posts')
            ) {
                $db->create_fulltext_index('posts', 'message');
            }
            if (
                !$db->is_fulltext('threads') &&
                $db->supports_fulltext('threads')
            ) {
        ...
        if (
            isset($mybb->input['upsetting']['delayedthreadviews']) &&
            $mybb->settings['delayedthreadviews'] !=
                $mybb->input['upsetting']['delayedthreadviews']
        ) {
            $db->update_query(
                'tasks',
                [
                    'enabled' =>
                        (int) $mybb->input['upsetting']['delayedthreadviews'],
                ],
                "file='threadviews'"
            );
        }
        ...
        if (
            isset($mybb->input['upsetting']['cookieprefix']) &&
            $mybb->input['upsetting']['cookieprefix'] !=
        ...
        if (
            isset($mybb->input['upsetting']['statstopreferrer']) &&
            $mybb->input['upsetting']['statstopreferrer'] !=
                $mybb->settings['statstopreferrer']
        ) {
        ...
        if (
            isset($mybb->input['upsetting']['statslimit']) &&
            $mybb->input['upsetting']['statslimit'] != $statslimit
        ) {
    ...
    if (isset($mybb->input['search'])) {
        ...
        if (!empty($search)) {
            $query = $db->query(
				...
				LEFT JOIN " .
                    TABLE_PREFIX .
                    "settinggroups g ON(s.gid=g.gid)
            ...
            while ($setting = $db->fetch_array($query)) {
                $search_in =
                    ...
                    $setting['gtitle'] .
                    ' ' .
                    $setting['gdescription'];
                foreach (
                    [
                        "setting_{$setting['name']}",
                        "setting_{$setting['name']}_desc",
                        "setting_group_{$setting['gname']}",
                        "setting_group_{$setting['gname']}_desc",
                    ]
                    as $search_in_lang_key
                ) {
        ...
        if (!count($cache_settings)) {
            if (isset($mybb->input['ajax_search'])) {
                echo json_encode([
                    'errors' => [$lang->error_no_settings_found],
                ]);
                exit();
            } else {
        ...
        if (!isset($mybb->input['ajax_search'])) {
            ...
            $page->output_header(
                $lang->board_settings . " - {$lang->settings_search}"
            );
        }
        ...
        $page->output_header(
            $lang->board_settings . " - {$groupinfo['title']}"
        );
    } else {
        // All settings list
        // Cache groups
        ...
        $page->output_header(
            $lang->board_settings . " - {$lang->show_all_settings}"
        );
    }
    ...
    foreach ($cache_groups as $groupinfo) {
        $form = new Form(
            'index.php?module=config-settings&amp;action=change',
            'post',
            'change'
        );
        ...
        foreach ($cache_settings[$groupinfo['gid']] as $setting) {
            ...
            if ($type[0] == 'text' || $type[0] == '') {
                $setting_code = $form->generate_text_box(
                    $element_name,
                    $setting['value'],
                    ['id' => $element_id]
                );
            } elseif ($type[0] == 'numeric') {
                $field_options = ['id' => $element_id];
                if (count($type) > 1) {
                    for ($i = 1; $i < count($type); $i++) {
                        $optionsexp = explode('=', $type[$i]);
                        $opt = array_map('trim', $optionsexp);
                ...
                $prefix_checked = ['all' => '', 'custom' => '', 'none' => ''];
                if ($setting['value'] == -1) {
                ...
                if ($type[0] == 'select') {
                    ...
                    if ($type[0] == 'checkbox') {
                        $setting_code .= $form->generate_hidden_field(
                            "isvisible_{$setting['name']}",
                            1
                        );
                    }
                }
...
EOF;
    ...
    echo $search->generate_text_box('search', $lang->settings_search, [
        'id' => 'search',
        'class' => 'search_default field150 field_small',
    ]);
    ...
    switch ($db->type) {
        case 'pgsql':
			...
			LEFT JOIN " .
                    TABLE_PREFIX .
                    "settings s ON (s.gid=g.gid)
			WHERE g.isdefault = 1
			GROUP BY " .
                    $db->build_fields_string('settinggroups', 'g.') .
                    "
    ...
    while ($group = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<strong><a href=\"index.php?module=config-settings&amp;action=change&amp;gid={$group['gid']}\">{$group_title}</a></strong> ({$group['settingcount']} {$lang->bbsettings})<br /><small>{$group_desc}</small>"
        );
        $table->construct_row();
    }

    $table->output(
        "<span style=\"float: right;\"><small><a href=\"index.php?module=config-settings&amp;action=change\">{$lang->show_all_settings}</a></small></span>{$lang->board_settings}"
    );
...
$(function(){
	SettingSearch.init("' .
        $lang->settings_search .
        '","' .
        $lang->error_ajax_unknown .
        '");
});
//-->
</script>';
