...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!trim($mybb->input['disporder'])) {
            ...
            $mybb->input['disporder'] = $mybb->get_input(
                'disporder',
                MyBB::INPUT_INT
            );
    ...
    $form = new Form(
        'index.php?module=config-smilies&amp;action=add',
        'post',
        'add'
    );
    ...
    if (!$mybb->get_input('disporder')) {
        $query = $db->simple_select(
            'smilies',
            'max(disporder) as dispordermax'
        );
        $mybb->input['disporder'] =
            $db->fetch_field($query, 'dispordermax') + 1;
    }
    ...
    $form_container->output_row(
        $lang->text_replace . ' <em>*</em>',
        $lang->text_replace_desc,
    ...
    $form_container->output_row(
        $lang->image_path . ' <em>*</em>',
        $lang->image_path_desc,
    ...
    $form_container->output_row(
        $lang->show_clickable . ' <em>*</em>',
        $lang->show_clickable_desc,
        $form->generate_yes_no_radio(
            'showclickable',
            $mybb->get_input('showclickable')
        )
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        'index.php?module=config-smilies&amp;action=edit',
        'post',
        'edit'
    );
    ...
    $form_container->output_row(
        $lang->text_replace . ' <em>*</em>',
        $lang->text_replace_desc,
        $form->generate_text_area('find', $mybb->input['find'], [
            'id' => 'find',
        ]),
        'find'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-smilies&amp;action=delete&amp;sid={$smilie['sid']}",
            $lang->confirm_smilie_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'add_multiple') {
    ...
    if ($mybb->request_method == 'post') {
        if ($mybb->input['step'] == 1) {
            ...
            if (!$errors) {
                ...
                $page->output_header(
                    $lang->smilies . ' - ' . $lang->add_multiple_smilies
                );
                ...
                $sub_tabs['add_multiple_smilies'] = [
                    'title' => $lang->add_multiple_smilies,
                    'link' =>
                        'index.php?module=config-smilies&amp;action=add_multiple',
                    'description' => $lang->add_multiple_smilies_desc,
                ];
                $sub_tabs['mass_edit'] = [
                    'title' => $lang->mass_edit,
                    'link' =>
                        'index.php?module=config-smilies&amp;action=mass_edit',
                ];
                ...
                $form_container = new FormContainer(
                    $lang->add_multiple_smilies
                );
                $form_container->output_row_header($lang->image, [
                    'class' => 'align_center',
                    'width' => '10%',
                ]);
                $form_container->output_row_header($lang->name);
                $form_container->output_row_header($lang->text_replace, [
                    'width' => '20%',
                ]);
                $form_container->output_row_header($lang->include, [
                    'class' => 'align_center',
                    'width' => '5%',
                ]);
                ...
                foreach ($smilies as $key => $file) {
                    ...
                    $form_container->output_cell(
                        "<img src=\"../" .
                            ...
                            $file .
                            "\" alt=\"\" /><br /><small>{$file}</small>",
                        ['class' => 'align_center', 'width' => 1]
                    );
                    $form_container->output_cell(
                    ...
                    $form_container->output_cell(
                        $form->generate_text_box(
                            "find[{$file}]",
                            ':' . $find . ':',
                            ['id' => 'find', 'style' => 'width: 95%']
                        )
                    );
                    $form_container->output_cell(
                        $form->generate_check_box("include[{$file}]", 1, '', [
                            'checked' => 1,
                        ]),
                        ['class' => 'align_center']
                    );
            ...
            $query = $db->simple_select(
                'smilies',
                'MAX(disporder) as max_disporder'
            );
    ...
    $form_container->output_row(
        $lang->path_to_images,
        $lang->path_to_images_desc,
        $form->generate_text_box('pathfolder', $mybb->get_input('pathfolder'), [
            'id' => 'pathfolder',
        ]),
        'pathfolder'
    );
...
if ($mybb->input['action'] == 'mass_edit') {
    ...
    if ($mybb->request_method == 'post') {
        foreach ($mybb->input['name'] as $sid => $name) {
            ...
            if (!empty($mybb->input['delete'][$sid])) {
                ...
                foreach (
                    array_merge(
                        array_keys($mybb->input['find'][$sid], ''),
                        array_keys($mybb->input['find'][$sid], ' ')
                    )
                    as $key
                ) {
                ...
                $smilie = [
                    ...
                    'showclickable' => $db->escape_string(
                        $mybb->input['showclickable'][$sid]
                    ),
                ];
    ...
    $form_container->output_row_header($lang->name);
    $form_container->output_row_header($lang->text_replace, ['width' => '20%']);
    $form_container->output_row_header($lang->order, ['width' => '5%']);
    $form_container->output_row_header($lang->mass_edit_show_clickable, [
        'width' => 165,
    ]);
    $form_container->output_row_header($lang->smilie_delete, [
        'class' => 'align_center',
        'width' => '5%',
    ]);
    ...
    while ($smilie = $db->fetch_array($query)) {
        ...
        $form_container->output_cell("<img src=\"{$image}\" alt=\"\" />", [
            'class' => 'align_center',
            'width' => 1,
        ]);
        $form_container->output_cell(
        ...
        $form_container->output_cell(
            $form->generate_numeric_field(
                "disporder[{$smilie['sid']}]",
                $smilie['disporder'],
                ['id' => 'disporder', 'style' => 'width: 80%', 'min' => 0]
            )
        );
        $form_container->output_cell(
            $form->generate_yes_no_radio(
                "showclickable[{$smilie['sid']}]",
    ...
    if ($form_container->num_rows() == 0) {
        $form_container->output_cell($lang->no_smilies, ['colspan' => 6]);
        $form_container->construct_row();
    }
...
if (!$mybb->input['action']) {
    ...
    $table = new Table();
    $table->construct_header($lang->image, [
        'class' => 'align_center',
        'width' => 1,
    ]);
    $table->construct_header($lang->name, ['width' => '35%']);
    $table->construct_header($lang->text_replace, ['width' => '35%']);
    ...
    while ($smilie = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<img src=\"{$image}\" alt=\"\" class=\"smilie smilie_{$smilie['sid']}\" />",
            ['class' => 'align_center']
        );
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=config-smilies&amp;action=edit&amp;sid={$smilie['sid']}\">{$lang->edit}</a>",
            ['class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-smilies&amp;action=delete&amp;sid={$smilie['sid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_smilie_deletion}')\">{$lang->delete}</a>",
            ['class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_smilies, ['colspan' => 5]);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            'index.php?module=config-smilies&amp;page={page}'
        );
