...
if ($mybb->input['action'] == 'add') {
    ...
    $form_container->output_row(
        $lang->user_agent . ' <em>*</em>',
        $lang->user_agent_desc,
        $form->generate_text_box('useragent', $mybb->get_input('useragent'), [
            'id' => 'useragent',
        ]),
        'useragent'
    );

    $languages = ['' => $lang->use_board_default];
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-spiders&action=delete&sid={$spider['sid']}",
            $lang->confirm_bot_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        "index.php?module=config-spiders&amp;action=edit&amp;sid={$spider['sid']}",
        'post'
    );
    ...
    $form_container->output_row(
        $lang->name . ' <em>*</em>',
        $lang->name_desc,
        $form->generate_text_box('name', $spider_data['name'], [
            'id' => 'name',
        ]),
        'name'
    );
    $form_container->output_row(
        $lang->user_agent . ' <em>*</em>',
        $lang->user_agent_desc,
        $form->generate_text_box('useragent', $spider_data['useragent'], [
            'id' => 'useragent',
        ]),
        'useragent'
    );
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->last_visit, [
        'class' => 'align_center',
        'width' => 200,
    ]);
    ...
    $query = $db->simple_select('spiders', '*', '', [
        'limit_start' => $start,
        'limit' => 20,
        'order_by' => 'lastvisit',
        'order_dir' => 'desc',
    ...
    while ($spider = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=config-spiders&amp;action=edit&amp;sid={$spider['sid']}\"><strong>{$spider['name']}</strong></a>"
        );
        $table->construct_cell($lastvisit, [
            'class' => 'align_center',
            'width' => 200,
        ]);
        $table->construct_cell(
            "<a href=\"index.php?module=config-spiders&amp;action=edit&amp;sid={$spider['sid']}\">{$lang->edit}</a>",
            ['class' => 'align_center', 'width' => 75]
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-spiders&amp;action=delete&amp;sid={$spider['sid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_bot_deletion}');\">{$lang->delete}</a>",
            ['class' => 'align_center', 'width' => 75]
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_bots, ['colspan' => 4]);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            'index.php?module=config-spiders&amp;page={page}'
        );
