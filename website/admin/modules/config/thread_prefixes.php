...
$page->add_breadcrumb_item(
    $lang->thread_prefixes,
    'index.php?module=config-thread_prefixes'
);
...
$sub_tabs = [
    ...
    'add_prefix' => [
        ...
        'link' =>
            'index.php?module=config-thread_prefixes&amp;action=add_prefix',
...
if ($mybb->input['action'] == 'add_prefix') {
    ...
    $page->output_header(
        $lang->thread_prefixes . ' - ' . $lang->add_new_thread_prefix
    );
    ...
    $form_container->output_row(
        $lang->prefix . ' <em>*</em>',
        $lang->prefix_desc,
        $form->generate_text_box('prefix', $mybb->input['prefix'], [
            'id' => 'prefix',
        ]),
        'prefix'
    );
    $form_container->output_row(
        $lang->display_style . ' <em>*</em>',
        $lang->display_style_desc,
        $form->generate_text_box('displaystyle', $mybb->input['displaystyle'], [
            'id' => 'displaystyle',
        ]),
        'displaystyle'
    );
...
if ($mybb->input['action'] == 'edit_prefix') {
    ...
    $page->output_header(
        $lang->thread_prefixes . ' - ' . $lang->edit_thread_prefix
    );
    ...
    $form = new Form(
        'index.php?module=config-thread_prefixes&amp;action=edit_prefix',
        'post'
    );
...
if ($mybb->input['action'] == 'delete_prefix') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-thread_prefixes&amp;action=delete_prefix&amp;pid={$prefix['pid']}",
            $lang->confirm_thread_prefix_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
        ...
        if ($fid) {
            ...
            if (!empty($forum)) {
                ...
                foreach ($prefixes as $key => $prefix) {
                    if (
                        $prefix['forums'] !== '-1' &&
                        !in_array($fid, $prefix['forum_fids'])
                    ) {
                        unset($prefixes[$key]);
                    }
                }
        ...
        foreach ($prefixes as $prefix) {
            if ($prefix['forums'] === '-1') {
                ...
                foreach ($prefix['forum_fids'] as $fid) {
                    ...
                    if (!empty($forum)) {
                        $forum_names[] =
                            '<a href="index.php?module=config-thread_prefixes&amp;fid=' .
                            (int) $fid .
                            '">' .
                            $forum['name'] .
                            '</a>';
            ...
            $table->construct_cell(
                "<a href=\"index.php?module=config-thread_prefixes&amp;action=edit_prefix&amp;pid={$prefix['pid']}\" style=\"color: inherit;\" title=\"" .
            ...
            $table->construct_cell(
                "<a href=\"index.php?module=config-thread_prefixes&amp;action=edit_prefix&amp;pid={$prefix['pid']}\">{$lang->edit}</a>",
                ['width' => 100, 'class' => 'align_center']
            );
            $table->construct_cell(
                "<a href=\"index.php?module=config-thread_prefixes&amp;action=delete_prefix&amp;pid={$prefix['pid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_thread_prefix_deletion}')\">{$lang->delete}</a>",
                ['width' => 100, 'class' => 'align_center']
            );
            $table->construct_row();
        }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_thread_prefixes, ['colspan' => 3]);
        $table->construct_row();
    }
...
function thread_prefix_sort($a, $b)
{
    ...
    elseif (
        count($a['forum_fids']) === 1 &&
        count($b['forum_fids']) === 1 &&
