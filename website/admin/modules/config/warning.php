...
if (
    $mybb->input['action'] == 'levels' ||
    $mybb->input['action'] == 'add_type' ||
    $mybb->input['action'] == 'add_level' ||
    !$mybb->input['action']
) {
...
if ($mybb->input['action'] == 'add_level') {
    ...
    if ($mybb->request_method == 'post') {
        if (
            !is_numeric($mybb->input['percentage']) ||
            $mybb->input['percentage'] > 100 ||
            $mybb->input['percentage'] < 0
        ) {
        ...
        if (!$errors) {
            ...
            if ($mybb->input['action_type'] == 1) {
                $action = [
                    ...
                    'length' => fetch_time_length(
                        $mybb->input['action_1_time'],
                        $mybb->input['action_1_period']
                    ),
                ];
            }
            // Suspend posting
            elseif ($mybb->input['action_type'] == 2) {
                $action = [
                    'type' => 2,
                    'length' => fetch_time_length(
                        $mybb->input['action_2_time'],
                        $mybb->input['action_2_period']
                    ),
                ];
            }
            // Moderate posts
            elseif ($mybb->input['action_type'] == 3) {
                $action = [
                    'type' => 3,
                    'length' => fetch_time_length(
                        $mybb->input['action_3_time'],
                        $mybb->input['action_3_period']
                    ),
                ];
            }
            $new_level = [
    ...
    $page->output_header(
        $lang->warning_levels . ' - ' . $lang->add_warning_level
    );

    $page->output_nav_tabs($sub_tabs, 'add_level');
    $form = new Form(
        'index.php?module=config-warning&amp;action=add_level',
        'post'
    );
    ...
    $form_container->output_row(
        $lang->warning_points_percentage,
        $lang->warning_points_percentage_desc,
        $form->generate_numeric_field(
            'percentage',
            $mybb->get_input('percentage'),
            ['id' => 'percentage', 'min' => 0, 'max' => 100]
        ),
        'percentage'
    );
	...
	<dl style=\"margin-top: 0; margin-bottom: 0; width: 100%;\">
        ...
        $form->generate_select_box(
            'action_1_usergroup',
            $banned_groups,
            $mybb->get_input('action_1_usergroup')
        ) .
        "</td>
				</tr>
				<tr>
        ...
        $form->generate_select_box(
            'action_1_period',
            $periods,
            $mybb->get_input('action_1_period')
        ) .
        "</td>
				</tr>
			</table>
        ...
        $form->generate_select_box(
            'action_2_period',
            $periods,
            $mybb->get_input('action_2_period')
        ) .
        "</td>
				</tr>
			</table>
        ...
        $form->generate_select_box(
            'action_3_period',
            $periods,
            $mybb->get_input('action_3_period')
        ) .
        "</td>
				</tr>
			</table>
    ...
    $form_container->output_row(
        $lang->action_to_be_taken,
        $lang->action_to_be_taken_desc,
        $actions
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'edit_level') {
    ...
    $page->output_header(
        $lang->warning_levels . ' - ' . $lang->edit_warning_level
    );
    ...
    $form = new Form(
        "index.php?module=config-warning&amp;action=edit_level&amp;lid={$level['lid']}",
        'post'
    );
    ...
    if ($errors) {
        ...
        if ($action['type'] == 1) {
            ...
            $mybb->input['action_1_time'] = isset($length['time'])
                ? $length['time']
                : null;
            $mybb->input['action_1_period'] = $length['period'];
        } elseif ($action['type'] == 2) {
            $length = fetch_friendly_expiration($action['length']);
            $mybb->input['action_2_time'] = isset($length['time'])
                ? $length['time']
                : null;
            $mybb->input['action_2_period'] = $length['period'];
        } elseif ($action['type'] == 3) {
            $length = fetch_friendly_expiration($action['length']);
            $mybb->input['action_3_time'] = isset($length['time'])
                ? $length['time']
                : null;
...
if ($mybb->input['action'] == 'delete_level') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-warning&amp;action=delete_level&amp;lid={$level['lid']}",
            $lang->confirm_warning_level_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'add_type') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            !is_numeric($mybb->input['points']) ||
            $mybb->input['points'] > $mybb->settings['maxwarningpoints'] ||
            $mybb->input['points'] <= 0
        ) {
            $errors[] = $lang->sprintf(
                $lang->error_missing_type_points,
        ...
        if (!$errors) {
            $new_type = [
                ...
                'expirationtime' => fetch_time_length(
                    $mybb->input['expire_time'],
                    $mybb->input['expire_period']
                ),
            ];
    ...
    $page->output_header(
        $lang->warning_types . ' - ' . $lang->add_warning_type
    );

    $page->output_nav_tabs($sub_tabs, 'add_type');
    $form = new Form(
        'index.php?module=config-warning&amp;action=add_type',
        'post'
    );
    ...
    $form_container->output_row(
        $lang->points_to_add . ' <em>*</em>',
        $lang->points_to_add_desc,
        $form->generate_numeric_field('points', $mybb->get_input('points'), [
            'id' => 'points',
            'min' => 0,
            'max' => $mybb->settings['maxwarningpoints'],
        ]),
        'points'
    );
    $expiration_periods = [
    ...
    $form_container->output_row(
        $lang->warning_expiry,
        $lang->warning_expiry_desc,
        $form->generate_numeric_field(
            'expire_time',
            $mybb->input['expire_time'],
            ['id' => 'expire_time', 'min' => 0]
        ) .
            ' ' .
            $form->generate_select_box(
                'expire_period',
...
if ($mybb->input['action'] == 'edit_type') {
    ...
    $page->output_header(
        $lang->warning_types . ' - ' . $lang->edit_warning_type
    );
    ...
    $form = new Form(
        "index.php?module=config-warning&amp;action=edit_type&amp;tid={$type['tid']}",
        'post'
    );
    ...
    $form_container->output_row(
        $lang->points_to_add . ' <em>*</em>',
        $lang->points_to_add_desc,
        $form->generate_numeric_field('points', $mybb->input['points'], [
            'id' => 'points',
            'min' => 0,
            'max' => $mybb->settings['maxwarningpoints'],
        ]),
...
if ($mybb->input['action'] == 'delete_type') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=config-warning&amp;action=delete_type&amp;tid={$type['tid']}",
            $lang->confirm_warning_type_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'levels') {
    ...
    $table = new Table();
    $table->construct_header($lang->percentage, [
        'width' => '5%',
        'class' => 'align_center',
    ]);
    ...
    while ($level = $db->fetch_array($query)) {
        $table->construct_cell("<strong>{$level['percentage']}%</strong>", [
            'class' => 'align_center',
        ]);
        $action = my_unserialize($level['action']);
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=config-warning&amp;action=edit_level&amp;lid={$level['lid']}\">{$lang->edit}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-warning&amp;action=delete_level&amp;lid={$level['lid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_warning_level_deletion}')\">{$lang->delete}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_warning_levels, ['colspan' => 4]);
        $table->construct_row();
    }
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->points, [
        'width' => '5%',
        'class' => 'align_center',
    ]);
    $table->construct_header($lang->expires_after, [
        'width' => '25%',
        'class' => 'align_center',
    ]);
    $table->construct_header($lang->controls, [
    ...
    $query = $db->simple_select('warningtypes', '*', '', [
        'order_by' => 'title',
    ]);
    while ($type = $db->fetch_array($query)) {
        $type['name'] = htmlspecialchars_uni($type['title']);
        $table->construct_cell(
            "<a href=\"index.php?module=config-warning&amp;action=edit_type&amp;tid={$type['tid']}\"><strong>{$type['name']}</strong></a>"
        );
        $table->construct_cell("{$type['points']}", [
            'class' => 'align_center',
        ]);
        $expiration = fetch_friendly_expiration($type['expirationtime']);
        $lang_str = 'expiration_' . $expiration['period'];
        if ($type['expirationtime'] > 0) {
            $table->construct_cell("{$expiration['time']} {$lang->$lang_str}", [
                'class' => 'align_center',
            ]);
        } else {
            $table->construct_cell($lang->never, ['class' => 'align_center']);
        }
        $table->construct_cell(
            "<a href=\"index.php?module=config-warning&amp;action=edit_type&amp;tid={$type['tid']}\">{$lang->edit}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=config-warning&amp;action=delete_type&amp;tid={$type['tid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_warning_type_deletion}')\">{$lang->delete}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_warning_types, ['colspan' => 5]);
        $table->construct_row();
    }
