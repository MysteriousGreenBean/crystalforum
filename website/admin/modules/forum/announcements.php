...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            if (isset($mybb->input['preview'])) {
                ...
                $parser_options['allow_mycode'] = $mybb->get_input(
                    'allowmycode',
                    MyBB::INPUT_INT
                );
                $parser_options['allow_smilies'] = $mybb->get_input(
                    'allowsmilies',
                    MyBB::INPUT_INT
                );
                $parser_options['allow_imgcode'] = 1;
                $parser_options['allow_videocode'] = 1;
                $parser_options['me_username'] = htmlspecialchars_uni(
                    $mybb->user['username']
                );
                ...
                $preview['message'] = $parser->parse_message(
                    $mybb->input['message'],
                    $parser_options
                );
                $preview['subject'] = htmlspecialchars_uni(
                    $mybb->input['title']
                );
            } else {
                $months = [
                ...
                $insert_announcement = [
                    ...
                    'enddate' => $enddate,
                    'allowhtml' =>
                        (int) ($mybb->settings['announcementshtml'] &&
                            $mybb->get_input('allowhtml', MyBB::INPUT_INT)),
    ...
    $form = new Form(
        'index.php?module=forum-announcements&amp;action=add',
        'post'
    );
    if ($errors) {
    ...
    if (!$mybb->input['starttime_time']) {
        $mybb->input['starttime_time'] = gmdate(
    ...
    if (!$mybb->input['endtime_time']) {
        $mybb->input['endtime_time'] = gmdate(
            $mybb->settings['timeformat'],
            TIME_NOW
        );
    }
    ...
    $form_container->output_row(
        $lang->start_date . ' <em>*</em>',
        $lang->start_date_desc,
        "<select name=\"starttime_day\">\n{$startdateday}</select>\n &nbsp; \n<select name=\"starttime_month\">\n{$startdatemonth}</select>\n &nbsp; \n<input type=\"text\" name=\"starttime_year\" value=\"{$startdateyear}\" size=\"4\" maxlength=\"4\" />\n - {$lang->time} " .
            $form->generate_text_box(
                'starttime_time',
                $mybb->input['starttime_time'],
                ['id' => 'starttime_time', 'style' => 'width: 50px;']
            )
    );
	...
	<dt><label style=\"display: block;\"><input type=\"radio\" name=\"endtime_type\" value=\"1\" {$endtime_checked[1]} class=\"endtimes_check\" onclick=\"checkAction('endtime');\" style=\"vertical-align: middle;\" /> <strong>{$lang->set_time}</strong></label></dt>
        ...
        $form->generate_text_box('endtime_time', $mybb->input['endtime_time'], [
            'id' => 'endtime_time',
            'style' => 'width: 50px;',
        ]) .
        "</td>
    ...
    $form_container->output_row(
        $lang->message . ' <em>*</em>',
        '',
        $form->generate_text_area('message', $mybb->input['message'], [
            'id' => 'message',
        ]),
        'message'
    );
    ...
    $form_container->output_row(
        $lang->forums_to_appear_in . ' <em>*</em>',
        $lang->forums_to_appear_in_desc,
        $form->generate_forum_select('fid', $mybb->input['fid'], [
            'size' => 5,
            'main_option' => $lang->all_forums,
        ])
    );

    if ($mybb->settings['announcementshtml']) {
        $form_container->output_row(
            $lang->allow_html . ' <em>*</em>',
            '',
            $form->generate_yes_no_radio(
                'allowhtml',
                $mybb->input['allowhtml'],
                ['style' => 'width: 2em;']
            )
        );
    }
    ...
    $form_container->output_row(
        $lang->allow_mycode . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'allowmycode',
            $mybb->input['allowmycode'],
            ['style' => 'width: 2em;']
        )
    );

    $form_container->output_row(
        $lang->allow_smilies . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio(
            'allowsmilies',
    ...
    $buttons[] = $form->generate_submit_button($lang->preview_announcement, [
        'name' => 'preview',
    ]);
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form = new Form(
        'index.php?module=forum-announcements&amp;action=edit',
        'post'
    );
    ...
    if ($errors || isset($mybb->input['preview'])) {
        ...
        $mybb->input['allowhtml'] =
            $mybb->settings['announcementshtml'] && $announcement['allowhtml'];
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=forum-announcements&amp;action=delete&amp;aid={$announcement['aid']}",
            $lang->confirm_announcement_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    if (!empty($global_announcements)) {
        ...
        foreach ($global_announcements as $aid => $announcement) {
            ...
            $table->construct_cell(
                $icon .
                    "<a href=\"index.php?module=forum-announcements&amp;action=edit&amp;aid={$aid}\">" .
                    htmlspecialchars_uni($announcement['subject']) .
                    '</a>'
            );
            $table->construct_cell(
                "<a href=\"index.php?module=forum-announcements&amp;action=edit&amp;aid={$aid}\">{$lang->edit}</a>",
                ['class' => 'align_center', 'width' => 75]
            );
            $table->construct_cell(
                "<a href=\"index.php?module=forum-announcements&amp;action=delete&amp;aid={$aid}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_announcement_deletion}')\">{$lang->delete}</a>",
                ['class' => 'align_center', 'width' => 75]
            );
            $table->construct_row();
        }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_forums, ['colspan' => '3']);
        $table->construct_row();
    }
...
function fetch_forum_announcements(&$table, $pid = 0, $depth = 1)
{
    ...
    foreach ($forums_by_parent[$pid] as $children) {
        foreach ($children as $forum) {
            ...
            $table->construct_cell(
                "<a href=\"index.php?module=forum-announcements&amp;action=add&amp;fid={$forum['fid']}\">{$lang->add_announcement}</a>",
            ...
            if (isset($announcements[$forum['fid']])) {
                foreach (
                    ...
                    $table->construct_cell(
                        "<div style=\"padding-left: " .
                            40 * $depth .
                            "px;\">{$icon}<a href=\"index.php?module=forum-announcements&amp;action=edit&amp;aid={$aid}\">" .
                            htmlspecialchars_uni($announcement['subject']) .
                            '</a></div>'
                    );
                    $table->construct_cell(
