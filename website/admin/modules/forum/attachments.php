...
$page->add_breadcrumb_item(
    $lang->attachments,
    'index.php?module=forum-attachments'
);

if (
    $mybb->input['action'] == 'stats' ||
    $mybb->input['action'] == 'orphans' ||
    !$mybb->input['action']
) {
...
if ($mybb->input['action'] == 'delete') {
    ...
    if (isset($mybb->input['aids'])) {
        if (!is_array($mybb->input['aids'])) {
            $mybb->input['aids'] = [$mybb->get_input('aid', MyBB::INPUT_INT)];
        } else {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $query = $db->simple_select(
            'attachments',
            'aid,pid,posthash,filename',
            'aid IN (' . implode(',', $mybb->input['aids']) . ')'
        );
        ...
        $page->output_confirm_action(
            "index.php?module=forum-attachments&amp;action=delete&amp;aids={$aids}",
            $lang->confirm_delete
        );
    }
}
...
if ($mybb->input['action'] == 'stats') {
    ...
    $query = $db->simple_select(
        'attachments',
        'COUNT(*) AS total_attachments, SUM(filesize) as disk_usage, SUM(downloads*filesize) as bandwidthused',
        "visible='1'"
    );
    ...
    if ($attachment_stats['total_attachments'] == 0) {
        $page->output_inline_error([$lang->error_no_attachments]);
        $page->output_footer();
        exit();
    }
    ...
    $table->construct_cell($lang->num_uploaded, ['width' => '25%']);
    $table->construct_cell(
        my_number_format($attachment_stats['total_attachments']),
        ['width' => '25%']
    );
    $table->construct_cell($lang->space_used, ['width' => '200']);
    $table->construct_cell(get_friendly_size($attachment_stats['disk_usage']), [
        'width' => '200',
    ]);
    $table->construct_row();

    $table->construct_cell($lang->bandwidth_used, ['width' => '25%']);
    $table->construct_cell(
        get_friendly_size(round($attachment_stats['bandwidthused'])),
        ['width' => '25%']
    );
    $table->construct_cell($lang->average_size, ['width' => '25%']);
    $table->construct_cell(
        get_friendly_size(
            round(
                $attachment_stats['disk_usage'] /
    ...
    $table = new Table();
    $table->construct_header($lang->attachments, ['colspan' => 2]);
    $table->construct_header($lang->size, [
        'width' => '10%',
        'class' => 'align_center',
    ]);
    $table->construct_header($lang->posted_by, [
        'width' => '20%',
        'class' => 'align_center',
    ]);
    ...
    $table->construct_header($lang->downloads, [
        'width' => '10%',
        'class' => 'align_center',
    ]);
    $table->construct_header($lang->date_uploaded, ['class' => 'align_center']);
    ...
    $table->construct_header($lang->total_size, [
        'width' => '20%',
        'class' => 'align_center',
    ]);
    ...
    while ($user = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=forum-attachments&amp;results=1&amp;username=" .
                urlencode($user['username']) .
                "\" target=\"_blank\">" .
                get_friendly_size($user['totalsize']) .
                '</a>',
            ['class' => 'align_center']
        );
        $table->construct_row();
...
if (
        ...
        foreach ($mybb->input['orphaned_files'] as $file) {
            ...
            if (
                ...
                strpos(
                    ...
                    str_replace('\\', '/', realpath(MYBB_ROOT)) . '/'
                ) !== 0 ||
                $real_path == realpath(MYBB_ROOT . 'install/lock')
            ) {
                $error_count++;
                continue;
            }
    ...
    if (is_array($mybb->input['orphaned_attachments'])) {
        $mybb->input['orphaned_attachments'] = array_map(
            'intval',
            $mybb->input['orphaned_attachments']
        );
        require_once MYBB_ROOT . 'inc/functions_upload.php';
        ...
        $query = $db->simple_select(
            ...
            'aid,pid,posthash',
            'aid IN (' .
                implode(',', $mybb->input['orphaned_attachments']) .
                ')'
        );
        while ($attachment = $db->fetch_array($query)) {
            if (!$attachment['pid']) {
    ...
    if ($success_count > 0) {
        if ($error_count > 0) {
            $message .=
                '<br />' . $lang->sprintf($lang->success_count, $success_count);
        } else {
...
if ($mybb->input['action'] == 'orphans') {
    ...
    if ($mybb->input['step'] == 3) {
        ...
        if (!empty($mybb->input['missing_attachment_files'])) {
            $missing_attachment_files = my_unserialize(
                $mybb->input['missing_attachment_files']
            );
        ...
        $form = new Form(
            'index.php?module=forum-attachments&amp;action=delete_orphans',
            'post'
        );

        $table = new Table();
        $table->construct_header(
            $form->generate_check_box('allbox', '1', '', [
                'class' => 'checkall',
            ]),
            ['width' => 1]
        );
        $table->construct_header($lang->size_attachments, ['colspan' => 2]);
        $table->construct_header($lang->reason_orphaned, [
            'width' => '20%',
            'class' => 'align_center',
        ]);
        $table->construct_header($lang->date_uploaded, [
            'class' => 'align_center',
        ]);
        ...
        if (is_array($bad_attachments)) {
            foreach ($bad_attachments as $file) {
                ...
                if (file_exists($file_path)) {
                    ...
                    $table->construct_cell(
                        "<span class=\"float_right\">{$filesize}</span>{$filename}"
                    );
                    $table->construct_cell($lang->reason_not_in_table, [
                        'class' => 'align_center',
                    ]);
                    $table->construct_cell(
                        my_date('relative', filemtime($file_path)),
                        ['class' => 'align_center']
                    );
                    $table->construct_row();
                }
        ...
        if (count($aids) > 0) {
            ...
            while ($attachment = $db->fetch_array($query)) {
                ...
                $table->construct_cell(
                    get_attachment_icon(get_extension($attachment['filename'])),
                    ['width' => 1]
                );
                $table->construct_cell(
                    "<span class=\"float_right\">" .
                        get_friendly_size($attachment['filesize']) .
                        "</span>{$attachment['filename']}"
                );
                $table->construct_cell($reason, ['class' => 'align_center']);
                if ($attachment['dateuploaded']) {
                    $table->construct_cell(
                        my_date('relative', $attachment['dateuploaded']),
                        ['class' => 'align_center']
                    );
                } else {
                    $table->construct_cell($lang->unknown, [
                        'class' => 'align_center',
                    ]);
                }
                $table->construct_row();
        ...
        $table->output(
            "{$lang->orphan_attachments_search} - {$results} {$lang->results}"
        );

        $buttons[] = $form->generate_submit_button(
            $lang->button_delete_orphans
        );
    ...
    elseif ($mybb->input['step'] == 2) {
        ...
        $page->output_header(
            "{$lang->orphan_attachments_search} - {$lang->step2}"
        );
        ...
        while ($attachment = $db->fetch_array($query)) {
            ...
            if (
                ...
                $missing_attachment_files[$attachment['aid']] =
                    $attachment['aid'];
            }
            ...
            elseif (
                !$attachment['attachment_pid'] &&
                $attachment['dateuploaded'] < TIME_NOW - 60 * 60 * 24 &&
                $attachment['dateuploaded'] != 0
            ) {
                $incomplete_attachments[$attachment['aid']] =
                    $attachment['aid'];
            }
        }
        ...
        $form = new Form(
            'index.php?module=forum-attachments&amp;action=orphans&amp;step=3',
            'post',
            'redirect_form',
            0,
            ''
        ...
        if ($mybb->get_input('bad_attachments')) {
            echo $form->generate_hidden_field(
                'bad_attachments',
                $mybb->input['bad_attachments']
            );
        }
        if (
            is_array($missing_attachment_files) &&
            count($missing_attachment_files) > 0
        ) {
        ...
        if (
            is_array($incomplete_attachments) &&
            count($incomplete_attachments) > 0
        ) {
    ...
    // Running first step, scan the file system
            ...
            if ($dh = opendir($real_dir)) {
                while (false !== ($file = readdir($dh))) {
                    ...
                    if (is_dir($real_dir . '/' . $file)) {
                        ...
                        $attachments_to_check["$false_dir$file"] =
                            $false_dir . $file;
                        // In allotments of 20, query the database for these attachments
                        if (count($attachments_to_check) >= 20) {
                            $attachments_to_check = array_map(
                                [$db, 'escape_string'],
                                $attachments_to_check
                            );
                            $attachment_names =
                                "'" .
                                implode("','", $attachments_to_check) .
                                "'";
                            $query = $db->simple_select(
                                'attachments',
                                'aid, attachname',
                                "attachname IN ($attachment_names)"
                            );
                            while ($attachment = $db->fetch_array($query)) {
                                unset(
                                    $attachments_to_check[
        ...
        $page->output_header(
            "{$lang->orphan_attachments_search} - {$lang->step1}"
        );
        ...
        $form = new Form(
            'index.php?module=forum-attachments&amp;action=orphans&amp;step=2',
            'post',
            'redirect_form',
            0,
            ''
...
if (!$mybb->input['action']) {
    ...
    if (
        $mybb->request_method == 'post' ||
        $mybb->get_input('results', MyBB::INPUT_INT) == 1
    ) {
        $search_sql = '1=1';
        ...
        foreach ($user_like_fields as $search_field) {
            if ($mybb->get_input($search_field)) {
                $search_sql .=
                    " AND a.{$search_field} LIKE '%" .
                    $db->escape_string_like($mybb->input[$search_field]) .
                    "%'";
            }
        }
        ...
        if (!empty($mybb->input['username'])) {
            ...
            if (!$user) {
                if ($user_types == 1) {
                    ...
                    $search_sql .=
                        " AND p.username LIKE '%" .
                        $db->escape_string_like($mybb->input['username']) .
                        "%'";
                }
            } else {
        ...
        if (!empty($mybb->input['forum'])) {
            if (!is_array($mybb->input['forum'])) {
                $mybb->input['forum'] = [$mybb->input['forum']];
            }
        ...
        if (!empty($mybb->input['dateuploaded'])) {
            $direction_fields['dateuploaded'] =
                TIME_NOW - $direction_fields['dateuploaded'] * 60 * 60 * 24;
        }
        ...
        foreach ($direction_fields as $field_name => $field_content) {
            ...
            if (
                !empty($mybb->input[$field_name]) &&
                !empty($mybb->input[$direction_field])
            ) {
                ...
                $search_sql .=
                    " AND a.{$field_name}{$direction}'" . $field_content . "'";
            }
        }
        if (!$errors) {
        ...
        if (!$errors) {
            ...
            $form = new Form(
                'index.php?module=forum-attachments&amp;action=delete',
                'post'
            );
            ...
            if ($num_results > $perpage) {
                $pagination_url =
                    'index.php?module=forum-attachments&amp;results=1';
                $pagination_vars = [
                    'perpage',
                    'sortby',
                    'order',
                ...
                foreach ($pagination_vars as $var) {
                    if ($mybb->get_input($var)) {
                        $pagination_url .=
                            "&{$var}=" . urlencode($mybb->input[$var]);
                    }
                }
                if (
                    !empty($mybb->input['forum']) &&
                    is_array($mybb->input['forum'])
                ) {
                    foreach ($mybb->input['forum'] as $fid) {
            ...
            $buttons[] = $form->generate_submit_button(
                $lang->button_delete_attachments
            );
    ...
    $form_container->output_row(
        $lang->type_contains,
        '',
        $form->generate_text_box('mimetype', $mybb->get_input('mimetype'), [
            'id' => 'mimetype',
        ]),
        'mimetype'
    );
    $form_container->output_row(
        ...
        $form->generate_forum_select(
            'forum[]',
            $mybb->get_input('forum', MyBB::INPUT_INT),
            ['multiple' => true, 'size' => 5, 'id' => 'forum']
        ),
        'forum'
    );
    $form_container->output_row(
    ...
    $form_container->output_row(
        ...
        $form->generate_select_box(
            'user_types',
            [
                '0' => $lang->poster_is_either,
                '1' => $lang->poster_is_user,
                '-1' => $lang->poster_is_guest,
            ],
            $mybb->get_input('user_types', MyBB::INPUT_INT),
            ['id' => 'guests']
        ),
        'user_types'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_select_box(
            'dateuploaded_dir',
            $more_options,
            $mybb->get_input('dateuploaded_dir'),
            ['id' => 'dateuploaded_dir']
        ) .
            ' ' .
            $form->generate_numeric_field(
                'dateuploaded',
                $mybb->get_input('dateuploaded', MyBB::INPUT_INT),
                ['id' => 'dateuploaded', 'min' => 0]
            ) .
            " {$lang->days_ago}",
        'dateuploaded'
    );
    $form_container->output_row(
        ...
        $form->generate_select_box(
            'filesize_dir',
            $greater_options,
            $mybb->get_input('filesize_dir'),
            ['id' => 'filesize_dir']
        ) .
            ' ' .
            $form->generate_numeric_field(
                'filesize',
                $mybb->get_input('filesize', MyBB::INPUT_INT),
                ['id' => 'filesize', 'min' => 0]
            ) .
            " {$lang->kb}",
        'dateuploaded'
    ...
    $form_container->output_row(
        $lang->download_count_is,
        '',
        $form->generate_select_box(
            'downloads_dir',
            $greater_options,
            $mybb->get_input('downloads_dir'),
            ['id' => 'downloads_dir']
        ) .
            ' ' .
            $form->generate_numeric_field(
                'downloads',
                $mybb->get_input('downloads', MyBB::INPUT_INT),
                ['id' => 'downloads', 'min' => 0]
            ) .
            '',
        'dateuploaded'
    ...
    $form_container->output_row(
        ...
        '',
        $form->generate_numeric_field('perpage', $perpage, [
            'id' => 'perpage',
            'min' => 1,
        ]),
        'perpage'
...
function build_attachment_row($attachment, &$table, $use_form = false)
{
    ...
    if (
        !file_exists(
            mk_path_abs($mybb->settings['uploadspath']) .
                "/{$attachment['attachname']}"
        )
    ) {
    ...
    if ($use_form == true && is_object($form)) {
        $table->construct_cell(
            $form->generate_check_box('aids[]', $attachment['aid'], '', [
                'checked' => $checked,
            ])
        );
    }
    ...
    $table->construct_cell(get_friendly_size($attachment['filesize']), [
        'class' => $cell_class,
    ]);
    ...
    $table->construct_cell(
        build_profile_link(
            htmlspecialchars_uni($attachment['username']),
            $attachment['uid'],
            '_blank'
        ),
        ['class' => 'align_center']
    ...
    $table->construct_cell(
        "<a href=\"../" .
            get_post_link($attachment['pid']) .
            "\" target=\"_blank\">" .
            htmlspecialchars_uni($attachment['subject']) .
            '</a>',
        ['class' => 'align_center']
    );
    $table->construct_cell(my_number_format($attachment['downloads']), [
        'class' => 'align_center',
    ]);
