...
if ($mybb->request_method == 'post') {
    ...
    if ($mybb->get_input('threads', MyBB::INPUT_ARRAY)) {
        ...
        $query = $db->simple_select(
            ...
            'tid IN (' .
                implode(
                    ',',
                    array_map('intval', array_keys($mybb->input['threads']))
                ) .
                ')'
        );
    ...
    } elseif ($mybb->get_input('posts', MyBB::INPUT_ARRAY)) {
    ...
    } elseif ($mybb->get_input('attachments', MyBB::INPUT_ARRAY)) {
        $query = $db->simple_select(
            'attachments',
            'aid, pid',
            'aid IN (' .
                implode(
                    ',',
                    array_map('intval', array_keys($mybb->input['attachments']))
                ) .
                ')'
        );
        ...
        admin_redirect(
            'index.php?module=forum-moderation_queue&type=attachments'
        );
    }
}
...
if (empty($mybb->input['type']) || $mybb->input['type'] == 'threads') {
    ...
    if ($unapproved_threads > 0) {
        ...
        $pagination = draw_admin_pagination(
            ...
            $per_page,
            $unapproved_threads,
            'index.php?module=forum-moderation_queue&amp;page={page}'
        );
        ...
        $table->construct_header($lang->author, [
            'class' => 'align_center',
            'width' => '20%',
        ]);
        $table->construct_header($lang->posted, [
            'class' => 'align_center',
            'width' => '20%',
        ]);
        ...
        while ($thread = $db->fetch_array($query)) {
            ...
            $table->construct_cell(
                "<a href=\"../{$thread['threadlink']}\" target=\"_blank\">{$thread['subject']}</a>"
            );
            ...
            $controls .=
                $form->generate_radio_button(
                    ...
                    'ignore',
                    $lang->ignore,
                    ['class' => 'radio_ignore', 'checked' => true]
                ) . ' ';
            $controls .=
                $form->generate_radio_button(
                    "threads[{$thread['tid']}]",
                    'delete',
                    $lang->delete,
                    ['class' => 'radio_delete', 'checked' => false]
                ) . ' ';
            $controls .= $form->generate_radio_button(
                "threads[{$thread['tid']}]",
                'approve',
                $lang->approve,
                ['class' => 'radio_approve', 'checked' => false]
            );
            $controls .= '</div>';
            ...
            $table->construct_cell(
                "<div class=\"modqueue_message\">{$controls}<div class=\"modqueue_meta\">{$forum}</div>{$thread['postmessage']}</div>",
                ['colspan' => 3]
            );
            $table->construct_row();
        }
...
if ($mybb->get_input('type') == 'posts' || $mybb->get_input('type') == '') {
    ...
    if ($unapproved_posts > 0) {
        ...
        $pagination = draw_admin_pagination(
            ...
            $per_page,
            $unapproved_posts,
            'index.php?module=forum-moderation_queue&amp;type=posts&amp;page={page}'
        );
        ...
        while ($post = $db->fetch_array($query)) {
            ...
            $table->construct_cell(
                "<a href=\"../{$post['postlink']}#pid{$post['pid']}\" target=\"_blank\">{$post['subject']}</a>"
            );
            $table->construct_cell($profile_link, ['class' => 'align_center']);
            ...
            $table->construct_cell(
                "<div class=\"modqueue_message\">{$controls}<div class=\"modqueue_meta\">{$forum}{$thread}</div>{$post['message']}</div>",
                ['colspan' => 3]
            );
            $table->construct_row();
        }
        ...
        $page->output_footer();
    } elseif ($mybb->get_input('type') == 'posts') {
...
if (
    $mybb->get_input('type') == 'attachments' ||
    $mybb->get_input('type') == ''
) {
    ...
    if ($unapproved_attachments > 0) {
        ...
        $pagination = draw_admin_pagination(
            ...
            'index.php?module=forum-moderation_queue&amp;type=attachments&amp;page={page}'
        );
        ...
        $table->construct_header($lang->uploadedby, [
            'class' => 'align_center',
            'width' => '20%',
        ]);
        $table->construct_header($lang->posted, [
        ...
        while ($attachment = $db->fetch_array($query)) {
            ...
            $table->construct_cell(
                "<a href=\"../attachment.php?aid={$attachment['aid']}\" target=\"_blank\">{$attachment['filename']}</a> ({$attachment['filesize']})<br /><small class=\"modqueue_meta\">{$lang->post} <a href=\"{$link}\" target=\"_blank\">{$attachment['postsubject']}</a></small>"
            );
            $table->construct_cell($profile_link, ['class' => 'align_center']);
        ...
        $page->output_footer();
    } elseif ($mybb->get_input('type') == 'attachments') {
        $page->output_header($lang->moderation_queue);
        $page->output_nav_tabs($sub_tabs, 'attachments');
