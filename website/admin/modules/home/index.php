...
if ($mybb->input['action'] == 'version_check') {
    ...
    $page->add_breadcrumb_item(
        $lang->version_check,
        'index.php?module=home-version_check'
    );
    ...
    $latest_version =
        '<strong>' .
        htmlspecialchars_uni($tree['mybb']['latest_version']['value']) .
        '</strong> (' .
        $latest_code .
        ')';
    if ($latest_code > $mybb->version_code) {
        $latest_version =
            "<span style=\"color: #C00;\">" . $latest_version . '</span>';
        $updated_cache['latest_version'] = $latest_version;
        $updated_cache['latest_version_code'] = $latest_code;
        $page->output_error(
            "<p><em>{$lang->error_out_of_date}</em> {$lang->update_forum}</p>"
        );
    } else {
        $latest_version =
            "<span style=\"color: green;\">" . $latest_version . '</span>';
    ...
    $table->construct_cell(
        '<strong>' . $mybb->version . '</strong> (' . $mybb->version_code . ')'
    );
    ...
    if ($feed_parser->error == '') {
        ...
        foreach ($feed_parser->items as $item) {
            ...
            $table->construct_cell(
                "<span style=\"font-size: 16px;\"><strong>{$title}</strong></span><br /><br />{$description}<strong><span style=\"float: right;\">{$stamp}</span><br /><br /><a href=\"{$link}\" target=\"_blank\" rel=\"noopener\">&raquo; {$lang->read_more}</a></strong>"
            );
            $table->construct_row();
        }
    } else {
        $table->construct_cell(
            "{$lang->error_fetch_news} <!-- error code: {$feed_parser->error} -->"
        );
        $table->construct_row();
    }
    ...
    $query = $db->simple_select(
        'threads',
        'COUNT(*) AS newthreads',
        "dateline > '$timecut' AND visible='1' AND closed NOT LIKE 'moved|%'"
    );
    ...
    $query = $db->simple_select(
        'posts',
        'COUNT(*) AS newposts',
        "dateline > '$timecut' AND visible='1'"
    );
    ...
    $query = $db->simple_select(
        'reportedcontent',
        'COUNT(*) AS reported_posts',
        "type = 'post' OR type = ''"
    );
    $reported_posts = my_number_format(
        $db->fetch_field($query, 'reported_posts')
    );

    // Get the number of reported posts that haven't been marked as read yet
    $query = $db->simple_select(
        'reportedcontent',
        'COUNT(*) AS new_reported_posts',
        "reportstatus='0' AND (type = 'post' OR type = '')"
    );
    $new_reported_posts = my_number_format(
        $db->fetch_field($query, 'new_reported_posts')
    );

    // Get the number and total file size of attachments
    $query = $db->simple_select(
        'attachments',
        'COUNT(*) AS numattachs, SUM(filesize) as spaceused',
        "visible='1' AND pid > '0'"
    );
    ...
    $unapproved_attachs = my_number_format(
        $db->fetch_field($query, 'numattachs')
    );
    ...
    if (
        isset($update_check['last_check']) &&
        $update_check['last_check'] <= TIME_NOW - 60 * 60 * 24 * 14
    ) {
        $lang->last_update_check_two_weeks = $lang->sprintf(
            $lang->last_update_check_two_weeks,
            'index.php?module=home&amp;action=version_check'
        );
    ...
    if (
        isset($update_check['latest_version_code']) &&
        $update_check['latest_version_code'] > $mybb->version_code
    ) {
        $lang->new_version_available = $lang->sprintf(
            $lang->new_version_available,
            "MyBB {$mybb->version}",
            "<a href=\"https://mybb.com/download\" target=\"_blank\" rel=\"noopener\">MyBB {$update_check['latest_version']}</a>"
        );
    ...
    $table = new Table();
    $table->construct_header($lang->mybb_server_stats, ['colspan' => 2]);
    $table->construct_header($lang->forum_stats, ['colspan' => 2]);

    $table->construct_cell("<strong>{$lang->mybb_version}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell($mybb->version, ['width' => '25%']);
    $table->construct_cell("<strong>{$lang->threads}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell(
        "<strong>{$threads}</strong> {$lang->threads}<br /><strong>{$newthreads}</strong> {$lang->new_today}<br /><a href=\"index.php?module=forum-moderation_queue&amp;type=threads\"><strong>{$unapproved_threads}</strong> {$lang->unapproved}</a>",
        ['width' => '25%']
    );
    $table->construct_row();

    $table->construct_cell("<strong>{$lang->php_version}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell(PHP_VERSION, ['width' => '25%']);
    $table->construct_cell("<strong>{$lang->posts}</strong>", [
        'width' => '25%',
    ]);

    $table->construct_cell(
        "<strong>{$posts}</strong> {$lang->posts}<br /><strong>{$newposts}</strong> {$lang->new_today}<br /><a href=\"index.php?module=forum-moderation_queue&amp;type=posts\"><strong>{$unapproved_posts}</strong> {$lang->unapproved}</a><br /><strong>{$reported_posts}</strong> {$lang->reported_posts}<br /><strong>{$new_reported_posts}</strong> {$lang->unread_reports}",
        ['width' => '25%']
    );

    $table->construct_row();

    $table->construct_cell("<strong>{$lang->sql_engine}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell($db->short_title . ' ' . $db->get_version(), [
        'width' => '25%',
    ]);
    $table->construct_cell("<strong>{$lang->users}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell(
        "<a href=\"index.php?module=user-users\"><strong>{$users}</strong> {$lang->registered_users}</a><br /><strong>{$activeusers}</strong> {$lang->active_users}<br /><strong>{$newusers}</strong> {$lang->registrations_today}<br /><a href=\"index.php?module=user-awaiting_activation\"><strong>{$awaitingusers}</strong> {$lang->awaiting_activation}</a>",
        ['width' => '25%']
    );
    $table->construct_row();

    $table->construct_cell("<strong>{$lang->server_load}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell($serverload, ['width' => '25%']);
    $table->construct_cell("<strong>{$lang->attachments}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell(
        "<strong>{$approved_attachs}</strong> {$lang->attachments}<br /><a href=\"index.php?module=forum-moderation_queue&amp;type=attachments\"><strong>{$unapproved_attachs}</strong> {$lang->unapproved}</a><br /><strong>{$attachs['spaceused']}</strong> {$lang->used}",
        ['width' => '25%']
    );
    $table->construct_row();
    ...
    $table->construct_cell(
        $form->generate_text_area('adminnotes', $adminmessage['adminmessage'], [
    ...
    if (!empty($update_check['news']) && is_array($update_check['news'])) {
        foreach ($update_check['news'] as $news_item) {
            ...
            $description = htmlspecialchars_uni(
                strip_tags($news_item['description'])
            );

            $table->construct_cell(
                "<strong><a href=\"{$link}\" target=\"_blank\" rel=\"noopener\">{$title}</a></strong><br /><span class=\"smalltext\">{$posted}</span>"
            );
            $table->construct_row();
