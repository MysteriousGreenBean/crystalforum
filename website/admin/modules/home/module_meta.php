...
function home_meta()
{
    ...
    $sub_menu['10'] = [
        ...
        'link' => 'index.php?module=home-dashboard',
    ];
    $sub_menu['20'] = [
        'id' => 'preferences',
    ...
    $sub_menu['30'] = [
        'id' => 'docs',
        'title' => $lang->mybb_documentation,
        'link' => 'https://docs.mybb.com',
    ];
    $sub_menu['40'] = [
        'id' => 'credits',
        'title' => $lang->mybb_credits,
        'link' => 'https://mybb.com/credits',
    ];
...
function home_action_handler($action)
{
    ...
    $actions = [
        ...
        'version_check' => [
            'active' => 'version_check',
            'file' => 'version_check.php',
        ],
        'dashboard' => ['active' => 'dashboard', 'file' => 'index.php'],
    ];
    ...
    if ($page->active_action == 'dashboard') {
        ...
        foreach ($sub_menu as $id => $sub) {
            if (
                !check_admin_permissions(
                    ['module' => $sub['module'], 'action' => $sub['action']],
                    false
                )
            ) {
                unset($sub_menu[$id]);
        ...
        if ($db->num_rows($query) == 1) {
            ...
            $online_admins[$mybb->user['username']] = [
                'uid' => $mybb->user['uid'],
        ...
        foreach ($online_admins as $user) {
            if (!isset($done_users["{$user['uid']}.{$user['ip']}"])) {
                ...
                $online_users .=
                    "<li title=\"{$lang->ipaddress} {$ip_address}\"{$class}>" .
                    build_profile_link(
