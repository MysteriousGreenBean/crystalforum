...
$page->add_breadcrumb_item(
    $lang->preferences_and_personal_notes,
    'index.php?module=home-preferences'
);
...
if ($mybb->input['action'] == 'recovery_codes') {
    $page->add_breadcrumb_item(
        $lang->recovery_codes,
        'index.php?module=home-preferences&action=recovery_codes'
    );
    ...
    $db->update_query(
        'adminoptions',
        ['recovery_codes' => $db->escape_string(my_serialize($codes))],
        "uid='{$mybb->user['uid']}'"
    );
    ...
    $table->construct_cell(
        "{$lang->recovery_codes_warning} <strong><a href=\"javascript:window.print()\">{$lang->print_recovery_codes}</a></strong>"
    );
    $table->construct_row();
...
if (!$mybb->input['action']) {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $sqlarray = [
            ...
            'recovery_codes' => $db->escape_string(
                $adminopts['recovery_codes']
            ),
        ];
    ...
    $query = $db->simple_select(
        'adminoptions',
        'notes, cpstyle, cplanguage, codepress, authsecret',
        "uid='" . $mybb->user['uid'] . "'",
        ['limit' => 1]
    );
    ...
    closedir($dir);
    ksort($folders);
    $styles = array_merge(['' => $lang->use_default], $folders);
    $setting_code = $form->generate_select_box(
        'cpstyle',
        $styles,
        $admin_options['cpstyle']
    );

    $languages = array_merge(
        ['' => $lang->use_default],
        $lang->get_languages(1)
    );
    $language_code = $form->generate_select_box(
        'cplanguage',
        $languages,
        $admin_options['cplanguage']
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_on_off_radio('codepress', $admin_options['codepress'])
    );
    ...
    if (!empty($admin_options['authsecret'])) {
        $lang->use_2fa_desc .=
            '<br />' .
            $lang->recovery_codes_desc .
            ' ' .
            $lang->recovery_codes_warning;
    }
    $form_container->output_row(
        $lang->my2fa,
        $lang->use_2fa_desc,
        $form->generate_on_off_radio(
            '2fa',
            (int) !empty($admin_options['authsecret'])
        )
    );

    if (!empty($admin_options['authsecret'])) {
        $qr = $auth->getQRCodeGoogleUrl(
            $mybb->user['username'] . '@AdminCP',
            $admin_options['authsecret'],
            str_replace(' ', '', $mybb->settings['bbname'])
        );
        $form_container->output_row(
            $lang->my2fa_qr . "<br /><img src=\"{$qr}\""
        );
    }
    ...
    $table->construct_cell(
        $form->generate_text_area('notes', $admin_options['notes'], [
