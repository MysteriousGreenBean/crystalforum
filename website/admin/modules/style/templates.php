...
if (
    $mybb->input['action'] == 'add_set' ||
    $mybb->input['action'] == 'add_template' ||
    ($mybb->input['action'] == 'add_template_group' && !$sid) ||
    $mybb->input['action'] == 'search_replace' ||
    $mybb->input['action'] == 'find_updated' ||
    (!$mybb->input['action'] && !$sid)
) {
    $sub_tabs['templates'] = [
    ...
    $sub_tabs['add_set'] = [
        'title' => $lang->add_set,
        'link' =>
            'index.php?module=style-templates&amp;action=add_set' . $expand_str,
    ];
    ...
    if ($mybb->get_input('sid', MyBB::INPUT_INT) != -1) {
        $sub_tabs['add_template_group'] = [
            ...
            'link' =>
                'index.php?module=style-templates&amp;action=add_template_group' .
                $expand_str,
    ...
    ($sid && !$mybb->input['action']) ||
    $mybb->input['action'] == 'edit_set' ||
    $mybb->input['action'] == 'check_set' ||
    $mybb->input['action'] == 'edit_template' ||
    $mybb->input['action'] == 'add_template_group'
) {
    $sub_tabs['manage_templates'] = [
        'title' => $lang->manage_templates,
        'link' =>
            'index.php?module=style-templates&amp;sid=' . $sid . $expand_str,
    ...
    if ($sid > 0) {
        $sub_tabs['edit_set'] = [
            'title' => $lang->edit_set,
            'link' =>
                'index.php?module=style-templates&amp;action=edit_set&amp;sid=' .
                $sid .
                $expand_str,
    ...
    $sub_tabs['add_template'] = [
        ...
        'link' =>
            'index.php?module=style-templates&amp;action=add_template&amp;sid=' .
            $sid .
            $expand_str,
    ...
    if ($mybb->get_input('sid', MyBB::INPUT_INT) != -1) {
        $sub_tabs['add_template_group'] = [
            'title' => $lang->add_template_group,
            'link' =>
                'index.php?module=style-templates&amp;action=add_template_group&amp;sid=' .
                $sid .
                $expand_str,
            'description' => $lang->add_template_group_desc,
        ];
...
while ($template_set = $db->fetch_array($query)) {
    $template_sets[$template_set['sid']] = htmlspecialchars_uni(
        $template_set['title']
    );
}
...
if ($mybb->input['action'] == 'add_set') {
    ...
    $form = new Form(
        'index.php?module=style-templates&amp;action=add_set',
        'post',
        'add_set'
    );
    ...
    if ($mybb->request_method == 'post') {
        if (empty($mybb->input['title'])) {
            ...
            $query = $db->simple_select(
                ...
                'COUNT(tid) as count',
                "title='" .
                    $db->escape_string($mybb->input['title']) .
                    "' AND (sid = '-2' OR sid = '{$sid}')"
            );
        ...
        if (!$errors) {
            ...
            if ($mybb->input['continue']) {
                ...
                admin_redirect(
                    'index.php?module=style-templates&sid=' .
                        $sid .
                        $expand_str2
                );
            }
...
';
    ...
    $form = new Form(
        "index.php?module=style-templates&amp;action=add_template{$expand_str}",
        'post',
        'add_template'
    );
    ...
    $form_container->output_row(
        $lang->template_name,
        $lang->template_name_desc,
        $form->generate_text_box('title', $template['title'], [
            'id' => 'title',
        ]),
        'title'
    );
    $form_container->output_row(
        ...
        $form->generate_select_box('sid', $template_sets, $sid),
        'sid'
    );
    $form_container->output_row(
        '',
        '',
        $form->generate_text_area('template', $template['template'], [
            'id' => 'template',
            'class' => '',
...
if ($mybb->input['action'] == 'add_template_group') {
    ...
    if ($mybb->get_input('sid', MyBB::INPUT_INT) == -1) {
        admin_redirect(
            "index.php?module=style-templates&amp;sid={$sid}" . $expand_str2
        );
    }
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            ...
            if ($prefix_count >= 1) {
                ...
                $plugins->run_hooks(
                    'admin_style_templates_add_template_group_commit'
                );
                ...
                if ($sid) {
                    admin_redirect(
                        "index.php?module=style-templates&amp;sid={$sid}" .
                            $expand_str2
                    );
                }
    ...
    $form = new Form(
        "index.php?module=style-templates&amp;action=add_template_group{$expand_str}",
        'post',
        'add_template_group'
    );
    ...
    $form_container->output_row(
        $lang->template_group_prefix,
        $lang->template_group_prefix_desc,
        $form->generate_text_box('prefix', $template_group['prefix'], [
            'id' => 'prefix',
        ]),
        'prefix'
    );
    $form_container->output_row(
        $lang->template_group_title,
        $lang->template_group_title_desc,
        $form->generate_text_box('title', $template_group['title'], [
            'id' => 'title',
        ]),
        'title'
    );
    ...
    $buttons = [$form->generate_submit_button($lang->add_template_group)];
...
if ($mybb->input['action'] == 'edit_set') {
    ...
    $form = new Form(
        "index.php?module=style-templates&amp;action=edit_set{$expand_str}",
        'post',
        'edit_set'
    );
    echo $form->generate_hidden_field('sid', $sid);
...
if ($mybb->input['action'] == 'edit_template') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            ...
            $plugins->run_hooks(
                'admin_style_templates_edit_template_commit_start'
            );
            ...
            if ($sid > 0) {
                ...
                if (
                    ($existing_sid == -2 && $existing_rows == 1) ||
                    $existing_rows == 0
                ) {
                    $template['tid'] = $db->insert_query(
                        'templates',
                        $template_array
                    );
                } else {
                ...
                if ($mybb->get_input('from') == 'diff_report') {
                    admin_redirect(
                        'index.php?module=style-templates&action=edit_template&title=' .
                            urlencode($mybb->input['title']) .
                            '&sid=' .
                            $mybb->get_input('sid', MyBB::INPUT_INT) .
                            $expand_str2 .
                            '&amp;from=diff_report'
                    );
                } else {
...
';
    ...
    $page->add_breadcrumb_item(
        $lang->edit_template_breadcrumb . $template['title'],
        "index.php?module=style-templates&amp;sid={$sid}"
    );

    $page->output_header(
        $lang->sprintf($lang->editing_template, $template['title'])
    );
    ...
    if ($mybb->input['from'] == 'diff_report') {
        ...
        $sub_tabs['diff_report'] = [
            ...
            'link' =>
                'index.php?module=style-templates&amp;action=diff_report&amp;title=' .
                urlencode($template['title']) .
                '&amp;sid1=' .
                (int) $template['sid'] .
                '&amp;sid2=-2',
        ];
    ...
    $sub_tabs['edit_template'] = [
        ...
        'link' =>
            'index.php?module=style-templates&amp;action=edit_template&amp;title=' .
            htmlspecialchars_uni($template['title']) .
            $expand_str,
    ...
    $form = new Form(
        "index.php?module=style-templates&amp;action=edit_template{$expand_str}",
        'post',
        'edit_template'
    );
...
if ($mybb->input['action'] == 'edit_template_group') {
    ...
    if (
        isset($template_group['isdefault']) &&
        $template_group['isdefault'] == 1
    ) {
        ...
        if (!$errors) {
            if ($prefix != $template_group['prefix']) {
                $query = $db->simple_select(
                    ...
                    'COUNT(gid) AS gid',
                    "prefix = '" .
                        $db->escape_string($mybb->input['prefix']) .
                        "'"
                );
                $prefix_count = $db->fetch_field($query, 'gid');
            ...
            if (!$errors) {
                ...
                $plugins->run_hooks(
                    'admin_style_templates_edit_template_group_commit'
                );

                $db->update_query(
                    'templategroups',
                    $update_array,
                    "gid = '{$template_group['gid']}'"
                );
    ...
    $lang->editing_template_group = $lang->sprintf(
        $lang->editing_template_group,
        htmlspecialchars_uni($template_group['title'])
    );
    ...
    $form = new Form(
        'index.php?module=style-templates&amp;action=edit_template_group',
        'post'
    );
    echo $form->generate_hidden_field('sid', $sid);
    ...
    $buttons = [$form->generate_submit_button($lang->save_template_group)];
    ...
    if ($mybb->request_method == 'post') {
        if ($mybb->input['type'] == 'templates') {
            ...
            if (!$mybb->input['find']) {
                ...
                admin_redirect(
                    'index.php?module=style-templates&action=search_replace'
                );
            } else {
                ...
                $plugins->run_hooks(
                    'admin_style_templates_search_replace_find'
                );
                ...
                $query = $db->query(
					...
					LEFT JOIN " .
                        TABLE_PREFIX .
                        "templatesets s ON (t.sid=s.sid)
					LEFT JOIN " .
                        TABLE_PREFIX .
                        "templates t2 ON (t.title=t2.title AND t2.sid='1')
					WHERE REPLACE(REPLACE(REPLACE(REPLACE(t.template, '\r\n', '\n'), '\r', '\n'), '  ', ''), '\t', '') LIKE '%" .
                        $db->escape_string_like($search_string) .
                        "%' AND NOT (t.sid = -2 AND (t2.tid) IS NOT NULL)
                ...
                if ($db->num_rows($query) == 0) {
                    $table->construct_cell(
                        $lang->sprintf(
                            $lang->search_noresults,
                            htmlspecialchars_uni($mybb->input['find'])
                        ),
                        ['class' => 'align_center']
                    );
                    ...
                    foreach ($template_list as $sid => $templates) {
                        ...
                        $search_header = $lang->sprintf(
                            $lang->search_header,
                            htmlspecialchars_uni($mybb->input['find']),
                            $template_sets[$sid]
                        );
                        $table->construct_header($search_header, [
                            'colspan' => 2,
                        ]);

                        foreach ($templates as $title => $template) {
                            // Do replacement
                            $newtemplate = str_ireplace(
                                $mybb->input['find'],
                                $mybb->input['replace'],
                                $template['template']
                            );
                            if (
                                ...
                                check_template($newtemplate) === false
                            ) {
                                ...
                                if (trim($mybb->input['replace']) != '') {
                                    if ($template['sid'] == -2) {
                                        ...
                                        $new_tid = $db->insert_query(
                                            'templates',
                                            $new_template
                                        );
                                        $label = $lang->sprintf(
                                            $lang->search_created_custom,
                                            $template['title']
                                        );
                                        $url =
                                            'index.php?module=style-templates&amp;action=edit_template&amp;title=' .
            ...
            if (!$mybb->input['title']) {
                ...
                $plugins->run_hooks(
                    'admin_style_templates_search_replace_title'
                );
                ...
                $query = $db->query(
					...
					LEFT JOIN " .
                        ...
                        "templates t2 ON (t.title=t2.title AND t2.sid='1')
					WHERE t.title LIKE '%" .
                        $db->escape_string_like($mybb->input['title']) .
                        "%'
					ORDER BY t.title ASC
				"
                );
                ...
                foreach ($templatessets as $sid => $templates) {
                    ...
                    $table->construct_header($template_sets[$sid], [
                        'colspan' => 2,
                    ]);

                    foreach ($templates as $template) {
                        $template['pretty_title'] = htmlspecialchars_uni(
                            $template['title']
                        );
                        ...
                        if (
                            ...
                            if ($sid > 0) {
                                ...
                                $popup->add_item(
                                    $lang->revert_to_orig,
                                    'index.php?module=style-templates&amp;action=revert&amp;title=' .
                                        urlencode($template['title']) .
                                        "&amp;sid={$sid}&amp;my_post_key={$mybb->post_code}",
                                    "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_revertion}')"
                                );
                            }

                            $template['pretty_title'] =
                        ...
                        elseif (
                            !isset($template['original']) ||
                            $template['original'] == false
                        ) {
                            $popup->add_item(
                                $lang->delete_template,
                                'index.php?module=style-templates&amp;action=delete_template&amp;title=' .
                                    urlencode($template['title']) .
                                    "&amp;sid={$sid}&amp;my_post_key={$mybb->post_code}",
                                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_deletion}')"
                            );
                        ...
                        $table->construct_cell(
                            "<span style=\"padding: 20px;\">{$template['pretty_title']}</span>",
                            ['width' => '85%']
                        );
                        $table->construct_cell($popup->fetch(), [
                            'class' => 'align_center',
...
';
    ...
    $form_container->output_row(
        $lang->search_for,
        '',
        $form->generate_text_area('find', $mybb->get_input('find'), [
            'id' => 'find',
            'class' => '',
    ...
    $form_container->output_row(
        $lang->replace_with,
        '',
        $form->generate_text_area('replace', $mybb->get_input('replace'), [
            'id' => 'replace',
            'class' => '',
            'style' => 'width: 100%; height: 200px;',
        ])
...
if ($mybb->input['action'] == 'find_updated') {
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "templates m ON (m.title=t.title AND m.sid=-2 AND m.version > t.version)
    ...
    foreach ($templates as $sid => $templates) {
        if (empty($done_set[$sid])) {
            $table->construct_header(
                htmlspecialchars_uni($templatesets[$sid]['title']),
                ['colspan' => 2]
            );
        ...
        foreach ($templates as $template) {
            ...
            $popup->add_item(
                ...
                'index.php?module=style-templates&amp;action=diff_report&amp;title=' .
                    ...
                    '&amp;sid1=' .
                    $template['sid'] .
                    '&amp;sid2=-2&amp;from=diff_report'
            );
            $popup->add_item(
                $lang->revert_to_orig,
                'index.php?module=style-templates&amp;action=revert&amp;title=' .
                    urlencode($template['title']) .
                    "&amp;sid={$sid}&amp;from=diff_report&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_revertion}')"
            );

            $table->construct_cell(
                "<a href=\"index.php?module=style-templates&amp;action=edit_template&amp;title=" .
                    urlencode($template['title']) .
                    "&amp;sid={$sid}&amp;from=diff_report\">" .
                    htmlspecialchars_uni($template['title']) .
                    '</a>',
...
if ($mybb->input['action'] == 'delete_template_group') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=style-templates&amp;action=delete_template_group&amp;gid={$template_group['gid']}&amp;sid={$sid}",
            $lang->confirm_template_group_delete
        );
    }
}
...
if ($mybb->input['action'] == 'delete_set') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=style-templates&amp;action=delete_set&amp;sid={$set['sid']}",
            $lang->confirm_template_set_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'delete_template') {
    ...
    if ($mybb->get_input('no')) {
        admin_redirect(
            "index.php?module=style-templates&sid={$template['sid']}{$expand_str2}"
        );
    }
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=style-templates&amp;action=delete_template&amp;sid={$template['sid']}{$expand_str}",
            $lang->confirm_template_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'diff_report') {
    ...
    if (
        empty($mybb->input['sid1']) ||
        !isset($template_sets[$mybb->input['sid1']])
    ) {
        $mybb->input['sid1'] = -2;
    }
    ...
    if (
        $mybb->input['sid2'] != -2 &&
        !isset($template_sets[$mybb->input['sid2']])
    ) {
        flash_message($lang->error_invalid_input, 'error');
        admin_redirect('index.php?module=style-templates');
    }
    ...
    $sub_tabs['diff_report'] = [
        ...
        'link' =>
            ...
            urlencode($mybb->input['title']) .
            '&amp;from=' .
            htmlspecialchars_uni($mybb->input['from']) .
            'sid1=' .
            $mybb->get_input('sid1', MyBB::INPUT_INT) .
            '&amp;sid2=' .
            $mybb->get_input('sid2', MyBB::INPUT_INT),
    ...
    $page->add_breadcrumb_item(
        $lang->diff_report . ': ' . htmlspecialchars_uni($template1['title']),
        'index.php?module=style-templates&amp;action=diff_report&amp;title=' .
            urlencode($mybb->input['title']) .
            '&amp;from=' .
            htmlspecialchars_uni($mybb->input['from']) .
    ...
    $table->construct_cell(
        "<pre class=\"differential\">" . $renderer->render($diff) . '</pre>'
    );
    $table->construct_row();

    $table->output(
        $lang->template_diff_analysis .
            ': ' .
            htmlspecialchars_uni($template1['title'])
    );
...
if ($mybb->input['action'] == 'revert') {
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "templatesets s ON(s.sid=t.sid)
		WHERE t.title='" .
            $db->escape_string($mybb->input['title']) .
            "' AND t.sid > 0 AND t.sid = '" .
            $mybb->get_input('sid', MyBB::INPUT_INT) .
            "'
	"
    );
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=style-templates&amp;sid={$template['sid']}{$expand_str}",
            $lang->confirm_template_revertion
        );
    }
}
...
if (!empty($mybb->input['sid']) && !$mybb->input['action']) {
    ...
    // Global Templates
    if ($sid == -1) {
        $query = $db->simple_select('templates', 'tid,title', "sid='-1'", [
            'order_by' => 'title',
            'order_dir' => 'ASC',
        ]);
        while ($template = $db->fetch_array($query)) {
            ...
            $popup->add_item(
                ...
                'index.php?module=style-templates&amp;action=delete_template&amp;title=' .
                    urlencode($template['title']) .
                    "&amp;sid=-1&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_deletion}')"
            );
        ...
        if ($table->num_rows() == 0) {
            $table->construct_cell($lang->no_global_templates, [
                'colspan' => 2,
            ]);
            $table->construct_row();
        }
    ...
    $query = $db->simple_select(
        'templates',
        '*',
        "sid='" . $mybb->get_input('sid', MyBB::INPUT_INT) . "' OR sid='-2'",
    ...
    while ($template = $db->fetch_array($query)) {
        ...
        if ($template['sid'] != -2) {
            $template['original'] = false;
            $template['modified'] = false;
            $template_groups[$group]['templates'][
                $template['title']
            ] = $template;
        } elseif (
            !in_array($template['gid'], $expand_array) &&
            !isset($expand_array[-1])
        ) {
            $template['original'] = true;
            $template['modified'] = false;
            $template_groups[$group]['templates'][
            ...
            // Save some memory!
            unset(
                $template_groups[$group]['templates'][$template['title']][
                    'template'
                ]
            );
        }
    ...
    foreach ($template_groups as $prefix => $group) {
        ...
        if (isset($group['isdefault']) && !$group['isdefault']) {
            $popup = new PopupMenu(
                "template_set_{$group['gid']}",
                $lang->options
            );
            $popup->add_item(
                $lang->edit_template_group,
                "index.php?module=style-templates&amp;sid={$sid}&amp;action=edit_template_group&amp;gid={$group['gid']}{$group['expand_str']}"
            );
            $popup->add_item(
                $lang->delete_template_group,
                "index.php?module=style-templates&amp;sid={$sid}&amp;action=delete_template_group&amp;gid={$group['gid']}&amp;my_post_key={$mybb->post_code}{$group['expand_str']}",
                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_group_delete}')"
            );
        ...
        if ($expanded == true) {
            ...
            $table->construct_cell(
                "{$set_popup}<strong><a href=\"index.php?module=style-templates&amp;sid={$sid}{$group['expand_str']}#group_{$group['gid']}\">{$group['title']}</a></strong>"
            );
            $table->construct_cell(
                "<a href=\"index.php?module=style-templates&amp;sid={$sid}{$group['expand_str']}#group_{$group['gid']}\">{$expand}</a>",
                ['class' => 'align_center']
            );
            $table->construct_row([
                'class' => 'alt_row',
                'id' => 'group_' . $group['gid'],
                'name' => 'group_' . $group['gid'],
            ]);

            if (
                isset($group['templates']) &&
                is_array($group['templates']) &&
                count($group['templates']) > 0
            ) {
                ...
                foreach ($templates as $template) {
                    ...
                    if (
                        ...
                        if ($sid > 0) {
                            ...
                            $popup->add_item(
                                ...
                                'index.php?module=style-templates&amp;action=revert&amp;title=' .
                                    urlencode($template['title']) .
                                    "&amp;sid={$sid}&amp;my_post_key={$mybb->post_code}{$expand_str}",
                                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_revertion}')"
                            );
                        }
                    ...
                    $table->construct_cell(
                        "<span style=\"padding-left: 20px;\"><a href=\"index.php?module=style-templates&amp;action=edit_template&amp;title=" .
                            urlencode($template['title']) .
                            "&amp;sid={$sid}{$expand_str}\" >{$template['pretty_title']}</a></span>"
                    );
                    $table->construct_cell($popup->fetch(), [
                        'class' => 'align_center',
                    ]);
                ...
                $table->construct_cell($lang->empty_template_set, [
                    'colspan' => 2,
                ]);
                $table->construct_row();
            }
...
if (!$mybb->input['action']) {
    ...
    while ($theme = $db->fetch_array($query)) {
        ...
        $themes[$tbits['templateset']][$theme['tid']] = htmlspecialchars_uni(
            $theme['name']
        );
    }
    ...
    foreach ($template_sets as $set) {
        if ($set['sid'] == -1) {
            $table->construct_cell(
                "<strong><a href=\"index.php?module=style-templates&amp;sid=-1\">{$lang->global_templates}</a></strong><br /><small>{$lang->used_by_all_themes}</small>"
            );
            $table->construct_cell(
                "<a href=\"index.php?module=style-templates&amp;sid=-1\">{$lang->expand_templates}</a>",
                ['class' => 'align_center']
            );
            $table->construct_row();
            continue;
        ...
        if ($set['sid'] == 1) {
            ...
            $popup->add_item(
                $lang->expand_templates,
                "index.php?module=style-templates&amp;sid={$set['sid']}"
            );

            if ($set['sid'] != 1) {
                $popup->add_item(
                    $lang->edit_template_set,
                    "index.php?module=style-templates&amp;action=edit_set&amp;sid={$set['sid']}"
                );

                if (empty($themes[$set['sid']])) {
                    $popup->add_item(
                        $lang->delete_template_set,
                        "index.php?module=style-templates&amp;action=delete_set&amp;sid={$set['sid']}&amp;my_post_key={$mybb->post_code}",
                        "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_set_deletion}')"
                    );
                }
            }
        ...
        $table->construct_cell(
            "<strong><a href=\"index.php?module=style-templates&amp;sid={$set['sid']}\">{$set['title']}</a></strong><br /><small>{$used_by_note}</small>"
        );
