...
if (
    $mybb->input['action'] == 'xmlhttp_stylesheet' &&
    ...
    $query = $db->simple_select(
        ...
        '*',
        "name='" .
            $db->escape_string($mybb->input['file']) .
            "' AND tid IN ({$parent_list})",
    ...
    $selector_list = get_selectors_as_options(
        $css_array,
        $mybb->input['selector']
    );
    ...
    $form = new Form(
        'index.php?module=style-themes&amp;action=stylesheet_properties',
        'post',
        'selector_form',
        0,
        '',
    ...
    echo $form->generate_hidden_field(
        'file',
        htmlspecialchars_uni($mybb->input['file']),
        ['id' => 'file']
    ) . "\n";
    echo $form->generate_hidden_field(
        'selector',
        htmlspecialchars_uni($mybb->input['selector']),
        ['id' => 'hidden_selector']
    ) . "\n";
    ...
    $table->construct_cell(
        "<div style=\"float: {$div_align};\">" .
            $form->generate_text_box(
                'css_bits[background]',
                $properties['background'],
                ['id' => 'css_bits[background]', 'style' => 'width: 260px;']
            ) .
            "</div><div><strong>{$lang->background}</strong></div>",
        ['style' => 'width: 20%;']
    );
    $table->construct_cell(
        "<strong>{$lang->extra_css_atribs}</strong><br /><div style=\"align: center;\">" .
            $form->generate_text_area('css_bits[extra]', $properties['extra'], [
                'id' => 'css_bits[extra]',
                'style' => 'width: 98%;',
                'rows' => '19',
            ]) .
    ...
    $table->construct_cell(
        "<div style=\"float: {$div_align};\">" .
            $form->generate_text_box('css_bits[color]', $properties['color'], [
                'id' => 'css_bits[color]',
                'style' => 'width: 260px;',
            ]) .
            "</div><div><strong>{$lang->color}</strong></div>",
        ['style' => 'width: 40%;']
    );
    $table->construct_row();
    $table->construct_cell(
        "<div style=\"float: {$div_align};\">" .
            $form->generate_text_box('css_bits[width]', $properties['width'], [
                'id' => 'css_bits[width]',
                'style' => 'width: 260px;',
            ]) .
            "</div><div><strong>{$lang->width}</strong></div>",
        ['style' => 'width: 40%;']
    );
    $table->construct_row();
    $table->construct_cell(
        "<div style=\"float: {$div_align};\">" .
            $form->generate_text_box(
                'css_bits[font_family]',
                $properties['font-family'],
                ['id' => 'css_bits[font_family]', 'style' => 'width: 260px;']
            ) .
            "</div><div><strong>{$lang->font_family}</strong></div>",
        ['style' => 'width: 40%;']
    );
    $table->construct_row();
    $table->construct_cell(
        "<div style=\"float: {$div_align};\">" .
            $form->generate_text_box(
                ...
                ['id' => 'css_bits[font_size]', 'style' => 'width: 260px;']
            ) .
            "</div><div><strong>{$lang->font_size}</strong></div>",
        ['style' => 'width: 40%;']
    );
    $table->construct_row();
    $table->construct_cell(
        "<div style=\"float: {$div_align};\">" .
            $form->generate_text_box(
                ...
                ['id' => 'css_bits[font_style]', 'style' => 'width: 260px;']
            ) .
            "</div><div><strong>{$lang->font_style}</strong></div>",
        ['style' => 'width: 40%;']
    );
    $table->construct_row();
    $table->construct_cell(
        "<div style=\"float: {$div_align};\">" .
            $form->generate_text_box(
                'css_bits[font_weight]',
                $properties['font-weight'],
                ['id' => 'css_bits[font_weight]', 'style' => 'width: 260px;']
            ) .
            "</div><div><strong>{$lang->font_weight}</strong></div>",
        ['style' => 'width: 40%;']
    );
    $table->construct_row();
    $table->construct_cell(
        "<div style=\"float: {$div_align};\">" .
            $form->generate_text_box(
                ...
                [
                    'id' => 'css_bits[text_decoration]',
                    'style' => 'width: 260px;',
                ]
            ) .
            "</div><div><strong>{$lang->text_decoration}</strong></div>",
        ['style' => 'width: 40%;']
    );
    $table->construct_row();

    $table->output(
        htmlspecialchars_uni($editable_selector['class_name']) .
...
if ($mybb->input['action'] == 'browse') {
    ...
    $contents = fetch_remote_file(
        "https://community.mybb.com/xmlbrowse.php?api=2&type=themes&version={$search_version}{$keywords}{$url_page}"
    );
    ...
    $table = new Table();
    $table->construct_header($lang->themes, ['colspan' => 2]);
    ...
    if (!empty($tree['results']['result'])) {
        ...
        foreach ($tree['results']['result'] as $result) {
            $result['thumbnail']['value'] = htmlspecialchars_uni(
                $result['thumbnail']['value']
            );
            $result['name']['value'] = htmlspecialchars_uni(
                $result['name']['value']
            );
            $result['description']['value'] = htmlspecialchars_uni(
                $result['description']['value']
            );
            $result['author']['url']['value'] = htmlspecialchars_uni(
                $result['author']['url']['value']
            );
            $result['author']['name']['value'] = htmlspecialchars_uni(
                $result['author']['name']['value']
            );
            $result['download_url']['value'] = htmlspecialchars_uni(
                html_entity_decode($result['download_url']['value'])
            );

            $table->construct_cell(
                "<img src=\"https://community.mybb.com/{$result['thumbnail']['value']}\" alt=\"{$lang->theme_thumbnail}\" title=\"{$lang->theme_thumbnail}\"/>",
                ['class' => 'align_center', 'width' => 100]
            );
            $table->construct_cell(
                "<strong>{$result['name']['value']}</strong><br /><small>{$result['description']['value']}</small><br /><i><small>{$lang->created_by} <a href=\"{$result['author']['url']['value']}\" target=\"_blank\" rel=\"noopener\">{$result['author']['name']['value']}</a></small></i>"
            );
            $table->construct_cell(
                "<strong><a href=\"https://community.mybb.com/{$result['download_url']['value']}\" target=\"_blank\" rel=\"noopener\">{$lang->download}</a></strong>",
                ['class' => 'align_center']
            );
            $table->construct_row();
        }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->error_no_results_found, ['colspan' => 3]);
        $table->construct_row();
    }

    $search = new Form(
        'index.php?module=style-themes&amp;action=browse',
        'post',
        'search_form'
    );
    ...
    echo $search->generate_text_box('keywords', $value, [
        'id' => 'search_keywords',
    ...
    if ($mybb->request_method == 'post') {
        $table->output(
            "<span style=\"float: right;\"><small><a href=\"https://community.mybb.com/mods.php?action=browse&category=themes\" target=\"_blank\" rel=\"noopener\">{$lang->browse_all_themes}</a></small></span>" .
                $lang->sprintf($lang->browse_results_for_mybb, $mybb->version)
        );
    } else {
        $table->output(
            "<span style=\"float: right;\"><small><a href=\"https://community.mybb.com/mods.php?action=browse&category=themes\" target=\"_blank\" rel=\"noopener\">{$lang->browse_all_themes}</a></small></span>" .
                $lang->sprintf(
                    $lang->recommended_themes_for_mybb,
                    $mybb->version
                )
        );
    }
    ...
    if (!empty($tree['results']['attributes']['total'])) {
        echo '<br />' .
            draw_admin_pagination(
                $mybb->input['page'],
                15,
                $tree['results']['attributes']['total'],
                "index.php?module=style-themes&amp;action=browse{$keywords}&amp;page={page}"
            );
    }
...
if ($mybb->input['action'] == 'import') {
        ...
        if (!$errors) {
            ...
            if ($_FILES['local_file']['error'] != 4) {
                ...
                if ($_FILES['local_file']['error'] != 0) {
                    $errors[] =
                        $lang->error_uploadfailed .
                        $lang->error_uploadfailed_detail;
            ...
            if (!$errors) {
                $options = [
                    'no_stylesheets' => $mybb->input['import_stylesheets']
                        ? 0
                        : 1,
                    'no_templates' => $mybb->input['import_templates'] ? 0 : 1,
                    'version_compat' => $mybb->get_input(
                        'version_compat',
                        MyBB::INPUT_INT
                    ),
                ...
                if ($theme_id > -1) {
                    ...
                    admin_redirect(
                        'index.php?module=style-themes&action=edit&tid=' .
                            $theme_id
                    );
                } else {
                    switch ($theme_id) {
	...
	<dt><label style="display: block;"><input type="radio" name="import" value="0" ' .
        $import_checked[1] .
        ' class="imports_check" onclick="checkAction(\'import\');" style="vertical-align: middle;" /> ' .
        $lang->local_file .
        '</label></dt>
	...
	<table cellpadding="4">
				<tr>
					<td>' .
        $form->generate_file_upload_box('local_file', [
            'style' => 'width: 230px;',
        ]) .
        '</td>
				</tr>
		...
		<dt><label style="display: block;"><input type="radio" name="import" value="1" ' .
        $import_checked[2] .
        ' class="imports_check" onclick="checkAction(\'import\');" style="vertical-align: middle;" /> ' .
        $lang->url .
        '</label></dt>
		...
		<table cellpadding="4">
				<tr>
					<td>' .
        $form->generate_text_box('url', $mybb->get_input('file')) .
        '</td>
				</tr>
		</table></dd>
	</dl>
    ...
    $form_container->output_row(
        $lang->import_from,
        $lang->import_from_desc,
        $actions,
        'file'
    );
    $form_container->output_row(
        $lang->parent_theme,
        $lang->parent_theme_desc,
        $form->generate_select_box('tid', $themes, $mybb->get_input('tid'), [
            'id' => 'tid',
        ]),
        'tid'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_check_box(
            'version_compat',
            '1',
            $lang->ignore_version_compatibility,
            [
                'checked' => $mybb->get_input('version_compat'),
                'id' => 'version_compat',
            ]
        ) .
            "<br /><small>{$lang->ignore_version_compat_desc}</small><br />" .
            $form->generate_check_box(
                'import_stylesheets',
                '1',
                $lang->import_stylesheets,
                [
                    'checked' => $mybb->get_input('import_stylesheets'),
                    'id' => 'import_stylesheets',
                ]
            ) .
            "<br /><small>{$lang->import_stylesheets_desc}</small><br />" .
            $form->generate_check_box(
                'import_templates',
                '1',
                $lang->import_templates,
                [
                    'checked' => $mybb->get_input('import_templates'),
                    'id' => 'import_templates',
                ]
            ) .
...
if ($mybb->input['action'] == 'export') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        foreach ($file_stylesheets as $file => $action_stylesheet) {
            ...
            foreach ($action_stylesheet as $action => $style) {
                foreach ($style as $stylesheet) {
                    ...
                    $array =
                        &$file_stylesheets['inherited'][$file . '_' . $action];

                    if (
                        is_array($array) &&
                        array_key_exists($stylesheet, $array)
                    ) {
                        $stylesheets[$stylesheet]['inherited'] =
                            $file_stylesheets['inherited'][
                                $file . '_' . $action
                            ];
                        foreach (
                            $file_stylesheets['inherited'][
        ...
        $inherited_themes = $theme_stylesheets = [];
        if (count($inherited_load) > 0) {
            $query = $db->simple_select(
                'themes',
                'tid, name',
                'tid IN (' . implode(',', $inherited_load) . ')'
            );
            while ($inherited_theme = $db->fetch_array($query)) {
                $inherited_themes[$inherited_theme['tid']] =
                    $inherited_theme['name'];
            }
        ...
        foreach ($stylesheets as $filename => $style) {
            ...
            if (
                $mybb->input['custom_theme'] == 1 &&
                $style['tid'] != $mybb->input['tid']
            ) {
                continue;
            }
    ...
    $page->add_breadcrumb_item(
        htmlspecialchars_uni($theme['name']),
        "index.php?module=style-themes&amp;action=edit&amp;tid={$mybb->input['tid']}"
    );

    $page->add_breadcrumb_item(
        $lang->export_theme,
        'index.php?module=style-themes&amp;action=export'
    );
    ...
    $form_container = new FormContainer(
        $lang->export_theme . ': ' . htmlspecialchars_uni($theme['name'])
    );
    $form_container->output_row(
        $lang->include_custom_only,
        $lang->include_custom_only_desc,
        $form->generate_yes_no_radio(
            'custom_theme',
            $mybb->get_input('custom_theme')
        ),
        'custom_theme'
    );
    $form_container->output_row(
        $lang->include_templates,
        $lang->include_templates_desc,
        $form->generate_yes_no_radio(
            'include_templates',
            $mybb->get_input('include_templates')
        ),
        'include_templates'
    );
...
if ($mybb->input['action'] == 'duplicate') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            ...
            if ($mybb->get_input('duplicate_templates')) {
                $nsid = $db->insert_query('templatesets', [
                    'title' =>
                        $db->escape_string($mybb->get_input('name')) .
                        ' Templates',
                ]);
            ...
            admin_redirect(
                'index.php?module=style-themes&action=edit&tid=' . $tid
            );
        }
    }
    ...
    $page->add_breadcrumb_item(
        htmlspecialchars_uni($theme['name']),
        "index.php?module=style-themes&amp;action=edit&amp;tid={$mybb->get_input(
            'tid'
        )}"
    );

    $page->add_breadcrumb_item(
        $lang->duplicate_theme,
        "index.php?module=style-themes&amp;action=duplicate&amp;tid={$theme['tid']}"
    );
    ...
    $sub_tabs['edit_stylesheets'] = [
        'title' => $lang->edit_stylesheets,
        'link' => "index.php?module=style-themes&amp;action=edit&amp;tid={$mybb->get_input(
            'tid'
        )}",
    ];
    ...
    $sub_tabs['add_stylesheet'] = [
        'title' => $lang->add_stylesheet,
        'link' => "index.php?module=style-themes&amp;action=add_stylesheet&amp;tid={$mybb->get_input(
            'tid'
        )}",
    ];
    ...
    $sub_tabs['export_theme'] = [
        'title' => $lang->export_theme,
        'link' => "index.php?module=style-themes&amp;action=export&amp;tid={$mybb->get_input(
            'tid'
        )}",
        'description' => $lang->export_theme_desc,
    ];
    ...
    $sub_tabs['duplicate_theme'] = [
        'title' => $lang->duplicate_theme,
        'link' => "index.php?module=style-themes&amp;action=duplicate&amp;tid={$mybb->get_input(
            'tid'
        )}",
        'description' => $lang->duplicate_theme_desc,
    ];
    ...
    $form_container->output_row(
        ...
        $form->generate_check_box(
            'duplicate_templates',
            '1',
            $lang->duplicate_templates,
            [
                'checked' => $mybb->get_input('duplicate_templates'),
...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        if (!$mybb->input['name']) {
            $errors[] = $lang->error_missing_name;
        } elseif (in_array($mybb->input['name'], $themes)) {
            $errors[] = $lang->error_theme_already_exists;
        }
    ...
    $page->add_breadcrumb_item(
        $lang->create_new_theme,
        'index.php?module=style-themes&amp;action=add'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        while ($theme2 = $db->fetch_array($query)) {
            ...
            $inherited_theme_cache[$theme2['tid']] =
                $theme2['stylesheets']['inherited'];
        }
        ...
        while ($cachefile = $db->fetch_array($query)) {
            @unlink(
                MYBB_ROOT .
                    "cache/themes/theme{$theme['tid']}/{$cachefile['cachefile']}"
            );

            $filename_min = str_replace(
                '.css',
                '.min.css',
                $cachefile['cachefile']
            );
            @unlink(
                MYBB_ROOT . "cache/themes/theme{$theme['tid']}/{$filename_min}"
            );
        }
        ...
        $db->update_query('users', ['style' => 0], "style='{$theme['tid']}'");
        ...
        $page->output_confirm_action(
            "index.php?module=style-themes&amp;action=delete&amp;tid={$theme['tid']}",
            $lang->confirm_theme_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'edit') {
    ...
    if ($mybb->request_method == 'post' && !$mybb->input['do']) {
        ...
        if (!$update_array['name']) {
            ...
            $query = $db->simple_select(
                ...
                'COUNT(tid) as numthemes',
                "name = '" .
                    $db->escape_string($update_array['name']) .
                    "' AND tid != '{$theme['tid']}'"
            );
            $numthemes = $db->fetch_field($query, 'numthemes');
        ...
        if (empty($errors)) {
            ...
            admin_redirect(
                "index.php?module=style-themes&action=edit&tid={$theme['tid']}"
            );
        }
    }
    ...
    if (count($inherited_load) > 0) {
        ...
        $query = $db->simple_select('themestylesheets', '*', '', [
            'order_by' => 'sid DESC, tid',
            'order_dir' => 'desc',
        ]);
        while ($theme_stylesheet = $db->fetch_array($query)) {
            if (
                !isset($theme_stylesheets[$theme_stylesheet['name']]) &&
                in_array($theme_stylesheet['tid'], $inherited_load)
            ) {
                $theme_stylesheets[
                    $theme_stylesheet['name']
                ] = $theme_stylesheet;
            }
    ...
    $table->construct_header($lang->display_order, [
        'class' => 'align_center',
        'width' => 50,
    ]);
    $table->construct_header($lang->controls, [
    ...
    // Display Order form
    $form = new Form(
        'index.php?module=style-themes&amp;action=edit',
        'post',
        'edit'
    );
    echo $form->generate_hidden_field('tid', $theme['tid']);
    ...
    foreach ($ordered_stylesheets as $filename => $style) {
        ...
        if (
            ...
            if (
                !isset($properties['colors']) ||
                !is_array($properties['colors'])
            ) {
                $properties['colors'] = [];
            }
            ...
            if (count($colors)) {
                ...
                $attached_to =
                    "<small>{$lang->attached_to} " .
                    $lang->sprintf($lang->colors_attached_to) .
                    " {$color_list}</small>";
            }
        ...
        $popup->add_item(
            $lang->edit_style,
            'index.php?module=style-themes&amp;action=edit_stylesheet&amp;file=' .
                htmlspecialchars_uni($filename) .
                "&amp;tid={$theme['tid']}"
        );
        $popup->add_item(
            $lang->properties,
            'index.php?module=style-themes&amp;action=stylesheet_properties&amp;file=' .
                htmlspecialchars_uni($filename) .
                "&amp;tid={$theme['tid']}"
        );
        ...
        if ($inherited == '') {
            $popup->add_item(
                $lang->delete_revert,
                'index.php?module=style-themes&amp;action=delete_stylesheet&amp;file=' .
                    htmlspecialchars_uni($filename) .
                    "&amp;tid={$theme['tid']}&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_stylesheet_deletion}')"
            );
        }

        $table->construct_cell(
            "<strong><a href=\"index.php?module=style-themes&amp;action=edit_stylesheet&amp;file=" .
                htmlspecialchars_uni($filename) .
                "&amp;tid={$theme['tid']}\">" .
                htmlspecialchars_uni($filename) .
                "</a></strong>{$inherited}<br />{$attached_to}"
        );
        $table->construct_cell(
            $form->generate_numeric_field(
                "disporder[{$theme_stylesheets[$filename]['sid']}]",
                $properties['disporder'][$filename],
                ['style' => 'width: 80%; text-align: center;', 'min' => 0]
            ),
            ['class' => 'align_center']
        );
    ...
    $table->output(
        "{$lang->stylesheets_in} " . htmlspecialchars_uni($theme['name'])
    );

    $buttons = [$form->generate_submit_button($lang->save_stylesheet_order)];
    $form->output_submit_wrapper($buttons);
    $form->end();
    ...
    $form_container->output_row(
        $lang->name . ' <em>*</em>',
        $lang->name_desc_edit,
        $form->generate_text_box('name', $theme['name'], ['id' => 'name']),
        'name'
    );

    $options = build_theme_array($theme['tid']);
    $form_container->output_row(
        $lang->parent_theme . ' <em>*</em>',
        $lang->parent_theme_desc,
        $form->generate_select_box('pid', $options, $theme['pid'], [
            'id' => 'pid',
        ]),
        'pid'
    );
    ...
    $form_container->output_row(
        $lang->allowed_user_groups,
        $lang->allowed_user_groups_desc,
        $form->generate_select_box(
            'allowedgroups[]',
            $options,
            explode(',', $theme['allowedgroups']),
            ['id' => 'allowedgroups', 'multiple' => true, 'size' => 5]
        ),
        'allowedgroups'
    );
    ...
    $form_container->output_row(
        $lang->template_set . ' <em>*</em>',
        $lang->template_set_desc,
        $form->generate_select_box(
            'templateset',
            $options,
    ...
    if ($dh = @opendir($editor_theme_root)) {
        while ($dir = readdir($dh)) {
            ...
            $options[$dir] = ucfirst(
                str_replace(['_', '.css'], [' ', ''], $dir)
            );
        }
    }

    $form_container->output_row(
        $lang->editor_theme . ' <em>*</em>',
        $lang->editor_theme_desc,
        $form->generate_select_box(
            'editortheme',
            $options,
            $properties['editortheme'],
    ...
    $form_container->output_row(
        $lang->img_directory,
        $lang->img_directory_desc,
        $form->generate_text_box('imgdir', $properties['imgdir'], [
            'id' => 'imgdir',
        ]),
        'imgdir'
    );
    $form_container->output_row(
        $lang->logo,
        $lang->logo_desc,
        $form->generate_text_box('logo', $properties['logo'], [
            'id' => 'boardlogo',
        ]),
        'logo'
    );
    $form_container->output_row(
        $lang->table_spacing,
        $lang->table_spacing_desc,
        $form->generate_numeric_field('tablespace', $properties['tablespace'], [
            'id' => 'tablespace',
            'min' => 0,
        ]),
        'tablespace'
    ...
    if (empty($properties['colors']) || !is_array($properties['colors'])) {
        ...
        $colors = ['none' => $lang->colors_please_select];
    ...
    $form_container->output_row(
        $lang->colors_setting,
        $lang->colors_setting_desc,
        $color_setting,
        'color'
    );
    $form_container->output_row(
        $lang->colors_add,
        $lang->colors_add_desc,
        $form->generate_text_area('colors', $mybb->get_input('colors'), [
            'style' => 'width: 200px;',
            'rows' => '5',
        ])
    );
...
if ($mybb->input['action'] == 'stylesheet_properties') {
    ...
    if (
        !array_key_exists($stylesheet['cachefile'], $stylesheets) &&
        array_key_exists(
            'css.php?stylesheet=' . $stylesheet['tid'],
            $stylesheets
        )
    ) {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            ...
            if ($theme['tid'] != $stylesheet['tid']) {
                $stylesheet['sid'] = copy_stylesheet_to_theme(
                    $stylesheet,
                    $theme['tid']
                );
            }
                ...
                foreach ($mybb->input as $id => $value) {
                    ...
                    if (strpos($id, 'attached_') !== false) {
                        ...
                        if ($mybb->input['action_' . $attached_id] == 1) {
                            ...
                            $actions_list =
                                $mybb->input['action_list_' . $attached_id];
                        }
            ...
            if ($stylesheet['name'] != $mybb->input['name']) {
                $update_array['cachefile'] = $db->escape_string(
                    str_replace('/', '', $mybb->input['name'])
                );
            }
            ...
            if ($stylesheet['name'] != $mybb->input['name']) {
                ...
                if (
                    ...
                    $db->update_query(
                        'themestylesheets',
                        [
                            'cachefile' => "css.php?stylesheet={$stylesheet['sid']}",
                        ],
                        "sid='{$stylesheet['sid']}'",
                        1
                    );
                ...
                @unlink(
                    MYBB_ROOT .
                        "cache/themes/theme{$theme['tid']}/{$stylesheet['cachefile']}"
                );
                ...
                @unlink(
                    MYBB_ROOT .
                        "cache/themes/theme{$theme['tid']}/{$filename_min}"
                );
            }
            ...
            flash_message(
                $lang->success_stylesheet_properties_updated,
                'success'
            );
            admin_redirect(
                "index.php?module=style-themes&action=edit&tid={$theme['tid']}"
    ...
    $page->add_breadcrumb_item(
        htmlspecialchars_uni($stylesheet['name']) . " {$lang->properties}",
        "index.php?module=style-themes&amp;action=edit_properties&amp;tid={$mybb->input['tid']}"
    );
    ...
    if ($this_stylesheet['inherited'][$stylesheet['name']]) {
        ...
        $stylesheet_parent = htmlspecialchars_uni(
        ...
        if ($stylesheet['tid'] == 1) {
            $page->output_alert(
                $lang->sprintf(
                    $lang->stylesheet_inherited_default,
                    $stylesheet_parent
                )
            );
        } else {
            $page->output_alert(
                $lang->sprintf($lang->stylesheet_inherited, $stylesheet_parent)
            );
        }
    }
        ...
        foreach ($mybb->input as $name => $value) {
            if (strpos($name, 'attached') !== false) {
                ...
                if ($mybb->input['action_' . $id] == 1) {
                    $applied_to[$value] = explode(
                        ',',
                        $mybb->input['action_list_' . $id]
                    );
                }
            }
        }
    ...
    if (
        is_array($applied_to) &&
        (!isset($applied_to['global']) || $applied_to['global'][0] != 'global')
    ) {
        $check_actions = '';
        $stylesheet['colors'] = [];
	...
	<dt><label style=\"display: block;\"><input type=\"radio\" name=\"action_{$count}\" value=\"0\" {$global_action_checked[1]} class=\"action_{$count}s_check\" onclick=\"checkAction('action_{$count}');\" style=\"vertical-align: middle;\" /> {$lang->globally}</label></dt>
		<dt><label style=\"display: block;\"><input type=\"radio\" name=\"action_{$count}\" value=\"1\" {$global_action_checked[2]} class=\"action_{$count}s_check\" onclick=\"checkAction('action_{$count}');\" style=\"vertical-align: middle;\" /> {$lang->specific_actions}</label></dt>
            ...
            $form_container->output_row(
                '',
                '',
                "<span style=\"float: right;\"><a href=\"\" id=\"delete_img_{$count}\"><img src=\"styles/{$page->style}/images/icons/cross.png\" alt=\"{$lang->delete}\" title=\"{$lang->delete}\" /></a></span>{$lang->file} &nbsp;" .
                    $form->generate_text_box("attached_{$count}", $name, [
                        'id' => "attached_{$count}",
                        'style' => 'width: 200px;',
                    ]),
                "attached_{$count}"
            ...
            $specific_files .=
                "<div id=\"attached_form_{$count}\">" .
                $form_container->end(true) .
                "</div><div id=\"attach_box_" .
                ($count + 1) .
                "\"></div>";
    ...
    if (is_array($properties['colors'])) {
        ...
        $specific_colors_option =
            '<dt><label style="display: block;"><input type="radio" name="attach" value="2" ' .
            $global_checked[3] .
            ' class="attachs_check" onclick="checkAction(\'attach\');" style="vertical-align: middle;" /> ' .
            $lang->colors_specific_color .
            '</label></dt><br />';
        ...
        $specific_color =
            ...
            $form->generate_select_box(
                ...
                $properties['colors'],
                $stylesheet['colors'],
                ['multiple' => true, 'size' => "5\" style=\"width: 200px;"]
            ) .
            "
		";
	...
	<dl style="margin-top: 0; margin-bottom: 0; width: 40%;">
		<dt><label style="display: block;"><input type="radio" name="attach" value="0" ' .
        $global_checked[1] .
        ' class="attachs_check" onclick="checkAction(\'attach\');" style="vertical-align: middle;" /> ' .
        $lang->globally .
        '</label></dt><br />
		<dt><label style="display: block;"><input type="radio" name="attach" value="1" ' .
        $global_checked[2] .
        ' class="attachs_check" onclick="checkAction(\'attach\');" style="vertical-align: middle;" /> ' .
        $lang->specific_files .
        ' (<a id="new_specific_file">' .
        $lang->add_another .
        '</a>)</label></dt><br />
		' .
        $specific_files .
    ...
    echo $form->generate_hidden_field(
        'file',
        htmlspecialchars_uni($stylesheet['name'])
    ) . "<br />\n";
    echo $form->generate_hidden_field('tid', $theme['tid']) . "<br />\n";

    $form_container = new FormContainer(
        "{$lang->edit_stylesheet_properties_for} " .
            htmlspecialchars_uni($stylesheet['name'])
    );
    $form_container->output_row(
        $lang->file_name,
    ...
    $buttons[] = $form->generate_submit_button(
        $lang->save_stylesheet_properties
    );
...
if (
    ...
    (!isset($mybb->input['mode']) || $mybb->input['mode'] == 'simple')
) {
    // Fetch the theme we want to edit this stylesheet in
    $query = $db->simple_select(
        'themes',
    ...
    if ($mybb->request_method == 'post') {
        ...
        $new_stylesheet = insert_into_css(
            $css_to_insert,
            $mybb->input['selector'],
            $new_stylesheet
        );
        ...
        if (!$mybb->input['ajax']) {
            ...
            if ($mybb->input['save_close']) {
                ...
                admin_redirect(
                    "index.php?module=style-themes&action=edit_stylesheet&tid={$theme['tid']}&file={$stylesheet['name']}"
                );
            }
        } else {
            echo '1';
    ...
    if (!$selector_list) {
        ...
        admin_redirect(
            "index.php?module=style-themes&action=edit_stylesheet&tid={$theme['tid']}&file=" .
                htmlspecialchars_uni($stylesheet['name']) .
                '&mode=advanced'
        );
        exit();
    ...
    $page->add_breadcrumb_item(
        "{$lang->editing} " . htmlspecialchars_uni($stylesheet['name']),
        "index.php?module=style-themes&amp;action=edit_stylesheet&amp;tid={$mybb->input['tid']}&amp;file=" .
            htmlspecialchars_uni($mybb->input['file']) .
            '&amp;mode=simple'
    );
    ...
    $buttons[] = $form->generate_submit_button($lang->save_changes, [
        'id' => 'save',
        'name' => 'save',
    ]);
    $buttons[] = $form->generate_submit_button($lang->save_changes_and_close, [
        'id' => 'save_close',
        'name' => 'save_close',
    ]);
...
$(function() {
//<![CDATA[
	ThemeSelector.init("./index.php?module=style-themes&action=xmlhttp_stylesheet", "./index.php?module=style-themes&action=edit_stylesheet", $("#selector"), $("#stylesheet"), "' .
        htmlspecialchars_uni($mybb->input['file']) .
        '", $("#selector_form"), "' .
        $mybb->input['tid'] .
        '");
...
';
    ...
    $page->output_header(
        "{$lang->themes} - {$lang->edit_stylesheet_advanced_mode}"
    );

    // If the stylesheet and theme do not match, we must be editing something that is inherited
    if (
        !empty($this_stylesheet['inherited']) &&
    ...
    $form = new Form(
        'index.php?module=style-themes&amp;action=edit_stylesheet&amp;mode=advanced',
        'post',
        'edit_stylesheet'
    );
    echo $form->generate_hidden_field('tid', $mybb->input['tid']) . "\n";
    ...
    $table = new Table();
    $table->construct_cell(
        $form->generate_text_area('stylesheet', $stylesheet['stylesheet'], [
            'id' => 'stylesheet',
            'style' => 'width: 99%;',
            'class' => '',
            'rows' => '30',
...
if ($mybb->input['action'] == 'delete_stylesheet') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=style-themes&amp;action=force&amp;tid={$theme['tid']}",
            $lang->confirm_stylesheet_deletion
        );
    }
}
        ...
        if (!$errors) {
            if ($mybb->input['add_type'] == 1) {
                ...
                $query = $db->simple_select(
                    ...
                    'stylesheet',
                    "name='" .
                        $db->escape_string($mybb->input['import']) .
                        "' AND tid IN ({$parent_list})",
                    ['limit' => 1, 'order_by' => 'tid', 'order_dir' => 'desc']
                );
            ...
            $insert_array = [
                ...
                'attachedto' => implode(
                    '|',
                    array_map([$db, 'escape_string'], $attached)
                ),
                'stylesheet' => $db->escape_string($stylesheet),
                'cachefile' => $db->escape_string(
                    str_replace('/', '', $mybb->input['name'])
                ),
                'lastmodified' => TIME_NOW,
            ];
            ...
            admin_redirect(
                "index.php?module=style-themes&action=edit_stylesheet&tid={$mybb->input['tid']}&sid={$sid}&file=" .
                    urlencode($mybb->input['name'])
            );
        }
    }
...
';
    ...
    if ($errors) {
        ...
        foreach ($mybb->input as $name => $value) {
            if (strpos($name, 'attached') !== false) {
                ...
                $mybb->input['applied_to'][$value] = [0 => 'global'];

                if ($mybb->input['action_' . $id] == 1) {
                    $mybb->input['applied_to'][$value] = explode(
                        ',',
                        $mybb->input['action_list_' . $id]
                    );
                }
    ...
    $form = new Form(
        'index.php?module=style-themes&amp;action=add_stylesheet',
        'post',
        'add_stylesheet'
    );
    ...
    if (
        $mybb->input['attach'] == 1 &&
        is_array($mybb->input['applied_to']) &&
        (!isset($mybb->input['applied_to']['global']) ||
            $mybb->input['applied_to']['global'][0] != 'global')
    ) {
		...
		<dt><label style=\"display: block;\"><input type=\"radio\" name=\"action_{$count}\" value=\"1\" {$global_action_checked[2]} class=\"action_{$count}s_check\" onclick=\"checkAction('action_{$count}');\" style=\"vertical-align: middle;\" /> {$lang->specific_actions}</label></dt>
            ...
            $specific_files .=
                "<div id=\"attached_form_{$count}\">" .
                $form_container->end(true) .
                "</div><div id=\"attach_box_{$count}\"></div>";
    ...
    $form_container = new FormContainer(
        $lang->add_stylesheet_to . ' ' . htmlspecialchars_uni($theme['name']),
        'tfixed'
    );
    $form_container->output_row(
        $lang->file_name,
	...
	<dt><label style=\"display: block;\"><input type=\"radio\" name=\"add_type\" value=\"1\" {$add_checked[1]} class=\"adds_check\" onclick=\"checkAction('add');\" style=\"vertical-align: middle;\" /> <strong>{$lang->import_stylesheet_from}</strong></label></dt>
		...
		<span id=\"add_2\" class=\"adds\"><br />" .
        $form->generate_text_area(
            'stylesheet',
            $mybb->get_input('stylesheet'),
            [
                'id' => 'stylesheet',
                'style' => 'width: 99%;',
                'class' => '',
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->num_users, [
        'class' => 'align_center',
        'width' => 100,
    ]);
    $table->construct_header($lang->controls, [
