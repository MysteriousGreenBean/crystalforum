...
$page->add_breadcrumb_item(
    $lang->admin_logs,
    'index.php?module=tools-adminlog'
);
...
if ($mybb->input['action'] == 'prune') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if ($mybb->input['filter_module']) {
            $where .=
                " AND module='" .
                $db->escape_string($mybb->input['filter_module']) .
                "'";
        }
        ...
        // Log admin action
        log_admin_action(
            $mybb->input['older_than'],
            $mybb->input['uid'],
            $mybb->input['filter_module'],
            $num_deleted
        );
    ...
    $page->add_breadcrumb_item(
        $lang->prune_admin_logs,
        'index.php?module=tools-adminlog&amp;action=prune'
    );
    ...
    while ($module = $db->fetch_array($query)) {
        $module_options[$module['module']] = str_replace(
            ' ',
            ' -&gt; ',
...
if (!$mybb->input['action']) {
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_adminlogs, ['colspan' => '4']);
        $table->construct_row();
    }
    ...
    if ($rescount > $perpage) {
        echo draw_admin_pagination(
            ...
            $perpage,
            $rescount,
            "index.php?module=tools-adminlog&amp;perpage=$perpage&amp;uid={$mybb->get_input(
                'uid'
            )}&amp;fid={$mybb->get_input('fid')}&amp;sortby={$mybb->get_input(
                'sortby'
            )}&amp;order={$order}&amp;filter_module=" .
                htmlspecialchars_uni($mybb->get_input('filter_module'))
        ) . '<br />';
    }
    ...
    $buttons[] = $form->generate_submit_button(
        $lang->filter_administrator_logs
    );
    $form->output_submit_wrapper($buttons);
    $form->end();
...
function get_admin_log_action($logitem)
{
    ...
    $lang_string =
        'admin_log_' . $module . '_' . $action . '_' . $logitem['action'];
    ...
    switch ($lang_string) {
        ...
        case 'admin_log_forum_management_': // add mod, permissions, forum orders
            ...
            if (
                $logitem['data'][0] == 'orders' &&
                !empty($logitem['data'][1])
            ) {
        ...
        case 'admin_log_tools_optimizedb_': // Optimize DB
            $logitem['data'][0] = @implode(
                ', ',
                my_unserialize($logitem['data'][0])
            );
            break;
    ...
    if (isset($lang->$lang_string)) {
        ...
        $string = call_user_func_array([$lang, 'sprintf'], $logitem['data']);
        if (!$string) {
            $string = $lang->$lang_string; // Fall back to the one in the language pack
        }
    } else {
        if (
            isset($logitem['data']['type']) &&
            $logitem['data']['type'] == 'admin_locked_out'
        ) {
            $string = $lang->sprintf(
                $lang->admin_log_admin_locked_out,
                (int) $logitem['data']['uid'],
                htmlspecialchars_uni($logitem['data']['username'])
            );
        } else {
