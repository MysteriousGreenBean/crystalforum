...
$page->add_breadcrumb_item(
    $lang->file_verification,
    'index.php?module=tools-file_verification'
);
...
if (!$mybb->input['action']) {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_header(
            $lang->file_verification . ' - ' . $lang->checking
        );

        $file = explode(
            "\n",
            @file_get_contents(
                "https://mybb.com/checksums/release_mybb_{$mybb->version_code}.txt"
            )
        );
        ...
        foreach ($file as $line) {
            ...
            if (substr($parts[1], 0, 7) == './admin') {
                $parts[1] =
                    "./{$mybb->config['admin_dir']}" . substr($parts[1], 7);
            }

            if (
                file_exists(MYBB_ROOT . 'forums.php') &&
                !file_exists(MYBB_ROOT . 'portal.php')
            ) {
            ...
            if (
                !file_exists(MYBB_ROOT . 'inc/plugins/hello.php') &&
                $parts[1] == './inc/plugins/hello.php'
            ) {
                continue;
            }

            if (
                !is_dir(MYBB_ROOT . 'install/') &&
                substr($parts[1], 0, 10) == './install/'
            ) {
                continue;
            }
        ...
        foreach ($bad_files as $file) {
            ...
            $table->construct_cell(
                "<strong><span style=\"color: {$color};\">" .
                    htmlspecialchars_uni(substr($file['path'], 2)) .
                    '</span></strong>'
            );

            $table->construct_cell(
                "<strong><span style=\"color: {$color};\">{$file['status']}</span></strong>",
                ['class' => 'align_center']
            );
            $table->construct_row();
        }
        ...
        if ($table->num_rows() == 0) {
            $no_errors = true;
            $table->construct_cell($lang->no_corrupt_files_found, [
                'colspan' => 3,
            ]);
            $table->construct_row();
        }
        ...
        if ($no_errors) {
            $table->output(
                $lang->file_verification . ': ' . $lang->no_problems_found
            );
        } else {
            $table->output(
                $lang->file_verification . ': ' . $lang->found_problems
            );
        }
    ...
    $page->output_confirm_action(
        'index.php?module=tools-file_verification',
        $lang->file_verification_message,
        $lang->file_verification
    );
}
