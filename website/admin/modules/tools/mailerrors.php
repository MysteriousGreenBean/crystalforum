...
$page->add_breadcrumb_item(
    $lang->system_email_log,
    'index.php?module=tools-mailerrors'
);
...
if ($mybb->input['action'] == 'prune' && $mybb->request_method == 'post') {
    ...
    } elseif (!empty($mybb->input['log']) && is_array($mybb->input['log'])) {
...
<div class="modal">
 ...
 $table->construct_cell(
     "<a href=\"mailto:{$log['toaddress']}\">{$log['toaddress']}</a>"
 );
 $table->construct_row();

 $table->construct_cell($lang->from . ':');
 $table->construct_cell(
     "<a href=\"mailto:{$log['fromaddress']}\">{$log['fromaddress']}</a>"
 );
 $table->construct_row();
...
if (!$mybb->input['action']) {
    ...
    $form = new Form(
        'index.php?module=tools-mailerrors&amp;action=prune',
        'post'
    );
    ...
    if (!empty($mybb->input['subject'])) {
        $additional_sql_criteria .=
            " AND subject LIKE '%" .
            $db->escape_string_like($mybb->input['subject']) .
            "%'";
        $additional_criteria[] =
    ...
    if (!empty($mybb->input['fromaddress'])) {
        $additional_sql_criteria .=
            ...
            $db->escape_string_like($mybb->input['fromaddress']) .
            "%'";
        $additional_criteria[] =
            'fromaddress=' . urlencode($mybb->input['fromaddress']);
        $form->generate_hidden_field(
            'fromaddress',
            $mybb->input['fromaddress']
        );
    }
    ...
    if (!empty($mybb->input['toaddress'])) {
        $additional_sql_criteria .=
            " AND toaddress LIKE '%" .
            $db->escape_string_like($mybb->input['toaddress']) .
            "%'";
        $additional_criteria[] =
            'toaddress=' . urlencode($mybb->input['toaddress']);
    ...
    if (!empty($mybb->input['error'])) {
        $additional_sql_criteria .=
            " AND error LIKE '%" .
            $db->escape_string_like($mybb->input['error']) .
            "%'";
    ...
    $table->construct_header($lang->error_message, [
        'class' => 'align_center',
        'width' => '30%',
    ]);
    ...
    while ($log = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            $form->generate_check_box("log[{$log['eid']}]", $log['eid'], '')
        );
        $table->construct_cell(
            "<a href=\"javascript:MyBB.popupWindow('index.php?module=tools-mailerrors&amp;action=view&amp;eid={$log['eid']}', null, true);\">{$log['subject']}</a>"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_logs, ['colspan' => 5]);
        $table->construct_row();
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            $per_page,
            $total_rows,
            "index.php?module=tools-mailerrors&amp;page={page}{$additional_criteria}"
        );
    ...
    $form_container->output_row(
        $lang->error_message_contains,
        '',
        $form->generate_text_box('error', $mybb->get_input('error'), [
            'id' => 'error',
        ]),
        'error'
    );
    $form_container->output_row(
        $lang->to_address_contains,
        '',
        $form->generate_text_box('toaddress', $mybb->get_input('toaddress'), [
            'id' => 'toaddress',
        ]),
        'toaddress'
    );
    $form_container->output_row(
        $lang->from_address_contains,
        '',
        $form->generate_text_box(
            'fromaddress',
            $mybb->get_input('fromaddress'),
            ['id' => 'fromaddress']
        ),
        'fromaddress'
    );
