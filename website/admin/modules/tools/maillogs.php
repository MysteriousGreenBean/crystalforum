...
$page->add_breadcrumb_item(
    $lang->user_email_log,
    'index.php?module=tools-maillogs'
);
...
if ($mybb->input['action'] == 'prune' && $mybb->request_method == 'post') {
    ...
    } elseif (is_array($mybb->input['log'])) {
...
if ($mybb->input['action'] == 'view') {
 ...
 $table->construct_cell(
     "<a href=\"mailto:{$log['toemail']}\">{$log['toemail']}</a>"
 );
 $table->construct_row();

 $table->construct_cell($lang->from . ':');
 $table->construct_cell(
     "<a href=\"mailto:{$log['fromemail']}\">{$log['fromemail']}</a>"
 );
 $table->construct_row();
...
if (!$mybb->input['action']) {
    ...
    if (!empty($mybb->input['subject'])) {
        ...
        $additional_criteria[] =
            'subject=' . urlencode($mybb->input['subject']);
    }
    ...
    } elseif (!empty($mybb->input['fromname'])) {
        $user = get_user_by_username($mybb->input['fromname'], [
            'fields' => 'uid, username',
        ]);
    ...
    } elseif (!empty($mybb->input['fromemail'])) {
        $additional_sql_criteria .= " AND l.fromemail LIKE '%{$fromemail}%'";
        $additional_criteria[] =
            'fromemail=' . urlencode($mybb->input['fromemail']);
    ...
    } elseif (!empty($mybb->input['toname'])) {
        $user = get_user_by_username($toname, ['fields' => 'username']);
    ...
    } elseif (!empty($mybb->input['toemail'])) {
        $additional_sql_criteria .= " AND l.toemail LIKE '%{$toemail}%'";
        $additional_criteria[] =
            'toemail=' . urlencode($mybb->input['toemail']);
    ...
    $form = new Form(
        'index.php?module=tools-maillogs&amp;action=prune',
        'post'
    );
    ...
    $table->construct_header($lang->subject, ['colspan' => 2]);
    $table->construct_header($lang->from, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->to, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->date_sent, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "users r ON (r.uid=l.touid)
		LEFT JOIN " .
            TABLE_PREFIX .
            "users f ON (f.uid=l.fromuid)
		LEFT JOIN " .
            TABLE_PREFIX .
    ...
    while ($log = $db->fetch_array($query)) {
        $table->construct_cell(
            $form->generate_check_box("log[{$log['mid']}]", $log['mid'], ''),
            ['width' => 1]
        );
        $log['subject'] = htmlspecialchars_uni($log['subject']);
        $log['dateline'] = my_date('relative', $log['dateline']);
        ...
        if ($log['type'] == 1) {
            $table->construct_cell(
                "<img src=\"styles/{$page->style}/images/icons/maillogs_user.png\" title=\"{$lang->email_sent_to_user}\" alt=\"\" />",
                ['width' => 1]
            );
            $table->construct_cell(
                "<a href=\"javascript:MyBB.popupWindow('index.php?module=tools-maillogs&amp;action=view&amp;mid={$log['mid']}', null, true);\">{$log['subject']}</a>"
            );
        } elseif ($log['type'] == 2) {
            if ($log['thread_subject']) {
                $log['thread_subject'] = htmlspecialchars_uni(
                    $log['thread_subject']
                );
                $thread_link =
                    "<a href=\"../" .
                    get_thread_link($log['tid']) .
                    "\">" .
                    $log['thread_subject'] .
                    '</a>';
            } else {
            ...
            $table->construct_cell(
                "<img src=\"styles/{$page->style}/images/icons/maillogs_thread.png\" title=\"{$lang->sent_using_send_thread_feature}\" alt=\"\" />",
                ['width' => 1]
            );
            $table->construct_cell(
                "<a href=\"javascript:MyBB.popupWindow('index.php?module=tools-maillogs&amp;action=view&amp;mid={$log['mid']}', null, true);\">{$log['subject']}</a><br /><small>{$lang->thread} {$thread_link}</small>"
            );
        } elseif ($log['type'] == 3) {
            $table->construct_cell(
                "<img src=\"styles/{$page->style}/images/icons/maillogs_contact.png\" title=\"{$lang->email_sent_using_contact_form}\" alt=\"\" />",
                ['width' => 1]
            );
            $table->construct_cell(
                "<a href=\"javascript:MyBB.popupWindow('index.php?module=tools-maillogs&amp;action=view&amp;mid={$log['mid']}', null, true);\">{$log['subject']}</a>"
            ...
            $table->construct_cell(
                "<img src=\"styles/{$page->style}/images/icons/default.png\" title=\"{$lang->email}\" alt=\"\" />",
                ['width' => 1]
            );
            $table->construct_cell(
                "<a href=\"javascript:MyBB.popupWindow('index.php?module=tools-maillogs&amp;action=view&amp;mid={$log['mid']}', null, true);\">{$log['subject']}</a>"
        ...
        if ($log['fromuid'] > 0) {
            $log[
                'find_from'
            ] = "<div class=\"float_right\"><a href=\"index.php?module=tools-maillogs&amp;fromuid={$log['fromuid']}\"><img src=\"styles/{$page->style}/images/icons/find.png\" title=\"{$lang->find_emails_by_user}\" alt=\"{$lang->find}\" /></a></div>";
        } else {
            $log['find_from'] = '';
        }
        ...
        if (!$log['from_username'] && $log['fromuid'] > 0) {
            $table->construct_cell(
                "{$log['find_from']}<div>{$lang->deleted_user}</div>"
            );
        } elseif ($log['fromuid'] == 0) {
            $log['fromemail'] = htmlspecialchars_uni($log['fromemail']);
            $table->construct_cell(
                "{$log['find_from']}<div>{$log['fromemail']}</div>"
            );
        } else {
            $table->construct_cell(
                "{$log['find_from']}<div><a href=\"../" .
                    get_profile_link($log['fromuid']) .
                    "\">{$log['from_username']}</a></div>"
            );
        }
        ...
        if ($log['touid'] > 0) {
            $log[
                'find_to'
            ] = "<div class=\"float_right\"><a href=\"index.php?module=tools-maillogs&amp;touid={$log['touid']}\"><img src=\"styles/{$page->style}/images/icons/find.png\" title=\"{$lang->find_emails_to_user}\" alt=\"{$lang->find}\" /></a></div>";
        } else {
            $log['find_to'] = '';
        }
        ...
        if (!$log['to_username'] && $log['touid'] > 0) {
            $table->construct_cell(
                "{$log['find_to']}<div>{$lang->deleted_user}</div>"
            );
        } elseif ($log['touid'] == 0) {
            $log['toemail'] = htmlspecialchars_uni($log['toemail']);
            $table->construct_cell(
                "{$log['find_to']}<div>{$log['toemail']}</div>"
            );
        } else {
            $table->construct_cell(
                "{$log['find_to']}<div><a href=\"../" .
                    get_profile_link($log['touid']) .
                    "\">{$log['to_username']}</a></div>"
            );
        }

        $table->construct_cell($log['dateline'], ['class' => 'align_center']);
        $table->construct_cell(
            my_inet_ntop($db->unescape_binary($log['ipaddress'])),
            ['class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_logs, ['colspan' => '7']);
        $table->construct_row();
        $table->output($lang->user_email_log);
    } else {
        $table->output($lang->user_email_log);
        $buttons[] = $form->generate_submit_button($lang->delete_selected, [
            'onclick' => "return confirm('{$lang->confirm_delete_logs}');",
        ]);
        $buttons[] = $form->generate_submit_button($lang->delete_all, [
            'name' => 'delete_all',
            'onclick' => "return confirm('{$lang->confirm_delete_all_logs}');",
        ]);
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            $per_page,
            $total_rows,
            "index.php?module=tools-maillogs&amp;page={page}{$additional_criteria}"
        );
    ...
    $form_container->output_row(
        $lang->subject_contains,
        '',
        $form->generate_text_box('subject', $mybb->get_input('subject'), [
            'id' => 'subject',
        ]),
        'subject'
    );
    ...
    if ($mybb->get_input('fromname')) {
        $from_type = 'user';
    } elseif ($mybb->get_input('fromemail')) {
        $from_type = 'email';
    }
    $form_container->output_row(
        $lang->from,
        '',
        $form->generate_select_box('from_type', $user_email, $from_type) .
            ' ' .
            $form->generate_text_box(
                'from_value',
                htmlspecialchars_uni($from_filter),
                ['id' => 'from_value']
            ),
        'from_value'
    );
    ...
    if ($mybb->get_input('toname')) {
        $to_type = 'user';
    } elseif ($mybb->get_input('toemail')) {
        $to_type = 'email';
    }
    $form_container->output_row(
        $lang->to,
        '',
        $form->generate_select_box('to_type', $user_email, $to_type) .
            ' ' .
            $form->generate_text_box(
                'to_value',
                htmlspecialchars_uni($to_filter),
                ['id' => 'to_value']
            ),
        'to_value'
    );
