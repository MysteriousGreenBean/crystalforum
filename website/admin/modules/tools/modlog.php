...
if ($mybb->input['action'] == 'prune') {
    ...
    if ($mybb->request_method == 'post') {
        $is_today = false;
        $mybb->input['older_than'] = $mybb->get_input(
            'older_than',
            MyBB::INPUT_INT
        );
        ...
        $where =
            'dateline < ' . (TIME_NOW - $mybb->input['older_than'] * 86400);

        // Searching for entries by a particular user
        if ($mybb->input['uid']) {
            $where .=
                " AND uid='" . $mybb->get_input('uid', MyBB::INPUT_INT) . "'";
        }

        // Searching for entries in a specific module
        if ($mybb->input['fid'] > 0) {
            $where .=
                " AND fid='" . $db->escape_string($mybb->input['fid']) . "'";
        } else {
            $mybb->input['fid'] = 0;
        }
    ...
    $page->add_breadcrumb_item(
        $lang->prune_mod_logs,
        'index.php?module=tools-modlog&amp;action=prune'
    );
    ...
    $form_container->output_row(
        $lang->forum,
        '',
        $form->generate_forum_select('fid', $mybb->get_input('fid'), [
            'id' => 'fid',
    ...
    $form_container->output_row(
        ...
        $lang->older_than .
            $form->generate_numeric_field(
                'older_than',
                $mybb->get_input('older_than'),
                ['id' => 'older_than', 'style' => 'width: 50px', 'min' => 0]
            ) .
            ' ' .
            $lang->days,
        'older_than'
...
if (!$mybb->input['action']) {
    ...
    $table = new Table();
    $table->construct_header($lang->username, ['width' => '10%']);
    $table->construct_header($lang->date, [
        'class' => 'align_center',
        'width' => '15%',
    ]);
    $table->construct_header($lang->action, [
        'class' => 'align_center',
        'width' => '35%',
    ]);
    $table->construct_header($lang->information, [
        'class' => 'align_center',
        'width' => '30%',
    ]);
    $table->construct_header($lang->ipaddress, [
        'class' => 'align_center',
        'width' => '10%',
    ]);
    ...
    while ($logitem = $db->fetch_array($query)) {
        ...
        if ($logitem['username']) {
            $username = format_name(
                htmlspecialchars_uni($logitem['username']),
                $logitem['usergroup'],
        ...
        if ($logitem['tsubject']) {
            $information =
                "<strong>{$lang->thread}</strong> <a href=\"../" .
                get_thread_link($logitem['tid']) .
                "\" target=\"_blank\">" .
                htmlspecialchars_uni($logitem['tsubject']) .
                '</a><br />';
        }
        if ($logitem['fname']) {
            $information .=
                "<strong>{$lang->forum}</strong> <a href=\"../" .
                get_forum_link($logitem['fid']) .
                "\" target=\"_blank\">" .
                htmlspecialchars_uni($logitem['fname']) .
                '</a><br />';
        }
        if ($logitem['psubject']) {
            $information .=
                "<strong>{$lang->post}</strong> <a href=\"../" .
                get_post_link($logitem['pid']) .
                "#pid{$logitem['pid']}\" target=\"_blank\">" .
                htmlspecialchars_uni($logitem['psubject']) .
                '</a>';
        }
        ...
        if (
            ...
            if (!empty($data['uid'])) {
                $information =
                    "<strong>{$lang->user_info}</strong> <a href=\"../" .
            ...
            if (!empty($data['aid'])) {
                $information =
                    "<strong>{$lang->announcement}</strong> <a href=\"../" .
                    get_announcement_link($data['aid']) .
                    "\" target=\"_blank\">" .
                    htmlspecialchars_uni($data['subject']) .
                    '</a>';
            }
        }
        ...
        $table->construct_cell(
            my_inet_ntop($db->unescape_binary($logitem['ipaddress'])),
            ['class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_modlogs, ['colspan' => '5']);
        $table->construct_row();
    }
    ...
    if ($rescount > $perpage) {
        echo draw_admin_pagination(
            ...
            $perpage,
            $rescount,
            "index.php?module=tools-modlog&amp;perpage=$perpage&amp;uid={$mybb->input['uid']}&amp;fid={$mybb->input['fid']}&amp;sortby={$mybb->input['sortby']}&amp;order={$order}"
        ) . '<br />';
    }
    ...
    $form_container->output_row(
        ...
        $form->generate_select_box(
            ...
            " {$lang->in} " .
            $form->generate_select_box('order', $order_array, $order, [
                'id' => 'order',
            ]) .
            " {$lang->order}",
        'order'
