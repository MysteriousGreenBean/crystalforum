...
$page->add_breadcrumb_item(
    $lang->recount_rebuild,
    'index.php?module=tools-recount_rebuild'
);
...
function acp_rebuild_thread_counters()
{
    ...
    check_proceed(
        ...
        $lang->success_rebuilt_thread_counters
    );
}
...
function acp_rebuild_poll_counters()
{
    ...
    $query = $db->simple_select('polls', 'pid', '', [
        'order_by' => 'pid',
        'order_dir' => 'asc',
        'limit_start' => $start,
        'limit' => $per_page,
...
function acp_recount_user_posts()
{
    ...
    check_proceed(
        ...
        'userposts',
        'do_recountuserposts',
        $lang->success_rebuilt_user_post_counters
    );
}
...
function acp_recount_user_threads()
{
    ...
    check_proceed(
        ...
        $lang->success_rebuilt_user_thread_counters
    );
}
...
function acp_recount_reputation()
{
    ...
    check_proceed(
        ...
        $per_page,
        'reputation',
        'do_recountreputation',
        $lang->success_rebuilt_reputation
    );
}
...
function acp_recount_private_messages()
{
    ...
    check_proceed(
        ...
        $lang->success_rebuilt_private_messages
    );
}
...
function acp_recount_thread_ratings()
{
    ...
    while ($thread = $db->fetch_array($query)) {
        ...
        $db->update_query(
            'threads',
            [
                'numratings' => (int) $recount['num_ratings'],
                'totalratings' => (int) $recount['total_rating'],
            ],
            "tid='{$thread['tid']}'"
        );
    }
    ...
    check_proceed(
        ...
        $per_page,
        'threadrating',
        'do_recountthreadrating',
        $lang->success_rebuilt_thread_ratings
    );
}
...
function acp_rebuild_attachment_thumbnails()
{
    ...
    while ($attachment = $db->fetch_array($query)) {
        $ext = my_strtolower(
            my_substr(strrchr($attachment['filename'], '.'), 1)
        );
        if (
            $ext == 'gif' ||
            $ext == 'png' ||
    ...
    check_proceed(
        ...
        $lang->success_rebuilt_attachment_thumbnails
    );
}
...
if (!$mybb->input['action']) {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            ...
            $mybb->get_input('page', MyBB::INPUT_INT) < 1
        ) {
            $mybb->input['page'] = 1;
        }
    ...
    $form_container->output_row_header($lang->data_per_page, ['width' => 50]);
    $form_container->output_row_header('&nbsp;');

    $form_container->output_cell(
        "<label>{$lang->rebuild_forum_counters}</label><div class=\"description\">{$lang->rebuild_forum_counters_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('forumcounters', 50, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_rebuildforumcounters',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->rebuild_thread_counters}</label><div class=\"description\">{$lang->rebuild_thread_counters_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('threadcounters', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_rebuildthreadcounters',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->rebuild_poll_counters}</label><div class=\"description\">{$lang->rebuild_poll_counters_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('pollcounters', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_rebuildpollcounters',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->recount_user_posts}</label><div class=\"description\">{$lang->recount_user_posts_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('userposts', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_recountuserposts',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->recount_user_threads}</label><div class=\"description\">{$lang->recount_user_threads_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('userthreads', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_recountuserthreads',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->rebuild_attachment_thumbs}</label><div class=\"description\">{$lang->rebuild_attachment_thumbs_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('attachmentthumbs', 20, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_rebuildattachmentthumbs',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->recount_stats}</label><div class=\"description\">{$lang->recount_stats_desc}</div>"
    );
    $form_container->output_cell($lang->na);
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, ['name' => 'do_recountstats'])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->recount_reputation}</label><div class=\"description\">{$lang->recount_reputation_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('reputation', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_recountreputation',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->recount_warning}</label><div class=\"description\">{$lang->recount_warning_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('warning', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_recountwarning',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->recount_private_messages}</label><div class=\"description\">{$lang->recount_private_messages_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('privatemessages', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_recountprivatemessages',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->recount_referrals}</label><div class=\"description\">{$lang->recount_referrals_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('referral', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
    $form_container->output_cell(
        $form->generate_submit_button($lang->go, [
            'name' => 'do_recountreferral',
        ])
    );
    $form_container->construct_row();

    $form_container->output_cell(
        "<label>{$lang->recount_thread_ratings}</label><div class=\"description\">{$lang->recount_thread_ratings_desc}</div>"
    );
    $form_container->output_cell(
        $form->generate_numeric_field('threadrating', 500, [
            'style' => 'width: 150px;',
            'min' => 0,
        ])
    );
