...
if ($mybb->input['action'] == 'prune') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if ($mybb->input['filter_username']) {
            $where .=
                " AND username='" .
                $db->escape_string($mybb->input['filter_username']) .
                "'";
        }
        ...
        if ($mybb->input['filter_email']) {
            $where .=
                " AND email='" .
                $db->escape_string($mybb->input['filter_email']) .
                "'";
        }
        ...
        log_admin_action(
            ...
            $mybb->input['filter_username'],
            $mybb->input['filter_email'],
            $num_deleted
        );
    ...
    $page->add_breadcrumb_item(
        $lang->prune_spam_logs,
        'index.php?module=tools-spamlog&amp;action=prune'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_text_box(
            'filter_username',
            $mybb->get_input('filter_username'),
            ['id' => 'filter_username']
        ),
        'filter_username'
    );
...
if (!$mybb->get_input('action')) {
    ...
    if ($mybb->get_input('username')) {
        $where .=
            " AND username='" .
            $db->escape_string($mybb->get_input('username')) .
            "'";
        $additional_criteria[] =
            'username=' . urlencode($mybb->get_input('username'));
    }
    ...
    if ($mybb->get_input('email')) {
        $where .=
            ...
            "'";
        $additional_criteria[] =
            'email=' . urlencode($mybb->get_input('email'));
    }
    ...
    if ($mybb->get_input('ipaddress') > 0) {
        $where .=
            ' AND ipaddress=' .
            $db->escape_binary(my_inet_pton($mybb->get_input('ipaddress')));
        $additional_criteria[] =
            'ipaddress=' . urlencode($mybb->get_input('ipaddress'));
    }
    ...
    $table = new Table();
    $table->construct_header($lang->spam_username, ['width' => '20%']);
    $table->construct_header($lang->spam_email, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->spam_ip, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->spam_date, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->spam_confidence, [
        'class' => 'align_center',
        'width' => '20%',
    ]);

    $query = $db->simple_select('spamlog', '*', $where, [
        'order_by' => $sortby,
        'order_dir' => $order,
        'limit_start' => $start,
        'limit' => $perpage,
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_spam_logs, ['colspan' => '5']);
        $table->construct_row();
    }
    ...
    if ($rescount > $perpage) {
        echo draw_admin_pagination(
            ...
            $perpage,
            $rescount,
            "index.php?module=tools-spamlog&amp;perpage={$perpage}{$additional_criteria}&amp;sortby={$mybb->get_input(
                'sortby'
            )}&amp;order={$order}"
        ) . '<br />';
    }
    ...
    $form_container->output_row(
        $lang->spam_ip,
        '',
        $form->generate_text_box('ipaddress', $mybb->get_input('ipaddress'), [
            'id' => 'ipaddress',
        ]),
        'ipaddress'
    );
