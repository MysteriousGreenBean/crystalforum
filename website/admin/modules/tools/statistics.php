...
$page->add_breadcrumb_item(
    $lang->statistics,
    'index.php?module=tools-statistics'
);
...
if (!$mybb->input['action']) {
    ...
    if ($mybb->get_input('from_year')) {
        $start_dateline = mktime(
            0,
            0,
            0,
            $mybb->get_input('from_month', MyBB::INPUT_INT),
            $mybb->get_input('from_day', MyBB::INPUT_INT),
            $mybb->get_input('from_year', MyBB::INPUT_INT)
        );
        $end_dateline = mktime(
            23,
            59,
            59,
            $mybb->get_input('to_month', MyBB::INPUT_INT),
            $mybb->get_input('to_day', MyBB::INPUT_INT),
            $mybb->get_input('to_year', MyBB::INPUT_INT)
        );
    ...
    if (
        !$mybb->get_input('from_year') ||
        $start_dateline > TIME_NOW ||
        $end_dateline > mktime(23, 59, 59)
    ) {
        ...
        list(
            $mybb->input['from_day'],
            $mybb->input['from_month'],
            $mybb->input['from_year'],
        ) = explode('-', date('j-n-Y', $start_dateline));
        list(
            $mybb->input['to_day'],
            $mybb->input['to_month'],
            $mybb->input['to_year'],
        ) = explode('-', date('j-n-Y', $end_dateline));
    ...
    while ($stat = $db->fetch_array($query)) {
        if ($last_dateline) {
            $stat['change_users'] =
                $stat['numusers'] - $stats[$last_dateline]['numusers'];
            $stat['change_threads'] =
                $stat['numthreads'] - $stats[$last_dateline]['numthreads'];
            $stat['change_posts'] =
                $stat['numposts'] - $stats[$last_dateline]['numposts'];
        } else {
            $stat['change_users'] = 0;
    ...
    if (!empty($stats)) {
        ...
        while ($stat = $db->fetch_array($query)) {
            $table->construct_cell(
                '<strong>' .
                    date($mybb->settings['dateformat'], $stat['dateline']) .
                    '</strong>'
            );
            $table->construct_cell(
                my_number_format($stat['numusers']) .
                    ' <small>' .
                    generate_growth_string(
                        $stats[$stat['dateline']]['change_users']
                    ) .
                    '</small>'
            );
            $table->construct_cell(
                my_number_format($stat['numthreads']) .
                    ' <small>' .
                    generate_growth_string(
                        $stats[$stat['dateline']]['change_threads']
                    ) .
                    '</small>'
            );
            $table->construct_cell(
                my_number_format($stat['numposts']) .
                    ' <small>' .
                    generate_growth_string(
                        $stats[$stat['dateline']]['change_posts']
                    ) .
                    '</small>'
            );
            $table->construct_row();
        ...
        echo draw_admin_pagination(
            ...
            $per_page,
            $total_rows,
            "index.php?module=tools-statistics{$url_range}&amp;page={page}"
        );
    }
...
function create_graph($type, $range = null)
{
    ...
    if ($start == 0) {
        $query = $db->simple_select(
            ...
            "dateline,num{$type}",
            "dateline <= '" . (int) $end . "'",
            ['order_by' => 'dateline', 'order_dir' => 'desc', 'limit' => 2]
        );
        while ($stat = $db->fetch_array($query)) {
            $stats[] = $stat['num' . $type];
        ...
        $query = $db->simple_select(
            ...
            "dateline,num{$type}",
            "dateline >= '" . (int) $start . "'",
            ['order_by' => 'dateline', 'order_dir' => 'asc', 'limit' => 2]
        );
        while ($stat = $db->fetch_array($query)) {
            $stats[] = $stat['num' . $type];
            $datelines[] = $stat['dateline'];
