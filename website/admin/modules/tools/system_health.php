...
$page->add_breadcrumb_item(
    $lang->system_health,
    'index.php?module=tools-system_health'
);
...
if (
    $mybb->input['action'] == 'do_check_templates' &&
    ...
    $page->output_inline_error([$lang->check_templates_info_desc]);
    ...
    foreach ($t_cache as $sid => $templates) {
        if (!$done_set[$sid]) {
            $table = new Table();
            $table->construct_header($templatesets[$sid]['title'], [
                'colspan' => 2,
            ]);
        ...
        if ($sid == -2) {
            ...
            $table->construct_cell($lang->error_master_templates_altered, [
                'colspan' => 2,
            ]);
            $table->construct_row();
        }
            ...
            if ($sid == -2) {
                ...
                $table->construct_cell(
                    "<a href=\"index.php?module=style-templates&amp;action=edit_template&amp;title=" .
                        urlencode($template['title']) .
                        "&amp;sid={$sid}&amp;from=diff_report\">{$template['title']}</a>",
                    ['width' => '80%']
                );
...
if ($mybb->input['action'] == 'check_templates') {
    ...
    $form_container->output_row(
        $lang->check_templates_title,
        '',
        $lang->check_templates_info
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'utf8_conversion') {
    ...
    if (
        $mybb->request_method == 'post' ||
        ($mybb->input['do'] == 'all' && !empty($mybb->input['table']))
    ) {
        if (
            !empty($mybb->input['mb4']) &&
            version_compare($db->get_version(), '5.5.3', '<')
        ) {
            flash_message($lang->error_utf8mb4_version, 'error');
            admin_redirect(
                'index.php?module=tools-system_health&action=utf8_conversion'
            );
        }
        @set_time_limit(0);
        ...
        $page->output_header(
            $lang->system_health . ' - ' . $lang->utf8_conversion
        );
        ...
        $table1 = $db->show_create_table(
            $db->escape_string($mybb->input['table'])
        );
        ...
        $table->output(
            $lang->sprintf($lang->converting_table, $mybb->input['table'])
        );
        ...
        $db->write_query(
            "ALTER TABLE {$mybb->input['table']} DEFAULT CHARACTER SET {$character_set} COLLATE {$collation}"
        );
        ...
        if ($db->supports_fulltext($mybb->input['table'])) {
            ...
            switch ($db->type) {
                ...
                case 'mysqli':
                    preg_match_all(
                        "#FULLTEXT KEY `?([a-zA-Z0-9_]+)`? \(([a-zA-Z0-9_`,']+)\)#i",
                        $table_structure,
                        $matches
                    );
                    if (is_array($matches)) {
                        foreach ($matches[0] as $key => $matched) {
                            $db->write_query(
                                "ALTER TABLE {$mybb->input['table']} DROP INDEX {$matches[1][$key]}"
                            );
                            $fulltext_to_create[$matches[1][$key]] =
                                $matches[2][$key];
                        }
                    }
            }
        ...
        while ($column = $db->fetch_array($query)) {
            ...
            if (array_key_exists($type, $types)) {
                ...
                if (
                    ($db->type == 'mysql' || $db->type == 'mysqli') &&
                    in_array($type, $blob_types)
                ) {
                    if ($column['Null'] == 'YES') {
                        $attributes = 'NULL';
                    } else {
                    ...
                    if ($column['Default'] == 'NULL') {
                        ...
                        $attributes .=
                            "'" . $db->escape_string($column['Default']) . "' ";
        ...
        if (!empty($convert_to_binary)) {
            ...
            $db->write_query(
                "ALTER TABLE {$mybb->input['table']} {$convert_to_binary}"
            );
            $db->write_query(
                "ALTER TABLE {$mybb->input['table']} {$convert_to_utf8}"
            );
        }
        ...
        if ($mybb->input['do'] == 'all') {
            ...
            foreach ($tables as $key => $tablename) {
                if (
                    substr($tablename, 0, strlen(TABLE_PREFIX)) == TABLE_PREFIX
                ) {
                    $table = $db->show_create_table($tablename);
                    preg_match(
                        '#CHARSET=([a-zA-Z0-9_]+)\s?#i',
                        $table,
                        $matches
                    );
                    if (
                        empty($mybb->input['mb4']) &&
                        (fetch_iconv_encoding($matches[1]) == 'utf-8' ||
                            $matches[1] == 'utf8mb4') &&
                        $mybb->input['table'] != $tablename
                    ) {
                        continue;
                    } elseif (
                        !empty($mybb->input['mb4']) &&
            ...
            foreach ($mybb_tables as $key => $tablename) {
                if ($is_next == true) {
                    $nexttable = $tablename;
                    break;
                } elseif ($mybb->input['table'] == $tablename) {
                    $is_next = true;
                }
            }
            ...
            if ($nexttable) {
                ...
                admin_redirect(
                    "index.php?module=tools-system_health&action=utf8_conversion&do=all&table={$nexttable}{$mb4}"
                );
                exit();
            }
        }
    ...
    if (!empty($mybb->input['table']) || $mybb->input['do'] == 'all') {
        ...
        if (
            $mybb->input['do'] != 'all' &&
            !$db->table_exists($db->escape_string($mybb->input['table']))
        ) {
            $db->set_table_prefix($old_table_prefix);
            flash_message($lang->error_invalid_table, 'error');
            admin_redirect(
        ...
        if ($mybb->input['do'] == 'all') {
            ...
            if (is_array($mybb_tables)) {
                ...
                $db->set_table_prefix($old_table_prefix);
                flash_message(
                    $lang->success_all_tables_already_converted,
                    'success'
                );
                admin_redirect('index.php?module=tools-system_health');
            }
        } else {
            $table = $db->show_create_table(
                $db->escape_string($mybb->input['table'])
            );
        }
        ...
        $form = new Form(
            "index.php?module=tools-system_health&amp;action=utf8_conversion{$mb4}",
            'post',
            'utf8_conversion'
        );
        ...
        if (!empty($mybb->input['mb4'])) {
            $table->construct_cell(
                '<strong>' .
                    $lang->sprintf($lang->convert_all_to_utf8mb4, $charset) .
                    '</strong>'
            );
            ...
            if ($mybb->input['do'] == 'all') {
                $table->construct_cell(
                    '<strong>' .
                        $lang->sprintf($lang->convert_all_to_utf, $charset) .
                        '</strong>'
                );
            } else {
                $table->construct_cell(
        ...
        if ($mybb->input['do'] == 'all') {
            ...
            $buttons[] = $form->generate_submit_button(
                $lang->convert_database_tables
            );
        } else {
            $table->output($lang->convert_table . ": {$mybb->input['table']}");
            $buttons[] = $form->generate_submit_button(
                $lang->convert_database_table
            );
        }
    ...
    foreach ($tables as $key => $tablename) {
        if (
            substr($tablename, 0, strlen($old_table_prefix)) ==
            $old_table_prefix
        ) {
            $table = $db->show_create_table($tablename);
            preg_match('#CHARSET=([a-zA-Z0-9_]+)\s?#i', $table, $matches);
    ...
    if (count($unique) > 1) {
        $page->output_error(
            "<p><em>{$lang->warning_multiple_encodings}</em></p>"
        );
    }

    if (
        in_array('utf8mb4', $unique) &&
    ...
    $table->construct_header($lang->status_utf8, ['class' => 'align_center']);
    $table->construct_header($lang->status_utf8mb4, [
        'class' => 'align_center',
    ]);
    ...
    $table->construct_cell($all_utf8, [
        'class' => 'align_center',
        'width' => '15%',
    ]);
    $table->construct_cell($all_utf8mb4, [
        'class' => 'align_center',
        'width' => '25%',
    ]);
    $table->construct_row();
...
if (!$mybb->input['action']) {
    ...
    $table = new Table();
    $table->construct_header($lang->totals, ['colspan' => 2]);
    $table->construct_header($lang->attachments, ['colspan' => 2]);

    $query = $db->simple_select(
        'attachments',
        'COUNT(*) AS numattachs, SUM(filesize) as spaceused, SUM(downloads*filesize) as bandwidthused',
        "visible='1' AND pid > '0'"
    );
    $attachs = $db->fetch_array($query);

    $table->construct_cell("<strong>{$lang->total_database_size}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell(get_friendly_size($db->fetch_size()), [
        'width' => '25%',
    ]);
    $table->construct_cell("<strong>{$lang->attachment_space_used}</strong>", [
        'width' => '200',
    ]);
    $table->construct_cell(get_friendly_size((int) $attachs['spaceused']), [
        'width' => '200',
    ]);
    $table->construct_row();

    if ($attachs['spaceused'] > 0) {
        $attach_average_size = round(
            $attachs['spaceused'] / $attachs['numattachs']
        );
    ...
    $table->construct_cell("<strong>{$lang->total_cache_size}</strong>", [
        'width' => '25%',
    ]);
    $table->construct_cell(get_friendly_size($cache->size_of()), [
        'width' => '25%',
    ]);
    $table->construct_cell(
        "<strong>{$lang->estimated_attachment_bandwidth_usage}</strong>",
        ['width' => '25%']
    );
    $table->construct_cell(get_friendly_size($bandwidth_average_usage), [
        'width' => '25%',
    ]);
    $table->construct_row();

    $table->construct_cell("<strong>{$lang->max_upload_post_size}</strong>", [
        'width' => '200',
    ]);
    $table->construct_cell(
    ...
    $table->construct_cell(
        "<strong>{$lang->average_attachment_size}</strong>",
        ['width' => '25%']
    );
    $table->construct_cell(get_friendly_size($attach_average_size), [
        'width' => '25%',
    ]);
    $table->construct_row();
    ...
    $table->construct_header($lang->run_time, [
        'width' => 200,
        'class' => 'align_center',
    ]);
    ...
    $query = $db->simple_select(
        'tasks',
        '*',
        "nextrun >= '{$nextrun}' AND enabled='1'",
        ['order_by' => 'nextrun', 'order_dir' => 'asc', 'limit' => 3]
    );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_tasks, ['colspan' => 2]);
        $table->construct_row();
    }
    ...
    if (
        isset($mybb->admin['permissions']['tools']['backupdb']) &&
        $mybb->admin['permissions']['tools']['backupdb'] == 1
    ) {
        $backups = [];
        ...
        while (($file = readdir($handle)) !== false) {
            if (filetype(MYBB_ADMIN_DIR . 'backups/' . $file) == 'file') {
                ...
                if ($ext == 'gz' || $ext == 'sql') {
                    $backups[
                        @filemtime(MYBB_ADMIN_DIR . 'backups/' . $file)
                    ] = [
                        'file' => $file,
                        'time' => @filemtime(
                            MYBB_ADMIN_DIR . 'backups/' . $file
                        ),
                        'type' => $ext,
                    ];
                }
        ...
        $table->construct_header($lang->backup_time, [
            'width' => 200,
            'class' => 'align_center',
        ]);
        ...
        foreach ($backups as $backup) {
            ...
            $table->construct_cell(
                "<a href=\"index.php?module=tools-backupdb&amp;action=dlbackup&amp;file={$backup['file']}\">{$backup['file']}</a>"
            );
        ...
        if ($count == 0) {
            $table->construct_cell($lang->no_backups, ['colspan' => 2]);
            $table->construct_row();
        }
    ...
    if ($errors) {
        $page->output_error(
            "<p><em>{$errors} {$lang->error_chmod}</span></strong> {$lang->chmod_info} <a href=\"https://docs.mybb.com/1.8/administration/security/file-permissions\" target=\"_blank\" rel=\"noopener\">MyBB Docs</a>.</em></p>"
        );
    } else {
    ...
    $table->construct_header($lang->location, ['colspan' => 2, 'width' => 250]);
