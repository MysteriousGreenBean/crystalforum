...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $mybb->input['minute'] = check_time_values(
            $mybb->input['minute'],
            0,
            59,
            'string'
        ...
        $mybb->input['hour'] = check_time_values(
            $mybb->input['hour'],
            0,
            59,
            'string'
        ...
        if ($mybb->input['day'] != '*' && $mybb->input['day'] != '') {
            $mybb->input['day'] = check_time_values(
                $mybb->input['day'],
                1,
                31,
                'string'
            ...
            $mybb->input['weekday'] = check_time_values(
                $mybb->input['weekday'],
                0,
                6,
                'array'
        ...
        $mybb->input['month'] = check_time_values(
            $mybb->get_input('month', MyBB::INPUT_ARRAY),
            1,
            12,
            'array'
        );
        ...
        if (!$errors) {
            $new_task = [
                ...
                'month' => $db->escape_string(
                    implode(',', $mybb->input['month'])
                ),
                'weekday' => $db->escape_string(
                    implode(',', $mybb->input['weekday'])
                ),
    ...
    $form = new Form(
        'index.php?module=tools-tasks&amp;action=add',
        'post',
        'add'
    );
    if ($errors) {
    ...
    foreach ($task_files as $task_file) {
        if (
            is_file(MYBB_ROOT . "inc/tasks/{$task_file}") &&
            get_extension($task_file) == 'php'
        ) {
            $file_id = preg_replace(
                '#\.' . get_extension($task_file) . "$#i",
                "$1",
                $task_file
            );
    ...
    $form_container->output_row(
        $lang->time_minutes,
        $lang->time_minutes_desc,
        $form->generate_text_box('minute', $mybb->get_input('minute'), [
            'id' => 'minute',
        ]),
        'minute'
    );
    $form_container->output_row(
        $lang->time_hours,
        $lang->time_hours_desc,
        $form->generate_text_box('hour', $mybb->get_input('hour'), [
            'id' => 'hour',
        ]),
        'hour'
    );
    $form_container->output_row(
        $lang->time_days_of_month,
        $lang->time_days_of_month_desc,
    ...
    $form_container->output_row(
        $lang->time_weekdays,
        $lang->time_weekdays_desc,
        $form->generate_select_box(
            'weekday[]',
            $options,
            $mybb->input['weekday'],
            ['id' => 'weekday', 'multiple' => true, 'size' => 8]
        ),
        'weekday'
    );
    ...
    $form_container->output_row(
        $lang->time_months,
        $lang->time_months_desc,
        $form->generate_select_box(
            'month[]',
            $options,
            $mybb->get_input('month', MyBB::INPUT_ARRAY),
            ['id' => 'month', 'multiple' => true, 'size' => 13]
        ),
        'month'
    );
...
if ($mybb->input['action'] == 'edit') {
    ...
    $form_container->output_row(
        $lang->title . ' <em>*</em>',
        '',
        $form->generate_text_box('title', $task_data['title'], [
            'id' => 'title',
        ]),
        'title'
    );
    $form_container->output_row(
        $lang->short_description . ' <em>*</em>',
        '',
        $form->generate_text_box('description', $task_data['description'], [
            'id' => 'description',
        ]),
        'description'
    );
    ...
    $form_container->output_row(
        $lang->task . ' <em>*</em>',
        $lang->task_file_desc,
        $form->generate_select_box('file', $task_list, $task_data['file'], [
            'id' => 'file',
        ]),
        'file'
    );
    $form_container->output_row(
        $lang->time_minutes,
        $lang->time_minutes_desc,
        $form->generate_text_box('minute', $task_data['minute'], [
            'id' => 'minute',
        ]),
        'minute'
    );
    $form_container->output_row(
        $lang->time_hours,
        $lang->time_hours_desc,
        $form->generate_text_box('hour', $task_data['hour'], ['id' => 'hour']),
        'hour'
    );
    $form_container->output_row(
        $lang->time_days_of_month,
        $lang->time_days_of_month_desc,
        $form->generate_text_box('day', $task_data['day'], ['id' => 'day']),
        'day'
    );
    ...
    $form_container->output_row(
        $lang->time_months,
        $lang->time_months_desc,
        $form->generate_select_box('month[]', $options, $task_data['month'], [
            'id' => 'month',
            'multiple' => true,
        ]),
        'month'
    ...
    $form_container->output_row(
        $lang->enable_logging . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio('logging', $task_data['logging'], true)
    );

    $form_container->output_row(
        $lang->enabled . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio('enabled', $task_data['enabled'], true)
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=tools-tasks&amp;action=delete&amp;tid={$task['tid']}",
            $lang->confirm_task_deletion
        );
    }
}
    ...
    if ($mybb->input['action'] == 'enable') {
        if ($task['file'] == 'backupdb' || $task['file'] == 'checktables') {
            ...
            if ($mybb->request_method == 'post') {
                ...
                $page->output_confirm_action(
                    "index.php?module=tools-tasks&amp;action=enable&amp;tid={$task['tid']}",
                    $lang->confirm_task_enable
                );
            }
        } else {
            $nextrun = fetch_next_run($task);
        ...
        $db->update_query('tasks', ['enabled' => 0], "tid='{$task['tid']}'");
...
if ($mybb->input['action'] == 'logs') {
    ...
    $table->construct_header($lang->data, ['width' => '60%']);
    ...
    $pagination = draw_admin_pagination(
        ...
        $per_page,
        $log_count,
        'index.php?module=tools-tasks&amp;action=logs&amp;page={page}'
    );
    ...
    while ($log_entry = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=tools-tasks&amp;action=edit&amp;tid={$log_entry['tid']}\">{$log_entry['title']}</a>"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_task_logs, ['colspan' => '3']);
        $table->construct_row();
    }
...
if (!$mybb->input['action']) {
    ...
    $table = new Table();
    $table->construct_header($lang->task);
    $table->construct_header($lang->next_run, [
        'class' => 'align_center',
        'width' => 200,
    ]);
    ...
    while ($task = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<div class=\"float_right\"><a href=\"index.php?module=tools-tasks&amp;action=run&amp;tid={$task['tid']}&amp;my_post_key={$mybb->post_code}\"><img src=\"styles/{$page->style}/images/icons/run_task.png\" title=\"{$lang->run_task_now}\" alt=\"{$lang->run_task}\" /></a></div><div>{$icon}<strong><a href=\"index.php?module=tools-tasks&amp;action=edit&amp;tid={$task['tid']}\">{$task['title']}</a></strong><br /><small>{$task['description']}</small></div>"
        );
        ...
        $popup->add_item(
            $lang->edit_task,
            "index.php?module=tools-tasks&amp;action=edit&amp;tid={$task['tid']}"
        );
        if ($task['enabled'] == 1) {
            $popup->add_item(
                $lang->run_task,
                "index.php?module=tools-tasks&amp;action=run&amp;tid={$task['tid']}&amp;my_post_key={$mybb->post_code}"
            );
            $popup->add_item(
                $lang->disable_task,
                "index.php?module=tools-tasks&amp;action=disable&amp;tid={$task['tid']}&amp;my_post_key={$mybb->post_code}"
            );
        } else {
            $popup->add_item(
                $lang->enable_task,
        ...
        $popup->add_item(
            $lang->delete_task,
            "index.php?module=tools-tasks&amp;action=delete&amp;tid={$task['tid']}&amp;my_post_key={$mybb->post_code}",
            "return AdminCP.deleteConfirmation(this, '{$lang->confirm_task_deletion}')"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_tasks, ['colspan' => 3]);
        $table->construct_row();
    }
