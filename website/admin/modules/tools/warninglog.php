...
if ($mybb->input['action'] == 'do_revoke' && $mybb->request_method == 'post') {
    ...
    if (!$warning) {
        ...
        admin_redirect(
            "index.php?module=tools-warninglog&amp;action=view&amp;wid={$warning['wid']}"
        );
    }
...
if ($mybb->input['action'] == 'view') {
    ...
    if ($warning['post_subject']) {
        ...
        $table->construct_cell(
            "<strong>{$lang->warned_user}</strong><br /><br />{$user_link}"
        );
        $table->construct_cell(
            "<strong>{$lang->post}</strong><br /><br /><a href=\"{$mybb->settings['bburl']}/{$post_link}\" target=\"_blank\">{$warning['post_subject']}</a>"
        );
        $table->construct_row();
    } else {
        $table->construct_cell(
    ...
    $issuedby = build_profile_link(
        htmlspecialchars_uni($warning['username']),
        $warning['issuedby'],
        '_blank'
    );
    ...
    $table->construct_cell(
        "<strong>{$lang->warning}</strong><br /><br />{$warning_type} {$points}",
        ['width' => '50%']
    );
    $table->construct_cell(
        "<strong>{$lang->date_issued}</strong><br /><br />{$date_issued}",
        ['width' => '50%']
    );
    $table->construct_row();

    $table->construct_cell(
        "<strong>{$lang->issued_by}</strong><br /><br />{$issuedby}",
        ['width' => '50%']
    );
    $table->construct_cell(
        "<strong>{$lang->expires}</strong><br /><br />{$expires}",
        ['width' => '50%']
    );
    $table->construct_row();

    $table->construct_cell(
        "<strong>{$lang->warning_note}</strong><br /><br />{$notes}",
        ['colspan' => 2]
    );
    $table->construct_row();

    $table->output(
        "<div class=\"float_right\" style=\"font-weight: normal;\">{$status}</div>" .
            $lang->warning_details
    );
    ...
    if (!$warning['daterevoked']) {
        ...
        $form_container->output_row(
            '',
            $lang->revoke_warning_desc,
            $form->generate_text_area('reason', $mybb->get_input('reason'), [
                'id' => 'reason',
            ]),
            'reason'
        );
        ...
        $revoked_by = build_profile_link(
            htmlspecialchars_uni($revoked_user['username']),
            $revoked_user['uid'],
            '_blank'
        );
        ...
        $revoke_table->construct_cell(
            "<strong>{$lang->revoked_by}</strong><br /><br />{$revoked_by}",
            ['width' => '50%']
        );
        $revoke_table->construct_cell(
            "<strong>{$lang->date_revoked}</strong><br /><br />{$date_revoked}",
            ['width' => '50%']
        );
        $revoke_table->construct_row();

        $revoke_table->construct_cell(
            "<strong>{$lang->reason}</strong><br /><br />{$revoke_reason}",
            ['colspan' => 2]
        );
        $revoke_table->construct_row();
...
if (!$mybb->input['action']) {
    ...
    $ordersel = [];
    if (
        empty($mybb->input['filter']['order']) ||
        $mybb->input['filter']['order'] != 'asc'
    ) {
        $order = 'desc';
    ...
    if (
        ...
        $mybb->get_input('page', MyBB::INPUT_INT) > 0
    ) {
    ...
    $table = new Table();
    $table->construct_header($lang->warned_user, ['width' => '15%']);
    $table->construct_header($lang->warning, [
        'class' => 'align_center',
        'width' => '25%',
    ]);
    $table->construct_header($lang->date_issued, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->expires, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->issued_by, [
        'class' => 'align_center',
        'width' => '15%',
    ]);
    ...
    while ($row = $db->fetch_array($query)) {
        ...
        if (!$row['uid']) {
            ...
            $username_link = build_profile_link(
                $username,
                $row['uid'],
                '_blank'
            );
        ...
        $mod_username = format_name(
            htmlspecialchars_uni($row['mod_username']),
            $row['mod_usergroup'],
        ...
        $table->construct_cell($expire_date . $revoked_text, [
            'class' => 'align_center',
        ]);
        $table->construct_cell($mod_username_link);
        $table->construct_cell(
            "<a href=\"index.php?module=tools-warninglog&amp;action=view&amp;wid={$row['wid']}\">{$lang->view}</a>",
            ['class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_warning_logs, ['colspan' => '6']);
        $table->construct_row();
    }
    ...
    foreach (['username', 'mod_username', 'reason', 'sortby'] as $key) {
    ...
    $form_container->output_row(
        $lang->filter_warned_user,
        '',
        $form->generate_text_box(
            'filter[username]',
            $user_filters['username'],
            ['id' => 'filter_username']
        ),
        'filter_username'
    );
    $form_container->output_row(
        $lang->filter_issued_by,
        '',
        $form->generate_text_box(
            'filter[mod_username]',
            $user_filters['mod_username'],
            ['id' => 'filter_mod_username']
        ),
        'filter_mod_username'
    );
    $form_container->output_row(
        $lang->filter_reason,
        '',
        $form->generate_text_box('filter[reason]', $user_filters['reason'], [
            'id' => 'filter_reason',
        ]),
        'filter_reason'
    );
    $form_container->output_row(
        ...
        $form->generate_select_box(
            ...
            " {$lang->in} " .
            $form->generate_select_box('filter[order]', $order_array, $order, [
                'id' => 'filter_order',
            ]) .
            " {$lang->order}",
        'filter_order'
    ...
    $form_container->output_row(
        ...
        '',
        $form->generate_numeric_field('filter[per_page]', $per_page, [
            'id' => 'filter_per_page',
            'min' => 1,
        ]),
        'filter_per_page'
