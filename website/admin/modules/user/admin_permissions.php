...
if (
    ($mybb->input['action'] == 'edit' && $mybb->input['uid'] == 0) ||
    $mybb->input['action'] == 'group' ||
    !$mybb->input['action']
) {
    ...
    $sub_tabs['default_permissions'] = [
        ...
        'link' =>
            'index.php?module=user-admin_permissions&amp;action=edit&amp;uid=0',
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=user-admin_permissions&amp;action=delete&amp;uid={$mybb->input['uid']}",
            $lang->confirm_perms_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'edit') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if ($existing_options > 0) {
            $db->update_query(
                'adminoptions',
                [
                    'permissions' => $db->escape_string(
                        my_serialize($mybb->input['permissions'])
                    ),
                ],
                "uid = '" . $mybb->get_input('uid', MyBB::INPUT_INT) . "'"
            );
    ...
    if ($uid > 0) {
        switch ($db->type) {
            ...
            case 'sqlite':
                $query = $db->query(
					...
					LEFT JOIN " .
                        TABLE_PREFIX .
                        "usergroups g ON (((','|| u.additionalgroups|| ',' LIKE '%,'|| g.gid|| ',%') OR u.usergroup = g.gid))
					WHERE u.uid='$uid'
					AND g.cancp=1
					LIMIT 1
				"
            ...
            default:
				...
				LEFT JOIN " .
                        TABLE_PREFIX .
                        "usergroups g ON (((CONCAT(',', u.additionalgroups, ',') LIKE CONCAT('%,', g.gid, ',%')) OR u.usergroup = g.gid))
				WHERE u.uid='$uid'
				AND g.cancp=1
				LIMIT 1
			"
        ...
        $page->add_breadcrumb_item(
            ...
            'index.php?module=user-admin_permissions&amp;action=group'
        );
    } else {
        $query = $db->simple_select('adminoptions', 'permissions', "uid='0'");
        $permission_data = my_unserialize(
            $db->fetch_field($query, 'permissions')
        );
    ...
    $form = new Form(
        'index.php?module=user-admin_permissions&amp;action=edit',
        'post',
        'edit'
    );
    ...
    while (($module = readdir($dir)) !== false) {
        if (
            ...
            !in_array($module, ['.', '..']) &&
            file_exists($modules_dir . '/' . $module . '/module_meta.php')
        ) {
...
if ($mybb->input['action'] == 'group') {
    ...
    while ($group = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<div class=\"float_right\"><img src=\"styles/{$page->style}/images/icons/{$perm_type}.png\" title=\"{$lang->permissions_type_group}\" alt=\"{$perm_type}\" /></div><div><strong><a href=\"index.php?module=user-admin_permissions&amp;action=edit&amp;uid={$uid}\" title=\"{$lang->edit_group}\">{$group['title']}</a></strong><br /></div>"
        );

        if ($group['permissions'] != '') {
            $popup = new PopupMenu("groupperm_{$uid}", $lang->options);
            $popup->add_item(
                $lang->edit_permissions,
                "index.php?module=user-admin_permissions&amp;action=edit&amp;uid={$uid}"
            );

            // Check permissions for Revoke
            $popup->add_item(
                $lang->revoke_permissions,
                "index.php?module=user-admin_permissions&amp;action=delete&amp;uid={$uid}&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '$lang->confirm_perms_deletion3')"
            );
            ...
            $table->construct_cell(
                "<a href=\"index.php?module=user-admin_permissions&amp;action=edit&amp;uid={$uid}\">{$lang->set_permissions}</a>",
                ['class' => 'align_center']
            );
        }
        $table->construct_row();
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_group_perms, ['colspan' => '3']);
        $table->construct_row();
    }
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->last_active, [
        'class' => 'align_center',
        'width' => 200,
    ]);
    $table->construct_header($lang->controls, [
    ...
    if (!empty($usergroups)) {
        ...
        while ($admin = $db->fetch_array($query)) {
            ...
            if ($admin['permissions'] != '') {
                ...
                $groups = explode(
                    ',',
                    $admin['additionalgroups'] . ',' . $admin['usergroup']
                );
            ...
            // Primary usergroup?
            if (
                !empty($usergroups[$admin['usergroup']]) &&
                $usergroups[$admin['usergroup']]['cancp'] == 1
            ) {
                $usergroup_list[] =
                    '<i>' .
                    htmlspecialchars_uni(
                        $usergroups[$admin['usergroup']]['title']
                    ) .
                    '</i>';
            }
            ...
            if (is_array($additional_groups)) {
                foreach ($additional_groups as $gid) {
                    if (
                        ...
                        $usergroups[$gid]['cancp'] == 1
                    ) {
                        $usergroup_list[] = htmlspecialchars_uni(
                            $usergroups[$gid]['title']
                        );
                    }
                }
            }
            ...
            $table->construct_cell(
                "<div class=\"float_right\"><img src=\"styles/{$page->style}/images/icons/{$perm_type}.png\" title=\"{$lang->permissions_type_user}\" alt=\"{$perm_type}\" /></div><div><strong><a href=\"index.php?module=user-admin_permissions&amp;action=edit&amp;uid={$admin['uid']}\" title=\"{$lang->edit_user}\">{$username}</a></strong><br /><small>{$usergroup_list}</small></div>"
            );
            ...
            if (!is_super_admin($admin['uid'])) {
                if ($admin['permissions'] != '') {
                    $popup->add_item(
                        $lang->edit_permissions,
                        "index.php?module=user-admin_permissions&amp;action=edit&amp;uid={$admin['uid']}"
                    );
                    $popup->add_item(
                        $lang->revoke_permissions,
                        "index.php?module=user-admin_permissions&amp;action=delete&amp;uid={$admin['uid']}&amp;my_post_key={$mybb->post_code}",
                        "return AdminCP.deleteConfirmation(this, '{$lang->confirm_perms_deletion2}')"
                    );
                } else {
                    $popup->add_item(
                        $lang->set_permissions,
                        "index.php?module=user-admin_permissions&amp;action=edit&amp;uid={$admin['uid']}"
                    );
                }
            }
            $popup->add_item(
                $lang->view_log,
                "index.php?module=tools-adminlog&amp;uid={$admin['uid']}"
            );
            $table->construct_cell($popup->fetch(), [
                'class' => 'align_center',
            ]);
    ...
    if (empty($usergroups) || $table->num_rows() == 0) {
        $table->construct_cell($lang->no_user_perms, ['colspan' => '3']);
        $table->construct_row();
    }
