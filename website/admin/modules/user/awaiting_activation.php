...
$page->add_breadcrumb_item(
    $lang->awaiting_activation,
    'index.php?module=user-awaiting_activation'
);

$sub_tabs['awaiting_activation'] = [
...
if ($mybb->input['action'] == 'activate' && $mybb->request_method == 'post') {
    ...
    if (!empty($mybb->input['delete'])) {
        ...
        $plugins->run_hooks(
            'admin_user_awaiting_activation_activate_delete_commit'
        );
    ...
    // Activate selected user(s)
    else {
        $query = $db->simple_select(
            'users',
            'uid, username, email, usergroup, coppauser',
            "uid IN ({$user_ids})"
        );
        while ($user = $db->fetch_array($query)) {
            ++$num_activated;
...
if (!$mybb->input['action']) {
    ...
    $form = new Form(
        'index.php?module=user-awaiting_activation&amp;action=activate',
        'post'
    );
    ...
    $table->construct_header($lang->username, ['width' => '20%']);
    $table->construct_header($lang->registered, [
        'width' => '15%',
        'class' => 'align_center',
    ]);
    ...
    $table->construct_header($lang->email, [
        'width' => '15%',
        'class' => 'align_center',
    ]);
    ...
    $table->construct_header($lang->type, ['class' => 'align_center']);
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "awaitingactivation a ON (a.uid=u.uid)
		WHERE u.usergroup='5'
    ...
    while ($user = $db->fetch_array($query)) {
        ...
        if (
            $user['reg_type'] == 'r' ||
            ($user['reg_type'] == 'b' && $user['validated'] == 0)
        ) {
        ...
        $table->construct_cell(
            $form->generate_check_box("user[{$user['uid']}]", $user['uid'], '')
        );
        $table->construct_cell($user['profilelink']);
        $table->construct_cell($user['regdate'], ['class' => 'align_center']);
        $table->construct_cell($user['lastactive'], [
            'class' => 'align_center',
        ]);
        $table->construct_cell($user['email'], ['class' => 'align_center']);
        $table->construct_cell($user['regip'], ['class' => 'align_center']);
        $table->construct_cell($user['type'], ['class' => 'align_center']);
        $table->construct_row();
    }

    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_users_awaiting_activation, [
            'colspan' => 7,
        ]);
        $table->construct_row();
        ...
        $table->output($lang->manage_awaiting_activation);
        $buttons[] = $form->generate_submit_button($lang->activate_users, [
            'onclick' => "return confirm('{$lang->confirm_activate_users}');",
        ]);
        $buttons[] = $form->generate_submit_button($lang->delete_users, [
            'name' => 'delete',
            'onclick' => "return confirm('{$lang->confirm_delete_users}');",
        ]);
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            $per_page,
            $total_rows,
            'index.php?module=user-awaiting_activation&amp;page={page}'
        );
