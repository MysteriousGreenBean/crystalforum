...
if ($mybb->input['action'] == 'prune') {
    ...
    if (
        ...
        flash_message(
            $lang->cannot_perform_action_super_admin_general,
            'error'
        );
        admin_redirect('index.php?module=user-banning');
    }
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=user-banning&amp;action=prune&amp;uid={$user['uid']}",
            $lang->confirm_prune
        );
    }
}
...
if ($mybb->input['action'] == 'lift') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=user-banning&amp;action=lift&amp;uid={$ban['uid']}",
            $lang->confirm_lift_ban
        );
    }
}
...
if ($mybb->input['action'] == 'edit') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        elseif (is_super_admin($ban['uid']) && !is_super_admin($ban['uid'])) {
    ...
    $form = new Form(
        "index.php?module=user-banning&amp;action=edit&amp;uid={$ban['uid']}",
        'post'
    );
    if ($errors) {
    ...
    if (
        $mybb->input['bantime'] == 'perm' ||
        $mybb->input['bantime'] == '' ||
        $mybb->input['lifted'] == 'perm' ||
        $mybb->input['lifted'] == ''
    ) {
        $mybb->input['bantime'] = '---';
        $mybb->input['lifted'] = '---';
...
if (!$mybb->input['action']) {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (is_array($user) && isset($mybb->input['search'])) {
            ...
            elseif (
                ...
                if (
                    !empty($usergroups[$user['usergroup']]) &&
                    $usergroups[$user['usergroup']]['isbannedgroup'] == 1
                ) {
                    $errors[] = $lang->error_already_banned;
                }
    ...
    $pagination = draw_admin_pagination(
        ...
        $per_page,
        $ban_count,
        'index.php?module=user-banning&amp;page={page}'
    );
    ...
    $buttons[] = $form->generate_submit_button($lang->search_for_a_user, [
        'name' => 'search',
    ]);
    $form->output_submit_wrapper($buttons);
    $form->end();
    ...
    $table->construct_header($lang->ban_lifts_on, [
        'class' => 'align_center',
        'width' => 150,
    ]);
    $table->construct_header($lang->time_left, [
        'class' => 'align_center',
        'width' => 150,
    ]);
    ...
    $table->construct_header($lang->moderation, [
        'class' => 'align_center',
        'colspan' => 1,
        'width' => 200,
    ]);
    ...
    while ($ban = $db->fetch_array($query)) {
        $profile_link = build_profile_link(
            htmlspecialchars_uni($ban['username']),
            $ban['uid'],
            '_blank'
        );
        ...
        $table->construct_cell(
            $lang->sprintf(
                $lang->bannedby_x_on_x,
                $profile_link,
                htmlspecialchars_uni($ban['adminuser']),
                $ban_date,
                $ban_period
            )
        );
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=user-banning&amp;action=edit&amp;uid={$ban['uid']}\">{$lang->edit}</a>",
            ['class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=user-banning&amp;action=lift&amp;uid={$ban['uid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_lift_ban}');\">{$lang->lift}</a>",
            ['class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=user-banning&amp;action=prune&amp;uid={$ban['uid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_prune}');\">{$lang->prune_threads_and_posts}</a>",
            ['class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_banned_users, ['colspan' => '6']);
        $table->construct_row();
    }
