...
if ($mybb->input['action'] == 'disable') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=user-group_promotions&amp;action=disable&amp;pid={$promotion['pid']}",
            $lang->confirm_promo_disable
        );
    }
}
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=user-group_promotions&amp;action=delete&amp;pid={$mybb->input['pid']}",
            $lang->confirm_promo_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'edit') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (!$errors) {
            if (in_array('*', $mybb->input['originalusergroup'])) {
                ...
                $mybb->input['originalusergroup'] = implode(
                    ',',
                    array_map('intval', $mybb->input['originalusergroup'])
                );
            }
            ...
            $allowed_times = ['hours', 'days', 'weeks', 'months', 'years'];
            $time_fields = ['timeregisteredtype', 'timeonlinetype'];
            ...
            $update_promotion = [
                ...
                'registered' => $mybb->get_input(
                    'timeregistered',
                    MyBB::INPUT_INT
                ),
                'registeredtype' => $db->escape_string(
                    $mybb->input['timeregisteredtype']
                ),
                'online' => $mybb->get_input('timeonline', MyBB::INPUT_INT),
                'onlinetype' => $db->escape_string(
                    $mybb->input['timeonlinetype']
                ),
                'reputations' => $mybb->get_input(
                    'reputationcount',
                    MyBB::INPUT_INT
                ),
                'reputationtype' => $db->escape_string(
                    $mybb->input['reputationtype']
                ),
                'referrals' => $mybb->get_input('referrals', MyBB::INPUT_INT),
                'referralstype' => $db->escape_string(
                    $mybb->input['referralstype']
                ),
                'warnings' => $mybb->get_input('warnings', MyBB::INPUT_INT),
                'warningstype' => $db->escape_string(
                    $mybb->input['warningstype']
                ),
                'requirements' => $db->escape_string(
                    implode(',', $mybb->input['requirements'])
                ),
                'originalusergroup' => $db->escape_string(
                    $mybb->input['originalusergroup']
                ),
                'newusergroup' => $mybb->get_input(
                    'newusergroup',
                    MyBB::INPUT_INT
                ),
                'usergrouptype' => $db->escape_string(
                    $mybb->input['usergroupchangetype']
                ),
    ...
    $page->output_header(
        $lang->user_group_promotions . ' - ' . $lang->edit_promotion
    );
    ...
    $form = new Form(
        'index.php?module=user-group_promotions&amp;action=edit',
        'post',
        'edit'
    );
    ...
    if ($errors) {
        ...
        $mybb->input['originalusergroup'] = explode(
            ',',
    ...
    $form_container->output_row(
        $lang->promo_requirements . ' <em>*</em>',
        $lang->promo_requirements_desc,
        $form->generate_select_box(
            'requirements[]',
            $options,
            $mybb->input['requirements'],
            ['id' => 'requirements', 'multiple' => true, 'size' => 5]
        ),
        'requirements'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field('postcount', $mybb->input['postcount'], [
            'id' => 'postcount',
            'min' => 0,
        ]) .
            ' ' .
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field(
            'threadcount',
            $mybb->input['threadcount'],
            ['id' => 'threadcount', 'min' => 0]
        ) .
            ' ' .
            $form->generate_select_box(
                'threadtype',
                $options_type,
                $mybb->input['threadtype'],
                ['id' => 'threadtype']
            ),
        'threadcount'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field(
            'reputationcount',
            $mybb->input['reputationcount'],
            ['id' => 'reputationcount', 'min' => 0]
        ) .
            ' ' .
            $form->generate_select_box(
                'reputationtype',
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field('referrals', $mybb->input['referrals'], [
            'id' => 'referrals',
            'min' => 0,
        ]) .
            ' ' .
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field('warnings', $mybb->input['warnings'], [
            'id' => 'warnings',
            'min' => 0,
        ]) .
            ' ' .
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field(
            'timeregistered',
            $mybb->input['timeregistered'],
            ['id' => 'timeregistered', 'min' => 0]
        ) .
            ' ' .
            $form->generate_select_box(
                'timeregisteredtype',
    ...
    while ($usergroup = $db->fetch_array($query)) {
        $options[(int) $usergroup['gid']] = htmlspecialchars_uni(
            $usergroup['title']
        );
    }

    $form_container->output_row(
        $lang->orig_user_group . ' <em>*</em>',
        $lang->orig_user_group_desc,
        $form->generate_select_box(
            'originalusergroup[]',
            $options,
            $mybb->input['originalusergroup'],
            ['id' => 'originalusergroup', 'multiple' => true, 'size' => 5]
        ),
        'originalusergroup'
    );
    ...
    $form_container->output_row(
        $lang->new_user_group . ' <em>*</em>',
        $lang->new_user_group_desc,
        $form->generate_select_box(
            'newusergroup',
            $options,
            $mybb->input['newusergroup'],
            ['id' => 'newusergroup']
        ),
        'newusergroup'
    );
    ...
    $form_container->output_row(
        $lang->user_group_change_type . ' <em>*</em>',
        $lang->user_group_change_type_desc,
        $form->generate_select_box(
            'usergroupchangetype',
            $options,
            $mybb->input['usergroupchangetype'],
            ['id' => 'usergroupchangetype']
        ),
        'usergroupchangetype'
    );
    ...
    $form_container->output_row(
        $lang->enabled . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio('enabled', $mybb->input['enabled'], true)
    );

    $form_container->output_row(
        $lang->enable_logging . ' <em>*</em>',
        '',
        $form->generate_yes_no_radio('logging', $mybb->input['logging'], true)
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'add') {
    ...
    $page->output_header(
        $lang->user_group_promotions . ' - ' . $lang->add_new_promotion
    );
    ...
    $form = new Form(
        'index.php?module=user-group_promotions&amp;action=add',
        'post',
        'add'
    );
    if ($errors) {
    ...
    $form_container->output_row(
        ...
        $form->generate_select_box(
            'requirements[]',
            $options,
            $mybb->get_input('requirements', MyBB::INPUT_ARRAY),
            ['id' => 'requirements', 'multiple' => true, 'size' => 5]
        ),
        'requirements'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field(
            'reputationcount',
            $mybb->get_input('reputationcount'),
            ['id' => 'reputationcount', 'min' => 0]
        ) .
            ' ' .
            $form->generate_select_box(
    ...
    $form_container->output_row(
        ...
        $form->generate_numeric_field(
            ...
            $form->generate_select_box(
                'timeregisteredtype',
                $options,
                $mybb->get_input('timeregisteredtype'),
                ['id' => 'timeregisteredtype']
            ),
        'timeregistered'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_select_box(
            'originalusergroup[]',
            $options,
            $mybb->get_input('originalusergroup', MyBB::INPUT_ARRAY),
            ['id' => 'originalusergroup', 'multiple' => true, 'size' => 5]
        ),
        'originalusergroup'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_select_box(
            'usergroupchangetype',
            $options,
            $mybb->get_input('usergroupchangetype'),
            ['id' => 'usergroupchangetype']
        ),
        'usergroupchangetype'
    );
...
if ($mybb->input['action'] == 'logs') {
    ...
    $page->output_header(
        $lang->user_group_promotions . ' - ' . $lang->promotion_logs
    );

    $page->output_nav_tabs($sub_tabs, 'promotion_logs');

    $table = new Table();
    $table->construct_header($lang->promoted_user, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->user_group_change_type, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->orig_user_group, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->new_user_group, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    $table->construct_header($lang->time_promoted, [
        'class' => 'align_center',
        'width' => '20%',
    ]);
    ...
    while ($log = $db->fetch_array($query)) {
        $log['username'] =
            "<a href=\"index.php?module=user-view&amp;action=edit&amp;uid={$log['uid']}\">" .
            htmlspecialchars_uni($log['username']) .
            '</a>';

        if (
            $log['type'] == 'secondary' ||
            (!empty($log['oldusergroup']) && strstr(',', $log['oldusergroup']))
        ) {
            $log['oldusergroup'] = '<i>' . $lang->multiple_usergroups . '</i>';
            $log['newusergroup'] = htmlspecialchars_uni(
                $groupscache[$log['newusergroup']]['title']
            );
        } else {
            $log['oldusergroup'] = htmlspecialchars_uni(
                $groupscache[$log['oldusergroup']]['title']
            );
            $log['newusergroup'] = htmlspecialchars_uni(
                $groupscache[$log['newusergroup']]['title']
            );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_promotion_logs, ['colspan' => '5']);
        $table->construct_row();
    }
    ...
    echo '<br />' .
        draw_admin_pagination(
            ...
            '20',
            $total_rows,
            'index.php?module=user-group_promotions&amp;action=logs&amp;page={page}'
        );
...
if (!$mybb->input['action']) {
    ...
    while ($promotion = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<div>{$icon}<strong><a href=\"index.php?module=user-group_promotions&amp;action=edit&amp;pid={$promotion['pid']}\">{$promotion['title']}</a></strong><br /><small>{$promotion['description']}</small></div>"
        );

        $popup = new PopupMenu("promotion_{$promotion['pid']}", $lang->options);
        $popup->add_item(
            $lang->edit_promotion,
            "index.php?module=user-group_promotions&amp;action=edit&amp;pid={$promotion['pid']}"
        );
        if ($promotion['enabled'] == 1) {
            $popup->add_item(
                $lang->disable_promotion,
                "index.php?module=user-group_promotions&amp;action=disable&amp;pid={$promotion['pid']}&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_promo_disable}')"
            );
        } else {
            $popup->add_item(
                $lang->enable_promotion,
                "index.php?module=user-group_promotions&amp;action=enable&amp;pid={$promotion['pid']}&amp;my_post_key={$mybb->post_code}"
            );
        }
        $popup->add_item(
            $lang->delete_promotion,
            "index.php?module=user-group_promotions&amp;action=delete&amp;pid={$promotion['pid']}&amp;my_post_key={$mybb->post_code}",
            "return AdminCP.deleteConfirmation(this, '{$lang->confirm_promo_deletion}')"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_promotions_set, ['colspan' => '2']);
        $table->construct_row();
    }
