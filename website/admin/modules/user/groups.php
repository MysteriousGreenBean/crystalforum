...
if ($mybb->input['action'] == 'approve_join_request') {
    ...
    if (!verify_post_check($mybb->get_input('my_post_key'))) {
        ...
        admin_redirect(
            "index.php?module=user-groups&action=join_requests&gid={$request['gid']}"
        );
    }
...
if ($mybb->input['action'] == 'join_requests') {
    ...
    if ($mybb->request_method == 'post' && is_array($mybb->input['users'])) {
        ...
        $db->delete_query(
            'joinrequests',
            "uid IN ({$uid_in}) AND gid='{$group['gid']}'"
        );
        ...
        admin_redirect(
            "index.php?module=user-groups&action=join_requests&gid={$group['gid']}"
        );
    }

    $page->add_breadcrumb_item(
        $lang->join_requests_for . ' ' . htmlspecialchars_uni($group['title'])
    );
    $page->output_header(
        $lang->join_requests_for . ' ' . htmlspecialchars_uni($group['title'])
    );
    ...
    $table->construct_header(
        $form->generate_check_box('allbox', 1, '', ['class' => 'checkall']),
        ['width' => 1]
    );
    $table->construct_header($lang->users);
    $table->construct_header($lang->reason);
    $table->construct_header($lang->date_requested, [
        'class' => 'align_center',
        'width' => 200,
    ]);
    ...
    while ($request = $db->fetch_array($query)) {
        $table->construct_cell(
            $form->generate_check_box('users[]', $request['uid'], '')
        );
        $table->construct_cell(
            '<strong>' .
                build_profile_link(
        ...
        $table->construct_cell(my_date('relative', $request['dateline']), [
            'class' => 'align_center',
        ]);
        ...
        $popup->add_item(
            $lang->approve,
            "index.php?module=user-groups&action=approve_join_request&amp;rid={$request['rid']}&amp;my_post_key={$mybb->post_code}"
        );
        $popup->add_item(
            $lang->deny,
            "index.php?module=user-groups&action=deny_join_request&amp;rid={$request['rid']}&amp;my_post_key={$mybb->post_code}"
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_join_requests, ['colspan' => 6]);
        $table->construct_row();
    }
    ...
    $buttons[] = $form->generate_submit_button(
        $lang->approve_selected_requests,
        ['name' => 'approve']
    );
...
if ($mybb->input['action'] == 'add_leader' && $mybb->request_method == 'post') {
    ...
    if (!$user) {
        ...
        $query = $db->simple_select(
            'groupleaders',
            'uid',
            "uid='{$user['uid']}' AND gid='{$group['gid']}'"
        );
    ...
    if (!$errors) {
        $new_leader = [
            'gid' => $group['gid'],
            'uid' => $user['uid'],
            'canmanagemembers' => $mybb->get_input(
                'canmanagemembers',
                MyBB::INPUT_INT
            ),
            'canmanagerequests' => $mybb->get_input(
                'canmanagerequests',
                MyBB::INPUT_INT
            ),
            'caninvitemembers' => $mybb->get_input(
                'caninvitemembers',
                MyBB::INPUT_INT
            ),
        ];

        $makeleadermember = $mybb->get_input(
            'makeleadermember',
            MyBB::INPUT_INT
        );
        ...
        flash_message(
            "{$username} " . $lang->success_user_made_leader,
            'success'
        );
        admin_redirect(
            "index.php?module=user-groups&action=leaders&gid={$group['gid']}"
        );
    } else {
...
if ($mybb->input['action'] == 'leaders') {
    ...
    $page->add_breadcrumb_item(
        $lang->group_leaders_for . ' ' . htmlspecialchars_uni($group['title'])
    );
    $page->output_header(
        $lang->group_leaders_for . ' ' . htmlspecialchars_uni($group['title'])
    );
    ...
    $table->construct_header($lang->can_manage_members, [
        'class' => 'align_center',
        'width' => 200,
    ]);
    $table->construct_header($lang->can_manage_join_requests, [
        'class' => 'align_center',
        'width' => 200,
    ]);
    $table->construct_header($lang->can_invite_members, [
        'class' => 'align_center',
        'width' => 200,
    ]);
    $table->construct_header($lang->controls, [
    ...
    while ($leader = $db->fetch_array($query)) {
        ...
        $table->construct_cell(
            "<a href=\"index.php?module=user-groups&amp;action=edit_leader&lid={$leader['lid']}\">{$lang->edit}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=user-groups&amp;action=delete_leader&amp;lid={$leader['lid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_group_leader_deletion}')\">{$lang->delete}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_assigned_leaders, ['colspan' => 5]);
        $table->construct_row();
    }
    ...
    $form = new Form(
        "index.php?module=user-groups&amp;action=add_leader&amp;gid={$group['gid']}",
        'post'
    );
    ...
    $form_container = new FormContainer(
        $lang->add_group_leader . ' ' . htmlspecialchars_uni($group['title'])
    );
    ...
    $form_container->output_row(
        $lang->can_manage_group_members,
        $lang->can_manage_group_members_desc,
        $form->generate_yes_no_radio(
            'canmanagemembers',
            $mybb->input['canmanagemembers']
        )
    );
    $form_container->output_row(
        $lang->can_manage_group_join_requests,
        $lang->can_manage_group_join_requests_desc,
        $form->generate_yes_no_radio(
            'canmanagerequests',
            $mybb->input['canmanagerequests']
        )
    );
    $form_container->output_row(
        $lang->can_invite_group_members,
        $lang->can_invite_group_members_desc,
        $form->generate_yes_no_radio(
            'caninvitemembers',
            $mybb->input['caninvitemembers']
        )
    );
    $form_container->output_row(
        $lang->make_user_member,
        $lang->make_user_member_desc,
        $form->generate_yes_no_radio(
            'makeleadermember',
            $mybb->input['makeleadermember']
        )
    );
    $form_container->end();
...
if ($mybb->input['action'] == 'delete_leader') {
    $query = $db->query(
		...
		WHERE l.lid='" .
            $mybb->get_input('lid', MyBB::INPUT_INT) .
            "'"
    );
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=user-groups&amp;action=delete_leader&amp;lid={$leader['lid']}",
            $lang->confirm_group_leader_deletion
        );
    }
}
...
if ($mybb->input['action'] == 'edit_leader') {
    ...
    $form = new Form(
        "index.php?module=user-groups&amp;action=edit_leader&amp;lid={$leader['lid']}",
        'post'
    );
...
if ($mybb->input['action'] == 'add') {
        ...
        if (!$errors) {
            ...
            // Copying permissions from another group
                ...
                $query = $db->simple_select(
                    ...
                    '*',
                    "gid='" .
                        $mybb->get_input('copyfrom', MyBB::INPUT_INT) .
                        "'"
                );
            ...
            if (
                in_array($mybb->input['title'], $grouptitles) &&
                count(array_keys($grouptitles, $mybb->input['title'])) > 1
            ) {
                $message = $lang->sprintf(
                    $lang->success_group_created_duplicate_title,
                    htmlspecialchars_uni($mybb->input['title'])
                );
            }

            flash_message($message, 'success');
            admin_redirect(
                "index.php?module=user-groups&action=edit&gid={$gid}"
            );
        }
    }
    ...
    $form_container->output_row(
        $lang->username_style,
        $lang->username_style_desc,
        $form->generate_text_box('namestyle', $mybb->get_input('namestyle'), [
            'id' => 'namestyle',
        ]),
        'namestyle'
    );
    ...
    $form_container->output_row(
        $lang->copy_permissions_from,
        $lang->copy_permissions_from_desc,
        $form->generate_select_box(
            'copyfrom',
            $options,
            $mybb->get_input('copyfrom'),
            ['id' => 'copyfrom']
        ),
        'copyfrom'
    );
...
if ($mybb->input['action'] == 'edit') {
    ...
    if (!$usergroup) {
        ...
        if (
            preg_match(
                '#<((m[^a])|(b[^diloru>])|(s[^aemptu >]))(\s*[^>]*)>#si',
                $mybb->get_input('namestyle')
            )
        ) {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            $mybb->get_input('moderate') == 1 &&
            $mybb->get_input('invite') == 1
        ) {
        ...
        if (!$errors) {
            ...
            $updated_group = [
                ...
                'isbannedgroup' => $mybb->get_input(
                    'isbannedgroup',
                    MyBB::INPUT_INT
                ),
                'canview' => $mybb->get_input('canview', MyBB::INPUT_INT),
                'canviewthreads' => $mybb->get_input(
                    'canviewthreads',
                    MyBB::INPUT_INT
                ),
                'canviewprofiles' => $mybb->get_input(
                    'canviewprofiles',
                    MyBB::INPUT_INT
                ),
                'candlattachments' => $mybb->get_input(
                    'candlattachments',
                    MyBB::INPUT_INT
                ),
                'canviewboardclosed' => $mybb->get_input(
                    'canviewboardclosed',
                    MyBB::INPUT_INT
                ),
                'canpostthreads' => $mybb->get_input(
                    'canpostthreads',
                    MyBB::INPUT_INT
                ),
                'canpostreplys' => $mybb->get_input(
                    'canpostreplys',
                    MyBB::INPUT_INT
                ),
                'canpostattachments' => $mybb->get_input(
                    'canpostattachments',
                    MyBB::INPUT_INT
                ),
                'canratethreads' => $mybb->get_input(
                    'canratethreads',
                    MyBB::INPUT_INT
                ),
                'modposts' => $mybb->get_input('modposts', MyBB::INPUT_INT),
                'modthreads' => $mybb->get_input('modthreads', MyBB::INPUT_INT),
                'mod_edit_posts' => $mybb->get_input(
                    'mod_edit_posts',
                    MyBB::INPUT_INT
                ),
                'modattachments' => $mybb->get_input(
                    'modattachments',
                    MyBB::INPUT_INT
                ),
                'caneditposts' => $mybb->get_input(
                    'caneditposts',
                    MyBB::INPUT_INT
                ),
                'candeleteposts' => $mybb->get_input(
                    'candeleteposts',
                    MyBB::INPUT_INT
                ),
                'candeletethreads' => $mybb->get_input(
                    'candeletethreads',
                    MyBB::INPUT_INT
                ),
                'caneditattachments' => $mybb->get_input(
                    'caneditattachments',
                    MyBB::INPUT_INT
                ),
                'canviewdeletionnotice' => $mybb->get_input(
                    'canviewdeletionnotice',
                    MyBB::INPUT_INT
                ),
                'canpostpolls' => $mybb->get_input(
                    'canpostpolls',
                    MyBB::INPUT_INT
                ),
                'canvotepolls' => $mybb->get_input(
                    'canvotepolls',
                    MyBB::INPUT_INT
                ),
                'canundovotes' => $mybb->get_input(
                    'canundovotes',
                    MyBB::INPUT_INT
                ),
                'canusepms' => $mybb->get_input('canusepms', MyBB::INPUT_INT),
                'cansendpms' => $mybb->get_input('cansendpms', MyBB::INPUT_INT),
                'cantrackpms' => $mybb->get_input(
                    'cantrackpms',
                    MyBB::INPUT_INT
                ),
                'candenypmreceipts' => $mybb->get_input(
                    'candenypmreceipts',
                    MyBB::INPUT_INT
                ),
                'pmquota' => $mybb->get_input('pmquota', MyBB::INPUT_INT),
                'maxpmrecipients' => $mybb->get_input(
                    'maxpmrecipients',
                    MyBB::INPUT_INT
                ),
                'cansendemail' => $mybb->get_input(
                    'cansendemail',
                    MyBB::INPUT_INT
                ),
                'cansendemailoverride' => $mybb->get_input(
                    'cansendemailoverride',
                    MyBB::INPUT_INT
                ),
                'maxemails' => $mybb->get_input('maxemails', MyBB::INPUT_INT),
                'emailfloodtime' => $mybb->get_input(
                    'emailfloodtime',
                    MyBB::INPUT_INT
                ),
                'canviewmemberlist' => $mybb->get_input(
                    'canviewmemberlist',
                    MyBB::INPUT_INT
                ),
                'canviewcalendar' => $mybb->get_input(
                    'canviewcalendar',
                    MyBB::INPUT_INT
                ),
                'canaddevents' => $mybb->get_input(
                    'canaddevents',
                    MyBB::INPUT_INT
                ),
                'canbypasseventmod' => $mybb->get_input(
                    'canbypasseventmod',
                    MyBB::INPUT_INT
                ),
                'canmoderateevents' => $mybb->get_input(
                    'canmoderateevents',
                    MyBB::INPUT_INT
                ),
                'canviewonline' => $mybb->get_input(
                    'canviewonline',
                    MyBB::INPUT_INT
                ),
                'canviewwolinvis' => $mybb->get_input(
                    'canviewwolinvis',
                    MyBB::INPUT_INT
                ),
                'canviewonlineips' => $mybb->get_input(
                    'canviewonlineips',
                    MyBB::INPUT_INT
                ),
                ...
                'canuploadavatars' => $mybb->get_input(
                    'canuploadavatars',
                    MyBB::INPUT_INT
                ),
                'canchangename' => $mybb->get_input(
                    'canchangename',
                    MyBB::INPUT_INT
                ),
                'canbereported' => $mybb->get_input(
                    'canbereported',
                    MyBB::INPUT_INT
                ),
                'canbeinvisible' => $mybb->get_input(
                    'canbeinvisible',
                    MyBB::INPUT_INT
                ),
                'canchangewebsite' => $mybb->get_input(
                    'canchangewebsite',
                    MyBB::INPUT_INT
                ),
                'showforumteam' => $mybb->get_input(
                    'showforumteam',
                    MyBB::INPUT_INT
                ),
                'usereputationsystem' => $mybb->get_input(
                    'usereputationsystem',
                    MyBB::INPUT_INT
                ),
                'cangivereputations' => $mybb->get_input(
                    'cangivereputations',
                    MyBB::INPUT_INT
                ),
                'candeletereputations' => $mybb->get_input(
                    'candeletereputations',
                    MyBB::INPUT_INT
                ),
                'reputationpower' => $mybb->get_input(
                    'reputationpower',
                    MyBB::INPUT_INT
                ),
                'maxreputationsday' => $mybb->get_input(
                    'maxreputationsday',
                    MyBB::INPUT_INT
                ),
                'maxreputationsperuser' => $mybb->get_input(
                    'maxreputationsperuser',
                    MyBB::INPUT_INT
                ),
                'maxreputationsperthread' => $mybb->get_input(
                    'maxreputationsperthread',
                    MyBB::INPUT_INT
                ),
                'attachquota' => $mybb->get_input(
                    'attachquota',
                    MyBB::INPUT_INT
                ),
                'cancustomtitle' => $mybb->get_input(
                    'cancustomtitle',
                    MyBB::INPUT_INT
                ),
                'canwarnusers' => $mybb->get_input(
                    'canwarnusers',
                    MyBB::INPUT_INT
                ),
                'canreceivewarnings' => $mybb->get_input(
                    'canreceivewarnings',
                    MyBB::INPUT_INT
                ),
                'maxwarningsday' => $mybb->get_input(
                    'maxwarningsday',
                    MyBB::INPUT_INT
                ),
                'canmodcp' => $mybb->get_input('canmodcp', MyBB::INPUT_INT),
                'showinbirthdaylist' => $mybb->get_input(
                    'showinbirthdaylist',
                    MyBB::INPUT_INT
                ),
                'canoverridepm' => $mybb->get_input(
                    'canoverridepm',
                    MyBB::INPUT_INT
                ),
                'canusesig' => $mybb->get_input('canusesig', MyBB::INPUT_INT),
                'canusesigxposts' => $mybb->get_input(
                    'canusesigxposts',
                    MyBB::INPUT_INT
                ),
                'signofollow' => $mybb->get_input(
                    'signofollow',
                    MyBB::INPUT_INT
                ),
                'edittimelimit' => $mybb->get_input(
                    'edittimelimit',
                    MyBB::INPUT_INT
                ),
                'maxposts' => $mybb->get_input('maxposts', MyBB::INPUT_INT),
                'showmemberlist' => $mybb->get_input(
                    'showmemberlist',
                    MyBB::INPUT_INT
                ),
                'canmanageannounce' => $mybb->get_input(
                    'canmanageannounce',
                    MyBB::INPUT_INT
                ),
                'canmanagemodqueue' => $mybb->get_input(
                    'canmanagemodqueue',
                    MyBB::INPUT_INT
                ),
                'canmanagereportedcontent' => $mybb->get_input(
                    'canmanagereportedcontent',
                    MyBB::INPUT_INT
                ),
                'canviewmodlogs' => $mybb->get_input(
                    'canviewmodlogs',
                    MyBB::INPUT_INT
                ),
                'caneditprofiles' => $mybb->get_input(
                    'caneditprofiles',
                    MyBB::INPUT_INT
                ),
                'canbanusers' => $mybb->get_input(
                    'canbanusers',
                    MyBB::INPUT_INT
                ),
                'canviewwarnlogs' => $mybb->get_input(
                    'canviewwarnlogs',
                    MyBB::INPUT_INT
                ),
                'canuseipsearch' => $mybb->get_input(
                    'canuseipsearch',
                    MyBB::INPUT_INT
                ),
            ];
            ...
            if ($usergroup['type'] != 1) {
                $updated_group['candisplaygroup'] = $mybb->get_input(
                    'candisplaygroup',
                    MyBB::INPUT_INT
                );
            }
            ...
            if (
                ...
                $message = $lang->sprintf(
                    $lang->success_group_updated_duplicate_title,
                    htmlspecialchars_uni($mybb->input['title'])
                );
            }
    ...
    $form = new Form(
        "index.php?module=user-groups&amp;action=edit&amp;gid={$usergroup['gid']}",
        'post'
    );
    ...
    $form_container->output_row(
        $lang->username_style,
        $lang->username_style_desc,
        $form->generate_text_box('namestyle', $mybb->input['namestyle'], [
            'id' => 'namestyle',
        ]),
        'namestyle'
    );
    $form_container->output_row(
        $lang->user_title,
        $lang->user_title_desc,
        $form->generate_text_box('usertitle', $mybb->input['usertitle'], [
            'id' => 'usertitle',
        ]),
        'usertitle'
    );
    ...
    $stars =
        ...
        $form->generate_numeric_field('stars', $mybb->input['stars'], [
            'class' => 'field50',
            'id' => 'stars',
            'min' => 0,
        ]) .
        '</td><td>' .
        $form->generate_text_box('starimage', $mybb->input['starimage'], [
            'id' => 'starimage',
        ]) .
        '</td></tr>';
    ...
    $general_options[] = $form->generate_check_box(
        ...
        1,
        $lang->member_list,
        ['checked' => $mybb->input['showmemberlist']]
    );
    if ($usergroup['gid'] != '1' && $usergroup['gid'] != '5') {
        $general_options[] = $form->generate_check_box(
            ...
            1,
            $lang->forum_team,
            ['checked' => $mybb->input['showforumteam']]
        );
    }
    $general_options[] = $form->generate_check_box(
        'isbannedgroup',
        1,
        $lang->is_banned_group,
        ['checked' => $mybb->input['isbannedgroup']]
    );

    $form_container->output_row(
        $lang->general_options,
        '',
        "<div class=\"group_settings_bit\">" .
            implode(
    ...
    if ($usergroup['type'] != 1) {
        $public_options = [
            $form->generate_check_box('joinable', 1, $lang->user_joinable, [
                'checked' => $mybb->input['joinable'],
            ]),
            $form->generate_check_box(
                'moderate',
                1,
                $lang->moderate_join_requests,
                ['checked' => $mybb->input['moderate']]
            ),
            $form->generate_check_box('invite', 1, $lang->invite_only, [
                'checked' => $mybb->input['invite'],
            ]),
            $form->generate_check_box(
                'candisplaygroup',
                1,
                $lang->can_set_as_display_group,
                ['checked' => $mybb->input['candisplaygroup']]
            ),
        ];
        $form_container->output_row(
            $lang->publicly_joinable_options,
            '',
            "<div class=\"group_settings_bit\">" .
                implode(
                    "</div><div class=\"group_settings_bit\">",
    ...
    $admin_options = [
        $form->generate_check_box('issupermod', 1, $lang->is_super_mod, [
            'checked' => $mybb->input['issupermod'],
        ]),
        $form->generate_check_box('canmodcp', 1, $lang->can_access_mod_cp, [
            'checked' => $mybb->input['canmodcp'],
        ]),
        $form->generate_check_box('cancp', 1, $lang->can_access_admin_cp, [
            'checked' => $mybb->input['cancp'],
        ]),
    ];
    $form_container->output_row(
        $lang->moderation_administration_options,
        '',
        "<div class=\"group_settings_bit\">" .
            implode(
                "</div><div class=\"group_settings_bit\">",
    ...
    $viewing_options = [
        $form->generate_check_box('canview', 1, $lang->can_view_board, [
            'checked' => $mybb->input['canview'],
        ]),
        $form->generate_check_box(
            'canviewthreads',
            1,
            $lang->can_view_threads,
            ['checked' => $mybb->input['canviewthreads']]
        ),
        $form->generate_check_box('cansearch', 1, $lang->can_search_forums, [
            'checked' => $mybb->input['cansearch'],
        ]),
        $form->generate_check_box(
            'canviewprofiles',
            1,
            $lang->can_view_profiles,
            ['checked' => $mybb->input['canviewprofiles']]
        ),
        $form->generate_check_box(
            'candlattachments',
            1,
            $lang->can_download_attachments,
            ['checked' => $mybb->input['candlattachments']]
        ),
        $form->generate_check_box(
            'canviewboardclosed',
            1,
            $lang->can_view_board_closed,
            ['checked' => $mybb->input['canviewboardclosed']]
        ),
    ];
    $form_container->output_row(
        $lang->viewing_options,
        '',
        "<div class=\"group_settings_bit\">" .
            implode(
                "</div><div class=\"group_settings_bit\">",
    ...
    $posting_options = [
        $form->generate_check_box(
            'canpostthreads',
            1,
            $lang->can_post_threads,
            ['checked' => $mybb->input['canpostthreads']]
        ),
        $form->generate_check_box('canpostreplys', 1, $lang->can_post_replies, [
            'checked' => $mybb->input['canpostreplys'],
        ]),
        $form->generate_check_box(
            'canratethreads',
            1,
            $lang->can_rate_threads,
            ['checked' => $mybb->input['canratethreads']]
        ),
        "{$lang->max_posts_per_day}<br /><small class=\"input\">{$lang->max_posts_per_day_desc}</small><br />" .
        $form->generate_numeric_field('maxposts', $mybb->input['maxposts'], [
            'id' => 'maxposts',
            'class' => 'field50',
            'min' => 0,
        ]),
    ...
    $moderator_options = [
        $form->generate_check_box('modposts', 1, $lang->mod_new_posts, [
            'checked' => $mybb->input['modposts'],
        ]),
        $form->generate_check_box('modthreads', 1, $lang->mod_new_threads, [
            'checked' => $mybb->input['modthreads'],
        ]),
        $form->generate_check_box(
            'modattachments',
            1,
            $lang->mod_new_attachments,
            ['checked' => $mybb->input['modattachments']]
        ),
        $form->generate_check_box('mod_edit_posts', 1, $lang->mod_after_edit, [
            'checked' => $mybb->input['mod_edit_posts'],
        ]),
    ];
    $form_container->output_row(
        $lang->moderation_options,
        '',
        "<div class=\"group_settings_bit\">" .
            implode(
                "</div><div class=\"group_settings_bit\">",
    ...
    $poll_options = [
        $form->generate_check_box('canpostpolls', 1, $lang->can_post_polls, [
            'checked' => $mybb->input['canpostpolls'],
        ]),
        $form->generate_check_box('canvotepolls', 1, $lang->can_vote_polls, [
            'checked' => $mybb->input['canvotepolls'],
        ]),
        $form->generate_check_box('canundovotes', 1, $lang->can_undo_votes, [
            'checked' => $mybb->input['canundovotes'],
        ]),
    ];
    $form_container->output_row(
        $lang->poll_options,
        '',
        "<div class=\"group_settings_bit\">" .
            implode("</div><div class=\"group_settings_bit\">", $poll_options) .
            '</div>'
    );
    ...
    $attachment_options = [
        $form->generate_check_box(
            'canpostattachments',
            1,
            $lang->can_post_attachments,
            ['checked' => $mybb->input['canpostattachments']]
        ),
        "{$lang->attach_quota}<br /><small class=\"input\">{$lang->attach_quota_desc}</small><br />" .
        $form->generate_numeric_field(
            'attachquota',
            $mybb->input['attachquota'],
            ['id' => 'attachquota', 'class' => 'field50', 'min' => 0]
        ) .
        'KB',
    ];
    $form_container->output_row(
        $lang->attachment_options,
        '',
        "<div class=\"group_settings_bit\">" .
            implode(
                "</div><div class=\"group_settings_bit\">",
    ...
    if ($usergroup['gid'] != 1) {
        $editing_options = [
            $form->generate_check_box(
                ...
                1,
                $lang->can_edit_posts,
                ['checked' => $mybb->input['caneditposts']]
            ),
            $form->generate_check_box(
                'candeleteposts',
                1,
                $lang->can_delete_posts,
                ['checked' => $mybb->input['candeleteposts']]
            ),
            $form->generate_check_box(
                'candeletethreads',
                1,
                $lang->can_delete_threads,
                ['checked' => $mybb->input['candeletethreads']]
            ),
            $form->generate_check_box(
                'caneditattachments',
                1,
                $lang->can_edit_attachments,
                ['checked' => $mybb->input['caneditattachments']]
            ),
            $form->generate_check_box(
                'canviewdeletionnotice',
                1,
                $lang->can_view_deletion_notices,
                ['checked' => $mybb->input['canviewdeletionnotice']]
            ),
            "{$lang->edit_time_limit}<br /><small class=\"input\">{$lang->edit_time_limit_desc}</small><br />" .
            $form->generate_numeric_field(
                'edittimelimit',
                $mybb->input['edittimelimit'],
                ['id' => 'edittimelimit', 'class' => 'field50', 'min' => 0]
            ),
        ];
        $form_container->output_row(
            $lang->editing_deleting_options,
            '',
            "<div class=\"group_settings_bit\">" .
                implode(
                    "</div><div class=\"group_settings_bit\">",
    ...
    $account_options = [
        $form->generate_check_box('canbereported', 1, $lang->can_be_reported, [
            'checked' => $mybb->input['canbereported'],
        ]),
        $form->generate_check_box(
            'canbeinvisible',
            1,
            $lang->can_be_invisible,
            ['checked' => $mybb->input['canbeinvisible']]
        ),
        $form->generate_check_box('canusercp', 1, $lang->can_access_usercp, [
            'checked' => $mybb->input['canusercp'],
        ]),
        $form->generate_check_box(
            'canchangename',
            1,
            $lang->can_change_username,
            ['checked' => $mybb->input['canchangename']]
        ),
        $form->generate_check_box(
            'cancustomtitle',
            1,
            $lang->can_use_usertitles,
            ['checked' => $mybb->input['cancustomtitle']]
        ),
        $form->generate_check_box(
            'canuploadavatars',
            1,
            $lang->can_upload_avatars,
            ['checked' => $mybb->input['canuploadavatars']]
        ),
        $form->generate_check_box('canusesig', 1, $lang->can_use_signature, [
            'checked' => $mybb->input['canusesig'],
        ]),
        $form->generate_check_box('signofollow', 1, $lang->uses_no_follow, [
            'checked' => $mybb->input['signofollow'],
        ]),
        $form->generate_check_box(
            'canchangewebsite',
            1,
            $lang->can_change_website,
            ['checked' => $mybb->input['canchangewebsite']]
        ),
        "{$lang->required_posts}<br /><small class=\"input\">{$lang->required_posts_desc}</small><br />" .
        $form->generate_numeric_field(
            'canusesigxposts',
            $mybb->input['canusesigxposts'],
            ['id' => 'canusesigxposts', 'class' => 'field50', 'min' => 0]
        ),
    ];
    $form_container->output_row(
    ...
    $reputation_options = [
        $form->generate_check_box(
            ...
            ['checked' => $mybb->input['usereputationsystem']]
        ),
        $form->generate_check_box(
            'cangivereputations',
            1,
            $lang->can_give_reputation,
            ['checked' => $mybb->input['cangivereputations']]
        ),
        $form->generate_check_box(
            'candeletereputations',
            1,
            $lang->can_delete_own_reputation,
            ['checked' => $mybb->input['candeletereputations']]
        ),
        "{$lang->points_to_award_take}<br /><small class=\"input\">{$lang->points_to_award_take_desc}</small><br />" .
        $form->generate_numeric_field(
            'reputationpower',
            $mybb->input['reputationpower'],
            ['id' => 'reputationpower', 'class' => 'field50', 'min' => 0]
        ),
        "{$lang->max_reputations_perthread}<br /><small class=\"input\">{$lang->max_reputations_perthread_desc}</small><br />" .
        $form->generate_numeric_field(
            'maxreputationsperthread',
            $mybb->input['maxreputationsperthread'],
            [
                'id' => 'maxreputationsperthread',
                'class' => 'field50',
                'min' => 0,
            ]
        ),
        "{$lang->max_reputations_peruser}<br /><small class=\"input\">{$lang->max_reputations_peruser_desc}</small><br />" .
        $form->generate_numeric_field(
            'maxreputationsperuser',
            $mybb->input['maxreputationsperuser'],
            ['id' => 'maxreputationsperuser', 'class' => 'field50', 'min' => 0]
        ),
        "{$lang->max_reputations_daily}<br /><small class=\"input\">{$lang->max_reputations_daily_desc}</small><br />" .
        $form->generate_numeric_field(
            'maxreputationsday',
            $mybb->input['maxreputationsday'],
            ['id' => 'maxreputationsday', 'class' => 'field50', 'min' => 0]
        ),
    ];
    $form_container->output_row(
        $lang->reputation_system,
        '',
        "<div class=\"group_settings_bit\">" .
            implode(
                "</div><div class=\"group_settings_bit\">",
    ...
    $warning_options = [
        $form->generate_check_box('canwarnusers', 1, $lang->can_send_warnings, [
            'checked' => $mybb->input['canwarnusers'],
        ]),
        $form->generate_check_box(
            'canreceivewarnings',
            1,
            $lang->can_receive_warnings,
            ['checked' => $mybb->input['canreceivewarnings']]
        ),
        "{$lang->warnings_per_day}<br />" .
        $form->generate_numeric_field(
            'maxwarningsday',
            $mybb->input['maxwarningsday'],
            ['id' => 'maxwarningsday', 'class' => 'field50']
        ),
    ];
    $form_container->output_row(
        $lang->warning_system,
    ...
    $pm_options = [
        $form->generate_check_box('canusepms', 1, $lang->can_use_pms, [
            'checked' => $mybb->input['canusepms'],
        ]),
        $form->generate_check_box('cansendpms', 1, $lang->can_send_pms, [
            'checked' => $mybb->input['cansendpms'],
        ]),
        $form->generate_check_box('canoverridepm', 1, $lang->can_override_pms, [
            'checked' => $mybb->input['canoverridepm'],
        ]),
        $form->generate_check_box('cantrackpms', 1, $lang->can_track_pms, [
            'checked' => $mybb->input['cantrackpms'],
        ]),
        $form->generate_check_box(
            'candenypmreceipts',
            1,
            $lang->can_deny_reciept,
            ['checked' => $mybb->input['candenypmreceipts']]
        ),
        "{$lang->message_quota}<br /><small>{$lang->message_quota_desc}</small><br />" .
        $form->generate_numeric_field('pmquota', $mybb->input['pmquota'], [
            'id' => 'pmquota',
            'class' => 'field50',
            'min' => 0,
        ]),
        "{$lang->max_recipients}<br /><small>{$lang->max_recipients_desc}</small><br />" .
        $form->generate_numeric_field(
            'maxpmrecipients',
            $mybb->input['maxpmrecipients'],
            ['id' => 'maxpmrecipients', 'class' => 'field50', 'min' => 0]
        ),
    ];
    $form_container->output_row(
        ...
        '',
        "<div class=\"group_settings_bit\">" .
            implode("</div><div class=\"group_settings_bit\">", $pm_options) .
            '</div>'
    );
    ...
    $calendar_options = [
        $form->generate_check_box(
            ...
            ['checked' => $mybb->input['canviewcalendar']]
        ),
        $form->generate_check_box('canaddevents', 1, $lang->can_post_events, [
            'checked' => $mybb->input['canaddevents'],
        ]),
        $form->generate_check_box(
            'canbypasseventmod',
            1,
            $lang->can_bypass_event_moderation,
            ['checked' => $mybb->input['canbypasseventmod']]
        ),
        $form->generate_check_box(
            'canmoderateevents',
            1,
            $lang->can_moderate_events,
            ['checked' => $mybb->input['canmoderateevents']]
        ),
    ];
    $form_container->output_row(
        $lang->calendar,
    ...
    $wol_options = [
        $form->generate_check_box(
            ...
            ['checked' => $mybb->input['canviewonline']]
        ),
        $form->generate_check_box(
            'canviewwolinvis',
            1,
            $lang->can_view_invisible,
            ['checked' => $mybb->input['canviewwolinvis']]
        ),
        $form->generate_check_box('canviewonlineips', 1, $lang->can_view_ips, [
            'checked' => $mybb->input['canviewonlineips'],
        ]),
    ];
    $form_container->output_row(
        $lang->whos_online,
        '',
        "<div class=\"group_settings_bit\">" .
            implode("</div><div class=\"group_settings_bit\">", $wol_options) .
            '</div>'
    );
    ...
    $misc_options = [
        $form->generate_check_box(
            ...
            ['checked' => $mybb->input['canviewmemberlist']]
        ),
        $form->generate_check_box(
            'showinbirthdaylist',
            1,
            $lang->show_in_birthday_list,
            ['checked' => $mybb->input['showinbirthdaylist']]
        ),
        $form->generate_check_box('cansendemail', 1, $lang->can_email_users, [
            'checked' => $mybb->input['cansendemail'],
        ]),
        $form->generate_check_box(
            'cansendemailoverride',
            1,
            $lang->can_email_users_override,
            ['checked' => $mybb->input['cansendemailoverride']]
        ),
        "{$lang->max_emails_per_day}<br /><small class=\"input\">{$lang->max_emails_per_day_desc}</small><br />" .
        $form->generate_numeric_field('maxemails', $mybb->input['maxemails'], [
            'id' => 'maxemails',
            'class' => 'field50',
            'min' => 0,
        ]),
        "{$lang->email_flood_time}<br /><small class=\"input\">{$lang->email_flood_time_desc}</small><br />" .
        $form->generate_numeric_field(
            'emailfloodtime',
            $mybb->input['emailfloodtime'],
            ['id' => 'emailfloodtime', 'class' => 'field50', 'min' => 0]
        ),
    ];
    $form_container->output_row(
        $lang->misc,
        '',
        "<div class=\"group_settings_bit\">" .
            implode("</div><div class=\"group_settings_bit\">", $misc_options) .
            '</div>'
    );
    ...
    $forum_post_options = [
        $form->generate_check_box(
            'canmanageannounce',
            1,
            $lang->can_manage_announce,
            ['checked' => $mybb->input['canmanageannounce']]
        ),
        $form->generate_check_box(
            'canmanagemodqueue',
            1,
            $lang->can_manage_mod_queue,
            ['checked' => $mybb->input['canmanagemodqueue']]
        ),
        $form->generate_check_box(
            'canmanagereportedcontent',
            1,
            $lang->can_manage_reported_content,
            ['checked' => $mybb->input['canmanagereportedcontent']]
        ),
        $form->generate_check_box(
            'canviewmodlogs',
            1,
            $lang->can_view_mod_logs,
            ['checked' => $mybb->input['canviewmodlogs']]
        ),
    ];
    $form_container->output_row(
        $lang->forum_post_options,
        '',
        "<div class=\"group_settings_bit\">" .
            implode(
                "</div><div class=\"group_settings_bit\">",
    ...
    $user_options = [
        $form->generate_check_box(
            'caneditprofiles',
            1,
            $lang->can_edit_profiles,
            ['checked' => $mybb->input['caneditprofiles']]
        ),
        $form->generate_check_box('canbanusers', 1, $lang->can_ban_users, [
            'checked' => $mybb->input['canbanusers'],
        ]),
        $form->generate_check_box(
            'canviewwarnlogs',
            1,
            $lang->can_view_warnlogs,
            ['checked' => $mybb->input['canviewwarnlogs']]
        ),
        $form->generate_check_box(
            'canuseipsearch',
            1,
            $lang->can_use_ipsearch,
            ['checked' => $mybb->input['canuseipsearch']]
        ),
    ];
    $form_container->output_row(
        $lang->user_options,
        '',
        "<div class=\"group_settings_bit\">" .
            implode("</div><div class=\"group_settings_bit\">", $user_options) .
            '</div>'
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        switch ($db->type) {
            ...
            case 'sqlite':
                $query = $db->simple_select(
                    'users',
                    'uid',
                    "','||additionalgroups||',' LIKE '%,{$usergroup['gid']},%'"
                );
                break;
            default:
                $query = $db->simple_select(
                    'users',
                    'uid',
                    "CONCAT(',',additionalgroups,',') LIKE '%,{$usergroup['gid']},%'"
                );
        }
        ...
        $db->update_query('banned', ['gid' => 7], "gid='{$usergroup['gid']}'");
        $db->update_query(
            'banned',
            ['oldgroup' => 2],
            "oldgroup='{$usergroup['gid']}'"
        );
        $db->update_query(
            'banned',
            ['olddisplaygroup' => 'oldgroup'],
            "olddisplaygroup='{$usergroup['gid']}'",
            '',
            true
        ); // No quotes = displaygroup=usergroup
        ...
        $db->delete_query(
            'moderators',
            "id='{$usergroup['gid']}' AND isgroup='1'"
        );
        ...
        $page->output_confirm_action(
            "index.php?module=user-groups&amp;action=delete&amp;gid={$usergroup['gid']}",
            $lang->confirm_group_deletion
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "usergroups g ON (g.gid=u.usergroup)
		GROUP BY g.gid
	"
    );
    ...
    switch ($db->type) {
        ...
        case 'sqlite':
            $query = $db->query(
				...
				LEFT JOIN " .
                    TABLE_PREFIX .
                    "usergroups g ON (','|| u.additionalgroups|| ',' LIKE '%,'|| g.gid|| ',%')
        ...
        default:
            $query = $db->query(
				...
				LEFT JOIN " .
                    TABLE_PREFIX .
                    "usergroups g ON (CONCAT(',', u.additionalgroups, ',') LIKE CONCAT('%,', g.gid, ',%'))
				WHERE g.gid != '0' AND g.gid is not NULL GROUP BY g.gid
			"
            );
    }
    ...
    while ($leader = $db->fetch_array($query)) {
        $leaders[$leader['gid']][] = build_profile_link(
    ...
    $form_container->output_row_header($lang->controls, [
        'class' => 'align_center',
    ]);
    ...
    while ($usergroup = $db->fetch_array($query)) {
        ...
        if (isset($leaders[$usergroup['gid']])) {
            $leaders_list =
                "<br />{$lang->group_leaders}: " .
                implode($lang->comma, $leaders[$usergroup['gid']]);
        }

        $join_requests = '';
        if (
            isset($joinrequests[$usergroup['gid']]) &&
            $joinrequests[$usergroup['gid']] > 1 &&
            $usergroup['type'] == 4
        ) {
        ...
        $form_container->output_cell(
            "<div class=\"float_right\">{$icon}</div><div><strong><a href=\"index.php?module=user-groups&amp;action=edit&amp;gid={$usergroup['gid']}\">" .
                format_name(
                    htmlspecialchars_uni($usergroup['title']),
                    $usergroup['gid']
                ) .
                "</a></strong>{$join_requests}<br /><small>" .
                htmlspecialchars_uni($usergroup['description']) .
                "{$leaders_list}</small></div>"
        );
        ...
        $form_container->output_cell(my_number_format($numusers), [
            'class' => 'align_center',
        ]);
        ...
        if ($usergroup['showforumteam'] == 1) {
            $form_container->output_cell(
                $form->generate_numeric_field(
                    "disporder[{$usergroup['gid']}]",
                    "{$usergroup['disporder']}",
                    ['class' => 'align_center', 'style' => 'width:80%']
                ),
                ['class' => 'align_center']
            );
        } else {
        ...
        $popup->add_item(
            $lang->list_users,
            "index.php?module=user-users&amp;action=search&amp;results=1&amp;conditions[usergroup]={$usergroup['gid']}"
        );
        if (
            isset($joinrequests[$usergroup['gid']]) &&
            $joinrequests[$usergroup['gid']] > 0 &&
            $usergroup['type'] == 4
        ) {
            $popup->add_item(
                $lang->join_requests,
                "index.php?module=user-groups&amp;action=join_requests&amp;gid={$usergroup['gid']}"
            );
        }
        $popup->add_item(
            $lang->group_leaders,
            "index.php?module=user-groups&amp;action=leaders&amp;gid={$usergroup['gid']}"
        );
        if ($usergroup['type'] > 1) {
            $popup->add_item(
                $lang->delete_group,
                "index.php?module=user-groups&amp;action=delete&amp;gid={$usergroup['gid']}&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '{$lang->confirm_group_deletion}')"
            );
        }
    ...
    if ($form_container->num_rows() == 0) {
        $form_container->output_cell($lang->no_groups, ['colspan' => 4]);
        $form_container->construct_row();
    }
