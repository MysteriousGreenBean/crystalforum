...
if ($mybb->input['action'] == 'edit') {
    ...
    $sub_tabs['edit_mass_mail'] = [
        ...
        'link' =>
            'index.php?module=user-mass_mail&amp;action=edit&amp;mid=' .
            $email['mid'],
    ...
    $html_personalisation =
        substr($html_personalisation, 0, -2) . "');\n// --></script>\n";
    $text_personalisation =
        substr($text_personalisation, 0, -2) . "');\n// --></script>\n";
    ...
    if ($mybb->request_method == 'post') {
        ...
        // Delivering in the future
            ...
            $delivery_date =
                gmmktime(
                    $mybb->input['deliveryhour'],
                    $mybb->input['deliveryminute'],
                    0,
                    $mybb->input['endtime_month'],
                    $mybb->input['endtime_day'],
                    $mybb->input['endtime_year']
                ) - $localized_time_offset;
        ...
        if ($mybb->input['type'] == 1) {
            ...
            if (
                $mybb->input['format'] == 2 &&
                $mybb->get_input('automatic_text') == 0 &&
                !$mybb->input['message']
            ) {
            ...
            if (
                ($mybb->input['format'] == 1 || $mybb->input['format'] == 2) &&
                !$mybb->input['htmlmessage']
            ) {
                $errors[] = $lang->error_missing_html;
            } elseif ($mybb->input['format'] == 0 && !$mybb->input['message']) {
                $errors[] = $lang->error_missing_plain_text;
            }
        }
        ...
        if (!$errors) {
            ...
            // Sending via email
                ...
                if (
                    $mybb->input['format'] == 2 &&
                    $mybb->get_input('automatic_text')
                ) {
                    $mybb->input['message'] = create_text_message(
                        $mybb->input['htmlmessage']
                    );
                } elseif ($mybb->input['format'] == 1) {
                    $mybb->input['message'] = '';
                } elseif ($mybb->input['format'] == 0) {
                    $mybb->input['htmlmessage'] = '';
                }
            }
            ...
            $updated_email = [
                ...
                'totalcount' => $num,
                'conditions' => $db->escape_string(
                    my_serialize($mybb->input['conditions'])
                ),
                'message' => $db->escape_string($mybb->input['message']),
                'subject' => $db->escape_string($mybb->input['subject']),
                'htmlmessage' => $db->escape_string(
                    $mybb->input['htmlmessage']
                ),
    ...
    if ($errors) {
        ...
        if ($email['senddate'] != 0) {
            if ($email['senddate'] <= TIME_NOW) {
                ...
                $offset = $input['delivery_type'] = 'future';
                $time = gmdate(
                    'd-n-Y',
                    $email['senddate'] + $localized_time_offset
                );
                $time = explode('-', $time);
                $input['deliverymonth'] = (int) $time[1];
                $input['deliveryday'] = (int) $time[0];
                $input['deliveryyear'] = (int) $time[2];
                $input['endtime_time'] = gmdate(
    ...
    $form_container = new FormContainer(
        "{$lang->edit_mass_mail}: {$lang->message_settings}"
    );

    $form_container->output_row(
        "{$lang->subject}: <em>*</em>",
        $lang->subject_desc,
        $form->generate_text_box('subject', $input['subject'], [
            'id' => 'subject',
        ]),
        'subject'
    );
    ...
    $type_options = [
        $form->generate_radio_button('type', 0, $lang->send_via_email, [
            'id' => 'type_email',
            'checked' => $type_email_checked,
        ]),
        $form->generate_radio_button('type', 1, $lang->send_via_pm, [
            'id' => 'type_pm',
            'checked' => $type_pm_checked,
        ]),
    ];
    $form_container->output_row(
        "{$lang->message_type}: <em>*</em>",
        '',
        implode('<br />', $type_options)
    );
    ...
    $actions =
        ...
        $form->generate_text_box('endtime_time', $input['endtime_time'], [
            'id' => 'endtime_time',
            'style' => 'width: 60px;',
        ]) .
        "</td>
    ...
    $form_container->output_row(
        "{$lang->delivery_date}: <em>*</em>",
        $lang->delivery_date_desc,
        $actions
    );
    ...
    $form_container->output_row(
        "{$lang->per_page}: <em>*</em>",
        $lang->per_page_desc,
        $form->generate_numeric_field('perpage', $input['perpage'], [
            'id' => 'perpage',
            'min' => 1,
        ]),
        'perpage'
    ...
    $form_container = new FormContainer(
        "{$lang->edit_mass_mail}: {$lang->define_html_message}"
    );
    $form_container->output_row(
        "{$lang->define_html_message_desc}:",
        $html_personalisation,
        $form->generate_text_area('htmlmessage', $input['htmlmessage'], [
            'id' => 'htmlmessage',
            'rows' => 15,
            'cols ' => 70,
            'style' => 'width: 95%',
            ...
            $form->generate_check_box(
                'automatic_text',
                1,
                $lang->auto_gen_plain_text,
                ['checked' => $automatic_text_check, 'id' => 'automatic_text']
            ) .
            '</div>'
    );
    $form_container->end();
    ...
    $form_container = new FormContainer(
        "{$lang->edit_mass_mail}: {$lang->define_text_version}"
    );
    $form_container->output_row(
        "{$lang->define_text_version_desc}:",
        $text_personalisation,
        $form->generate_text_area('message', $input['message'], [
            'id' => 'message',
            'rows' => 15,
            'cols ' => 70,
            'style' => 'width: 95%',
    ...
    if (
        !isset($input['conditions']['usergroup']) ||
        !is_array($input['conditions']['usergroup'])
    ) {
        $input['conditions']['usergroup'] = [];
    }

    $form_container = new FormContainer(
        "{$lang->edit_mass_mail}: {$lang->define_the_recipients}"
    );
...
if ($mybb->input['action'] == 'send') {
    ...
    if ($mybb->get_input('step')) {
        $query = $db->simple_select(
            ...
            '*',
            "status=0 and mid='" .
                $mybb->get_input('mid', MyBB::INPUT_INT) .
                "'"
        );
        $email = $db->fetch_array($query);
    ...
    if ($mybb->get_input('step') == 4) {
        ...
        if ($mybb->request_method == 'post') {
            ...
            if (!$errors) {
                ...
                $plugins->run_hooks(
                    'admin_user_mass_email_send_finalize_commit'
                );
        ...
        if ($errors) {
            ...
            $delivery_type_checked[$input['delivery_type']] =
                " checked=\"checked\"";
        } else {
            $input = [];
            if ($email['senddate'] != 0) {
        ...
        $table = new Table();
        $table->construct_cell("<strong>{$lang->delivery_method}:</strong>", [
            'width' => '25%',
        ]);
        if ($email['type'] == 1) {
        ...
        $table->construct_cell(
            my_number_format($email['totalcount']) .
                " - <a href=\"index.php?module=user-mass_mail&amp;action=send&amp;step=3&amp;mid={$email['mid']}\">{$lang->change_recipient_conds}</a>"
        );
        $table->construct_row();

        $table->output(
            "{$lang->send_mass_mail}: {$lang->step_four} - {$lang->review_message}"
        );
        ...
        $form = new Form(
            "index.php?module=user-mass_mail&amp;action=send&amp;step=4&amp;mid={$email['mid']}",
            'post'
        );
        $form_container = new FormContainer(
            "{$lang->send_mass_mail}: {$lang->step_four} - {$lang->define_delivery_date}"
        );
        ...
        $form = new Form(
            "index.php?module=user-mass_mail&amp;action=send&amp;step=3&amp;mid={$email['mid']}",
            'post'
        );
        $page->output_nav_tabs($sub_tabs, 'send_mass_mail');
        ...
        $form_container = new FormContainer(
            "{$lang->send_mass_mail}: {$lang->step_three} - {$lang->define_the_recipients}"
        );
    ...
    elseif ($mybb->get_input('step') == 2) {
        ...
        $form = new Form(
            "index.php?module=user-mass_mail&amp;action=send&amp;step=2&amp;mid={$email['mid']}",
            'post'
        );
        $page->output_nav_tabs($sub_tabs, 'send_mass_mail');
        ...
        $form_container = new FormContainer(
            "{$lang->send_mass_mail}: {$lang->step_two} - {$lang->review_text_version}"
        );
        $form_container->output_row(
            "{$lang->review_text_version_desc}:",
            $text_personalisation,
            $form->generate_text_area('message', $email['message'], [
                'id' => 'message',
                'rows' => 15,
                'cols ' => 70,
                'style' => 'width: 95%',
        ...
        if ($mybb->request_method == 'post') {
            ...
            if (!$errors) {
                if (!$new_email['mid']) {
                    ...
                    $plugins->run_hooks(
                        'admin_user_mass_email_send_insert_commit'
                    );
                }
                // Updating an existing one
                    ...
                    $plugins->run_hooks(
                        'admin_user_mass_email_send_update_commit'
                    );
                ...
                admin_redirect(
                    "index.php?module=user-mass_mail&action=send&step={$next}&mid={$mid}"
                );
            }
        }
        ...
        $form = new Form(
            "index.php?module=user-mass_mail&amp;action=send{$mid_add}",
            'post'
        );
        $page->output_nav_tabs($sub_tabs, 'send_mass_mail');
        ...
        $form_container = new FormContainer(
            "{$lang->send_mass_mail}: {$lang->step_one} - {$lang->message_settings}"
        );
        ...
        $form_container->output_row(
            ...
            $form->generate_text_box(
                'subject',
                !empty($input['subject']) ? $input['subject'] : null,
                ['id' => 'subject']
            ),
            'subject'
        );
        ...
        if ($mybb->get_input('format') == 2) {
            ...
            $mybb->get_input('format') == 1 &&
            $mybb->get_input('type') != 1
        ) {
            $text_display = 'display: none;';
        } elseif (
            $mybb->get_input('format') == 0 ||
            $mybb->get_input('type') == 1
        ) {
            $html_display = 'display: none';
        }
        ...
        echo "<div id=\"message_html\" style=\"{$html_display}\">";
        $form_container = new FormContainer(
            "{$lang->send_mass_mail}: {$lang->step_one} - {$lang->define_html_message}"
        );
        $form_container->output_row(
            "{$lang->define_html_message_desc}:",
            $html_personalisation,
            $form->generate_text_area(
                'htmlmessage',
                !empty($input['htmlmessage']) ? $input['htmlmessage'] : null,
                [
                    'id' => 'htmlmessage',
                    'rows' => 15,
                    'cols ' => 70,
        ...
        echo "<div id=\"message_text\" style=\"{$text_display}\">";
        $form_container = new FormContainer(
            "{$lang->send_mass_mail}: {$lang->step_one} - {$lang->define_text_version}"
        );
        $form_container->output_row(
            "{$lang->define_text_version_desc}:",
            $text_personalisation,
            $form->generate_text_area(
                'message',
                !empty($input['message']) ? $input['message'] : null,
                [
                    'id' => 'message',
                    'rows' => 15,
                    'cols ' => 70,
    ...
    if ($mybb->request_method == 'post') {
        ...
        if ($mybb->input['archive'] == 1) {
            $page->output_confirm_action(
                "index.php?module=user-mass_mail&amp;action=delete&amp;mid={$mass_email['mid']}&amp;archive=1",
                $lang->mass_mail_deletion_confirmation
            );
        } else {
            $page->output_confirm_action(
                "index.php?module=user-mass_mail&amp;action=delete&amp;mid={$mass_email['mid']}",
                $lang->mass_mail_deletion_confirmation
            );
        }
    }
...
if ($mybb->input['action'] == 'resend') {
    ...
    // Need to perform the search to fetch the number of users we're emailing
    $member_query = build_mass_mail_query(
        my_unserialize($mass_email['conditions'])
    );
    ...
    admin_redirect(
        "index.php?module=user-mass_mail&action=send&step=4&mid={$mid}"
    );
    exit();
}
...
if ($mybb->input['action'] == 'archive') {
    ...
    $table->construct_header($lang->delivery_date, [
        'width' => '130',
        'class' => 'align_center',
    ]);
    $table->construct_header($lang->recipients, [
        'width' => '130',
        'class' => 'align_center',
    ]);
    ...
    $query = $db->simple_select('massemails', '*', 'status NOT IN (0, 1, 2)', [
        'order_by' => 'senddate',
    ]);
    while ($email = $db->fetch_array($query)) {
        ...
        $delivery_date = my_date(
            $mybb->settings['dateformat'],
            $email['senddate']
        );

        $table->construct_cell($delivery_date, ['class' => 'align_center']);
        $table->construct_cell(my_number_format($email['totalcount']), [
            'class' => 'align_center',
        ]);

        $table->construct_cell(
            "<a href=\"index.php?module=user-mass_mail&amp;action=resend&amp;mid={$email['mid']}\">{$lang->resend}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=user-mass_mail&amp;action=delete&amp;mid={$email['mid']}&amp;my_post_key={$mybb->post_code}&amp;archive=1\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->mass_mail_deletion_confirmation}')\">{$lang->delete}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_archived_messages, ['colspan' => 6]);
        $table->construct_row();
    }
...
if (!$mybb->input['action']) {
    ...
    $query = $db->simple_select('massemails', '*', 'status IN (0, 1, 2)', [
        'order_by' => 'senddate',
    ]);
    while ($email = $db->fetch_array($query)) {
        $email['subject'] = htmlspecialchars_uni($email['subject']);
        if (TIME_NOW >= $email['senddate'] && $email['status'] > 1) {
            $table->construct_cell(
                "<a href=\"index.php?module=user-mass_mail&amp;action=edit&amp;mid={$email['mid']}\"><strong>{$email['subject']}</strong></a>"
            );
        } else {
            $table->construct_cell("<strong>{$email['subject']}</strong>");
        }
        if ($email['status'] == 0) {
            ...
            $progress = ceil(
                ($email['sentcount'] / $email['totalcount']) * 100
            );
            if ($progress > 100) {
                $progress = 100;
            }
        ...
        if (TIME_NOW >= $email['senddate'] && $email['status'] > 1) {
            $table->construct_cell(
                "<a href=\"index.php?module=user-mass_mail&amp;action=cancel&amp;mid={$email['mid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->mass_mail_cancel_confirmation}')\">{$lang->cancel}</a>",
                ['width' => 100, 'colspan' => 2, 'class' => 'align_center']
            );
        } else {
            $table->construct_cell(
                "<a href=\"index.php?module=user-mass_mail&amp;action=edit&amp;mid={$email['mid']}\">{$lang->edit}</a>",
                ['width' => 100, 'class' => 'align_center']
            );
            $table->construct_cell(
                "<a href=\"index.php?module=user-mass_mail&amp;action=delete&amp;mid={$email['mid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->mass_mail_deletion_confirmation}')\">{$lang->delete}</a>",
                ['width' => 100, 'class' => 'align_center']
            );
        }
        $table->construct_row();
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_unsent_messages, ['colspan' => 6]);
        $table->construct_row();
    }
