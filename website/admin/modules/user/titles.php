...
if ($mybb->input['action'] == 'add') {
    ...
    $page->output_header(
        $lang->user_titles . ' - ' . $lang->add_new_user_title
    );
    ...
    $form_container->output_row(
        $lang->title_to_assign . '<em>*</em>',
        $lang->title_to_assign_desc,
    ...
    $form_container->output_row(
        $lang->minimum_posts,
        $lang->minimum_posts_desc,
        $form->generate_numeric_field('posts', $mybb->get_input('posts'), [
            'id' => 'posts',
            'min' => 0,
        ]),
        'posts'
    ...
    $form_container->output_row(
        $lang->number_of_stars,
        $lang->number_of_stars_desc,
        $form->generate_numeric_field('stars', $mybb->get_input('stars'), [
            'id' => 'stars',
            'min' => 0,
        ]),
        'stars'
    ...
    $form_container->output_row(
        $lang->star_image,
        $lang->star_image_desc,
        $form->generate_text_box('starimage', $mybb->get_input('starimage'), [
            'id' => 'starimage',
        ]),
        'starimage'
    );
...
if ($mybb->input['action'] == 'edit') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $query = $db->simple_select(
            ...
            "posts= '" .
                ...
                "' AND utid!= '" .
                $mybb->get_input('utid', MyBB::INPUT_INT) .
                "'"
        );
        if ($db->num_rows($query)) {
            $errors[] = $lang->error_cannot_have_same_posts;
    ...
    $sub_tabs['edit_title'] = [
        ...
        'link' =>
            'index.php?module=user-titles&amp;action=edit&amp;utid=' .
            $usertitle['utid'],
    ...
    $form = new Form(
        "index.php?module=user-titles&amp;action=edit&amp;utid={$usertitle['utid']}",
        'post'
    );
    ...
    $form_container->output_row(
        $lang->minimum_posts,
        $lang->minimum_posts_desc,
        $form->generate_numeric_field('posts', $mybb->input['posts'], [
            'id' => 'posts',
            'min' => 0,
        ]),
        'posts'
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=user-titles&amp;action=delete&amp;utid={$usertitle['utid']}",
            $lang->user_title_deletion_confirmation
        );
    }
}
...
if (!$mybb->input['action']) {
    ...
    $table->construct_header($lang->minimum_posts, [
        'width' => '130',
        'class' => 'align_center',
    ]);
    ...
    $query = $db->simple_select('usertitles', '*', '', ['order_by' => 'posts']);
    while ($usertitle = $db->fetch_array($query)) {
        $usertitle['title'] = htmlspecialchars_uni($usertitle['title']);
        $table->construct_cell(
            "<a href=\"index.php?module=user-titles&amp;action=edit&amp;utid={$usertitle['utid']}\"><strong>{$usertitle['title']}</strong></a>"
        );
        $table->construct_cell($usertitle['posts'], [
            'class' => 'align_center',
        ]);
        $table->construct_cell(
            "<a href=\"index.php?module=user-titles&amp;action=edit&amp;utid={$usertitle['utid']}\">{$lang->edit}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_cell(
            "<a href=\"index.php?module=user-titles&amp;action=delete&amp;utid={$usertitle['utid']}&amp;my_post_key={$mybb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->user_title_deletion_confirmation}')\">{$lang->delete}</a>",
            ['width' => 100, 'class' => 'align_center']
        );
        $table->construct_row();
    }
    ...
    if ($table->num_rows() == 0) {
        $table->construct_cell($lang->no_user_titles, ['colspan' => 4]);
        $table->construct_row();
    }
