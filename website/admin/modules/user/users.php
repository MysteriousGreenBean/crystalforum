...
if ($mybb->input['action'] == 'iplookup') {
    ...
    $lang->ipaddress_misc_info = $lang->sprintf(
        $lang->ipaddress_misc_info,
        htmlspecialchars_uni($mybb->input['ipaddress'])
    );
    ...
    if (!strstr($mybb->input['ipaddress'], '*')) {
        ...
        if (function_exists('geoip_record_by_name')) {
            ...
            if ($ip_record) {
                $ipaddress_location = htmlspecialchars_uni(
                    utf8_encode($ip_record['country_name'])
                );
                if ($ip_record['city']) {
                    $ipaddress_location .=
                        $lang->comma .
                        htmlspecialchars_uni(utf8_encode($ip_record['city']));
                }
            }
        }

        if (filter_var($mybb->input['ipaddress'], FILTER_VALIDATE_IP)) {
            $ipaddress_host_name = htmlspecialchars_uni(
                @gethostbyaddr($mybb->input['ipaddress'])
            );
...
if ($mybb->input['action'] == 'activate_user') {
    ...
    $message = $lang->sprintf(
        $lang->email_adminactivateaccount,
        $user['username'],
    ...
    my_mail(
        $user['email'],
        $lang->sprintf(
            $lang->emailsubject_activateaccount,
            $mybb->settings['bbname']
        ),
        $message
    );
...
if ($mybb->input['action'] == 'add') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            !empty($mybb->input['additionalgroups']) &&
            is_array($mybb->input['additionalgroups'])
        ) {
            ...
            $additionalgroups = implode(
                ',',
                array_map('intval', $mybb->input['additionalgroups'])
            );
        } else {
            $additionalgroups = '';
        }
        ...
        if (!$userhandler->validate_user()) {
            ...
            admin_redirect(
                "index.php?module=user-users&action=edit&uid={$user_info['uid']}"
            );
        }
    }

    // Fetch custom profile fields - only need required profile fields here
    $query = $db->simple_select('profilefields', '*', 'required=1', [
        'order_by' => 'disporder',
    ]);
    ...
    if ($errors) {
        ...
        $mybb->input = array_merge($mybb->input, ['usergroup' => 2]);
    }

    $mybb->input['profile_fields'] = $mybb->get_input(
        'profile_fields',
        MyBB::INPUT_ARRAY
    );
    ...
    $form_container->output_row(
        $lang->password . ' <em>*</em>',
        '',
        $form->generate_password_box('password', $mybb->get_input('password'), [
            'id' => 'password',
            'autocomplete' => 'off',
        ]),
        'password'
    ...
    $form_container->output_row(
        $lang->confirm_password . ' <em>*</em>',
        '',
        $form->generate_password_box(
            'confirm_password',
            $mybb->get_input('confirm_password'),
            ['id' => 'confirm_new_password']
        ),
        'confirm_new_password'
    );
    $form_container->output_row(
        $lang->email_address . ' <em>*</em>',
        '',
        $form->generate_text_box('email', $mybb->get_input('email'), [
            'id' => 'email',
        ]),
        'email'
    );
    ...
    while ($usergroup = $db->fetch_array($query)) {
        ...
        $display_group_options[$usergroup['gid']] = htmlspecialchars_uni(
            $usergroup['title']
        );
    }

    $form_container->output_row(
        $lang->primary_user_group . ' <em>*</em>',
        '',
    ...
    $form_container->output_row(
        $lang->additional_user_groups,
        $lang->additional_user_groups_desc,
        $form->generate_select_box(
            'additionalgroups[]',
            $options,
            $mybb->get_input('additionalgroups', MyBB::INPUT_ARRAY),
            ['id' => 'additionalgroups', 'multiple' => true, 'size' => 5]
        ),
        'additionalgroups'
    );
    $form_container->output_row(
        $lang->display_user_group . ' <em>*</em>',
        '',
        $form->generate_select_box(
            'displaygroup',
            $display_group_options,
            $mybb->get_input('displaygroup'),
            ['id' => 'displaygroup']
        ),
        'displaygroup'
    );
    ...
    output_custom_profile_fields(
        ...
        $mybb->get_input('profile_fields', MyBB::INPUT_ARRAY),
        $form_container,
        $form
    );
...
if ($mybb->input['action'] == 'edit') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if (
            is_super_admin($mybb->input['uid']) &&
            $mybb->user['uid'] != $mybb->input['uid'] &&
        ...
        if (!empty($mybb->input['away_day'])) {
            ...
            $return_year = min(
                $mybb->get_input('away_year', MyBB::INPUT_INT),
                9999
            );
            ...
            if (
                $return_year < my_date('Y', $awaydate) ||
                ($returntimestamp < $awaytimestamp &&
                    $return_year == my_date('Y', $awaydate))
            ) {
                $away_in_past = true;
            }
        ...
        if ($mybb->get_input('new_password')) {
            ...
            $updated_user['password2'] = $mybb->get_input(
                'confirm_new_password'
            );
        }
        ...
        if ($mybb->settings['usertppoptions']) {
            $updated_user['options']['tpp'] = $mybb->get_input(
                'tpp',
                MyBB::INPUT_INT
            );
        }
        ...
        if ($mybb->settings['userpppoptions']) {
            $updated_user['options']['ppp'] = $mybb->get_input(
                'ppp',
                MyBB::INPUT_INT
            );
        }
        ...
        if (!$userhandler->validate_user()) {
            ...
            elseif (
                ...
                $mybb->input['avatar_url'] != $user['avatar']
            ) {
                if (!$mybb->settings['allowremoteavatars']) {
                    $errors = [$lang->error_remote_avatar_not_allowed];
                } else {
                    if (
                        filter_var(
                        ...
                        // Gravatar
                        $email = md5(
                            strtolower(trim($mybb->input['avatar_url']))
                        );
                        ...
                        list($maxwidth, $maxheight) = preg_split(
                            '/[|x]/',
                            my_strtolower($mybb->settings['maxavatardims'])
                        );
                        ...
                        $mybb->input['avatar_url'] = preg_replace(
                            '#script:#i',
                            '',
                            $mybb->input['avatar_url']
                        );
                        ...
                        if (!$file) {
                            ...
                            $tmp_name =
                                '../' .
                                $mybb->settings['avataruploadpath'] .
                                '/remote_' .
                                md5(random_str());
                        ...
                        if (empty($avatar_error)) {
                            if (
                                ...
                                if (
                                    ...
                                    $lang->error_avatartoobig = $lang->sprintf(
                                        $lang->error_avatartoobig,
                                        $maxwidth,
                                        $maxheight
                                    );
            ...
            foreach ($moderator_options as $option) {
                if (empty($mybb->input[$option['action']])) {
                    ...
                    if (
                        (int) $mybb->input[$option['time']] == 0 &&
                        $mybb->input[$option['period']] != 'never' &&
                    ...
                    if (!is_array($errors)) {
                        $suspend_length = fetch_time_length(
                            (int) $mybb->input[$option['time']],
                            $mybb->input[$option['period']]
                        );

                        if (
                            $user[$option['update_field']] == 1 &&
                            ($mybb->input[$option['time']] ||
                                $mybb->input[$option['period']] == 'never')
                        ) {
                            ...
                            if ($suspend_length == '-1') {
                                ...
                                $extra_user_updates[$option['update_length']] =
                                    TIME_NOW + $suspend_length;
                            }
            ...
            if (
                !empty($extra_user_updates['moderateposts']) &&
                !empty($extra_user_updates['suspendposting'])
            ) {
            ...
            if (!$errors) {
                ...
                if (
                    $mybb->input['update_posts'] == 'enable' ||
                    $mybb->input['update_posts'] == 'disable'
                ) {
                    $update_signature = [
                        'includesig' =>
                            $mybb->input['update_posts'] == 'enable' ? 1 : 0,
                    ];
                    $db->update_query(
                        'posts',
                        $update_signature,
    ...
    if (
        $mybb->get_input('away_day') ||
        $mybb->get_input('away_month') ||
        $mybb->get_input('away_year')
    ) {
        $mybb->input['away_year'] = $mybb->get_input(
            'away_year',
            MyBB::INPUT_INT
        );
    } else {
        ...
        if ($user['returndate']) {
            list(
                $mybb->input['away_day'],
                $mybb->input['away_month'],
                $mybb->input['away_year'],
            ) = explode('-', $user['returndate']);
        }
    }

    // Fetch custom profile fields
    $query = $db->simple_select('profilefields', '*', '', [
        'order_by' => 'disporder',
    ]);
    ...
    $page->add_breadcrumb_item(
        $lang->edit_user . ': ' . htmlspecialchars_uni($user['username'])
    );
...
EOF;
    ...
    $form = new Form(
        "index.php?module=user-users&amp;action=edit&amp;uid={$user['uid']}",
        'post',
        '',
        1
    );
    ...
    $table = new Table();
    $table->construct_header($lang->avatar, ['class' => 'align_center']);
    $table->construct_header($lang->general_account_stats, [
        'colspan' => '2',
        'class' => 'align_center',
    ]);
    ...
    if (
        ...
        if (my_validate_url($mybb->settings['useravatar'])) {
            ...
            $user['avatar'] =
                '../' .
                str_replace('{theme}', 'images', $mybb->settings['useravatar']);
        }
    }
    ...
    $localtime = $lang->sprintf(
        $lang->local_time_format,
        $local_date,
        $local_time
    );
    ...
    if (
        $user_permissions['usereputationsystem'] == 1 &&
    ...
    if (
        ...
        $user_permissions['canreceivewarnings'] != 0
    ) {
        ...
        $warning_level = round(
            ($user['warningpoints'] / $mybb->settings['maxwarningpoints']) * 100
        );
    ...
    $table->construct_cell(
        "<div style=\"width: 126px; height: 126px;\" class=\"user_avatar\"><img src=\"" .
            htmlspecialchars_uni($user['avatar']) .
            "\" style=\"margin-top: {$avatar_top}px\" width=\"{$scaled_dimensions['width']}\" height=\"{$scaled_dimensions['height']}\" alt=\"\" /></div>",
        ['rowspan' => 6, 'width' => 1]
    );
    $table->construct_cell(
        "<strong>{$lang->email_address}:</strong> <a href=\"mailto:" .
            htmlspecialchars_uni($user['email']) .
            "\">" .
            htmlspecialchars_uni($user['email']) .
            '</a>'
    );
    $table->construct_cell(
        "<strong>{$lang->last_active}:</strong> {$last_active}"
    );
    $table->construct_row();
    $table->construct_cell(
        "<strong>{$lang->registration_date}:</strong> {$reg_date}"
    );
    $table->construct_cell(
    ...
    $table->construct_row();
    $table->construct_cell(
        "<strong>{$lang->posts_per_day}:</strong> {$posts_per_day}"
    );
    $table->construct_cell(
        "<strong>{$lang->reputation}:</strong> {$reputation}"
    );
    $table->construct_row();
    $table->construct_cell(
        "<strong>{$lang->percent_of_total_posts}:</strong> {$percent_posts}"
    );
    $table->construct_cell(
        "<strong>{$lang->warning_level}:</strong> {$warning_level}"
    );
    $table->construct_row();
    $table->construct_cell(
        "<strong>{$lang->registration_ip}:</strong> " .
            my_inet_ntop($db->unescape_binary($user['regip']))
    );
    $table->construct_cell(
        "<strong>{$lang->last_known_ip}:</strong> " .
            my_inet_ntop($db->unescape_binary($user['lastip']))
    );
    $table->construct_row();
    ...
    $form_container->output_row(
        $lang->username . ' <em>*</em>',
        '',
        $form->generate_text_box('username', $mybb->input['username'], [
            'id' => 'username',
        ]),
        'username'
    );
    $form_container->output_row(
        $lang->new_password,
        $lang->new_password_desc,
        $form->generate_password_box(
            'new_password',
            $mybb->input['new_password'],
    ...
    $form_container->output_row(
        ...
        $form->generate_password_box(
            'confirm_new_password',
            $mybb->input['confirm_new_password'],
            ['id' => 'confirm_new_password']
        ),
        'confirm_new_password'
    );
    $form_container->output_row(
        $lang->email_address . ' <em>*</em>',
        '',
        $form->generate_text_box('email', $mybb->input['email'], [
            'id' => 'email',
        ]),
        'email'
    );
    ...
    if (
        isset($mybb->input['additionalgroups']) &&
        !is_array($mybb->input['additionalgroups'])
    ) {
        $mybb->input['additionalgroups'] = explode(
            ',',
            $mybb->input['additionalgroups']
        );
    }
    ...
    $form_container->output_row(
        $lang->post_count . ' <em>*</em>',
        '',
        $form->generate_numeric_field('postnum', $mybb->get_input('postnum'), [
            'id' => 'postnum',
            'min' => 0,
        ]),
        'postnum'
    ...
    $form_container->output_row(
        $lang->thread_count . ' <em>*</em>',
        '',
        $form->generate_numeric_field(
            'threadnum',
            $mybb->get_input('threadnum'),
            ['id' => 'threadnum', 'min' => 0]
        ),
        'threadnum'
    );
    ...
    $form_container = new FormContainer(
        $lang->optional_profile_info .
            ': ' .
            htmlspecialchars_uni($user['username'])
    );
    $form_container->output_row(
        $lang->custom_user_title,
        $lang->custom_user_title_desc,
        $form->generate_text_box('usertitle', $mybb->get_input('usertitle'), [
            'id' => 'usertitle',
        ]),
        'usertitle'
    );
    ...
    $form_container->output_row(
        $lang->icq_number,
        '',
        $form->generate_numeric_field('icq', $mybb->get_input('icq'), [
            'id' => 'icq',
            'min' => 0,
        ]),
        'icq'
    ...
    $form_container->output_row(
        $lang->skype_handle,
        '',
        $form->generate_text_box('skype', $mybb->get_input('skype'), [
            'id' => 'skype',
        ]),
        'skype'
    );
    $form_container->output_row(
        $lang->google_handle,
        '',
        $form->generate_text_box('google', $mybb->get_input('google'), [
            'id' => 'google',
        ]),
        'google'
    );
    ...
    $birthday_row = $form->generate_select_box(
        'bday1',
        $birthday_days,
        $mybb->input['bday'][0],
        ['id' => 'bday_day']
    ...
    $birthday_row .=
        ' ' .
        $form->generate_numeric_field('bday3', $mybb->input['bday'][2], [
            'id' => 'bday_year',
            'style' => 'width: 4em;',
            'min' => 0,
        ]);
    ...
    output_custom_profile_fields(
        $profile_fields['optional'],
        $mybb->input['profile_fields'],
        $form_container,
        $form
    );
    ...
    if ($mybb->settings['allowaway'] != 0) {
        ...
        $form_container->output_row(
            $lang->away_status,
            $lang->away_status_desc,
            $form->generate_radio_button('away', 1, $lang->im_away, [
                'id' => 'away',
                'checked' => $awaycheck[0],
            ]) .
                ' ' .
                $form->generate_radio_button('away', 0, $lang->im_here, [
                    'id' => 'away2',
                    'checked' => $awaycheck[1],
                ]),
            'away'
        ...
        $form_container->output_row(
            $lang->away_reason,
            $lang->away_reason_desc,
            $form->generate_text_box('awayreason', $mybb->input['awayreason'], [
                'id' => 'awayreason',
            ]),
            'awayreason'
        );
        ...
        $return_row = $form->generate_select_box(
            'away_day',
            $birthday_days,
            $mybb->input['away_day'],
            ['id' => 'away_day']
        ...
        $return_row .=
            ...
            $form->generate_numeric_field(
                'away_year',
                $mybb->input['away_year'],
                ['id' => 'away_year', 'style' => 'width: 4em;', 'min' => 0]
            );

        $form_container->output_row(
            $lang->return_date,
            $lang->return_date_desc,
            $return_row,
            'away_date'
        );
    ...
    $form_container = new FormContainer(
        $lang->account_settings . ': ' . htmlspecialchars_uni($user['username'])
    );
    $login_options = [
        $form->generate_check_box(
            'invisible',
            1,
            $lang->hide_from_whos_online,
            ['checked' => $mybb->get_input('invisible')]
        ),
    ];
    $form_container->output_row(
        $lang->login_cookies_privacy,
        '',
        "<div class=\"user_settings_bit\">" .
            implode("</div><div class=\"user_settings_bit\">", $login_options) .
            '</div>'
    );
    ...
    $messaging_options = [
        $form->generate_check_box(
            'allownotices',
            1,
            $lang->recieve_admin_emails,
            ['checked' => $mybb->get_input('allownotices')]
        ),
        $form->generate_check_box(
            'hideemail',
            1,
            $lang->hide_email_from_others,
            ['checked' => $mybb->get_input('hideemail')]
        ),
        $form->generate_check_box(
            'receivepms',
            1,
            $lang->recieve_pms_from_others,
            ['checked' => $mybb->get_input('receivepms')]
        ),
        $form->generate_check_box(
            'receivefrombuddy',
            1,
            $lang->recieve_pms_from_buddy,
            ['checked' => $mybb->get_input('receivefrombuddy')]
        ),
        $form->generate_check_box('pmnotice', 1, $lang->alert_new_pms, [
            'checked' => $mybb->get_input('pmnotice'),
        ]),
        $form->generate_check_box('pmnotify', 1, $lang->email_notify_new_pms, [
            'checked' => $mybb->get_input('pmnotify'),
        ]),
        $form->generate_check_box(
            'buddyrequestspm',
            1,
            $lang->buddy_requests_pm,
            ['checked' => $mybb->get_input('buddyrequestspm')]
        ),
        $form->generate_check_box(
            'buddyrequestsauto',
            1,
            $lang->buddy_requests_auto,
            ['checked' => $mybb->get_input('buddyrequestsauto')]
        ),
        "<label for=\"subscriptionmethod\">{$lang->default_thread_subscription_mode}:</label><br />" .
        $form->generate_select_box(
            'subscriptionmethod',
            [
                $lang->do_not_subscribe,
                $lang->no_notification,
                $lang->instant_email_notification,
                $lang->instant_pm_notification,
            ],
            $mybb->get_input('subscriptionmethod'),
            ['id' => 'subscriptionmethod']
        ),
    ];

    // Allow plugins to add messaging options
    $messaging_options = $plugins->run_hooks(
        'admin_user_users_edit_messaging_options',
        $messaging_options
    );

    // Output messaging options
    $form_container->output_row(
        $lang->messaging_and_notification,
        '',
        "<div class=\"user_settings_bit\">" .
            implode(
    ...
    $date_format_options = [$lang->use_default];
    foreach ($date_formats as $key => $format) {
        $date_format_options[$key] = my_date($format, TIME_NOW, '', 0);
    }

    $time_format_options = [$lang->use_default];
    foreach ($time_formats as $key => $format) {
        $time_format_options[$key] = my_date($format, TIME_NOW, '', 0);
    }

    $date_options = [
        "<label for=\"dateformat\">{$lang->date_format}:</label><br />" .
        $form->generate_select_box(
            'dateformat',
            $date_format_options,
            $mybb->get_input('dateformat'),
            ['id' => 'dateformat']
        ),
        "<label for=\"dateformat\">{$lang->time_format}:</label><br />" .
        $form->generate_select_box(
            'timeformat',
            $time_format_options,
            $mybb->get_input('timeformat'),
            ['id' => 'timeformat']
        ),
        "<label for=\"timezone\">{$lang->time_zone}:</label><br />" .
        build_timezone_select('timezone', $mybb->get_input('timezone')),
        "<label for=\"dstcorrection\">{$lang->daylight_savings_time_correction}:</label><br />" .
        $form->generate_select_box(
            'dstcorrection',
            [
                2 => $lang->automatically_detect,
                1 => $lang->always_use_dst_correction,
                0 => $lang->never_use_dst_correction,
            ],
            $mybb->get_input('dstcorrection'),
            ['id' => 'dstcorrection']
        ),
    ];

    // Allow plugins to add date options
    $date_options = $plugins->run_hooks(
        'admin_user_users_edit_date_options',
        $date_options
    );

    // Output date options
    $form_container->output_row(
        $lang->date_and_time_options,
        '',
        "<div class=\"user_settings_bit\">" .
            implode("</div><div class=\"user_settings_bit\">", $date_options) .
            '</div>'
    );

    $tpp_options = [$lang->use_default];
    if ($mybb->settings['usertppoptions']) {
    ...
    $forum_options = [
        "<label for=\"tpp\">{$lang->threads_per_page}:</label><br />" .
        $form->generate_select_box(
            'tpp',
            $tpp_options,
            $mybb->get_input('tpp'),
        ...
        "<label for=\"daysprune\">{$lang->default_thread_age_view}:</label><br />" .
        $form->generate_select_box(
            'daysprune',
            $thread_age_options,
            $mybb->get_input('daysprune'),
            ['id' => 'daysprune']
        ),
    ];

    // Allow plugins to add forum options
    $forum_options = $plugins->run_hooks(
        'admin_user_users_edit_forum_options',
        $forum_options
    );

    // Output forum options
    $form_container->output_row(
        $lang->forum_display_options,
        '',
        "<div class=\"user_settings_bit\">" .
            implode("</div><div class=\"user_settings_bit\">", $forum_options) .
            '</div>'
    );

    $ppp_options = [$lang->use_default];
    if ($mybb->settings['userpppoptions']) {
    ...
    $thread_options = [
        $form->generate_check_box('showimages', 1, $lang->display_images, [
            'checked' => $mybb->get_input('showimages'),
        ]),
        $form->generate_check_box('showvideos', 1, $lang->display_videos, [
            'checked' => $mybb->get_input('showvideos'),
        ]),
        $form->generate_check_box('showsigs', 1, $lang->display_users_sigs, [
            'checked' => $mybb->get_input('showsigs'),
        ]),
        $form->generate_check_box(
            'showavatars',
            1,
            $lang->display_users_avatars,
            ['checked' => $mybb->get_input('showavatars')]
        ),
        $form->generate_check_box(
            'showquickreply',
            1,
            $lang->show_quick_reply,
            ['checked' => $mybb->get_input('showquickreply')]
        ),
        "<label for=\"ppp\">{$lang->posts_per_page}:</label><br />" .
        $form->generate_select_box(
            'ppp',
            $ppp_options,
            $mybb->get_input('ppp'),
        ...
        "<label for=\"threadmode\">{$lang->default_thread_view_mode}:</label><br />" .
        $form->generate_select_box(
            'threadmode',
            [
                '' => $lang->use_default,
                'linear' => $lang->linear_mode,
                'threaded' => $lang->threaded_mode,
            ],
            $mybb->input['threadmode'],
            ['id' => 'threadmode']
        ),
    ];

    // Allow plugins to add thread options
    $thread_options = $plugins->run_hooks(
        'admin_user_users_edit_thread_options',
        $thread_options
    );

    // Output thread options
    $form_container->output_row(
        $lang->thread_view_options,
        '',
        "<div class=\"user_settings_bit\">" .
            implode(
                "</div><div class=\"user_settings_bit\">",
    ...
    $other_options = [
        $form->generate_check_box('showredirect', 1, $lang->show_redirect, [
            'checked' => $mybb->get_input('showredirect'),
        ]),
        $form->generate_check_box(
            'showcodebuttons',
            '1',
            $lang->show_code_buttons,
            ['checked' => $mybb->get_input('showcodebuttons')]
        ),
        $form->generate_check_box('sourceeditor', '1', $lang->source_editor, [
            'checked' => $mybb->get_input('sourceeditor'),
        ]),
        "<label for=\"style\">{$lang->theme}:</label><br />" .
        build_theme_select(
            'style',
        ...
        "<label for=\"language\">{$lang->board_language}:</label><br />" .
        $form->generate_select_box(
            'language',
            $languages,
            $mybb->get_input('language'),
            ['id' => 'language']
        ),
    ];

    // Allow plugins to add other options
    $other_options = $plugins->run_hooks(
        'admin_user_users_edit_other_options',
        $other_options
    );
    ...
    $form_container->output_row(
        ...
        '',
        "<div class=\"user_settings_bit\">" .
            implode("</div><div class=\"user_settings_bit\">", $other_options) .
            '</div>'
    );
    ...
    // SIGNATURE EDITOR
    //
    $signature_editor = $form->generate_text_area(
        'signature',
        $mybb->get_input('signature'),
        [
            'id' => 'signature',
            'rows' => 15,
            'cols' => '70',
    ...
    $form_container = new FormContainer(
        $lang->signature . ': ' . htmlspecialchars_uni($user['username'])
    );
    $form_container->output_row(
        $lang->signature,
        $lang->sprintf(
    ...
    if ($mybb->get_input('suspendsignature')) {
        ...
        if ($user['suspendsigtime'] == '0') {
            ...
            $expired = nice_time($remaining, ['seconds' => false]);
            ...
            $lang->suspend_expire_info = $lang->sprintf(
                $lang->suspend_expire_info,
                $expired,
                $color
            );
        }
	...
	<dl style="margin-top: 0; margin-bottom: 0; width: 100%;">
        ...
        $form->generate_check_box(
            'suspendsignature',
            1,
            $lang->suspend_sig_box,
            ['checked' => $sig_checked, 'onclick' => 'toggleAction();']
        ) .
        '</dt>
        ...
        $form->generate_numeric_field(
            'action_time',
            $mybb->input['action_time'],
            ['style' => 'width: 3em;', 'min' => 0]
        ) .
        ' ' .
        $form->generate_select_box(
            'action_period',
            $periods,
            $mybb->input['action_period']
        ) .
        '</td>
				</tr>
			</table>
    ...
    $form_container->output_row(
        $lang->suspend_sig,
        $lang->suspend_sig_info,
        $actions
    );

    $signature_options = [
        $form->generate_radio_button(
            'update_posts',
            'enable',
            $lang->enable_sig_in_all_posts,
            ['checked' => 0]
        ),
        $form->generate_radio_button(
            'update_posts',
            'disable',
            $lang->disable_sig_in_all_posts,
            ['checked' => 0]
        ),
        $form->generate_radio_button('update_posts', 'no', $lang->do_nothing, [
            'checked' => 1,
        ]),
    ];

    $form_container->output_row(
        $lang->signature_preferences,
        '',
        implode('<br />', $signature_options)
    );
    ...
    $table = new Table();
    $table->construct_header($lang->current_avatar, ['colspan' => 2]);

    $table->construct_cell(
        "<div style=\"width: 126px; height: 126px;\" class=\"user_avatar\"><img src=\"" .
            htmlspecialchars_uni($user['avatar']) .
            "\" width=\"{$scaled_dimensions['width']}\" style=\"margin-top: {$avatar_top}px\" height=\"{$scaled_dimensions['height']}\" alt=\"\" /></div>",
        ['width' => 1]
    );
    ...
    if ($mybb->settings['maxavatardims'] != '') {
        list($max_width, $max_height) = preg_split(
            '/[|x]/',
            my_strtolower($mybb->settings['maxavatardims'])
        );
    ...
    if ($user['avatar']) {
        $remove_avatar =
            ...
            $form->generate_check_box(
                'remove_avatar',
                1,
                "<strong>{$lang->remove_avatar}</strong>"
            );
    }

    $table->construct_cell(
        $lang->avatar_desc . "{$remove_avatar}<br /><small>{$max_size}</small>"
    );
    $table->construct_row();

    $table->output(
        $lang->avatar . ': ' . htmlspecialchars_uni($user['username'])
    );
    ...
    $form_container->output_row(
        $lang->upload_avatar,
        $auto_resize,
        $form->generate_file_upload_box('avatar_upload', [
            'id' => 'avatar_upload',
        ]),
        'avatar_upload'
    );
    if ($mybb->settings['allowremoteavatars']) {
        $form_container->output_row(
            $lang->or_specify_avatar_url,
            '',
            $form->generate_text_box('avatar_url', $avatar_url, [
                'id' => 'avatar_url',
            ]),
            'avatar_url'
        );
    ...
    $form_container = new FormContainer(
        $lang->mod_options . ': ' . htmlspecialchars_uni($user['username'])
    );
    $form_container->output_row(
        $lang->user_notes,
        '',
        $form->generate_text_area('usernotes', $mybb->input['usernotes'], [
            'id' => 'usernotes',
        ]),
        'usernotes'
    );
    ...
    // Mod posts
    // Generate check box
    $modpost_options = $form->generate_select_box(
        'modpost_period',
        $periods,
        $mybb->input['modpost_period'],
        ['id' => 'modpost_period']
    );
    ...
    if (
        $user['moderateposts'] ||
        ($mybb->get_input('moderateposting') && !empty($errors))
    ) {
        ...
        if ($user['moderationtime'] != 0) {
            ...
            $existing_info = $lang->sprintf(
                $lang->moderate_length,
                $expired,
                $color
            );
        } else {
    ...
    $modpost_div =
        ...
        $form->generate_numeric_field(
            'modpost_time',
            $mybb->get_input('modpost_time'),
            ['style' => 'width: 3em;', 'min' => 0]
        ) .
        ' ' .
        $modpost_options .
        '</div>';
    $lang->moderate_posts_info = $lang->sprintf(
        $lang->moderate_posts_info,
        htmlspecialchars_uni($user['username'])
    );
    $form_container->output_row(
        $form->generate_check_box('moderateposting', 1, $lang->moderate_posts, [
            'id' => 'moderateposting',
            'onclick' => "toggleBox('modpost');",
            'checked' => $mybb->get_input('moderateposting'),
        ]),
        $lang->moderate_posts_info,
        $modpost_div
    );
    ...
    // Suspend posts
    // Generate check box
    $suspost_options = $form->generate_select_box(
        'suspost_period',
        $periods,
        $mybb->get_input('suspost_period'),
        ['id' => 'suspost_period']
    );
    ...
    if (
        $user['suspendposting'] ||
        ($mybb->get_input('suspendposting') && !empty($errors))
    ) {
        $mybb->input['suspendposting'] = 1;

        if (
            $user['suspensiontime'] == 0 ||
            $mybb->get_input('suspost_period') == 'never'
        ) {
            $existing_info = $lang->suspended_perm;
        } else {
            $remaining = $user['suspensiontime'] - TIME_NOW;
            $suspost_date = nice_time($remaining, ['seconds' => false]);
    ...
    $suspost_div =
        ...
        $form->generate_numeric_field(
            'suspost_time',
            $mybb->get_input('suspost_time'),
            ['style' => 'width: 3em;', 'min' => 0]
        ) .
        ' ' .
        $suspost_options .
        '</div>';
    $lang->suspend_posts_info = $lang->sprintf(
        $lang->suspend_posts_info,
        htmlspecialchars_uni($user['username'])
    );
    $form_container->output_row(
        $form->generate_check_box('suspendposting', 1, $lang->suspend_posts, [
            'id' => 'suspendposting',
            'onclick' => "toggleBox('suspost');",
            'checked' => $mybb->get_input('suspendposting'),
        ]),
        $lang->suspend_posts_info,
        $suspost_div
    );
...
if ($mybb->input['action'] == 'delete') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        $page->output_confirm_action(
            "index.php?module=user-users&action=delete&uid={$user['uid']}",
            $lang->user_deletion_confirmation
        );
    }
}
...
if ($mybb->input['action'] == 'referrers') {
    ...
    $query = $db->simple_select(
        'adminviews',
        '*',
        "type='user' AND (vid='{$default_view}' OR uid=0)",
        ['order_by' => 'uid', 'order_dir' => 'desc']
    );
...
if ($mybb->input['action'] == 'ipaddresses') {
    ...
    if (empty($user['lastip'])) {
        ...
        $popup->add_item(
            $lang->show_users_regged_with_ip,
            'index.php?module=user-users&amp;action=search&amp;results=1&amp;conditions[regip]=' .
                $user['lastip']
        );
        $popup->add_item(
            $lang->show_users_posted_with_ip,
            'index.php?module=user-users&amp;results=1&amp;action=search&amp;conditions[postip]=' .
                $user['lastip']
        );
        $popup->add_item(
            $lang->info_on_ip,
            "index.php?module=user-users&amp;action=iplookup&ipaddress={$user['lastip']}",
            "MyBB.popupWindow('index.php?module=user-users&amp;action=iplookup&ipaddress={$user['lastip']}', null, true); return false;"
        );
        $popup->add_item(
            $lang->ban_ip,
            "index.php?module=config-banning&amp;filter={$user['lastip']}"
        );
        $controls = $popup->fetch();
    }
    $table->construct_cell(
        "<strong>{$lang->last_known_ip}:</strong> " . $user['lastip']
    );
    ...
    if (empty($user['regip'])) {
        ...
        $popup->add_item(
            $lang->show_users_regged_with_ip,
            'index.php?module=user-users&amp;results=1&amp;action=search&amp;conditions[regip]=' .
                $user['regip']
        );
        $popup->add_item(
            $lang->show_users_posted_with_ip,
        ...
        $popup->add_item(
            $lang->info_on_ip,
            "index.php?module=user-users&amp;action=iplookup&ipaddress={$user['regip']}",
            "MyBB.popupWindow('index.php?module=user-users&amp;action=iplookup&ipaddress={$user['regip']}', null, true); return false;"
        );
        $popup->add_item(
            $lang->ban_ip,
            "index.php?module=config-banning&amp;filter={$user['regip']}"
        );
        $controls = $popup->fetch();
    }
    $table->construct_cell(
        "<strong>{$lang->registration_ip}:</strong> " . $user['regip']
    );
    $table->construct_cell($controls, ['class' => 'align_center']);
    $table->construct_row();
    ...
    while ($ip = $db->fetch_array($query)) {
        ...
        $popup->add_item(
            $lang->info_on_ip,
            "index.php?module=user-users&amp;action=iplookup&ipaddress={$ip['ipaddress']}",
            "MyBB.popupWindow('index.php?module=user-users&amp;action=iplookup&ipaddress={$ip['ipaddress']}', null, true); return false;"
        );
        $popup->add_item(
            $lang->ban_ip,
            "index.php?module=config-banning&amp;filter={$ip['ipaddress']}"
        );
        $controls = $popup->fetch();
    ...
    $table->output(
        $lang->ip_address_for . ' ' . htmlspecialchars_uni($user['username'])
    );
...
if ($mybb->input['action'] == 'merge') {
    ...
    if ($mybb->request_method == 'post') {
        foreach (['source', 'destination'] as $target) {
            ${$target . '_user'} = get_user_by_username(
                $mybb->input[$target . '_username'],
                ['fields' => '*']
            );
        ...
        if (
            !is_super_admin($mybb->user['uid']) &&
            (is_super_admin($source_user['uid']) ||
                is_super_admin($destination_user['uid']))
        ) {
            flash_message($lang->error_no_perms_super_admin, 'error');
            admin_redirect('index.php?module=user-users');
        }
        ...
        if (
            ...
            $source_user['uid'] == $destination_user['uid'] &&
            !empty($source_user['uid'])
        ) {
        ...
        if (empty($errors)) {
            ...
            $db->update_query(
                'warnings',
                ['revokedby' => $destination_user['uid']],
                "revokedby='{$source_user['uid']}'"
            );
            $db->update_query(
                'warnings',
                ['issuedby' => $destination_user['uid']],
                "issuedby='{$source_user['uid']}'"
            );
            ...
            $db->update_query(
                'users',
                ['referrer' => $destination_user['uid']],
                "referrer='{$source_user['uid']}' AND uid!='{$destination_user['uid']}'"
            );
            ...
            if (
                ($destination_user['referrer'] == 0 &&
                    $source_user['referrer'] > 0 &&
                    $source_user['referrer'] != $destination_user['uid']) ||
                $destination_user['referrer'] == $source_user['uid']
            ) {
                $db->update_query(
                    'users',
                    ['referrer' => $source_user['referrer']],
                    "uid='{$destination_user['uid']}'"
                );
            }
            $query = $db->simple_select(
                'users',
                'COUNT(uid) as total_referrals',
                "referrer='{$destination_user['uid']}' AND uid!='{$source_user['uid']}'"
            );
            $new_referrals = $db->fetch_field($query, 'total_referrals');
            $db->update_query(
                'users',
                ['referrals' => (int) $new_referrals],
                "uid='{$destination_user['uid']}'"
            );
            ...
            while ($rep = $db->fetch_array($query)) {
                if (
                    $rep['pid'] == 0 &&
                    $mybb->settings['multirep'] == 0 &&
                    $last_result['uid'] == $rep['uid']
                ) {
                ...
                if (
                    $last_result['uid'] == $destination_user['uid'] ||
                    $rep['uid'] == $destination_user['uid']
                ) {
            ...
            $db->update_query(
                'users',
                ['reputation' => (int) $total_reputation],
                "uid='{$destination_user['uid']}'"
            );
            ...
            $db->update_query(
                ...
                [
                    'warningpoints' =>
                        (int) $original_warn_level + $new_warn_level,
                ],
                "uid='{$destination_user['uid']}'"
            );
            ...
            $db->update_query(
                'privatemessages',
                $to_uid,
                "toid='{$source_user['uid']}'"
            );

            // Buddy/ignore lists
            $destination_buddies = explode(',', $destination_user['buddylist']);
            $source_buddies = explode(',', $source_user['buddylist']);
            $buddies = array_unique(
                array_merge($source_buddies, $destination_buddies)
            );
            // Make sure the new buddy list doesn't contain either users
            $buddies_array = array_diff($buddies, [
                $destination_user['uid'],
                $source_user['uid'],
            ]);

            $destination_ignored = explode(
                ',',
                $destination_user['ignorelist']
            );
            $source_ignored = explode(',', $destination_user['ignorelist']);
            $ignored = array_unique(
                array_merge($source_ignored, $destination_ignored)
            );
            // ... and the same for the new ignore list
            $ignored_array = array_diff($ignored, [
                $destination_user['uid'],
                $source_user['uid'],
            ]);
            ...
            $query = $db->simple_select(
                'posts',
                'COUNT(*) AS postnum',
                "uid='" . $destination_user['uid'] . "' {$fids_not_in}"
            );
            ...
            $destination_username = htmlspecialchars_uni(
                $destination_user['username']
            );
            flash_message(
                "<strong>{$username}</strong> {$lang->success_merged} {$destination_username}",
                'success'
            );
            admin_redirect('index.php?module=user-users');
            exit();
    ...
    $form_container->output_row(
        $lang->source_account . ' <em>*</em>',
        $lang->source_account_desc,
        $form->generate_text_box(
            'source_username',
            $mybb->get_input('source_username'),
            ['id' => 'source_username']
        ),
        'source_username'
    );
    $form_container->output_row(
        $lang->destination_account . ' <em>*</em>',
        $lang->destination_account_desc,
        $form->generate_text_box(
            'destination_username',
            $mybb->get_input('destination_username'),
...
if ($mybb->input['action'] == 'search') {
    ...
    if ($mybb->request_method == 'post' || $mybb->get_input('results') == 1) {
        ...
        if (
            $mybb->get_input('search_id') &&
            $admin_session['data']['user_views'][$mybb->get_input('search_id')]
        ) {
            $admin_view =
                $admin_session['data']['user_views'][
                    $mybb->get_input('search_id')
                ];
        ...
        if (!empty($mybb->input['conditions'])) {
            $admin_view['conditions'] = $mybb->get_input(
                'conditions',
                MyBB::INPUT_ARRAY
            );
        }
        ...
        if (!empty($mybb->input['profile_fields'])) {
            $admin_view['custom_profile_fields'] = $mybb->get_input(
                'profile_fields',
                MyBB::INPUT_ARRAY
            );
        }
    ...
    $form_container->output_row(
        ...
        '',
        $form->generate_numeric_field('perpage', $mybb->get_input('perpage'), [
            'id' => 'perpage',
            'min' => 1,
        ]),
        'perpage'
    ...
    $form_container->output_row(
        $lang->display_results_as,
        '',
        $form->generate_radio_button('displayas', 'table', $lang->table, [
            'checked' => $mybb->get_input('displayas') != 'card' ? true : false,
        ]) .
            '<br />' .
            $form->generate_radio_button(
                'displayas',
                ...
                [
                    'checked' =>
                        $mybb->get_input('displayas') == 'card' ? true : false,
                ]
            )
    );
    $form_container->end();
    ...
    switch ($mybb->input['inline_action']) {
        case 'multiactivate':
            ...
            if (is_array($selected)) {
                ...
                $query = $db->simple_select(
                    'users',
                    'uid, username, email',
                    "usergroup = '5' AND uid IN (" . $sql_array . ')'
                );
                $user_mail_data = [];
            ...
            if (isset($to_update) && is_array($to_update)) {
                ...
                $db->write_query(
                    'UPDATE ' .
                        TABLE_PREFIX .
                        "users SET usergroup = '2' WHERE uid IN (" .
                        $sql_array .
                        ')'
                );
                ...
                $lang->inline_activated = $lang->sprintf(
                    $lang->inline_activated,
                    my_number_format($to_update_count)
                );

                if (
                    is_array($selected) &&
                    $to_update_count != count($selected)
                ) {
                    // The update count is different to how many we selected!
                    $not_updated_count = count($selected) - $to_update_count;
                    $lang->inline_activated_more = $lang->sprintf(
                        $lang->inline_activated_more,
                        my_number_format($not_updated_count)
                    );
                    $lang->inline_activated =
                        $lang->inline_activated .
                        '<br />' .
            ...
            if ($mybb->request_method == 'post') {
                ...
                while ($ban = $db->fetch_array($query)) {
                    $updated_group = [
                        ...
                        'additionalgroups' => $db->escape_string(
                            $ban['oldadditionalgroups']
                        ),
                ...
                $lang->success_ban_lifted = $lang->sprintf(
                    $lang->success_ban_lifted,
                    my_number_format($to_be_unbanned)
                );
                flash_message($lang->success_ban_lifted, 'success');
                admin_redirect('index.php?module=user-users' . $vid_url);
            } else {
                $page->output_confirm_action(
                    'index.php?module=user-users&amp;action=inline_edit&amp;inline_action=multilift',
                    $lang->confirm_multilift
                );
            }
        ...
        case 'multiban':
            if ($mybb->input['processed'] == 1) {
                ...
                $query = $db->simple_select(
                    'users',
                    'uid, username, usergroup, additionalgroups, displaygroup',
                    'uid IN (' . $sql_array . ')'
                );
                ...
                while ($user = $db->fetch_array($query)) {
                    ...
                    if (
                        is_array($bannedcache) &&
                        in_array('u_' . $user['uid'], $bannedcache)
                    ) {
                        // User already has a ban, update it!
                        $update_array = [
                            'admin' => (int) $mybb->user['uid'],
                            'dateline' => TIME_NOW,
                            'bantime' => $db->escape_string(
                                $mybb->input['bantime']
                            ),
                        ...
                        $insert_array = [
                            ...
                            'oldadditionalgroups' => $db->escape_string(
                ...
                $lang->users_banned = $lang->sprintf(
                    $lang->users_banned,
                    $banned_count
                );
            ...
            $table = new Table();
            $lang->mass_ban_info = $lang->sprintf(
                $lang->mass_ban_info,
                count($selected)
            );
            ...
            $form_container = new FormContainer(
                '<div class="float_right"><a href="index.php?module=user-users&amp;action=inline_edit&amp;inline_action=multilift&amp;my_post_key=' .
                    $mybb->post_code .
                    '">' .
                    $lang->lift_bans .
                    '</a></div>' .
            ...
            $form_container->output_row(
                $lang->ban_reason,
                '',
                $form->generate_text_area('reason', $mybb->input['reason'], [
                    'id' => 'reason',
                    'maxlength' => '255',
                ]),
                'reason'
            ...
            foreach ($ban_times as $time => $period) {
                if ($time != '---') {
                    $friendly_time = my_date(
                        "D, jS M Y @ {$mybb->settings['timeformat']}",
                        ban_date2timestamp($time)
                    );
        ...
        case 'multidelete':
            if ($mybb->get_input('no')) {
                ...
                if ($mybb->input['processed'] == 1) {
                    ...
                    $lang->users_deleted = $lang->sprintf(
                        $lang->users_deleted,
                        $to_be_deleted
                    );
                ...
                $lang->confirm_multidelete = $lang->sprintf(
                    $lang->confirm_multidelete,
                    my_number_format($to_be_deleted)
                );
                $page->output_confirm_action(
                    "index.php?module=user-users&amp;action=inline_edit&amp;inline_action=multidelete&amp;my_post_key={$mybb->post_code}&amp;processed=1",
                    $lang->confirm_multidelete
                );
            }
            break;
        case 'multiprune':
                ...
                if (!$errors) {
                    ...
                    while ($user = $db->fetch_array($query)) {
                        ...
                        if ($return_array && is_array($return_array)) {
                            $prune_array = array_merge_recursive(
                                $prune_array,
                                $return_array
                            );
                        }
                    ...
                    if (!is_array($prune_array) || count($prune_array) == 0) {
                        ...
                        admin_redirect(
                            'index.php?module=user-users' . $vid_url
                        );
                    }
                    ...
                    if (
                        is_array($prune_array['to_delete']) &&
                        count($prune_array['to_delete']) > 0
                    ) {
                    ...
                    if (
                        is_array($prune_array['thread_update']) &&
                        count($prune_array['thread_update']) > 0
                    ) {
                        $sql_array = implode(
                            ',',
                            $prune_array['thread_update']
                        );
                        $query = $db->simple_select(
                            'threads',
                            'tid',
                            'tid IN (' . $sql_array . ')',
                            ['order_by' => 'tid', 'order_dir' => 'asc']
                        );
                    ...
                    if (
                        is_array($prune_array['forum_update']) &&
                        count($prune_array['forum_update']) > 0
                    ) {
            ...
            $table = new Table();
            $lang->mass_prune_info = $lang->sprintf(
                $lang->mass_prune_info,
                count($selected)
            );
            ...
            $date_box = $form->generate_select_box(
                'day',
                $day_options,
                $mybb->input['day']
            );
            $month_box = $form->generate_select_box(
                'month',
                $month_options,
                $mybb->input['month']
            );
            $year_box = $form->generate_numeric_field(
                'year',
                $mybb->input['year'],
                ['id' => 'year', 'style' => 'width: 50px;', 'min' => 0]
            );
        ...
        case 'multiusergroup':
            if ($mybb->get_input('processed', \MyBB::INPUT_INT) === 1) {
                ...
                $update_array = [
                    'usergroup' => $mybb->get_input(
                        'usergroup',
                        MyBB::INPUT_INT
                    ),
                    'additionalgroups' => $additionalgroups,
                    'displaygroup' => $mybb->get_input(
                        'displaygroup',
                        MyBB::INPUT_INT
                    ),
                ];
                ...
                $hook_params = $plugins->run_hooks(
                    'admin_user_multiusergroup',
                    $hook_params
                );
            ...
            // Display a table warning
            $table = new Table();
            $lang->usergroup_info = $lang->sprintf(
                $lang->usergroup_info,
                count($selected)
            );
            ...
            while ($usergroup = $db->fetch_array($query)) {
                $options[$usergroup['gid']] = htmlspecialchars_uni(
                    $usergroup['title']
                );
                $display_group_options[
                    $usergroup['gid']
            ...
            if (!$mybb->get_input('additionalgroups', \MyBB::INPUT_ARRAY)) {
                $mybb->input['additionalgroups'] = explode(
                    ',',
                    $mybb->get_input('additionalgroups')
                );
            }

            $form_container->output_row(
                $lang->primary_user_group,
                '',
                $form->generate_select_box(
                    'usergroup',
                    $options,
...
if (!$mybb->input['action']) {
    ...
    if (
        isset($mybb->input['search_id']) &&
        $admin_session['data']['user_views'][$mybb->input['search_id']]
    ) {
        $admin_view =
            $admin_session['data']['user_views'][$mybb->input['search_id']];
        unset($admin_view['extra_sql']);
    ...
    $query = $db->simple_select(
        'adminviews',
        '*',
        "type='user' AND (visibility=2 OR uid={$mybb->user['uid']})",
        ['order_by' => 'title']
    );
    while ($view = $db->fetch_array($query)) {
        $popup->add_item(
            htmlspecialchars_uni($view['title']),
            "index.php?module=user-users&amp;vid={$view['vid']}"
        );
    }
    $popup->add_item(
        "<em>{$lang->manage_views}</em>",
        'index.php?module=user-users&amp;action=views'
    );
    ...
    if (!$results) {
        ...
        if ($admin_session['data']['from'] == 'home') {
            flash_message(
                $admin_session['data']['flash_message2']['message'],
                $admin_session['data']['flash_message2']['type']
            );
            update_admin_session('flash_message2', '');
...
function build_users_view($view)
{
    ...
    if (!is_array($view['custom_profile_fields'])) {
        $view['custom_profile_fields'] = my_unserialize(
    ...
    if (isset($mybb->input['username'])) {
        ...
        $view['url'] .=
            '&amp;username=' .
            urlencode(htmlspecialchars_uni($mybb->input['username']));
    }
    if (!empty($view['vid'])) {
        ...
        $view['url'] .=
            '&amp;search_id=' . htmlspecialchars_uni($mybb->input['search_id']);
    }

    if (
        !isset($admin_session['data']['last_users_view']) ||
        $admin_session['data']['last_users_view'] !=
            str_replace('&amp;', '&', $view['url'])
    ) {
        update_admin_session(
            'last_users_url',
            str_replace('&amp;', '&', $view['url'])
    ...
    if (isset($view['conditions']['referrer'])) {
        $view['url'] .=
            '&amp;action=referrers&amp;uid=' .
            htmlspecialchars_uni($view['conditions']['referrer']);
    }
    ...
    foreach ($user_like_fields as $search_field) {
        if (
            !empty($view['conditions'][$search_field]) &&
            empty($view['conditions'][$search_field . '_blank'])
        ) {
            $search_sql .=
                " AND u.{$search_field} LIKE '%" .
                $db->escape_string_like($view['conditions'][$search_field]) .
                "%'";
    ...
    foreach ($user_exact_fields as $search_field) {
        if (!empty($view['conditions'][$search_field])) {
            $search_sql .=
                " AND u.{$search_field}='" .
                $db->escape_string($view['conditions'][$search_field]) .
                "'";
        }
    }
    ...
    foreach ($direction_fields as $search_field) {
        ...
        if (
            isset($view['conditions'][$search_field]) &&
            ($view['conditions'][$search_field] ||
                $view['conditions'][$search_field] === '0') &&
            $view['conditions'][$direction_field]
        ) {
    ...
    foreach ($reg_fields as $search_field) {
        if (
            !empty($view['conditions'][$search_field]) &&
            (int) $view['conditions'][$search_field]
        ) {
            $threshold =
                TIME_NOW -
                (int) $view['conditions'][$search_field] * 24 * 60 * 60;
        ...
        if (!empty($view['conditions'][$search_field])) {
            ...
            if (!is_array($ip_range)) {
                ...
                $ip_sql =
                    ...
                    $db->escape_binary($ip_range[0]) .
                    ' AND ' .
                    $db->escape_binary($ip_range[1]);
            }
    ...
    if ($view['custom_profile_fields']) {
        ...
        if ($userfield_sql != '1=1') {
            ...
            $search_sql .=
                ' AND u.uid IN(' . implode(',', $userfield_uids) . ')';
    ...
    if (isset($view['conditions']['usergroup'])) {
        ...
        $search_sql .=
            ...
            implode(
                ',',
                array_map('intval', $view['conditions']['usergroup'])
            ) .
            ") {$additional_sql})";
    }
    ...
    // Generate the list of results
        ...
        while ($user = $db->fetch_array($query)) {
            ...
            $user['view']['username'] =
                "<a href=\"index.php?module=user-users&amp;action=edit&amp;uid={$user['uid']}\">" .
                format_name(
                    $user['username'],
                    $user['usergroup'],
                    $user['displaygroup']
                ...
                '</a>';
            $user['view']['usergroup'] = htmlspecialchars_uni(
                $usergroups[$user['usergroup']]['title']
            );
            if ($user['additionalgroups']) {
                ...
                foreach ($additional_groups as $group) {
                    $groups_list .=
                        $comma .
                        htmlspecialchars_uni($usergroups[$group]['title']);
                    $comma = $lang->comma;
                }
            }
            if (!$groups_list) {
            ...
            $popup->add_item(
                $lang->view_profile,
                $mybb->settings['bburl'] . '/' . get_profile_link($user['uid'])
            );
            $popup->add_item(
                $lang->edit_profile_and_settings,
                "index.php?module=user-users&amp;action=edit&amp;uid={$user['uid']}"
            );
            ...
            if ($usergroups[$user['usergroup']]['isbannedgroup'] == 1) {
                ...
                $popup->add_item(
                    $lang->edit_ban,
                    "index.php?module=user-banning&amp;uid={$user['uid']}#username"
                );
                $popup->add_item(
                    $lang->lift_ban,
                    "index.php?module=user-banning&action=lift&uid={$user['uid']}&my_post_key={$mybb->post_code}"
                );
            } else {
            ...
            if ($user['usergroup'] == 5) {
                if ($user['coppauser']) {
                    $popup->add_item(
                        $lang->approve_coppa_user,
                        "index.php?module=user-users&amp;action=activate_user&amp;uid={$user['uid']}&amp;my_post_key={$mybb->post_code}{$from_bit}"
                    );
                } else {
                    $popup->add_item(
                        $lang->approve_user,
            ...
            $popup->add_item(
                $lang->delete_user,
                "index.php?module=user-users&amp;action=delete&amp;uid={$user['uid']}&amp;my_post_key={$mybb->post_code}",
                "return AdminCP.deleteConfirmation(this, '{$lang->user_deletion_confirmation}')"
            );
            $popup->add_item(
                $lang->show_referred_users,
                "index.php?module=user-users&amp;action=referrers&amp;uid={$user['uid']}"
            );
            $popup->add_item(
                $lang->show_ip_addresses,
                "index.php?module=user-users&amp;action=ipaddresses&amp;uid={$user['uid']}"
            );
            $popup->add_item(
                $lang->show_attachments,
                'index.php?module=forum-attachments&amp;results=1&amp;username=' .
            ...
            // Fetch the reputation for this user
            if (
                $usergroups[$user['usergroup']]['usereputationsystem'] == 1 &&
                $mybb->settings['enablereputation'] == 1
            ) {
                $user['view']['reputation'] = get_reputation(
                    $user['reputation']
                );
            } else {
                $reputation = '-';
            ...
            if (
                $mybb->settings['enablewarningsystem'] != 0 &&
                $usergroups[$user['usergroup']]['canreceivewarnings'] != 0
            ) {
                if ($mybb->settings['maxwarningpoints'] < 1) {
                    $mybb->settings['maxwarningpoints'] = 10;
                }
                ...
                $warning_level = round(
                    ($user['warningpoints'] /
                        $mybb->settings['maxwarningpoints']) *
                        100
                );
                if ($warning_level > 100) {
                    $warning_level = 100;
                }
                $user['view']['warninglevel'] = get_colored_warning_level(
                    $warning_level
                );
            }
            ...
            $user['view']['avatar'] =
                "<img src=\"" .
                $avatar['image'] .
                "\" alt=\"\" {$avatar['width_height']} />";

            // Convert IP's to readable
            $user['regip'] = my_inet_ntop($db->unescape_binary($user['regip']));
            $user['lastip'] = my_inet_ntop(
                $db->unescape_binary($user['lastip'])
            );
    ...
    if ($num_results > $view['perpage']) {
        ...
        $pagination = draw_admin_pagination(
            $mybb->input['page'],
            $view['perpage'],
            $num_results,
            $view['url'] . "&amp;type={$view_type}"
        );
    ...
    $search = new Form(
        htmlspecialchars_uni($search_action),
        'post',
        'search_form',
        0,
        '',
    ...
    $built_view .=
        $search->generate_text_box('username', htmlspecialchars_uni($value), [
            'id' => 'search_keywords',
            'class' => "{$default_class} field150 field_small",
        ]) . "\n";
    ...
    $built_view .=
        ...
        addcslashes($lang->search_for_user, "'") .
        "')
			{
    ...
    $built_view .= $table->construct_html(
        "{$switch_view}<div>{$checkbox}{$lang->users}{$view_title}</div>",
        1,
        '',
        $view['table_id']
    );
...
<script type="text/javascript" src="' .
        $mybb->settings['bburl'] .
        '/jscripts/inline_moderation.js?ver=1838"></script>
...
<div class="float_right"><span class="smalltext"><strong>' .
        $lang->inline_edit .
        '</strong></span>
<select name="inline_action">
	<option value="multiactivate">' .
        $lang->inline_activate .
        '</option>
	<option value="multiban">' .
        $lang->inline_ban .
        '</option>
	<option value="multiusergroup">' .
        $lang->inline_usergroup .
        '</option>
	<option value="multidelete">' .
        $lang->inline_delete .
        '</option>
	<option value="multiprune">' .
        $lang->inline_prune .
        '</option>
</select>
<input type="submit" class="submit_button inline_element" name="go" value="' .
        $lang->go .
        ' (0)" id="inline_go" />&nbsp;
<input type="button" onclick="javascript:inlineModeration.clearChecked();" value="' .
        $lang->clear .
...
function build_user_view_table($user, $view, &$table)
{
    ...
    $table->construct_cell(
        "<input type=\"checkbox\" class=\"checkbox\" name=\"inlinemod_{$user['uid']}\" id=\"inlinemod_{$user['uid']}\" value=\"1\" onclick=\"$('#uid_{$user['uid']}').toggleClass('inline_selected');\" />"
    );
 ...
 * @param bool $search
 */
function output_custom_profile_fields(
    $fields,
    $values,
    &$form_container,
    &$form,
    ...
    foreach ($fields as $profile_field) {
        ...
        $profile_field['description'] = htmlspecialchars_uni(
        ...
        switch ($type) {
            case 'multiselect':
                ...
                $code = $form->generate_select_box(
                    "profile_fields[{$field_name}][]",
                    $options,
                    $selected_options,
                    [
                        'id' => "profile_field_{$field_name}",
                        'multiple' => true,
                        'size' => $profile_field['length'],
                    ]
                );
                break;
            case 'select':
                ...
                if ($search == true) {
                    $code = $form->generate_select_box(
                        ...
                        $options,
                        htmlspecialchars_uni($values[$field_name]),
                        [
                            'id' => "profile_field_{$field_name}",
                            'size' => $profile_field['length'],
                        ]
                ...
                foreach ($radio_options as $val) {
                    ...
                    $code .=
                        $form->generate_radio_button(
                            "profile_fields[{$field_name}]",
                            $val,
                            htmlspecialchars_uni($val),
                            [
                                'id' => "profile_field_{$field_name}",
                                'checked' =>
                                    $val == $values[$field_name] ? true : false,
                            ]
                        ) . '<br />';
                }
                break;
            ...
            case 'textarea':
                $extra = '';
                if (
                    isset($mybb->input['action']) &&
                    $mybb->input['action'] == 'search'
                ) {
                    $extra =
                        " {$lang->or} " .
                        $form->generate_check_box(
                            "profile_fields[{$field_name}_blank]",
                            1,
                            $lang->is_not_blank,
                            [
                                'id' => "{$field_name}_blank",
                                'checked' => $values[$field_name . '_blank'],
                            ]
                        );
                }
            ...
            default:
                ...
                $code =
                    $form->generate_text_box(
                        ...
                        [
                            'id' => "profile_field_{$field_name}",
                            'maxlength' => $profile_field['maxlength'],
                            'length' => $profile_field['length'],
                        ]
                    ) . $extra;
                break;
        }
...
function user_search_conditions($input, &$form)
{
    ...
    if (!is_array($input['profile_fields'])) {
        $input['profile_fields'] = (array) my_unserialize(
            $input['profile_fields']
        );
    }
    ...
    $form_container->output_row(
        $lang->email_address_contains,
        '',
        $form->generate_text_box(
            'conditions[email]',
            $input['conditions']['email'],
            ['id' => 'email']
        ),
        'email'
    );
    ...
    $form_container->output_row(
        $lang->website_contains,
        '',
        $form->generate_text_box(
            'conditions[website]',
            $input['conditions']['website'],
            ['id' => 'website']
        ) .
            " {$lang->or} " .
            $form->generate_check_box(
                'conditions[website_blank]',
                1,
                $lang->is_not_blank,
                [
                    'id' => 'website_blank',
                    'checked' => $input['conditions']['website_blank'],
                ]
            ),
        'website'
    );
    $form_container->output_row(
        $lang->icq_number_contains,
        '',
        $form->generate_text_box(
            'conditions[icq]',
            $input['conditions']['icq'],
            ['id' => 'icq']
        ) .
            " {$lang->or} " .
            $form->generate_check_box(
                'conditions[icq_blank]',
                1,
                $lang->is_not_blank,
                [
                    'id' => 'icq_blank',
                    'checked' => $input['conditions']['icq_blank'],
                ]
            ),
        'icq'
    );
    $form_container->output_row(
        ...
        $form->generate_text_box(
            'conditions[skype]',
            $input['conditions']['skype'],
            ['id' => 'skype']
        ) .
            " {$lang->or} " .
            $form->generate_check_box(
                'conditions[skype_blank]',
                1,
                $lang->is_not_blank,
                [
                    'id' => 'skype_blank',
                    'checked' => $input['conditions']['skype_blank'],
                ]
            ),
        'skype'
    );
    $form_container->output_row(
        $lang->google_contains,
        '',
        $form->generate_text_box(
            'conditions[google]',
            $input['conditions']['google'],
            ['id' => 'google']
        ) .
            " {$lang->or} " .
            $form->generate_check_box(
                'conditions[google_blank]',
                1,
                $lang->is_not_blank,
                [
                    'id' => 'google_blank',
                    'checked' => $input['conditions']['google_blank'],
                ]
            ),
        'google'
    );
    $form_container->output_row(
        $lang->signature_contains,
        '',
        $form->generate_text_box(
            'conditions[signature]',
            $input['conditions']['signature'],
            ['id' => 'signature']
        ) .
            " {$lang->or} " .
            $form->generate_check_box(
                'conditions[signature_blank]',
                1,
                $lang->is_not_blank,
                [
                    'id' => 'signature_blank',
                    'checked' => $input['conditions']['signature_blank'],
                ]
            ),
        'signature'
    );
    $form_container->output_row(
        $lang->user_title_contains,
        '',
        $form->generate_text_box(
            'conditions[usertitle]',
            $input['conditions']['usertitle'],
            ['id' => 'usertitle']
        ) .
            " {$lang->or} " .
            $form->generate_check_box(
                'conditions[usertitle_blank]',
                1,
                $lang->is_not_blank,
                [
                    'id' => 'usertitle_blank',
                    'checked' => $input['conditions']['usertitle_blank'],
                ]
            ),
        'usertitle'
    );
    ...
    $form_container->output_row(
        ...
        $form->generate_select_box(
            ...
            $form->generate_text_box(
                'conditions[postnum]',
                $input['conditions']['postnum'],
                ['id' => 'numposts']
            ),
        'numposts'
    );
    $form_container->output_row(
        $lang->thread_count_is,
        '',
        $form->generate_select_box(
            'conditions[threadnum_dir]',
            $greater_options,
            $input['conditions']['threadnum_dir'],
            ['id' => 'numthreads_dir']
        ) .
            ' ' .
            $form->generate_text_box(
                'conditions[threadnum]',
                $input['conditions']['threadnum'],
                ['id' => 'numthreads']
            ),
        'numthreads'
    );
...
function merge_thread_ratings($source_uid, $destination_uid)
{
    ...
    $source_ratings = $dest_threads = $delete_list = $decrement_list = [];

    // Get all thread ratings from both accounts
    $query = $db->simple_select(
        'threadratings',
        'tid, uid, rid, rating',
        "uid IN ({$destination_uid}, {$source_uid})"
    );
        ...
        foreach ($decrement_list as $tid => $ratings) {
            if (is_array($ratings)) {
                $db->update_query(
                    'threads',
                    [
                        'numratings' => 'numratings-' . count($ratings),
                        'totalratings' => 'totalratings-' . array_sum($ratings),
                    ],
                    "tid='{$tid}'",
                    1,
                    true
