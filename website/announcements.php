...
$templatelist =
    'announcement,postbit_groupimage,postbit_reputation,postbit_avatar,postbit_online,postbit_offline,postbit_away,postbit_find,postbit_pm,postbit_email,postbit_author_user';
$templatelist .=
    ',forumdisplay_password_wrongpass,forumdisplay_password,postbit_author_guest,postbit_userstar,announcement_quickdelete,postbit,postbit_www,announcement_edit';
...
$announcementarray['subject'] = $parser->parse_badwords(
    $announcementarray['subject']
);
$lang->forum_announcement = $lang->sprintf(
...
if ($announcementarray['startdate'] > $mybb->user['lastvisit']) {
    $setcookie = true;
    if (
        isset($mybb->cookies['mybb']['announcements']) &&
    ...
    if ($setcookie) {
        my_set_array_cookie(
            'announcements',
            $announcementarray['aid'],
            $announcementarray['startdate'],
            -1
        );
    }
}
