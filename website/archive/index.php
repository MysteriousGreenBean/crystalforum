...
switch ($action) {
    ...
    case 'announcement':
        ...
        $announcement['message'] = $parser->parse_message(
        ...
        archive_header(
            ...
            $announcement['subject'],
            $mybb->settings['bburl'] . "/announcements.php?aid={$id}"
        );

        // Format announcement contents.
        $announcement['startdate'] = my_date(
            'relative',
    ...
    case 'thread':
        ...
        if ($thread['visible'] != 1) {
            if (is_moderator($forum['fid'], 'canviewunapprove')) {
                archive_error(
                    $lang->sprintf(
                        $lang->error_unapproved_thread,
                        $mybb->settings['bburl'] .
                            '/' .
                            get_thread_link($thread['tid'], $page)
                    )
                );
            } else {
        ...
        while ($post = $db->fetch_array($query)) {
            ...
            if (
                ...
                isset($acache[$post['pid']]) &&
                is_array($acache[$post['pid']])
            ) {
                foreach ($acache[$post['pid']] as $aid => $attachment) {
                    $post['message'] = str_replace(
                        ...
                        "[<a href=\"" .
                            $mybb->settings['bburl'] .
                            "/attachment.php?aid={$attachment['aid']}\">attachment={$attachment['aid']}</a>]",
                        $post['message']
                    );
                }
            }
            ...
            $post['username'] = build_profile_link(
    ...
    case 'forum':
        ...
        archive_header(
            strip_tags($forum['name']),
            $forum['name'],
            $mybb->settings['bburl'] . '/' . get_forum_link($id, $page),
            1
        );
        ...
        if ($forum['type'] == 'f') {
            ...
            if ($db->num_rows($query) > 0) {
                ...
                while ($announcement = $db->fetch_array($query)) {
                    ...
                    echo "<li><a href=\"{$base_url}announcement-{$announcement['aid']}.html\">" .
        ...
        if ($forum['type'] == 'f') {
            ...
            $query = $db->simple_select(
                'threads',
                '*',
                "fid='{$id}' AND visible='1' AND sticky='1' AND closed NOT LIKE 'moved|%' {$useronly}",
                $options
            );
            if ($db->num_rows($query) > 0) {
                ...
                while ($sticky = $db->fetch_array($query)) {
                    $sticky['subject'] = htmlspecialchars_uni(
                        $parser->parse_badwords($sticky['subject'])
                    );
                    if ($sticky['replies'] != 1) {
        ...
        if ($forum['type'] == 'f') {
            ...
            $query = $db->simple_select(
                'threads',
                '*',
                "fid='{$id}' AND visible='1' AND sticky='0' AND closed NOT LIKE 'moved|%' {$useronly}",
                $options
            );
            if ($db->num_rows($query) > 0) {
                echo "<div class=\"threadlist\">\n";
