...
$templatelist =
    'calendar_weekdayheader,calendar_weekrow_day,calendar_weekrow,calendar,calendar_addevent,calendar_year,calendar_day,calendar_select,calendar_repeats,calendar_weekview_day_event_time,calendar_weekview_nextlink';
$templatelist .=
    ',calendar_weekview_day,calendar_weekview_day_event,calendar_mini_weekdayheader,calendar_mini_weekrow_day,calendar_mini_weekrow,calendar_mini,calendar_mini_weekrow_day_link,calendar_weekview_prevlink';
$templatelist .=
    ',calendar_event_editbutton,calendar_event_modoptions,calendar_dayview_event,calendar_dayview,codebuttons,calendar_weekrow_day_events,calendar_weekview_month,calendar_addeventlink,calendar_weekview';
$templatelist .=
    ',calendar_jump,calendar_jump_option,calendar_editevent,calendar_dayview_birthdays_bday,calendar_dayview_birthdays,calendar_dayview_noevents,calendar_addevent_calendarselect_hidden,calendar_nextlink';
$templatelist .=
    ',calendar_weekrow_day_birthdays,calendar_weekview_day_birthdays,calendar_year_sel,calendar_event_userstar,calendar_addevent_calendarselect,calendar_eventbit,calendar_event,calendar_move,calendar_prevlink';
...
if (
    ...
    if (
        $calendar_permissions['canviewcalendar'] != 1 ||
        $calendar_permissions['canaddevents'] != 1
    ) {
        error_no_permission();
    }
    ...
    if ($mybb->input['type'] == 'single') {
        ...
        $event['ignoretimezone'] = $mybb->get_input(
            'ignoretimezone',
            MyBB::INPUT_INT
        );
        $repeats = [];
        switch ($mybb->input['repeats']) {
            ...
            case 3:
                $repeats['repeats'] = 3;
                $repeats['weeks'] = $mybb->get_input(
                    'repeats_3_weeks',
                    MyBB::INPUT_INT
                );
                $mybb->input['repeats_3_days'] = $mybb->get_input(
                    'repeats_3_days',
                    MyBB::INPUT_ARRAY
                );
            ...
            case 4:
                ...
                if ($mybb->get_input('repeats_4_type', MyBB::INPUT_INT) == 1) {
                    $repeats['day'] = $mybb->get_input(
                        'repeats_4_day',
                        MyBB::INPUT_INT
                    );
                    $repeats['months'] = $mybb->get_input(
                        'repeats_4_months',
                        MyBB::INPUT_INT
                    );
                } else {
                    ...
                    $repeats['occurance'] = $mybb->get_input(
                        'repeats_4_occurance'
                    );
                    $repeats['weekday'] = $mybb->get_input(
                        'repeats_4_weekday',
                        MyBB::INPUT_INT
                    );
                }
            ...
            case 5:
                ...
                if ($mybb->get_input('repeats_5_type', MyBB::INPUT_INT) == 1) {
                    ...
                    $repeats['month'] = $mybb->get_input(
                        'repeats_5_month',
                        MyBB::INPUT_INT
                    );
                    $repeats['years'] = $mybb->get_input(
                        'repeats_5_years',
                        MyBB::INPUT_INT
                    );
                } else {
    ...
    if (!$eventhandler->validate_event()) {
        ...
        if ($details['visible'] == 1) {
            redirect(
                get_event_link($details['eid']),
                $lang->redirect_eventadded
            );
        } else {
            redirect(
                get_calendar_link($event['cid']),
                $lang->redirect_eventadded_moderation
            );
        }
    }
}
...
if ($mybb->input['action'] == 'addevent') {
    ...
    add_breadcrumb(
        htmlspecialchars_uni($calendar['name']),
        get_calendar_link($calendar['cid'])
    );
    ...
    $single_month = $start_month = $end_month = $repeats_sel = $repeats_3_days = $repeats_4_occurance = $repeats_4_weekday = $repeats_5_month = $repeats_5_occurance = $repeats_5_weekday = $repeats_5_month2 = [];
    foreach (range(1, 12) as $number) {
        $single_month[$number] = $start_month[$number] = $end_month[
            $number
        ] = $repeats_5_month[$number] = $repeats_5_month2[$number] = '';
    }
    foreach (range(1, 5) as $number) {
        $repeats_sel[$number] = '';
    }
    foreach (range(0, 6) as $number) {
        $repeats_3_days[$number] = $repeats_4_weekday[
            $number
        ] = $repeats_5_weekday[$number] = '';
    }
    ...
    if ($mybb->request_method == 'post') {
        ...
        $single_month[$mybb->get_input('single_month', MyBB::INPUT_INT)] =
            " selected=\"selected\"";
        $single_year = $mybb->get_input('single_year', MyBB::INPUT_INT);
        $start_day = $mybb->get_input('start_day', MyBB::INPUT_INT);
        $start_month[$mybb->get_input('start_month', MyBB::INPUT_INT)] =
            " selected=\"selected\"";
        ...
        $end_month[$mybb->get_input('end_month', MyBB::INPUT_INT)] =
            " selected=\"selected\"";
        ...
        $repeats_4_months = $mybb->get_input(
            'repeats_4_months',
            MyBB::INPUT_INT
        );
        $repeats_4_occurance[$mybb->get_input('repeats_4_occurance')] =
            "selected=\"selected\"";
        $repeats_4_weekday[
            $mybb->get_input('repeats_4_weekday', MyBB::INPUT_INT)
        ] = "selected=\"selected\"";
        $repeats_4_months2 = $mybb->get_input(
            'repeats_4_months2',
            MyBB::INPUT_INT
        );
        if ($mybb->get_input('repeats_5_type', MyBB::INPUT_INT) == 1) {
        ...
        $repeats_5_month[$mybb->get_input('repeats_5_month', MyBB::INPUT_INT)] =
            "selected=\"selected\"";
        $repeats_5_years = $mybb->get_input('repeats_5_years', MyBB::INPUT_INT);
        $repeats_5_occurance[$mybb->get_input('repeats_5_occurance')] =
            "selected=\"selected\"";
        $repeats_5_weekday[
            $mybb->get_input('repeats_5_weekday', MyBB::INPUT_INT)
        ] = "selected=\"selected\"";
        $repeats_5_month2[
            $mybb->get_input('repeats_5_month2', MyBB::INPUT_INT)
        ] = "selected=\"selected\"";
        $repeats_5_years2 = $mybb->get_input(
            'repeats_5_years2',
            MyBB::INPUT_INT
        );
        ...
        if (!empty($mybb->input['day'])) {
            $single_day = $start_day = $end_day = $mybb->get_input(
                'day',
                MyBB::INPUT_INT
            );
        } else {
        ...
        $single_month[$month] = $start_month[$month] = $end_month[$month] =
            "selected=\"selected\"";
        if (!empty($mybb->input['year'])) {
            $single_year = $start_year = $end_year = $mybb->get_input(
                'year',
                MyBB::INPUT_INT
            );
        } else {
    ...
    for ($year = my_date('Y'); $year < my_date('Y') + 5; ++$year) {
        ...
        if ($year == $start_year) {
            $selected = "selected=\"selected\"";
            eval(
                "\$start_years .= \"" . $templates->get('calendar_year') . "\";"
            );
        } else {
            $selected = '';
            eval(
        ...
        if ($year == $end_year) {
            $selected = "selected=\"selected\"";
            eval(
                "\$end_years .= \"" . $templates->get('calendar_year') . "\";"
            );
        } else {
            $selected = '';
            eval(
    ...
    for ($day = 1; $day <= 31; ++$day) {
        if ($day == $single_day) {
            $selected = "selected=\"selected\"";
            eval(
                "\$single_days .= \"" . $templates->get('calendar_day') . "\";"
            );
        } else {
            $selected = '';
            eval(
        ...
        if ($day == $start_day) {
            $selected = "selected=\"selected\"";
            eval(
                "\$start_days .= \"" . $templates->get('calendar_day') . "\";"
            );
        } else {
            $selected = '';
            eval(
    ...
    if ($calendarcount > 1) {
        eval(
            "\$calendar_select .= \"" .
                $templates->get('calendar_addevent_calendarselect') .
                "\";"
        );
    } else {
        eval(
            "\$calendar_select .= \"" .
                $templates->get('calendar_addevent_calendarselect_hidden') .
                "\";"
        );
    }
...
// Delete an event
if (
    $mybb->input['action'] == 'do_deleteevent' &&
    $mybb->request_method == 'post'
) {
    ...
    if (
        ($event['uid'] != $mybb->user['uid'] || $mybb->user['uid'] == 0) &&
...
if ($mybb->input['action'] == 'editevent') {
    ...
    if (isset($event_errors)) {
        ...
        $start_date = explode(
            '-',
            gmdate('j-n-Y', $event['starttime'] + $event['timezone'] * 3600)
        );
        ...
        if ($event['usingtime']) {
            $start_time = gmdate(
                ...
                $event['starttime'] + $event['timezone'] * 3600
            );
        } else {
            $start_time = '';
        }
        if ($event['endtime']) {
            $end_date = explode(
                '-',
                gmdate('j-n-Y', $event['endtime'] + $event['timezone'] * 3600)
            );
            $end_day = $end_date[0];
            ...
            if ($event['usingtime']) {
                $end_time = gmdate(
                    $mybb->settings['timeformat'],
                    $event['endtime'] + $event['timezone'] * 3600
                );
            } else {
                $end_time = '';
            }
                ...
                switch ($repeats['repeats']) {
                    ...
                    case 4:
                        ...
                        if ($repeats['day']) {
                            ...
                            $repeats_4_months = $repeats_4_months2 =
                                $repeats['months'];
                        } else {
                            $repeats_4_type[2] = "checked=\"checked\"";
                            $repeats_4_day = 1;
                            $repeats_4_months2 = $repeats_4_months =
                                $repeats['months'];
                            $repeats_4_occurance[$repeats['occurance']] =
                                "selected=\"selected\"";
                            $repeats_4_weekday[$repeats['weekday']] =
                                "selected=\"selected\"";
                        }
                        $repeats_5_type[1] = "checked=\"checked\"";
                        $repeats_5_day = 1;
                    ...
                    case 5:
                        ...
                        if ($repeats['day']) {
                            ...
                            $repeats_5_month[
                                $repeats['month']
                            ] = $repeats_5_month2[$repeats['month']] =
                                "selected=\"selected\"";
                            $repeats_5_years = $repeats_5_years2 =
                                $repeats['years'];
                        } else {
                            $repeats_5_type[2] = "checked=\"checked\"";
                            $repeats_5_occurance[$repeats['occurance']] =
                                "selected=\"selected\"";
                            $repeats_5_weekday[$repeats['weekday']] =
                                "selected=\"selected\"";
                            $repeats_5_month[
                                $repeats['month']
                            ] = $repeats_5_month2[$repeats['month']] =
...
if ($mybb->input['action'] == 'do_move' && $mybb->request_method == 'post') {
    ...
    $query = $db->simple_select(
        'calendars',
        '*',
        "cid='" . $mybb->get_input('new_calendar', MyBB::INPUT_INT) . "'"
    );
    ...
    if (
        $calendar_permissions[$mybb->input['new_calendar']][
            'canviewcalendar'
        ] != 1
    ) {
        error_no_permission();
...
if ($mybb->input['action'] == 'event') {
    ...
    if (
        !$event ||
        ($event['private'] == 1 && $event['uid'] != $mybb->user['uid'])
    ) {
        error($lang->error_invalidevent);
    }
    ...
    $event['description'] = $parser->parse_message(
        $event['description'],
        $event_parser_options
    );
    ...
    if ($event['uid'] > 0 && $event['username']) {
        ...
        $event['profilelink'] = build_profile_link(
            format_name(
                $event['username'],
                $event['usergroup'],
                $event['displaygroup']
        ...
        for ($i = 0; $i < $event['stars']; ++$i) {
            eval(
                "\$event['userstars'] .= \"" .
                    $templates->get('calendar_event_userstar', 1, 0) .
                    "\";"
            );
        }
    ...
    if ($event['endtime'] > 0 && $event['endtime'] != $event['starttime']) {
        ...
        $start_day = adodb_gmmktime(
            0,
            0,
            0,
            gmdate('n', $event['starttime_user']),
            gmdate('j', $event['starttime_user']),
            gmdate('Y', $event['starttime_user'])
        );
        $end_day = adodb_gmmktime(
            0,
            0,
            0,
            gmdate('n', $event['endtime_user']),
            gmdate('j', $event['endtime_user']),
            gmdate('Y', $event['endtime_user'])
        );
        ...
        if ($start_day == $end_day && $event['repeats']['repeats'] == 0) {
            ...
            $time_period =
                ...
                ', ' .
                gmdate($mybb->settings['timeformat'], $event['starttime_user']);
            $time_period .= ' - ';
            $time_period .=
                gmdate($mybb->settings['dateformat'], $event['endtime_user']) .
                ', ' .
                gmdate($mybb->settings['timeformat'], $event['endtime_user']);
        }
    } else {
        $time_period = gmdate(
            $mybb->settings['dateformat'],
    ...
    if (
        $calendar_permissions['canmoderateevents'] == 1 ||
        ($mybb->user['uid'] > 0 && $mybb->user['uid'] == $event['uid'])
    ) {
        eval(
            "\$edit_event = \"" .
                $templates->get('calendar_event_editbutton') .
                "\";"
        );
        if ($calendar_permissions['canmoderateevents'] == 1) {
            if ($event['visible'] == 1) {
            ...
            eval(
                "\$moderator_options = \"" .
                    $templates->get('calendar_event_modoptions') .
                    "\";"
            );
        }
    ...
    if ($mybb->usergroup['canaddevents'] == 1) {
        eval(
            "\$addevent = \"" . $templates->get('calendar_addeventlink') . "\";"
        );
    }
...
if ($mybb->input['action'] == 'dayview') {
    ...
    // Incoming year?
    if (
        isset($mybb->input['year']) &&
        $mybb->get_input('year', MyBB::INPUT_INT) <= my_date('Y') + 5 &&
        $mybb->get_input('year', MyBB::INPUT_INT) >= 1901
    ) {
    ...
    if (
        $mybb->input['day'] &&
        $mybb->input['day'] <=
            gmdate('t', adodb_gmmktime(0, 0, 0, $month, 1, $year))
    ) {
        $day = $mybb->input['day'];
    } else {
        $day = my_date('j');
    ...
    add_breadcrumb(
        ...
        get_calendar_link($calendar['cid'], $year, $month, $day)
    );
    ...
    if ($calendar['showbirthdays']) {
            ...
            foreach ($birthdays2 as $birthday) {
                if ($birthday['birthdayprivacy'] == 'all') {
                    ...
                    if ($bday[2] && $bday[2] < $year) {
                        $age = $year - $bday[2];
                        $age =
                            ' (' . $lang->sprintf($lang->years_old, $age) . ')';
                    } else {
                        $age = '';
                    }

                    $birthday['username'] = format_name(
                        htmlspecialchars_uni($birthday['username']),
                        $birthday['usergroup'],
                        $birthday['displaygroup']
                    );
                    $birthday['profilelink'] = build_profile_link(
                        $birthday['username'],
                        $birthday['uid']
                    );
                    eval(
        ...
        if ($birthday_list) {
            $bdaydate = my_date(
                $mybb->settings['dateformat'],
                adodb_gmmktime(0, 0, 0, $month, $day, $year),
                0,
                0
            );
            $lang->birthdays_on_day = $lang->sprintf(
                $lang->birthdays_on_day,
                $bdaydate
            );
            eval(
                "\$birthdays = \"" .
                    $templates->get('calendar_dayview_birthdays', 1, 0) .
                    "\";"
            );
        }
    }
    ...
    $events = '';
    if (
        isset($events_cache["$day-$month-$year"]) &&
        is_array($events_cache["$day-$month-$year"])
    ) {
        foreach ($events_cache["$day-$month-$year"] as $event) {
            ...
            if ($event['uid'] > 0 && $event['username']) {
                ...
                if (trim($event['usertitle']) != '') {
                    ...
                    is_array($titles_cache) &&
                    !$user_usergroup['usertitle']
                ) {
                    reset($titles_cache);
                    foreach ($titles_cache as $title) {
                        if ($event['postnum'] >= $title['posts']) {
            ...
            if ($repeats) {
                eval(
                    "\$repeats = \"" .
                        $templates->get('calendar_repeats') .
                        "\";"
                );
            }
            ...
            eval(
                "\$events .= \"" .
                    $templates->get('calendar_dayview_event') .
                    "\";"
            );
        }
    }
    ...
    if (!$events) {
        ...
        eval(
            "\$events = \"" .
                $templates->get('calendar_dayview_noevents') .
                "\";"
        );
    }
...
if ($mybb->input['action'] == 'weekview') {
    ...
    if (empty($mybb->input['week'])) {
        ...
        $mybb->input['week'] = adodb_gmmktime(
            0,
            0,
            0,
            $month,
        ...
        $mybb->input['week'] = (int) str_replace(
            'n',
            '-',
            $mybb->get_input('week')
        );
    ...
    $friendly_week_from = gmdate(
        $mybb->settings['dateformat'],
        $mybb->input['week']
    );
    $week_to_stamp = adodb_gmmktime(
        0,
        0,
        0,
        $week_from[1],
    ...
    add_breadcrumb(
        "{$monthnames[$week_from[1]]} {$week_from[2]}",
        get_calendar_link($calendar['cid'], $week_from[2], $week_from[1])
    );
    ...
    if (my_date('Y', $next_week) + 1 <= my_date('Y') + 5) {
        ...
        eval(
            "\$next_week_link = \"" .
                $templates->get('calendar_weekview_nextlink') .
                "\";"
        );
    }
    ...
    while ($weekday_date <= $week_to_stamp) {
        ...
        if (
            ...
            array_key_exists(
                "{$weekday_day}-{$weekday_month}-{$weekday_year}",
                $events_cache
            )
        ) {
            foreach (
                ...
                if ($time_period) {
                    eval(
                        "\$event_time = \"" .
                            $templates->get(
                                'calendar_weekview_day_event_time'
                            ) .
                            "\";"
                    );
                }
                ...
                eval(
                    "\$day_events .= \"" .
                        $templates->get('calendar_weekview_day_event') .
                        "\";"
                );
            }
        }
        ...
        if (
            $calendar['showbirthdays'] &&
            is_array($birthdays) &&
            array_key_exists("{$weekday_day}-{$weekday_month}", $birthdays)
        ) {
            ...
            eval(
                "\$day_birthdays = \"" .
                    $templates->get('calendar_weekview_day_birthdays') .
                    "\";"
            );
        }
        ...
        eval(
            "\$day_bits[$weekday_month] .= \"" .
                $templates->get('calendar_weekview_day') .
                "\";"
        );
        $day_events = $day_birthdays = '';
        $weekday_date = adodb_gmmktime(
            0,
            0,
            0,
            $weekday_month,
    ...
    foreach ($week_months as $month) {
        ...
        eval(
            "\$weekday_bits .= \"" .
                $templates->get('calendar_weekview_month') .
                "\";"
        );
    }
...
if (!$mybb->input['action']) {
    ...
    add_breadcrumb(
        ...
        get_calendar_link($calendar['cid'], $year, $month)
    );
    ...
    if ($prev_month['year'] >= 1901) {
        ...
        eval(
            "\$prev_month_link = \"" .
                $templates->get('calendar_prevlink') .
                "\";"
        );
    }
    ...
    if ($next_month['year'] <= my_date('Y') + 5) {
        ...
        eval(
            "\$next_month_link = \"" .
                $templates->get('calendar_nextlink') .
                "\";"
        );
    }
    ...
    $month_start_weekday = gmdate(
        'w',
        adodb_gmmktime(0, 0, 0, $month, $calendar['startofweek'] + 1, $year)
    );

    $prev_month_days = gmdate(
        't',
        adodb_gmmktime(0, 0, 0, $prev_month['month'], 1, $prev_month['year'])
    );

    // This is if we have days in the previous month to show
    if ($month_start_weekday != $weekdays[0] || $calendar['startofweek'] != 0) {
        $prev_days = $day = gmdate(
            't',
            adodb_gmmktime(
                0,
                0,
    ...
    $start_timestamp = adodb_gmmktime(
        0,
        0,
        0,
        $calendar_month,
    ...
    $month_end_weekday = gmdate(
        'w',
        adodb_gmmktime(0, 0, 0, $month, $num_days, $year)
    );
    ...
    foreach ($weekdays as $weekday) {
        ...
        eval(
            "\$weekday_headers .= \"" .
                $templates->get('calendar_weekdayheader') .
                "\";"
        );
    }
    ...
    for (
        $row = 0;
        $row < 6;
        ++$row // Iterate weeks (each week gets a row)
    ) {
        foreach ($weekdays as $weekday_id => $weekday) {
            ...
            if ($weekday_id == 0) {
                ...
                $week_link = get_calendar_week_link(
                    $calendar['cid'],
                    $week_stamp
                );
            }
            ...
            if (
                is_array($events_cache) &&
                array_key_exists(
                    "{$day}-{$calendar_month}-{$calendar_year}",
                    $events_cache
                )
            ) {
                $total_events = count(
                    $events_cache["$day-$calendar_month-$calendar_year"]
                );
                if (
                    $total_events > $calendar['eventlimit'] &&
                    $calendar['eventlimit'] != 0
                ) {
                    if ($total_events > 1) {
                    ...
                    eval(
                        "\$day_events = \"" .
                            $templates->get('calendar_weekrow_day_events') .
                            "\";"
                    );
                } else {
                    foreach (
                        ...
                        $event['eventlink'] = get_event_link($event['eid']);
                        $event['fullname'] = htmlspecialchars_uni(
                            $event['name']
                        );
                        if (my_strlen($event['name']) > 15) {
                            $event['name'] =
                                my_substr($event['name'], 0, 15) . '...';
                        }
                        $event['name'] = htmlspecialchars_uni($event['name']);
                        if ($event['private'] == 1) {
                            $event_class = ' private_event';
                        ...
                        eval(
                            "\$day_events .= \"" .
                                $templates->get('calendar_eventbit') .
                                "\";"
                        );
                    }
                }
            ...
            if (
                $calendar['showbirthdays'] &&
                is_array($birthdays) &&
                array_key_exists("$day-$calendar_month", $birthdays)
            ) {
                ...
                eval(
                    "\$day_birthdays = \"" .
                        $templates->get('calendar_weekrow_day_birthdays') .
                        "\";"
                );
            }
            ...
            // Is the current day
            if (
                $day . $calendar_month . $year == $today &&
                $month == $calendar_month
            ) {
                eval(
                    "\$day_bits .= \"" .
                        $templates->get('calendar_weekrow_currentday') .
                        "\";"
                );
            }
            // Not in this month
            elseif ($in_month == 0) {
                eval(
                    "\$day_bits .= \"" .
                        $templates->get('calendar_weekrow_day') .
                        "\";"
                );
            }
            // Just a normal day in this month
                ...
                eval(
                    "\$day_bits .= \"" .
                        $templates->get('calendar_weekrow_thismonth') .
                        "\";"
                );
            }
        ...
        if ($day_bits) {
            eval(
                "\$calendar_rows .= \"" .
                    $templates->get('calendar_weekrow') .
                    "\";"
            );
        }
        $day_bits = '';
