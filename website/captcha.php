...
if ($mybb->input['imagehash']) {
    $query = $db->simple_select(
        ...
        '*',
        "imagehash='" .
            $db->escape_string($mybb->get_input('imagehash')) .
            "' AND used=0",
        ['limit' => 1]
    );
    ...
    $db->update_query(
        'captcha',
        ['used' => 1],
        "imagehash='" . $db->escape_string($regimage['imagehash']) . "'"
    );
...
if (function_exists('imagefttext')) {
    ...
    if ($ttfdir !== false) {
        while (($file = readdir($ttfdir)) !== false) {
            ...
            if (
                is_file(MYBB_ROOT . 'inc/captcha_fonts/' . $file) &&
                get_extension($file) == 'ttf'
            ) {
