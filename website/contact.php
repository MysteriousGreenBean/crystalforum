...
$templatelist =
    'contact,post_captcha,post_captcha_recaptcha_invisible,post_captcha_nocaptcha,post_captcha_hcaptcha_invisible,post_captcha_hcaptcha';
...
if (
    ...
    (!$mybb->user['uid'] && $mybb->settings['contact_guests'] == 1)
) {
    error_no_permission();
}
...
if ($mybb->request_method == 'post') {
    ...
    if (
        strlen($mybb->input['subject']) >
            $mybb->settings['contact_maxsubjectlength'] &&
        $mybb->settings['contact_maxsubjectlength'] > 0
    ) {
        $errors[] = $lang->sprintf(
            $lang->subject_too_long,
            $mybb->settings['contact_maxsubjectlength'],
            strlen($mybb->input['subject'])
        );
    }
    ...
    if (
        strlen($mybb->input['message']) >
            $mybb->settings['contact_maxmessagelength'] &&
        $mybb->settings['contact_maxmessagelength'] > 0
    ) {
        $errors[] = $lang->sprintf(
            $lang->message_too_long,
            $mybb->settings['contact_maxmessagelength'],
            strlen($mybb->input['message'])
        );
    }
    ...
    if (
        ...
        $mybb->settings['contact_minmessagelength'] > 0
    ) {
        $errors[] = $lang->sprintf(
            $lang->message_too_short,
            $mybb->settings['contact_minmessagelength'],
            strlen($mybb->input['message'])
        );
    }
    ...
    if (empty($errors)) {
        if ($mybb->settings['contact_badwords'] == 1) {
            ...
            $mybb->input['subject'] = $parser->parse_badwords(
                $mybb->input['subject']
            );
            $mybb->input['message'] = $parser->parse_badwords(
                $mybb->input['message']
            );
        }
        ...
        if ($mybb->user['uid']) {
            $user =
                ...
                $mybb->settings['bburl'] .
                '/' .
                get_profile_link($mybb->user['uid']);
        }

        $subject = $lang->sprintf(
            $lang->email_contact_subject,
            $mybb->input['subject']
        );
        ...
        my_mail(
            ...
            'text',
            '',
            $mybb->get_input('email', MyBB::INPUT_STRING)
        );
...
if (isset($mybb->input['from'])) {
    ...
    strpos($_SERVER['HTTP_REFERER'], $mybb->settings['bburl']) !== false &&
    strpos($_SERVER['HTTP_REFERER'], 'contact.php') === false
) {
