...
$templatelist =
    'editpost,previewpost,changeuserbox,codebuttons,post_attachments_attachment_postinsert,post_attachments_attachment_mod_unapprove,postbit_attachments_thumbnails,postbit_profilefield_multiselect_value';
$templatelist .=
    ',editpost_delete,forumdisplay_password_wrongpass,forumdisplay_password,editpost_reason,post_attachments_attachment_remove,post_attachments_update,post_subscription_method,postbit_profilefield_multiselect';
$templatelist .=
    ',postbit_avatar,postbit_find,postbit_pm,postbit_rep_button,postbit_www,postbit_email,postbit_reputation,postbit_warn,postbit_warninglevel,postbit_author_user,posticons';
$templatelist .=
    ',postbit_signature,postbit,postbit_attachments_thumbnails_thumbnail,postbit_attachments_images_image,postbit_attachments_attachment,postbit_attachments_attachment_unapproved';
$templatelist .=
    ',posticons_icon,post_prefixselect_prefix,post_prefixselect_single,newthread_postpoll,editpost_disablesmilies,post_attachments_attachment_mod_approve,post_attachments_attachment_unapproved';
$templatelist .=
    ',postbit_warninglevel_formatted,postbit_reputation_formatted_link,editpost_signature,attachment_icon,post_attachments_attachment,post_attachments_add,post_attachments,editpost_postoptions,post_attachments_viewlink';
$templatelist .=
    ',postbit_attachments_images,global_moderation_notice,post_attachments_new,postbit_attachments,postbit_online,postbit_away,postbit_offline,postbit_gotopost,postbit_userstar,postbit_icon';
...
if (
    !$post ||
    ($post['visible'] == -1 && $mybb->input['action'] != 'restorepost')
) {
...
if (
    ...
    ($thread['visible'] < -1 && $thread['uid'] != $mybb->user['uid'])
) {
    if (
        $thread['visible'] == 0 &&
        !(
...
if (
    ($forum['open'] == 0 && !is_moderator($fid, 'caneditposts')) ||
...
add_breadcrumb(
    $breadcrumbprefix . $thread['subject'],
...
if ($mybb->input['action'] == 'deletepost' && $mybb->request_method == 'post') {
    if (
        (!is_moderator($fid, 'candeleteposts') &&
            !is_moderator($fid, 'cansoftdeleteposts') &&
    ...
    $mybb->input['action'] == 'restorepost' &&
    $mybb->request_method == 'post'
) {
    if (
        (!is_moderator($fid, 'canrestoreposts') &&
            $pid != $thread['firstpost']) ||
    ...
    if (!is_moderator($fid, 'caneditposts')) {
        ...
        $time = TIME_NOW;
        if (
            $mybb->usergroup['edittimelimit'] != 0 &&
            $post['dateline'] < $time - $mybb->usergroup['edittimelimit'] * 60
        ) {
            $lang->edit_time_limit = $lang->sprintf(
                $lang->edit_time_limit,
                $mybb->usergroup['edittimelimit']
            );
...
if (
    ...
    ($mybb->get_input('newattachment') ||
        ...
        ((($mybb->input['action'] == 'do_editpost' &&
            isset($mybb->input['submitbutton'])) ||
            ($mybb->input['action'] == 'editpost' &&
...
if (
    ...
    isset($mybb->input['attachmentact']) &&
    $mybb->input['action'] == 'do_editpost' &&
    $mybb->request_method == 'post'
) {
    ...
    $mybb->input['attachmentaid'] = $mybb->get_input(
        'attachmentaid',
        MyBB::INPUT_INT
    );
    if ($mybb->input['attachmentact'] == 'remove') {
        ...
        $mybb->get_input('attachmentact') == 'approve' &&
        is_moderator($fid, 'canapproveunapproveattachs')
    ) {
        $update_sql = ['visible' => 1];
        $db->update_query(
            'attachments',
            $update_sql,
            "aid='{$mybb->input['attachmentaid']}'"
        );
        update_thread_counters($post['tid'], ['attachmentcount' => '+1']);
    } elseif (
        $mybb->get_input('attachmentact') == 'unapprove' &&
        is_moderator($fid, 'canapproveunapproveattachs')
    ) {
...
if ($mybb->input['action'] == 'deletepost' && $mybb->request_method == 'post') {
    ...
    if ($mybb->get_input('delete', MyBB::INPUT_INT) == 1) {
        ...
        if ($firstpost) {
            if (
                $forumpermissions['candeletethreads'] == 1 ||
                is_moderator($fid, 'candeletethreads') ||
                is_moderator($fid, 'cansoftdeletethreads')
            ) {
            ...
            if (
                $forumpermissions['candeleteposts'] == 1 ||
                is_moderator($fid, 'candeleteposts') ||
                is_moderator($fid, 'cansoftdeleteposts')
            ) {
                ...
                $query = $db->simple_select(
                    'posts',
                    'pid',
                    "tid='{$tid}' AND dateline <= '{$post['dateline']}'",
                    ['limit' => 1, 'order_by' => 'dateline DESC, pid DESC']
                );
                $next_post = $db->fetch_array($query);
                if ($next_post['pid']) {
                    $redirect =
                        get_post_link($next_post['pid'], $tid) .
                        "#pid{$next_post['pid']}";
                } else {
...
if (
    ...
    // No errors were found, we can call the update method.
        ...
        if (
            ...
            $visible == 0 &&
            $first_post &&
            !is_moderator($fid, 'canviewunapprove', $mybb->user['uid'])
        ) {
            // Moderated post
...
if (!$mybb->input['action'] || $mybb->input['action'] == 'editpost') {
    ...
    if (
        ...
        $thread['poll'] < 1 &&
        (is_moderator($fid, 'canmanagepolls') ||
            $thread['dateline'] >
    ...
    if (
        ...
        eval(
            "\$signature = \"" . $templates->get('editpost_signature') . "\";"
        );
    }
    ...
    if ($forum['allowsmilies'] != 0) {
        eval(
            "\$disablesmilies = \"" .
                $templates->get('editpost_disablesmilies') .
                "\";"
        );
    }
    ...
    if (!empty($signature) || !empty($disablesmilies)) {
        eval(
            "\$postoptions = \"" .
                $templates->get('editpost_postoptions') .
                "\";"
        );
    }
