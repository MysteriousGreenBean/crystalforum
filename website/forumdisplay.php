...
$templatelist =
    'forumdisplay,forumdisplay_thread,forumbit_depth1_cat,forumbit_depth2_cat,forumbit_depth2_forum,forumdisplay_subforums,forumdisplay_threadlist,forumdisplay_moderatedby,forumdisplay_searchforum,forumdisplay_forumsort,forumdisplay_thread_rating,forumdisplay_threadlist_rating';
$templatelist .=
    ',forumbit_depth1_forum_lastpost,forumdisplay_thread_multipage_page,forumdisplay_thread_multipage,forumdisplay_thread_multipage_more,forumdisplay_thread_gotounread,forumbit_depth2_forum_lastpost,forumdisplay_rules_link,forumdisplay_orderarrow,forumdisplay_newthread';
$templatelist .=
    ',multipage,multipage_breadcrumb,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start,forumdisplay_thread_unapproved_posts,forumdisplay_nothreads';
$templatelist .=
    ',forumjump_advanced,forumjump_special,forumjump_bit,forumdisplay_password_wrongpass,forumdisplay_password,forumdisplay_inlinemoderation_custom_tool,forumbit_subforums,forumbit_moderators,forumbit_depth2_forum_lastpost_never,forumbit_depth2_forum_lastpost_hidden';
$templatelist .=
    ',forumdisplay_usersbrowsing_user,forumdisplay_usersbrowsing,forumdisplay_inlinemoderation,forumdisplay_thread_modbit,forumdisplay_inlinemoderation_col,forumdisplay_inlinemoderation_selectall,forumdisplay_threadlist_clearpass,forumdisplay_thread_rating_moved';
$templatelist .=
    ',forumdisplay_announcements_announcement,forumdisplay_announcements,forumdisplay_threads_sep,forumbit_depth3_statusicon,forumbit_depth3,forumdisplay_sticky_sep,forumdisplay_thread_attachment_count,forumdisplay_rssdiscovery,forumbit_moderators_group';
$templatelist .=
    ',forumdisplay_inlinemoderation_openclose,forumdisplay_inlinemoderation_stickunstick,forumdisplay_inlinemoderation_softdelete,forumdisplay_inlinemoderation_restore,forumdisplay_inlinemoderation_delete,forumdisplay_inlinemoderation_manage,forumdisplay_nopermission';
$templatelist .=
    ',forumbit_depth2_forum_unapproved_posts,forumbit_depth2_forum_unapproved_threads,forumbit_moderators_user,forumdisplay_inlinemoderation_standard,forumdisplay_threadlist_prefixes_prefix,forumdisplay_threadlist_prefixes,forumdisplay_thread_icon,forumdisplay_rules';
$templatelist .=
    ',forumdisplay_thread_deleted,forumdisplay_announcements_announcement_modbit,forumbit_depth2_forum_viewers,forumdisplay_threadlist_sortrating,forumdisplay_inlinemoderation_custom,forumdisplay_announcement_rating,forumdisplay_inlinemoderation_approveunapprove,forumdisplay_threadlist_subscription';
...
if ($mybb->user['uid'] == 0) {
    ...
    $query = $db->simple_select('forums', '*', 'active != 0', [
...
if (!empty($child_forums) && !empty($child_forums['forum_list'])) {
    ...
    $lang->sub_forums_in = $lang->sprintf(
        $lang->sub_forums_in,
        $foruminfo['name']
    );
    eval(
...
if ($fpermissions['cansearch'] != 0 && $foruminfo['type'] == 'f') {
    eval(
        "\$searchforum = \"" .
            $templates->get('forumdisplay_searchforum') .
            "\";"
    );
}
...
if (is_array($forum_stats)) {
    if (
        ...
        !empty($forum_stats[$fid]['modtools'])
    ) {
    ...
    if (
        !empty($forum_stats[-1]['announcements']) ||
        !empty($forum_stats[$fid]['announcements'])
    ) {
        ...
        foreach ($moderatorcache[$mfid] as $modtype) {
            foreach ($modtype as $moderator) {
                if ($moderator['isgroup']) {
                    ...
                    $moderator['title'] = htmlspecialchars_uni(
                        $moderator['title']
                    );

                    eval(
                        "\$moderators .= \"" .
                            $templates->get('forumbit_moderators_group', 1, 0) .
                            "\";"
                    );
                    ...
                    $moderator['profilelink'] = get_profile_link(
                        $moderator['id']
                    );
                    ...
                    eval(
                        "\$moderators .= \"" .
                            $templates->get('forumbit_moderators_user', 1, 0) .
                            "\";"
                    );
...
if ($moderators) {
    eval(
        "\$moderatedby = \"" .
            $templates->get('forumdisplay_moderatedby') .
            "\";"
    );
} else {
    $moderatedby = '';
...
if ($mybb->settings['browsingthisforum'] != 0) {
    ...
    while ($user = $db->fetch_array($query)) {
        if (
            ...
            if (
                ...
                $user['profilelink'] = build_profile_link(
                    $user['username'],
                    $user['uid']
                );
                eval(
    ...
    if ($guestcount) {
        $guestsonline = $lang->sprintf(
            $lang->users_browsing_forum_guests,
            $guestcount
        );
    }
    ...
    if ($inviscount && $mybb->usergroup['canviewwolinvis'] != 1) {
        $invisonline = $lang->sprintf(
            $lang->users_browsing_forum_invis,
            $inviscount
        );
    }
    ...
    eval(
        "\$usersbrowsing = \"" .
            $templates->get('forumdisplay_usersbrowsing') .
            "\";"
    );
}
...
if ($foruminfo['rulestype'] != 0 && $foruminfo['rules']) {
    if (!$foruminfo['rulestitle']) {
        $foruminfo['rulestitle'] = $lang->sprintf(
            $lang->forum_rules,
            $foruminfo['name']
        );
    }
    ...
    $foruminfo['rules'] = $parser->parse_message(
        $foruminfo['rules'],
        $rules_parser
    );
...
if (is_moderator($fid)) {
    eval(
        "\$inlinemodcol = \"" .
            $templates->get('forumdisplay_inlinemoderation_col') .
            "\";"
    );
    $ismod = true;
    $inlinecount = '0';
...
if ($mybb->user['uid'] && $mybb->settings['showownunapproved']) {
    $visible_condition .=
        ' OR (t.visible=0 AND t.uid=' . (int) $mybb->user['uid'] . ')';
}
...
if ($mybb->input['page'] > 1) {
    $sorturl =
        get_forum_link($fid, $mybb->input['page']) .
        $string .
        "datecut=$datecut&amp;prefix=$tprefix";
} else {
    $sorturl =
    ...
    if ($useronly === '' && $datecutsql === '' && $prefixsql === '') {
        ...
        if (in_array(0, $visible_states)) {
            ...
            $query = $db->simple_select(
                ...
                "fid = '$fid' AND t.visible=0 AND t.uid=" .
                    (int) $mybb->user['uid']
            );
        ...
        $query = $db->simple_select(
            'threads t',
            'COUNT(tid) AS threads',
            "fid = '$fid' $tuseronly $tvisibleonly $datecutsql2 $prefixsql2"
        );
...
if (
    ...
    eval(
        "\$ratingcol = \"" .
            $templates->get('forumdisplay_threadlist_rating') .
            "\";"
    );
    eval(
        "\$ratingsort = \"" .
            $templates->get('forumdisplay_threadlist_sortrating') .
            "\";"
    );
    $colspan = '7';
} else {
...
if ($has_announcements == true) {
    ...
    if (isset($mybb->cookies['mybb']['announcements'])) {
        $cookie = my_unserialize(
            stripslashes($mybb->cookies['mybb']['announcements']),
            false
        );
    }
    ...
    while ($announcement = $db->fetch_array($query)) {
        if (
            $announcement['startdate'] > $mybb->user['lastvisit'] &&
        ...
        if (
            isset($cookie[$announcement['aid']]) &&
            $cookie[$announcement['aid']] < $mybb->user['lastvisit']
        ) {
            unset($cookie[$announcement['aid']]);
        }

        $announcement['announcementlink'] = get_announcement_link(
            $announcement['aid']
        );
        $announcement['subject'] = $parser->parse_badwords(
        ...
        if (
            ...
            eval(
                "\$rating = \"" .
                    $templates->get('forumdisplay_announcement_rating') .
                    "\";"
            );
            $lpbackground = 'trow2';
        } else {
        ...
        eval(
            "\$announcements .= \"" .
                $templates->get('forumdisplay_announcements_announcement') .
                "\";"
        );
        $bgcolor = alt_trow();
    }
    ...
    if ($announcements) {
        eval(
            "\$announcementlist = \"" .
                $templates->get('forumdisplay_announcements') .
                "\";"
        );
        $shownormalsep = true;
    }
    ...
    if (empty($cookie)) {
        ...
        my_setcookie(
            'mybb[announcements]',
            addslashes(my_serialize($cookie)),
            -1
        );
    }
} else {
...
if ($fpermissions['canviewthreads'] != 0) {
    ...
    if (
        ...
        $query = $db->simple_select(
            'threadratings',
            'tid, uid',
            "tid IN ({$imp}) AND uid = '{$mybb->user['uid']}'"
        );
...
if (
    ...
    $query = $db->simple_select(
        'posts',
        'DISTINCT tid,uid',
        "uid='{$mybb->user['uid']}' AND tid IN ({$tids}) {$visibleonly}"
    );
...
if (!empty($threadcache) && is_array($threadcache)) {
    ...
    foreach ($threadcache as $thread) {
        ...
        if (!$thread['username']) {
            if (!$thread['threadusername']) {
                ...
                $thread['username'] = $thread[
                    'profilelink'
                ] = htmlspecialchars_uni($thread['threadusername']);
            }
        } else {
        ...
        if ($thread['sticky'] == '1' && !isset($donestickysep)) {
            eval(
                "\$threads .= \"" .
                    $templates->get('forumdisplay_sticky_sep') .
                    "\";"
            );
            $shownormalsep = true;
            $donestickysep = true;
            ...
            eval(
                "\$threads .= \"" .
                    $templates->get('forumdisplay_threads_sep') .
                    "\";"
            );
            $shownormalsep = false;
        }
        ...
        if (
            ...
            if (
                $moved[0] == 'moved' ||
                ($fpermissions['canviewdeletionnotice'] != 0 &&
                    $thread['visible'] == -1)
            ) {
                eval(
                    "\$rating = \"" .
                        $templates->get('forumdisplay_thread_rating_moved') .
                        "\";"
                );
            } else {
                ...
                if ($thread['numratings'] == 0) {
                    ...
                    $thread['width'] =
                        (int) round($thread['averagerating']) * 20;
                }
                ...
                $ratingvotesav = $lang->sprintf(
                    $lang->rating_votes_average,
                    $thread['numratings'],
                    $thread['averagerating']
                );
                eval(
                    "\$rating = \"" .
                        $templates->get('forumdisplay_thread_rating') .
                        "\";"
                );
            }
        }
        ...
        if ($thread['posts'] > $mybb->settings['postsperpage']) {
            ...
            if ($thread['pages'] > $mybb->settings['maxmultipagelinks']) {
                ...
                eval(
                    "\$morelink = \"" .
                        $templates->get('forumdisplay_thread_multipage_more') .
                        "\";"
                );
            } else {
            ...
            for ($i = 1; $i <= $pagesstop; ++$i) {
                ...
                eval(
                    "\$threadpages .= \"" .
                        $templates->get('forumdisplay_thread_multipage_page') .
                        "\";"
                );
            }
            ...
            eval(
                "\$thread['multipage'] = \"" .
                    $templates->get('forumdisplay_thread_multipage') .
                    "\";"
            );
        } else {
            $threadpages = '';
        ...
        if ($ismod) {
            if (
                isset($mybb->cookies[$inlinecookie]) &&
                my_strpos(
                    $mybb->cookies[$inlinecookie],
                    "|{$thread['tid']}|"
                ) !== false
            ) {
            ...
            eval(
                "\$modbit = \"" .
                    $templates->get('forumdisplay_thread_modbit') .
                    "\";"
            );
        } else {
            $modbit = '';
        ...
        if (
            $thread['unapprovedposts'] > 0 &&
            is_moderator($fid, 'canviewunapprove')
        ) {
            if ($thread['unapprovedposts'] > 1) {
                $unapproved_posts_count = $lang->sprintf(
                    $lang->thread_unapproved_posts_count,
                    $thread['unapprovedposts']
                );
            } else {
                $unapproved_posts_count = $lang->sprintf(
                    $lang->thread_unapproved_post_count,
                    1
                );
            }
            ...
            eval(
                "\$unapproved_posts = \"" .
                    $templates->get('forumdisplay_thread_unapproved_posts') .
                    "\";"
            );
        } else {
            $unapproved_posts = '';
        ...
        if (
            ...
            if ($thread['attachmentcount'] > 1) {
                $attachment_count = $lang->sprintf(
                    $lang->attachment_count_multiple,
            ...
            eval(
                "\$attachment_count = \"" .
                    $templates->get('forumdisplay_thread_attachment_count') .
                    "\";"
            );
        } else {
            $attachment_count = '';
        ...
        if (
            ...
            eval(
                "\$threads .= \"" .
                    $templates->get('forumdisplay_thread_deleted') .
                    "\";"
            );
        } else {
            $thread['start_datetime'] = my_date(
                'relative',
                $thread['dateline']
            );
            eval(
                "\$threads .= \"" .
                    $templates->get('forumdisplay_thread') .
                    "\";"
            );
        }
    }
    ...
    if ($ismod) {
        ...
        if (is_moderator($fid, 'canopenclosethreads')) {
            eval(
                "\$inlinemodopenclose = \"" .
                    $templates->get('forumdisplay_inlinemoderation_openclose') .
                    "\";"
            );
        }
        ...
        if (is_moderator($fid, 'canstickunstickthreads')) {
            eval(
                "\$inlinemodstickunstick = \"" .
                    $templates->get(
                        'forumdisplay_inlinemoderation_stickunstick'
                    ) .
                    "\";"
            );
        }
        ...
        if (is_moderator($fid, 'cansoftdeletethreads')) {
            eval(
                "\$inlinemodsoftdelete = \"" .
                    $templates->get(
                        'forumdisplay_inlinemoderation_softdelete'
                    ) .
                    "\";"
            );
        }
        ...
        if (is_moderator($fid, 'canrestorethreads')) {
            eval(
                "\$inlinemodrestore = \"" .
                    $templates->get('forumdisplay_inlinemoderation_restore') .
                    "\";"
            );
        }
        ...
        if (is_moderator($fid, 'candeletethreads')) {
            eval(
                "\$inlinemoddelete = \"" .
                    $templates->get('forumdisplay_inlinemoderation_delete') .
                    "\";"
            );
        }
        ...
        if (is_moderator($fid, 'canmanagethreads')) {
            eval(
                "\$inlinemodmanage = \"" .
                    $templates->get('forumdisplay_inlinemoderation_manage') .
                    "\";"
            );
        }
        ...
        if (is_moderator($fid, 'canapproveunapprovethreads')) {
            eval(
                "\$inlinemodapproveunapprove = \"" .
                    $templates->get(
                        'forumdisplay_inlinemoderation_approveunapprove'
                    ) .
                    "\";"
            );
        }
        ...
        if (
            !empty($inlinemodopenclose) ||
            !empty($inlinemodstickunstick) ||
            ...
            eval(
                "\$standardthreadtools = \"" .
                    $templates->get('forumdisplay_inlinemoderation_standard') .
                    "\";"
            );
        }
        ...
        if (!empty($standardthreadtools) || !empty($customthreadtools)) {
            eval(
                "\$inlinemod = \"" .
                    $templates->get('forumdisplay_inlinemoderation') .
                    "\";"
            );
        }
    }
...
if ($mybb->user['uid']) {
    ...
    eval(
        "\$addremovesubscription = \"" .
            $templates->get('forumdisplay_threadlist_subscription') .
            "\";"
    );
}
...
if ($foruminfo['type'] != 'c') {
    if ($fpermissions['canviewthreads'] != 1) {
        eval(
            "\$threads = \"" .
                $templates->get('forumdisplay_nopermission') .
                "\";"
        );
    }
    ...
    if ($foruminfo['password'] != '') {
        eval(
            "\$clearstoredpass = \"" .
                $templates->get('forumdisplay_threadlist_clearpass') .
                "\";"
        );
    }
    ...
    $forumsort = '';
    eval(
        "\$forumsort = \"" . $templates->get('forumdisplay_forumsort') . "\";"
    );

    $plugins->run_hooks('forumdisplay_threadlist');

    $lang->rss_discovery_forum = $lang->sprintf(
        $lang->rss_discovery_forum,
        htmlspecialchars_uni(strip_tags($foruminfo['name']))
    );
    eval(
        "\$rssdiscovery = \"" .
            $templates->get('forumdisplay_rssdiscovery') .
            "\";"
    );
    eval(
        "\$threadslist = \"" .
            $templates->get('forumdisplay_threadlist') .
            "\";"
    );
} else {
    $rssdiscovery = '';
