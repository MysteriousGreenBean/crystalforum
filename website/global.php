...
if (
    (isset($mybb->input['action']) &&
        isset($nosession[$mybb->input['action']])) ||
    (isset($mybb->input['thumbnail']) && $current_page == 'attachment.php')
) {
    define('NO_ONLINE', 1);
}
...
if (
    ...
    $lang->language_exists($mybb->get_input('language')) &&
...
elseif (
    !$mybb->user['uid'] &&
    !empty($mybb->cookies['mybblang']) &&
    $lang->language_exists($mybb->cookies['mybblang'])
) {
...
if (
    isset($mybb->cookies['lockoutexpiry']) &&
    $mybb->cookies['lockoutexpiry'] < TIME_NOW
) {
...
if (in_array($current_page, $valid)) {
    ...
    elseif (isset($mybb->input['pid']) && THIS_SCRIPT == 'polls.php') {
        $query = $db->query(
            'SELECT t.fid FROM ' .
                ...
                'polls p INNER JOIN ' .
                TABLE_PREFIX .
                "threads t ON (t.tid=p.tid) WHERE p.pid = '{$mybb->input['pid']}' LIMIT 1"
        );
    ...
    elseif (
        isset($mybb->input['fid']) &&
        isset($forum_cache[$mybb->input['fid']])
    ) {
...
if ($loadstyle != "def='1'") {
    $query = $db->simple_select(
        'themes',
        'name, tid, properties, stylesheets, allowedgroups',
        $loadstyle,
        ['limit' => 1]
    );
    ...
    if (
        ...
        if ($load_from_user == 1) {
            $db->update_query(
                'users',
                ['style' => 0],
                "style='{$mybb->user['style']}' AND uid='{$mybb->user['uid']}'"
            );
        }
...
if ($isDevMode) {
    ...
    if (isset($theme_stylesheet_map[$theme['tid']])) {
        ...
        if (is_dir($stylesheets_dir)) {
            ...
            $cache_content =
                "<?php\nreturn " .
                var_export($theme_stylesheet_map, true) .
                ";\n";
    ...
    if ($theme_stylesheet_file) {
        $theme_stylesheet_content = file_get_contents(
            $theme_stylesheet_path . $theme_stylesheet_file
        );
...
foreach ($stylesheet_scripts as $stylesheet_script) {
    ...
    foreach ($stylesheet_actions as $stylesheet_action) {
        ...
        if (
            !empty(
                $theme['stylesheets'][$stylesheet_script][$stylesheet_action]
            )
        ) {
...
if (my_validate_url($theme['imgdir'])) {
    ...
    if (!empty($mybb->user['language'])) {
        ...
        if (!empty($mybb->settings['bblanguage'])) {
            $theme['imglangdir'] =
                $theme['imgdir'] . '/' . $mybb->settings['bblanguage'];
        }
    ...
    if (
        ...
        is_dir($img_directory . '/' . $mybb->user['language'])
    ) {
        $theme['imglangdir'] = $theme['imgdir'] . '/' . $mybb->user['language'];
    } else {
        // Check if a custom language directory exists for this theme
...
if (
    !preg_match("#^(\.\.?(/|$)|([a-z0-9]+)://)#i", $theme['logo']) &&
    substr($theme['logo'], 0, 1) != '/'
) {
...
$templatelist .=
    'headerinclude,header,footer,gobutton,htmldoctype,header_welcomeblock_member,header_welcomeblock_member_user,header_welcomeblock_member_moderator,header_welcomeblock_member_admin,error';
$templatelist .=
    ',global_pending_joinrequests,global_awaiting_activation,nav,nav_sep,nav_bit,nav_sep_active,nav_bit_active,footer_languageselect,footer_themeselect,global_unreadreports,footer_contactus';
$templatelist .=
    ',global_boardclosed_warning,global_bannedwarning,error_inline,error_inline_item,error_nopermission_loggedin,error_nopermission,global_pm_alert,header_menu_search,header_menu_portal,redirect,footer_languageselect_option';
$templatelist .=
    ',video_dailymotion_embed,video_facebook_embed,video_liveleak_embed,video_metacafe_embed,video_myspacetv_embed,video_mixer_embed,video_vimeo_embed,video_yahoo_embed,video_youtube_embed,debug_summary';
$templatelist .=
    ',smilieinsert_row,smilieinsert_row_empty,smilieinsert,smilieinsert_getmore,smilieinsert_smilie,global_board_offline_modal,footer_showteamlink,footer_themeselector,task_image,usercp_themeselector_option,php_warnings';
$templatelist .=
    ',mycode_code,mycode_email,mycode_img,mycode_php,mycode_quote_post,mycode_size_int,mycode_url,global_no_permission_modal,global_boardclosed_reason,nav_dropdown,global_remote_avatar_notice,global_modqueue,global_modqueue_notice';
$templatelist .=
    ',header_welcomeblock_member_buddy,header_welcomeblock_member_pms,header_welcomeblock_member_search,header_welcomeblock_guest,header_welcomeblock_guest_login_modal,header_welcomeblock_guest_login_modal_lockout';
$templatelist .=
    ',header_menu_calendar,header_menu_memberlist,global_dst_detection,header_quicksearch,smilie,modal,modal_button';
$templates->cache($db->escape_string($templatelist));

// Set the current date and time now
$datenow = my_date($mybb->settings['dateformat'], TIME_NOW, '', false);
$timenow = my_date($mybb->settings['timeformat'], TIME_NOW);
$lang->welcome_current_time = $lang->sprintf(
    $lang->welcome_current_time,
    $datenow . $lang->comma . $timenow
);
...
if (
    ...
    eval(
        '$bbclosedwarning = "' .
            $templates->get('global_boardclosed_warning') .
            '";'
    );
}
...
if ($mybb->user['uid'] != 0) {
    ...
    if (
        ...
        eval(
            '$admincplink = "' .
                $templates->get('header_welcomeblock_member_admin') .
                '";'
        );
    }
    ...
    if ($mybb->usergroup['canmodcp'] == 1) {
        eval(
            '$modcplink = "' .
                $templates->get('header_welcomeblock_member_moderator') .
                '";'
        );
    }
    ...
    if ($mybb->usergroup['canusercp'] == 1) {
        eval(
            '$usercplink = "' .
                $templates->get('header_welcomeblock_member_user') .
                '";'
        );
    }
    ...
    if (!empty($mybb->user['buddylist'])) {
        eval(
            '$buddylink = "' .
                $templates->get('header_welcomeblock_member_buddy') .
                '";'
        );
    }
    ...
    if ($mybb->usergroup['cansearch'] == 1) {
        eval(
            '$searchlink = "' .
                $templates->get('header_welcomeblock_member_search') .
                '";'
        );
    }
    ...
    if (
        ...
        $lang->welcome_pms_usage = $lang->sprintf(
            $lang->welcome_pms_usage,
            my_number_format($mybb->user['pms_unread']),
            my_number_format($mybb->user['pms_total'])
        );

        eval(
            '$pmslink = "' .
                $templates->get('header_welcomeblock_member_pms') .
                '";'
        );
    }
...
// Otherwise, we have a guest
    ...
    if (!empty($mybb->cookies['lockoutexpiry'])) {
        ...
        $lang->failed_login_wait = $lang->sprintf(
        ...
        eval(
            '$loginform = "' .
                $templates->get('header_welcomeblock_guest_login_modal') .
                '";'
        );
    }
    ...
    eval(
        '$welcomeblock = "' .
            $templates->get('header_welcomeblock_guest') .
            '";'
    );
}
...
if (
    $mybb->settings['enablememberlist'] == 1 &&
    $mybb->usergroup['canviewmemberlist'] == 1
) {
    eval(
        '$menu_memberlist = "' .
            $templates->get('header_menu_memberlist') .
            '";'
    );
}

if (
    $mybb->settings['enablecalendar'] == 1 &&
...
if (
    $mybb->user['uid'] != 0 &&
    is_array($groupleaders) &&
    array_key_exists($mybb->user['uid'], $groupleaders)
) {
    ...
    foreach ($groupleader as $user) {
        ...
        if (
            !empty($groupscache[$user['gid']]['type']) &&
            $groupscache[$user['gid']]['type'] == 4
        ) {
            $showjoinnotice = true;
            $gids .= ",'{$user['gid']}'";
        }
    ...
    if ($showjoinnotice) {
        ...
        if ($total_joinrequests > 0) {
            if ($total_joinrequests == 1) {
                ...
                $lang->pending_joinrequests = $lang->sprintf(
                    $lang->pending_joinrequests,
                    my_number_format($total_joinrequests)
                );
            }

            eval(
                '$pending_joinrequests = "' .
                    $templates->get('global_pending_joinrequests') .
                    '";'
            );
        }
    }
        ...
        if ($reported['unread'] > 0) {
            ...
            if ($unread > 0) {
                if ($unread == 1) {
                    ...
                    $lang->unread_reports = $lang->sprintf(
                        $lang->unread_reports,
                        my_number_format($unread)
                    );
                }

                eval(
                    '$moderation_queue[] = "' .
                        $templates->get('global_unreadreports', 1, 0) .
                        '";'
                );
            }
        }
...
if (
    !(defined('THIS_SCRIPT') && THIS_SCRIPT == 'editpost.php') &&
    ($can_access_moderationqueue ||
        ($mybb->user['ismoderator'] &&
            $mybb->usergroup['canmodcp'] == 1 &&
    ...
    $query = $db->simple_select(
        'attachments',
        'COUNT(aid) AS unapprovedattachments',
        'visible=0'
    );
    $unapproved_attachments = $db->fetch_field($query, 'unapprovedattachments');

    $modqueue_types = ['threads', 'posts', 'attachments'];

    foreach ($modqueue_types as $modqueue_type) {
        if (!empty(${'unapproved_' . $modqueue_type})) {
            if (${'unapproved_' . $modqueue_type} == 1) {
                $modqueue_message =
                    $lang->{'unapproved_' . substr($modqueue_type, 0, -1)};
            } else {
                $modqueue_message = $lang->sprintf(
                    $lang->{'unapproved_' . $modqueue_type},
                    my_number_format(${'unapproved_' . $modqueue_type})
                );
            }

            eval(
                '$moderation_queue[] = "' .
                    $templates->get('global_modqueue', 1, 0) .
                    '";'
            );
        }
    }
...
if (!empty($moderation_queue)) {
    ...
    if (empty($moderation_queue)) {
        ...
        $moderation_queue =
            implode($lang->comma, $moderation_queue) .
            ' ' .
            $lang->and .
            ' ' .
            $moderation_queue_last;
    }
...
$pm_notice = '';
if (
    isset($mybb->user['pmnotice']) &&
    $mybb->user['pmnotice'] == 2 &&
    $mybb->user['pms_unread'] > 0 &&
    $mybb->settings['enablepms'] != 0 &&
    $mybb->usergroup['canusepms'] != 0 &&
    $mybb->usergroup['canview'] != 0 &&
    ($current_page != 'private.php' || $mybb->get_input('action') != 'read')
) {
    if (!isset($parser)) {
    ...
    if ($mybb->user['pms_unread'] == 1) {
        $privatemessage_text = $lang->sprintf(
...
if (
    isset($mybb->user['avatartype']) &&
    ($mybb->user['avatartype'] === 'remote' ||
        $mybb->user['avatartype'] === 'gravatar') &&
    !$mybb->settings['allowremoteavatars']
) {
    eval(
        '$remote_avatar_notice = "' .
            $templates->get('global_remote_avatar_notice') .
            '";'
    );
}
...
if ($mybb->settings['awactialert'] == 1 && $mybb->usergroup['cancp'] == 1) {
    ...
    if (
        isset($awaitingusers['time']) &&
        $awaitingusers['time'] + 86400 < TIME_NOW
    ) {
    ...
    if ($awaitingusers > 0) {
        if ($awaitingusers == 1) {
            ...
            $awaiting_message = $lang->sprintf(
                $lang->awaiting_message_plural,
                $awaitingusers
            );
        }
        ...
        eval(
            '$awaitingusers = "' .
                $templates->get('global_awaiting_activation') .
                '";'
        );
    } else {
        $awaitingusers = '';
...
$jsTemplates = [];
foreach (['modal', 'modal_button'] as $template) {
    eval(
        '$jsTemplates["' .
            $template .
            '"] = "' .
            $templates->get($template, 1, 0) .
            '";'
    );
...
if ($mybb->settings['showlanguageselect'] != 0) {
    ...
    if (count($languages) > 1) {
        foreach ($languages as $key => $language) {
            ...
            eval(
                '$lang_options .= "' .
                    $templates->get('footer_languageselect_option') .
                    '";'
            );
        }
...
if ($mybb->user['uid'] > 0 && $mybb->user['dstcorrection'] == 2) {
    ...
    eval(
        '$auto_dst_detection = "' .
            $templates->get('global_dst_detection') .
            '";'
    );
}
...
$closed_bypass = [
    'member.php' => ['login', 'do_login', 'logout'],
    'captcha.php',
    'contact.php',
];
...
if (
    ...
    if (!$mybb->get_input('modal')) {
        ...
        eval(
            '$output = "' .
                $templates->get('global_board_offline_modal', 1, 0) .
                '";'
        );
        echo $output;
    }
...
if (
    $mybb->usergroup['cancp'] != 1 &&
    $mybb->settings['load'] > 0 &&
    ($load = get_server_load()) &&
    $load != $lang->unknown &&
    $load > $mybb->settings['load']
) {
...
if (
    ...
    (isset($mybb->input['referrer']) || isset($mybb->input['referrername']))
) {
    if (isset($mybb->input['referrername'])) {
        $condition =
            "username = '" .
            $db->escape_string($mybb->get_input('referrername')) .
            "'";
    } else {
        $condition =
            "uid = '" . $mybb->get_input('referrer', MyBB::INPUT_INT) . "'";
    }

    $query = $db->simple_select('users', 'uid', $condition, ['limit' => 1]);
...
if ($mybb->usergroup['canview'] != 1) {
    ...
    if ($notallowed == true) {
        if (!$mybb->get_input('modal')) {
            ...
            eval(
                '$output = "' .
                    $templates->get('global_no_permission_modal', 1, 0) .
                    '";'
            );
            echo $output;
            exit();
...
if (
    $mybb->user['uid'] &&
    is_banned_email($mybb->user['email']) &&
    $mybb->settings['emailkeep'] != 1
) {
    if (
        !(
            THIS_SCRIPT == 'usercp.php' &&
            in_array($mybb->get_input('action'), ['email', 'do_email'])
        ) &&
        !(
            THIS_SCRIPT == 'member.php' &&
            $mybb->get_input('action') == 'activate'
        )
    ) {
        redirect('usercp.php?action=email');
    } elseif ($mybb->request_method != 'post') {
        $banned_email_error = inline_error([$lang->banned_email_warning]);
    }
}
