    ...
    while ($theme = $db->fetch_array($themesQuery)) {
        ...
        while ($stylesheet = $db->fetch_array($stylesheetsQuery)) {
            echo 'Cache refreshed for: ' .
                ...
                '' .
                $endline;
            echo 'Stylesheet cache for theme ID ' .
                $theme['tid'] .
                ' and with fileName ' .
                $stylesheet['cachefile'] .
                ' has been updated' .
...
function rebuild_stylesheet_cache_for_specific_theme($themeid, $cachefile)
{
    ...
    $stylesheetsQuery = $db->simple_select(
        'themestylesheets',
        'stylesheet',
        'tid = ' . $themeid . " AND cachefile = '" . $cachefile . "'"
    );
    $stylesheet = $db->fetch_array($stylesheetsQuery);
    if ($stylesheet) {
        echo 'Cache refreshed for: ' .
            cache_stylesheet($themeid, $cachefile, $stylesheet['stylesheet']) .
            '' .
            $endline;
        echo 'Stylesheet cache for theme ID ' .
            $themeid .
        ...
        echo 'No stylesheet found for theme ID ' .
            $themeid .
            ' and with fileName' .
            $endline;
    }
...
function rebuild_templates_in_category($category, $sid)
{
    ...
    $templateFiles = array_diff(scandir($category), ['.', '..']);
    $templateInserts = [];
        ...
        if (is_file($templateFilePath)) {
            $templateInserts[] = [
                'title' => $db->escape_string(
                    pathinfo($templateFilePath, PATHINFO_FILENAME)
                ),
...
function rebuild_templates_in_directory($directory, $sid)
{
    ...
    if (!$isForDev) {
        $templateCategories = array_diff(scandir($directory), ['.', '..']);
        foreach ($templateCategories as $templateCategory) {
            rebuild_templates_in_category(
                $directory . '/' . $templateCategory,
                $sid
            );
        }
    }
...
function rebuild_stylesheets_in_directory($directory)
{
    ...
    if ($tid == -100) {
        echo "No theme ID found in _properties.json, skipping directory: $directory" .
            $endline;
        return;
    }
    ...
    if (!$isForDev) {
        $stylsheetfiles = array_diff(scandir($directory), [
            '.',
            '..',
            '_stylesheets.json',
            '_properties.json',
        ...
        foreach ($stylsheetfiles as $stylesheetFile) {
            ...
            if (is_file($stylesheetFilePath)) {
                $stylesheetInserts[] = [
                    'name' => $stylesheetFile,
                    'tid' => $tid,
                    'attachedto' => gather_attached_to(
                        $stylesheetsData,
                        $stylesheetFile
                    ),
                    'stylesheet' => $db->escape_string(
                        file_get_contents($stylesheetFilePath)
                    ),
...
function rebuild_all_templates()
{
    ...
    $directories = array_diff(scandir($templatesDirectory), ['.', '..']);
    foreach ($directories as $directory) {
        ...
        rebuild_templates_in_directory(
            $fullPath,
            get_template_id_from_directory($directory)
        );
    }
}
...
function rebuild_all_stylesheets()
{
    ...
    $directories = array_diff(scandir($stylesheetsDirectory), ['.', '..']);
    foreach ($directories as $directory) {
...
if (isset($_GET['rebuild']) && $_GET['rebuild'] == 'stylesheets') {
    if (isset($_GET['themeid']) && isset($_GET['cachefile'])) {
        rebuild_stylesheet_cache_for_specific_theme(
            $_GET['themeid'],
            $_GET['cachefile']
        );
    } else {
        ...
        $db->write_query(
            'ALTER TABLE mybb_themestylesheets AUTO_INCREMENT = 1'
        );
...
if (isset($_GET['rebuild']) && $_GET['rebuild'] == 'templates') {
    global $db;
    $query = $db->simple_select('templates', 'tid', '1=1', ['limit' => 1]);
