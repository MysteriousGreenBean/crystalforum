...
abstract class AbstractPdoDbDriver implements DB_Base
{
     ...
     */
    abstract protected function getDsn($hostname, $db, $port, $encoding);
    ...
    public function connect($config)
    {
        ...
        foreach (['read', 'write'] as $type) {
            ...
            foreach ($connections[$type] as $singleConnection) {
                ...
                list($hostname, $port) = self::parseHostname(
                    $singleConnection['hostname']
                );
                ...
                if ($this->$link !== null) {
                    $this->connections[] =
                        '[' .
                        strtoupper($type) .
                        "] {$singleConnection['username']}@{$singleConnection['hostname']} (Connected in " .
                    ...
                    $this->connections[] =
                        ...
                        strtoupper($type) .
                        "] {$singleConnection['username']}@{$singleConnection['hostname']}</span>";
                }
            }
        }
        ...
        if ($openingSquareBracket === 0) {
            ...
            if ($closingSquareBracket !== false) {
                ...
                throw new InvalidArgumentException(
                    "Hostname is missing a closing square bracket for IPv6 address: {$hostname}"
                );
            }
        }
    ...
    public function error_string()
    {
        if (
            $this->lastPdoException !== null &&
            isset($this->lastPdoException->errorInfo[2])
        ) {
    ...
    public function query($string, $hideErrors = false, $writeQuery = false)
    {
        ...
        try {
            if (preg_match('/^\\s*SELECT\\b/i', $string) === 1) {
                ...
                $query = $this->current_link->prepare($string, [
    ...
    public function get_version()
    {
        ...
        $this->version = $this->read_link->getAttribute(
