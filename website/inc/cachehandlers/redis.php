...
class redisCacheHandler implements CacheHandlerInterface
{
    ...
    function connect()
    {
        ...
        if (!$mybb->config['redis']['host']) {
            $message =
                'Please configure the redis settings in inc/config.php before attempting to use this cache handler';
            $error_handler->trigger($message, MYBB_CACHEHANDLER_LOAD_ERROR);
            die();
        }
        if ($mybb->config['redis']['port']) {
            $ret = $this->redis->pconnect(
                $mybb->config['redis']['host'],
                $mybb->config['redis']['port']
            );
        } else {
