...
class MyBB
{
    ...
    function __construct()
    {
        ...
        if (
            isset($_SERVER['MYBB_DEV_MODE']) &&
            $_SERVER['MYBB_DEV_MODE'] == 1
        ) {
            $this->dev_mode = 1;
        }
        ...
        if (
            isset($this->input['action']) &&
            $this->input['action'] == 'mybb_logo'
        ) {
    ...
    function parse_cookies()
    {
        ...
        foreach ($_COOKIE as $key => $val) {
            if (
                $prefix_length &&
                substr($key, 0, $prefix_length) ==
                    $this->settings['cookieprefix']
            ) {
        ...
        foreach ($this->clean_variables as $type => $variables) {
            foreach ($variables as $var) {
                ...
                if (isset($this->input[$var])) {
                    switch ($type) {
                        case 'int':
                            $this->input[$var] = $this->get_input(
                                $var,
                                MyBB::INPUT_INT
                            );
                            break;
                        case 'a-z':
                            $this->input[$var] = preg_replace(
                                '#[^a-z\.\-_]#i',
                                '',
                                $this->get_input($var)
                            );
    ...
    public function get_asset_url($path = '', $use_cdn = true)
    {
        ...
        if (substr($path, 0, 4) != 'http') {
            ...
            if (
                $use_cdn &&
                $this->settings['usecdn'] &&
                !empty($this->settings['cdnurl'])
            ) {
    ...
    function trigger_generic_error($code)
    {
        ...
        switch ($code) {
            ...
            case 'sql_load_error':
                $message =
                    "MyBB was unable to load the SQL extension. Please contact the MyBB Group for support. <a href=\"https://mybb.com\">MyBB Website</a>";
                $error_code = MYBB_SQL_LOAD_ERROR;
                break;
            case 'apc_load_error':
                $message =
                    'APC needs to be configured with PHP to use the APC cache support.';
                $error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
                break;
            case 'apcu_load_error':
                $message =
                    'APCu needs to be configured with PHP to use the APCu cache support.';
                $error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
                break;
            ...
            case 'memcache_load_error':
                $message =
                    'Your server does not have memcache support enabled.';
                $error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
                break;
            case 'memcached_load_error':
                $message =
                    'Your server does not have memcached support enabled.';
                $error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
                break;
            case 'xcache_load_error':
                $message =
                    'Xcache needs to be configured with PHP to use the Xcache cache support.';
                $error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
                break;
            ...
            default:
                $message =
                    "MyBB has experienced an internal error. Please contact the MyBB Group for support. <a href=\"https://mybb.com\">MyBB Website</a>";
...
$groupzerolesser = ['canusesigxposts', 'emailfloodtime'];
$groupxgreater = [
    'reputationpower' => 0,
];
$grouppermbyswitch = [
