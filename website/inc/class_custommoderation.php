...
class CustomModeration extends Moderation
{
    ...
    function execute($tool_id, $tids = 0, $pids = 0)
    {
        ...
        $deleted_thread = 0;
        if ($tool['type'] == 'p') {
            $deleted_thread = $this->execute_post_moderation(
                $tids,
                $post_options,
                $pids
            );
     ...
     * @return boolean true
     */
    function execute_post_moderation($tid, $post_options = [], $pids = [])
    {
        ...
        $plugins->run_hooks(
            'class_custommoderation_execute_post_moderation_start',
            $args
        );
        ...
        if ($post_options['deleteposts'] == 1) {
            ...
            if ($post_options['softdeleteposts'] == 'softdelete') {
                // Soft delete posts
                $this->soft_delete_posts($pids);
            } elseif ($post_options['softdeleteposts'] == 'restore') {
                // Restore posts
                $this->restore_posts($pids);
            } elseif ($post_options['softdeleteposts'] == 'toggle') {
                // Toggle post visibility
                $this->toggle_post_softdelete($pids);
            }

            if (
                $post_options['splitposts'] > 0 ||
                $post_options['splitposts'] == -2
            ) {
                // Split posts
                $query = $db->simple_select(
                    'posts',
                ...
                if (empty($post_options['splitpostsnewsubject'])) {
                    ...
                    ] = "{$lang->split_thread_subject} {$thread['subject']}";
                }
                ...
                $new_tid = $this->split_posts(
                    $pids,
                    $tid,
                    $post_options['splitposts'],
                    $new_thread_subject
                );
                ...
                if ($post_options['splitpostsunapprove'] == 'unapprove') {
                    // Unapprove new thread
                    $this->unapprove_threads($new_tid, $thread['fid']);
                }
                if ($post_options['splitthreadprefix'] != '0') {
                    $this->apply_thread_prefix(
                        $new_tid,
                        $post_options['splitthreadprefix']
                    ); // Add thread prefix to new thread
                }
                if (!empty($post_options['splitpostsaddreply'])) {
                    // Add reply to new thread
                    ...
                    $plugins->run_hooks(
                        'class_custommoderation_splitpostsaddreply',
                        $args
                    );
        ...
        $plugins->run_hooks(
            'class_custommoderation_execute_post_moderation_end',
            $args
        );
    ...
    function execute_thread_moderation($thread_options = [], $tids = [])
    {
        ...
        $plugins->run_hooks(
            'class_custommoderation_execute_thread_moderation_start',
            $args
        );
        ...
        if ($thread_options['deletethread'] == 1) {
            ...
            if ($thread_options['mergethreads'] == 1 && count($tids) > 1) {
                // Merge Threads (ugly temp code until find better fix)
                $tid_list = implode(',', $tids);
                $options = ['order_by' => 'dateline', 'order_dir' => 'DESC'];
                $query = $db->simple_select(
                    'threads',
                    'tid, subject',
                    "tid IN ($tid_list)",
            ...
            if ($thread_options['threadprefix'] != '-1') {
                $this->apply_thread_prefix(
                    $tids,
                    $thread_options['threadprefix']
                ); // Update thread prefix
            }

            if (
                my_strtolower(trim($thread_options['newsubject'])) !=
                '{subject}'
            ) {
                // Update thread subjects
                $this->change_thread_subject(
                    $tids,
                    $thread_options['newsubject']
                );
            }
            if (!empty($thread_options['addreply'])) {
                ...
                $query = $db->query(
                    "
					SELECT u.uid, u.username, t.fid, t.subject, t.tid, t.firstpost, t.closed FROM " .
                        TABLE_PREFIX .
                        "threads t
					LEFT JOIN " .
                        TABLE_PREFIX .
                ...
                while ($thread = $db->fetch_array($query)) {
                    ...
                    $plugins->run_hooks(
                        'class_custommoderation_addreply',
                        $args
                    );
            ...
            if (
                $thread_options['movethread'] > 0 &&
                $thread_options['movethread'] != $thread['fid']
            ) {
                // Move thread
                if ($thread_options['movethreadredirect'] == 1) {
                    ...
                    $time =
                        TIME_NOW +
                        $thread_options['movethreadredirectexpire'] * 86400;
                    foreach ($tids as $tid) {
                        $this->move_thread(
                            $tid,
            ...
            if (
                $thread_options['copythread'] > 0 ||
                $thread_options['copythread'] == -2
            ) {
                // Copy thread
                ...
                foreach ($tids as $tid) {
                    $new_tid = $this->move_thread(
                        $tid,
                        $thread_options['copythread'],
                        'copy'
                    );
                }
            }
        ...
        if (
            isset($thread_options['pm_subject']) &&
            $thread_options['pm_subject'] != '' &&
            isset($thread_options['pm_message']) &&
            $thread_options['pm_message'] != ''
        ) {
            $tid_list = implode(',', $tids);
            ...
            $query = $db->query(
                "
				SELECT u.uid, u.username, t.subject FROM " .
                    TABLE_PREFIX .
                    "threads t
				LEFT JOIN " .
                    TABLE_PREFIX .
            ...
            while ($thread = $db->fetch_array($query)) {
                ...
                $pm_subject = str_ireplace(
                    $find,
                    $replace,
                    $thread_options['pm_subject']
                );
                $pm_message = str_ireplace(
                    $find,
                    $replace,
                    $thread_options['pm_message']
                );
        ...
        $plugins->run_hooks(
            'class_custommoderation_execute_thread_moderation_end',
            $args
        );
