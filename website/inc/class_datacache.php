...
class datacache
{
    ...
    function cache()
    {
        ...
        if ($this->handler instanceof CacheHandlerInterface) {
            ...
            while ($data = $db->fetch_array($query)) {
                ...
                $this->cache[$data['title']] = native_unserialize(
                    $data['cache']
                );
            }
        }
    ...
    function read($name, $hard = false)
    {
        ...
        elseif (
            $hard == false &&
            !($this->handler instanceof CacheHandlerInterface)
        ) {
            return false;
        }
    ...
    function delete($name, $greedy = false)
    {
        ...
        if ($greedy) {
            ...
            if ($this->handler instanceof CacheHandlerInterface) {
                ...
                foreach (
                    (array) @glob(MYBB_ROOT . "cache/{$name}*.php")
                    as $filename
                ) {
                    if ($filename) {
                        $filename = substr(
...
<tr>
	<td style=\"background-color: #ccc;\">{$debug_extra}<div><strong>#{$this->call_count} - " .
            ucfirst($cache_method) .
            " Call</strong></div></td>
</tr>
<tr style=\"background-color: #fefefe;\">
	<td><span style=\"font-family: Courier; font-size: 14px;\">({$mybb->config['cache_store']}) [{$hit_status}] " .
            htmlspecialchars_uni($cache_key) .
            "</span></td>
</tr>
<tr>
	<td bgcolor=\"#ffffff\">Call Time: " .
...
<br />\n";
    ...
    function update_forumpermissions()
    {
        ...
        $this->forum_permissions = $this->built_forum_permissions = [0];
        ...
        foreach ($forum_cache as $fid => $forum) {
            $this->forum_permissions_forum_cache[$forum['pid']][
                $forum['disporder']
            ][$forum['fid']] = $forum;
        }
        ...
        while ($forum_permission = $db->fetch_array($query)) {
            $this->forum_permissions[$forum_permission['fid']][
                $forum_permission['gid']
            ] = $forum_permission;
        }
            ...
            foreach ($this->forum_permissions_forum_cache[$pid] as $main) {
                foreach ($main as $forum) {
                    ...
                    foreach ($usergroups as $gid) {
                        if (
                            isset(
                                $this->forum_permissions[$forum['fid']][$gid]
                            ) &&
                            $this->forum_permissions[$forum['fid']][$gid]
                        ) {
                            $perms[$gid] =
                        ...
                        if (!empty($perms[$gid])) {
                            ...
                            $this->built_forum_permissions[$forum['fid']][
                                $gid
                            ] = $perms[$gid];
                        }
                    }
    ...
    function update_statistics()
    {
        ...
        $query = $db->simple_select(
            ...
            'referrals>0',
            ['order_by' => 'referrals', 'order_dir' => 'DESC', 'limit' => 1]
        );
    ...
    function update_moderators()
    {
        ...
        // Resort in to the structure we require
        foreach ($forum_cache as $fid => $forum) {
            $this->moderators_forum_cache[$forum['pid']][$forum['disporder']][
                $forum['fid']
            ] = $forum;
        }
        ...
        $query = $db->query(
			...
			LEFT JOIN " .
                TABLE_PREFIX .
                "users u ON (m.id=u.uid)
			WHERE m.isgroup = '0'
			ORDER BY u.username
		"
        );
        while ($moderator = $db->fetch_array($query)) {
            $this->moderators[$moderator['fid']]['users'][
                $moderator['id']
            ] = $moderator;
        }
        ...
        $query = $db->query(
			...
			LEFT JOIN " .
                TABLE_PREFIX .
                "usergroups u ON (m.id=u.gid)
			WHERE m.isgroup = '1'
			ORDER BY u.title
		"
        );
        while ($moderator = $db->fetch_array($query)) {
            $this->moderators[$moderator['fid']]['usergroups'][
                $moderator['id']
            ] = $moderator;
        }
                ...
                foreach ($main as $forum) {
                    ...
                    if (isset($this->moderators[$forum['fid']])) {
                        if (is_array($forum_mods) && count($forum_mods)) {
                            $forum_mods = array_merge(
                                $forum_mods,
                                $this->moderators[$forum['fid']]
                            );
                        } else {
    ...
    function update_birthdays()
    {
        ...
        $query = $db->simple_select(
            'users',
            'uid, username, usergroup, displaygroup, birthday, birthdayprivacy',
    ...
    function update_forumsdisplay()
    {
        ...
        $query = $db->simple_select('modtools', 'forums, tid', '', [
            'order_by' => 'tid',
        ]);
    ...
    function update_reportreasons($no_plugins = false)
    {
        ...
        $content_types = ['post', 'profile', 'reputation'];
        if (!$no_plugins) {
            global $plugins;
            $content_types = $plugins->run_hooks(
        ...
        $query = $db->simple_select('reportreasons', '*', '', [
            'order_by' => 'disporder',
        ]);
    ...
    function reload_mostonline()
    {
        ...
        $this->update(
            'mostonline',
            my_unserialize($db->fetch_field($query, 'cache'))
        );
    }
