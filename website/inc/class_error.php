...
class errorHandler
{
     ...
     * @var array
     */
    public $ignore_types = [E_DEPRECATED, E_NOTICE, E_USER_NOTICE, E_STRICT];
    ...
    function __construct()
    {
        ...
        set_error_handler([&$this, 'error_callback'], $error_types);
    }
    ...
    function error(
        ...
        if (
            $type == MYBB_SQL ||
            strpos(strtolower($this->error_types[$type]), 'warning') === false
        ) {
            $this->has_errors = true;
        }

        // For some reason in the installer this setting is set to "<"
        $accepted_error_types = ['both', 'error', 'warning', 'none'];
        if (
            isset($mybb->settings['errortypemedium']) &&
        ...
        if ($allow_output === true) {
            ...
            // PHP Error
            elseif (
                strpos(strtolower($this->error_types[$type]), 'warning') ===
                false
            ) {
                $this->output_error($type, $message, $file, $line);
            }
            // PHP Warning
            elseif (in_array($errortypemedium, ['warning', 'both'])) {
                global $templates;

                $warning =
                    "<strong>{$this->error_types[$type]}</strong> [$type] $message - Line: $line - File: $file PHP " .
                    PHP_VERSION .
                    ' (' .
                    PHP_OS .
                    ")<br />\n";
                if (
                    is_object($templates) &&
                    method_exists($templates, 'get') &&
                    !defined('IN_ADMINCP')
                ) {
                    $this->warnings .= $warning;
                    $this->warnings .= $this->generate_backtrace();
                } else {
                    echo "<div class=\"php_warning\">{$warning}" .
                        $this->generate_backtrace() .
                        '</div>';
                }
            }
        }
    ...
    function email_error($type, $message, $file, $line)
    {
        ...
        @my_mail(
            ...
            "MyBB error on {$mybb->settings['bbname']}",
            $message,
            $mybb->settings['adminemail']
        );
    ...
    function output_error($type, $message, $file, $line)
    {
        ...
        $show_details =
            ...
            in_array($errortypemedium, ['both', 'error']) ||
            defined('IN_INSTALL') ||
            defined('IN_UPGRADE');
        ...
        if ($type == MYBB_SQL) {
            ...
            if ($show_details) {
                ...
                if (!empty($file)) {
                    ...
                    if (
                        !@preg_match('#config\.php|settings\.php#', $file) &&
                        @file_exists($file)
                    ) {
                        $code_pre = @file($file);
                        ...
                        if (
                            !is_object($parser) ||
                            !method_exists($parser, 'mycode_parse_php')
                        ) {
                            if (
        ...
        $contact_site_owner = '';
        $is_in_contact =
            defined('THIS_SCRIPT') && THIS_SCRIPT === 'contact.php';
        if (
            ...
            ((!empty($mybb->settings['contact']) &&
                ...
                ($mybb->settings['contactlink'] == 'contact.php' &&
                    (!isset($mybb->user['uid']) ||
                        ($mybb->settings['contact_guests'] != 1 &&
                            $mybb->user['uid'] == 0) ||
                        $mybb->user['uid'] > 0))) ||
                $mybb->settings['contactlink'] != 'contact.php')
        ) {
