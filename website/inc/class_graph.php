...
class Graph
{
     ...
     */
    private function image_create_rectangle(
        $x1,
        $y1,
        $x2,
        $y2,
    ...
    public function render()
    {
        ...
        foreach ($this->points as $x => $y) {
            if (
                ($x_label_scale == 0 || ($x + 1) % $x_label_scale == 0) &&
                $x != 0
            ) {
                $x_label_points[] = $x;
                ...
                imagefilledellipse(
                    $this->im,
                    $this->inside_x + $x_delta * $x,
                    $y_initial - $next_y_scaled + 0.5,
                    8,
                    8,
                    $this->color(84, 92, 209)
                );
            }
            ...
            $this->imagelinethick(
                $this->inside_x + $x_delta * $x,
                $y_initial - $y_scaled,
                $this->inside_x + $x_delta * ($x + 1),
                $y_initial - $next_y_scaled,
                $this->color(84, 92, 209),
                3
            );
        }
        ...
        imagestring(
            ...
            2,
            5,
            $this->inside_y + $this->inside_height - 7,
            my_number_format($min),
            $this->color(0, 0, 0)
        );
        imagestring(
            $this->im,
