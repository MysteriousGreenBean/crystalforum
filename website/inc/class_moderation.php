...
class Moderation
{
    ...
    function close_threads($tids)
    {
        ...
        $db->update_query(
            'threads',
            $openthread,
            "tid IN ($tid_list) AND closed NOT LIKE 'moved|%'"
        );
    ...
    function delete_thread($tid)
    {
        ...
        while ($post = $db->fetch_array($query)) {
            ...
            if (
                ($post['visible'] == 0 && $thread['visible'] != -1) ||
                $thread['visible'] == 0
            ) {
        ...
        if (!empty($pids)) {
            ...
            $db->delete_query(
                'reportedcontent',
                "id IN ($pids) AND (type = 'post' OR type = '')"
            );
        }
    ...
    function approve_threads($tids)
    {
        ...
        $tid_list = $forum_counters = $user_counters = $posts_to_approve = [];
        ...
        while ($thread = $db->fetch_array($query)) {
            ...
            $forum_counters[$forum['fid']]['num_posts'] +=
                $thread['replies'] + 1; // Remove implied visible from count
            $forum_counters[$forum['fid']]['num_deleted_posts'] +=
                $thread['deletedposts'];
            $forum_counters[$forum['fid']]['num_unapproved_posts'] +=
                $thread['deletedposts'] + $thread['replies'] + 1;

            if ($forum['usepostcounts'] != 0) {
                // On approving thread restore user post counts
                $query2 = $db->simple_select(
                    'posts',
                    'COUNT(pid) as posts, uid',
                ...
                while ($counter = $db->fetch_array($query2)) {
                    if (!isset($user_counters[$counter['uid']])) {
                        $user_counters[$counter['uid']] = [
                            'num_posts' => 0,
                            'num_threads' => 0,
                        ];
                    }
                    $user_counters[$counter['uid']]['num_posts'] +=
                        $counter['posts'];
                }
            }
        ...
        if (!empty($tid_list)) {
            ...
            if (!empty($posts_to_approve)) {
                $db->update_query(
                    'posts',
                    $approve,
                    'pid IN (' . implode(',', $posts_to_approve) . ')'
                );
            }
    ...
    function unapprove_threads($tids)
    {
        ...
        $forum_counters = $user_counters = $posts_to_unapprove = [];
        ...
        while ($thread = $db->fetch_array($query)) {
            ...
            if ($thread['visible'] == 1 || $thread['visible'] == -1) {
                ...
                $forum_counters[$forum['fid']]['num_unapprovedposts'] +=
                    $thread['replies'] + $thread['deletedposts'] + 1;

                if ($thread['visible'] == 1) {
                    ++$forum_counters[$forum['fid']]['num_threads'];
                    $forum_counters[$forum['fid']]['num_posts'] +=
                        $thread['replies'] + 1; // Add implied invisible to count
                    $forum_counters[$forum['fid']]['num_deletedposts'] +=
                        $thread['deletedposts'];
                } else {
                    ...
                    $forum_counters[$forum['fid']]['num_deletedposts'] +=
                        ...
                        $thread['deletedposts'] +
                        1; // Add implied invisible to count
                    $forum_counters[$forum['fid']]['num_unapprovedposts'] +=
                        $thread['unapprovedposts'];
                }
        ...
        if (!empty($posts_to_unapprove)) {
            $db->update_query(
                'posts',
                $approve,
                'pid IN (' . implode(',', $posts_to_unapprove) . ')'
            );
        }
    ...
    function delete_post($pid)
    {
        ...
        if (
            $forum['usepostcounts'] != 0 &&
            $post['visible'] != -1 &&
            $post['visible'] != 0 &&
            $post['threadvisible'] != 0 &&
            $post['threadvisible'] != -1
        ) {
        ...
        // Update unapproved post count
        if (
            ($post['visible'] == 0 && $post['threadvisible'] != -1) ||
            $post['threadvisible'] == 0
        ) {
            $update_array = [
                'unapprovedposts' => '-1',
    ...
    function merge_posts($pids = [], $tid = 0, $sep = 'new_line')
    {
        ...
        $query = $db->query(
			...
			LEFT JOIN " .
                TABLE_PREFIX .
                "attachments a ON (a.pid=p.pid AND a.visible=1)
        ...
        $message = '';
        $threads = $forum_counters = $thread_counters = $user_counters = [];
        while ($post = $db->fetch_array($query)) {
            $threads[$post['tid']] = $post['tid'];
            if (!isset($thread_counters[$post['tid']])) {
                $thread_counters[$post['tid']] = [
                    'replies' => 0,
                    'unapprovedposts' => 0,
                    'deletedposts' => 0,
                    'attachmentcount' => 0,
            ...
            if ($first == 1) {
                ...
                if ($post['visible'] == 1) {
                    ...
                    if (
                        $post['threadfirstpost'] == $post['pid'] &&
                        $forum['usethreadcounts'] != 0 &&
                        $post['threadvisible'] == 1
                    ) {
                        --$user_counters[$post['uid']]['num_threads'];
                    }
                    $thread_counters[$post['tid']]['attachmentcount'] -=
                ...
                if ($visible == 1) {
                    $thread_counters[$mastertid]['attachmentcount'] +=
                        $post['attachmentcount'];
                }
            }
        }
        ...
        $db->delete_query(
            'posts',
            "pid IN({$pidin}) AND pid != '{$masterpid}'"
        );
        ...
        $query = $db->simple_select(
            'threads',
            'tid, uid, fid, visible',
            "firstpost IN({$pidin}) AND firstpost != '{$masterpid}'"
        );
        while ($thread = $db->fetch_array($query)) {
            ...
            if (
                $new_firstpost['uid'] != $thread['uid'] &&
                $forum['usethreadcounts'] != 0 &&
                $thread['visible'] == 1
            ) {
                if (!isset($user_counters[$new_firstpost['uid']])) {
                    $user_counters[$new_firstpost['uid']] = [
                        'num_posts' => 0,
                        'num_threads' => 0,
                    ];
                }
    ...
    function move_thread(
        ...
        switch ($method) {
            case 'redirect': // move (and leave redirect) thread
                $arguments = ['tid' => $tid, 'new_fid' => $new_fid];
                $plugins->run_hooks(
                ...
                $query = $db->simple_select(
                    'threads',
                    'tid',
                    "closed='moved|$tid' AND fid='$new_fid'"
                );
                while ($redirect_tid = $db->fetch_field($query, 'tid')) {
                    $this->delete_thread($redirect_tid);
                }
                ...
                if ($thread['prefix'] != 0) {
                    switch ($db->type) {
                        ...
                        case 'sqlite':
                            $query = $db->simple_select(
                                'threadprefixes',
                                'COUNT(*) as num_prefixes',
                                "(','||forums||',' LIKE '%,$new_fid,%' OR forums='-1') AND pid='" .
                                    $thread['prefix'] .
                                    "'"
                            );
                            break;
                        default:
                            $query = $db->simple_select(
                                'threadprefixes',
                                'COUNT(*) as num_prefixes',
                                "(CONCAT(',',forums,',') LIKE '%,$new_fid,%' OR forums='-1') AND pid='" .
                                    $thread['prefix'] .
                                    "'"
                            );
                    }
                ...
                while ($post = $db->fetch_array($query)) {
                    ...
                    while ($attachment = $db->fetch_array($query2)) {
                        $attachment_array = [
                            ...
                            'filetype' => $db->escape_string(
                                $attachment['filetype']
                            ),
                            'filesize' => $attachment['filesize'],
                            'attachname' => $db->escape_string(
                                $attachment['attachname']
                            ),
                            'downloads' => $attachment['downloads'],
                            'visible' => $attachment['visible'],
                            'thumbnail' => $db->escape_string(
                                $attachment['thumbnail']
                            ),
                        ];
                        $new_aid = $db->insert_query(
                            'attachments',
                            $attachment_array
                        );
        ...
        while ($posters = $db->fetch_array($query)) {
            ...
            if (
                ...
                $method != 'copy' &&
                $newforum['usepostcounts'] == 0 &&
                $thread['visible'] == 1
            ) {
                $pcount -= $posters['posts'];
            }
            if (
                ($forum['usepostcounts'] == 0 || $method == 'copy') &&
                $newforum['usepostcounts'] == 1 &&
                $thread['visible'] == 1
            ) {
                $pcount += $posters['posts'];
            }
            ...
            if ($pcount > 0) {
                update_user_counters($posters['uid'], [
                    'postnum' => "+$pcount",
                ]);
            } elseif ($pcount < 0) {
                update_user_counters($posters['uid'], ['postnum' => $pcount]);
            }
        }
        ...
        if (
            ...
            $method != 'copy' &&
            $newforum['usethreadcounts'] == 0 &&
            $thread['visible'] == 1
        ) {
            update_user_counters($thread['uid'], ['threadnum' => '-1']);
        } elseif (
            ($forum['usethreadcounts'] == 0 || $method == 'copy') &&
            $newforum['usethreadcounts'] == 1 &&
            $thread['visible'] == 1
        ) {
            update_user_counters($thread['uid'], ['threadnum' => '+1']);
        }
    ...
    function merge_threads($mergetid, $tid, $subject)
    {
        ...
        $user_posts = [];
        if (
            $thread['visible'] != $mergethread['visible'] ||
            $forum_cache[$thread['fid']]['usepostcounts'] !=
                $forum_cache[$mergethread['fid']]['usepostcounts']
        ) {
            $query = $db->query(
                "
            ...
            while ($post = $db->fetch_array($query)) {
                ...
                if (
                    $mergethread['visible'] == 1 &&
                    $forum_cache[$mergethread['fid']]['usepostcounts'] == 1
                ) {
                    $user_posts[$post['uid']]['postnum'] -= $post['postnum'];
                } elseif (
                    $thread['visible'] == 1 &&
                    $forum_cache[$thread['fid']]['usepostcounts'] == 1
                ) {
                    $user_posts[$post['uid']]['postnum'] += $post['postnum'];
                }
            }
        ...
        $new_threadrating =
            $thread['totalratings'] + $mergethread['totalratings'];
        ...
        $query = $db->simple_select(
            'threadsubscriptions',
            'tid, uid',
            "tid='{$mergetid}' OR tid='{$tid}'"
        );
        while ($subscription = $db->fetch_array($query)) {
            if (!isset($subscriptions[$subscription['tid']])) {
                $subscriptions[$subscription['tid']] = [];
            }
        ...
        if (!empty($subscriptions[$mergetid])) {
            ...
            foreach ($subscriptions[$mergetid] as $user) {
                if (
                    !isset($subscriptions[$tid]) ||
                    !in_array($user, $subscriptions[$tid])
                ) {
            ...
            if (!empty($update_users)) {
                ...
                $db->update_query(
                    'threadsubscriptions',
                    $update_array,
                    "tid = '{$mergetid}' AND uid IN ({$update_users})"
                );
            }
        }
        ...
        $db->update_query(
            'posts',
            ['replyto' => $new_firstpost['pid']],
            "tid='{$tid}' AND replyto = 0 AND pid != '{$new_firstpost['pid']}'"
        );

        // Update thread count if thread has a new firstpost and is visible
        if (
            $thread['uid'] != $new_firstpost['uid'] &&
            $thread['visible'] == 1 &&
            $forum_cache[$thread['fid']]['usethreadcounts'] == 1
        ) {
        ...
        elseif ($mergethread['visible'] != $thread['visible']) {
            ...
            // If old thread is unapproved, implied counter comes in to effect
            if ($mergethread['visible'] == 0) {
                $updated_stats['unapprovedposts'] -=
                    $mergethread['replies'] + $mergethread['deletedposts'];
                $updated_stats['posts'] += $mergethread['replies'];
                $updated_stats['deletedposts'] += $mergethread['deletedposts'];
            } elseif ($mergethread['visible'] == -1) {
                $updated_stats['deletedposts'] -=
                    $mergethread['replies'] + $mergethread['unapprovedposts'];
                $updated_stats['posts'] += $mergethread['replies'];
                $updated_stats['unapprovedposts'] +=
                    $mergethread['unapprovedposts'];
            }
    ...
    function split_posts(
        ...
        $query = $db->simple_select(
            'posts',
            'pid,uid,visible,icon,username,dateline',
            'pid IN (' . $pids_list . ')',
            ['order_by' => 'dateline, pid', 'limit' => 1]
        );
        ...
        while ($post = $db->fetch_array($original_posts_query)) {
            ...
            if ($post['visible'] == 1) {
                ...
                if (
                    $post['threadvisible'] == 1 &&
                    $forum_cache[$post['fid']]['usepostcounts'] == 1 &&
                    ($forum_cache[$moveto]['usepostcounts'] == 0 ||
                        $newthread['visible'] != 1)
                ) {
            ...
            $thread_counters[$post['tid']]['attachmentcount'] -=
                $post['postattachmentcount'];
            $thread_counters[$newtid]['attachmentcount'] +=
                $post['postattachmentcount'];
            ...
            if ($post['firstpost'] == $post['pid']) {
                ...
                if (
                    $post['threadvisible'] == 1 &&
                    $forum_cache[$post['fid']]['usethreadcounts'] == 1 &&
                    $post['uid'] != $new_firstpost['uid']
                ) {
            ...
            if (
                $post['pid'] == $post_info['pid'] &&
                $post['dateline'] < $newthread['dateline']
            ) {
                // Update post counters if visibility changes
                if ($post['visible'] != $newthread['visible']) {
                    $db->update_query(
                        'posts',
                        ['visible' => $newthread['visible']],
                        "pid='{$post['pid']}'"
                    );
                ...
                if (
                    $newthread['visible'] == 1 &&
                    $forum_cache[$newthread['fid']]['usethreadcounts'] == 1 &&
                    $post['uid'] != $newthread['uid']
                ) {
                    // Add thread to new thread opener
                    ++$user_counters[$post['uid']]['threadnum'];
                    if (!isset($user_counters[$newthread['uid']])) {
            ...
            if ($post['visible'] == 1) {
                ...
                if (
                    $newthread['visible'] == 1 &&
                    ($forum_cache[$post['fid']]['usepostcounts'] == 0 ||
                        $post['threadvisible'] != 1) &&
                    $forum_cache[$moveto]['usepostcounts'] == 1
                ) {
            ...
            if ($newthread['visible'] == 1 && $post['visible'] == 1) {
                ...
                $newthread['visible'] == 0 ||
                ($post['visible'] == 0 && $newthread['visible'] == 1)
            ) {
        ...
        $db->update_query(
            'posts',
            ['replyto' => $post_info['pid']],
            "tid='{$newtid}' AND replyto = 0 AND pid != '{$post_info['pid']}'"
        );
    ...
    function move_threads($tids, $moveto)
    {
        ...
        $query = $db->simple_select(
            'threads',
            'fid, visible, replies, unapprovedposts, deletedposts, tid, uid',
            "tid IN ($tid_list)"
        );
        while ($thread = $db->fetch_array($query)) {
            $forum = get_forum($thread['fid']);
            ...
            if ($thread['visible'] == 1) {
                ...
                while ($posters = $db->fetch_array($query1)) {
                    ...
                    if (
                        ...
                        $user_counters[$posters['uid']]['num_posts'] +=
                            $posters['posts'];
                    } elseif (
                        $newforum['usepostcounts'] == 0 &&
                        $forum['usepostcounts'] != 0
                ...
                $forum_counters[$thread['fid']]['deletedposts'] +=
                    ...
                    $thread['unapprovedposts'] +
                    $thread['deletedposts'] +
                    1; // Implied deleted posts counter for deleted threads
                ...
                $forum_counters[$thread['fid']]['unapprovedposts'] +=
                    ...
                    $thread['unapprovedposts'] +
                    $thread['deletedposts'] +
                    1; // Implied unapproved posts counter for unapproved threads
            ...
            $redirects_query = $db->simple_select(
                'threads',
                'tid',
                "closed='moved|{$thread['tid']}' AND fid='$moveto'"
            );
            ...
            switch ($db->type) {
                ...
                case 'sqlite':
                    $query = $db->simple_select(
                        'threadprefixes',
                        'COUNT(*) as num_prefixes',
                        "(','||forums||',' LIKE '%,$moveto,%' OR forums='-1') AND pid='" .
                            $thread['prefix'] .
                            "'"
                    );
                    break;
                default:
                    $query = $db->simple_select(
                        'threadprefixes',
                        'COUNT(*) as num_prefixes',
                        "(CONCAT(',',forums,',') LIKE '%,$moveto,%' OR forums='-1') AND pid='" .
                            $thread['prefix'] .
                            "'"
                    );
            }
        ...
        $arguments = ['tids' => $tids, 'moveto' => $moveto];
    ...
    function approve_posts($pids)
    {
        ...
        $thread_counters = $forum_counters = $user_counters = [];
        ...
        if (!empty($user_counters)) {
            foreach ($user_counters as $uid => $counter) {
                update_user_counters($uid, ['postnum' => "+{$counter}"]);
            }
        }
    ...
    function change_thread_subject($tids, $format)
    {
        ...
        $query = $db->query(
            "
			SELECT u.uid, u.username, t.tid, t.subject FROM " .
                TABLE_PREFIX .
                "threads t
			LEFT JOIN " .
                TABLE_PREFIX .
        ...
        while ($thread = $db->fetch_array($query)) {
            ...
            $plugins->run_hooks(
                'class_moderation_change_thread_subject_newsubject',
                $args
            );
            ...
            $db->update_query(
                'posts',
                $update_subject,
                "tid='{$thread['tid']}' AND replyto='0'"
            );
        }

        $arguments = ['tids' => $tids, 'format' => $format];
        $plugins->run_hooks(
    ...
    function remove_thread_subscriptions($tids, $all = true, $fid = 0)
    {
        ...
        if (!$all) {
            ...
            $query = $db->simple_select(
                'forumpermissions',
                'gid',
                "fid IN ({$forum_parentlist}) AND (canview=0 OR canviewthreads=0)"
            );
            $groups = [];
            $additional_groups = '';
            ...
            if (count($groups) > 0) {
                ...
                while ($subscription = $db->fetch_array($query)) {
                    $db->delete_query(
                        'threadsubscriptions',
                        "uid='{$subscription['uid']}' AND tid='{$subscription['tid']}'"
                    );
                }
            }
        }
    ...
    function apply_thread_prefix($tids, $prefix = 0)
    {
        ...
        $update_thread = ['prefix' => (int) $prefix];
        $db->update_query('threads', $update_thread, "tid IN ({$tids_csv})");

        $arguments = ['tids' => $tids, 'prefix' => $prefix];
    ...
    function restore_threads($tids)
    {
        ...
        $tid_list = $forum_counters = $user_counters = $posts_to_restore = [];
        ...
        while ($thread = $db->fetch_array($query)) {
            ...
            $forum_counters[$forum['fid']]['num_posts'] +=
                $thread['replies'] + 1; // Remove implied visible from count
            $forum_counters[$forum['fid']]['num_deleted_posts'] +=
                $thread['replies'] + $thread['unapprovedposts'] + 1;
            $forum_counters[$forum['fid']]['num_unapproved_posts'] +=
                $thread['unapprovedposts'];
        ...
        if (!empty($tid_list)) {
            ...
            if (!empty($posts_to_restore)) {
                $db->update_query(
                    'posts',
                    $update,
                    'pid IN (' . implode(',', $posts_to_restore) . ')'
                );
            }
    ...
    function soft_delete_threads($tids)
    {
        ...
        $forum_counters = $user_counters = $posts_to_delete = [];
        ...
        while ($thread = $db->fetch_array($query)) {
            ...
            if ($thread['visible'] == 1 || $thread['visible'] == 0) {
                ...
                if ($thread['visible'] == 1) {
                    ...
                    $forum_counters[$forum['fid']]['num_posts'] +=
                        $thread['replies'] + 1; // Add implied invisible to count
                    $forum_counters[$forum['fid']]['unapproved_posts'] +=
                        $thread['unapprovedposts'];
                } else {
        ...
        if (!empty($posts_to_delete)) {
            $db->update_query(
                'posts',
                $update,
                'pid IN (' . implode(',', $posts_to_delete) . ')'
            );
        }
