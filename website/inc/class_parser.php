...
class postParser
{
     ...
     */
    function parse_message($message, $options = [])
    {
        global $plugins, $mybb;
        ...
        $code_matches = [];
        if (
            !empty($this->options['allow_mycode']) &&
            $mybb->settings['allowcodemycode'] == 1
        ) {
            ...
            foreach ($code_matches as $point => $part) {
                if (isset($part[3])) {
                    $part[1] = '[' . $part[1] . ']';
                    $code_matches[$point][2] = substr_replace(
                        $part[0],
                        '',
                        strrpos($part[0], $part[3]),
                        strlen($part[3])
                    );
                    $code_matches[$point][2] = substr_replace(
                        $code_matches[$point][2],
                        '',
                        strpos($code_matches[$point][2], $part[1]),
                        strlen($part[1])
                    );
                }
            }
        ...
        if (
            !empty($this->options['me_username']) &&
            $mybb->settings['allowmemycode'] == 1
        ) {
            global $lang;

            $message = preg_replace(
                '#(>|^|\r|\n)/me ([^\r\n<]*)#i',
                "\\1<span style=\"color: red;\" class=\"mycode_me\">* {$this->options['me_username']} \\2</span>",
                $message
            );
            $message = preg_replace(
                '#(>|^|\r|\n)/slap ([^\r\n<]*)#i',
                "\\1<span style=\"color: red;\" class=\"mycode_slap\">* {$this->options['me_username']} {$lang->slaps} \\2 {$lang->with_trout}</span>",
                $message
            );
        }
        ...
        if (!empty($this->options['highlight'])) {
            $message = $this->highlight_message(
                $message,
    ...
    function cache_mycode()
    {
        ...
        if ($mybb->settings['allowbasicmycode'] == 1) {
            ...
            $standard_mycode['b']['replacement'] =
                "<span style=\"font-weight: bold;\" class=\"mycode_b\">$1</span>";

            $standard_mycode['u']['regex'] = '#\[u\](.*?)\[/u\]#si';
            $standard_mycode['u']['replacement'] =
                "<span style=\"text-decoration: underline;\" class=\"mycode_u\">$1</span>";

            $standard_mycode['i']['regex'] = '#\[i\](.*?)\[/i\]#si';
            $standard_mycode['i']['replacement'] =
                "<span style=\"font-style: italic;\" class=\"mycode_i\">$1</span>";

            $standard_mycode['s']['regex'] = '#\[s\](.*?)\[/s\]#si';
            $standard_mycode['s']['replacement'] =
                "<span style=\"text-decoration: line-through;\" class=\"mycode_s\">$1</span>";
        ...
        if ($mybb->settings['allowemailmycode'] == 1) {
            $callback_mycode['email_simple']['regex'] =
                '#\[email\]((?:[a-zA-Z0-9-_\+\.]+?)@[a-zA-Z0-9-]+\.[a-zA-Z0-9\.-]+(?:\?.*?)?)\[/email\]#i';
        ...
        if ($mybb->settings['allowcolormycode'] == 1) {
            ...
            $nestable_mycode['color']['replacement'] =
                "<span style=\"color: $1;\" class=\"mycode_color\">$2</span>";
        ...
        if ($mybb->settings['allowsizemycode'] == 1) {
            ...
            $nestable_mycode['size']['replacement'] =
                "<span style=\"font-size: $1;\" class=\"mycode_size\">$2</span>";
        ...
        if ($mybb->settings['allowalignmycode'] == 1) {
            ...
            $nestable_mycode['align']['replacement'] =
                "<div style=\"text-align: $1;\" class=\"mycode_align\">$2</div>";

            ++$nestable_count;
        }

        if ($mybb->settings['allowfontmycode'] == 1) {
            $nestable_callback_mycode['font']['regex'] =
                "#\[font=\\s*(\"?)([a-z0-9 ,\-_'\"]+)\\1\\s*\](.*?)\[/font\]#si";
            $nestable_callback_mycode['font']['replacement'] = [
                $this,
        ...
        foreach ($nestable_callback_mycode as $code) {
            $this->mycode_cache['nestable_callback'][] = [
                'find' => $code['regex'],
                'replacement' => $code['replacement'],
            ];
        }
     ...
     * @return string The parsed message.
     */
    function parse_mycode($message, $options = [])
    {
        global $lang, $mybb;
        ...
        if (!empty($this->options['allow_imgcode'])) {
            $message = preg_replace_callback(
                "#\[img\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",
                [$this, 'mycode_parse_img_callback1'],
                $message
            );
            $message = preg_replace_callback(
                "#\[img=([1-9][0-9]*)x([1-9][0-9]*)\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",
                [$this, 'mycode_parse_img_callback2'],
                $message
            );
            $message = preg_replace_callback(
                "#\[img align=(left|right)\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",
                [$this, 'mycode_parse_img_callback3'],
                $message
            );
            $message = preg_replace_callback(
                "#\[img=([1-9][0-9]*)x([1-9][0-9]*) align=(left|right)\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",
                [$this, 'mycode_parse_img_callback4'],
                $message
            );
        } else {
            $message = preg_replace_callback(
                "#\[img\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",
                [$this, 'mycode_parse_img_disabled_callback1'],
                $message
            );
            $message = preg_replace_callback(
                "#\[img=([1-9][0-9]*)x([1-9][0-9]*)\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",
                [$this, 'mycode_parse_img_disabled_callback2'],
                $message
            );
            $message = preg_replace_callback(
                "#\[img align=(left|right)\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",
                [$this, 'mycode_parse_img_disabled_callback3'],
                $message
            );
            $message = preg_replace_callback(
                "#\[img=([1-9][0-9]*)x([1-9][0-9]*) align=(left|right)\](\r\n?|\n?)(https?://([^<>\"']+?))\[/img\]#is",
                [$this, 'mycode_parse_img_disabled_callback4'],
                $message
            );
        }
        ...
        if (!empty($this->options['allow_videocode'])) {
            $message = preg_replace_callback(
                '#\[video=(.*?)\](.*?)\[/video\]#i',
                [$this, 'mycode_parse_video_callback'],
                $message
            );
        } else {
            $message = preg_replace_callback(
                '#\[video=(.*?)\](.*?)\[/video\]#i',
                [$this, 'mycode_parse_video_disabled_callback'],
                $message
            );
        }
        ...
        if ($mybb->settings['allowlistmycode'] == 1) {
            ...
            for ($i = $this->list_count; $i > 0; $i--) {
                ...
                $message = preg_replace_callback(
                    ...
                    [$this, 'mycode_parse_list_callback'],
                    $message,
                    1
                );
            }
        ...
        if (
            (!isset($this->options['allow_auto_url']) ||
                $this->options['allow_auto_url'] == 1) &&
     ...
     * @return string The parsed message.
     */
    function parse_badwords($message, $options = [])
    {
        if (empty($this->options)) {
            $this->options = $options;
        }
        ...
        if (is_array($this->badwords_cache)) {
            ...
            foreach ($this->badwords_cache as $bid => $badword) {
                ...
                if (!$badword['regex']) {
                    $badword['badword'] = $this->generate_regex(
                        $badword['badword']
                    );
                }
    ...
    function mycode_handle_size($size, $text)
    {
        ...
        eval(
            "\$mycode_size = \"" .
                $templates->get('mycode_size_int', 1, 0) .
                "\";"
        );
        return $mycode_size;
    }
    ...
    function mycode_parse_post_quotes($message, $username, $text_only = false)
    {
        ...
        if (isset($match[1]) && (int) $match[1]) {
            ...
            if (defined('IN_ARCHIVE')) {
                ...
                eval(
                    "\$linkback = \" " .
                        $templates->get('postbit_gotopost', 1, 0) .
                        "\";"
                );
            }
        ...
        if ($text_only) {
            ...
            eval(
                "\$mycode_quote = \"" .
                    $templates->get('mycode_quote_post', 1, 0) .
                    "\";"
            );
            return $mycode_quote;
        }
    ...
    {
        return $this->mycode_parse_post_quotes(
            $matches[4],
            $matches[2] . $matches[3]
        );
    }
    ...
    function mycode_parse_code($code, $text_only = false)
    {
        ...
        eval(
            "\$mycode_code = \"" . $templates->get('mycode_code', 1, 0) . "\";"
        );
        return $mycode_code;
    }
    ...
    function mycode_parse_url($url, $name = '')
    {
        ...
        if (
            $name == $url &&
            (!isset($this->options['shorten_urls']) ||
                !empty($this->options['shorten_urls']))
        ) {
     ...
     * @return string
     */
    function mycode_parse_img($url, $dimensions = [], $align = '')
    {
        global $lang, $templates;
        $url = trim($url);
        $url = str_replace("\n", '', $url);
    ...
    function mycode_parse_email($email, $name = '')
    {
        ...
        eval(
            "\$mycode_email = \"" .
                $templates->get('mycode_email', 1, 0) .
                "\";"
        );
        return $mycode_email;
    }
    ...
    function mycode_parse_video($video, $url)
    {
        ...
        $fragments = empty($parsed_url['fragment'])
            ? []
            : explode('&', $parsed_url['fragment']);
        ...
        $queries = empty($parsed_url['query'])
            ? []
        ...
        eval(
            "\$video_code = \"" .
                $templates->get("video_{$video}_embed", 1, 0) .
                "\";"
        );
        return $video_code;
    }
     ...
     * @return string The parsed message.
     */
    function text_parse_message($message, $options = [])
    {
        global $plugins;
        ...
        $message = preg_replace_callback(
            "#\[php\](.*?)\[/php\](\r\n?|\n?)#is",
            [$this, 'mycode_parse_php_callback'],
            $message
        );
        $message = preg_replace_callback(
            "#\[code\](.*?)\[/code\](\r\n?|\n?)#is",
            [$this, 'mycode_parse_code_callback'],
            $message
        );
        ...
        if (!empty($this->options['me_username'])) {
            ...
            $message = preg_replace(
                '#(>|^|\r|\n)/me ([^\r\n<]*)#i',
                "\\1* {$this->options['me_username']} \\2",
                $message
            );
            $message = preg_replace(
                '#(>|^|\r|\n)/slap ([^\r\n<]*)#i',
                "\\1* {$this->options['me_username']} {$lang->slaps} \\2 {$lang->with_trout}",
                $message
            );
        }
    ...
    {
        if (
            $this->output_validation_policy === self::VALIDATION_DISABLE ||
            !empty($this->options['allow_html'])
        ) {
            return true;
        } else {
            ...
            if (
                $this->output_validation_policy === self::VALIDATION_REPORT_ONLY
            ) {
                return true;
            } else {
    ...
    function validate_output($source, $output)
    {
        ...
        if (
            $errors &&
            array_diff(array_column($errors, 'code'), $ignored_error_codes)
        ) {
            $data = [
