...
class pluginSystem
{
    ...
    function load()
    {
        ...
        if (
            !empty($plugin_list['active']) &&
            is_array($plugin_list['active'])
        ) {
            foreach ($plugin_list['active'] as $plugin) {
                if (
                    $plugin != '' &&
                    file_exists(MYBB_ROOT . 'inc/plugins/' . $plugin . '.php')
                ) {
    ...
    function add_hook($hook, $function, $priority = 10, $file = '')
    {
        if (is_array($function)) {
            ...
            if (
                ...
                is_array($this->hooks[$hook][$priority][$method_representation])
            ) {
                return true;
            }
            ...
            // Check to see if we already have this hook running at this priority
            if (
                !empty($this->hooks[$hook][$priority][$function]) &&
                is_array($this->hooks[$hook][$priority][$function])
            ) {
                return true;
            }
    ...
    function remove_hook($hook, $function, $file = '', $priority = 10)
    {
        if (is_array($function)) {
            ...
            if (
                !isset($this->hooks[$hook][$priority][$method_representation])
            ) {
                return true;
            }
    ...
    function is_compatible($plugin)
    {
        ...
        if (
            empty($plugin_info['compatibility']) ||
