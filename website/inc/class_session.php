...
class session
{
     ...
     * @var array
     */
    public $ignore_parameters = ['my_post_key', 'logoutkey'];
    ...
    function init()
    {
            ...
            if (!$this->sid) {
                ...
                if (is_array($spiders)) {
                    foreach ($spiders as $spider) {
                        if (
                            my_strpos(
                                my_strtolower($this->useragent),
                                my_strtolower($spider['useragent'])
                            ) !== false
                        ) {
        ...
        if (
            $this->sid &&
            (!isset($mybb->cookies['sid']) ||
                $mybb->cookies['sid'] != $this->sid) &&
            $this->is_spider != true
        ) {
    ...
    function load_user($uid, $loginkey = '')
    {
        ...
        if (
            ($mybb->user['totalpms'] == -1 || $mybb->user['unreadpms'] == -1) &&
        ...
        if (
            $mybb->user['lastip'] != $this->packedip &&
        ...
        if ($time - $mybb->user['lastactive'] > 900) {
            $db->shutdown_query(
                'UPDATE ' .
                    TABLE_PREFIX .
                    "users SET lastvisit='{$mybb->user['lastactive']}', lastactive='$time'{$lastip_add} WHERE uid='{$mybb->user['uid']}'"
            );
            ...
            $db->shutdown_query(
                'UPDATE ' .
                    TABLE_PREFIX .
                    "users SET lastactive='$time', timeonline=timeonline+$timespent{$lastip_add} WHERE uid='{$mybb->user['uid']}'"
            );
        }
        ...
        if ($mybb->user['dateformat'] != 0 && $mybb->user['dateformat'] != '') {
            ...
            if (!empty($date_formats[$mybb->user['dateformat']])) {
                $mybb->settings['dateformat'] =
                    $date_formats[$mybb->user['dateformat']];
            }
        }
        ...
        if ($mybb->user['timeformat'] != 0 && $mybb->user['timeformat'] != '') {
            ...
            if (!empty($time_formats[$mybb->user['timeformat']])) {
                $mybb->settings['timeformat'] =
                    $time_formats[$mybb->user['timeformat']];
            }
        }
        ...
        if (
            !empty($usergroups[$mybb->user['usergroup']]) &&
            $usergroups[$mybb->user['usergroup']]['isbannedgroup'] == 1
        ) {
            $ban = $db->fetch_array(
                $db->simple_select(
                    'banned',
        ...
        if (
            ...
            $db->shutdown_query(
                'UPDATE ' .
                    TABLE_PREFIX .
                    "users SET usergroup='" .
                    (int) $mybb->user['banoldgroup'] .
                    "', additionalgroups='" .
                    $db->escape_string($mybb->user['banoldadditionalgroups']) .
                    "', displaygroup='" .
                    (int) $mybb->user['banolddisplaygroup'] .
                    "' WHERE uid='" .
                    $mybb->user['uid'] .
                    "'"
            );
    ...
    function load_guest()
    {
        ...
        if (isset($mybb->cookies['mybb']['lastvisit'])) {
            if (!isset($mybb->cookies['mybb']['lastactive'])) {
                ...
                $mybb->cookies['mybb']['lastactive'] =
                    $mybb->user['lastactive'];
            } else {
                $mybb->user['lastactive'] =
                    (int) $mybb->cookies['mybb']['lastactive'];
            }
    ...
    function load_spider($spider_id)
    {
        ...
        if (
            $spider['language'] &&
            $lang->language_exists($spider['language'])
        ) {
    ...
    function update_session($sid, $uid = 0)
    {
        ...
        $onlinedata['location'] = $db->escape_string(
            substr(
                get_current_location(false, $this->ignore_parameters),
                0,
                150
            )
        );
        $onlinedata['useragent'] = $db->escape_string(
            my_substr($this->useragent, 0, 200)
        );
    ...
    function get_special_locations()
    {
        ...
        $array = ['1' => '', '2' => ''];
        if (
            preg_match('#forumdisplay.php#', $_SERVER['PHP_SELF']) &&
            $mybb->get_input('fid', MyBB::INPUT_INT) > 0 &&
            $mybb->get_input('fid', MyBB::INPUT_INT) < 4294967296
        ) {
            $array[1] = $mybb->get_input('fid', MyBB::INPUT_INT);
        } elseif (preg_match('#showthread.php#', $_SERVER['PHP_SELF'])) {
            if (
                $mybb->get_input('tid', MyBB::INPUT_INT) > 0 &&
                $mybb->get_input('tid', MyBB::INPUT_INT) < 4294967296
            ) {
