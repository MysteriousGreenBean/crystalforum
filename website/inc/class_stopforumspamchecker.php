...
class StopForumSpamChecker
{
    ...
    public function __construct(
        &$plugins,
        $min_weighting_before_spam = 50.0,
     ...
     */
    public function is_user_a_spammer(
        $username = '',
        $email = '',
        $ip_address = ''
    ) {
        ...
        $check_url = sprintf(
            self::STOP_FORUM_SPAM_API_URL_FORMAT,
            $username_encoded,
            $email_encoded,
            $ip_address
        );
        ...
        if ($this->plugins) {
            ...
            $this->plugins->run_hooks(
                'stopforumspam_check_spammer_pre_return',
                $params
            );
        }

        if ($this->log_blocks && $is_spammer) {
            log_spam_block($username, $email, $ip_address, [
    ...
    public function getErrorText($sfsSettingsEnabled)
    {
        ...
        foreach ($sfsSettingsEnabled as $setting) {
            if (
                $setting == 'stopforumspam_check_usernames' &&
                $mybb->settings[$setting]
            ) {
            ...
            if (
                $setting == 'stopforumspam_check_emails' &&
                $mybb->settings[$setting]
            ) {
            ...
            if (
                $setting == 'stopforumspam_check_ips' &&
                $mybb->settings[$setting]
            ) {
        ...
        if (sizeof($settingsenabled) > 1) {
            ...
            $stopforumspamerror =
                implode($lang->comma, $settingsenabled) .
                " {$lang->sfs_error_or} " .
                $lastsetting;
        } else {
