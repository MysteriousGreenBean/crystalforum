...
class EventDataHandler extends DataHandler
{
    ...
    function verify_date()
    {
        ...
        if ($event['type'] == 'ranged') {
            ...
            if ($event['start_date']['time'] || $event['end_date']['time']) {
                ...
                $start_time = ['hour' => 0, 'min' => 0];
                $end_time = ['hour' => 23, 'min' => 59];
                $event['usingtime'] = 0;
            }
        }
    ...
    {
        preg_match(
            '#^(0?[1-9]|1[012])\s?([:\.]?)\s?([0-5][0-9])?(\s?[ap]m)|([01][0-9]|2[0-3])\s?([:\.])\s?([0-5][0-9])$#i',
            $time,
            $matches
        );
        if (count($matches) == 0) {
        ...
        // 12 hour time
            ...
            if (my_strtolower($matches[4]) == 'pm' && $hour != 12) {
                $hour += 12;
            } elseif (my_strtolower($matches[4]) == 'am' && $hour == 12) {
                $hour = 0;
            }
        }
    ...
    function verify_repeats()
    {
        ...
        switch ($event['repeats']['repeats']) {
            ...
            case 3:
                ...
                if (
                    is_array($event['repeats']['days']) &&
                    count($event['repeats']['days']) == 0
                ) {
            ...
            case 4:
                if ($event['repeats']['day']) {
                    ...
                    if ($event['repeats']['occurance'] != 'last') {
                        $event['repeats']['occurance'] =
                            (int) $event['repeats']['occurance'];
                    }
                    $event['repeats']['weekday'] =
                        (int) $event['repeats']['weekday'];
                }
                $event['repeats']['months'] = (int) $event['repeats']['months'];
                if (
                    $event['repeats']['months'] <= 0 ||
                    $event['repeats']['months'] > 12
                ) {
            ...
            case 5:
                ...
                if (
                    $event['repeats']['month'] <= 0 ||
                    $event['repeats']['month'] > 12
                ) {
                    $this->set_error('invalid_repeat_month_interval');
                    return false;
                }
                ...
                if (
                    $event['repeats']['years'] <= 0 ||
                    $event['repeats']['years'] > 4
                ) {
        ...
        $next_occurance = fetch_next_occurance(
            $event,
    ...
    function validate_event()
    {
        ...
        if (
            $this->method == 'insert' ||
            array_key_exists('start_date', $event) ||
            array_key_exists('end_date', $event)
        ) {
            $this->verify_date();
        }

        if (
            ($this->method == 'insert' && $event['endtime']) ||
            array_key_exists('repeats', $event)
        ) {
            $this->verify_repeats();
        }
    ...
    function insert_event()
    {
        ...
        if (!$this->get_validated()) {
            die(
                'The event needs to be validated before inserting it into the DB.'
            );
        }
        ...
        if (isset($event['timezone'])) {
            $this->event_insert_data['timezone'] = $db->escape_string(
                (float) $event['timezone']
            );
        }

        if (isset($event['ignoretimezone'])) {
            $this->event_insert_data['ignoretimezone'] =
                (int) $event['ignoretimezone'];
        }
        ...
        if (isset($event['repeats'])) {
            $this->event_insert_data['repeats'] = $db->escape_string(
                my_serialize($event['repeats'])
            );
        } else {
    ...
    function update_event()
    {
        ...
        if (isset($event['name'])) {
            $this->event_update_data['name'] = $db->escape_string(
                $event['name']
            );
        }

        if (isset($event['description'])) {
            $this->event_update_data['description'] = $db->escape_string(
                $event['description']
            );
        }
        ...
        if (isset($event['repeats'])) {
            ...
            $this->event_update_data['repeats'] = $db->escape_string(
                $event['repeats']
            );
        }

        if (isset($event['timezone'])) {
            $this->event_update_data['timezone'] = $db->escape_string(
                (float) $event['timezone']
            );
        }

        if (isset($event['ignoretimezone'])) {
            $this->event_update_data['ignoretimezone'] =
                (int) $event['ignoretimezone'];
        }
        ...
        if (isset($event['visible'])) {
            $this->event_update_data['visible'] = $db->escape_string(
                $event['visible']
            );
        }
