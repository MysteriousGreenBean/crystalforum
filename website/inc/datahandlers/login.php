...
class LoginDataHandler extends DataHandler
{
    ...
    function verify_captcha()
    {
        ...
        if ($user['imagestring'] || $mybb->settings['captchaimage'] != 1) {
            ...
            $mybb->input['quick_login'] == 1 &&
            $mybb->input['quick_password'] &&
    ...
    function invalid_combination($show_login_attempts = false)
    {
        ...
        if ($show_login_attempts) {
            if (
                ...
                $this->login_data['uid'] != 0
            ) {
                $logins =
                    login_attempt_check($this->login_data['uid'], false) + 1;
                $login_text = $lang->sprintf(
                    $lang->failed_login_again,
                    $mybb->settings['failedlogincount'] - $logins
                );
            }
        }
