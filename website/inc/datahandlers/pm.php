...
class PMDataHandler extends DataHandler
{
    ...
    function verify_message()
    {
        ...
        elseif (strlen($message) > 65535) {
            $this->set_error('message_too_long', ['65535', strlen($message)]);
            return false;
        }
    ...
    function verify_sender()
    {
        ...
        if (
            isset($pm['options']['savecopy']) &&
            $pm['options']['savecopy'] != 0 &&
            empty($pm['saveasdraft'])
        ) {
            if (
                $sender_permissions['pmquota'] != 0 &&
                $sender['totalpms'] >= $sender_permissions['pmquota'] &&
    ...
    function verify_recipient()
    {
        ...
        if (array_key_exists('to', $pm)) {
            foreach (['to', 'bcc'] as $recipient_type) {
                if (!isset($pm[$recipient_type])) {
                    $pm[$recipient_type] = [];
                }
                if (!is_array($pm[$recipient_type])) {
                    $pm[$recipient_type] = [$pm[$recipient_type]];
                }
        ...
        // We have recipient IDs
            ...
            foreach (['toid', 'bccid'] as $recipient_type) {
                if (!isset($pm[$recipient_type])) {
                    $pm[$recipient_type] = [];
                }
                if (!is_array($pm[$recipient_type])) {
                ...
                $recipientUids =
                    "'" . implode("','", $pm[$recipient_type]) . "'";

                $query = $db->simple_select(
                    'users',
                    '*',
                    'uid IN(' . $recipientUids . ')'
                );
        ...
        if (count($invalid_recipients) > 0) {
            $invalid_recipients = implode(
                $lang->comma,
                array_map('htmlspecialchars_uni', $invalid_recipients)
            );
            $this->set_error('invalid_recipients', [$invalid_recipients]);
            return false;
        }
        ...
        if ($pm['fromid'] > 0) {
            ...
            if (
                $sender_permissions['maxpmrecipients'] > 0 &&
                count($recipients) > $sender_permissions['maxpmrecipients'] &&
                $this->admin_override != true
            ) {
                $this->set_error('too_many_recipients', [
                    $sender_permissions['maxpmrecipients'],
                ]);
            }
        }
        ...
        foreach ($recipients as $user) {
            ...
            if (
                $this->admin_override != true &&
                empty($sender_permissions['canoverridepm'])
            ) {
                if (
                    !empty($user['ignorelist']) &&
                    strpos(
                        ',' . $user['ignorelist'] . ',',
                        ',' . $pm['fromid'] . ','
                    ) !== false
                ...
                if (
                    ...
                    $user['receivefrombuddy'] == 1 &&
                    !empty($user['buddylist']) &&
                    strpos(
                        ',' . $user['buddylist'] . ',',
                        ',' . $pm['fromid'] . ','
                    ) === false
                ) {
                    $this->set_error('recipient_has_buddy_only', [
                        htmlspecialchars_uni($user['username']),
                    ]);
                }
                ...
                if (
                    ($user['receivepms'] == 0 ||
                        $recipient_permissions['canusepms'] == 0) &&
                    empty($pm['saveasdraft'])
                ) {
            ...
            if (
                $recipient_permissions['pmquota'] != 0 &&
                $user['totalpms'] >= $recipient_permissions['pmquota'] &&
                empty($sender_permissions['cancp']) &&
                empty($pm['saveasdraft']) &&
                !$this->admin_override
            ) {
                if (
                    trim($user['language']) != '' &&
                    $lang->language_exists($user['language'])
                ) {
    ...
    function verify_pm_flooding()
    {
        ...
        if (
            $mybb->settings['pmfloodsecs'] > 0 &&
            $pm['fromid'] > 0 &&
            $this->admin_override == false &&
            !is_moderator(0, '', $pm['fromid'])
        ) {
            // Fetch the senders profile data.
            $sender = get_user($pm['fromid']);
            ...
            if (
                TIME_NOW - $sender['lastpm'] <=
                $mybb->settings['pmfloodsecs']
            ) {
                ...
                if ($time_to_wait == 1) {
                    ...
                    $this->set_error('pm_flooding', [$time_to_wait]);
                }
                return false;
            }
        }
    ...
    function insert_pm()
    {
        ...
        if (!$this->get_validated()) {
            die(
                'The PM needs to be validated before inserting it into the DB.'
            );
        }
        if (count($this->get_errors()) > 0) {
        ...
        if (!empty($pm['saveasdraft'])) {
            ...
            $this->pmid = $db->insert_query(
                'privatemessages',
                $this->pm_insert_data
            );

            $plugins->run_hooks(
                'datahandler_pm_insert_updatedraft_commit',
                $this
            );
        ...
        foreach ($pm['recipients'] as $recipient) {
            ...
            $query = $db->simple_select(
                'privatemessages',
                'dateline',
                "uid='" . $recipient['uid'] . "' AND folder='1'",
                ['order_by' => 'dateline', 'order_dir' => 'desc', 'limit' => 1]
            );
            $lastpm = $db->fetch_array($query);
            if (
                $recipient['pmnotify'] == 1 &&
                (empty($lastpm['dateline']) ||
                ...
                $pm['message'] = $parser->text_parse_message(
                    $pm['message'],
                    $parser_options
                );
        ...
        if ($pm['pmid']) {
            if ($pm['do'] == 'reply' || $pm['do'] == 'replyall') {
                ...
                $db->update_query(
                    'privatemessages',
                    $sql_array,
                    "pmid={$pm['pmid']} AND uid={$pm['sender']['uid']}"
                );
        ...
        if ($pm['options']['savecopy'] != 0) {
            if (
                isset($recipient_list['to']) &&
                is_array($recipient_list['to']) &&
                count($recipient_list['to']) == 1
            ) {
