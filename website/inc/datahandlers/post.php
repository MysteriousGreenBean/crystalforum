...
class PostDataHandler extends DataHandler
{
    ...
    function verify_author()
    {
        ...
        elseif ($post['uid'] == 0 && $post['username'] != '') {
            // Set up user handler
    ...
    function verify_message()
    {
        ...
        if (my_strlen($post['message']) == 0) {
            ...
            if (stripos($db->type, 'my') !== false) {
                ...
                $type =
                    $fields[
                        array_search('message', array_column($fields, 'Field'))
                    ]['Type'];
            ...
            if ($limit > 0 || $dblimit > 0) {
                ...
                if (
                    strlen($post['message']) > $limit &&
                    (!$is_moderator || $limit == $dblimit)
                ) {
            ...
            if (!$mybb->settings['mycodemessagelength']) {
                ...
                if (
                    my_strlen($message) < $mybb->settings['minmessagelength'] &&
                    $mybb->settings['minmessagelength'] > 0 &&
                    !is_moderator($post['fid'], '', $post['uid'])
                ) {
                    $this->set_error('message_too_short', [
                        $mybb->settings['minmessagelength'],
                    ]);
                    return false;
                }
            } elseif (
                my_strlen($post['message']) <
                    $mybb->settings['minmessagelength'] &&
                $mybb->settings['minmessagelength'] > 0 &&
                !is_moderator($post['fid'], '', $post['uid'])
            ) {
    ...
    function verify_post_flooding()
    {
        ...
        if (
            $mybb->settings['postfloodcheck'] == 1 &&
            $post['uid'] != 0 &&
            ...
            if (
                TIME_NOW - $user['lastpost'] <=
                    $mybb->settings['postfloodsecs'] &&
                !is_moderator($post['fid'], '', $user['uid'])
            ) {
    ...
    function verify_post_merge($simple_mode = false)
    {
        ...
        if (
            !$thread ||
            ((int) $mybb->settings['postmergemins'] != 0 &&
                trim($mybb->settings['postmergemins']) != '' &&
                TIME_NOW - $thread['lastpost'] >
                    (int) $mybb->settings['postmergemins'] * 60)
        ) {
            return true;
        }
    ...
    function verify_image_count()
    {
        ...
        if (
            (!isset($post['savedraft']) || $post['savedraft'] != 1) &&
            ...
            $image_check = $parser->parse_message(
                $post['message'],
                $parser_options
            );
            ...
            if ($image_count > $mybb->settings['maxpostimages']) {
                ...
                $this->set_error('too_many_images', [
                    1 => $image_count,
                    2 => $mybb->settings['maxpostimages'],
                ]);
                return false;
            }
        }
    ...
    function verify_video_count()
    {
        ...
        if (
            ...
            if ($video_count > $mybb->settings['maxpostvideos']) {
                // Throw back a message if over the count with the number of images as well as the maximum number of images per post.
                $this->set_error('too_many_videos', [
                    1 => $video_count,
                    2 => $mybb->settings['maxpostvideos'],
                ]);
                return false;
            }
        }
    ...
    function insert_post()
    {
        ...
        if (!$this->get_validated()) {
            die(
                'The post needs to be validated before inserting it into the DB.'
            );
        }
        if (count($this->get_errors()) > 0) {
            ...
            if ($double_post !== true && $double_post['visible'] == $visible) {
                ...
                if ($this->validate_post()) {
                    ...
                    if ($post['posthash']) {
                        ...
                        $post['posthash'] = $db->escape_string(
                            $post['posthash']
                        );
                        ...
                        $attachmentcount = $db->fetch_field(
                            $query,
                            'attachmentcount'
                        );
                        ...
                        if ($attachmentcount > 0) {
                            // Update forum count
                            update_thread_counters($post['tid'], [
        ...
        if ($visible == 1 && $thread['visible'] == 1) {
            ...
            $excerpt =
                my_substr($excerpt, 0, $mybb->settings['subscribeexcerpt']) .
                $lang->emailbit_viewthread;
            ...
            while ($subscribedmember = $db->fetch_array($query)) {
                ...
                $args['users'][$subscribedmember['uid']] =
                    (int) $subscribedmember['uid'];

                $done_users[$subscribedmember['uid']] = 1;

                $forumpermissions = forum_permissions(
                    $thread['fid'],
                    $subscribedmember['uid']
                );
                if (
                ...
                if (
                    $thread['uid'] != $subscribedmember['uid'] &&
                    $forumpermissions['canonlyviewownthreads'] == 1 &&
                    !is_moderator($thread['fid'], '', $subscribedmember['uid'])
                ) {
                ...
                if (
                    $subscribedmember['language'] != '' &&
                    $lang->language_exists($subscribedmember['language'])
                ) {
                ...
                if ($uselang == $mybb->settings['bblanguage']) {
                    ...
                    if (
                        ($subscribedmember['notification'] == 1 &&
                            !isset(
                                $langcache[$uselang][
                                    'emailsubject_subscription'
                                ]
                            )) ||
                        !isset($langcache[$uselang]['guest'])
                    ) {
                        ...
                        if ($subscribedmember['notification'] == 1) {
                            ...
                            $langcache[$uselang]['emailsubject_subscription'] =
                                $userlang->emailsubject_subscription;
                            $langcache[$uselang]['email_subscription'] =
                                $userlang->email_subscription;
                        }
                ...
                if ($subscribedmember['notification'] == 1) {
                    ...
                    $new_email = [
                        'mailto' => $db->escape_string(
                            $subscribedmember['email']
                        ),
                        'mailfrom' => '',
        ...
        elseif ($visible == 0) {
            ...
            update_thread_counters($post['tid'], ['unapprovedposts' => '+1']);
            update_forum_counters($post['fid'], ['unapprovedposts' => '+1']);
            ...
            // Update the unapproved posts count for the current forum
            $thread_update = ['replies' => '+1'];
            update_forum_counters($post['fid'], ['deletedposts' => '+1']);
        }
    ...
    function validate_thread()
    {
        ...
        if (
            $this->method == 'insert' ||
            array_key_exists('dateline', $thread)
        ) {
            $this->verify_dateline();
        }
    ...
    function insert_thread()
    {
        ...
        if (!$this->get_validated()) {
            die(
                'The thread needs to be validated before inserting it into the DB.'
            );
        }
        if (count($this->get_errors()) > 0) {
        ...
        if (!$thread['savedraft']) {
            if (
                $thread['options']['subscriptionmethod'] != '' &&
                $thread['uid'] > 0
            ) {
                ...
                while ($subscribedmember = $db->fetch_array($query)) {
                    ...
                    if ($uselang == $mybb->settings['bblanguage']) {
                        ...
                        if (
                            !isset(
                                $langcache[$uselang][
                                    'emailsubject_forumsubscription'
                                ]
                            )
                        ) {
                            $userlang = new MyLanguage();
                            ...
                            $langcache[$uselang]['email_forumsubscription'] =
                                $userlang->email_forumsubscription;
                            $langcache[$uselang]['guest'] = $userlang->guest;
                            unset($userlang);
                        }
                        $emailsubject =
        ...
        if ($visible == 1) {
            ...
            update_forum_counters($thread['fid'], [
                'threads' => '+1',
                'posts' => '+1',
            ]);
            update_forum_lastpost($thread['fid']);
        ...
        $attachmentcount = $db->fetch_field($query, 'attachmentcount');
        if ($attachmentcount > 0) {
            update_thread_counters($this->tid, [
                'attachmentcount' => "+{$attachmentcount}",
            ]);
        }
    ...
    function update_post()
    {
        ...
        if ($this->first_post) {
            ...
            if (isset($post['subject'])) {
                $this->thread_update_data['subject'] = $db->escape_string(
                    $post['subject']
                );
            }
            ...
            if (isset($post['subject'])) {
                $query = $db->simple_select(
                    'threads',
                    'tid, closed',
                    "closed='moved|" . $this->tid . "'"
                );
                if ($db->num_rows($query) > 0) {
                    $update_data['subject'] = $db->escape_string(
                        $post['subject']
                    );
        ...
        if (isset($post['subject'])) {
            $this->post_update_data['subject'] = $db->escape_string(
                $post['subject']
            );
        }

        if (isset($post['message'])) {
            $this->post_update_data['message'] = $db->escape_string(
                $post['message']
            );
        }

        if (isset($post['editreason']) && trim($post['editreason']) != '') {
            $this->post_update_data['editreason'] = $db->escape_string(
                trim($post['editreason'])
            );
        ...
        if (isset($post['options'])) {
            if (isset($post['options']['disablesmilies'])) {
                $this->post_update_data['smilieoff'] = $db->escape_string(
                    $post['options']['disablesmilies']
                );
            }
            if (isset($post['options']['signature'])) {
                $this->post_update_data['includesig'] = $db->escape_string(
                    $post['options']['signature']
                );
            }
        }
        ...
        if (
            ...
            ($mybb->settings['showeditedbyadmin'] == 1 &&
                is_moderator($post['fid'], 'caneditposts', $post['edit_uid']))
        ) {
        ...
        if (
            $post &&
            !empty($post['options']['subscriptionmethod']) &&
            $uid > 0
        ) {
            switch ($post['options']['subscriptionmethod']) {
                case 'pm':
            ...
            $db->delete_query(
                'threadsubscriptions',
                "uid='" . (int) $uid . "' AND tid='" . (int) $post['tid'] . "'"
            );
        }
