...
class UserDataHandler extends DataHandler
{
    ...
    function verify_username()
    {
        ...
        if (
            ($mybb->settings['maxnamelength'] != 0 &&
                my_strlen($username) > $mybb->settings['maxnamelength']) ||
            ($mybb->settings['minnamelength'] != 0 &&
                my_strlen($username) < $mybb->settings['minnamelength'])
        ) {
    ...
    function verify_usertitle()
    {
        ...
        if (
            ...
            my_strlen($usertitle) > $mybb->settings['customtitlemaxlength']
        ) {
            $this->set_error(
                'invalid_usertitle_length',
                $mybb->settings['customtitlemaxlength']
            );
            return false;
        }
    ...
    function verify_password()
    {
        ...
        if (
            my_strlen($user['password']) <
                $mybb->settings['minpasswordlength'] ||
            my_strlen($user['password']) > $mybb->settings['maxpasswordlength']
        ) {
            $this->set_error('invalid_password_length', [
                $mybb->settings['minpasswordlength'],
                $mybb->settings['maxpasswordlength'],
            ]);
            return false;
        }
        ...
        if (!empty($user['email']) && !empty($user['username'])) {
            if (
                ...
                strpos($user['password'], $user['email']) !== false ||
                strpos($user['password'], $user['username']) !== false ||
                strpos($user['email'], $user['password']) !== false ||
                strpos($user['username'], $user['password']) !== false
            ) {
    ...
    function verify_email()
    {
        ...
        if (
            $mybb->settings['allowmultipleemails'] == 0 &&
            !defined('IN_ADMINCP')
        ) {
            $uid = 0;
            if (isset($user['uid'])) {
    ...
    function verify_birthday()
    {
        ...
        if (
            $mybb->settings['coppa'] == 'enabled' &&
            ($birthday['year'] == 0 || !$birthday['year'])
        ) {
            $this->set_error('invalid_birthday_coppa');
            return false;
        } elseif (
            $mybb->settings['coppa'] == 'deny' &&
            $birthday['year'] > date('Y') - 13 &&
            !is_moderator()
        ) {
    ...
    function verify_profile_fields()
    {
        ...
        if (is_array($pfcache)) {
            ...
            foreach ($pfcache as $profilefield) {
                if (
                    isset($this->data['profile_fields_editable']) ||
                ...
                if ($type != 'multiselect' && $type != 'checkbox') {
                    if (
                        trim($profile_fields[$field]) == '' &&
                        $profilefield['required'] == 1 &&
                        !defined('IN_ADMINCP') &&
                        THIS_SCRIPT != 'modcp.php'
                    ) {
                        $this->set_error('missing_required_profile_field', [
                            $profilefield['name'],
                        ]);
                    }
                } elseif (
                ...
                if (
                    ($type == 'multiselect' || $type == 'checkbox') &&
                    is_array($profile_fields[$field])
                ) {
                    $expoptions = explode("\n", $thing[1]);
                    $expoptions = array_map('trim', $expoptions);
                    foreach ($profile_fields[$field] as $value) {
                        if (
                            !in_array(htmlspecialchars_uni($value), $expoptions)
                        ) {
                            $this->set_error('bad_profile_field_values', [
                                $profilefield['name'],
                            ]);
                        }
                        if ($options) {
                    ...
                    if (
                        !in_array(
                            htmlspecialchars_uni($profile_fields[$field]),
                            $expoptions
                        ) &&
                        trim($profile_fields[$field]) != ''
                    ) {
                    ...
                    if (
                        ...
                        my_strlen($profile_fields[$field]) >
                            $profilefield['maxlength']
                    ) {
                        $this->set_error('max_limit_reached', [
                            $profilefield['name'],
                            $profilefield['maxlength'],
                        ]);
                    }
                    ...
                    if (
                        !empty($profilefield['regex']) &&
                        !empty($profile_fields[$field]) &&
                        !preg_match(
                            '#' . $profilefield['regex'] . '#i',
                            $profile_fields[$field]
                        )
                    ) {
    ...
    function verify_referrer()
    {
        ...
        if ($mybb->settings['usereferrals'] == 1 && !empty($user['referrer'])) {
            ...
            if (!$referrer) {
                $this->set_error('invalid_referrer', [$user['referrer']]);
                return false;
            }
    ...
    function verify_options()
    {
        ...
        if (array_key_exists('subscriptionmethod', $options)) {
            ...
            $options['subscriptionmethod'] =
                (int) $options['subscriptionmethod'];
            if (
                $options['subscriptionmethod'] < 0 ||
                $options['subscriptionmethod'] > 3
            ) {
                $options['subscriptionmethod'] = 0;
            }
        }
        ...
        if (
            $this->method == 'insert' ||
            array_key_exists('daysprune', $options)
        ) {
    ...
    function verify_away()
    {
        ...
        if (
            ...
            if ($reasonlength > 200) {
                $this->set_error('away_too_long', [$reasonlength - 200]);
                return false;
            }

            list($returnday, $returnmonth, $returnyear) = explode(
                '-',
            ...
            $user['away']['returndate'] =
                substr($returnday, 0, 2) .
                '-' .
                substr($returnmonth, 0, 2) .
                '-' .
                substr($returnyear, 0, 4);
        }
        return true;
    }
    ...
    function verify_style()
    {
        ...
        if (!empty($user['style'])) {
            ...
            if (
                empty($theme) ||
                (!is_member($theme['allowedgroups'], $user) &&
    ...
    function verify_timezone()
    {
        ...
        if (
            !isset($user['timezone']) ||
            !array_key_exists($user['timezone'], $timezones)
        ) {
    ...
    function validate_user()
    {
        ...
        if (
            $this->method == 'insert' ||
            (isset($user['birthday']) && is_array($user['birthday']))
        ) {
        ...
        if (
            $this->method == 'insert' ||
            array_key_exists('profile_fields', $user)
        ) {
        ...
        if (
            $this->method == 'insert' ||
            array_key_exists('lastactive', $user)
        ) {
        ...
        if (
            $this->method == 'insert' &&
            array_key_exists('regcheck1', $user) &&
            array_key_exists('regcheck2', $user)
        ) {
    ...
    function insert_user()
    {
        ...
        $array = ['subscriptionmethod', 'dstcorrection'];
        foreach ($array as $value) {
        ...
        $this->user_insert_data = [
            ...
            'usertitle' => $db->escape_string(
                htmlspecialchars_uni($user['usertitle'])
            ),
            ...
            'subscriptionmethod' =>
                (int) $user['options']['subscriptionmethod'],
    ...
    function update_user()
    {
        ...
        if (isset($user['username'])) {
            $this->user_update_data['username'] = $db->escape_string(
                $user['username']
            );
        }
        ...
        if (isset($user['email'])) {
            $this->user_update_data['email'] = $db->escape_string(
                $user['email']
            );
        }
        ...
        if (isset($user['avatar'])) {
            $this->user_update_data['avatar'] = $db->escape_string(
                $user['avatar']
            );
            $this->user_update_data['avatartype'] = $db->escape_string(
                $user['avatartype']
            );
        }
        if (isset($user['usergroup'])) {
        ...
        if (isset($user['additionalgroups'])) {
            $this->user_update_data['additionalgroups'] = $db->escape_string(
                $user['additionalgroups']
            );
        }
        if (isset($user['displaygroup'])) {
            $this->user_update_data['displaygroup'] =
                (int) $user['displaygroup'];
        }
        if (isset($user['usertitle'])) {
            $this->user_update_data['usertitle'] = $db->escape_string(
                $user['usertitle']
            );
        }
        ...
        if (isset($user['signature'])) {
            $this->user_update_data['signature'] = $db->escape_string(
                $user['signature']
            );
        }
        if (isset($user['website'])) {
            $this->user_update_data['website'] = $db->escape_string(
                $user['website']
            );
        }
        if (isset($user['icq'])) {
        ...
        if (isset($user['skype'])) {
            $this->user_update_data['skype'] = $db->escape_string(
                $user['skype']
            );
        }
        if (isset($user['google'])) {
            $this->user_update_data['google'] = $db->escape_string(
                $user['google']
            );
        }
        if (isset($user['bday'])) {
        ...
        if (isset($user['birthdayprivacy'])) {
            $this->user_update_data['birthdayprivacy'] = $db->escape_string(
        ...
        if (isset($user['timezone'])) {
            $this->user_update_data['timezone'] = $db->escape_string(
                $user['timezone']
            );
        }
        if (isset($user['dateformat'])) {
            $this->user_update_data['dateformat'] = $db->escape_string(
                $user['dateformat']
            );
        }
        if (isset($user['timeformat'])) {
            $this->user_update_data['timeformat'] = $db->escape_string(
                $user['timeformat']
            );
        }
        if (isset($user['regip'])) {
            $this->user_update_data['regip'] = $db->escape_binary(
                $user['regip']
            );
        }
        if (isset($user['lastip'])) {
            $this->user_update_data['lastip'] = $db->escape_binary(
                $user['lastip']
            );
        }
        if (isset($user['language'])) {
            $this->user_update_data['language'] = $db->escape_string(
                $user['language']
            );
        }
        if (isset($user['away'])) {
            $this->user_update_data['away'] = (int) $user['away']['away'];
            $this->user_update_data['awaydate'] = $db->escape_string(
                $user['away']['date']
            );
            $this->user_update_data['returndate'] = $db->escape_string(
                $user['away']['returndate']
            );
            $this->user_update_data['awayreason'] = $db->escape_string(
                $user['away']['awayreason']
            );
        }
        if (isset($user['notepad'])) {
            $this->user_update_data['notepad'] = $db->escape_string(
                $user['notepad']
            );
        }
        if (isset($user['usernotes'])) {
            $this->user_update_data['usernotes'] = $db->escape_string(
                $user['usernotes']
            );
        }
    ...
    function delete_user($delete_uids, $prunecontent = 0)
    {
        ...
        $db->update_query(
            'privatemessages',
            ['fromid' => 0],
            "fromid IN({$this->delete_uids})"
        );
        $db->update_query(
            'users',
            ['referrer' => 0],
            "referrer IN({$this->delete_uids})"
        );
        ...
        $db->update_query(
            'forums',
            ['lastposteruid' => 0],
            "lastposteruid IN({$this->delete_uids})"
        );
        $db->update_query(
            'threads',
            ['lastposteruid' => 0],
    ...
    function delete_content($delete_uids = false)
    {
        ...
        $db->delete_query(
            'moderators',
            "id IN({$this->delete_uids}) AND isgroup = 0"
        );
        ...
        $db->delete_query(
            'reputation',
            "uid IN({$this->delete_uids}) OR adduid IN({$this->delete_uids})"
        );
        $db->delete_query(
            'buddyrequests',
            "uid IN({$this->delete_uids}) OR touid IN({$this->delete_uids})"
        );
        $db->delete_query(
            'posts',
        ...
        $db->delete_query(
            'reportedcontent',
            "type='reputation' AND id3 IN({$this->delete_uids}) OR type='reputation' AND id2 IN({$this->delete_uids})"
        );
        $db->delete_query(
            'reportedcontent',
            "type='profile' AND id IN({$this->delete_uids})"
        );
    ...
    public function verify_signature()
    {
        ...
        $parsed_sig = $parser->parse_message(
            $this->data['signature'],
            $parser_options
        );

        if (
            ($mybb->settings['sigimgcode'] == 0 &&
                $mybb->settings['sigsmilies'] != 1 &&
                substr_count($parsed_sig, '<img') > 0) ||
            (($mybb->settings['sigimgcode'] == 1 ||
                $mybb->settings['sigsmilies'] == 1) &&
                substr_count($parsed_sig, '<img') >
                    $mybb->settings['maxsigimages'])
        ) {
            $imgsallowed = 0;
            ...
            $this->set_error('too_many_sig_images2', [$imgsallowed]);
        }

        if ($mybb->settings['sigcountmycode'] == 0) {
            $parsed_sig = $parser->text_parse_message(
                $this->data['signature'],
                ['signature_parse' => '1']
            );
        } else {
        ...
        if ($mybb->settings['siglength'] > 0) {
            ...
            if ($sig_length > $mybb->settings['siglength']) {
                ...
                if ($sig_length - $mybb->settings['siglength'] > 1) {
                    $this->set_error('sig_remove_chars_plural', [
