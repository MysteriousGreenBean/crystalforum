...
class WarningsHandler extends DataHandler
{
    ...
    function validate_maximum()
    {
        ...
        if ($mybb->usergroup['maxwarningsday'] != 0) {
            ...
            if ($given_today >= $mybb->usergroup['maxwarningsday']) {
                $this->set_error('reached_max_warnings_day', [
                    my_number_format($mybb->usergroup['maxwarningsday']),
                ]);
                return false;
            }
        }
    ...
    function validate_type()
    {
        ...
        if ($warning['type'] == 'custom') {
            ...
            if (
                !$warning['custom_points'] ||
                $warning['custom_points'] >
                    $mybb->settings['maxwarningpoints'] ||
                $warning['custom_points'] < 0
            ) {
                $this->set_error('error_invalid_custom_points', [
                    my_number_format($mybb->settings['maxwarningpoints']),
                ]);
                return false;
            }
        ...
        // Using a predefined warning type
            ...
            if ($this->warning_type['expirationtime']) {
                $warning['expires'] =
                    TIME_NOW + $this->warning_type['expirationtime'];
            }
        }
    ...
    function expire_warnings()
    {
        ...
        $query = $db->query(
			...
			LEFT JOIN " .
                TABLE_PREFIX .
                "users u ON (u.uid=w.uid)
			WHERE expires<" .
                TIME_NOW .
                " AND expires!=0 AND expired!=1
		"
        );
        ...
        if ($method == 'insert') {
                ...
                switch ($action['type']) {
                    ...
                    case 1:
                        ...
                        $query = $db->simple_select(
                            'banned',
                            '*',
                            "uid='{$user['uid']}' AND gid='{$action['usergroup']}' AND lifted>" .
                                TIME_NOW
                        );
                        ...
                        if (
                            ($expiration > $existing_ban['lifted'] &&
                                $existing_ban['lifted'] != 0) ||
                            $expiration == 0 ||
                            !$existing_ban['uid']
                        ) {
                            if (!$warning['title']) {
                                $warning['title'] =
                                    $this->warning_type['title'];
                            }
                            ...
                            if ($existing_ban['uid']) {
                                $db->delete_query(
                                    'banned',
                                    "uid='{$user['uid']}' AND gid='{$action['usergroup']}'"
                                );
                                // Override new ban details with old group info
                                $new_ban['oldgroup'] =
                                    $existing_ban['oldgroup'];
                                $new_ban[
                                    'oldadditionalgroups'
                                ] = $db->escape_string(
                                    $existing_ban['oldadditionalgroups']
                                );
                                $new_ban['olddisplaygroup'] =
                                    $existing_ban['olddisplaygroup'];
                            }
                            ...
                            $group_name =
                                $groupscache[$action['usergroup']]['title'];
                            $this->friendly_action = $lang->sprintf(
                                $lang->redirect_warned_banned,
                                $group_name,
                                $period
                            );

                            $db->insert_query('banned', $new_ban);
                            $this->updated_user['usergroup'] =
                                $action['usergroup'];
                    ...
                    case 2:
                        ...
                        if (
                            $expiration == 0 ||
                            $expiration > $user['suspensiontime']
                        ) {
                            if (
                                ...
                                $this->friendly_action = $lang->sprintf(
                                    $lang->redirect_warned_suspended,
                                    $period
                                );
                    ...
                    case 3:
                        ...
                        if (
                            $expiration == 0 ||
                            $expiration > $user['moderationtime']
                        ) {
                            if (
                                ($user['moderationtime'] != 0 &&
                                    $user['moderateposts']) ||
                                !$user['suspendposting']
                            ) {
                                $period = $lang->expiration_never;
            ...
            if ($warning['expired'] != 1) {
                ...
                $query = $db->simple_select(
                    'warninglevels',
                    'action',
                    "percentage>{$this->new_warning_level} AND percentage<=$current_level"
                );
                        ...
                        if ($check_levels[$i]) {
                            ...
                            if ($lower_levels[$i]) {
                                ...
                                if ($max_expiration_times[$i]) {
                                    ...
                                    if (
                                        $max_expiration_times[$i] <=
                                        $lower_expiration_times[$i]
                                    ) {
                                ...
                                $this->updated_user[$current_expiry_field] =
                                    $user[$current_expiry_field] +
                                    $expire_offset;
    ...
    function insert_warning()
    {
        ...
        $this->write_warning_data['wid'] = $db->insert_query(
            'warnings',
            $this->write_warning_data
        );
