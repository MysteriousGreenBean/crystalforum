...
HTML;
    ...
    public function list_tables($database, $prefix = '')
    {
        if ($prefix) {
            if (version_compare($this->get_version(), '5.0.2', '>=')) {
                ...
                $query = $this->query(
                    "SHOW TABLES FROM `{$database}` LIKE '{$this->escape_string(
                        $prefix
                    )}%'"
                );
            }
    ...
    public function table_exists($table)
    {
        ...
        if (version_compare($this->get_version(), '5.0.2', '>=')) {
            $query = $this->query(
                "SHOW FULL TABLES FROM `{$this->database}` WHERE table_type = 'BASE TABLE' AND `Tables_in_{$this->database}` = '{$this->table_prefix}{$table}'"
            );
        } else {
            $query = $this->query(
