...
class DB_MySQLi implements DB_Base
{
    ...
    function connect($config)
    {
        ...
        foreach (['read', 'write'] as $type) {
            ...
            foreach ($connections[$type] as $single_connection) {
                ...
                $persist = '';
                if (
                    !empty($single_connection['pconnect']) &&
                ...
                if ($port) {
                    ...
                    $this->$link = @$connect_function(
                        $persist . $single_connection['hostname'],
                        $single_connection['username'],
                        $single_connection['password']
                    );
                }
    ...
    function select_db($database)
    {
        ...
        ($master_success = @mysqli_select_db($this->read_link, $database)) or
            $this->error('[READ] Unable to select database', $this->read_link);
        if ($this->write_link) {
            ($slave_success = @mysqli_select_db(
                $this->write_link,
                $database
            )) or
                $this->error(
                    '[WRITE] Unable to select slave database',
                    $this->write_link
                );
    ...
    function explain_query($string, $qtime)
    {
        ...
        if (preg_match('#^\s*select#i', $string)) {
            ...
            $this->explain .=
                ...
                "<tr>\n" .
                "<td colspan=\"8\" style=\"background-color: #ccc;\">{$debug_extra}<div><strong>#" .
                $this->query_count .
                " - Select Query</strong></div></td>\n" .
                "</tr>\n" .
                "<tr>\n" .
            ...
            $this->explain .=
                "<table style=\"background-color: #666;\" width=\"95%\" cellpadding=\"4\" cellspacing=\"1\" align=\"center\">\n" .
                "<tr>\n" .
                "<td style=\"background-color: #ccc;\">{$debug_extra}<div><strong>#" .
                $this->query_count .
                " - Write Query</strong></div></td>\n" .
                "</tr>\n" .
    ...
    function list_tables($database, $prefix = '')
    {
        if ($prefix) {
            if (version_compare($this->get_version(), '5.0.2', '>=')) {
                $query = $this->query(
                    "SHOW FULL TABLES FROM `$database` WHERE table_type = 'BASE TABLE' AND `Tables_in_$database` LIKE '" .
        ...
        if (version_compare($this->get_version(), '5.0.2', '>=')) {
            $query = $this->query(
                'SHOW FULL TABLES FROM `' .
                    $this->database .
                    "` WHERE table_type = 'BASE TABLE' AND `Tables_in_" .
                    $this->database .
                    "` = '{$this->table_prefix}$table'"
            );
        } else {
            $query = $this->query(
                "SHOW TABLES LIKE '{$this->table_prefix}$table'"
            );
        }
    ...
    {
        $this->write_query(
            'OPTIMIZE TABLE ' . $this->table_prefix . $table . ''
        );
    }
    ...
    {
        $this->write_query(
            'ANALYZE TABLE ' . $this->table_prefix . $table . ''
        );
    }
    ...
    {
        $query = $this->write_query(
            'SHOW CREATE TABLE ' . $this->table_prefix . $table . ''
        );
    ...
    {
        $query = $this->write_query(
            'SHOW FIELDS FROM ' . $this->table_prefix . $table . ''
        );
        $field_info = [];
    ...
    function supports_fulltext($table)
    {
        ...
        $query = $this->write_query(
            "SHOW TABLE STATUS LIKE '{$this->table_prefix}$table'"
        );
        $status = $this->fetch_array($query);
        $table_type = my_strtoupper($status['Engine']);
        if (
            version_compare($version, '3.23.23', '>=') &&
            ($table_type == 'MYISAM' || $table_type == 'ARIA')
        ) {
            return true;
        } elseif (
    ...
    function index_exists($table, $index)
    {
        $index_exists = false;
        $query = $this->write_query(
            "SHOW INDEX FROM {$this->table_prefix}{$table}"
        );
    ...
    {
        $this->write_query(
            "ALTER TABLE {$this->table_prefix}$table ADD FULLTEXT $name ($column)"
        );
    }
    ...
    function rename_table($old_table, $new_table, $table_prefix = true)
    {
        ...
        return $this->write_query(
            "RENAME TABLE {$table_prefix}{$old_table} TO {$table_prefix}{$new_table}"
        );
    }
    ...
    function replace_query(
        ...
        foreach ($replacements as $column => $value) {
            if (
                isset($mybb->binary_fields[$table][$column]) &&
                ...
                $values .=
                    $comma . '`' . $column . '`=' . $this->quote_val($value);
            }
        ...
        return $this->write_query(
            "REPLACE INTO {$this->table_prefix}{$table} SET {$values}"
        );
    }
    ...
    function modify_column(
        ...
        return (bool) $this->write_query(
            "ALTER TABLE {$this->table_prefix}{$table} MODIFY `{$column}` {$new_definition} {$not_null} {$default}"
        );
    }
    ...
    function rename_column(
        ...
        return (bool) $this->write_query(
            "ALTER TABLE {$this->table_prefix}{$table} CHANGE `{$old_column}` `{$new_column}` {$new_definition} {$not_null} {$default}"
        );
    }
    ...
    function fetch_db_charsets()
    {
        if (
            $this->write_link &&
            version_compare($this->get_version(), '4.1', '<')
        ) {
            return false;
        }
        return [
