...
class DB_PgSQL implements DB_Base
{
        ...
        foreach (['read', 'write'] as $type) {
            ...
            foreach ($connections[$type] as $single_connection) {
                ...
                if (strpos($single_connection['hostname'], ':') !== false) {
                    list(
                        $single_connection['hostname'],
                        $single_connection['port'],
                    ) = explode(':', $single_connection['hostname']);
                } else {
                ...
                if ($this->$link) {
                    $this->connections[] =
                        '[' .
                        strtoupper($type) .
                        "] {$single_connection['username']}@{$single_connection['hostname']} (Connected in " .
                        format_time_duration($time_spent) .
                        ')';
                    break;
                } else {
                    $this->connections[] =
                        ...
                        strtoupper($type) .
                        "] {$single_connection['username']}@{$single_connection['hostname']}</span>";
                }
            }
        }
    ...
    function explain_query($string, $qtime)
    {
        if (preg_match('#^\s*select#i', $string)) {
            ...
            $this->explain .=
                ...
                "<tr>\n" .
                "<td colspan=\"8\" style=\"background-color: #ccc;\"><strong>#" .
                $this->query_count .
                " - Select Query</strong></td>\n" .
                "</tr>\n" .
                "<tr>\n" .
                "<td colspan=\"8\" style=\"background-color: #fefefe;\"><span style=\"font-family: Courier; font-size: 14px;\">" .
            ...
            $this->explain .=
                "<tr>\n" .
                "<td colspan=\"8\" style=\"background-color: #fff;\">Query Time: " .
            ...
            $this->explain .=
                ...
                $this->query_count .
                " - Write Query</strong></td>\n" .
                "</tr>\n" .
                "<tr style=\"background-color: #fefefe;\">\n" .
                "<td><span style=\"font-family: Courier; font-size: 14px;\">" .
                htmlspecialchars_uni($string) .
                "</span></td>\n" .
                "</tr>\n" .
                "<tr>\n" .
                "<td bgcolor=\"#ffffff\">Query Time: " .
                format_time_duration($qtime) .
                "</td>\n" .
                "</tr>\n" .
                "</table>\n" .
    ...
    function insert_id()
    {
        ...
        $query = $this->query(
            "SELECT column_name FROM information_schema.constraint_column_usage WHERE table_name = '{$table}' and constraint_name = '{$table}_pkey' LIMIT 1"
        );
        ...
        $id = $this->write_query(
            "SELECT currval(pg_get_serial_sequence('{$table}', '{$field}')) AS last_value"
        );
    ...
    function table_exists($table)
    {
        ...
        $query = $this->write_query(
            "SELECT COUNT(table_name) as table_names FROM information_schema.tables WHERE table_schema = 'public' AND table_name='{$this->table_prefix}{$table}'"
        );
    ...
    {
        $query = $this->write_query(
            "SELECT COUNT(column_name) as column_names FROM information_schema.columns WHERE table_name='{$this->table_prefix}{$table}' AND column_name='{$field}'"
        );
    ...
    function simple_select(
        ...
        if (isset($options['limit_start']) && isset($options['limit'])) {
            $query .=
                ' LIMIT ' . $options['limit_start'] . ', ' . $options['limit'];
    ...
    function insert_query($table, $array, $insert_id = true)
    {
        ...
        foreach ($array as $field => $value) {
            if (
                isset($mybb->binary_fields[$table][$field]) &&
                $mybb->binary_fields[$table][$field]
            ) {
                $array[$field] = $value;
            } else {
    ...
    function show_create_table($table)
    {
        ...
        if (!empty($primary_key)) {
            $lines[] =
                "  CONSTRAINT $primary_key_name PRIMARY KEY (" .
                implode(', ', $primary_key) .
                ')';
        }
    ...
    {
        $query = $this->write_query(
            "SELECT column_name FROM information_schema.constraint_column_usage WHERE table_name = '{$this->table_prefix}{$table}' and constraint_name = '{$this->table_prefix}{$table}_pkey' LIMIT 1"
        );
        ...
        while ($field = $this->fetch_array($query)) {
            ...
            if (!is_null($field['character_maximum_length'])) {
                $field['data_type'] .=
                    '(' . (int) $field['character_maximum_length'] . ')';
            }
            // numeric/decimal fields.
            elseif (
                $field['numeric_precision_radix'] == 10 &&
                !is_null($field['numeric_precision']) &&
                !is_null($field['numeric_scale'])
            ) {
                $field['data_type'] .=
                    '(' .
                    (int) $field['numeric_precision'] .
                    ',' .
                    (int) $field['numeric_scale'] .
                    ')';
            }
    ...
    function drop_table($table, $hard = false, $table_prefix = true)
    {
        ...
        $this->table_prefix = '';
        $fields = array_column(
            $this->show_fields_from($table_prefix . $table),
            'Field'
        );
        ...
        if (!empty($fields)) {
            ...
            if (version_compare($this->get_version(), '8.2.0', '>=')) {
                ...
                $fields = "'" . implode("', '", $fields) . "'";
                $query = $this->query(
                    "SELECT sequence_name as field FROM information_schema.sequences WHERE sequence_name in ({$fields}) AND sequence_schema = 'public'"
                );
                while ($row = $this->fetch_array($query)) {
    ...
    function rename_table($old_table, $new_table, $table_prefix = true)
    {
        ...
        return $this->write_query(
            "ALTER TABLE {$table_prefix}{$old_table} RENAME TO {$table_prefix}{$new_table}"
        );
    }
    ...
    function replace_query(
        ...
        foreach ($main_field as $field) {
            if (
                ...
                $search_bit[] =
                    "{$field} = " . $this->quote_val($replacements[$field]);
            }
        }
        ...
        $query = $this->write_query(
            'SELECT COUNT(' .
                $main_field[0] .
                ") as count FROM {$this->table_prefix}{$table} WHERE {$search_bit} LIMIT 1"
        );
    ...
    {
        return $this->write_query(
            "ALTER TABLE {$this->table_prefix}{$table} DROP {$column}"
        );
    }
    ...
    {
        return $this->write_query(
            "ALTER TABLE {$this->table_prefix}{$table} ADD {$column} {$definition}"
        );
    }
    ...
    function modify_column(
        ...
        if ($new_definition !== false) {
            $result1 = $this->write_query(
                "ALTER TABLE {$this->table_prefix}{$table} ALTER COLUMN {$column} TYPE {$new_definition}"
            );
        }
        ...
        if ($new_not_null !== false) {
            ...
            $result2 = $this->write_query(
                "ALTER TABLE {$this->table_prefix}{$table} ALTER COLUMN {$column} {$set_drop} NOT NULL"
            );
        }

        if ($new_default_value !== null) {
            if ($new_default_value !== false) {
                $result3 = $this->write_query(
                    "ALTER TABLE {$this->table_prefix}{$table} ALTER COLUMN {$column} SET DEFAULT {$new_default_value}"
                );
            } else {
                $result3 = $this->write_query(
                    "ALTER TABLE {$this->table_prefix}{$table} ALTER COLUMN {$column} DROP DEFAULT"
                );
            }
        }
    ...
    function rename_column(
        ...
        $result1 = $this->write_query(
            "ALTER TABLE {$this->table_prefix}{$table} RENAME COLUMN {$old_column} TO {$new_column}"
        );
        $result2 = $this->modify_column(
            $table,
            $new_column,
            $new_definition,
            $new_not_null,
