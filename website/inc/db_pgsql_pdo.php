...
HTML;
    ...
    public function list_tables($database, $prefix = '')
    {
        if ($prefix) {
            $query = $this->write_query(
                "SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name LIKE '{$this->escape_string(
                    $prefix
                )}%'"
            );
        } else {
    ...
    public function insert_query_multiple($table, $array)
    {
        ...
        foreach ($array as $values) {
            $insert_rows[] =
                '(' . $this->build_value_string($table, $values) . ')';
        }
    ...
    public function show_fields_from($table)
    {
        ...
        while ($field = $this->fetch_array($query)) {
            ...
            if (
                !is_null($field['column_default']) &&
                stripos($field['column_default'], 'nextval') !== false
            ) {
    ...
    public function replace_query(
        ...
        if (version_compare($this->get_version(), '9.5.0', '>=')) {
            ...
            $reassignments = $this->build_field_value_string(
                $table,
                $reassignment_values,
                true
            );
    ...
    public function fetch_size($table = '')
    {
        if (!empty($table)) {
            $query = $this->query(
                "SELECT SUM(reltuples), SUM(relpages) FROM pg_class WHERE relname = '{$this->table_prefix}{$table}'"
            );
        } else {
            $query = $this->query(
                'SELECT SUM(reltuples), SUM(relpages) FROM pg_class'
            );
        }
     ...
     * @return string
     */
    protected function build_field_value_string(
        $table,
        $array,
        $no_quote = false
    ) {
