...
class DB_SQLite implements DB_Base
{
    ...
    function connect($config)
    {
        ...
        $this->connections[] =
            "[WRITE] {$config['database']} (Connected in " .
            format_time_duration($query_time) .
            ')';
    ...
    function query($string, $hide_errors = 0, $write_query = 0)
    {
        ...
        if (strtolower(substr(ltrim($string), 0, 5)) == 'alter') {
            ...
            if (strtolower($queryparts[1]) != 'table' || $queryparts[2] == '') {
                ...
                if (
                    strtolower(substr(ltrim($alterdefs), 0, 3)) == 'add' ||
                    strtolower(substr(ltrim($alterdefs), 0, 9)) == 'rename to'
                ) {
                    $query = $this->db->query($string);
                    $query->closeCursor();
                } else {
                    $query = $this->alter_table_parse(
                        $tablename,
                        $alterdefs,
                        $string
                    );
    ...
    function error($string = '', $query = null, $error = '', $error_no = 0)
    {
        if ($this->error_reporting) {
            ...
            if (class_exists('errorHandler')) {
                ...
                trigger_error(
                    "<strong>[SQL] [{$error_no}] {$error}</strong><br />{$string}",
                    E_USER_ERROR
                );
            }
        }
    ...
    {
        $query = $this->query(
            "SELECT COUNT(name) as count FROM sqlite_master WHERE type='table' AND name='{$this->table_prefix}{$table}'"
        );
    ...
    {
        $query = $this->query(
            "PRAGMA table_info('{$this->table_prefix}{$table}')"
        );
    ...
    function show_create_table($table)
    {
        ...
        $query = $this->simple_select(
            'sqlite_master',
            'sql',
            "type = 'table' AND name = '{$old_tbl_prefix}{$table}' ORDER BY type DESC, name"
        );
    ...
    function replace_query(
        ...
        if ($default_field == '') {
            $query = $this->query(
                "REPLACE INTO {$this->table_prefix}{$table} ({$columns}) VALUES({$values})"
            );
            $query->closeCursor();
            return $query;
        } else {
            ...
            if (is_array($default_field) && !empty($default_field)) {
                ...
                $query = $this->write_query(
                    "SELECT {$default_field} FROM {$this->table_prefix}{$table}"
                );
                $search_bit =
                    "{$default_field}='" . $replacements[$default_field] . "'";

                while ($column = $this->fetch_array($query)) {
                    if (
                        $column[$default_field] == $replacements[$default_field]
                    ) {
                        $update = true;
                        break;
                    }
    ...
    function alter_table_parse($table, $alterdefs, $fullquery = '')
    {
        ...
        if ($alterdefs != '') {
            $result = $this->query(
                "SELECT sql,name,type FROM sqlite_master WHERE tbl_name = '{$table}' ORDER BY type DESC"
            );
            if ($this->num_rows($result) > 0) {
                ...
                $oldcols = preg_split(
                    '/[,]+/',
                    substr(
                        trim($createtemptableSQL),
                        strpos(trim($createtemptableSQL), '(') + 1
                    ),
                    -1,
                    PREG_SPLIT_NO_EMPTY
                );
