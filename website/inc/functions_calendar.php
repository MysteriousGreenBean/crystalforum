...
function build_mini_calendar($calendar, $month, $year, &$events_cache)
{
    ...
    $month_start_weekday = gmdate(
        'w',
        gmmktime(0, 0, 0, $month, $calendar['startofweek'] + 1, $year)
    );

    $prev_month_days = gmdate(
        't',
        gmmktime(0, 0, 0, $prev_month['month'], 1, $prev_month['year'])
    );
    ...
    foreach ($weekdays as $weekday) {
        ...
        eval(
            "\$weekday_headers .= \"" .
                $templates->get('calendar_mini_weekdayheader') .
                "\";"
        );
    }
    ...
    for (
        ...
        foreach ($weekdays as $weekday_id => $weekday) {
            ...
            if ($link_to_day) {
                ...
                eval(
                    "\$day_link = \"" .
                        $templates->get('calendar_mini_weekrow_day_link') .
                        "\";"
                );
            } else {
                $day_link = $day;
            ...
            eval(
                "\$day_bits .= \"" .
                    $templates->get('calendar_mini_weekrow_day') .
                    "\";"
            );
            ++$day;
        }
        if ($day_bits) {
            eval(
                "\$calendar_rows .= \"" .
                    $templates->get('calendar_mini_weekrow') .
                    "\";"
            );
        }
        $day_bits = '';
...
function get_calendar_permissions($cid = 0)
{
    ...
    if (!is_array($calendar_permissions)) {
        ...
        while ($permission = $db->fetch_array($query)) {
            $calendar_permissions[$permission['cid']][
                $permission['gid']
            ] = $permission;
        }
    ...
    if ($cid > 0) {
        if (isset($calendar_permissions[$cid])) {
            $permissions = fetch_calendar_permissions(
                $cid,
                $gid,
                $calendar_permissions[$cid]
            );
        }
        ...
        foreach ($calendars as $calendar) {
            if (isset($calendar_permissions[$calendar['cid']])) {
                $permissions[$calendar['cid']] = fetch_calendar_permissions(
                    $calendar['cid'],
                    $gid,
                    $calendar_permissions[$calendar['cid']]
                );
            }
...
function build_calendar_jump($selected = 0)
{
    ...
    foreach ($calendars as $calendar) {
        ...
        $sel = '';
        if (
            $selected == $calendar['cid'] ||
            ($selected == 0 && $calendar['disporder'] == 1)
        ) {
        ...
        eval(
            "\$jump_options .= \"" .
                $templates->get('calendar_jump_option') .
                "\";"
        );
    }
...
function get_events($calendar, $start, $end, $unapproved = 0, $private = 1)
{
    ...
    while ($event = $db->fetch_array($query)) {
        ...
        // Ranged event
            ...
            $event_date = explode(
                '-',
                gmdate('j-n-Y', $event['starttime_user'])
            );
            ...
            if ($event['repeats']['repeats'] == 0) {
                ...
                $range_start = fetch_next_occurance(
                    $event,
                    ['start' => $start, 'end' => $end],
                    $start_day,
                    true
            ...
            while ($range_start < $range_end) {
                ...
                if ($range_start >= $start) {
                    ...
                    if ($first && $day_date != "{$first}") {
                        $events_cache[$day_date][] =
                            &$events_cache["{$first}"][$count];
                    } elseif (!$first) {
                        if (
                            !isset($events_cache[$day_date]) ||
                            !is_array($events_cache[$day_date])
                        ) {
                            $events_cache[$day_date] = [];
                        }
...
function get_birthdays($months, $day = 0)
{
    ...
    foreach ($months as $month) {
        ...
        if (
            $month == 3 &&
            ($day == 1 || !$day) &&
            my_date('L', gmmktime(0, 0, 0, $month, 1, $year)) != 1
        ) {
    ...
    $query = $db->simple_select(
        'users',
        'uid, username, birthday, birthdayprivacy, usergroup, displaygroup',
        $where
    );
...
function fetch_next_occurance($event, $range, $last_occurance, $first = false)
{
    ...
    elseif ($repeats['repeats'] == 4) {
        ...
        // The 1st/etc (weekday) of every X months
            ...
            if ($first == true) {
                $new_time = fetch_weekday_monthly_repetition(
                    $repeats,
                    $last_month,
                    $last_year
                );
            ...
            if (
                ...
                foreach ($event['repeats']['days'] as $id => $weekday) {
                    ...
                    if ($event['repeats']['days'][$id + 1] && $weekday) {
                        $weekdays .= $lang->comma;
                    } elseif (!$event['repeats']['days'][$id + 1] && $weekday) {
                        $weekdays .= " {$lang->and} ";
                    }
        ...
        case 4:
            if ($event['repeats']['day']) {
                if ($event['repeats']['months'] == 1) {
                    return $lang->sprintf(
                        $lang->every_month_on_day,
                        $event['repeats']['day']
                    );
                } else {
                    return $lang->sprintf(
                        $lang->every_x_months_on_day,
                        $event['repeats']['day'],
                        $event['repeats']['months']
                    );
                }
            } else {
                $weekday_name = fetch_weekday_name(
                    $event['repeats']['weekday']
                );
                $occurance =
                    'weekday_occurance_' . $event['repeats']['occurance'];
                $occurance = $lang->$occurance;
                if ($event['repeats']['months'] == 1) {
                    return $lang->sprintf(
                        $lang->every_month_on_weekday,
                        $occurance,
                        $weekday_name
                    );
                } else {
                    return $lang->sprintf(
                        $lang->every_x_months_on_weekday,
                        $occurance,
                        $weekday_name,
                        $event['repeats']['months']
                    );
        ...
        case 5:
            ...
            if ($event['repeats']['day']) {
                if ($event['repeats']['years'] == 1) {
                    return $lang->sprintf(
                        $lang->every_year_on_day,
                        $event['repeats']['day'],
                        $month
                    );
                } else {
                    return $lang->sprintf(
                        $lang->every_x_years_on_day,
                        $event['repeats']['day'],
                        $month,
                        $event['repeats']['years']
                    );
...
function fetch_weekday_monthly_repetition($repeats, $month, $year)
{
    ...
    if ($repeats['occurance'] != 'last') {
        ...
        $last_dow = gmdate(
            'w',
            gmmktime(0, 0, 0, $month, gmdate('t', $first_last), $year)
        );
