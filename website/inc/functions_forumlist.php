    ...
    foreach ($fcache[$pid] as $parent) {
        foreach ($parent as $forum) {
            ...
            if (
                ...
                $mybb->settings['hideprivateforums'] == 1
            ) {
                continue;
            }
            ...
            if (
                $permissions['canview'] != 1 ||
                (isset($permissions['canviewthreads']) &&
            ...
            if (
                isset($permissions['canonlyviewownthreads']) &&
                ...
                if (!is_array($private_forums)) {
                    ...
                    if (!empty($fids)) {
                        ...
                        $query = $db->simple_select(
                            ...
                            "uid = '{$mybb->user['uid']}' AND fid IN ({$fids}) AND visible != '-2'",
                ...
                if (!empty($private_forums[$forum['fid']]['lastpost'])) {
                    ...
                    if (
                        !$private_forums[$forum['fid']]['lastposteruid'] &&
                        !$private_forums[$forum['fid']]['lastposter']
                    ) {
                        $private_forums[$forum['fid']]['lastposter'] =
                            $lang->guest; // htmlspecialchars_uni'd when formatted later
                    }
                    ...
                    $lastpost_data = [
                        'lastpost' =>
                            $private_forums[$forum['fid']]['lastpost'],
                        'lastpostsubject' =>
                            $private_forums[$forum['fid']]['subject'],
                        'lastposter' =>
                            $private_forums[$forum['fid']]['lastposter'],
            ...
            if (
                is_array($forum_viewers) &&
                isset($forum_viewers[$forum['fid']]) &&
                $forum_viewers[$forum['fid']] > 0
            ) {
            ...
            if (isset($fcache[$forum['fid']])) {
                ...
                $forum['unapprovedthreads'] +=
                    $forum_info['counters']['unapprovedthreads'];
                $forum['unapprovedposts'] +=
                    $forum_info['counters']['unapprovedposts'];
                ...
                if (
                    isset($forum_info['lastpost']['lastpost']) &&
                    $forum_info['lastpost']['lastpost'] >
            ...
            if (
                (!isset($parent_lastpost) ||
                    $lastpost_data['lastpost'] >
                        $parent_lastpost['lastpost']) &&
                $hideinfo != true
            ) {
            ...
            if ($hideinfo != true && $hidecounters != true) {
                ...
                $parent_counters['unapprovedposts'] +=
                    $forum['unapprovedposts'];
                $parent_counters['unapprovedthreads'] +=
                    $forum['unapprovedthreads'];
            ...
            if ($depth == 2 && $sub_forums) {
                eval(
                    "\$subforums = \"" .
                        $templates->get('forumbit_subforums') .
                        "\";"
                );
            }
            ...
            elseif ($depth == 3) {
                if ($donecount < $mybb->settings['subforumsindex']) {
                    ...
                    eval(
                        "\$forum_list .= \"" .
                            $templates->get('forumbit_depth3', 1, 0) .
                            "\";"
                    );
                    $comma = $lang->comma;
                }
            ...
            if ($forum['linkto'] == '') {
                ...
                if ($lastpost_data['lastpost'] == 0 && $hideinfo != true) {
                    ...
                    $lastpost_data['lastposter'] = htmlspecialchars_uni(
                        $lastpost_data['lastposter']
                    );
                    ...
                    $lastpost_link = get_thread_link(
                        $lastpost_data['lastposttid'],
                        0,
                        'lastpost'
                    );
                    $lastpost_subject = $full_lastpost_subject = $parser->parse_badwords(
                        $lastpost_data['lastpostsubject']
                    );
                    if (my_strlen($lastpost_subject) > 25) {
                        $lastpost_subject =
                            my_substr($lastpost_subject, 0, 25) . '...';
                    }
                    $lastpost_subject = htmlspecialchars_uni($lastpost_subject);
                    $full_lastpost_subject = htmlspecialchars_uni(
                    ...
                    if ($depth != 1) {
                        eval(
                            "\$lastpost = \"" .
                                $templates->get(
                                    "forumbit_depth{$depth}_forum_lastpost"
                                ) .
                                "\";"
                        );
                    }
                ...
                if (
                    $mybb->settings['showforumviewing'] != 0 &&
                    $forum['viewers'] > 0
                ) {
                    if ($forum['viewers'] == 1) {
                        $forum_viewers_text = $lang->viewing_one;
                    } else {
                        $forum_viewers_text = $lang->sprintf(
                            $lang->viewing_multiple,
                            $forum['viewers']
                        );
                    }
                    ...
                    eval(
                        "\$forum_viewers_text = \"" .
                            $templates->get('forumbit_depth2_forum_viewers') .
                            "\";"
                    );
                }
            }
            ...
            if ($mybb->settings['modlist'] != 0) {
                ...
                if ($moderators) {
                    eval(
                        "\$modlist = \"" .
                            $templates->get('forumbit_moderators') .
                            "\";"
                    );
                }
            }
            ...
            if (
                isset($collapsed[$collapsed_name]) &&
                $collapsed[$collapsed_name] == 'display: none;'
            ) {
            ...
            eval(
                "\$forum_list .= \"" .
                    $templates->get("forumbit_depth$depth$forumcat") .
                    "\";"
            );
        }
    }
...
function get_forum_lightbulb($forum, $lastpost, $locked = 0)
{
    ...
    elseif ($forum['open'] == 0 || $locked) {
        ...
        if (!empty($forum['lastread'])) {
            ...
            $forum_read = 0;
            $threadcut =
                TIME_NOW - 60 * 60 * 24 * $mybb->settings['threadreadcut'];

            // If the user is a guest, do they have a forumsread cookie?
            if (
                !$mybb->user['uid'] &&
                isset($mybb->cookies['mybb']['forumread'])
            ) {
    ...
    if (is_moderator($forum['fid'], 'canviewunapprove')) {
        ...
        if ($forum['unapprovedposts']) {
            if ($forum['unapprovedposts'] > 1) {
                $unapproved_posts_count = $lang->sprintf(
                    $lang->forum_unapproved_posts_count,
                    $forum['unapprovedposts']
                );
            } else {
                $unapproved_posts_count = $lang->sprintf(
                    $lang->forum_unapproved_post_count,
                    1
                );
            }

            $forum['unapprovedposts'] = my_number_format(
                $forum['unapprovedposts']
            );
            eval(
                "\$unapproved_posts = \"" .
                    $templates->get('forumbit_depth2_forum_unapproved_posts') .
                    "\";"
            );
        }
        ...
        if ($forum['unapprovedthreads']) {
            if ($forum['unapprovedthreads'] > 1) {
                $unapproved_threads_count = $lang->sprintf(
                    $lang->forum_unapproved_threads_count,
                    $forum['unapprovedthreads']
                );
            } else {
                $unapproved_threads_count = $lang->sprintf(
                    $lang->forum_unapproved_thread_count,
                    1
                );
            }

            $forum['unapprovedthreads'] = my_number_format(
                $forum['unapprovedthreads']
            );
            eval(
                "\$unapproved_threads = \"" .
                    $templates->get(
                        'forumbit_depth2_forum_unapproved_threads'
                    ) .
                    "\";"
            );
        }
