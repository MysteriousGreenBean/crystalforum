    ...
    if ($imgwidth >= $maxwidth || $imgheight >= $maxheight) {
        ...
        if ($imgtype == 3) {
            ...
            imagefill(
                ...
                0,
                0,
                imagecolorallocatealpha($thumbim, 0, 0, 0, 127)
            );
            ...
            if ($trans_color >= 0 && $trans_color < imagecolorstotal($im)) {
                ...
                $new_trans_color = imagecolorallocate(
                    $thumbim,
                    $trans['red'],
                    $trans['blue'],
                    $trans['green']
...
function check_thumbnail_memory($width, $height, $type, $bitdepth, $channels)
{
    ...
    $limit = preg_match(
        "#^([0-9]+)\s?([kmg])b?$#i",
        trim(my_strtolower($memory_limit)),
        $matches
    );
    ...
    $thumbnail_memory = round(
        (($width * $height * $bitdepth * $channels) / 8) * 5
    );
    ...
    if ($thumbnail_memory > $free_memory) {
        if ($matches[1] && $matches[2]) {
            switch ($matches[2]) {
                case 'k':
                    $memory_limit =
                        ceil(($memory_limit + $thumbnail_memory) / 1024) . 'K';
                    break;
                case 'm':
                    $memory_limit =
                        ceil(($memory_limit + $thumbnail_memory) / 1048576) .
                        'M';
                    break;
                case 'g':
                    $memory_limit =
                        ceil(($memory_limit + $thumbnail_memory) / 1073741824) .
                        'G';
            }
        }
