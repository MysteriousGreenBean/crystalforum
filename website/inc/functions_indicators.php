        ...
        switch ($db->type) {
            ...
            default:
                $db->write_query(
					...
					REPLACE INTO " .
                        TABLE_PREFIX .
                        "threadsread (tid, uid, dateline)
					VALUES('$tid', '{$mybb->user['uid']}', '" .
                        TIME_NOW .
                        "')
				"
                );
...
function fetch_unread_count($fid)
{
    ...
    if (!empty($forums_own)) {
        ...
        $where2 =
            '(t.fid IN (' .
            implode(',', $forums_own) .
            ") AND t.uid = {$mybb->user['uid']})";
    }
    if (!empty($forums_all)) {
        if (isset($where)) {
            $where = "({$where} OR fid IN (" . implode(',', $forums_all) . '))';
            $where2 =
                "({$where2} OR t.fid IN (" . implode(',', $forums_all) . '))';
        } else {
    ...
    if ($mybb->user['uid'] == 0) {
        ...
        if (!empty($tids)) {
            ...
            while ($thread = $db->fetch_array($query)) {
                if (
                    (!isset($threadsread[$thread['tid']]) ||
                        $thread['lastpost'] >
                            (int) $threadsread[$thread['tid']]) &&
        ...
        switch ($db->type) {
            case 'pgsql':
                $query = $db->query(
					...
					LEFT JOIN " .
                        TABLE_PREFIX .
                        "threadsread tr ON (tr.tid=t.tid AND tr.uid='{$mybb->user['uid']}')
					LEFT JOIN " .
                        TABLE_PREFIX .
                        "forumsread fr ON (fr.fid=t.fid AND fr.uid='{$mybb->user['uid']}')
        ...
        switch ($db->type) {
            ...
            default:
                ...
                $db->shutdown_query(
					...
					REPLACE INTO " .
                        TABLE_PREFIX .
                        "forumsread (fid, uid, dateline)
					VALUES('{$fid}', '{$mybb->user['uid']}', '" .
                        TIME_NOW .
                        "'){$child_sql}
				"
                );
