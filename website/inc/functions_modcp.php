...
function modcp_can_manage_user($uid)
{
    ...
    if (
        $mybb->usergroup['issupermod'] == 0 &&
        ($user_permissions['issupermod'] == 1 ||
            $user_permissions['cancp'] == 1)
    ) {
        return false;
    }
    ...
    elseif (
        ...
        ($mybb->usergroup['cancp'] != 1 ||
            (is_super_admin($uid) && !is_super_admin($mybb->user['uid'])))
    ) {
        return false;
    }
    return true;
...
function fetch_forum_announcements($pid = 0, $depth = 1)
{
    ...
    if (!is_array($parent_forums) && $mybb->usergroup['issupermod'] != 1) {
        ...
        foreach ($moderated_forums as $mfid) {
            $parent_forums = array_merge(
                $parent_forums,
                explode(',', $forum_cache[$mfid]['parentlist'])
            );
        }
    }
    ...
    foreach ($forums_by_parent[$pid] as $children) {
        foreach ($children as $forum) {
            if (
                $forum['linkto'] ||
                (is_array($unviewableforums) &&
                    in_array($forum['fid'], $unviewableforums))
            ) {
                continue;
            }
            ...
            if (
                $forum['active'] == 0 ||
                !is_moderator($forum['fid'], 'canmanageannouncements')
            ) {
                // Check if this forum is a parent of a moderated forum
                if (
                    is_array($parent_forums) &&
                    in_array($forum['fid'], $parent_forums)
                ) {
                    // A child is moderated, so print out this forum's title.  RECURSE!
                    $trow = alt_trow();
                    eval(
                        "\$announcements_forum .= \"" .
                            $templates->get('modcp_announcements_forum_nomod') .
                            "\";"
                    );
                } else {
                ...
                eval(
                    "\$announcements_forum .= \"" .
                        $templates->get('modcp_announcements_forum') .
                        "\";"
                );
...
function send_report($report, $report_type = 'post')
{
    ...
    if ($report['reason']) {
        $report_reason = $lang->sprintf(
            $lang->email_report_comment_extra,
            $report_reason,
            $report['reason']
        );
    }
    ...
    if (
        empty($lang->$lang_string_subject) ||
        empty($lang->$lang_string_message)
    ) {
        return false;
    }
    ...
    $emailsubject = $lang->sprintf(
        $lang->$lang_string_subject,
        $mybb->settings['bbname']
    );
    $emailmessage = $lang->sprintf(
        $lang->$lang_string_message,
        $mybb->user['username'],
        $mybb->settings['bbname'],
        $send_report_subject,
        $mybb->settings['bburl'],
    ...
    while ($mod = $db->fetch_array($query)) {
        if (
            $mybb->settings['reportmethod'] == 'pms' &&
            $mod['receivepms'] != 0 &&
...
function add_report($report, $type = 'post')
{
    ...
    $insert_array = [
        ...
        'dateline' => TIME_NOW,
        'lastreport' => TIME_NOW,
        'reporters' => $db->escape_string(my_serialize([$report['uid']])),
    ];

    if (
        $mybb->settings['reportmethod'] == 'email' ||
        $mybb->settings['reportmethod'] == 'pms'
    ) {
