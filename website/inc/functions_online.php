...
$uid_list = $aid_list = $pid_list = $tid_list = $fid_list = $ann_list = $eid_list = [];
...
function fetch_wol_activity($location, $nopermission = false)
{
    ...
    switch ($filename) {
        ...
        case 'modcp':
            ...
            $accepted_parameters['new_announcement'] = [
                'do_new_announcement',
                'new_announcement',
            ];
            $accepted_parameters['delete_announcement'] = [
                'do_delete_announcement',
                'delete_announcement',
            ];
            $accepted_parameters['edit_announcement'] = [
                'do_edit_announcement',
                'edit_announcement',
            ];
            $accepted_parameters['mod_queue'] = ['do_modqueue', 'modqueue'];
            $accepted_parameters['editprofile'] = [
                'do_editprofile',
                'editprofile',
            ];
            $accepted_parameters['banning'] = [
                'do_banuser',
                'banning',
                'liftban',
                'banuser',
        ...
        case 'usercp':
            ...
            if (
                ...
                $parameters['action'] == 'do_profile'
            ) {
                $user_activity['activity'] = 'usercp_profile';
            } elseif (
                $parameters['action'] == 'options' ||
                $parameters['action'] == 'do_options'
            ) {
                $user_activity['activity'] = 'usercp_options';
            } elseif (
                $parameters['action'] == 'password' ||
                $parameters['action'] == 'do_password'
            ) {
                $user_activity['activity'] = 'usercp_password';
            } elseif (
                $parameters['action'] == 'editsig' ||
                $parameters['action'] == 'do_editsig'
            ) {
                $user_activity['activity'] = 'usercp_editsig';
            } elseif (
                $parameters['action'] == 'avatar' ||
                $parameters['action'] == 'do_avatar'
            ) {
                $user_activity['activity'] = 'usercp_avatar';
            } elseif (
                $parameters['action'] == 'editlists' ||
                $parameters['action'] == 'do_editlists'
            ) {
                ...
                $parameters['action'] == 'addsubscription' ||
                ...
                $parameters['action'] == 'notepad' ||
                $parameters['action'] == 'do_notepad'
            ) {
    ...
    $user_activity = $plugins->run_hooks(
...
function build_friendly_wol_location($user_activity)
{
    ...
    if (!is_array($usernames) && count($uid_list) > 0) {
        ...
        if ($uid_sql != $mybb->user['uid']) {
            ...
            while ($user = $db->fetch_array($query)) {
                $usernames[$user['uid']] = htmlspecialchars_uni(
                    $user['username']
                );
            }
        } else {
            $usernames[$mybb->user['uid']] = htmlspecialchars_uni(
                $mybb->user['username']
            );
        }
    }
    ...
    if (!is_array($announcements) && count($ann_list) > 0) {
        ...
        $query = $db->simple_select(
            'announcements',
            'aid,subject',
            "aid IN ({$aid_sql}) {$fidnot}"
        );
        while ($announcement = $db->fetch_array($query)) {
            $announcement_title = htmlspecialchars_uni(
    ...
    if (!is_array($threads) && count($tid_list) > 0) {
        ...
        while ($thread = $db->fetch_array($query)) {
            ...
            if (
                isset($perms[$thread['fid']]['canonlyviewownthreads']) &&
                $perms[$thread['fid']]['canonlyviewownthreads'] == 1 &&
                $thread['uid'] != $mybb->user['uid'] &&
                !is_moderator($thread['fid'])
            ) {
                continue;
            }
            ...
            if (is_moderator($thread['fid']) || $thread['visible'] == 1) {
                ...
                $thread_title .= htmlspecialchars_uni(
    ...
    if (!is_array($events) && count($eid_list) > 0) {
        ...
        while ($event = $db->fetch_array($query)) {
            $events[$event['eid']] = htmlspecialchars_uni(
                $parser->parse_badwords($event['name'])
            );
        }
    }
    ...
    switch ($user_activity['activity']) {
        ...
        case 'announcements':
            if (!empty($announcements[$user_activity['ann']])) {
                $location_name = $lang->sprintf(
                    $lang->viewing_announcements,
                    get_announcement_link($user_activity['ann']),
                    $announcements[$user_activity['ann']]
                );
            } else {
        ...
        case 'forumdisplay':
            if (!empty($forums[$user_activity['fid']])) {
                if ($forums_linkto[$user_activity['fid']]) {
                    $location_name = $lang->sprintf(
                        $lang->forum_redirect_to,
                        get_forum_link($user_activity['fid']),
        ...
        case 'reputation_report':
            if (!empty($usernames[$user_activity['uid']])) {
                $location_name = $lang->sprintf(
                    $lang->viewing_reputation_report,
                    "reputation.php?uid={$user_activity['uid']}",
                    $usernames[$user_activity['uid']]
                );
            } else {
                $location_name = $lang->sprintf(
                    $lang->viewing_reputation_report2
                );
            }
            break;
        ...
        case 'showpost':
            if (
                !empty($posts[$user_activity['pid']]) &&
                !empty($threads[$posts[$user_activity['pid']]])
            ) {
                $pagenote = '';
                $location_name = $lang->sprintf(
                    $lang->reading_thread2,
                    get_thread_link($posts[$user_activity['pid']]),
                    $threads[$posts[$user_activity['pid']]],
                    $pagenote
                );
            } else {
                $location_name = $lang->reading_thread;
    ...
    if (
        isset($user_activity['nopermission']) &&
        $user_activity['nopermission'] == 1
    ) {
...
function build_wol_row($user)
{
    ...
    if ($mybb->usergroup['canviewonlineips'] == 1) {
        ...
        if (
            ...
            eval(
                "\$lookup = \"" .
                    $templates->get('online_row_ip_lookup') .
                    "\";"
            );
        }
