...
function remove_message_quotes(&$text, $rmdepth = null)
{
    ...
    while (!empty($soffsets) && !empty($eoffsets)) {
        // don't rely on this condition - an end offset before the start offset will cause this to loop indefinitely
        $last_offset = 0;
        foreach ($soffsets as $sk => &$soffset) {
            if ($soffset >= $last_offset) {
                ...
                foreach (
                    $eoffsets
                    as $ek =>
                        &$eoffset // use foreach instead of for to get around indexing issues with unset
                ) {
                    if ($eoffset > $soffset) {
                        // we've found a pair
