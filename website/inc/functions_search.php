 ...
 */
function make_searchable_forums(
    $pid = 0,
    $selitem = 0,
    $addselect = 1,
    $depth = ''
    ...
    if (!is_array($pforumcache)) {
        ...
        $query = $db->simple_select(
            'forums',
            'pid,disporder,fid,password,name',
            "linkto='' AND active!=0",
            ['order_by' => 'pid, disporder']
        );
        while ($forum = $db->fetch_array($query)) {
            $pforumcache[$forum['pid']][$forum['disporder']][
                $forum['fid']
            ] = $forum;
        }
    }
            ...
            foreach ($main as $key => $forum) {
                ...
                if (
                    ...
                    if (forum_password_validated($forum, true)) {
                        eval(
                            "\$forumlistbits .= \"" .
                                $templates->get('search_forumlist_forum') .
                                "\";"
                        );
                    }
                    if (!empty($pforumcache[$forum['fid']])) {
                        $newdepth = $depth . '&nbsp;&nbsp;&nbsp;&nbsp;';
                        $forumlistbits .= make_searchable_forums(
                            $forum['fid'],
                            $selitem,
                            0,
                            $newdepth
    ...
    if ($mybb->usergroup['issupermod'] == 1) {
        ...
        if ($moderated_fids !== false) {
            ...
            if (!empty($unapprove_forums)) {
                $unapproved_where .=
                    " OR ({$aliasdot}visible = 0 AND {$aliasdot}fid IN(" .
                    implode(',', $unapprove_forums) .
                    '))';
            }
            if (!empty($deleted_forums)) {
                $unapproved_where .=
                    " OR ({$aliasdot}visible = -1 AND {$aliasdot}fid IN(" .
 ...
 */
function get_password_protected_forums($fids = [])
{
...
function clean_keywords_ft($keywords)
{
    ...
    $keywords = preg_replace(
        "/(\b.{$word_length_regex})(\s)|(\b.{$word_length_regex}$)/u",
        '$2',
        $keywords
    );
    ...
    while ($depth > 0) {
        $words[$depth - 1][] =
            $boolean[$depth - 1] . '(' . implode(' ', $words[$depth]) . ')';
        --$depth;
    }
...
function privatemessage_perform_search_mysql($search)
{
                ...
                if (!$inquote) {
                    ...
                    for ($i = 0; $i < $count_matches; ++$i) {
                        ...
                        // Otherwise check the length of the word as it is a normal search term
                            ...
                            if (
                                my_strlen($word) <
                                $mybb->settings['minsearchword']
                            ) {
                                $lang->error_minsearchlength = $lang->sprintf(
                                    $lang->error_minsearchlength,
                                    $mybb->settings['minsearchword']
                                );
    ...
    if ($search['sender']) {
        ...
        $query = $db->simple_select(
            ...
            'uid',
            "{$field} LIKE '%" .
                $db->escape_string_like($search['sender']) .
                "%'"
        );
...
function perform_search_mysql($search)
{
    ...
    if ($search['author']) {
        ...
        if ($search['matchusername']) {
            ...
            $query = $db->simple_select(
                ...
                'uid',
                "{$field} LIKE '%" .
                    $db->escape_string_like($search['author']) .
                    "%'"
            );
            while ($user = $db->fetch_array($query)) {
                $userids[] = $user['uid'];
    ...
    if ($search['numreplies'] != '' && $search['findthreadst']) {
        if ((int) $search['findthreadst'] == 1) {
            $thread_replycut =
                " AND t.replies >= '" . (int) $search['numreplies'] . "'";
        } else {
            $thread_replycut =
                " AND t.replies <= '" . (int) $search['numreplies'] . "'";
        }
    ...
    $prefixlist = [];
    if (
        !empty($search['threadprefix']) &&
        $search['threadprefix'][0] != 'any'
    ) {
    ...
    if (count($prefixlist) == 1) {
        ...
        if (count($prefixlist) > 1) {
            $thread_prefixcut =
                ' AND t.prefix IN (' . implode(',', $prefixlist) . ')';
        }
    }
    ...
    if (
        !empty($search['forums']) &&
        (!is_array($search['forums']) || $search['forums'][0] != 'all')
    ) {
        if (!is_array($search['forums'])) {
            $search['forums'] = [(int) $search['forums']];
        }
    ...
    // Searching only thread titles
        ...
        if ($firstposts) {
            $query = $db->simple_select(
                'posts',
                'pid',
                "pid IN ($firstposts) {$plain_post_visiblesql} {$limitsql}"
            );
            while ($post = $db->fetch_array($query)) {
                $posts[$post['pid']] = $post['pid'];
            }
