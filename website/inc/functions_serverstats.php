...
function build_server_stats(
    ...
    if (
        isset(
            $phpinfo['iconv']['iconv implementation'],
            $phpinfo['iconv']['iconv library version']
        )
    ) {
        $info['iconvlib'] =
            html_entity_decode($phpinfo['iconv']['iconv implementation']) .
            '|' .
            $phpinfo['iconv']['iconv library version'];
    }
    ...
    $check = [
        'classes' => [
            'dom' => ['bitwise' => 1, 'title' => 'DOMElement'],
            'soap' => ['bitwise' => 2, 'title' => 'SoapClient'],
            'xmlwriter' => ['bitwise' => 4, 'title' => 'XMLWriter'],
            'imagemagick' => ['bitwise' => 8, 'title' => 'Imagick'],
        ],

        'extensions' => [
            'zendopt' => ['bitwise' => 1, 'title' => 'Zend Optimizer'],
            'xcache' => ['bitwise' => 2, 'title' => 'XCache'],
            'eaccelerator' => ['bitwise' => 4, 'title' => 'eAccelerator'],
            'ioncube' => ['bitwise' => 8, 'title' => 'ionCube Loader'],
            'PDO' => ['bitwise' => 16, 'title' => 'PDO'],
            'pdo_mysql' => ['bitwise' => 32, 'title' => 'pdo_mysql'],
            'pdo_pgsql' => ['bitwise' => 64, 'title' => 'pdo_pgsql'],
            'pdo_sqlite' => ['bitwise' => 128, 'title' => 'pdo_sqlite'],
            'pdo_oci' => ['bitwise' => 256, 'title' => 'pdo_oci'],
            'pdo_odbc' => ['bitwise' => 512, 'title' => 'pdo_odbc'],
        ],

        'phpinfo' => [
            'zlib' => ['bitwise' => 1, 'title' => 'zlib'],
            'mbstring' => ['bitwise' => 2, 'title' => 'mbstring'],
            'exif' => ['bitwise' => 4, 'title' => 'exif'],
            'zlib' => ['bitwise' => 8, 'title' => 'zlib'],
        ],

        'functions' => [
            'sockets' => ['bitwise' => 1, 'title' => 'fsockopen'],
            'mcrypt' => ['bitwise' => 2, 'title' => 'mcrypt_encrypt'],
            'simplexml' => ['bitwise' => 4, 'title' => 'simplexml_load_string'],
            'ldap' => ['bitwise' => 8, 'title' => 'ldap_connect'],
            'mysqli' => ['bitwise' => 16, 'title' => 'mysqli_connect'],
            'imap' => ['bitwise' => 32, 'title' => 'imap_open'],
            'ftp' => ['bitwise' => 64, 'title' => 'ftp_login'],
            'pspell' => ['bitwise' => 128, 'title' => 'pspell_new'],
            'apc' => ['bitwise' => 256, 'title' => 'apc_cache_info'],
            'curl' => ['bitwise' => 512, 'title' => 'curl_init'],
            'iconv' => ['bitwise' => 1024, 'title' => 'iconv'],
        ],
    ...
    $server_stats_url =
        'https://community.mybb.com/server_stats.php?' . $string;
        ...
        if (preg_match('#<h2[^>]*>([^<]+)<\/h2>#', $phpinfo_html[$i], $match)) {
            ...
            foreach ($tmp2 as $one) {
                ...
                if (preg_match($pat3, $one, $match)) {
                    $modules[$name][trim($match[1])] = [
                        trim($match[2]),
                        trim($match[3]),
                    ];
                }
                // 2 columns
                elseif (preg_match($pat2, $one, $match)) {
