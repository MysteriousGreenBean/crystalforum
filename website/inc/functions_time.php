        ...
        switch ($fmt[$i]) {
            ...
            case 'r': // Thu, 21 Dec 2000 16:01:07 +0200
                ...
                $dates .=
                    gmdate(
                        'D',
                        $_day_power * (3 + adodb_dow($year, $month, $day))
                    ) .
                    ', ' .
                    ($day < 10 ? '0' . $day : $day) .
                    ' ' .
                    date('M', mktime(0, 0, 0, $month, 2, 1971)) .
                    ' ' .
                    $year .
                    ' ';
            ...
            case 'Z':
                $dates .= $is_gmt
                    ? 0
                    : -adodb_get_gmt_diff($year, $month, $day);
                break;
            case 'O':
...
function adodb_mktime(
    ...
    if (!defined('ADODB_TEST_DATES')) {
        ...
        $usephpfns =
            (1971 < $year && $year < 2038) ||
            (!defined('ADODB_NO_NEGATIVE_TS') &&
                (1901 < $year && $year < 2038));

        if (
            $usephpfns &&
            $year + $mon / 12 + $day / 365.25 + $hr / (24 * 365.25) >= 2038
        ) {
            $usephpfns = false;
        }
