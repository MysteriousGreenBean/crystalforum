...
function remove_attachment($pid, $posthash, $aid)
{
    ...
    if ($db->fetch_field($query, 'numreferences') == 0) {
        ...
        if ($attachment['thumbnail'] && $attachment['thumbnail'] !== 'SMALL') {
            delete_uploaded_file(
                $uploadspath_abs . '/' . $attachment['thumbnail']
            );
        }

        $date_directory = explode('/', $attachment['attachname']);
        $query_indir = $db->simple_select(
            'attachments',
            'COUNT(aid) as indir',
            "attachname LIKE '" .
                $db->escape_string_like($date_directory[0]) .
                "/%'"
        );
        if (
            $db->fetch_field($query_indir, 'indir') == 0 &&
            @is_dir($uploadspath_abs . '/' . $date_directory[0])
        ) {
            delete_upload_directory(
                $uploadspath_abs . '/' . $date_directory[0]
            );
        }
    }
 ...
 */
function upload_avatar($avatar = [], $uid = 0)
{
    ...
    if ($mybb->settings['maxavatardims'] != '') {
        ...
        if (
            ($maxwidth && $img_dimensions[0] > $maxwidth) ||
            ($maxheight && $img_dimensions[1] > $maxheight)
        ) {
            // Automatic resizing enabled?
            if (
                $mybb->settings['avatarresizing'] == 'auto' ||
                ($mybb->settings['avatarresizing'] == 'user' &&
                    $mybb->input['auto_resize'] == 1)
            ) {
                ...
                if (empty($thumbnail['filename'])) {
                    ...
                    $ret['error'] .=
                        '<br /><br />' . $lang->error_avatarresizefailed;
                    delete_uploaded_file($avatarpath . '/' . $filename);
                    return $ret;
                } else {
                    // Copy scaled image to CDN
                    copy_file_to_cdn(
                        $avatarpath . '/' . $thumbnail['filename']
                    );
                    // Reset filesize
                    $avatar['size'] = filesize($avatarpath . '/' . $filename);
                    // Reset dimensions
                    $img_dimensions = @getimagesize(
                        $avatarpath . '/' . $filename
                    );
                }
            } else {
                ...
                if ($mybb->settings['avatarresizing'] == 'user') {
                    $ret['error'] .=
                        '<br /><br />' . $lang->error_avataruserresize;
                }
                delete_uploaded_file($avatarpath . '/' . $filename);
                return $ret;
            }
    ...
    foreach ($attachtypes as $attachtype) {
        if (
            defined('IN_ADMINCP') ||
            (is_member($attachtype['groups']) && $attachtype['avatarfile'])
        ) {
            $allowed_mime_types[$attachtype['mimetype']] =
                $attachtype['maxsize'];
        }
    }
    ...
    if (
        empty($allowed_mime_types[$avatar['type']]) ||
    ...
    if (
        ($mybb->settings['avatarsize'] > 0 &&
            $avatar['size'] > $mybb->settings['avatarsize'] * 1024) ||
        $avatar['size'] > $allowed_mime_types[$avatar['type']] * 1024
    ) {
        delete_uploaded_file($avatarpath . '/' . $filename);
...
function upload_attachment($attachment, $update_attachment = false)
{
    ...
    if (
        ...
        $ret['error'] =
            $lang->error_uploadfailed . $lang->error_uploadfailed_php4;
        return $ret;
    }
    ...
    if (my_strlen($attachment['name']) > $maxFileNameLength) {
        $ret['error'] = $lang->sprintf(
            $lang->error_attach_filename_length,
            htmlspecialchars_uni($attachment['name']),
            $maxFileNameLength
        );
        return $ret;
    }
    ...
    // Check the size
    if (
        $attachment['size'] > $attachtype['maxsize'] * 1024 &&
        $attachtype['maxsize'] != ''
    ) {
        $ret['error'] = $lang->sprintf(
    ...
    $query = $db->simple_select(
        ...
        '*',
        "filename='" .
            $db->escape_string($attachment['name']) .
            "' AND " .
            $uploaded_query
    );
    ...
    if ($mybb->settings['maxattachments'] > 0 && $update_attachment == false) {
        $query = $db->simple_select(
            'attachments',
            'COUNT(aid) AS numattachs',
            $uploaded_query
        );
        $attachcount = $db->fetch_field($query, 'numattachs');
        if ($attachcount >= $mybb->settings['maxattachments']) {
            $ret['error'] = $lang->sprintf(
                $lang->error_maxattachpost,
                $mybb->settings['maxattachments']
            );
            return $ret;
        }
    }
    ...
    if (!empty($file['error'])) {
        $ret['error'] =
            $lang->error_uploadfailed . $lang->error_uploadfailed_detail;
        switch ($file['error']) {
            case 1:
    ...
    if (
        ...
        $attacharray = $plugins->run_hooks(
            'upload_attachment_thumb_start',
            $attacharray
        );

        $thumbnail = generate_thumbnail(
            $uploadspath_abs . '/' . $filename,
            $uploadspath_abs,
            $thumbname,
    ...
    if (
        ...
        !is_moderator($forum['fid'], 'canapproveunapproveattachs')
    ) {
    ...
    if ($prevattach && $update_attachment == true) {
        ...
        $query = $db->simple_select(
            'attachments',
            'COUNT(aid) as numreferences',
            "attachname='" . $db->escape_string($prevattach['attachname']) . "'"
        );
        if ($db->fetch_field($query, 'numreferences') == 0) {
            delete_uploaded_file(
            ...
            if ($prevattach['thumbnail']) {
                delete_uploaded_file(
                    $uploadspath_abs . '/' . $prevattach['thumbnail']
                );
            }
...
function check_parse_php_upload_err($FILE)
{
    ...
    if (
        isset($FILE['error']) &&
        $FILE['error'] != 0 &&
        ($FILE['error'] != UPLOAD_ERR_NO_FILE || $FILE['name'])
    ) {
...
function add_attachments($pid, $forumpermissions, $attachwhere, $action = false)
{
    ...
    if ($forumpermissions['canpostattachments']) {
        ...
        $total = isset($_FILES['attachments']['name'])
        ...
        for ($i = 0; $i < $total; ++$i) {
            ...
            if (
                ...
                $filenames .=
                    $delim . "'" . $db->escape_string($FILE['name']) . "'";
                $delim = ',';
            }
        }
        ...
        if ($filenames != '') {
            $query = $db->simple_select(
                'attachments',
                'filename',
                "{$attachwhere} AND filename IN (" . $filenames . ')'
            );
        ...
        foreach ($attachments as $FILE) {
            ...
            } elseif (!empty($FILE['name']) && !empty($FILE['type'])) {
                if ($FILE['size'] > 0) {
                    ...
                    if (
                        !$exists &&
                        $mybb->get_input('updateattachment') &&
                        $mybb->get_input('updateconfirmed', MyBB::INPUT_INT) !=
                            1
                    ) {
                        $ret['errors'][] = $lang->sprintf(
                            $lang->error_updatefailed,
                            $filename
                        );
                    } else {
                        $attachedfile = upload_attachment(
                            $FILE,
                            $update_attachment
                        );
                        ...
                        if (!empty($attachedfile['error'])) {
                            ...
                            isset($attachedfile['aid']) &&
                            $mybb->get_input('ajax', MyBB::INPUT_INT) == 1
                        ) {
                            $ret['success'][] = [
                                $attachedfile['aid'],
                                get_attachment_icon(get_extension($filename)),
                                htmlspecialchars_uni($filename),
                                get_friendly_size($FILE['size']),
                            ];
                        }
                    }
                } else {
                    $ret['errors'][] = $lang->sprintf(
                        $lang->error_uploadempty,
                        htmlspecialchars_uni($FILE['name'])
                    );
                    $mybb->input['action'] = $action;
                }
            }
