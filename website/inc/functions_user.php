...
function usercp_menu()
{
    ...
    if ($mybb->usergroup['canusercp'] == 1) {
        eval(
            "\$ucp_nav_home = \"" . $templates->get('usercp_nav_home') . "\";"
        );
    }
...
function usercp_menu_messenger()
{
    ...
    if ($mybb->usergroup['cansendpms'] == 1) {
        eval(
            "\$ucp_nav_compose = \"" .
                $templates->get('usercp_nav_messenger_compose') .
                "\";"
        );
    }
    ...
    foreach ($foldersexploded as $key => $folders) {
        ...
        eval(
            "\$folderlinks .= \"" .
                $templates->get('usercp_nav_messenger_folder') .
                "\";"
        );
    }
...
function usercp_menu_profile()
{
    ...
    if ($mybb->usergroup['canchangename'] != 0) {
        eval(
            "\$changenameop = \"" .
                $templates->get('usercp_nav_changename') .
                "\";"
        );
    }
    ...
    if (
        ...
        if (
            ...
            eval(
                "\$changesigop = \"" .
                    $templates->get('usercp_nav_editsignature') .
                    "\";"
            );
        }
    }
...
function usercp_menu_misc()
{
    ...
    if ($count > 0) {
        $draftcount = $lang->sprintf(
            $lang->ucp_nav_drafts_active,
            my_number_format($count)
        );
    }
    ...
    if ($mybb->settings['enableattachments'] != 0) {
        eval(
            "\$attachmentop = \"" .
                $templates->get('usercp_nav_attachments') .
                "\";"
        );
    }
...
function update_pm_count($uid = 0, $count_to_update = 7)
{
    ...
    if (
        $count_to_update & 2 &&
        $db->field_exists('unreadpms', 'users') == true
    ) {
        $query = $db->simple_select(
            'privatemessages',
...
function purgespammer_show($post_count, $usergroup, $uid)
{
    ...
    return $mybb->user['uid'] != $uid &&
        is_member($mybb->settings['purgespammergroups']) &&
        !is_super_admin($uid) &&
        !$usergroups[$usergroup]['cancp'] &&
        !$usergroups[$usergroup]['canmodcp'] &&
        !$usergroups[$usergroup]['issupermod'] &&
        (str_replace($mybb->settings['thousandssep'], '', $post_count) <=
            $mybb->settings['purgespammerpostlimit'] ||
            $mybb->settings['purgespammerpostlimit'] == 0) &&
        !is_member($bangroup, $uid) &&
