...
if (!$settings['internal']['encryption_key']) {
    $cache->update('internal_settings', ['encryption_key' => random_str(32)]);
    $settings['internal'] = $cache->read('internal_settings');
}
...
if (
    !defined('IN_INSTALL') &&
    !defined('IN_UPGRADE') &&
    $version['version_code'] < $mybb->version_code
) {
    $version_history = $cache->read('version_history');
    if (
        empty($version_history) ||
        file_exists(
            MYBB_ROOT .
                'install/resources/upgrade' .
                (int) (end($version_history) + 1) .
                '.php'
        )
    ) {
        $mybb->trigger_generic_error('board_not_upgraded');
...
if (
    ...
    ($mybb->settings['seourls'] == 'auto' &&
        isset($_SERVER['SEO_SUPPORT']) &&
    ...
    define(
        'CALENDAR_URL_MONTH',
        'calendar-{calendar}-year-{year}-month-{month}.html'
    );
    define(
        'CALENDAR_URL_DAY',
        'calendar-{calendar}-year-{year}-month-{month}-day-{day}.html'
    );
    ...
    define(
        'CALENDAR_URL_MONTH',
        'calendar.php?calendar={calendar}&year={year}&month={month}'
    );
    define(
        'CALENDAR_URL_DAY',
        'calendar.php?action=dayview&calendar={calendar}&year={year}&month={month}&day={day}'
    );
    define(
        'CALENDAR_URL_WEEK',
        'calendar.php?action=weekview&calendar={calendar}&week={week}'
    );
