...
class SmtpMail extends MailHandler
{
    ...
    function __construct()
    {
        ...
        } elseif (!empty($mybb->settings['smtp_port'])) {
    ...
    function send()
    {
        ...
        if ($this->connected()) {
            if (!$this->send_data('MAIL FROM:<' . $this->from . '>', 250)) {
                $this->fatal_error(
                    'The mail server does not understand the MAIL FROM command. Reason: ' .
                        $this->get_error()
                );
                return false;
            }
            ...
            foreach ($emails as $to) {
                ...
                if (!$this->send_data('RCPT TO:<' . $to . '>', 250)) {
                    $this->fatal_error(
                        'The mail server does not understand the RCPT TO command. Reason: ' .
                            $this->get_error()
                    );
                    return false;
                }
            ...
            if (!$this->send_data('.', 250)) {
                $this->fatal_error(
                    'Mail may not be delivered. Reason: ' . $this->get_error()
                );
            }
    ...
    function connect()
    {
        ...
        if (is_resource($this->connection)) {
            ...
            if (!$data) {
                $this->fatal_error(
                    "The server did not understand the {$helo} command"
                );
                return false;
            }
            ...
            if ($this->use_tls && preg_match('#250( |-)STARTTLS#mi', $data)) {
                if (!$this->send_data('STARTTLS', 220)) {
                    $this->fatal_error(
                        'The server did not understand the STARTTLS command. Reason: ' .
                            $this->get_error()
                    );
                    return false;
                }
                ...
                if (!$data) {
                    $this->fatal_error(
                        'The server did not understand the EHLO command'
                    );
                    return false;
                }
            }
            ...
            $this->fatal_error(
                "Unable to connect to the mail server with the given details. Reason: {$error_number}: {$error_string}"
            );
            return false;
        }
    }
    ...
    function auth($auth_methods)
    {
        ...
        if (in_array('LOGIN', $auth_methods)) {
            if (!$this->send_data('AUTH LOGIN', 334)) {
                ...
                $this->fatal_error(
                    'The SMTP server did not respond correctly to the AUTH LOGIN command'
                );
                return false;
            }
            ...
            if (!$this->send_data(base64_encode($this->username), 334)) {
                $this->fatal_error(
                    'The SMTP server rejected the supplied SMTP username. Reason: ' .
                        $this->get_error()
                );
                return false;
            }
            ...
            if (!$this->send_data(base64_encode($this->password), 235)) {
                $this->fatal_error(
                    'The SMTP server rejected the supplied SMTP password. Reason: ' .
                        $this->get_error()
                );
                return false;
            }
            ...
            if (!$this->send_data('AUTH PLAIN', 334)) {
                ...
                $this->fatal_error(
                    'The SMTP server did not respond correctly to the AUTH PLAIN command'
                );
                return false;
            }
            $auth = base64_encode(
                chr(0) . $this->username . chr(0) . $this->password
            );
            if (!$this->send_data($auth, 235)) {
                $this->fatal_error(
                    'The SMTP server rejected the supplied login username and password. Reason: ' .
                        $this->get_error()
                );
                return false;
            }
            ...
            if (!$data) {
                ...
                $this->fatal_error(
                    'The SMTP server did not respond correctly to the AUTH CRAM-MD5 command'
                );
                return false;
            }
            ...
            $auth = base64_encode(
                $this->username .
                    ' ' .
                    $this->cram_md5_response($this->password, $challenge)
            );
    ...
    function send_data($data, $status_num = false)
    {
        if ($this->connected()) {
            if (fwrite($this->connection, $data . "\r\n")) {
                ...
                $this->fatal_error(
                    'Unable to send the data to the SMTP server'
                );
                return false;
            }
        }
