...
<br />',
        'post' =>
            '<br /><br /><strong>{$lang->hello}:</strong><br />{$messages}',
        'message' => '<br /> - {$message}',
    ];
    ...
    $db->delete_query(
        'settings',
        "gid='{$gid}' AND description='HELLODELETEMARKER'"
    );
...
function hello_uninstall()
{
    ...
    if ($mybb->request_method != 'post') {
        ...
        $page->output_confirm_action(
            'index.php?module=config-plugins&action=deactivate&uninstall=1&plugin=hello',
            $lang->hello_uninstall_message,
            $lang->hello_uninstall
        );
    }
    ...
    $db->delete_query(
        'settings',
        "name IN ('hello_display1','hello_display2')"
    );
...
function hello_index()
{
    ...
    $messages = '';
    $query = $db->simple_select('hello_messages', 'message', '', [
        'order_by' => 'mid',
        'order_dir' => 'DESC',
    ]);
...
function hello_new()
{
    ...
    $db->insert_query('hello_messages', ['message' => $message]);
