...
$plugins->add_hook(
    'xmlhttp_get_multiquoted_end',
    'hidecontent_xmlhttp_multiquote'
);
...
function hidecontent_install()
{
    ...
    $setting0 = [
        ...
        'title' => 'Usergroups',
        'description' =>
            "Enter the user group ID\'s (separated by comma) of the usergroups allowed to use the Hide Content tag. (\'all\' for all usergroups)",
        'optionscode' => 'text',
        'value' => '',
        'disporder' => 1,
        'gid' => $gid,
    ...
    $setting1 = [
        ...
        'description' =>
            "Enter the user group ID\'s (separated by comma) of the usergroups that do not require to pay.",
        'optionscode' => 'text',
        'value' => '',
        'disporder' => 2,
        'gid' => $gid,
    ...
    $setting2 = [
        ...
        'title' => 'Points',
        'description' =>
            "Enter the price to unlock a post\'s hidden content. Leave this set to 0 to disable this unlocking method.",
        'optionscode' => 'text',
        'value' => '',
        'disporder' => 2,
        'gid' => $gid,
    ...
    $setting3 = [
        ...
        'description' =>
            'Enter the % of the total points that are paid to the post author. E.g. 50 for 50%. Only used if the setting above is greater than 0.',
        'optionscode' => 'text',
        'value' => '50',
        'disporder' => 3,
        'gid' => $gid,
    ...
    $setting4 = [
        ...
        'description' =>
            'Select whether or not users can reply to the thread in order to unlock the hidden content within the thread.',
        'optionscode' => 'yesno',
        'value' => '0',
        'disporder' => 4,
        'gid' => $gid,
...
function hidecontent_parse_tags(
    &$post,
    $hide = false,
    $msg = '',
    $strip = false
    ...
    if ($strip === true) {
        ...
        foreach ($matches as $val) {
            $message = str_replace(
                $val[0],
                $lang->hidecontent_stripped_content,
                $message
            );
        }
    ...
    if (
        hidecontent_check_permissions(
            $mybb->settings['hidecontent_usergroups'],
            $user
        ) ||
        $bypass === true
    ) {
        ...
        foreach ($matches as $val) {
            ...
            if ($hide === true) {
                ...
                if (
                    !hidecontent_check_permissions(
                        $mybb->settings['hidecontent_usergroups_bypass'],
                        $mybb->user
                    )
                ) {
                    $message = str_replace(
                        $val[1],
                        $lang->hidecontent_bad_content,
                        $message
                    );
                }
            } else {
                ...
                if (
                    ...
                    $message = str_replace(
                        $val[0],
                        $lang->sprintf(
                            $lang->hidecontent_content_unlocked,
                            $val[1]
                        ),
                        $message
                    );
                    ...
                    if ($mybb->settings['hidecontent_reply'] == 1) {
                        ...
                        if ($haveireplied) {
                            ...
                            // No
                            if (
                                $mybb->settings['hidecontent_reply'] == 1 &&
                                (float) $mybb->settings['hidecontent_points'] >
                                    0
                            ) {
                                $message = str_replace(
                                    $val[0],
                                    $lang->sprintf(
                                        $lang->hidecontent_not_both,
                                ...
                                $message = str_replace(
                                    $val[0],
                                    $lang->hidecontent_not_replied,
                                    $message
                                );
                            } elseif (
                                $mybb->settings['hidecontent_reply'] == 0 &&
                                (float) $mybb->settings['hidecontent_points'] >
                                    0
                            ) {
                                $message = str_replace(
                        ...
                        if (
                            $mybb->settings['hidecontent_reply'] == 1 &&
                            (float) $mybb->settings['hidecontent_points'] > 0
                        ) {
                            $message = str_replace(
                                $val[0],
                                $lang->sprintf(
                                    $lang->hidecontent_not_both,
                                    newpoints_format_points(
                                        $mybb->settings['hidecontent_points']
                                    ),
                                    $mybb->post_code,
                                    $post['pid']
                                ),
...
function hidecontent_highlight($message)
{
    ...
    if (THIS_SCRIPT == 'showthread.php') {
        // Disable highlight for this post if we have hidden content
        $pattern = "#\[hide\](.*?)\[\/hide\](\r\n?|\n?)#si";
...
function hidecontent_misc()
{
    ...
    $q = $db->simple_select(
        'hidecontent',
        '*',
        'uid=' . (int) $mybb->user['uid'] . ' AND pid=' . $post['pid']
    );
    ...
    if (!empty($entry)) {
        ...
        newpoints_addpoints(
            $post['uid'],
            ($points * (float) $mybb->settings['hidecontent_commission']) / 100
        );

        // Insert entry
        $db->insert_query('hidecontent', [
            'uid' => (int) $mybb->user['uid'],
            'pid' => $pid,
        ]);
                ...
                if ($tlid) {
                    ...
                    if ($post['tyl_pnumtyls'] == 0) {
                        ...
                        $db->write_query(
                            'UPDATE ' .
                                TABLE_PREFIX .
                                "users SET tyl_unumptyls=tyl_unumptyls+1 WHERE uid='" .
                                intval($post['uid']) .
                                "'"
                        );
                    }
                    $db->write_query(
                        'UPDATE ' .
                            TABLE_PREFIX .
                            "posts SET tyl_pnumtyls=tyl_pnumtyls+1 WHERE pid='" .
                            intval($post['pid']) .
                            "'"
                    );
                    $db->write_query(
                        'UPDATE ' .
                            TABLE_PREFIX .
                            "threads SET tyl_tnumtyls=tyl_tnumtyls+1 WHERE tid='" .
                            intval($post['tid']) .
                            "'"
                    );
                    $db->write_query(
                        'UPDATE ' .
                            TABLE_PREFIX .
                            "users SET tyl_unumtyls=tyl_unumtyls+1 WHERE uid='" .
                            intval($mybb->user['uid']) .
                            "'"
                    );
                    $db->write_query(
                        'UPDATE ' .
                            TABLE_PREFIX .
                            "users SET tyl_unumrcvtyls=tyl_unumrcvtyls+1 WHERE uid='" .
                            intval($post['uid']) .
                            "'"
                    );
                    $db->write_query(
                        'UPDATE ' .
                            TABLE_PREFIX .
                            $prefix .
                            "stats SET value=value+1 WHERE title='total'"
                    );
                }
            }
        }
        ...
        if (function_exists('quickdonation_xmlhttp')) {
            ...
            $donors[$mybb->user['uid']] = [
                $mybb->user['username'],
                TIME_NOW,
                (float) $mybb->settings['quickdonation_amount'],
            ];
            ...
            $db->update_query(
                'posts',
                ['quickdonation' => $db->escape_string($donors)],
                'pid=' . (int) $post['pid']
            );
        }
    }
    ...
    redirect(
        ...
        $lang->hidecontent_unlocked_content
    );
}
