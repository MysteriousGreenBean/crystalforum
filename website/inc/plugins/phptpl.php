...
function phptpl_info()
{
    return [
        ...
        'description' =>
            'Allows you to use PHP conditionals in templates.  This is the &quot;safe&quot; variant of the <em>PHP and Conditionals in Templates</em> plugin.',
...
class tplcond
{
    public static $if_stack;
    public static $_expr_unesc;
    static function parsetpl(&$ourtpl)
    {
        self::$if_stack = [];
        if (function_exists('preg_replace_callback_array')) {
            $ourtpl = preg_replace_callback_array(
                [
                    '#\<((?:else)?if\s+(.*?)\s+then|else\s*/?|/if)\>#si' => function (
                        $m
                    ) {
                        return tplcond::iif($m[1], $m[2]);
                    },
                    '#\<func (htmlspecialchars|htmlspecialchars_uni|intval|floatval|urlencode|rawurlencode|addslashes|stripslashes|trim|crc32|ltrim|rtrim|chop|md5|nl2br|sha1|strrev|strtoupper|strtolower|my_strtoupper|my_strtolower|alt_trow|get_friendly_size|filesize|strlen|my_strlen|my_wordwrap|random_str|unicode_chr|bin2hex|str_rot13|str_shuffle|strip_tags|ucfirst|ucwords|basename|dirname|unhtmlentities)\>#i' => function (
                        $m
                    ) {
                        return '".' . $m[1] . '("';
                    },
                    ...
                    '#\<template\s+([a-z0-9_ \-+!(),.]+)(\s*/)?\>#i' => function (
                        $m
                    ) {
                    ...
                    '#\<setvar\s+([a-z0-9_\-+!(),.]+)\>(.*?)\</setvar\>#i' => function (
                        $m
                    ) {
                        return '".(($GLOBALS["tplvars"][\'' .
                            $m[1] .
                            '\'] = (' .
                            self::expr_parse($m[2]) .
                            '))?"":"")."';
    ...
    static function is_safe_expression($s)
    {
        ...
        if (
            preg_match(
                '~(?<![a-z0-9_$])(?:new|exit|die|eval|include|include_once|require|require_once|__file__|__dir__|__line__|__function__|__class__|__method__|php_version|php_os|php_sapi|default_include_path|pear_install_dir|pear_extension_dir|php_extension_dir|php_prefix|php_bindir|php_libdir|php_datadir|php_sysconfdir|php_localstatedir|php_config_file_path|php_config_file_scan_dir|php_shlib_suffix|mybb_root)(?![a-z0-9_$])~i',
                $check
            )
        ) {
            return false;
        ...
        if (preg_match('~[\]})]\s*\(~', $check)) {
            return false;
        } // note that this expression may block "{[statement]} ([statement])" type structures; this shouldn't be an issue with template conditionals, but I guess a workaround, if it does eventually be an issue, is to insert something like a "0+" before the bracket to trick the parser
            ...
            $allowed_funcs = array_flip(
                explode(
                    ...
                    str_replace(
                        ...
                        '',
                        @file_get_contents(
                            MYBB_ROOT . 'inc/plugins/phptpl_allowed_funcs.txt'
                        )
                    )
                )
            );
    ...
    static function eval_text($__s)
    {
        ...
        self::parsetpl($__s);
        $__s = preg_replace_callback(
            '~\\\\\\$([a-zA-Z_][a-zA-Z_0-9]*)((-\\>[a-zA-Z_][a-zA-Z_0-9]*|\\[(\'|\\\\"|)[a-zA-Z_ 0-9]+\\4\\])*)~e',
            function ($m) {
                return '$' . $m[1] . strtr($m[2], ['\\"' => "'"]); // rewrite double-quote to single quotes, cos it's faster
            },
            strtr($__s, ['$' => '\\$'])
        );
        return eval('return "' . $__s . '";');
