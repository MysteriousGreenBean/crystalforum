...
function tpref_info()
{
    ...
    if ($mybb->settings['tpref_enable'] == 1) {
        $config =
            '<div style="float: right;"><span style="color:Green; padding: 21px; text-decoration: none;">Plugin Working</span></div>';
    } elseif ($mybb->settings['tpref_enable'] == 0) {
        $config =
            '<div style="float: right;"><span style="color:Red; padding: 21px; text-decoration: none;">Plugin Disabled</span></div>';
    }
    return [
...
function tpref_activate()
{
    ...
    find_replace_templatesets(
        ...
        '#' . preg_quote('{$lastpost_subject}') . '#',
        '{$forum[\'tpicon\']}{$forum[\'tpprefix\']}{$lastpost_subject}',
        0
    );
}
...
function tpref_deactivate()
{
    ...
    $db->delete_query(
        'settings',
        "name IN ('tpref_enable','tpref_prefix','tpref_icon')"
    );
    rebuild_settings();
    require_once MYBB_ROOT . 'inc/adminfunctions_templates.php';
    find_replace_templatesets(
        'forumbit_depth2_forum_lastpost',
        '#' . preg_quote('{$forum[\'tpicon\']}') . '#',
        '',
        0
    );
    find_replace_templatesets(
        'forumbit_depth2_forum_lastpost',
        '#' . preg_quote('{$forum[\'tpprefix\']}') . '#',
        '',
        0
    );
}
...
function forumlist_tprefix(&$_f)
{
    ...
    if (
        $mybb->settings['tpref_enable'] == 0 ||
        ($mybb->settings['tpref_prefix'] == 0 &&
            $mybb->settings['tpref_icon'] == 0)
    ) {
        return false;
    }
    if (!isset($cache->cache['tprefix_cache'])) {
        ...
        $forums = new RecursiveIteratorIterator(
            new RecursiveArrayIterator($fcache)
        );
        ...
        foreach ($forums as $_forum) {
            ...
            if ($forum['fid']) {
                ...
                if (
                    ...
                    $lastpost_data['lastpost'] > $parent_lastpost['lastpost']
                ) {
                    $parent_lastpost = $lastpost_data;
                }
                if (
                    isset($tprefix_cache) &&
                    $lastpost_data['lastposteruid'] > 0
                ) {
                    $tprefix_cache[$forum['fid']]['tpreftid'] =
                        $lastpost_data['lastposttid'];
                    $tprefix_cache[$forum['fid']]['lastpost'] =
                        $lastpost_data['lastpost'];
                    $tprefix_cache[$forum['fid']]['lastposter'] =
        ...
        if (!empty($threads)) {
            ...
            $query = $db->query(
                "SELECT t.tid, t.fid, t.prefix, t.lastposter, t.icon, tp.displaystyle, tp.prefix as prefijo ,i.iid, i.path
			FROM " .
                    TABLE_PREFIX .
                    "threads t
			LEFT JOIN " .
    ...
    $_f['tprefix_lastpost'] =
        $cache->cache['tprefix_cache'][$_f['fid']]['tprefix_fid'];
    if ($mybb->settings['tpref_icon'] == 1) {
        $_f['tpicon'] = $_f['tprefix_lastpost']['tpicon'];
        if (!empty($_f['tpicon'])) {
            $_f[
                'tpicon'
            ] = "<img src=\"{$_f['tpicon']}\" alt=\"Thread icon\" width=\"16\" height=\"16\" />&nbsp;";
        }
    }
