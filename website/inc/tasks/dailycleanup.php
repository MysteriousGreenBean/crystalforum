...
function task_dailycleanup($task)
{
    ...
    $time = [
        ...
        'threadreadcut' =>
            TIME_NOW - ((int) $mybb->settings['threadreadcut']) * 60 * 60 * 24,
        'privatemessages' => TIME_NOW - 60 * 60 * 24 * 7,
        'deleteinvite' =>
            TIME_NOW - ((int) $mybb->settings['deleteinvites']) * 60 * 60 * 24,
    ...
    if ($mybb->settings['threadreadcut'] > 0) {
        $db->delete_query(
            'threadsread',
            "dateline < '" . (int) $time['threadreadcut'] . "'"
        );
        $db->delete_query(
            'forumsread',
            "dateline < '" . (int) $time['threadreadcut'] . "'"
    ...
    $query = $db->simple_select(
        'privatemessages',
        'pmid, uid, folder',
        "deletetime<='" . (int) $time['privatemessages'] . "' AND folder='4'"
    );
    ...
    if ($mybb->settings['deleteinvites'] > 0) {
        $db->delete_query(
            'joinrequests',
            "dateline < '" . (int) $time['deleteinvite'] . "' AND invite='1'"
        );
    }
    ...
    $db->update_query(
        'privatemessages',
        $sql_array,
        "receipt='2' AND folder!='3' AND status!='0' AND readtime < '" .
            (int) $time['stoppmtracking'] .
            "'"
    );
