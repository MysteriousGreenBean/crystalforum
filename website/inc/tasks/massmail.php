...
function task_massmail($task)
{
    ...
    $query = $db->simple_select(
        'massemails',
        '*',
        "senddate <= '" . TIME_NOW . "' AND status IN (1,2)"
    );
    while ($mass_email = $db->fetch_array($query)) {
        ...
        if (
            strpos($mass_email['htmlmessage'], '<br />') === false &&
            strpos($mass_email['htmlmessage'], '<br>') === false
        ) {
        ...
        $query2 = $db->simple_select(
            'users u',
            'u.uid, u.language, u.pmnotify, u.lastactive, u.username, u.email',
            $member_query,
            [
                'limit_start' => $mass_email['sentcount'],
                'limit' => $mass_email['perpage'],
                'order_by' => 'u.uid',
                'order_dir' => 'asc',
            ]
        );
        while ($user = $db->fetch_array($query2)) {
            ...
            foreach ($replacement_fields as $find => $replace) {
                $mass_email['message'] = str_replace(
                    $find,
                    $replace,
                    $mass_email['message']
                );
                $mass_email['htmlmessage'] = str_replace(
                    $find,
                    $replace,
                    $mass_email['htmlmessage']
                );
            }
