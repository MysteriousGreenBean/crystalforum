...
function task_promotions($task)
{
    ...
    while ($promotion = $db->fetch_array($query)) {
        ...
        if (
            in_array('postcount', $requirements) &&
            (int) $promotion['posts'] >= 0 &&
            !empty($promotion['posttype'])
        ) {
        ...
        if (
            in_array('threadcount', $requirements) &&
            (int) $promotion['threads'] >= 0 &&
            !empty($promotion['threadtype'])
        ) {
        ...
        if (
            in_array('reputation', $requirements) &&
            !empty($promotion['reputationtype'])
        ) {
        ...
        if (
            in_array('referrals', $requirements) &&
            (int) $promotion['referrals'] >= 0 &&
            !empty($promotion['referralstype'])
        ) {
        ...
        if (
            in_array('warnings', $requirements) &&
            (int) $promotion['warnings'] >= 0 &&
            !empty($promotion['warningstype'])
        ) {
        ...
        if (
            in_array('timeregistered', $requirements) &&
            (int) $promotion['registered'] > 0 &&
            !empty($promotion['registeredtype'])
        ) {
        ...
        if (
            in_array('timeonline', $requirements) &&
            (int) $promotion['online'] > 0 &&
            !empty($promotion['onlinetype'])
        ) {
        ...
        if (
            !empty($promotion['originalusergroup']) &&
        ...
        while ($user = $db->fetch_array($query2)) {
            ...
            if (count($uids) % 20 == 0) {
                if ($usergroup_select == 'usergroup') {
                    $db->update_query(
                        'users',
                        ['usergroup' => $promotion['newusergroup']],
                        'uid IN(' . implode(',', $uids) . ')'
                    );
                }
