...
function task_usercleanup($task)
{
    ...
    $query = $db->simple_select(
        'users',
        'uid, moderationtime, suspensiontime',
        '(moderationtime!=0 AND moderationtime<' .
            TIME_NOW .
            ') OR (suspensiontime!=0 AND suspensiontime<' .
            TIME_NOW .
            ')'
    );
    ...
    $query = $db->simple_select(
        'users',
        'uid, suspendsigtime',
        "suspendsignature != 0 AND suspendsigtime < '" . TIME_NOW . "'"
    );
    while ($user = $db->fetch_array($query)) {
        if (
            $user['suspendsigtime'] != 0 &&
            $user['suspendsigtime'] < TIME_NOW
        ) {
            $updated_user = [
                'suspendsignature' => 0,
                'suspendsigtime' => 0,
