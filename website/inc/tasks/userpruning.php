...
function task_userpruning($task)
{
    ...
    if ($mybb->settings['enableprunebyposts'] == 1) {
        ...
        foreach ($usergroups as $gid => $usergroup) {
            ...
            if (
                $usergroup['canmodcp'] == 1 ||
                $usergroup['cancp'] == 1 ||
                $usergroup['issupermod'] == 1 ||
                $usergroup['isbannedgroup'] == 1
            ) {
                continue;
            }
            $in_usergroups[] = $gid;
        ...
        $regdate =
            TIME_NOW -
            (int) $mybb->settings['dayspruneregistered'] * 24 * 60 * 60;
        ...
        $query = $db->simple_select(
            'users',
            'uid',
            "regdate<={$regdate} AND postnum<={$prunepostcount} AND usergroup IN({$usergroups})"
        );
        ...
        if ($users && $mybb->settings['prunepostcountall']) {
            $query = $db->simple_select(
                'posts',
                'uid, COUNT(pid) as posts',
                "uid IN ('" . implode("','", $users) . "') AND visible>0",
                ['group_by' => 'uid']
            );
    ...
    if ($mybb->settings['pruneunactived'] == 1) {
        $regdate =
            TIME_NOW -
            (int) $mybb->settings['dayspruneunactivated'] * 24 * 60 * 60;
        $query = $db->simple_select(
            'users',
            'uid',
            "regdate<={$regdate} AND usergroup='5'"
        );
        while ($user = $db->fetch_array($query)) {
