...
$(function () {
    ...
    var NavaT = 0
    var myTimer
    if (typeof lpaname === 'undefined') return false
        ...
        myTimer = setTimeout(function () {
            $.ajax({
                ...
                beforeSend: function () {
                    ...
                    $('div#' + lpaname + 'mod' + lpamyid).html(
                        "<center><img src='images/spinner_big.gif' alt='Retrieving Data'><br>Loading...<br></center>"
                    )
                },
...
;(function ($) {
    ...
    $.fn.avatarep_bg = function (options) {
        ...
        return this.each(function () {
            ...
            var cobj = $('<text text-anchor="middle"></text>')
                .attr({
                    y: '50%',
                    x: '50%',
                    dy: '0.35em',
            ...
            if (settings.color == null) {
                var colorIndex = Math.floor(
                    (c.charCodeAt(0) + settings.seed) % colors.length
                )
            ...
            svg.append(cobj)
            // svg.append(group);
            var svgHtml = window.btoa(
                unescape(
                    encodeURIComponent($('<div>').append(svg.clone()).html())
                )
            )
