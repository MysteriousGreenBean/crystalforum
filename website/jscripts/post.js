var Post = {
    ...
    addAttachments: function () {
        ...
        if (Post.fileInput.prop('files').length) {
            ...
            if (common.length) {
                ...
                MyBB.prompt(
                    lang.update_confirm.replace('{1}', list.outerHTML),
                    {
                        buttons: [
                            { title: yes_confirm, value: true },
                            { title: no_confirm, value: false },
                        ...
                        submit: function (e, v, m, f) {
                            if (v == true) {
                                Post.form.append(
                                    '<input type="hidden" class="temp_input" name="updateconfirmed" value="1" />'
                                )
    ...
    uploadAttachments: function (type) {
        if (use_xmlhttprequest != 1) {
            ...
            Post.form.append(
                '<input type="hidden" class="temp_input" name="' +
                    type +
                    '" value="1" />'
            )
    ...
    regenAttachbuttons: function () {
        var attachButton = $(
            'input[name=newattachment], input[name=updateattachment]'
        )
            .eq(0)
            .clone()
        ...
        if (Post.getAttachments().length < mybb_max_file_uploads) {
            if (!$('input[name=newattachment]').length) {
                var newButton = attachButton
                    .clone()
                    .prop('name', 'newattachment')
                    .prop('value', lang.add_attachment)
                    .prop('tabindex', '13')
                $("input[name='updateattachment']")
                    .after(newButton)
                    .after('&nbsp;')
            }
        } else {
    ...
    checkAttachments: function (e) {
        var submitter =
            $.type(e) === 'undefined' ? '' : e.originalEvent.submitter.name
        ...
        if (
            !file.files.length &&
            (submitter == 'newattachment' || submitter == 'updateattachment')
        ) {
        ...
        if (mybb_max_file_uploads != 0) {
            ...
            var moreAllowed =
                mybb_max_file_uploads - (Post.getAttachments().length - common)
            if (moreAllowed < 0 || (!moreAllowed && file.files.length)) {
                $.jGrowl(
                    lang.error_maxattachpost.replace(
                        '{1}',
                        mybb_max_file_uploads
                    ),
                    { theme: 'jgrowl_error' }
                )
                file.value = ''
                ...
                $.jGrowl(
                    lang.attachment_max_allowed_files.replace(
                        '{1}',
                        moreAllowed - common
                    ),
                    { theme: 'jgrowl_error' }
