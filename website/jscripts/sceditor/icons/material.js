...
!(function (V) {
    ...
    var A = V.dom,
        t = {
            ...
            horizontalrule:
                '<path d="M 3,3 21,3 21,5 3,5 3,3 M 3,7 15,7 15,9 3,9 3,7 m 0,4 18,0 0,4 -18,0 0,-4" />',
        ...
        return {
            create: function (H) {
                return (
                    H in t &&
                        ((M[H] = V.dom.parseHTML(
                            '<svg xmlns="http://www.w3.org/2000/svg" viewbox="1 1 22 22" unselectable="on">' +
                                t[H] +
                                '</svg>'
                        ).firstChild),
                        'color' === H &&
                            (L = M[H].querySelector('.sce-color'))),
                    M[H]
                )
            },
            update: function (H, V) {
                ...
                L &&
                    ...
                    !H &&
                        V &&
                        (M = V.ownerDocument.queryCommandValue('forecolor')),
                    A.css(L, 'fill', M))
            },
            rtl: function (H) {
                var V = M.grip
                V &&
                    ((H = H ? 'scaleX(-1)' : ''),
                    A.css(V, 'transform', H),
                    A.css(V, 'msTransform', H),
                    A.css(V, 'webkitTransform', H))
            },
        }
    }),
        (V.icons.material.icons = t)
})((document, sceditor))
