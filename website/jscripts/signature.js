document.addEventListener("DOMContentLoaded", function() {
    const signatures = document.querySelectorAll('.signature .sig_text, .signature2 .sig_text');
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    signatures.forEach(sig => {
        ...
        if (quoteColorMarker) {
            ...
            signatureHTML = signatureHTML.replace(/\[quote_color=([#a-zA-Z0-9]+)\]/, '');
            sig.innerHTML = signatureHTML;
            const quotes = sig.closest('.sig_shadow').querySelectorAll('.sig_quote');
            quotes.forEach(quote => {
        ...
        // Handle urls color
        const urlsColorMarker = signatureHTML.match(/\[urls_color=([#a-zA-Z0-9]+)\]/);
        if (urlsColorMarker) {
            const urlsColor = urlsColorMarker[1];
            signatureHTML = signatureHTML.replace(/\[urls_color=([#a-zA-Z0-9]+)\]/, '');
            sig.innerHTML = signatureHTML;
            const postWrapper = sig.closest('.post');
        ...
        const imgBorderColorMarker = signatureHTML.match(/\[img_border_color=([#a-zA-Z0-9]+)\]/);
        if (imgBorderColorMarker) {
            const imgBorderColor = imgBorderColorMarker[1];
            signatureHTML = signatureHTML.replace(/\[img_border_color=([#a-zA-Z0-9]+)\]/, '');
            sig.innerHTML = signatureHTML;
        ...
        const shadowBgColorMarker = signatureHTML.match(/\[shadow_bg_color=([#a-zA-Z0-9]+)\]/);
        if (shadowBgColorMarker) {
            const shadowBgColor = shadowBgColorMarker[1];
            signatureHTML = signatureHTML.replace(/\[shadow_bg_color=([#a-zA-Z0-9]+)\]/, '');
            sig.innerHTML = signatureHTML;
            const sigShadow = sig.closest('.sig_shadow');
            if (sigShadow) {
        ...
        const signatureBgImageMarker = signatureHTML.match(/\[signature_bg_image=([^\]]+)\]/);
        if (signatureBgImageMarker) {
            const signatureBgImage = signatureBgImageMarker[1];
            signatureHTML = signatureHTML.replace(/\[signature_bg_image=([^\]]+)\]/, '');
            sig.innerHTML = signatureHTML;
        ...
        const signatureBoxShadowMarker = signatureHTML.match(/\[signature_box_shadow=([^\]]+)\]/);
        if (signatureBoxShadowMarker) {
            const signatureBoxShadow = signatureBoxShadowMarker[1];
            signatureHTML = signatureHTML.replace(/\[signature_box_shadow=([^\]]+)\]/, '');
            sig.innerHTML = signatureHTML;
            const signatureElement = sig.closest('.signature, .signature2');
            if (signatureElement) {
        ...
        const signatureBorderMarker = signatureHTML.match(/\[signature_border=([^\]]+)\]/);
        if (signatureBorderMarker) {
            const signatureBorder = signatureBorderMarker[1];
            signatureHTML = signatureHTML.replace(/\[signature_border=([^\]]+)\]/, '');
            sig.innerHTML = signatureHTML;
            const signatureElement = sig.closest('.signature, .signature2');
            if (signatureElement) {
        ...
        const shadowBoxShadowMarker = signatureHTML.match(/\[shadow_box_shadow=([^\]]+)\]/);
        if (shadowBoxShadowMarker) {
            const shadowBoxShadow = shadowBoxShadowMarker[1];
            signatureHTML = signatureHTML.replace(/\[shadow_box_shadow=([^\]]+)\]/, '');
            sig.innerHTML = signatureHTML;
            const sigShadow = sig.closest('.sig_shadow');
            if (sigShadow) {
        ...
        const shadowBorderMarker = signatureHTML.match(/\[shadow_border=([^\]]+)\]/);
        if (shadowBorderMarker) {
            const shadowBorder = shadowBorderMarker[1];
            signatureHTML = signatureHTML.replace(/\[shadow_border=([^\]]+)\]/, '');
            sig.innerHTML = signatureHTML;
            const sigShadow = sig.closest('.sig_shadow');
            if (sigShadow) {
