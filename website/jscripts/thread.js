var Thread = {
    init: function () {
        $(function () {
            ...
            if (thread_deleted == '1') {
                $(
                    '#quick_reply_form, .new_reply_button, .thread_tools, .inline_rating'
                ).hide()
    ...
    splitToolHandler: function () {
        if (
            $(thread_deleted !== '1' && '#moderator_options_selector')
                .length !== 0
        ) {
            var splitTool = $('#moderator_options_selector').find(
                'option[value=split]'
            )
    ...
    statsOnClick: function (event) {
        ...
        if (userStatsElement.classList.contains('visible')) {
            ...
            userStatsElement.style.maxHeight =
                userStatsElement.scrollHeight + 'px'
