...
$templatelist =
    'managegroup_leaders_bit,managegroup_leaders,postbit_pm,postbit_email,managegroup_user_checkbox,managegroup_user,managegroup_adduser,managegroup_removeusers,managegroup,managegroup_joinrequests_request,managegroup_joinrequests';
$templatelist .=
    ',managegroup_requestnote,managegroup_no_users,multipage,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start';
...
$lang->nav_group_management = $lang->sprintf(
    $lang->nav_group_management,
...
if ($mybb->input['action'] == 'do_add' && $mybb->request_method == 'post') {
    ...
    $mybb->input['action'] == 'do_joinrequests' &&
    $mybb->request_method == 'post'
) {
    // Verify incoming POST request
    verify_post_check($mybb->get_input('my_post_key'));
    ...
    if (is_array($mybb->get_input('request', MyBB::INPUT_ARRAY))) {
        $uidin = [];
        foreach (
            $mybb->get_input('request', MyBB::INPUT_ARRAY)
            as $uid => $what
        ) {
            if ($what == 'accept') {
    ...
    while ($user = $db->fetch_array($query)) {
        ...
        eval(
            "\$users .= \"" .
                $templates->get('managegroup_joinrequests_request') .
                "\";"
        );
    }
    if (!$users) {
    ...
    $lang->join_requests = $lang->sprintf(
        $lang->join_requests_title,
        htmlspecialchars_uni($usergroup['title'])
    );
    ...
    eval(
        "\$joinrequests = \"" .
            $templates->get('managegroup_joinrequests') .
            "\";"
    );
    output_page($joinrequests);
} elseif (
    $mybb->input['action'] == 'do_manageusers' &&
    $mybb->request_method == 'post'
) {
    // Verify incoming POST request
    verify_post_check($mybb->get_input('my_post_key'));
    ...
    $lang->members_of = $lang->sprintf(
        $lang->members_of,
        htmlspecialchars_uni($usergroup['title'])
    );
    $lang->add_member = $lang->sprintf(
        $lang->add_member,
        htmlspecialchars_uni($usergroup['title'])
    );
    $lang->invite_member = $lang->sprintf(
        $lang->invite_member,
        htmlspecialchars_uni($usergroup['title'])
    );
    $joinrequests = '';
    if ($usergroup['type'] == 5) {
        ...
        if ($numrequests['req']) {
            $lang->num_requests_pending = $lang->sprintf(
                $lang->num_requests_pending,
                $numrequests['req']
            );
            eval(
                "\$joinrequests = \"" .
                    $templates->get('managegroup_requestnote') .
                    "\";"
            );
        }
    ...
    if ($db->num_rows($query)) {
        ...
        while ($leader = $db->fetch_array($query)) {
            ...
            $leader_name = format_name(
                $leader['username'],
                $leader['usergroup'],
                $leader['displaygroup']
            );
            $leader_profile_link = build_profile_link(
                $leader_name,
                $leader['uid']
            );
            ...
            eval(
                "\$leaders .= \"" .
                    $templates->get('managegroup_leaders_bit') .
                    "\";"
            );
        }
        ...
        eval(
            "\$group_leaders = \"" .
                $templates->get('managegroup_leaders') .
                "\";"
        );
    }
    ...
    switch ($db->type) {
        ...
        case 'sqlite':
            $query = $db->simple_select(
                'users',
                '*',
                "','||additionalgroups||',' LIKE '%,{$gid},%' OR usergroup='{$gid}'",
                ['order_by' => 'username']
            );
            break;
        default:
            $query = $db->simple_select(
                'users',
                '*',
                "CONCAT(',',additionalgroups,',') LIKE '%,{$gid},%' OR usergroup='{$gid}'",
                ['order_by' => 'username']
            );
    }
    ...
    while ($user = $db->fetch_array($query)) {
        ...
        if ($groupleader['canmanagemembers'] == 1) {
            eval(
                "\$checkbox = \"" .
                    $templates->get('managegroup_user_checkbox') .
                    "\";"
            );
        }
    ...
    if ($groupleader['canmanagemembers'] == 1) {
        eval(
            "\$add_user = \"" . $templates->get('managegroup_adduser') . "\";"
        );
        if ($removeable_count) {
            eval(
                "\$remove_users = \"" .
                    $templates->get('managegroup_removeusers') .
                    "\";"
            );
        }
    }
    ...
    if ($usergroup['type'] == 5 && $groupleader['caninvitemembers'] == 1) {
        eval(
            "\$invite_user = \"" .
                $templates->get('managegroup_inviteuser') .
                "\";"
        );
    }
