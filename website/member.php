...
define(
    'ALLOWABLE_PAGE',
    'register,do_register,login,do_login,logout,lostpw,do_lostpw,activate,resendactivation,do_resendactivation,resetpassword,viewnotes'
);

$nosession['avatar'] = 1;

$templatelist =
    'member_register,member_register_hiddencaptcha,member_register_coppa,member_register_agreement_coppa,member_register_agreement,member_register_customfield,member_register_requiredfields,member_profile_findthreads';
$templatelist .=
    ',member_loggedin_notice,member_profile_away,member_register_regimage,member_register_regimage_recaptcha_invisible,member_register_regimage_nocaptcha,post_captcha_hcaptcha_invisible,post_captcha_hcaptcha,post_captcha_hidden,post_captcha,member_register_referrer';
$templatelist .=
    ',member_profile_email,member_profile_offline,member_profile_reputation,member_profile_warn,member_profile_warninglevel,member_profile_warninglevel_link,member_profile_customfields_field,member_profile_customfields,member_profile_adminoptions_manageban,member_profile_adminoptions,member_profile';
$templatelist .=
    ',member_profile_signature,member_profile_avatar,member_profile_groupimage,member_referrals_link,member_profile_referrals,member_profile_website,member_profile_reputation_vote,member_activate,member_lostpw,member_register_additionalfields';
$templatelist .=
    ',member_profile_modoptions_manageuser,member_profile_modoptions_editprofile,member_profile_modoptions_banuser,member_profile_modoptions_viewnotes,member_profile_modoptions_editnotes,member_profile_modoptions_purgespammer';
$templatelist .=
    ',usercp_profile_profilefields_select_option,usercp_profile_profilefields_multiselect,usercp_profile_profilefields_select,usercp_profile_profilefields_textarea,usercp_profile_profilefields_radio,member_viewnotes';
$templatelist .=
    ',member_register_question,member_register_question_refresh,usercp_options_timezone,usercp_options_timezone_option,usercp_options_language_option,member_profile_customfields_field_multi_item,member_profile_customfields_field_multi';
$templatelist .=
    ',member_profile_contact_fields_google,member_profile_contact_fields_icq,member_profile_contact_fields_skype,member_profile_pm,member_profile_contact_details,member_profile_modoptions_manageban';
$templatelist .=
    ',member_profile_banned_remaining,member_profile_addremove,member_emailuser_guest,member_register_day,usercp_options_tppselect_option,postbit_warninglevel_formatted,member_profile_userstar,member_profile_findposts';
$templatelist .=
    ',usercp_options_tppselect,usercp_options_pppselect,member_resetpassword,member_login,member_profile_online,usercp_options_pppselect_option,postbit_reputation_formatted,member_emailuser,usercp_profile_profilefields_text';
$templatelist .=
    ',member_profile_modoptions_ipaddress,member_profile_modoptions,member_profile_banned,member_register_language,member_resendactivation,usercp_profile_profilefields_checkbox,member_register_password,member_coppa_form';
...
if (
    ...
    if (
        $mybb->settings['betweenregstime'] &&
        $mybb->settings['maxregsbetweentime']
    ) {
        $time = TIME_NOW;
        ...
        $query = $db->simple_select(
            ...
            '*',
            'regip=' .
                $db->escape_binary($session->packedip) .
                " AND regdate > '$datecut'"
        );
        $regcount = $db->num_rows($query);
        if ($regcount >= $mybb->settings['maxregsbetweentime']) {
            $lang->error_alreadyregisteredtime = $lang->sprintf(
                $lang->error_alreadyregisteredtime,
                $regcount,
                $mybb->settings['betweenregstime']
            );
...
if (
    ...
    if ($mybb->settings['regtime'] > 0) {
        ...
        if (isset($mybb->input['regtime'])) {
            ...
            if ($timetook < $mybb->settings['regtime']) {
                ...
                $lang->error_spam_deny_time = $lang->sprintf(
                    $lang->error_spam_deny_time,
                    $mybb->settings['regtime'],
                    $timetook
                );
    ...
    if ($mybb->settings['regtype'] == 'randompass') {
        ...
        if ($password_length < 8) {
            $password_length = min(
                8,
                (int) $mybb->settings['maxpasswordlength']
            );
        }

        $mybb->input['password'] = random_str(
            $password_length,
            $mybb->settings['requirecomplexpasswords']
        );
    ...
    if (
        ...
        $mybb->get_input('coppa', MyBB::INPUT_INT) == 1
    ) {
        $usergroup = 5;
    } else {
        $usergroup = 2;
    ...
    if (isset($mybb->cookies['coppadob'])) {
        list($dob_day, $dob_month, $dob_year) = explode(
            '-',
            $mybb->cookies['coppadob']
        );
        $user['birthday'] = [
            'day' => $dob_day,
            'month' => $dob_month,
    ...
    if ($mybb->settings['enablestopforumspam_on_register']) {
        ...
        try {
            if (
                $stop_forum_spam_checker->is_user_a_spammer(
                    $user['username'],
                    $user['email'],
                    get_ip()
                )
    ...
    if (!empty($errors)) {
        ...
        if ($mybb->get_input('subscriptionmethod', MyBB::INPUT_INT) == 1) {
            ...
            $mybb->get_input('subscriptionmethod', MyBB::INPUT_INT) == 2
        ) {
            $instant_email_subscribe_selected = "selected=\"selected\"";
        } elseif (
            $mybb->get_input('subscriptionmethod', MyBB::INPUT_INT) == 3
        ) {
        ...
        } elseif ($mybb->get_input('dstcorrection', MyBB::INPUT_INT) == 1) {
        ...
        if (
            ...
            empty($mybb->cookies['coppauser'])
        ) {
            // Log them in
            my_setcookie(
                'mybbuser',
                $user_info['uid'] . '_' . $user_info['loginkey'],
                null,
                true,
                'lax'
            );
        ...
        if (!empty($mybb->cookies['coppauser'])) {
            $lang->redirect_registered_coppa_activate = $lang->sprintf(
                $lang->redirect_registered_coppa_activate,
                $mybb->settings['bbname'],
                htmlspecialchars_uni($user_info['username'])
            );
            ...
            $lang->redirect_registered_activation = $lang->sprintf(
                $lang->redirect_registered_activation,
                $mybb->settings['bbname'],
                htmlspecialchars_uni($user_info['username'])
            );
        ...
        } elseif ($mybb->settings['regtype'] == 'randompass') {
            $emailsubject = $lang->sprintf(
                $lang->emailsubject_randompassword,
                $mybb->settings['bbname']
            );
            switch ($mybb->settings['username_method']) {
                case 0:
                    $emailmessage = $lang->sprintf(
                        $lang->email_randompassword,
                        $user['username'],
                        $mybb->settings['bbname'],
                        $user_info['username'],
        ...
        } elseif ($mybb->settings['regtype'] == 'admin') {
            ...
            if (!empty($admingroups)) {
                ...
                while ($recipient = $db->fetch_array($q)) {
                    ...
                    if (!$is_super_admin) {
                        ...
                        require_once MYBB_ROOT .
                            $mybb->config['admin_dir'] .
                            '/modules/user/module_meta.php';
                        if (function_exists('user_admin_permissions')) {
                            // Get admin permissions
                            $adminperms = get_admin_permissions(
                                $recipient['uid']
                            );
                    ...
                    // Load language
                    if (
                        $recipient['language'] != $lang->language &&
                        $lang->language_exists($recipient['language'])
                    ) {
                        $reset_lang = true;
                    ...
                    $subject = $lang->sprintf(
                        $lang->newregistration_subject,
                        $mybb->settings['bbname']
                    );
                    $message = $lang->sprintf(
                        $lang->newregistration_message,
                        $recipient['username'],
                        $mybb->settings['bbname'],
                        $user['username']
                    );
            ...
            $lang->redirect_registered_admin_activate = $lang->sprintf(
                $lang->redirect_registered_admin_activate,
                $mybb->settings['bbname'],
                htmlspecialchars_uni($user_info['username'])
            );
        ...
        } elseif ($mybb->settings['regtype'] == 'both') {
            $groups = $cache->read('usergroups');
            $admingroups = [];
            if (!empty($groups)) {
                // Shouldn't be...
            ...
            $lang->redirect_registered = $lang->sprintf(
                $lang->redirect_registered,
                $mybb->settings['bbname'],
                htmlspecialchars_uni($user_info['username'])
            );
...
if ($mybb->input['action'] == 'register') {
    ...
    for ($day = 1; $day <= 31; ++$day) {
        ...
        eval(
            "\$bdaysel .= \"" . $templates->get('member_register_day') . "\";"
        );
    }
    ...
    if (
        ...
        if ($mybb->input['bday1'] && $mybb->input['bday2'] && $birthday_year) {
            ...
            my_setcookie(
                'coppadob',
                "{$mybb->input['bday1']}-{$mybb->input['bday2']}-{$birthday_year}",
                -1
            );
            ...
            if (
                $bdaytime >=
                mktime(0, 0, 0, my_date('n'), my_date('d'), my_date('Y') - 13)
            ) {
        ...
        // Show DOB select form
            ...
            $coppa_desc =
                $mybb->settings['coppa'] == 'deny'
                    ? $lang->coppa_desc_for_deny
                    : $lang->coppa_desc;
            eval(
                "\$coppa = \"" .
                    $templates->get('member_register_coppa') .
                    "\";"
            );
            output_page($coppa);
            exit();
    ...
    if (
        ...
        if (
            $mybb->settings['coppa'] != 'disabled' &&
            (!empty($mybb->cookies['coppauser']) || $under_thirteen)
        ) {
            if ($mybb->settings['coppa'] == 'deny') {
                error($lang->error_need_to_be_thirteen);
            }
            $lang->coppa_agreement_1 = $lang->sprintf(
                $lang->coppa_agreement_1,
                $mybb->settings['bbname']
            );
            eval(
                "\$coppa_agreement = \"" .
                    $templates->get('member_register_agreement_coppa') .
                    "\";"
            );
        }
        ...
        eval(
            "\$agreement = \"" .
                $templates->get('member_register_agreement') .
                "\";"
        );
        ...
        // JS validator extra
        if (
            $mybb->settings['maxnamelength'] > 0 &&
            $mybb->settings['minnamelength'] > 0
        ) {
            $lang->js_validator_username_length = $lang->sprintf(
                $lang->js_validator_username_length,
                $mybb->settings['minnamelength'],
                $mybb->settings['maxnamelength']
            );
        }
        ...
        if ($mybb->settings['usereferrals'] == 1 && !$mybb->user['uid']) {
            if (isset($mybb->cookies['mybb']['referrer'])) {
                $query = $db->simple_select(
                    'users',
                    'uid,username',
                    "uid='" . (int) $mybb->cookies['mybb']['referrer'] . "'"
                );
            ...
            eval(
                "\$referrer = \"" .
                    $templates->get('member_register_referrer') .
                    "\";"
            );
        } else {
            $referrer = '';
        ...
        if (is_array($pfcache)) {
            ...
            foreach ($pfcache as $profilefield) {
                ...
                $code = $select = $val = $options = $expoptions = $useropts =
                    '';
                $seloptions = [];
                $profilefield['type'] = htmlspecialchars_uni(
                    $profilefield['type']
                );
                $thing = explode("\n", $profilefield['type'], 2);
                $type = trim($thing[0]);
                ...
                $profilefield['name'] = htmlspecialchars_uni(
                    $profilefield['name']
                );
                if (
                    !empty($errors) &&
                    isset($mybb->input['profile_fields'][$field])
                ) {
                ...
                if ($profilefield['required'] == 1) {
                    ...
                    eval(
                        "\$requiredfields .= \"" .
                            $templates->get('member_register_customfield') .
                            "\";"
                    );
                } else {
                    eval(
            ...
            if ($requiredfields) {
                eval(
                    "\$requiredfields = \"" .
                        $templates->get('member_register_requiredfields') .
                        "\";"
                );
            }
            ...
            if ($customfields) {
                eval(
                    "\$customfields = \"" .
                        $templates->get('member_register_additionalfields') .
                        "\";"
                );
            }
        }
        ...
        if ($mybb->settings['securityquestion']) {
            ...
            if ($db->num_rows($query) > 0) {
                ...
                //Parse question
                $question['question'] = $parser->parse_message(
                    $question['question'],
                    $parser_options
                );
                ...
                eval(
                    "\$questionbox = \"" .
                        $templates->get('member_register_question') .
                        "\";"
                );
            }
        }
        ...
        if ($mybb->settings['hiddencaptchaimage']) {
            ...
            eval(
                "\$hiddencaptcha = \"" .
                    $templates->get('member_register_hiddencaptcha') .
                    "\";"
            );
        }
        if ($mybb->settings['regtype'] != 'randompass') {
            // JS validator extra
            $lang->js_validator_password_length = $lang->sprintf(
                $lang->js_validator_password_length,
                $mybb->settings['minpasswordlength']
            );

            // See if the board has "require complex passwords" enabled.
            if ($mybb->settings['requirecomplexpasswords'] == 1) {
                $lang->password = $lang->complex_password = $lang->sprintf(
                    $lang->complex_password,
                    $mybb->settings['minpasswordlength']
                );
            }
            eval(
                "\$passboxes = \"" .
                    $templates->get('member_register_password') .
                    "\";"
            );
        }
        ...
        if (count($languages) > 1) {
            ...
            eval(
                '$boardlanguage = "' .
                    $templates->get('member_register_language') .
                    '";'
            );
        }
...
if ($mybb->input['action'] == 'activate') {
    ...
    if (isset($mybb->input['code']) && $user) {
        $query = $db->simple_select(
            'awaitingactivation',
            '*',
            "uid='" . $user['uid'] . "' AND (type='r' OR type='e' OR type='b')"
        );
        ...
        $db->delete_query(
            'awaitingactivation',
            "uid='" . $user['uid'] . "' AND (type='r' OR type='e')"
        );
        ...
        if ($activation['type'] == 'e') {
            ...
            $db->update_query(
                'awaitingactivation',
                $update,
                "uid='" . $user['uid'] . "' AND type='b'"
            );
            $plugins->run_hooks('member_activate_emailactivated');

            redirect(
                'index.php',
                $lang->redirect_accountactivated_admin,
                '',
                true
            );
        } else {
...
if (
    $mybb->input['action'] == 'do_resendactivation' &&
    $mybb->request_method == 'post'
) {
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "awaitingactivation a ON (a.uid=u.uid AND (a.type='r' OR a.type='b'))
		WHERE u.email='" .
            $db->escape_string($mybb->get_input('email')) .
            "'
	"
    );
...
if ($mybb->input['action'] == 'resetpassword') {
    ...
    if (isset($mybb->input['code']) && $user) {
        ...
        $emailsubject = $lang->sprintf(
            $lang->emailsubject_passwordreset,
            $mybb->settings['bbname']
        );
        $emailmessage = $lang->sprintf(
            $lang->email_passwordreset,
            $username,
            $mybb->settings['bbname'],
            $password
        );
...
if ($mybb->input['action'] == 'do_login' && $mybb->request_method == 'post') {
    ...
    if (
        $mybb->get_input('quick_password') &&
        $mybb->get_input('quick_username')
    ) {
    ...
    if (!$validated) {
        ...
        if (!empty($loginhandler->login_data)) {
            ...
            $user['loginattempts'] =
                (int) $loginhandler->login_data['loginattempts'];
        }
        ...
        if (
            ...
            ((isset($user['loginattempts']) &&
                $user['loginattempts'] >
                    $mybb->settings['failedcaptchalogincount']) ||
                (isset($mybb->cookies['loginattempts']) &&
                    (int) $mybb->cookies['loginattempts'] >
                        $mybb->settings['failedcaptchalogincount']))
        ) {
            $do_captcha = true;
        ...
        if (
            !empty($mybb->input['url']) &&
            my_strpos(basename($mybb->input['url']), 'member.php') === false &&
            !preg_match('#^javascript:#i', $mybb->input['url'])
        ) {
            if (
                (my_strpos(basename($mybb->input['url']), 'newthread.php') !==
                    false ||
                    my_strpos(basename($mybb->input['url']), 'newreply.php') !==
                        false) &&
                my_strpos($mybb->input['url'], '&processed=1') !== false
            ) {
                $mybb->input['url'] = str_replace(
                    '&processed=1',
                    '',
                    $mybb->input['url']
                );
...
if ($mybb->input['action'] == 'login') {
    ...
    if ($mybb->user['uid'] != 0) {
        ...
        $lang->already_logged_in = $lang->sprintf(
            $lang->already_logged_in,
            build_profile_link($mybb->user['username'], $mybb->user['uid'])
        );
        eval(
            "\$member_loggedin_notice = \"" .
                $templates->get('member_loggedin_notice') .
                "\";"
        );
    }
    ...
    if (
        ...
        strpos($_SERVER['HTTP_REFERER'], 'action=login') === false
    ) {
...
if ($mybb->input['action'] == 'logout') {
    ...
    if (
        isset($mybb->input['sid']) &&
        $mybb->get_input('sid') !== $session->sid
    ) {
    ...
    if ($mybb->user['uid']) {
        ...
        $db->shutdown_query(
            'UPDATE ' .
                TABLE_PREFIX .
                "users SET lastvisit='{$time}', lastactive='{$time}' WHERE uid='{$mybb->user['uid']}'"
        );
...
if ($mybb->input['action'] == 'viewnotes') {
    ...
    $user['username'] = htmlspecialchars_uni($user['username']);
    $lang->view_notes_for = $lang->sprintf(
        $lang->view_notes_for,
        $user['username']
    );
    ...
    eval(
        "\$viewnotes = \"" . $templates->get('member_viewnotes', 1, 0) . "\";"
    );
    echo $viewnotes;
    exit();
}
...
if ($mybb->input['action'] == 'profile') {
    ...
    $lang->nav_profile = $lang->sprintf(
        $lang->nav_profile,
        $memprofile['username']
    );
    add_breadcrumb($lang->nav_profile);

    $lang->users_forum_info = $lang->sprintf(
        $lang->users_forum_info,
        $memprofile['username']
    );
    $lang->users_contact_details = $lang->sprintf(
        $lang->users_contact_details,
        $memprofile['username']
    );
    $lang->send_pm = $lang->sprintf($lang->send_pm, $memprofile['username']);
    $lang->away_note = $lang->sprintf(
        $lang->away_note,
        $memprofile['username']
    );
    $lang->users_additional_info = $lang->sprintf(
        $lang->users_additional_info,
        $memprofile['username']
    );
    $lang->users_signature = $lang->sprintf(
        $lang->users_signature,
        $memprofile['username']
    );
    $lang->send_user_email = $lang->sprintf(
        $lang->send_user_email,
        $memprofile['username']
    );

    $useravatar = format_avatar(
        $memprofile['avatar'],
        $memprofile['avatardimensions']
    );
    ...
    if (
        my_validate_url($memprofile['website']) &&
        !is_member($mybb->settings['hidewebsite']) &&
        $memperms['canchangewebsite'] == 1
    ) {
        $memprofile['website'] = htmlspecialchars_uni($memprofile['website']);
        $bgcolor = alt_trow();
        eval(
            "\$website = \"" . $templates->get('member_profile_website') . "\";"
        );
    }
    ...
    if (
        ...
        (my_strpos(
            ',' . $memprofile['ignorelist'] . ',',
            ',' . $mybb->user['uid'] . ','
        ) === false ||
            $mybb->usergroup['cansendemailoverride'] != 0)
    ) {
        $bgcolor = alt_trow();
        eval(
    ...
    if (
        ...
        (($memprofile['receivepms'] != 0 &&
            ...
            $mybb->usergroup['canoverridepm'] == 1)
    ) {
        $bgcolor = alt_trow();
    ...
    foreach (['icq', 'skype', 'google'] as $field) {
        $contact_fields[$field] = '';
        $settingkey = 'allow' . $field . 'field';

        if (
            !empty($memprofile[$field]) &&
            is_member($mybb->settings[$settingkey], [
                'usergroup' => $memprofile['usergroup'],
                'additionalgroups' => $memprofile['additionalgroups'],
            ])
        ) {
    ...
    if ($any_contact_field || $sendemail || $sendpm || $website) {
        eval(
            '$contact_details = "' .
                $templates->get('member_profile_contact_details') .
                '";'
        );
    }
    ...
    if (
        ...
        if (
            ($mybb->user['uid'] != 0 && $mybb->user['showimages'] != 1) ||
            ($mybb->settings['guestimages'] != 1 && $mybb->user['uid'] == 0)
        ) {
            $sig_parser['allow_imgcode'] = 0;
        }

        $memprofile['signature'] = $parser->parse_message(
            $memprofile['signature'],
            $sig_parser
        );
        eval(
            "\$signature = \"" .
                $templates->get('member_profile_signature') .
                "\";"
        );
    }
    ...
    if ($mybb->usergroup['cansearch'] == 1) {
        if (!empty($memprofile['postnum'])) {
            eval(
                "\$findposts = \"" .
                    $templates->get('member_profile_findposts') .
                    "\";"
            );
        }
        if (!empty($memprofile['threadnum'])) {
            eval(
                "\$findthreads = \"" .
                    $templates->get('member_profile_findthreads') .
                    "\";"
            );
        }
    }
    ...
    if ($memprofile['away'] == 1 && $mybb->settings['allowaway'] != 0) {
        ...
        if ($memprofile['away'] == 1) {
            eval(
                "\$awaybit = \"" .
                    $templates->get('member_profile_away') .
                    "\";"
            );
        }
    }
    ...
    $memlocaldate = gmdate(
        $mybb->settings['dateformat'],
        TIME_NOW + $memprofile['timezone'] * 3600
    );
    $memlocaltime = gmdate(
    ...
    if ($memprofile['birthday']) {
        ...
        if ($memprofile['birthdayprivacy'] != 'none') {
            if ($membday[0] && $membday[1] && $membday[2]) {
                $lang->membdayage = $lang->sprintf(
                    $lang->membdayage,
                    get_age($memprofile['birthday'])
                );

                $bdayformat = fix_mktime(
                    $mybb->settings['dateformat'],
                    $membday[2]
                );
                $membday = mktime(
    ...
    if (!empty($memperms['image'])) {
        ...
        eval(
            "\$groupimage = \"" .
                $templates->get('member_profile_groupimage') .
                "\";"
        );
    }
    ...
    if (!empty($starimage)) {
        ...
        for ($i = 0; $i < $stars; ++$i) {
            eval(
                "\$userstars .= \"" .
                    $templates->get('member_profile_userstar', 1, 0) .
                    "\";"
            );
        }
    }
    ...
    $query = $db->simple_select(
        ...
        "uid='$uid' AND time>'{$timesearch}'",
        ['order_by' => 'time', 'order_dir' => 'DESC', 'limit' => 1]
    );
    ...
    if (!empty($last_seen)) {
        ...
        if (
            ...
            if (!empty($session)) {
                ...
                eval(
                    "\$online_status = \"" .
                        $templates->get('member_profile_online') .
                        "\";"
                );
            }
        }
    ...
    if (!isset($online_status)) {
        eval(
            "\$online_status = \"" .
                $templates->get('member_profile_offline') .
                "\";"
        );
    }
    ...
    if ($mybb->settings['usereferrals'] == 1) {
        ...
        eval(
            "\$referrals = \"" .
                $templates->get('member_profile_referrals') .
                "\";"
        );
    }
    ...
    $reputation = '';
    if (
        $memperms['usereputationsystem'] == 1 &&
        ...
        if (
            ...
            $memprofile['uid'] != $mybb->user['uid'] &&
            ($mybb->settings['posrep'] ||
                $mybb->settings['neurep'] ||
                $mybb->settings['negrep'])
        ) {
            eval(
                "\$vote_link = \"" .
                    $templates->get('member_profile_reputation_vote') .
                    "\";"
            );
        }
        ...
        eval(
            "\$reputation = \"" .
                $templates->get('member_profile_reputation') .
                "\";"
        );
    }
    ...
    if (
        ...
        if ($mybb->usergroup['canwarnusers'] != 0) {
            eval(
                "\$warn_user = \"" .
                    $templates->get('member_profile_warn') .
                    "\";"
            );
            eval(
                "\$warning_level = \"" .
            ...
            eval(
                "\$warning_level = \"" .
                    $templates->get('member_profile_warninglevel') .
                    "\";"
            );
        }
    }
        ...
        foreach ($pfcache as $customfield) {
            if (
                ($mybb->usergroup['cancp'] != 1 &&
                    ...
                    !is_member($customfield['viewableby'])) ||
                !$customfield['profile']
            ) {
                continue;
            }
            ...
            if (isset($userfields[$field])) {
                ...
                if (
                    ...
                    $customfieldval = $parser->parse_message(
                        $userfields[$field],
                        $parser_options
                    );
                }
            ...
            if ($customfieldval) {
                $customfield['name'] = htmlspecialchars_uni(
                    $customfield['name']
                );
                eval(
                    "\$customfields .= \"" .
                        $templates->get('member_profile_customfields_field') .
                        "\";"
                );
                $bgcolor = alt_trow();
            }
    ...
    if ($customfields) {
        eval(
            "\$profilefields = \"" .
                $templates->get('member_profile_customfields') .
                "\";"
        );
    }

    $memprofile['postnum'] = my_number_format($memprofile['postnum']);
    $lang->ppd_percent_total = $lang->sprintf(
        $lang->ppd_percent_total,
        my_number_format($ppd),
        $post_percent
    );

    $memprofile['threadnum'] = my_number_format($memprofile['threadnum']);
    $lang->tpd_percent_total = $lang->sprintf(
        $lang->tpd_percent_total,
        my_number_format($tpd),
        $thread_percent
    );

    $formattedname = format_name(
        $memprofile['username'],
    ...
    $bannedbit = '';
    if (
        $memperms['isbannedgroup'] == 1 &&
        $mybb->usergroup['canbanusers'] == 1
    ) {
        ...
        if ($db->num_rows($query)) {
            ...
            if ($memban['reason']) {
                $memban['reason'] = htmlspecialchars_uni(
                    $parser->parse_badwords($memban['reason'])
                );
            } else {
            ...
            eval(
                '$timeremaining = "' .
                    $templates->get('member_profile_banned_remaining') .
                    '";'
            );

            $memban['adminuser'] = build_profile_link(
                htmlspecialchars_uni($memban['adminuser']),
                $memban['admin']
            );
            ...
            eval(
                '$bannedbit = "' .
                    $templates->get('member_profile_banned') .
                    '";'
            );
        } else {
    ...
    if (
        $mybb->usergroup['cancp'] == 1 &&
        $mybb->config['hide_admin_links'] != 1
    ) {
        if ($memperms['isbannedgroup'] == 1) {
            eval(
                "\$adminoptions = \"" .
                    $templates->get('member_profile_adminoptions_manageban') .
                    "\";"
            );
        } else {
            eval(
                "\$adminoptions = \"" .
                    $templates->get('member_profile_adminoptions') .
                    "\";"
            );
        }
    }
    ...
    $modoptions = $viewnotes = $editnotes = $editprofile = $banuser = $manageban = $manageuser =
        '';
    $can_purge_spammer = purgespammer_show(
        $memprofile['postnum'],
        $memprofile['usergroup'],
        $memprofile['uid']
    );
    if ($mybb->usergroup['canmodcp'] == 1 || $can_purge_spammer) {
        if ($mybb->usergroup['canuseipsearch'] == 1) {
            $memprofile['regip'] = my_inet_ntop(
                $db->unescape_binary($memprofile['regip'])
            );
            $memprofile['lastip'] = my_inet_ntop(
                $db->unescape_binary($memprofile['lastip'])
            );

            eval(
                "\$ipaddress = \"" .
                    $templates->get('member_profile_modoptions_ipaddress') .
                    "\";"
            );
        }

        $memprofile['usernotes'] = nl2br(
            htmlspecialchars_uni($memprofile['usernotes'])
        );
        ...
        if (!empty($memprofile['usernotes'])) {
            if (strlen($memprofile['usernotes']) > 100) {
                eval(
                    "\$viewnotes = \"" .
                        $templates->get('member_profile_modoptions_viewnotes') .
                        "\";"
                );
                $memprofile['usernotes'] =
                    my_substr($memprofile['usernotes'], 0, 100) .
                    "... {$viewnotes}";
            }
        } else {
        ...
        if (
            $mybb->usergroup['caneditprofiles'] == 1 &&
            modcp_can_manage_user($memprofile['uid'])
        ) {
            if (modcp_can_manage_user($memprofile['uid'])) {
                ...
                eval(
                    "\$editnotes = \"" .
                        $templates->get('member_profile_modoptions_editnotes') .
                        "\";"
                );
            }
        }
        ...
        if (
            ...
            if (
                ...
                eval(
                    "\$banuser = \"" .
                        $templates->get('member_profile_modoptions_banuser') .
                        "\";"
                );
            }
        }
        ...
        if ($can_purge_spammer) {
            eval(
                "\$purgespammer = \"" .
                    $templates->get('member_profile_modoptions_purgespammer') .
                    "\";"
            );
        }
        ...
        if (
            ...
            eval(
                "\$manageuser = \"" .
                    $templates->get('member_profile_modoptions_manageuser') .
                    "\";"
            );
        }
        ...
        eval(
            "\$modoptions = \"" .
                $templates->get('member_profile_modoptions') .
                "\";"
        );
    }
    ...
    if ($mybb->user['uid'] != $memprofile['uid'] && $mybb->user['uid'] != 0) {
        ...
        if (in_array($uid, $buddy_list)) {
            $add_remove_options = [
                'url' => "usercp.php?action=do_editlists&amp;delete={$uid}&amp;my_post_key={$mybb->post_code}",
                'class' => 'remove_buddy_button',
                'lang' => $lang->remove_from_buddy_list,
            ];
        } else {
            $add_remove_options = [
                'url' =>
                    'usercp.php?action=do_editlists&amp;add_username=' .
                    urlencode($memprofile['username']) .
                    "&amp;my_post_key={$mybb->post_code}",
                'class' => 'add_buddy_button',
                'lang' => $lang->add_to_buddy_list,
            ];
        }
        ...
        if (!in_array($uid, $ignore_list)) {
            eval(
                "\$buddy_options = \"" .
                    $templates->get('member_profile_addremove') .
                    "\";"
            ); // Add/Remove Buddy
        }
        ...
        if (in_array($uid, $ignore_list)) {
            $add_remove_options = [
                'url' => "usercp.php?action=do_editlists&amp;manage=ignored&amp;delete={$uid}&amp;my_post_key={$mybb->post_code}",
                'class' => 'remove_ignore_button',
                'lang' => $lang->remove_from_ignore_list,
            ];
        } else {
            $add_remove_options = [
                'url' =>
                    'usercp.php?action=do_editlists&amp;manage=ignored&amp;add_username=' .
                    urlencode($memprofile['username']) .
                    "&amp;my_post_key={$mybb->post_code}",
                'class' => 'add_ignore_button',
                'lang' => $lang->add_to_ignore_list,
            ];
        }
        ...
        if (
            isset($memperms['canbereported']) &&
            $memperms['canbereported'] == 1
        ) {
            $reportable = true;
            $query = $db->simple_select(
                'reportedcontent',
                'reporters',
                "reportstatus != '1' AND id = '{$memprofile['uid']}' AND type = 'profile'"
            );
            if ($db->num_rows($query)) {
            ...
            if ($reportable) {
                $add_remove_options = [
                    'url' => "javascript:Report.reportUser({$memprofile['uid']});",
                    'class' => 'report_user_button',
                    'lang' => $lang->report_user,
                ];
                eval(
                    "\$report_options = \"" .
                        $templates->get('member_profile_addremove') .
...
if (
    $mybb->input['action'] == 'do_emailuser' &&
    $mybb->request_method == 'post'
) {
    ...
    if ($mybb->usergroup['maxemails'] > 0) {
        ...
        if ($sent_count >= $mybb->usergroup['maxemails']) {
            $lang->error_max_emails_day = $lang->sprintf(
                $lang->error_max_emails_day,
                $mybb->usergroup['maxemails']
            );
    ...
    if ($mybb->usergroup['emailfloodtime'] > 0) {
        ...
        if (isset($last_email['mid'])) {
            $remaining_time =
                $mybb->usergroup['emailfloodtime'] * 60 -
                (TIME_NOW - $last_email['dateline']);

            if ($remaining_time == 1) {
                $lang->error_emailflooding = $lang->sprintf(
                    $lang->error_emailflooding_1_second,
                    $mybb->usergroup['emailfloodtime']
                );
            } elseif ($remaining_time < 60) {
                $lang->error_emailflooding = $lang->sprintf(
                    $lang->error_emailflooding_seconds,
                    $mybb->usergroup['emailfloodtime'],
                    $remaining_time
                );
            } elseif ($remaining_time > 60 && $remaining_time < 120) {
                $lang->error_emailflooding = $lang->sprintf(
                    $lang->error_emailflooding_1_minute,
                    $mybb->usergroup['emailfloodtime']
                );
            } else {
                $remaining_time_minutes = ceil($remaining_time / 60);
                $lang->error_emailflooding = $lang->sprintf(
                    $lang->error_emailflooding_minutes,
                    $mybb->usergroup['emailfloodtime'],
...
if ($mybb->input['action'] == 'emailuser') {
    ...
    if (
        ...
        (my_strpos(
            ...
            $mybb->usergroup['cansendemailoverride'] != 1)
    ) {
        error_no_permission();
    }
    ...
    if ($mybb->user['uid'] == 0) {
        eval(
            "\$from_email = \"" .
                $templates->get('member_emailuser_guest') .
                "\";"
        );
    }
...
if ($mybb->input['action'] == 'referrals') {
    ...
    $lang->nav_referrals = $lang->sprintf(
        $lang->nav_referrals,
        $user['username']
    );
    ...
    if ($referral_count == 0) {
        eval(
            "\$referral_rows = \"" .
                $templates->get('member_no_referrals') .
                "\";"
        );
    } else {
        ...
        $multipage = multipage(
            ...
            $perpage,
            $page,
            "member.php?action=referrals&amp;uid={$uid}"
        );
        ...
        foreach (get_user_referrals($uid, $start, $perpage) as $referral) {
            ...
            eval(
                "\$referral_rows .= \"" .
                    $templates->get('member_referral_row') .
                    "\";"
            );
