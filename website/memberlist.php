...
$templatelist =
    'memberlist,memberlist_search,memberlist_user,memberlist_user_groupimage,memberlist_user_avatar,memberlist_user_userstar,memberlist_search_contact_field,memberlist_referrals,memberlist_referrals_bit';
$templatelist .=
    ',multipage,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start,memberlist_error,memberlist_orderarrow';
...
if ($mybb->get_input('action') == 'search') {
    ...
    $contact_fields = [];
    foreach (['skype', 'google', 'icq'] as $field) {
        $contact_fields[$field] = '';
        $settingkey = 'allow' . $field . 'field';

        if (
            $mybb->settings[$settingkey] != '' &&
            ...
            eval(
                '$contact_fields[\'' .
                    $field .
                    '\'] = "' .
                    $templates->get('memberlist_search_contact_field') .
                    '";'
            );
        }
    }
    ...
    if ($mybb->settings['usereferrals'] == 1) {
        eval(
            "\$referrals_option = \"" .
                $templates->get('memberlist_referrals_option') .
                "\";"
        );
    }
    ...
    if (isset($mybb->input['order'])) {
        ...
        $mybb->input['order'] = strtolower(
            $mybb->settings['default_memberlist_order']
        );
    }
    ...
    if (
        ...
        (!$mybb->input['order'] && $mybb->input['sort'] == 'username')
    ) {
        $sort_order = 'ASC';
    ...
    if (
        $sort_field == 'u.lastactive' &&
        $mybb->usergroup['canviewwolinvis'] == 0
    ) {
        $sort_field =
            'u.invisible ASC, CASE WHEN u.invisible = 1 THEN u.regdate ELSE u.lastactive END';
    }
    ...
    } elseif ($mybb->settings['membersperpage']) {
        $per_page = $mybb->input['perpage'] =
            (int) $mybb->settings['membersperpage'];
    } else {
    ...
    if ($search_username != '') {
        ...
        if ($mybb->get_input('username_match') == 'begins') {
            $search_query .=
                " AND u.username {$like} '" . $username_like_query . "%'";
            $search_url .= '&username_match=begins';
        }
        // Just contains
        elseif ($mybb->get_input('username_match') == 'contains') {
            $search_query .=
                " AND u.username {$like} '%" . $username_like_query . "%'";
    ...
    foreach (['icq', 'google', 'skype'] as $cfield) {
        ...
        if ($mybb->input[$cfield] && $mybb->settings[$csetting] != '') {
            ...
            if ($cfield == 'icq') {
                $search_query .=
                    " AND u.{$cfield} LIKE '%" .
                    (int) $mybb->input[$cfield] .
                    "%'";
            } else {
                $search_query .=
                    " AND u.{$cfield} {$like} '%" .
                    $db->escape_string_like($mybb->input[$cfield]) .
                    "%'";
            }
    ...
    $sorturl = htmlspecialchars_uni(
        "memberlist.php?perpage={$mybb->input['perpage']}{$search_url}"
    );
    $search_url = htmlspecialchars_uni(
        "memberlist.php?sort={$mybb->input['sort']}&order={$mybb->input['order']}&perpage={$mybb->input['perpage']}{$search_url}"
    );
    ...
    eval(
        "\$orderarrow['{$sort}'] = \"" .
            $templates->get('memberlist_orderarrow') .
            "\";"
    );
    ...
    if ($mybb->settings['usereferrals'] == 1) {
        ...
        eval(
            "\$referral_header = \"" .
                $templates->get('memberlist_referrals') .
                "\";"
        );
    }
    ...
    while ($user = $db->fetch_array($query)) {
        ...
        if ($mybb->settings['usereferrals'] == 1) {
            ...
            eval(
                "\$referral_bit = \"" .
                    $templates->get('memberlist_referrals_bit') .
                    "\";"
            );
        }
        ...
        if (!empty($usergroup['image'])) {
            ...
            eval(
                "\$usergroup['groupimage'] = \"" .
                    $templates->get('memberlist_user_groupimage') .
                    "\";"
            );
        }
        ...
        if (!empty($user['starimage']) && isset($user['stars'])) {
            ...
            for ($i = 0; $i < $user['stars']; ++$i) {
                eval(
                    "\$user['userstars'] .= \"" .
                        $templates->get('memberlist_user_userstar', 1, 0) .
                        "\";"
                );
            }
        }
        ...
        $useravatar = format_avatar(
            ...
            my_strtolower($mybb->settings['memberlistmaxavatarsize'])
        );
        eval(
            "\$user['avatar'] = \"" .
                $templates->get('memberlist_user_avatar') .
                "\";"
        );
