...
$templatelist =
    'misc_rules_forum,misc_help_helpdoc,misc_whoposted_poster,misc_whoposted,misc_smilies_popup_smilie,misc_smilies_popup,misc_smilies_popup_empty,misc_smilies_popup_row,multipage_start';
$templatelist .=
    ',misc_buddypopup,misc_buddypopup_user,misc_buddypopup_user_none,misc_buddypopup_user_online,misc_buddypopup_user_offline,misc_buddypopup_user_sendpm,misc_syndication_forumlist';
$templatelist .=
    ',misc_smilies,misc_smilies_smilie,misc_help_section_bit,misc_help_section,misc_help,forumdisplay_password_wrongpass,forumdisplay_password,misc_helpresults,misc_helpresults_bit';
$templatelist .=
    ',multipage,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,misc_whoposted_page';
$templatelist .=
    ',misc_smilies_popup_no_smilies,misc_smilies_no_smilies,misc_syndication,misc_help_search,misc_helpresults_noresults,misc_syndication_forumlist_forum,misc_syndication_feedurl';
...
if (
    ...
    if (
        $mybb->user['uid'] &&
        verify_post_check($mybb->get_input('my_post_key'), true) !== true
    ) {
    ...
    if (isset($mybb->input['fid'])) {
        ...
        if (!isset($mybb->input['ajax'])) {
            redirect(
                get_forum_link($mybb->input['fid']),
                $lang->redirect_markforumread
            );
        } else {
            echo 1;
            exit();
    ...
    if (isset($mybb->input['fid'])) {
        ...
        my_unsetcookie(
            'forumpass[' . $mybb->get_input('fid', MyBB::INPUT_INT) . ']'
        );
    ...
    $mybb->input['action'] == 'do_helpsearch' &&
    $mybb->request_method == 'post'
) {
    ...
    if (
        $mybb->get_input('name', MyBB::INPUT_INT) != 1 &&
        $mybb->get_input('document', MyBB::INPUT_INT) != 1
    ) {
    ...
    redirect(
        "misc.php?action=helpresults&sid={$sid}",
    ...
    $query = $db->simple_select(
        'helpdocs',
        'COUNT(*) AS total',
        'hid IN(' . $db->escape_string($search['querycache']) . ')'
    );
    ...
    $multipage = multipage(
        ...
        $perpage,
        $page,
        "misc.php?action=helpresults&amp;sid='" .
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "helpsections s ON (s.sid=h.sid)
		WHERE h.hid IN(" .
            $db->escape_string($search['querycache']) .
            ") AND h.enabled='1' AND s.enabled='1'
		LIMIT {$start}, {$perpage}
	"
    );
    while ($helpdoc = $db->fetch_array($query)) {
        ...
        if (my_strlen($helpdoc['name']) > 50) {
            $helpdoc['name'] = htmlspecialchars_uni(
                my_substr($helpdoc['name'], 0, 50) . '...'
            );
        } else {
        ...
        $helpdoc['helpdoc'] = $parser->parse_message(
            $helpdoc['document'],
            $parser_options
        );
        ...
        eval(
            "\$helpdoclist .= \"" .
                $templates->get('misc_helpresults_bit') .
                "\";"
        );
    }
    ...
    if ($db->num_rows($query) == 0) {
        eval(
            "\$helpdoclist = \"" .
                $templates->get('misc_helpresults_noresults') .
                "\";"
        );
    }
    ...
    if ($hid) {
        ...
        if ($helpdoc['section'] != 0 && $helpdoc['enabled'] != 0) {
            ...
            if (
                !empty($mybb->input['highlight']) &&
                $helpdoc['usetranslation'] != 1
            ) {
                require_once MYBB_ROOT . 'inc/class_parser.php';
                $parser = new postParser();

                $highlight = $mybb->input['highlight'];
                $helpdoc['name'] = $parser->highlight_message(
                    $helpdoc['name'],
                    $highlight
                );
                $helpdoc['document'] = $parser->highlight_message(
                    $helpdoc['document'],
                    $highlight
                );
            }
            ...
            eval(
                "\$helppage = \"" . $templates->get('misc_help_helpdoc') . "\";"
            );
        ...
        $query = $db->simple_select('helpdocs', '*', '', [
            'order_by' => 'sid, disporder',
        ]);
        while ($helpdoc = $db->fetch_array($query)) {
            $helpdocs[$helpdoc['sid']][$helpdoc['disporder']][
                $helpdoc['hid']
            ] = $helpdoc;
        }
        unset($helpdoc);
        $sections = '';
        $query = $db->simple_select('helpsections', '*', 'enabled != 0', [
            'order_by' => 'disporder',
        ]);
        while ($section = $db->fetch_array($query)) {
            ...
            if (is_array($helpdocs[$section['sid']])) {
                ...
                foreach ($helpdocs[$section['sid']] as $key => $bit) {
                    foreach ($bit as $key => $helpdoc) {
                        if ($helpdoc['enabled'] != 0) {
                            ...
                            eval(
                                "\$helpbits .= \"" .
                                    $templates->get('misc_help_section_bit') .
                                    "\";"
                            );
                        }
                    }
                    ...
                    if (
                        isset($collapsed[$sname]) &&
                        $collapsed[$sname] == 'display: none;'
                    ) {
                ...
                eval(
                    "\$sections .= \"" .
                        $templates->get('misc_help_section') .
                        "\";"
                );
            }
        }
    ...
    if (
        isset($mybb->input['removebuddy']) &&
        verify_post_check($mybb->get_input('my_post_key'))
    ) {
        $buddies = $mybb->user['buddylist'];
        $namesarray = explode(',', $buddies);
        $mybb->input['removebuddy'] = $mybb->get_input(
            'removebuddy',
            MyBB::INPUT_INT
        );
    ...
    if ($mybb->user['buddylist'] != '') {
        ...
        while ($buddy = $db->fetch_array($query)) {
            ...
            $send_pm = '';
            if (
                $mybb->user['receivepms'] != 0 &&
                $buddy['receivepms'] != 0 &&
                $groupscache[$buddy['usergroup']]['canusepms'] != 0
            ) {
                eval(
                    "\$send_pm = \"" .
                        $templates->get('misc_buddypopup_user_sendpm') .
                        "\";"
                );
            }

            if ($buddy['lastactive']) {
                $last_active = $lang->sprintf(
                    $lang->last_active,
                    my_date('relative', $buddy['lastactive'])
                );
            } else {
                $last_active = $lang->sprintf($lang->last_active, $lang->never);
            }

            $buddy['avatar'] = format_avatar(
                $buddy['avatar'],
                $buddy['avatardimensions'],
                '44x44'
            );

            if (
                $buddy['lastactive'] > $timecut &&
                ($buddy['invisible'] == 0 || $mybb->user['usergroup'] == 4) &&
                $buddy['lastvisit'] != $buddy['lastactive']
            ) {
                $bonline_alt = alt_trow();
                eval(
                    "\$buddys['online'] .= \"" .
                        $templates->get('misc_buddypopup_user_online') .
                        "\";"
                );
            } else {
                ...
                eval(
                    "\$buddys['offline'] .= \"" .
                        $templates->get('misc_buddypopup_user_offline') .
                        "\";"
                );
            }
        }
        ...
        if (empty($buddys['online'])) {
            ...
            eval(
                "\$buddys['online'] = \"" .
                    $templates->get('misc_buddypopup_user_none') .
                    "\";"
            );
        }
        ...
        eval(
            "\$buddies = \"" . $templates->get('misc_buddypopup_user') . "\";"
        );
    } else {
        // No buddies? :(
        $colspan = '';
    ...
    if (
        ($thread['visible'] == -1 &&
            !is_moderator($thread['fid'], 'canviewdeleted')) ||
        ($thread['visible'] == 0 &&
            !is_moderator($thread['fid'], 'canviewunapprove')) ||
        $thread['visible'] > 1
    ) {
        error($lang->error_invalidthread);
    }
    ...
    while ($poster = $db->fetch_array($query)) {
        ...
        eval(
            "\$whoposted .= \"" .
                $templates->get('misc_whoposted_poster') .
                "\";"
        );
        $altbg = alt_trow();
    }
    ...
    if ($modal) {
        ...
        eval(
            "\$whoposted = \"" . $templates->get('misc_whoposted_page') . "\";"
        );
    ...
    if (!empty($mybb->input['popup']) && !empty($mybb->input['editor'])) {
        ...
        if (is_array($smilies_cache)) {
            ...
            foreach ($smilies_cache as $smilie) {
                ...
                eval(
                    '$smilie_image = "' . $templates->get('smilie', 1, 0) . '";'
                );
                eval(
                    "\$smile_icons .= \"" .
                        $templates->get('misc_smilies_popup_smilie') .
                        "\";"
                );
                if ($e == 2) {
                    eval(
                        "\$smilies .= \"" .
                            $templates->get('misc_smilies_popup_row') .
                            "\";"
                    );
                    $smile_icons = '';
                    $e = 1;
        ...
        if ($e == 2) {
            eval(
                "\$smilies .= \"" .
                    $templates->get('misc_smilies_popup_empty') .
                    "\";"
            );
        }
        ...
        if (!$smilies) {
            eval(
                "\$smilies = \"" .
                    $templates->get('misc_smilies_popup_no_smilies') .
                    "\";"
            );
        }
        ...
        eval(
            "\$smiliespage = \"" .
                $templates->get('misc_smilies_popup', 1, 0) .
                "\";"
        );
        ...
        if (is_array($smilies_cache)) {
            ...
            foreach ($smilies_cache as $smilie) {
                ...
                eval(
                    "\$smilies .= \"" .
                        $templates->get('misc_smilies_smilie') .
                        "\";"
                );
                $class = alt_trow();
            }
        ...
        if (!$smilies) {
            eval(
                "\$smilies = \"" .
                    $templates->get('misc_smilies_no_smilies') .
                    "\";"
            );
        }
    ...
    eval(
        "\$feedurl = \"" . $templates->get('misc_syndication_feedurl') . "\";"
    );
...
function makesyndicateforums(
    ...
    if (!is_array($forumcache)) {
        ...
        while ($forum = $db->fetch_array($query)) {
            $forumcache[$forum['pid']][$forum['disporder']][
                $forum['fid']
            ] = $forum;
        }
    }
            ...
            foreach ($main as $key => $forum) {
                ...
                if (
                    ...
                    if (!empty($forumcache[$forum['fid']])) {
                        $newdepth = $depth . '&nbsp;&nbsp;&nbsp;&nbsp;';
                        $forumlistbits .= makesyndicateforums(
                            $forum['fid'],
                            '',
                            0,
                            $newdepth
    ...
    if ($addselect) {
        ...
        eval(
            "\$forumlist = \"" .
                $templates->get('misc_syndication_forumlist') .
                "\";"
        );
    }
