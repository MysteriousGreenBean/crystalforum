...
$templatelist =
    'changeuserbox,loginbox,moderation_delayedmoderation_custommodtool,moderation_delayedmodaction_notes,moderation_delayedmoderation_merge,moderation_delayedmoderation_move,moderation_threadnotes';
$templatelist .=
    ',moderation_delayedmoderation,moderation_deletethread,moderation_deletepoll,moderation_mergeposts_post,moderation_viewthreadnotes,moderation_confirmation,moderation_purgespammer,forumjump_bit';
$templatelist .=
    ',moderation_move,moderation_threadnotes_modaction,moderation_threadnotes_delayedmodaction,moderation_getip_modoptions,moderation_getip,moderation_getpmip,moderation_getpmip_modal';
$templatelist .=
    ',moderation_split_post,moderation_inline_deletethreads,moderation_inline_movethreads,moderation_inline_deleteposts,moderation_inline_mergeposts,moderation_threadnotes_modaction_error';
$templatelist .=
    ',moderation_inline_splitposts,forumjump_special,forumjump_advanced,forumdisplay_password_wrongpass,forumdisplay_password,moderation_inline_moveposts,moderation_delayedmodaction_error';
$templatelist .=
    ',moderation_delayedmodaction_notes_thread_single,moderation_delayedmodaction_notes_thread_multiple,moderation_delayedmodaction_notes_forum,moderation_delayedmodaction_notes_new_forum';
$templatelist .=
    ',moderation_delayedmodaction_notes_redirect,moderation_delayedmodaction_notes_merge,moderation_delayedmoderation_thread,moderation_threadnotes_modaction_thread,moderation_getip_modal';
$templatelist .=
    ',moderation_delayedmoderation_date_day,moderation_delayedmoderation_date_month,moderation_threadnotes_modaction_post,moderation_merge,moderation_split,moderation_threadnotes_modaction_forum';
$templatelist .=
    ',moderation_delayedmoderation_openclose,moderation_delayedmoderation_softdeleterestore,moderation_delayedmoderation_delete,moderation_delayedmoderation_stick,moderation_delayedmoderation_approve';
...
if (
    ...
    !in_array($mybb->input['action'], $allowable_moderation_actions)
) {
    error_no_permission();
}
...
switch ($mybb->input['action']) {
    ...
    case 'cancel_delayedmoderation':
        ...
        if ($tid == 0) {
            moderation_redirect(
                get_forum_link($fid),
                $lang->redirect_delayed_moderation_cancelled
            );
        } else {
            moderation_redirect(
    ...
    case 'delayedmoderation':
        ...
        $localized_time_offset =
            $mybb->user['timezone'] * 3600 + $mybb->user['dst'] * 3600;

        if (!$mybb->get_input('date_day', MyBB::INPUT_INT)) {
            $mybb->input['date_day'] = gmdate(
                'd',
                TIME_NOW + $localized_time_offset
            );
        }
        if (!$mybb->get_input('date_month', MyBB::INPUT_INT)) {
            $mybb->input['date_month'] = gmdate(
                'm',
                TIME_NOW + $localized_time_offset
            );
        }
            ...
            switch ($db->type) {
                ...
                case 'sqlite':
                    $query = $db->simple_select(
                        'modtools',
                        'tid, name, `groups`',
                        "(','||forums||',' LIKE '%,$fid,%' OR ','||forums||',' LIKE '%,-1,%' OR forums='') AND type = 't'"
                    );
                    break;
                default:
                    $query = $db->simple_select(
                        'modtools',
                        'tid, name, `groups`',
                        "(CONCAT(',',forums,',') LIKE '%,$fid,%' OR CONCAT(',',forums,',') LIKE '%,-1,%' OR forums='') AND type = 't'"
                    );
            }
            while ($tool = $db->fetch_array($query)) {
                if (is_member($tool['groups'])) {
                    ...
                    eval(
                        "\$customthreadtools .= \"" .
                            $templates->get(
                                'moderation_delayedmoderation_custommodtool'
                            ) .
                            "\";"
                    );
                }
        ...
        $mybb->input['delayedmoderation'] = $mybb->get_input(
            'delayedmoderation',
            MyBB::INPUT_ARRAY
        );

        if (
            $mybb->input['action'] == 'do_delayedmoderation' &&
            $mybb->request_method == 'post'
        ) {
            ...
            if (
                $mybb->input['type'] == 'move' &&
                (!isset($mybb->input['delayedmoderation']['method']) ||
                    !in_array($mybb->input['delayedmoderation']['method'], [
                        'move',
                        'redirect',
                        'copy',
                    ]))
            ...
            if ($mybb->input['type'] == 'move') {
                ...
                if (
                    $newperms['canview'] == 0 ||
                    !is_moderator($newfid, 'canmovetononmodforum')
                ) {
                ...
                if (
                    !$newforum ||
                    $newforum['type'] != 'f' ||
                    ($newforum['type'] == 'f' && $newforum['linkto'] != '')
                ) {
            ...
            if (
                $mybb->input['date_month'] > 12 ||
                $mybb->input['date_month'] < 1
            ) {
                $errors[] = $lang->error_delayedmoderation_invalid_date_month;
            }

            if (
                $mybb->input['date_year'] <
                gmdate('Y', TIME_NOW + $localized_time_offset)
            ) {
            ...
            $rundate =
                gmmktime(
                    ...
                    (int) $date_time[1],
                    date('s', TIME_NOW),
                    $mybb->get_input('date_month', MyBB::INPUT_INT),
                    $mybb->get_input('date_day', MyBB::INPUT_INT),
                    $mybb->get_input('date_year', MyBB::INPUT_INT)
                ) - $localized_time_offset;
            ...
            if (!$errors) {
                ...
                $did = $db->insert_query('delayedmoderation', [
                ...
                $lang->redirect_delayed_moderation_thread = $lang->sprintf(
                    $lang->redirect_delayed_moderation_thread,
                    $rundate_format
                );
                ...
                if (!empty($mybb->input['tid'])) {
                    ...
                    if ($mybb->get_input('inlinetype') == 'search') {
                        moderation_redirect(
                            get_forum_link($fid),
                            $lang->sprintf(
                                $lang->redirect_delayed_moderation_search,
                                $rundate_format
                            )
                        );
                    } else {
                        moderation_redirect(
                            get_forum_link($fid),
                            $lang->sprintf(
                                $lang->redirect_delayed_moderation_forum,
                                $rundate_format
                            )
                        );
                    }
                ...
                $type_selected[$mybb->get_input('type')] =
                    "checked=\"checked\"";
                $method_selected = [
                    'move' => '',
                    'redirect' => '',
                ...
                foreach (
                    ['redirect_expire', 'new_forum', 'subject', 'threadurl']
                    as $value
                ) {
                ...
                $mybb->input['delayedmoderation']['new_forum'] =
                    (int) $mybb->input['delayedmoderation']['new_forum'];
                $mybb->input['delayedmoderation'][
                    'subject'
                ] = htmlspecialchars_uni(
                    $mybb->input['delayedmoderation']['subject']
                );
                $mybb->input['delayedmoderation'][
                    'threadurl'
                ] = htmlspecialchars_uni(
            ...
            $mybb->input['delayedmoderation']['redirect_expire'] = '';
            $mybb->input['delayedmoderation']['subject'] = isset(
                $thread['subject']
            )
                ? $thread['subject']
                : '';
        ...
        while ($tool = $db->fetch_array($query)) {
            $actions['modtool_' . $tool['tid']] = htmlspecialchars_uni(
                $tool['name']
            );
        }
        ...
        if ($tid == 0) {
            ...
            $query = $db->query(
				...
				LEFT JOIN " .
                    TABLE_PREFIX .
                    "users u ON (u.uid=d.uid)
				LEFT JOIN " .
                    TABLE_PREFIX .
                    "forums f ON (f.fid=d.fid)
				WHERE " .
                    $where_statement .
                    "
        ...
        while ($delayedmod = $db->fetch_array($query)) {
            $delayedmod['dateline'] = my_date(
                'normal',
                $delayedmod['delaydateline'],
                '',
                2
            );
            $delayedmod['username'] = htmlspecialchars_uni(
                $delayedmod['username']
            );
            $delayedmod['profilelink'] = build_profile_link(
                $delayedmod['username'],
                $delayedmod['uid']
            );
            ...
            if (strpos($delayedmod['tids'], ',') === false) {
                ...
                $delayed_thread['link'] = get_thread_link(
                    $delayed_thread['tid']
                );
                $delayed_thread['subject'] = htmlspecialchars_uni(
                    $parser->parse_badwords($delayed_thread['subject'])
                );
                eval(
                    "\$info .= \"" .
                        $templates->get(
            ...
            if ($delayedmod['fname']) {
                ...
                $delayedmod['fname'] = htmlspecialchars_uni(
                    $delayedmod['fname']
                );
                eval(
                    "\$info .= \"" .
            ...
            if ($delayedmod['type'] == 'move') {
                $delayedmod['link'] = get_forum_link(
                    $delayedmod['inputs']['new_forum']
                );
                $delayedmod['name'] = htmlspecialchars_uni(
                    $forum_cache[$delayedmod['inputs']['new_forum']]['name']
                );
                eval(
                    "\$info .= \"" .
                        $templates->get(
                ...
                if ($delayedmod['inputs']['method'] == 'redirect') {
                    if ((int) $delayedmod['inputs']['redirect_expire'] == 0) {
                        ...
                        $redirect_expire_bit =
                            (int) $delayedmod['inputs']['redirect_expire'] .
                            " {$lang->days}";
                    }
                ...
                $delayedmod['subject'] = htmlspecialchars_uni(
                    $delayedmod['inputs']['subject']
                );
                $delayedmod['threadurl'] = htmlspecialchars_uni(
                    $delayedmod['inputs']['threadurl']
                );
                eval(
                    "\$info .= \"" .
                        $templates->get(
            ...
            eval(
                "\$delayedmods .= \"" .
                    $templates->get('moderation_delayedmodaction_notes') .
                    "\";"
            );
            $trow = alt_trow();
        }
        if (!$delayedmods) {
            ...
            eval(
                "\$delayedmods = \"" .
                    $templates->get('moderation_delayedmodaction_error') .
                    "\";"
            );
        }
        ...
        if ($mybb->get_input('tid', MyBB::INPUT_INT)) {
            ...
            $delayedmoderation_subject =
                $mybb->input['delayedmoderation']['subject'];
            $delayedmoderation_threadurl =
                $mybb->input['delayedmoderation']['threadurl'];
            eval(
                "\$threads = \"" .
                    $templates->get('moderation_delayedmoderation_thread') .
                    "\";"
            );
            eval(
                "\$moderation_delayedmoderation_merge = \"" .
                    $templates->get('moderation_delayedmoderation_merge') .
                    "\";"
            );
        } else {
            if ($mybb->get_input('inlinetype') == 'search') {
        ...
        for ($day = 1; $day <= 31; ++$day) {
            ...
            eval(
                '$dateday .= "' .
                    $templates->get('moderation_delayedmoderation_date_day') .
                    '";'
            );
        }
        ...
        foreach (
            ...
            $datemonth[$month] = '';
            if (
                $mybb->get_input('date_month', MyBB::INPUT_INT) == (int) $month
            ) {
        ...
        eval(
            '$datemonth = "' .
                $templates->get('moderation_delayedmoderation_date_month') .
                '";'
        );
        ...
        if (is_moderator($fid, 'canopenclosethreads')) {
            eval(
                '$openclosethread = "' .
                    $templates->get('moderation_delayedmoderation_openclose') .
                    '";'
            );
        }
        ...
        if (is_moderator($fid, 'candeletethreads')) {
            eval(
                '$deletethread = "' .
                    $templates->get('moderation_delayedmoderation_delete') .
                    '";'
            );
        }
        ...
        if (is_moderator($fid, 'canstickunstickthreads')) {
            eval(
                '$stickunstickthread = "' .
                    $templates->get('moderation_delayedmoderation_stick') .
                    '";'
            );
        }
        ...
        if (is_moderator($fid, 'canapproveunapprovethreads')) {
            eval(
                '$approveunapprovethread = "' .
                    $templates->get('moderation_delayedmoderation_approve') .
                    '";'
            );
        }
        ...
        eval(
            "\$delayedmoderation = \"" .
                $templates->get('moderation_delayedmoderation') .
                "\";"
        );
    ...
    case 'removeredirects':
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_redirectsremoved
        );
        break;
    ...
    case 'deletethread':
        ...
        if (!is_moderator($fid, 'candeletethreads')) {
            if (
                $permissions['candeletethreads'] != 1 ||
        ...
        eval(
            "\$deletethread = \"" .
                $templates->get('moderation_deletethread') .
                "\";"
        );
    ...
    case 'do_deletethread':
        ...
        $lang->thread_deleted = $lang->sprintf(
            $lang->thread_deleted,
            $thread['subject']
        );
    ...
    case 'deletepoll':
        ...
        eval(
            "\$deletepoll = \"" .
                $templates->get('moderation_deletepoll') .
                "\";"
        );
    ...
    case 'do_deletepoll':
        ...
        $lang->poll_deleted = $lang->sprintf(
            $lang->poll_deleted,
            $thread['subject']
        );
    ...
    case 'approvethread':
        ...
        $lang->thread_approved = $lang->sprintf(
            $lang->thread_approved,
            $thread['subject']
        );
    ...
    case 'unapprovethread':
        ...
        $lang->thread_unapproved = $lang->sprintf(
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_threadunapproved
        );
        break;
    ...
    case 'restorethread':
        ...
        $lang->thread_restored = $lang->sprintf(
            $lang->thread_restored,
            $thread['subject']
        );
    ...
    case 'softdeletethread':
        ...
        $lang->thread_soft_deleted = $lang->sprintf(
            $lang->thread_soft_deleted,
            $thread['subject']
        );
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_threadsoftdeleted
        );
        break;
    ...
    case 'do_move':
        ...
        if (
            !is_moderator($moveto, 'canmanagethreads') &&
            !is_moderator($fid, 'canmovetononmodforum')
        ) {
            error_no_permission();
        }
        ...
        if ($mybb->get_input('redirect_expire', MyBB::INPUT_INT) > 0) {
            $expire =
                TIME_NOW +
                $mybb->get_input('redirect_expire', MyBB::INPUT_INT) * 86400;
        }
        ...
        moderation_redirect(
            get_thread_link($newtid),
            $lang->redirect_threadmoved
        );
        break;
    ...
    case 'viewthreadnotes':
        ...
        eval(
            "\$viewthreadnotes = \"" .
                $templates->get('moderation_viewthreadnotes', 1, 0) .
                "\";"
        );
    ...
    case 'threadnotes':
        ...
        $thread['notes'] = htmlspecialchars_uni(
            $parser->parse_badwords($thread['notes'])
        );
        $trow = alt_trow(1);
        ...
        if (is_moderator($fid, 'canviewmodlog')) {
            ...
            while ($modaction = $db->fetch_array($query)) {
                $modaction['dateline'] = my_date(
                    'relative',
                    $modaction['dateline']
                );
                $modaction['username'] = htmlspecialchars_uni(
                    $modaction['username']
                );
                $modaction['profilelink'] = build_profile_link(
                    $modaction['username'],
                    $modaction['uid']
                );
                $modaction['action'] = htmlspecialchars_uni(
                    $modaction['action']
                );
                $info = '';
                if ($modaction['tsubject']) {
                    $modaction['tsubject'] = htmlspecialchars_uni(
                        $parser->parse_badwords($modaction['tsubject'])
                    );
                    $modaction['threadlink'] = get_thread_link(
                        $modaction['tid']
                    );
                    eval(
                        "\$info .= \"" .
                ...
                if ($modaction['fname']) {
                    $modaction['fname'] = htmlspecialchars_uni(
                        $modaction['fname']
                    );
                ...
                if ($modaction['psubject']) {
                    $modaction['psubject'] = htmlspecialchars_uni(
                        $parser->parse_badwords($modaction['psubject'])
                    );
                ...
                eval(
                    "\$modactions .= \"" .
                        $templates->get('moderation_threadnotes_modaction') .
                        "\";"
                );
                $trow = alt_trow();
            }
            if (!$modactions) {
                eval(
                    "\$modactions = \"" .
                        $templates->get(
                            'moderation_threadnotes_modaction_error'
                        ) .
                        "\";"
                );
            }
        ...
        while ($delayedmod = $db->fetch_array($query)) {
            ...
            eval(
                "\$delayedmods .= \"" .
                    $templates->get('moderation_threadnotes_delayedmodaction') .
                    "\";"
            );
            $trow = alt_trow();
        }
        ...
        eval(
            "\$threadnotes = \"" .
                $templates->get('moderation_threadnotes') .
                "\";"
        );
    ...
    case 'do_threadnotes':
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_threadnotesupdated
        );
        break;
    ...
    case 'getip':
        ...
        $post['ipaddress'] = my_inet_ntop(
            $db->unescape_binary($post['ipaddress'])
        );
        ...
        if (
            ...
            $mybb->usergroup['canuseipsearch'] == 1
        ) {
            $ipaddress = $post['ipaddress'];
            eval(
                "\$modoptions = \"" .
                    $templates->get('moderation_getip_modoptions') .
                    "\";"
            );
        }
        ...
        if ($modal) {
            eval(
                "\$getip = \"" .
                    $templates->get('moderation_getip_modal', 1, 0) .
                    "\";"
            );
            echo $getip;
            exit();
    ...
    case 'getpmip':
        ...
        $pm['subject'] = htmlspecialchars_uni(
            $parser->parse_badwords($pm['subject'])
        );
        add_breadcrumb(
            $pm['subject'],
            "private.php?action=read&amp;pmid={$pmid}"
        );
        ...
        $name = $db->fetch_field(
            $db->simple_select('users', 'username', "uid = {$pm['fromid']}"),
            'username'
        );
        ...
        if ($modal) {
            eval(
                "\$getpmip = \"" .
                    $templates->get('moderation_getpmip_modal', 1, 0) .
                    "\";"
            );
            echo $getpmip;
            exit();
            ...
            eval(
                "\$getpmip = \"" . $templates->get('moderation_getpmip') . "\";"
            );
            output_page($getpmip);
            break;
        }
    ...
    case 'split':
        ...
        while ($post = $db->fetch_array($query)) {
            ...
            eval(
                "\$posts .= \"" .
                    $templates->get('moderation_split_post') .
                    "\";"
            );
            $altbg = alt_trow();
        }
    ...
    case 'do_split':
        ...
        $mybb->input['splitpost'] = $mybb->get_input(
            'splitpost',
            MyBB::INPUT_ARRAY
        );
        ...
        while ($post = $db->fetch_array($query)) {
            if (
                isset($mybb->input['splitpost'][$post['pid']]) &&
                $mybb->input['splitpost'][$post['pid']] == 1
            ) {
                $pids[] = $post['pid'];
            }
        ...
        $newtid = $moderation->split_posts(
            $pids,
            $tid,
            $moveto,
            $mybb->get_input('newsubject')
        );
    ...
    case 'removesubscriptions':
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_removed_subscriptions
        );
        break;
    ...
    case 'multideletethreads':
        ...
        if ($mybb->get_input('inlinetype') == 'search') {
            clearinline(
                $mybb->get_input('searchid', MyBB::INPUT_INT),
                'search'
            );
        } else {
        ...
        eval(
            "\$multidelete = \"" .
                $templates->get('moderation_inline_deletethreads') .
                "\";"
        );
    ...
    case 'do_multideletethreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadsdeleted
        );
        break;
    ...
    case 'multiopenthreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadsopened
        );
        break;
    ...
    case 'multiclosethreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadsclosed
        );
        break;
    ...
    case 'multiapprovethreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadsapproved
        );
        break;
    ...
    case 'multiunapprovethreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadsunapproved
        );
        break;
    ...
    case 'multirestorethreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadsrestored
        );
        break;
    ...
    case 'multisoftdeletethreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadssoftdeleted
        );
        break;
    ...
    case 'multistickthreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadsstuck
        );
        break;
    ...
    case 'multiunstickthreads':
        ...
        moderation_redirect(
            get_forum_link($fid),
            $lang->redirect_inline_threadsunstuck
        );
        break;
    ...
    case 'multimovethreads':
        ...
        eval(
            "\$movethread = \"" .
                $templates->get('moderation_inline_movethreads') .
                "\";"
        );
        output_page($movethread);
        break;
    ...
    case 'do_multimovethreads':
        ...
        if (
            ($newperms['canview'] == 0 ||
                !is_moderator($moveto, 'canmanagethreads')) &&
            !is_moderator_by_tids($tids, 'canmovetononmodforum')
        ) {
            error($lang->error_movetononmodforum, $lang->error);
        }
        ...
        moderation_redirect(
            get_forum_link($moveto),
            $lang->redirect_inline_threadsmoved
        );
        break;
    ...
    case 'multideleteposts':
        ...
        eval(
            "\$multidelete = \"" .
                $templates->get('moderation_inline_deleteposts') .
                "\";"
        );
        output_page($multidelete);
        break;
    ...
    case 'do_multideleteposts':
        ...
        $lang->deleted_selective_posts = $lang->sprintf(
            $lang->deleted_selective_posts,
            $deletecount
        );
    ...
    case 'multimergeposts':
        ...
        foreach ($mybb->cookies as $key => $value) {
            if (
                strpos($key, 'inlinemod_thread') !== false &&
                $key != "inlinemod_thread$tid"
            ) {
        ...
        while ($post = $db->fetch_array($query)) {
            ...
            eval(
                "\$postlist .= \"" .
                    $templates->get('moderation_mergeposts_post') .
                    "\";"
            );
            $altbg = alt_trow();
        }
        ...
        eval(
            "\$multimerge = \"" .
                $templates->get('moderation_inline_mergeposts') .
                "\";"
        );
    ...
    case 'do_multimergeposts':
        ...
        $masterpid = $moderation->merge_posts(
            $postlist,
            $tid,
            $mybb->input['sep']
        );
        ...
        moderation_redirect(
            get_post_link($masterpid) . "#pid$masterpid",
            $lang->redirect_inline_postsmerged
        );
        break;
    ...
    case 'multisplitposts':
        ...
        eval(
            "\$splitposts = \"" .
                $templates->get('moderation_inline_splitposts') .
                "\";"
        );
    ...
    case 'do_multisplitposts':
        ...
        $lang->split_selective_posts = $lang->sprintf(
            $lang->split_selective_posts,
            $pid_list,
            $newtid
        );
    ...
    case 'multimoveposts':
        ...
        eval(
            "\$moveposts = \"" .
                $templates->get('moderation_inline_moveposts') .
                "\";"
        );
    ...
    case 'do_multimoveposts':
        ...
        $newtid = $moderation->split_posts(
            ...
            $tid,
            $newthread['fid'],
            $db->escape_string($newthread['subject']),
            $newtid
        );

        $pid_list = implode(', ', $posts);
        $lang->move_selective_posts = $lang->sprintf(
            $lang->move_selective_posts,
            $pid_list,
            $newtid
        );
    ...
    case 'multiapproveposts':
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_inline_postsapproved
        );
        break;
    ...
    case 'multiunapproveposts':
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_inline_postsunapproved
        );
        break;
    ...
    case 'multirestoreposts':
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_inline_postsrestored
        );
        break;
    ...
    case 'multisoftdeleteposts':
        ...
        moderation_redirect(
            get_thread_link($thread['tid']),
            $lang->redirect_inline_postssoftdeleted
        );
        break;
    ...
    case 'purgespammer':
        ...
        if ($mybb->input['action'] == 'do_purgespammer') {
            ...
            if ($mybb->settings['purgespammerbandelete'] == 'ban') {
                ...
                // Clear the profile
                $userhandler->clear_profile(
                    $uid,
                    $mybb->settings['purgespammerbangroup']
                );
            ...
            if (!empty($mybb->settings['purgespammerapikey'])) {
                $sfs = @fetch_remote_file(
                    'http://stopforumspam.com/add.php?username=' .
                        ...
                        '&api_key=' .
                        urlencode($mybb->settings['purgespammerapikey'])
                );
            }

            log_moderator_action(
                ['uid' => $uid, 'username' => $user['username']],
                $lang->purgespammer_modlog
            );
            ...
            $lang->purgespammer_purge = $lang->sprintf(
                $lang->purgespammer_purge,
                htmlspecialchars_uni($user['username'])
            );
            if ($mybb->settings['purgespammerbandelete'] == 'ban') {
                $lang->purgespammer_purge_desc = $lang->sprintf(
                    $lang->purgespammer_purge_desc,
                    $lang->purgespammer_ban
                );
            } else {
                $lang->purgespammer_purge_desc = $lang->sprintf(
                    $lang->purgespammer_purge_desc,
                    $lang->purgespammer_delete
                );
            }
            eval(
                "\$purgespammer = \"" .
                    $templates->get('moderation_purgespammer') .
                    "\";"
            );
    ...
    default:
        ...
        $tool = $custommod->tool_info(
            $mybb->get_input('action', MyBB::INPUT_INT)
        );
        if ($tool !== false) {
            // Verify incoming POST request
            verify_post_check($mybb->get_input('my_post_key'));
            ...
            if (
                !empty($options['confirmation']) &&
                empty($mybb->input['confirm'])
            ) {
                add_breadcrumb($lang->confirm_execute_tool);

                $lang->confirm_execute_tool_desc = $lang->sprintf(
                    $lang->confirm_execute_tool_desc,
                    htmlspecialchars_uni($tool['name'])
                );

                $action = $mybb->get_input('action', MyBB::INPUT_INT);
                $modtype = htmlspecialchars_uni($mybb->get_input('modtype'));
                $inlinetype = htmlspecialchars_uni(
                    $mybb->get_input('inlinetype')
                );
                ...
                eval(
                    '$page = "' .
                        $templates->get('moderation_confirmation') .
                        '";'
                );
            ...
            if (
                $tool['type'] == 't' &&
                $mybb->get_input('modtype') == 'inlinethread'
            ) {
                if ($mybb->get_input('inlinetype') == 'search') {
                    $tids = getids($mybb->get_input('searchid'), 'search');
                } else {
                ...
                if (
                    $thread_options['movethread'] &&
                    $forum_cache[$thread_options['movethread']]['type'] != 'f'
                ) {
                ...
                if ($mybb->get_input('inlinetype') == 'search') {
                    ...
                    $lang->redirect_customtool_search = $lang->sprintf(
                    ...
                    clearinline($fid, 'forum');
                    $lang->redirect_customtool_forum = $lang->sprintf(
                        $lang->redirect_customtool_forum,
                        $tool['name']
                    );
                    redirect(
                        get_forum_link($fid),
                ...
                if ($ret == 'forum') {
                    ...
                    $lang->redirect_customtool_thread = $lang->sprintf(
                        $lang->redirect_customtool_thread,
                        $tool['name']
                    );
                    moderation_redirect(
                        get_thread_link($thread['tid']),
                ...
                $tool['type'] == 'p' &&
                $mybb->get_input('modtype') == 'inlinepost'
            ) {
                if ($mybb->get_input('inlinetype') == 'search') {
