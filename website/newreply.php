...
$templatelist =
    'newreply,previewpost,loginbox,changeuserbox,posticons,newreply_threadreview,newreply_threadreview_post,forumdisplay_rules_link,newreply_multiquote_external,post_attachments_add,post_subscription_method';
$templatelist .=
    ',codebuttons,post_attachments_new,post_attachments,post_savedraftbutton,newreply_modoptions,newreply_threadreview_more,postbit_online,postbit_pm,newreply_disablesmilies_hidden,post_attachments_update';
$templatelist .=
    ',postbit_warninglevel,postbit_author_user,postbit_edit,postbit_quickdelete,postbit_inlinecheck,postbit_posturl,postbit_quote,postbit_multiquote,newreply_modoptions_close,newreply_modoptions_stick';
$templatelist .=
    ',post_attachments_attachment_postinsert,post_attachments_attachment_remove,post_attachments_attachment_unapproved,post_attachments_attachment,post_attachments_viewlink,postbit_attachments_attachment,newreply_signature';
$templatelist .=
    ',post_captcha_recaptcha_invisible,post_captcha_hidden,post_captcha,post_captcha_nocaptcha,post_captcha_hcaptcha_invisible,post_captcha_hcaptcha,post_javascript,postbit_groupimage,postbit_attachments,newreply_postoptions';
$templatelist .=
    ',postbit_rep_button,postbit_author_guest,postbit_signature,postbit_attachments_thumbnails_thumbnailpostbit_attachments_images_image,postbit_attachments_attachment_unapproved';
$templatelist .=
    ',postbit_attachments_thumbnails,postbit_attachments_images,postbit_gotopost,forumdisplay_password_wrongpass,forumdisplay_password,posticons_icon,attachment_icon,postbit_reputation_formatted_link';
$templatelist .=
    ',global_moderation_notice,newreply_disablesmilies,postbit_userstar,newreply_draftinput,postbit_avatar,forumdisplay_rules,postbit_offline,postbit_find,postbit_warninglevel_formatted,postbit_ignored';
$templatelist .=
    ',postbit_profilefield_multiselect_value,postbit_profilefield_multiselect,postbit_reputation,postbit_www,postbit_away,postbit_icon,postbit_email,postbit_report,postbit,postbit_warn';
...
if (
    ...
    $tid = (int) $post['tid'];
    eval(
        "\$editdraftpid = \"" . $templates->get('newreply_draftinput') . "\";"
    );
}
...
if (
    isset($post) &&
    (($post['visible'] == 0 && !is_moderator($fid, 'canviewunapprove')) ||
        ($post['visible'] < 0 && $post['uid'] != $mybb->user['uid']))
) {
    if (
        $post['visible'] == 0 &&
        !(
...
if ($mybb->user['suspendposting'] == 1) {
    ...
    if ($mybb->user['suspensiontime']) {
        $suspendedpostingtype = $lang->sprintf(
            $lang->error_suspendedposting_temporal,
            my_date(
                $mybb->settings['dateformat'],
                $mybb->user['suspensiontime']
            )
        );
    }

    $lang->error_suspendedposting = $lang->sprintf(
        $lang->error_suspendedposting,
        $suspendedpostingtype,
        my_date($mybb->settings['timeformat'], $mybb->user['suspensiontime'])
    );
...
if (
    isset($forumpermissions['canonlyreplyownthreads']) &&
...
if (
    $mybb->get_input('method') == 'quickreply' &&
    !isset($mybb->input['previewpost'])
) {
...
if (!$mybb->get_input('posthash') && !$pid) {
    $mybb->input['posthash'] = md5(
        $thread['tid'] . $mybb->user['uid'] . random_str()
    );
}
...
if (
    empty($_POST) &&
    empty($_FILES) &&
    $mybb->get_input('processed', MyBB::INPUT_INT) == 1
) {
...
if (
    ...
    if ($pid) {
        ...
        $attachwhere =
            "posthash='" .
            $db->escape_string($mybb->get_input('posthash')) .
            "'";
    }
    ...
    if ($mybb->get_input('ajax', MyBB::INPUT_INT) == 1) {
        if (isset($ret['success'])) {
            ...
            eval(
                "\$attach_rem_options = \"" .
                    $templates->get('post_attachments_attachment_remove') .
                    "\";"
            );
            $attach_mod_options = '';
            eval(
    ...
    if (!$mybb->get_input('submit') && !$mybb->get_input('savedraft')) {
        eval(
            "\$editdraftpid = \"" .
                $templates->get('newreply_draftinput') .
                "\";"
        );
        $mybb->input['action'] = 'newreply';
    }
...
if (
    ...
    if ($mybb->get_input('ajax', MyBB::INPUT_INT) == 1) {
        ...
        echo json_encode([
            'success' => true,
            'usage' => get_friendly_size($usage['ausage']),
        ]);
        exit();
    }
}
...
if ($mybb->usergroup['maxposts'] > 0) {
    ...
    if ($post_count >= $mybb->usergroup['maxposts']) {
        $lang->error_maxposts = $lang->sprintf(
            $lang->error_maxposts,
            $mybb->usergroup['maxposts']
        );
...
if (
    ...
    if ($mybb->settings['captchaimage'] && !$mybb->user['uid']) {
        ...
        if (
            $mybb->get_input('ajax', MyBB::INPUT_INT) &&
            $post_captcha->type == 1
        ) {
    ...
    if (count($post_errors) > 0) {
        ...
        if ($visible == -2) {
            ...
            // Moderated post
            $lang->redirect_newreply .=
                '<br />' . $lang->redirect_newreply_moderation;
        ...
        if (
            isset($mybb->input['quoted_ids']) &&
            isset($mybb->cookies['multiquote']) &&
        ...
        if ($mybb->get_input('ajax', MyBB::INPUT_INT)) {
            ...
            if ($visible == 1) {
                ...
                if ($mybb->get_input('lastpid', MyBB::INPUT_INT)) {
                    ...
                    if (
                        $new_post['pid'] !=
                        $mybb->get_input('lastpid', MyBB::INPUT_INT)
                    ) {
                ...
                if ($mybb->settings['postsperpage'] > 0) {
                    $post_page = ceil(
                        ($postcounter + 1) / $mybb->settings['postsperpage']
                    );
                } else {
                    $post_page = 1;
                }
                ...
                if (
                    $post_page > $mybb->get_input('from_page', MyBB::INPUT_INT)
                ) {
                    redirect(get_thread_link($tid, 0, 'lastpost'));
                    exit();
                }
                ...
                if ($closed == 1) {
                    $data .=
                        "$('#quick_reply_form .trow1').removeClass('trow1 trow2').addClass('trow_shaded');\n";
                } else {
                    $data .=
            ...
            $lang->redirect_newreply .= $lang->sprintf(
...
if (
        ...
        if (count($quoted_posts) > 0) {
            ...
            if ($external_quotes > 0) {
                if ($external_quotes == 1) {
                    ...
                    $multiquote_deselect =
                        $lang->multiquote_external_one_deselect;
                    $multiquote_quote = $lang->multiquote_external_one_quote;
                } else {
                    $multiquote_text = $lang->sprintf(
                        $lang->multiquote_external,
                        $external_quotes
                    );
                ...
                eval(
                    "\$multiquote_external = \"" .
                        $templates->get('newreply_multiquote_external') .
                        "\";"
                );
            }
    ...
    if (!empty($mybb->input['previewpost']) || $reply_errors != '') {
        ...
        if (
            isset($postoptions['signature']) &&
            $postoptions['signature'] == 1
        ) {
            $postoptionschecked['signature'] = " checked=\"checked\"";
        }
        if (
            isset($postoptions['disablesmilies']) &&
            $postoptions['disablesmilies'] == 1
        ) {
    ...
    if (!empty($mybb->input['previewpost'])) {
        ...
        if (count($post_errors) > 0) {
            ...
            $mybb->input['postoptions'] = $mybb->get_input(
                'postoptions',
                MyBB::INPUT_ARRAY
            );
            if (isset($mybb->input['postoptions']['disablesmilies'])) {
                $post['smilieoff'] =
                    $mybb->input['postoptions']['disablesmilies'];
            }
    ...
    if (
        ...
        $forumpermissions['canpostattachments'] != 0
    ) {
        $attachcount = 0;
        if ($pid) {
            $attachwhere = "pid='$pid'";
        ...
        while ($attachment = $db->fetch_array($query)) {
            ...
            $attachment['icon'] = get_attachment_icon(
                get_extension($attachment['filename'])
            );
        ...
        if (
            $usage['ausage'] > $mybb->usergroup['attachquota'] * 1024 &&
            $mybb->usergroup['attachquota'] != 0
        ) {
            $noshowattach = 1;
        }
        ...
        if ($mybb->usergroup['attachquota'] == 0) {
            ...
            $friendlyquota = get_friendly_size(
                $mybb->usergroup['attachquota'] * 1024
            );
        }
        $lang->attach_quota = $lang->sprintf(
            $lang->attach_quota,
            $friendlyquota
        );
        ...
        if ($usage['ausage'] !== null) {
            ...
            $lang->attach_usage = $lang->sprintf(
                $lang->attach_usage,
                $friendlyusage
            );
            eval(
                "\$link_viewattachments = \"" .
                    $templates->get('post_attachments_viewlink') .
                    "\";"
            );
        } else {
            $lang->attach_usage = '';
        ...
        $attach_add_options = '';
        if (
            $mybb->settings['maxattachments'] == 0 ||
            ($mybb->settings['maxattachments'] != 0 &&
                $attachcount < $mybb->settings['maxattachments'] &&
                !$noshowattach)
        ) {
            eval(
                "\$attach_add_options = \"" .
                    $templates->get('post_attachments_add') .
                    "\";"
            );
        }
        ...
        if (
            ($mybb->usergroup['caneditattachments'] ||
        ...
        if ($attach_add_options || $attach_update_options) {
            eval(
                "\$newattach = \"" .
                    $templates->get('post_attachments_new') .
                    "\";"
            );
        }
    ...
    if ($mybb->settings['captchaimage'] && !$mybb->user['uid']) {
        ...
        if (
            (!empty($mybb->input['previewpost']) || $hide_captcha == true) &&
            $post_captcha->type == 1
        ) {
    ...
    if ($mybb->settings['threadreview'] != 0) {
        ...
        if ($numposts > $mybb->settings['postsperpage']) {
            ...
            $lang->thread_review_more = $lang->sprintf(
                $lang->thread_review_more,
                $mybb->settings['postsperpage'],
                get_thread_link($tid)
            );
            eval(
                "\$reviewmore = \"" .
                    $templates->get('newreply_threadreview_more') .
                    "\";"
            );
        }
        ...
        $query = $db->simple_select(
            ...
            [
                ...
                'limit' => $mybb->settings['postsperpage'],
            ]
        );
        ...
        if (!empty($pidin)) {
            ...
            while ($post = $db->fetch_array($query)) {
                ...
                $post['message'] = $parser->parse_message(
                    $post['message'],
                    $parser_options
                );
                ...
                eval(
                    "\$reviewbits .= \"" .
                        $templates->get('newreply_threadreview_post') .
                        "\";"
                );
                if ($altbg == 'trow1') {
                    $altbg = 'trow2';
            ...
            eval(
                "\$threadreview = \"" .
                    $templates->get('newreply_threadreview') .
                    "\";"
            );
        }
    }
    ...
    if (
        ...
        eval(
            "\$signature = \"" . $templates->get('newreply_signature') . "\";"
        );
    }
    ...
    if ($forum['allowsmilies'] != 0) {
        eval(
            "\$disablesmilies = \"" .
                $templates->get('newreply_disablesmilies') .
                "\";"
        );
    }
    ...
    if (!empty($signature) || !empty($disablesmilies)) {
        eval(
            "\$postoptions = \"" .
                $templates->get('newreply_postoptions') .
                "\";"
        );
        $bgcolor = 'trow2';
    } else {
    ...
    if (is_moderator($fid)) {
        if ($mybb->get_input('processed', MyBB::INPUT_INT)) {
            $mybb->input['modoptions'] = $mybb->get_input(
                'modoptions',
                MyBB::INPUT_ARRAY
            );
        ...
        if (is_moderator($thread['fid'], 'canopenclosethreads')) {
            eval(
                "\$closeoption = \"" .
                    $templates->get('newreply_modoptions_close') .
                    "\";"
            );
        }
        ...
        if (is_moderator($thread['fid'], 'canstickunstickthreads')) {
            eval(
                "\$stickoption = \"" .
                    $templates->get('newreply_modoptions_stick') .
                    "\";"
            );
        }
        ...
        if (!empty($closeoption) || !empty($stickoption)) {
            eval(
                "\$modoptions = \"" .
                    $templates->get('newreply_modoptions') .
                    "\";"
            );
            $bgcolor = 'trow1';
        } else {
    ...
    eval(
        "\$subscriptionmethod = \"" .
            $templates->get('post_subscription_method') .
            "\";"
    );

    $lang->post_reply_to = $lang->sprintf(
        $lang->post_reply_to,
        $thread['subject']
    );
    ...
    if ($forum['rulestype'] >= 2 && $forum['rules']) {
        ...
        $forum['rules'] = $parser->parse_message(
            $forum['rules'],
            $rules_parser
        );
        $foruminfo = $forum;
        ...
        if ($forum['rulestype'] == 3) {
            eval(
                "\$forumrules = \"" .
                    $templates->get('forumdisplay_rules') .
                    "\";"
            );
        } elseif ($forum['rulestype'] == 2) {
            eval(
                "\$forumrules = \"" .
                    $templates->get('forumdisplay_rules_link') .
                    "\";"
            );
        }
    }
    ...
    if (!is_moderator($forum['fid'], 'canapproveunapproveattachs')) {
        if (
            $forumpermissions['modattachments'] == 1 &&
            $forumpermissions['canpostattachments'] != 0
        ) {
