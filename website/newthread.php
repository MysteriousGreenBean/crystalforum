...
$templatelist =
    'newthread,previewpost,loginbox,changeuserbox,newthread_postpoll,posticons,codebuttons,postbit,post_attachments_attachment_unapproved,newreply_modoptions_close,newreply_modoptions_stick';
$templatelist .=
    ',newthread_disablesmilies,post_attachments_new,post_attachments,post_savedraftbutton,post_subscription_method,post_attachments_attachment_remove,postbit_warninglevel_formatted,postbit_icon';
$templatelist .=
    ',forumdisplay_rules,forumdisplay_rules_link,post_attachments_attachment_postinsert,post_attachments_attachment,newthread_signature,post_prefixselect_prefix,post_prefixselect_single,posticons_icon';
$templatelist .=
    ',post_captcha_hidden,post_captcha_recaptcha_invisible,post_captcha_nocaptcha,post_captcha_hcaptcha_invisible,post_captcha_hcaptcha,post_javascript,postbit_gotopost,newthread_postoptions,post_attachments_add,post_attachments_viewlink';
$templatelist .=
    ',postbit_avatar,postbit_find,postbit_pm,postbit_rep_button,postbit_www,postbit_email,postbit_reputation,postbit_warn,postbit_warninglevel,postbit_author_user,postbit_author_guest,post_captcha';
$templatelist .=
    ',postbit_signature,postbit_attachments_thumbnails_thumbnail,postbit_attachments_images_image,postbit_attachments_attachment,postbit_attachments_attachment_unapproved';
$templatelist .=
    ',postbit_attachments_thumbnails,postbit_attachments_images,postbit_attachments,postbit_reputation_formatted_link,post_attachments_update,postbit_offline,newreply_modoptions,newthread_multiquote_external';
$templatelist .=
    ',postbit_profilefield_multiselect_value,postbit_profilefield_multiselect,newthread_draftinput,global_moderation_notice,postbit_online,postbit_away,attachment_icon,postbit_userstar,postbit_groupimage';
...
if (
    ...
    $query = $db->simple_select(
        'posts',
        '*',
        "tid='" . $mybb->input['tid'] . "' AND visible='-2'",
    ...
    $tid = $thread['tid'];
    eval(
        "\$editdraftpid = \"" . $templates->get('newthread_draftinput') . "\";"
    );
} else {
...
if (
    ...
    if ($mybb->user['uid'] == 0) {
        ...
        if (
            !$mybb->user['uid'] &&
            $mybb->settings['stopforumspam_on_newthread']
        ) {
    ...
    if (!$mybb->get_input('savedraft') && !$pid) {
        $query = $db->simple_select(
            'posts p',
            'p.pid',
            "$user_check AND p.fid='{$forum['fid']}' AND p.subject='" .
    ...
    $new_thread['modoptions'] = $mybb->get_input(
        'modoptions',
        MyBB::INPUT_ARRAY
    );
    ...
    // No errors were found, it is safe to insert the thread.
        ...
        elseif (
            $mybb->get_input('postpoll', MyBB::INPUT_INT) &&
            $forumpermissions['canpostpolls']
        ) {
            $url =
                "polls.php?action=newpoll&tid=$tid&polloptions=" .
                $mybb->get_input('numpolloptions', MyBB::INPUT_INT);
        ...
        if (!$mybb->get_input('postpoll', MyBB::INPUT_INT)) {
            $lang->redirect_newthread .= $lang->sprintf(
...
if (
            ...
            if ($mybb->get_input('load_all_quotes', MyBB::INPUT_INT) == 1) {
                ...
                if ($external_quotes > 0) {
                    if ($external_quotes == 1) {
                        ...
                        $multiquote_quote =
                            $lang->multiquote_external_one_quote;
                    } else {
                        ...
                        $multiquote_deselect =
                            $lang->multiquote_external_deselect;
                        $multiquote_quote = $lang->multiquote_external_quote;
                    }
                    eval(
                        "\$multiquote_external = \"" .
                            $templates->get('newthread_multiquote_external') .
                            "\";"
                    );
                }
            }
    ...
    $prefixselect = build_prefix_select(
        $forum['fid'],
    ...
    if (
        ...
        eval(
            "\$signature = \"" . $templates->get('newthread_signature') . "\";"
        );
    }
    ...
    if ($forum['allowsmilies'] != 0) {
        eval(
            "\$disablesmilies = \"" .
                $templates->get('newthread_disablesmilies') .
                "\";"
        );
    }
    ...
    if (!empty($signature) || !empty($disablesmilies)) {
        eval(
            "\$postoptions = \"" .
                $templates->get('newthread_postoptions') .
                "\";"
        );
        $bgcolor = 'trow2';
        $bgcolor2 = 'trow1';
    ...
    if (is_moderator($fid)) {
        ...
        if (
            isset($modoptions['closethread']) &&
            $modoptions['closethread'] == 1
        ) {
