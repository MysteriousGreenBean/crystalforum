...
$templatelist =
    'online,online_row,online_row_ip,online_today,online_today_row,online_row_ip_lookup,online_refresh,multipage,multipage_end,multipage_start';
$templatelist .=
    ',multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage';
...
if ($mybb->get_input('action') == 'today') {
    ...
    $query = $db->simple_select(
        'users',
        'COUNT(uid) AS users',
        "lastactive > '{$threshold}' AND invisible = '1'"
    );
    $invis_count = $db->fetch_field($query, 'users');

    if (
        !$mybb->settings['wolusersperpage'] ||
        (int) $mybb->settings['wolusersperpage'] < 1
    ) {
    ...
    while ($online = $db->fetch_array($query)) {
        ...
        if (
            ...
            $online['uid'] == $mybb->user['uid']
        ) {
            $username = format_name(
                htmlspecialchars_uni($online['username']),
                $online['usergroup'],
                $online['displaygroup']
            );
            $online['profilelink'] = build_profile_link(
                $username,
                $online['uid']
            );
            ...
            eval(
                "\$todayrows .= \"" .
                    $templates->get('online_today_row') .
                    "\";"
            );
        }
    }
    ...
    if ($todaycount == 1) {
        ...
        $onlinetoday = $lang->sprintf(
            $lang->members_were_online_today,
            $todaycount
        );
    }
    ...
    // Otherwise sort by last refresh
        ...
        switch ($db->type) {
            ...
            case 'pgsql':
                $sql =
                    'CASE WHEN s.uid > 0 THEN 1 ELSE 0 END DESC, s.time DESC';
                break;
            default:
        ...
        $query = $db->query(
			...
			FROM
                ...
                "sessions s2
						LEFT JOIN " .
                TABLE_PREFIX .
                "sessions s3 ON (s2.sid = s3.sid AND s2.time < s3.time)
    ...
    while ($user = $db->fetch_array($query)) {
        ...
        if ($user['uid'] > 0) {
            if (
                empty($users[$user['uid']]) ||
                $users[$user['uid']]['time'] < $user['time']
            ) {
                $users[$user['uid']] = $user;
            }
        }
