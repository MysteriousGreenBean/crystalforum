...
$templatelist =
    'changeuserbox,loginbox,polls_newpoll_option,polls_newpoll,polls_editpoll_option,polls_editpoll,polls_showresults_resultbit,polls_showresults';
...
if ($mybb->input['action'] == 'newpoll') {
    ...
    if (
        ...
        $lang->poll_time_limit = $lang->sprintf(
            $lang->poll_time_limit,
            $mybb->settings['polltimelimit']
        );
    ...
    if ($mybb->get_input('numpolloptions', MyBB::INPUT_INT) > 0) {
        $mybb->input['polloptions'] = $mybb->get_input(
            'numpolloptions',
            MyBB::INPUT_INT
        );
    }
    if (
        ...
        $mybb->get_input('polloptions', MyBB::INPUT_INT) >
            $mybb->settings['maxpolloptions']
    ) {
        // Too big
    ...
    for ($i = 1; $i <= $polloptions; ++$i) {
        ...
        eval(
            "\$optionbits .= \"" .
                $templates->get('polls_newpoll_option') .
                "\";"
        );
        $option = '';
    }
    ...
    if (
        $mybb->get_input('maxoptions', MyBB::INPUT_INT) > 0 &&
        $mybb->get_input('maxoptions', MyBB::INPUT_INT) < $polloptions
    ) {
...
if ($mybb->input['action'] == 'do_newpoll' && $mybb->request_method == 'post') {
    ...
    for ($i = 1; $i <= $polloptions; ++$i) {
        ...
        if (
            $mybb->settings['polloptionlimit'] != 0 &&
            my_strlen($options[$i]) > $mybb->settings['polloptionlimit']
        ) {
            $lengtherror = 1;
            break;
        }
    ...
    if ($thread['visible'] == 1) {
        ...
        redirect(
            ...
            $lang->redirect_pollpostedmoderated
        );
    }
}
...
if ($mybb->input['action'] == 'editpoll') {
    ...
    if (empty($mybb->input['updateoptions'])) {
        ...
        for ($i = 0; $i < $numoptions; ++$i) {
            ...
            eval(
                "\$optionbits .= \"" .
                    $templates->get('polls_editpoll_option') .
                    "\";"
            );
            $option = '';
            $optionvotes = '';
        ...
        if (
            $mybb->settings['maxpolloptions'] &&
            $mybb->get_input('numoptions', MyBB::INPUT_INT) >
                $mybb->settings['maxpolloptions']
        ) {
...
if (
    ...
    $query = $db->simple_select(
        'threads',
        '*',
        "poll='" . $mybb->get_input('pid', MyBB::INPUT_INT) . "'"
    );
    $thread = $db->fetch_array($query);
    if (!$thread) {
        error($lang->error_invalidthread);
    ...
    if (
        $mybb->get_input('maxoptions', MyBB::INPUT_INT) > 0 &&
        $mybb->get_input('maxoptions', MyBB::INPUT_INT) < $numoptions
    ) {
        $maxoptions = $mybb->get_input('maxoptions', MyBB::INPUT_INT);
    } else {
        $maxoptions = 0;
...
if ($mybb->input['action'] == 'showresults') {
    ...
    while ($voter = $db->fetch_array($query)) {
        ...
        if ($voter['uid'] == 0 || $voter['username'] == '') {
            ...
            $voters[$voter['voteoption']][$voter['uid']] = htmlspecialchars_uni(
                $voter['username']
            );
        }
    }
    ...
    for ($i = 1; $i <= $poll['numoptions']; ++$i) {
        ...
        eval(
            "\$polloptions .= \"" .
                $templates->get('polls_showresults_resultbit') .
                "\";"
        );
    }
...
if ($mybb->input['action'] == 'vote' && $mybb->request_method == 'post') {
    ...
    if ($poll['multiple'] == 1) {
        if (is_array($option)) {
            ...
            if (
                $total_options > $poll['maxoptions'] &&
                $poll['maxoptions'] != 0
            ) {
                error(
                    $lang->sprintf(
                        $lang->error_maxpolloptions,
                        $poll['maxoptions']
                    )
                );
            }
...
if ($mybb->input['action'] == 'do_undovote') {
    ...
    if (
        $poll['closed'] == 1 ||
        $thread['closed'] == 1 ||
        ($expiretime < TIME_NOW && $poll['timeout'])
    ) {
        error($lang->error_pollclosed);
    }
