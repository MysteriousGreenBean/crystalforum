...
$templatelist =
    'portal,portal_welcome_membertext,portal_stats,portal_search,portal_whosonline_memberbit,portal_whosonline,portal_latestthreads_thread,portal_latestthreads,portal_announcement_numcomments_no';
$templatelist .=
    ',postbit_attachments_thumbnails_thumbnail,postbit_attachments_images_image,postbit_attachments_attachment,postbit_attachments_thumbnails,postbit_attachments_images,postbit_attachments';
$templatelist .=
    ',multipage,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start,portal_announcement_send_item';
$templatelist .=
    ',portal_stats_nobody,portal_announcement_avatar,portal_announcement_numcomments,portal_announcement_icon,portal_pms,portal_welcome,portal_announcement,portal_welcome_guesttext';
...
if ($mybb->settings['portal_showwelcome'] != 0) {
    if ($mybb->user['uid'] != 0) {
        ...
        if ($newposts) {
            ...
            if (!empty($mybb->settings['portal_announcementsfid'])) {
                ...
                if ($mybb->settings['portal_announcementsfid'] != -1) {
                    $announcementsfids = explode(
                        ',',
                        (string) $mybb->settings['portal_announcementsfid']
                    );
        ...
        if ($newann == 1) {
            ...
            $lang->new_announcements = $lang->sprintf(
        ...
        $lang->guest_welcome_registration = $lang->sprintf(
            $lang->guest_welcome_registration,
        ...
        eval(
            "\$welcometext = \"" .
                $templates->get('portal_welcome_guesttext') .
                "\";"
        );
    }
...
if ($mybb->settings['portal_showpms'] != 0) {
    if (
        ...
        $lang->pms_received_new = $lang->sprintf(
            $lang->pms_received_new,
            $mybb->user['username'],
            $messages['pms_unread']
        );
...
if ($mybb->settings['portal_showstats'] != 0) {
    ...
    if (!$stats['lastusername']) {
        eval(
            "\$newestmember = \"" .
                $templates->get('portal_stats_nobody') .
                "\";"
        );
    } else {
        $newestmember = build_profile_link(
            $stats['lastusername'],
            $stats['lastuid']
        );
    }
...
if (
    $mybb->settings['portal_showwol'] != 0 &&
    $mybb->usergroup['canviewonline'] != 0
) {
    ...
    $doneusers = $onlinemembers = $onlinebots = [];

    $query = $db->simple_select(
        'sessions',
        'COUNT(DISTINCT ip) AS guestcount',
        "uid = 0 AND time > $timesearch"
    );
    ...
    while ($user = $db->fetch_array($query)) {
        ...
        if ($user['uid'] > 0) {
            ...
            my_strpos($user['sid'], 'bot=') !== false &&
            $spiders[$botkey] &&
            $mybb->settings['woldisplayspiders'] == 1
        ) {
    ...
    $lang->online_counts = $lang->sprintf(
        $lang->online_counts,
        $membercount,
        $guestcount
    );
...
if (
    ...
    $mybb->settings['portal_excludediscussion'] != -1
) {
    $altbg = alt_trow();
    $threadlist = '';
    ...
    while ($thread = $db->fetch_array($query)) {
        ...
        if (
            isset($forumpermissions[$thread['fid']]['canonlyviewownthreads']) &&
        ...
        $thread['subject'] = $thread['fullsubject'] = $parser->parse_badwords(
            $thread['subject']
        );
        ...
        eval(
            "\$threadlist .= \"" .
                $templates->get('portal_latestthreads_thread') .
                "\";"
        );
        $altbg = alt_trow();
    }
    if ($threadlist) {
        ...
        eval(
            "\$latestthreads = \"" .
                $templates->get('portal_latestthreads') .
                "\";"
        );
    }
}
...
if (!empty($mybb->settings['portal_announcementsfid'])) {
    ...
    if (!empty($posts)) {
        ...
        while ($announcement = $db->fetch_array($query)) {
            ...
            if (
                isset(
                    $forumpermissions[$announcement['fid']][
                ...
                $announcement['uid'] != $mybb->user['uid']
            ) {
                continue;
            }
            ...
            $announcement['smilieoff'] =
                $posts[$announcement['tid']]['smilieoff'];
            $announcement['threadlink'] = get_thread_link($announcement['tid']);
            $announcement['forumlink'] = get_forum_link($announcement['fid']);
            $announcement['forumname'] =
                $forum_cache[$announcement['fid']]['name'];
            $announcement['username'] = htmlspecialchars_uni(
                $announcement['username']
            );
            if (!$announcement['uid'] && !$announcement['threadusername']) {
                $announcement['threadusername'] = htmlspecialchars_uni(
                    $lang->guest
                );
            } else {
                $announcement['threadusername'] = htmlspecialchars_uni(
                    $announcement['threadusername']
                );
            }
            ...
            if (
                ...
                eval(
                    "\$icon = \"" .
                        $templates->get('portal_announcement_icon') .
                        "\";"
                );
            } else {
                $icon = '&nbsp;';
            ...
            $useravatar = format_avatar(
                $announcement['avatar'],
                $announcement['avatardimensions']
            );
            eval(
                "\$avatar = \"" .
                    $templates->get('portal_announcement_avatar') .
                    "\";"
            );
            ...
            if ($announcement['replies']) {
                eval(
                    "\$numcomments = \"" .
                        $templates->get('portal_announcement_numcomments') .
                        "\";"
                );
            } else {
                eval(
                    "\$numcomments = \"" .
                        $templates->get('portal_announcement_numcomments_no') .
                        "\";"
                );
                $lastcomment = '';
            }
            ...
            if (
                ...
                eval(
                    "\$senditem = \"" .
                        $templates->get('portal_announcement_send_item') .
                        "\";"
                );
            }
            ...
            $parser_options = [
                'allow_html' =>
                    ...
                    $forum[$announcement['fid']]['allowhtml'],
                ...
                'allow_videocode' =>
                    $forum[$announcement['fid']]['allowvideocode'],
                'filter_badwords' => 1,
            ];
            ...
            if (
                ...
                isset($attachcache[$announcement['pid']]) &&
                is_array($attachcache[$announcement['pid']])
            ) {
                // This post has 1 or more attachments
                $validationcount = 0;
                $id = $announcement['pid'];
                $post['attachmentlist'] = $post['thumblist'] = $post[
                    'imagelist'
                ] = $post['attachedthumbs'] = $post['attachedimages'] = '';
