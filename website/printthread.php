...
$templatelist =
    'printthread,printthread_post,printthread_nav,forumdisplay_password_wrongpass,forumdisplay_password,printthread_multipage,printthread_multipage_page,printthread_multipage_page_current';
...
if ($postcount > $perpage) {
    $multipage = printthread_multipage(
        $postcount,
        $perpage,
        $page,
        "printthread.php?tid={$tid}"
    );
} else {
    $multipage = '';
}
...
if ($mybb->settings['enableattachments']) {
    $queryAttachments = $db->simple_select(
        'attachments',
        '*',
        "pid IN ('{$pids}')"
    );
...
foreach ($postrow_cache as $postrow) {
    ...
    $postrow['subject'] = htmlspecialchars_uni(
        $parser->parse_badwords($postrow['subject'])
    );
    $postrow['date'] = my_date(
        $mybb->settings['dateformat'],
        $postrow['dateline'],
        null,
        0
    );
    $postrow['profilelink'] = build_profile_link(
        $postrow['username'],
        $postrow['uid']
    );

    $postrow['message'] = $parser->parse_message(
        $postrow['message'],
        $parser_options
    );
...
function makeprintablenav($pid = 0, $depth = '--')
{
    ...
    if (!is_array($pforumcache)) {
        ...
        $query = $db->simple_select('forums', 'name, fid, pid', "$parlist", [
    ...
    if ($count > $perpage) {
        ...
        for ($page = 1; $page <= $pages; ++$page) {
            if ($page == $current_page) {
                eval(
                    "\$mppage .= \"" .
                        $templates->get('printthread_multipage_page_current') .
                        "\";"
                );
            } else {
                eval(
                    "\$mppage .= \"" .
                        $templates->get('printthread_multipage_page') .
                        "\";"
                );
            }
        }
        ...
        eval(
            "\$multipage = \"" .
                $templates->get('printthread_multipage') .
                "\";"
        );
    }
    return $multipage;
