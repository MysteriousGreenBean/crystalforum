...
$templatelist =
    'private_send,private_send_buddyselect,private_tracking,private_tracking_readmessage,private_tracking_unreadmessage,usercp_nav_attachments,usercp_nav_messenger_compose,private_tracking_readmessage_stop';
$templatelist .=
    ',private_folders,private_folders_folder,private_folders_folder_unremovable,private,usercp_nav,private_empty_folder,private_archive_txt,private_archive_csv,private_archive_html,private_tracking_unreadmessage_stop';
$templatelist .=
    ',usercp_nav_messenger,usercp_nav_changename,multipage,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start';
$templatelist .=
    ',private_messagebit,codebuttons,posticons,private_send_autocomplete,private_messagebit_denyreceipt,postbit_warninglevel_formatted,private_emptyexportlink,postbit_purgespammer,postbit_gotopost,private_read';
$templatelist .=
    ',postbit_delete_pm,postbit,private_tracking_nomessage,private_nomessages,postbit_author_guest,private_multiple_recipients_user,private_multiple_recipients_bcc,private_multiple_recipients,usercp_nav_messenger_folder';
$templatelist .=
    ',private_search_messagebit,private_search_results_nomessages,private_search_results,private_advanced_search,previewpost,private_send_tracking,private_send_signature,private_read_bcc,private_composelink';
$templatelist .=
    ',private_archive,private_quickreply,private_pmspace,private_limitwarning,postbit_groupimage,postbit_offline,postbit_www,postbit_replyall_pm,postbit_signature,postbit_reputation_formatted_link';
$templatelist .=
    ',private_archive_folders_folder,private_archive_folders,postbit_warninglevel,postbit_author_user,postbit_forward_pm,private_messagebit_icon,private_jump_folders_folder,private_advanced_search_folders,usercp_nav_home';
$templatelist .=
    ',private_jump_folders,postbit_avatar,postbit_warn,postbit_rep_button,postbit_email,postbit_reputation,private_move,private_read_action,postbit_away,postbit_pm,usercp_nav_messenger_tracking,postbit_find';
$templatelist .=
    ',usercp_nav_editsignature,posticons_icon,postbit_icon,postbit_iplogged_hiden,usercp_nav_profile,usercp_nav_misc,postbit_userstar,private_read_to,postbit_online,private_empty,private_orderarrow,postbit_reply_pm';
...
foreach ($foldersexploded as $key => $folders) {
    ...
    eval(
        "\$folderjump_folder .= \"" .
            $templates->get('private_jump_folders_folder') .
            "\";"
    );
...
eval(
    "\$foldersearch = \"" .
        $templates->get('private_advanced_search_folders') .
        "\";"
);
...
if (
    ($mybb->input['action'] == 'do_search' ||
        ($mybb->input['action'] == 'do_stuff' &&
            ($mybb->get_input('quick_search') ||
                (!$mybb->get_input('hop') &&
                    !$mybb->get_input('moveto') &&
    ...
    if (
        $mybb->settings['searchfloodtime'] > 0 &&
        ...
        $query = $db->simple_select(
            'searchlog',
            '*',
            "uid='{$mybb->user['uid']}' AND dateline > '$timecut'",
        ...
        if ($last_search['sid']) {
            $remaining_time =
                $mybb->settings['searchfloodtime'] -
                (TIME_NOW - $last_search['dateline']);
            if ($remaining_time == 1) {
                $lang->error_searchflooding = $lang->sprintf(
                    $lang->error_searchflooding_1,
                    $mybb->settings['searchfloodtime']
                );
            } else {
                $lang->error_searchflooding = $lang->sprintf(
                    $lang->error_searchflooding,
                    $mybb->settings['searchfloodtime'],
                    $remaining_time
                );
            }
    ...
    if (
        $mybb->get_input('subject', MyBB::INPUT_INT) != 1 &&
        $mybb->get_input('message', MyBB::INPUT_INT) != 1
    ) {
    ...
    redirect(
        'private.php?action=results&sid=' .
            $sid .
            '&sortby=' .
            $sortby .
            '&order=' .
            $sortorder,
        $lang->redirect_searchresults
    );
}
...
if ($mybb->input['action'] == 'results') {
    ...
    $query = $db->simple_select(
        'searchlog',
        '*',
        "sid='" . $db->escape_string($sid) . "' AND uid='{$mybb->user['uid']}'"
    );
    ...
    $query = $db->simple_select(
        'privatemessages',
        'COUNT(*) AS total',
        'pmid IN(' . $db->escape_string($search['querycache']) . ')'
    );
    ...
    $multipage = multipage(
        ...
        $page,
        'private.php?action=results&amp;sid=' .
            htmlspecialchars_uni($mybb->get_input('sid')) .
    ...
    while ($row = $db->fetch_array($users_query)) {
        ...
        if (
            isset($recipients['bcc']) &&
            is_array($recipients['bcc']) &&
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "users fu ON (fu.uid=pm.fromid)
		LEFT JOIN " .
            TABLE_PREFIX .
            "users tu ON (tu.uid=pm.toid)
		WHERE pm.pmid IN(" .
            $db->escape_string($search['querycache']) .
            ") AND pm.uid='{$mybb->user['uid']}'
    ...
    while ($message = $db->fetch_array($query)) {
        ...
        if ($folder == 2 || $folder == 3) {
            ...
            if (
                isset($recipients['to']) &&
                (count($recipients['to']) > 1 ||
                    (count($recipients['to']) == 1 &&
                        isset($recipients['bcc']) &&
                        count($recipients['bcc']) > 0))
            ) {
                ...
                if (
                    ...
                    eval(
                        "\$bcc_users = \"" .
                            $templates->get('private_multiple_recipients_bcc') .
                            "\";"
                    );
                    foreach ($recipients['bcc'] as $uid) {
                        $profilelink = get_profile_link($uid);
                        $user = $cached_users[$uid];
                        $user['username'] = htmlspecialchars_uni(
                            $user['username']
                        );
                        $username = format_name(
                            $user['username'],
                ...
                eval(
                    "\$tofromusername = \"" .
                        $templates->get('private_multiple_recipients') .
                        "\";"
                );
            } elseif ($message['toid']) {
        ...
        if ($message['icon'] > 0 && $icon_cache[$message['icon']]) {
            ...
            eval(
                "\$icon = \"" .
                    $templates->get('private_messagebit_icon') .
                    "\";"
            );
        } else {
            $icon = '&#009;';
        ...
        if (my_strlen($message['subject']) > 50) {
            $message['subject'] = htmlspecialchars_uni(
                my_substr($message['subject'], 0, 50) . '...'
            );
        } else {
        ...
        $message['message'] = strip_tags(
            $parser->parse_message($message['message'], $parser_options)
        );
        if (my_strlen($message['message']) > 200) {
            $message['message'] =
                my_substr($message['message'], 0, 200) . '...';
        }

        eval(
            "\$messagelist .= \"" .
                $templates->get('private_search_messagebit') .
                "\";"
        );
    }
    ...
    if ($db->num_rows($query) == 0) {
        eval(
            "\$messagelist = \"" .
                $templates->get('private_search_results_nomessages') .
                "\";"
        );
    }
...
if ($mybb->input['action'] == 'advanced_search') {
    ...
    eval(
        "\$advanced_search = \"" .
            $templates->get('private_advanced_search') .
            "\";"
    );
...
if ($mybb->input['action'] == 'do_send' && $mybb->request_method == 'post') {
    ...
    $to_escaped = implode(
        "','",
        array_map([$db, 'escape_string'], array_map('my_strtolower', $to))
    );
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "users u ON(u.uid=pm.toid)
		WHERE LOWER(u.username) IN ('{$to_escaped}') AND pm.dateline > {$time_cutoff} AND pm.fromid='{$mybb->user['uid']}' AND pm.subject='" .
            $db->escape_string($mybb->get_input('subject')) .
            "' AND pm.message='" .
            $db->escape_string($mybb->get_input('message')) .
            "' AND pm.folder!='3'
		LIMIT 0, 1
	"
    );
    ...
    $pm['options'] = [];
    if (
        isset($mybb->input['options']['signature']) &&
        $mybb->input['options']['signature'] == 1
    ) {
    ...
    if (
        isset($mybb->input['options']['savecopy']) &&
        $mybb->input['options']['savecopy'] == 1
    ) {
...
if ($mybb->input['action'] == 'send') {
    ...
    $message = htmlspecialchars_uni(
        $parser->parse_badwords($mybb->get_input('message'))
    );
    $subject = htmlspecialchars_uni(
        $parser->parse_badwords($mybb->get_input('subject'))
    );
    ...
    if (!empty($mybb->input['preview']) || $send_errors) {
        ...
        if (
            isset($options['disablesmilies']) &&
            $options['disablesmilies'] == 1
        ) {
        ...
        $to = htmlspecialchars_uni(
            implode(
                ', ',
                array_unique(
                    array_map('trim', explode(',', $mybb->get_input('to')))
                )
            )
        );
        $bcc = htmlspecialchars_uni(
            implode(
                ', ',
                array_unique(
                    array_map('trim', explode(',', $mybb->get_input('bcc')))
                )
            )
        );
    }
    ...
    if (
        ...
        $query = $db->query(
			...
			LEFT JOIN " .
                TABLE_PREFIX .
                "users u ON (u.uid=pm.fromid)
			WHERE pm.pmid='" .
                $mybb->get_input('pmid', MyBB::INPUT_INT) .
                "' AND pm.uid='{$mybb->user['uid']}'
		"
        );
        ...
        $message = htmlspecialchars_uni(
            $parser->parse_badwords($pm['message'])
        );
        $subject = htmlspecialchars_uni(
        ...
        if ($pm['folder'] == '3') {
            ...
            if ($mybb->settings['maxpmquotedepth'] != '0') {
                $message = remove_message_quotes(
                    $message,
                    $mybb->settings['maxpmquotedepth']
                );
            }
    ...
    if ($mybb->usergroup['maxpmrecipients'] > 0) {
        $max_recipients = $lang->sprintf(
            $lang->max_recipients,
            $mybb->usergroup['maxpmrecipients']
        );
    }
    ...
    eval(
        "\$autocompletejs = \"" .
            $templates->get('private_send_autocomplete') .
            "\";"
    );
    ...
    if (
        $mybb->user['buddylist'] != '' &&
        $mybb->settings['use_xmlhttprequest'] == 1
    ) {
        $buddy_select = 'to';
        eval(
            "\$buddy_select_to = \"" .
                $templates->get('private_send_buddyselect') .
                "\";"
        );
        $buddy_select = 'bcc';
        eval(
    ...
    if ($mybb->usergroup['cantrackpms']) {
        eval(
            "\$private_send_tracking = \"" .
                $templates->get('private_send_tracking') .
                "\";"
        );
    }
...
if ($mybb->input['action'] == 'read') {
    ...
    if ($pm['receipt'] == 1) {
        if (
            $mybb->usergroup['candenypmreceipts'] == 1 &&
            $mybb->get_input('denyreceipt', MyBB::INPUT_INT) == 1
        ) {
            $receiptadd = 0;
        } else {
            $receiptadd = 2;
    ...
    if (
        isset($pm['recipients']['bcc']) &&
        count($pm['recipients']['bcc']) > 0 &&
        $mybb->usergroup['cancp'] == 1
    ) {
        $show_bcc = 1;
    ...
    while ($recipient = $db->fetch_array($query)) {
        ...
        if (
            $show_bcc &&
            in_array($recipient['uid'], $pm['recipients']['bcc'])
        ) {
            $bcc_recipients[] = build_profile_link(
                $recipient['username'],
                $recipient['uid']
            );
        ...
        elseif (in_array($recipient['uid'], $pm['recipients']['to'])) {
            $to_recipients[] = build_profile_link(
                $recipient['username'],
                $recipient['uid']
            );
        }
    ...
    if (
        ...
        if ($mybb->settings['maxpmquotedepth'] != '0') {
            $quoted_message = remove_message_quotes(
                $quoted_message,
                $mybb->settings['maxpmquotedepth']
            );
        }
...
if ($mybb->input['action'] == 'tracking') {
    ...
    $query = $db->simple_select(
        'privatemessages',
        'COUNT(pmid) as readpms',
        "receipt='2' AND folder!='3' AND status!='0' AND fromid='" .
            $mybb->user['uid'] .
            "'"
    );
    ...
    $read_multipage = multipage(
        $postcount,
        $perpage,
        $page,
        'private.php?action=tracking&amp;read_page={page}'
    );
    ...
    while ($readmessage = $db->fetch_array($query)) {
        $readmessage['subject'] = htmlspecialchars_uni(
            $parser->parse_badwords($readmessage['subject'])
        );
        $readmessage['tousername'] = htmlspecialchars_uni(
            $readmessage['tousername']
        );
        $readmessage['profilelink'] = build_profile_link(
            $readmessage['tousername'],
            $readmessage['toid']
        );
        ...
        eval(
            "\$readmessages .= \"" .
                $templates->get('private_tracking_readmessage') .
                "\";"
        );
    }
    ...
    if (!empty($readmessages)) {
        eval(
            "\$stoptrackingread = \"" .
                $templates->get('private_tracking_readmessage_stop') .
                "\";"
        );
    }
    ...
    if (!$readmessages) {
        eval(
            "\$readmessages = \"" .
                $templates->get('private_tracking_nomessage') .
                "\";"
        );
    }
    ...
    $query = $db->simple_select(
        'privatemessages',
        'COUNT(pmid) as unreadpms',
        "receipt='1' AND folder!='3' AND status='0' AND fromid='" .
            $mybb->user['uid'] .
            "'"
    );
    ...
    $unread_multipage = multipage(
        $postcount,
        $perpage,
        $page,
        'private.php?action=tracking&amp;unread_page={page}'
    );
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "users u ON (u.uid=pm.toid)
		WHERE pm.receipt='1' AND pm.folder!='3' AND pm.status='0' AND pm.fromid='" .
            $mybb->user['uid'] .
            "'
    ...
    while ($unreadmessage = $db->fetch_array($query)) {
        $unreadmessage['subject'] = htmlspecialchars_uni(
            $parser->parse_badwords($unreadmessage['subject'])
        );
        $unreadmessage['tousername'] = htmlspecialchars_uni(
            $unreadmessage['tousername']
        );
        $unreadmessage['profilelink'] = build_profile_link(
            $unreadmessage['tousername'],
            $unreadmessage['toid']
        );
        ...
        eval(
            "\$unreadmessages .= \"" .
                $templates->get('private_tracking_unreadmessage') .
                "\";"
        );
    }
    ...
    if (!empty($unreadmessages)) {
        eval(
            "\$stoptrackingunread = \"" .
                $templates->get('private_tracking_unreadmessage_stop') .
                "\";"
        );
    }
...
if (
    ...
    if (!empty($mybb->input['stoptracking'])) {
        $mybb->input['readcheck'] = $mybb->get_input(
            'readcheck',
            MyBB::INPUT_ARRAY
        );
        if (!empty($mybb->input['readcheck'])) {
            foreach ($mybb->input['readcheck'] as $key => $val) {
                ...
                $db->update_query(
                    'privatemessages',
                    $sql_array,
                    'pmid=' . (int) $key . ' AND fromid=' . $mybb->user['uid']
                );
            }
        }
        $plugins->run_hooks('private_do_tracking_end');
        redirect(
            'private.php?action=tracking',
            $lang->redirect_pmstrackingstopped
        );
    } elseif (!empty($mybb->input['stoptrackingunread'])) {
        $mybb->input['unreadcheck'] = $mybb->get_input(
            'unreadcheck',
            MyBB::INPUT_ARRAY
        );
        ...
        if (!empty($mybb->input['unreadcheck'])) {
            ...
            $query = $db->simple_select(
                'privatemessages',
                'uid',
                "pmid IN ($pmids) AND fromid='" . $mybb->user['uid'] . "'"
            );
            ...
            $db->delete_query(
                'privatemessages',
                "pmid IN ($pmids) AND receipt='1' AND status='0' AND fromid='" .
                    $mybb->user['uid'] .
                    "'"
            );
        ...
        redirect(
            'private.php?action=tracking',
            $lang->redirect_pmstrackingcanceled
        );
    }
}
...
if ($mybb->input['action'] == 'stopalltracking') {
    ...
    redirect(
        'private.php?action=tracking',
        $lang->redirect_allpmstrackingstopped
    );
}
...
if ($mybb->input['action'] == 'folders') {
    ...
    foreach ($foldersexploded as $key => $folders) {
        ...
        if ((int) $folderinfo[0] < 5) {
            ...
            eval(
                "\$folderlist .= \"" .
                    $templates->get('private_folders_folder_unremovable') .
                    "\";"
            );
            unset($name);
        } else {
            eval(
                "\$folderlist .= \"" .
                    $templates->get('private_folders_folder') .
                    "\";"
            );
        }
    }
...
if ($mybb->input['action'] == 'do_folders' && $mybb->request_method == 'post') {
    ...
    foreach ($mybb->input['folder'] as $key => $val) {
        if (empty($donefolders[$val])) {
            // Probably was a check for duplicate folder names, but doesn't seem to be used now
            ...
            if ($val != '' || (is_numeric($key) && $key <= 4)) {
                ...
                $db->delete_query(
                    'privatemessages',
                    "folder='$fid' AND uid='" . $mybb->user['uid'] . "'"
                );
            }
        }
    }
...
if ($mybb->input['action'] == 'empty') {
    ...
    foreach ($foldersexploded as $key => $folders) {
        ...
        $query = $db->simple_select(
            'privatemessages',
            'COUNT(*) AS pmsinfolder',
            " folder='$fid'$unread AND uid='" . $mybb->user['uid'] . "'"
        );
        $thing = $db->fetch_array($query);
        $foldercount = my_number_format($thing['pmsinfolder']);
        eval(
            "\$folderlist .= \"" .
                $templates->get('private_empty_folder') .
                "\";"
        );
    }
...
if ($mybb->input['action'] == 'do_empty' && $mybb->request_method == 'post') {
    ...
    if (!empty($mybb->input['empty'])) {
        ...
        if ($emptyq != '') {
            $db->delete_query(
                'privatemessages',
                "($emptyq) AND uid='" . $mybb->user['uid'] . "'{$keepunreadq}"
            );
        }
    }
...
if ($mybb->input['action'] == 'do_stuff' && $mybb->request_method == 'post') {
    ...
    if (!empty($mybb->input['hop'])) {
        ...
        $pms = array_map(
            'intval',
            array_keys($mybb->get_input('check', MyBB::INPUT_ARRAY))
        );
        if (!empty($pms)) {
            if (!$mybb->input['fid']) {
                $mybb->input['fid'] = 1;
        ...
        if (!empty($mybb->input['fromfid'])) {
            redirect(
                'private.php?fid=' .
                    $mybb->get_input('fromfid', MyBB::INPUT_INT),
                $lang->redirect_pmsmoved
            );
        } else {
        ...
        if (!empty($mybb->input['check'])) {
            ...
            foreach ($mybb->input['check'] as $key => $val) {
                ...
                if (!empty($deletepms[$key])) {
                    $db->delete_query(
                        'privatemessages',
                        "pmid='$key' AND uid='" . $mybb->user['uid'] . "'"
                    );
                } else {
                    $sql_array = [
                        'folder' => 4,
...
if ($mybb->input['action'] == 'export') {
    ...
    foreach ($foldersexploded as $key => $folders) {
        ...
        eval(
            "\$folderlist_folder .= \"" .
                $templates->get('private_archive_folders_folder') .
                "\";"
        );
    }

    eval(
        "\$folderlist = \"" . $templates->get('private_archive_folders') . "\";"
    );
...
if ($mybb->input['action'] == 'do_export' && $mybb->request_method == 'post') {
    ...
    $lang->private_messages_for = $lang->sprintf(
        $lang->private_messages_for,
    ...
    $lang->exported_date = $lang->sprintf(
        $lang->exported_date,
        $exdate,
        $extime
    );
    ...
    if ($mybb->get_input('pmid', MyBB::INPUT_INT)) {
        ...
        if (
            $mybb->get_input('daycut', MyBB::INPUT_INT) &&
            $mybb->get_input('dayway') != 'disregard'
        ) {
            $datecut =
                TIME_NOW - $mybb->get_input('daycut', MyBB::INPUT_INT) * 86400;
            $wsql = 'pm.dateline';
        ...
        $mybb->input['exportfolders'] = $mybb->get_input(
            'exportfolders',
            MyBB::INPUT_ARRAY
        );
    ...
    while ($message = $db->fetch_array($query)) {
        if ($message['folder'] == 2 || $message['folder'] == 3) {
            ...
            if ($message['toid']) {
                ...
                if ($mybb->input['exporttype'] == 'txt') {
                    ...
                    $tofromusername = build_profile_link(
                        $message['tousername'],
                        $tofromuid
                    );
                }
        ...
        if ($mybb->input['exporttype'] == 'html') {
            ...
            $message['message'] = $parser->parse_message(
                $message['message'],
                $parser_options
            );
            $message['subject'] = htmlspecialchars_uni($message['subject']);
    ...
    if ($mybb->input['exporttype'] == 'html') {
        ...
        $query = $db->simple_select(
            'themestylesheets',
            'stylesheet',
            "tid in ({$css_tid}'2','1') AND name = 'global.css'",
    ...
    eval(
        "\$archived = \"" .
            $templates->get(
                'private_archive_' . $mybb->input['exporttype'],
                1,
                0
            ) .
            "\";"
...
if (!$mybb->input['action']) {
    ...
    if (
        !$mybb->input['fid'] ||
        !array_key_exists($mybb->input['fid'], $foldernames)
    ) {
        $mybb->input['fid'] = 0;
    }
    ...
    eval(
        "\$orderarrow['$sortby'] = \"" .
            $templates->get('private_orderarrow') .
            "\";"
    );
    ...
    $query = $db->simple_select(
        'privatemessages',
        'COUNT(*) AS total',
        "uid='" . $mybb->user['uid'] . "' AND folder='$folder'$selective"
    );
    $pmscount = $db->fetch_field($query, 'total');
    ...
    $query = $db->query(
		...
		LEFT JOIN " .
            TABLE_PREFIX .
            "users tu ON (tu.uid=pm.toid)
		WHERE pm.folder='$folder' AND pm.uid='" .
            $mybb->user['uid'] .
            "'{$selective}
    ...
    if ($db->num_rows($query) > 0) {
        ...
        while ($message = $db->fetch_array($query)) {
            ...
            if ($folder == 2 || $folder == 3) {
                ...
                if (
                    ...
                    $tofromusername = htmlspecialchars_uni(
                        $message['tousername']
                    );
                    $tofromuid = $message['toid'];
                } else {
            ...
            if (
                ...
                eval(
                    "\$denyreceipt = \"" .
                        $templates->get('private_messagebit_denyreceipt') .
                        "\";"
                );
            } else {
                $denyreceipt = '';
            ...
            $message['subject'] = htmlspecialchars_uni(
                $parser->parse_badwords($message['subject'])
            );
            if ($message['folder'] != '3') {
                $senddate = my_date('relative', $message['dateline']);
            } else {
            ...
            eval(
                "\$messagelist .= \"" .
                    $templates->get('private_messagebit') .
                    "\";"
            );
            $bgcolor = alt_trow();
        }
        ...
        eval(
            "\$messagelist .= \"" .
                $templates->get('private_nomessages') .
                "\";"
        );
    }
    ...
    if ($mybb->usergroup['pmquota'] != 0) {
        ...
        if ($pmscount['total'] == 0) {
            ...
            $spaceused =
                ($pmscount['total'] / $mybb->usergroup['pmquota']) * 100;
        }
    ...
    if ($mybb->usergroup['cansendpms'] == 1) {
        eval(
            "\$composelink = \"" .
                $templates->get('private_composelink') .
                "\";"
        );
    }
    ...
    if ($mybb->user['totalpms'] > 0) {
        eval(
            "\$emptyexportlink = \"" .
                $templates->get('private_emptyexportlink') .
                "\";"
        );
    }
    ...
    $limitwarning = '';
    if (
        $mybb->usergroup['pmquota'] != 0 &&
        $pmscount['total'] >= $mybb->usergroup['pmquota']
    ) {
        eval(
            "\$limitwarning = \"" .
                $templates->get('private_limitwarning') .
                "\";"
        );
    }
