...
$templatelist =
    'report,report_thanks,report_error,report_reasons,report_error_nomodal,forumdisplay_password_wrongpass,forumdisplay_password';
...
if ($report_type == 'post') {
    ...
    $query = $db->simple_select(
        'reputation',
        '*',
        "rid = '" . $mybb->get_input('pid', MyBB::INPUT_INT) . "'"
    );
...
if (!empty($report_type_db)) {
    $query = $db->simple_select(
        'reportedcontent',
        '*',
        "reportstatus != '1' AND id = '{$id}' AND {$report_type_db}"
    );
...
if (
    ...
    if (!empty($report)) {
        ...
        echo $report_thanks;
        echo sprintf(
            "<script type='text/javascript'>$('%s').remove();</script>",
            $button
        );
        exit();
    } else {
...
if (!$mybb->input['action']) {
    if (!empty($error)) {
        if ($mybb->input['no_modal']) {
            eval(
                "\$report_reasons = \"" .
                    $templates->get('report_error_nomodal') .
                    "\";"
            );
        } else {
            eval(
                "\$report_reasons = \"" .
                    $templates->get('report_error') .
                    "\";"
            );
        }
    } else {
        if (!empty($report)) {
            eval(
                "\$report_reasons = \"" .
                    $templates->get('report_duplicate') .
                    "\";"
            );
        } else {
            ...
            foreach ($reasons as $reason) {
                $reason['title'] = htmlspecialchars_uni(
                    $lang->parse($reason['title'])
                );
                eval(
                    "\$reasonslist .= \"" .
                        $templates->get('report_reason') .
                        "\";"
                );
            }
            eval(
                "\$report_reasons = \"" .
                    $templates->get('report_reasons') .
                    "\";"
            );
        }
    }
