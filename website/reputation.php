...
$templatelist =
    'reputation_addlink,reputation_no_votes,reputation,reputation_vote,multipage,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start,reputation_vote_delete';
$templatelist .=
    ',reputation_add_delete,reputation_add_neutral,reputation_add_positive,reputation_add_negative,reputation_add_error,reputation_add_error_nomodal,reputation_add,reputation_added,reputation_deleted,reputation_vote_report,postbit_reputation_formatted_link';
...
if ($mybb->input['action'] == 'add' || $mybb->input['action'] == 'do_add') {
    ...
    if ($mybb->usergroup['cangivereputations'] != 1) {
        ...
        if ($mybb->input['nomodal']) {
            eval(
                "\$error = \"" .
                    $templates->get('reputation_add_error_nomodal', 1, 0) .
                    "\";"
            );
        } else {
            eval(
                "\$error = \"" .
                    $templates->get('reputation_add_error', 1, 0) .
                    "\";"
            );
        }
        echo $error;
    ...
    if (
        $mybb->settings['multirep'] != 1 &&
        $mybb->get_input('pid', MyBB::INPUT_INT) == 0
    ) {
        $query = $db->simple_select(
            'reputation',
            '*',
            "adduid='" . $mybb->user['uid'] . "' AND uid='{$uid}' AND pid='0'"
        );
    ...
    if (
        $rid == 0 &&
        ($mybb->input['action'] != 'do_add' ||
            ($mybb->input['action'] == 'do_add' &&
                empty($mybb->input['delete'])))
    ) {
        $message = '';
        ...
        if ($mybb->usergroup['maxreputationsday'] != 0) {
            ...
            $query = $db->simple_select(
                'reputation',
                '*',
                "adduid='{$mybb->user['uid']}' AND dateline>'$timesearch'"
            );
        ...
        if (!$message && $mybb->usergroup['maxreputationsperuser'] != 0) {
            ...
            $query = $db->simple_select(
                'reputation',
                '*',
                "uid='{$uid}' AND adduid='{$mybb->user['uid']}' AND dateline>'$timesearch'"
            );
            $numtoday = $db->num_rows($query);
        ...
        if (
            !$message &&
            !empty($was_post) &&
            $mybb->usergroup['maxreputationsperthread'] != 0
        ) {
            $timesearch = TIME_NOW - 60 * 60 * 24;
            $query = $db->query(
                "
...
if ($mybb->input['action'] == 'do_add' && $mybb->request_method == 'post') {
    ...
    if (!empty($mybb->input['delete'])) {
        ...
        if (
            ...
            ($mybb->usergroup['candeletereputations'] != 1 ||
                $existing_reputation['adduid'] != $mybb->user['uid'] ||
                $mybb->user['uid'] == 0)
        ) {
            error_no_permission();
        }
        ...
        if ($mybb->get_input('pid', MyBB::INPUT_INT) != 0) {
            ...
            $db->delete_query(
                'reputation',
                "rid='{$rid}' AND uid='{$uid}' AND adduid='" .
                    $mybb->user['uid'] .
                    "'"
            );
        }
        ...
        $query = $db->simple_select(
            'reputation',
            'SUM(reputation) AS reputation_count',
            "uid='{$uid}'"
        );
        ...
        $db->update_query(
            'users',
            ['reputation' => (int) $reputation_value],
            "uid='{$uid}'"
        );
        eval(
            "\$error = \"" . $templates->get('reputation_deleted', 1, 0) . "\";"
        );
        echo $error;
        exit();
    }
    ...
    if (
        my_strlen($mybb->input['comments']) < $mybb->settings['minreplength'] &&
        $mybb->get_input('pid', MyBB::INPUT_INT) == 0
    ) {
    ...
    if (
        $mybb->get_input('reputation', MyBB::INPUT_INT) < 0 &&
        $mybb->settings['negrep'] != 1
    ) {
    ...
    eval(
        "\$reputation = \"" . $templates->get('reputation_added', 1, 0) . "\";"
    );
    echo $reputation;
    exit();
}
...
if ($mybb->input['action'] == 'add') {
    ...
    if (!empty($existing_reputation['uid'])) {
        $vote_title = $lang->sprintf(
            $lang->update_reputation_vote,
            $user['username']
        );
        ...
        if (
            ...
            eval(
                "\$delete_button = \"" .
                    $templates->get('reputation_add_delete') .
                    "\";"
            );
        }
    }
    ...
    $lang->user_comments = $lang->sprintf(
        $lang->user_comments,
        $user['username']
    );

    if ($mybb->get_input('pid', MyBB::INPUT_INT)) {
        $post_rep_info = $lang->sprintf(
            $lang->add_reputation_to_post,
            $user['username']
        );
    ...
    if (
        ...
        foreach (
            range(
                -$mybb->usergroup['reputationpower'],
                $mybb->usergroup['reputationpower']
            )
            as $value
        ) {
        ...
        if (!empty($existing_reputation['uid']) && !$was_post) {
            $vote_check[$existing_reputation['reputation']] =
                " selected=\"selected\"";
        }
        ...
        if ($mybb->settings['neurep']) {
            ...
            eval(
                "\$neutral_power = \"" .
                    $templates->get('reputation_add_neutral') .
                    "\";"
            );
        }
        ...
        for ($value = 1; $value <= $reputationpower; ++$value) {
            if ($mybb->settings['posrep']) {
                $positive_title = $lang->sprintf(
                    $lang->power_positive,
                    '+' . $value
                );
                eval(
                    "\$positive_power = \"" .
                        $templates->get('reputation_add_positive') .
                        "\";"
                );
            }

            if ($mybb->settings['negrep']) {
                $negative_title = $lang->sprintf(
                    $lang->power_negative,
                    '-' . $value
                );
                $neg_value = "-{$value}";
                eval(
                    "\$negative_power .= \"" .
                        $templates->get('reputation_add_negative') .
                        "\";"
                );
            }
        }
        ...
        eval(
            "\$reputation_add = \"" .
                $templates->get('reputation_add', 1, 0) .
                "\";"
        );
    } else {
...
if (!$mybb->input['action']) {
    ...
    $lang->reputation_report = $lang->sprintf(
    ...
    if (
        ...
        eval(
            "\$add_reputation = \"" .
                $templates->get('reputation_addlink') .
                "\";"
        );
    } else {
        $add_reputation = '';
    ...
    $sort_selected = ['username' => '', 'last_updated' => ''];
    ...
    $query = $db->simple_select(
        'reputation',
        'COUNT(rid) AS rep_posts',
        "uid = '" . $user['uid'] . "' AND pid > 0"
    );
    ...
    if ($reputation_count > 0) {
        $multipage = multipage(
            ...
            $perpage,
            $page,
            "reputation.php?uid={$user['uid']}" . $s_url
        );
    }
    ...
    $reputation_cache = $post_cache = $post_reputation = $not_reportable = [];
    ...
    while ($reputation_vote = $db->fetch_array($query)) {
        ...
        if (
            $reputation_vote['pid'] &&
            !isset($post_cache[$reputation_vote['pid']])
        ) {
    ...
    if (!empty($post_cache)) {
        ...
        while ($post = $db->fetch_array($query)) {
            if (
                ($post['visible'] == 0 || $post['thread_visible'] == 0) &&
            ...
            if (
                ($post['visible'] == -1 || $post['thread_visible'] == -1) &&
            ...
            if (!isset($forumpermissions[$post['fid']])) {
                $forumpermissions[$post['fid']] = forum_permissions(
                    $post['fid']
                );
            }
            ...
            if (
                isset(
                    $forumpermissions[$post['fid']]['canonlyviewownthreads']
                ) &&
    ...
    if (!empty($reputation_cache) && $mybb->user['uid'] != 0) {
        ...
        $query = $db->query(
            "
			SELECT id, reporters FROM " .
                TABLE_PREFIX .
                "reportedcontent WHERE reportstatus != '1' AND id IN (" .
                $reputation_ids .
                ") AND type = 'reputation'
		"
        );
        while ($report = $db->fetch_array($query)) {
            ...
            if (
                is_array($reporters) &&
                in_array($mybb->user['uid'], $reporters)
            ) {
    ...
    foreach ($reputation_cache as $reputation_vote) {
        ...
        $reputation_vote['user_reputation'] = get_reputation(
        ...
        if (!$reputation_vote['username']) {
            ...
            $reputation_vote['username'] = format_name(
                htmlspecialchars_uni($reputation_vote['username']),
                $reputation_vote['user_usergroup'],
                $reputation_vote['user_displaygroup']
            );
            $reputation_vote['username'] = build_profile_link(
        ...
        if ($reputation_vote['pid']) {
            $postrep_given = $lang->sprintf(
                $lang->postrep_given_nolink,
                $user['username']
            );
            if (isset($post_reputation[$reputation_vote['pid']])) {
                $thread_link = get_thread_link(
                    $post_reputation[$reputation_vote['pid']]['tid']
                );
                $subject = htmlspecialchars_uni(
                    $parser->parse_badwords(
                        $post_reputation[$reputation_vote['pid']]['subject']
                    )
                );

                $thread_link = $lang->sprintf(
                    $lang->postrep_given_thread,
                    $thread_link,
                    $subject
                );
                $link =
                    get_post_link($reputation_vote['pid']) .
                    "#pid{$reputation_vote['pid']}";
        ...
        if (
            $mybb->usergroup['issupermod'] == 1 ||
            ($mybb->usergroup['candeletereputations'] == 1 &&
                $reputation_vote['adduid'] == $mybb->user['uid'] &&
                $mybb->user['uid'] != 0)
        ) {
            eval(
                "\$delete_link = \"" .
                    $templates->get('reputation_vote_delete') .
                    "\";"
            );
        }
        ...
        if (
            $mybb->user['uid'] != 0 &&
            !in_array($reputation_vote['rid'], $not_reportable)
        ) {
            eval(
                "\$report_link = \"" .
                    $templates->get('reputation_vote_report') .
                    "\";"
            );
        }
        ...
        $reputation_vote['comments'] = $parser->parse_message(
    ...
    if (!$reputation_votes) {
        eval(
            "\$reputation_votes = \"" .
                $templates->get('reputation_no_votes') .
                "\";"
        );
    }
