...
$templatelist =
    'search,forumdisplay_thread_gotounread,search_results_threads_thread,search_results_threads,search_results_posts,search_results_posts_post,search_results_icon,search_forumlist_forum,search_forumlist';
$templatelist .=
    ',multipage,multipage_breadcrumb,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start';
$templatelist .=
    ',search_results_posts_inlinecheck,search_results_posts_nocheck,search_results_threads_inlinecheck,search_results_threads_nocheck,search_results_inlinemodcol,search_results_inlinemodcol_empty,search_results_posts_inlinemoderation_custom_tool';
$templatelist .=
    ',search_results_posts_inlinemoderation_custom,search_results_posts_inlinemoderation,search_results_threads_inlinemoderation_custom_tool,search_results_threads_inlinemoderation_custom,search_results_threads_inlinemoderation';
$templatelist .=
    ',forumdisplay_thread_attachment_count,search_threads_inlinemoderation_selectall,search_posts_inlinemoderation_selectall,post_prefixselect_prefix,post_prefixselect_multiple,search_orderarrow';
$templatelist .=
    ',search_results_posts_forumlink,search_results_threads_forumlink,forumdisplay_thread_multipage_more,forumdisplay_thread_multipage_page,forumdisplay_thread_multipage,search_moderator_options';
...
if ($mybb->input['action'] == 'results') {
    ...
    switch ($sortby) {
        ...
        default:
            if (
                isset($search['resulttype']) &&
                $search['resulttype'] == 'threads'
            ) {
    ...
    eval(
        "\$orderarrow['$sortby'] = \"" .
            $templates->get('search_orderarrow') .
            "\";"
    );
    ...
    if ($is_supermod || is_moderator()) {
        ...
        $is_mod = true;
        $return_url =
            'search.php?' . htmlspecialchars_uni($_SERVER['QUERY_STRING']);
    }
    ...
    if (isset($search['resulttype']) && $search['resulttype'] == 'threads') {
        ...
        if ($search['querycache'] != '') {
            ...
            $query = $db->simple_select(
                ...
                't.tid',
                $where_conditions .
                    " AND ({$unapproved_where_t}) AND t.closed NOT LIKE 'moved|%' ORDER BY t.lastpost DESC {$limitsql}"
            );
        ...
        // This search doesn't use a query cache, results stored in search table.
            ...
            $query = $db->simple_select(
                ...
                $where_conditions .
                    " AND ({$unapproved_where_t}) AND t.closed NOT LIKE 'moved|%' {$limitsql}"
            );
        ...
        if (
            ...
            $query = $db->simple_select(
                'posts',
                'DISTINCT tid,uid',
                "uid='{$mybb->user['uid']}' AND tid IN({$thread_ids}) AND ({$unapproved_where_p})"
            );
            while ($thread = $db->fetch_array($query)) {
        ...
        if ($mybb->user['uid'] && $mybb->settings['threadreadcut'] > 0) {
            ...
            while ($readthread = $db->fetch_array($query)) {
                $thread_cache[$readthread['tid']]['lastread'] =
                    $readthread['dateline'];
            }
        }
        ...
        foreach ($thread_cache as $thread) {
            ...
            if (isset($icon_cache[$thread['icon']])) {
                ...
                eval(
                    "\$icon = \"" .
                        $templates->get('search_results_icon') .
                        "\";"
                );
            } else {
                $icon = '&nbsp;';
            ...
            if ($thread['lastpost'] > $last_read && $last_read) {
                ...
                $thread['newpostlink'] =
                    get_thread_link($thread['tid'], 0, 'newpost') . $highlight;
                eval(
                    "\$gotounread = \"" .
            ...
            if (
                is_moderator($thread['fid'], 'canviewdeleted') == true ||
                is_moderator($thread['fid'], 'canviewunapprove') == true
            ) {
                ...
                $group_permissions[$thread['fid']]['canviewdeletionnotice'] != 0
            ) {
                $thread['posts'] += $thread['deletedposts'];
            }
            ...
            if ($thread['posts'] > $mybb->settings['postsperpage']) {
                ...
                if ($thread['pages'] > $mybb->settings['maxmultipagelinks']) {
                    ...
                    $page_link =
                        get_thread_link($thread['tid'], $thread['pages']) .
                        $highlight;
                    eval(
                        "\$morelink = \"" .
                            $templates->get(
                ...
                for ($i = 1; $i <= $pagesstop; ++$i) {
                    $page_link =
                        get_thread_link($thread['tid'], $i) . $highlight;
                    eval(
                        "\$threadpages .= \"" .
                            $templates->get(
            ...
            if ($forumcache[$thread['fid']]) {
                ...
                eval(
                    "\$thread['forumlink'] = \"" .
                        $templates->get('search_results_threads_forumlink') .
                        "\";"
                );
            }
            ...
            if (
                ...
                is_moderator($thread['fid'], 'caneditposts')
            ) {
        ...
        $multipage = multipage(
            ...
            $perpage,
            $page,
            "search.php?action=results&amp;sid=$sid&amp;sortby=$sortby&amp;order=$order&amp;uid=" .
        ...
        if ($show_inline_moderation) {
            eval(
                "\$inlinemodcol = \"" .
                    $templates->get('search_results_inlinemodcol') .
                    "\";"
            );
            ...
            switch ($db->type) {
                ...
                case 'sqlite':
                    $query = $db->simple_select(
                        'modtools',
                        'tid, name',
                        "type='t' AND (','||forums||',' LIKE '%,-1,%' OR forums='')"
                    );
                    break;
                default:
                    $query = $db->simple_select(
                        'modtools',
                        'tid, name',
                        "type='t' AND (CONCAT(',',forums,',') LIKE '%,-1,%' OR forums='')"
                    );
            }
            ...
            eval(
                "\$inlinemod = \"" .
                    $templates->get('search_results_threads_inlinemoderation') .
                    "\";"
            );
        } elseif ($is_mod) {
            eval(
                "\$inlinemodcol = \"" .
                    $templates->get('search_results_inlinemodcol_empty') .
                    "\";"
            );
        }
        ...
        eval(
            "\$searchresults = \"" .
                $templates->get('search_results_threads') .
                "\";"
        );
        output_page($searchresults);
    }
    // Displaying results as posts
    else {
        ...
        $post_cache_options = [];
        if ((int) $mybb->settings['searchhardlimit'] > 0) {
            $post_cache_options['limit'] =
                (int) $mybb->settings['searchhardlimit'];
        }
        ...
        $query = $db->simple_select(
            ...
            'pid, tid',
            'pid IN(' .
                $db->escape_string($search['posts']) .
                ") AND ({$unapproved_where})",
            $post_cache_options
        );
        ...
        if (!empty($pids)) {
            ...
            $query = $db->simple_select(
                ...
                'tid',
                'tid IN(' .
                    $db->escape_string(implode(',', array_keys($tids))) .
                    ") AND (NOT ({$unapproved_where}){$permsql} OR closed LIKE 'moved|%')"
            );
            while ($thread = $db->fetch_array($query)) {
        ...
        if ($mybb->settings['dotfolders'] != 0 && $mybb->user['uid'] != 0) {
            $query = $db->simple_select(
                'posts',
                'DISTINCT tid,uid',
                "uid='{$mybb->user['uid']}' AND tid IN({$db->escape_string(
                    $tids
                )}) AND ({$unapproved_where})"
            );
            while ($post = $db->fetch_array($query)) {
        ...
        while ($post = $db->fetch_array($query)) {
            ...
            $post['thread_subject'] = $parser->parse_badwords(
                $post['thread_subject']
            );
            $post['thread_subject'] = htmlspecialchars_uni(
                $post['thread_subject']
            );
            ...
            if (!empty($forumcache[$post['fid']])) {
                ...
                eval(
                    "\$post['forumlink'] = \"" .
                        $templates->get('search_results_posts_forumlink') .
                        "\";"
                );
            }
            ...
            if (
                $post['thread_replies'] >= $mybb->settings['hottopic'] ||
                $post['thread_views'] >= $mybb->settings['hottopicviews']
            ) {
                $folder .= 'hot';
                $folder_label .= $lang->icon_hot;
            }
            ...
            $post['message'] = strip_tags(
                $parser->text_parse_message($post['message'], $parser_options)
            );
            ...
            eval(
                "\$results .= \"" .
                    $templates->get('search_results_posts_post') .
                    "\";"
            );
        }
        if (!$results) {
        ...
        $multipage = multipage(
            ...
            $perpage,
            $page,
            'search.php?action=results&amp;sid=' .
                htmlspecialchars_uni($mybb->get_input('sid')) .
                "&amp;sortby=$sortby&amp;order=$order&amp;uid=" .
                $mybb->get_input('uid', MyBB::INPUT_INT)
        );
        if ($upper > $postcount) {
            $upper = $postcount;
        ...
        if ($show_inline_moderation) {
            ...
            eval(
                "\$selectall = \"" .
                    $templates->get('search_posts_inlinemoderation_selectall') .
                    "\";"
            );
            ...
            eval(
                "\$inlinemod = \"" .
                    $templates->get('search_results_posts_inlinemoderation') .
                    "\";"
            );
        } elseif ($is_mod) {
            eval(
        ...
        eval(
            "\$searchresults = \"" .
                $templates->get('search_results_posts') .
                "\";"
        );
        output_page($searchresults);
    }
    ...
    redirect(
        'search.php?action=results&sid=' . $sid,
    ...
    if (!empty($onlyusfids)) {
        $where_sql .=
            'AND ((fid IN(' .
            implode(',', $onlyusfids) .
            ") AND uid='{$mybb->user['uid']}') OR fid NOT IN(" .
            implode(',', $onlyusfids) .
            '))';
    }
    ...
    if ($mybb->get_input('fid', MyBB::INPUT_INT)) {
        $where_sql .=
            " AND fid='" . $mybb->get_input('fid', MyBB::INPUT_INT) . "'";
    ...
    if (
        my_strtolower($mybb->get_input('sortordr')) == 'asc' ||
        my_strtolower($mybb->get_input('sortordr') == 'desc')
    ) {
    ...
    if (is_moderator()) {
        ...
        eval(
            "\$moderator_options = \"" .
                $templates->get('search_moderator_options') .
                "\";"
        );
    }
