...
$templatelist =
    'sendthread,sendthread_fromemail,forumdisplay_password_wrongpass,forumdisplay_password,post_captcha,post_captcha_nocaptcha,post_captcha_hcaptcha';
...
if (
    $mybb->input['action'] == 'do_sendtofriend' &&
...
if (!$mybb->input['action']) {
    ...
    if (count($errors) >= 1) {
        ...
        $subject = $lang->sprintf(
            $lang->emailsubject_sendtofriend,
            $mybb->settings['bbname']
        );
        $message = '';
    }
    ...
    if ($mybb->user['uid'] == 0) {
        eval(
            "\$from_email = \"" .
                $templates->get('sendthread_fromemail') .
                "\";"
        );
    }
