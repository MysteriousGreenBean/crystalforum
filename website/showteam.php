...
$templatelist =
    'showteam,postbit_email,postbit_pm,showteam_usergroup_user,showteam_usergroup,showteam_moderators_mod,showteam_moderators,showteam_moderators_forum';
...
if ($mybb->settings['showgroupleaders']) {
    ...
    if (!empty($leaders)) {
        ...
        while ($leaded_group = $db->fetch_array($query)) {
            $leaded_groups[$leaded_group['gid']] = str_replace(
                '{username}',
                $leaded_group['title'],
                $leaded_group['namestyle']
            );
        }
        ...
        $usergroups[0] = [
            'gid' => 0,
            'title' => $lang->group_leaders,
            'usertitle' => $lang->group_leaders,
        ];
        foreach ($leaderlist as $uid => $leaded) {
            foreach ($leaded as $gid) {
                $leadlist[] = $leaded_groups[$gid];
            }
            $usergroups[0]['user_list'][$uid]['leaded'] = implode(
                ', ',
                $leadlist
            );
            unset($leadlist);
        ...
        $users_in = implode(
            ...
            array_keys(
                array_flip(
                    explode(
                        ',',
                        implode(',', array_keys($leaderlist)) . ',' . $users_in
                    )
                )
            )
        );
...
$query = $db->simple_select(
    'users',
    'uid, username, displaygroup, usergroup, additionalgroups, ignorelist, hideemail, receivepms, lastactive, lastvisit, invisible, away',
    $query_part .
...
while ($user = $db->fetch_array($query)) {
    ...
    if (isset($moderators[$user['uid']])) {
        ...
        foreach ($moderators[$user['uid']] as $forum) {
            if ($forum_permissions[$forum['fid']]['canview'] == 1) {
                ...
                eval(
                    "\$forumlist .= \"" .
                        $templates->get('showteam_moderators_forum') .
                        "\";"
                );
            }
        }
    ...
    if (
        $mybb->settings['showgroupleaders'] &&
        isset($usergroups[0]['user_list'])
    ) {
...
foreach ($usergroups as $usergroup) {
    ...
    foreach ($usergroup['user_list'] as $user) {
        ...
        if (
            ($usergroup['gid'] == 0 && !empty($user['leaded'])) ||
            ($usergroup['gid'] == 6 && !empty($user['forumlist']))
        ) {
            $scopelist =
                $usergroup['gid'] == 0 ? $user['leaded'] : $user['forumlist'];
            eval(
                "\$modrows .= \"" .
                    $templates->get('showteam_moderators_mod') .
                    "\";"
            );
        } else {
            eval(
                "\$usergrouprows .= \"" .
                    $templates->get('showteam_usergroup_user') .
                    "\";"
            );
        }
    }
    ...
    if ($modrows && in_array($usergroup['gid'], [0, 6])) {
        $modscope =
            $usergroup['gid'] == 6 ? $lang->mod_forums : $lang->mod_groups;
        eval(
