...
$templatelist =
    'showthread,postbit,postbit_author_user,postbit_author_guest,showthread_newthread,showthread_newreply,showthread_newreply_closed,postbit_avatar,postbit_find,postbit_pm,postbit_www,postbit_email,postbit_edit,postbit_quote,postbit_report';
$templatelist .=
    ',multipage,multipage_breadcrumb,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start,showthread_inlinemoderation_softdelete,showthread_poll_editpoll';
$templatelist .=
    ',postbit_editedby,showthread_similarthreads,showthread_similarthreads_bit,postbit_iplogged_show,postbit_iplogged_hiden,postbit_profilefield,showthread_quickreply,showthread_printthread,showthread_add_poll,showthread_send_thread,showthread_inlinemoderation_restore';
$templatelist .=
    ',forumjump_advanced,forumjump_special,forumjump_bit,postbit_attachments,postbit_attachments_attachment,postbit_attachments_thumbnails,postbit_attachments_images_image,postbit_attachments_images,showthread_quickreply_options_stick,postbit_status';
$templatelist .=
    ',postbit_inlinecheck,showthread_inlinemoderation,postbit_attachments_thumbnails_thumbnail,postbit_ignored,postbit_multiquote,showthread_moderationoptions_custom_tool,showthread_moderationoptions_custom,showthread_inlinemoderation_custom_tool';
$templatelist .=
    ',showthread_usersbrowsing,showthread_usersbrowsing_user,showthread_poll_option,showthread_poll,showthread_quickreply_options_signature,showthread_threaded_bitactive,showthread_threaded_bit,postbit_attachments_attachment_unapproved';
$templatelist .=
    ',showthread_moderationoptions_openclose,showthread_moderationoptions_stickunstick,showthread_moderationoptions_delete,showthread_moderationoptions_threadnotes,showthread_moderationoptions_manage,showthread_moderationoptions_deletepoll';
$templatelist .=
    ',postbit_userstar,postbit_reputation_formatted_link,postbit_warninglevel_formatted,postbit_quickrestore,forumdisplay_password,forumdisplay_password_wrongpass,postbit_purgespammer,showthread_inlinemoderation_approve,forumdisplay_thread_icon';
$templatelist .=
    ',showthread_moderationoptions_softdelete,showthread_moderationoptions_restore,post_captcha,post_captcha_recaptcha_invisible,post_captcha_nocaptcha,post_captcha_hcaptcha_invisible,post_captcha_hcaptcha,showthread_moderationoptions,showthread_inlinemoderation_standard,showthread_inlinemoderation_manage';
$templatelist .=
    ',showthread_ratethread,postbit_posturl,postbit_icon,postbit_editedby_editreason,attachment_icon,global_moderation_notice,showthread_poll_option_multiple,postbit_gotopost,postbit_rep_button,postbit_warninglevel,showthread_threadnoteslink';
$templatelist .=
    ',showthread_moderationoptions_approve,showthread_moderationoptions_unapprove,showthread_inlinemoderation_delete,showthread_moderationoptions_standard,showthread_quickreply_options_close,showthread_inlinemoderation_custom,showthread_search';
$templatelist .=
    ',postbit_profilefield_multiselect_value,postbit_profilefield_multiselect,showthread_subscription,postbit_deleted_member,postbit_away,postbit_warn,postbit_reputation,postbit_deleted,postbit_offline,postbit_online,postbit_signature';
$templatelist .=
    ',postbit_editreason,postbit_quickdelete,showthread_threadnotes_viewnotes,showthread_threadedbox,showthread_poll_resultbit,showthread_poll_results,showthread_threadnotes,showthread_poll_undovote,postbit_groupimage';
...
if (!empty($mybb->input['pid']) && !isset($mybb->input['tid'])) {
    ...
    if (
        isset($style) &&
        $style['pid'] == $mybb->get_input('pid', MyBB::INPUT_INT) &&
        $style['tid']
    ) {
            ...
            ($post['visible'] == 0 &&
                !(
                    ...
                    ($mybb->user['uid'] &&
                        ...
                        $mybb->settings['showownunapproved'])
                )) ||
            ($post['visible'] == -1 &&
...
if ($thread['prefix'] != 0) {
    ...
    if (!empty($threadprefix['prefix'])) {
        $thread['threadprefix'] =
            htmlspecialchars_uni($threadprefix['prefix']) . '&nbsp;';
...
$visible_condition =
    'visible IN (' . implode(',', array_unique($visible_states)) . ')';
...
if ($mybb->user['uid'] && $mybb->settings['showownunapproved']) {
    ...
    $visibleonly_p_t =
        sprintf($own_unapproved, 'p.') . sprintf($own_unapproved, 't.');
} else {
    $visibleonly = ' AND ' . $visible_condition;
    $visibleonly_p = ' AND p.' . $visible_condition;
    $visibleonly_p_t =
        'AND p.' . $visible_condition . ' AND t.' . $visible_condition;
}

// Make sure we are looking at a real thread here.
if (
    ($thread['visible'] != 1 && $ismod == false) ||
    ($thread['visible'] == 0 && !is_moderator($fid, 'canviewunapprove')) ||
    ($thread['visible'] == -1 && !is_moderator($fid, 'canviewdeleted'))
) {
...
if (
    $forumpermissions['canview'] != 1 ||
    $forumpermissions['canviewthreads'] != 1
) {
    error_no_permission();
}
...
if (is_moderator($fid, 'canmanagethreads') && !empty($thread['notes'])) {
    eval(
        '$threadnoteslink = "' .
            $templates->get('showthread_threadnoteslink') .
            '";'
    );
}
...
if ($mybb->input['action'] == 'newpost') {
    ...
    if (!$lastread) {
        $readcookie = $threadread = (int) my_get_array_cookie(
            'threadread',
            $thread['tid']
        );
    ...
    if ($newpost && $lastread) {
        ...
        header(
            'Location: ' .
                htmlspecialchars_decode(get_post_link($newpost['pid'], $tid)) .
                $highlight .
                "#pid{$newpost['pid']}"
        );
    } else {
...
if ($mybb->input['action'] == 'lastpost') {
    if (my_strpos($thread['closed'], 'moved|')) {
        $query = $db->query(
			...
			WHERE t.fid='" .
                $thread['fid'] .
                "' AND t.closed NOT LIKE 'moved|%' {$visibleonly_p_t}
    ...
    header(
        'Location: ' .
            htmlspecialchars_decode(get_post_link($pid, $tid)) .
            "#pid{$pid}"
    );
    exit();
}
...
if ($mybb->input['action'] == 'nextnewest') {
    ...
    $query = $db->simple_select(
        'threads',
        '*',
        "fid={$thread['fid']} AND lastpost > {$thread['lastpost']} {$visibleonly} AND closed NOT LIKE 'moved|%'",
        $options
    );
    ...
    header(
        'Location: ' .
            htmlspecialchars_decode(get_post_link($pid, $nextthread['tid'])) .
            "#pid{$pid}"
    );
    exit();
}
...
if ($mybb->input['action'] == 'nextoldest') {
    ...
    $query = $db->simple_select(
        ...
        'fid=' .
            ...
            ' AND lastpost < ' .
            $thread['lastpost'] .
            " {$visibleonly} AND closed NOT LIKE 'moved|%'",
        $options
    );
    $nextthread = $db->fetch_array($query);
...
if ($mybb->settings['showforumpagesbreadcrumb']) {
    ...
    $query = $db->simple_select(
        'forums',
        'threads, unapprovedthreads, deletedthreads',
        "fid = '{$fid}'",
        ['limit' => 1]
    );
    ...
    if (
        is_moderator($fid, 'canviewdeleted') == true ||
        is_moderator($fid, 'canviewunapprove') == true
    ) {
        if (is_moderator($fid, 'canviewdeleted') == true) {
        ...
        case 'pgsql':
            $query = $db->query(
				...
				FROM " .
                    TABLE_PREFIX .
                    "threads
				WHERE fid = '$fid' AND (lastpost >= '" .
                    (int) $thread['lastpost'] .
                    "'{$stickybit}) {$visibleonly} {$uid_only}
				GROUP BY lastpost
...
if ($mybb->input['action'] == 'thread') {
    ...
    if ($thread['poll']) {
        ...
        if (
            ...
            $forumpermissions['canvotepolls'] != 1
        ) {
            $showresults = 1;
        }
        ...
        for ($i = 1; $i <= $poll['numoptions']; ++$i) {
            ...
            if (
                ($mybb->user['uid'] != 0 && $mybb->user['showvideos'] != 1) ||
                ($mybb->settings['guestvideos'] != 1 && $mybb->user['uid'] == 0)
            ) {
                $parser_options['allow_videocode'] = 0;
            }

            $option = $parser->parse_message(
                $optionsarray[$i - 1],
                $parser_options
            );
            ...
            if (isset($alreadyvoted) || isset($showresults)) {
                ...
                eval(
                    "\$polloptions .= \"" .
                        $templates->get('showthread_poll_resultbit') .
                        "\";"
                );
            } else {
                if ($poll['multiple'] == 1) {
                    eval(
                        "\$polloptions .= \"" .
                            $templates->get('showthread_poll_option_multiple') .
                            "\";"
                    );
                } else {
                    eval(
                        "\$polloptions .= \"" .
                            $templates->get('showthread_poll_option') .
                            "\";"
                    );
                }
            }
        ...
        if (is_moderator($fid, 'canmanagepolls')) {
            eval(
                "\$edit_poll = \"" .
                    $templates->get('showthread_poll_editpoll') .
                    "\";"
            );
        }
        ...
        if (
            ...
            if (isset($alreadyvoted)) {
                ...
                if ($mybb->usergroup['canundovotes'] == 1) {
                    eval(
                        "\$undovote = \"" .
                            $templates->get('showthread_poll_undovote') .
                            "\";"
                    );
                }
            ...
            $lang->total_votes = $lang->sprintf(
                $lang->total_votes,
                $totalvotes
            );
            eval(
                "\$pollbox = \"" .
                    $templates->get('showthread_poll_results') .
                    "\";"
            );
    ...
    if ($forum['open'] != 0 && $forum['type'] == 'f') {
        if (
            ...
            eval(
                "\$newthread = \"" .
                    $templates->get('showthread_newthread') .
                    "\";"
            );
        }
        ...
        if (
            ...
            $mybb->user['suspendposting'] != 1 &&
            ($thread['closed'] != 1 ||
                is_moderator($fid, 'canpostclosedthreads')) &&
            ($thread['uid'] == $mybb->user['uid'] ||
                empty($forumpermissions['canonlyreplyownthreads']))
        ) {
            eval(
                "\$newreply = \"" .
                    $templates->get('showthread_newreply') .
                    "\";"
            );
        } elseif ($thread['closed'] == 1) {
            eval(
                "\$newreply = \"" .
                    $templates->get('showthread_newreply_closed') .
                    "\";"
            );
        }
    }
    ...
    if ($ismod == true) {
        ...
        if (is_moderator($thread['fid'], 'canopenclosethreads')) {
            eval(
                "\$closeoption .= \"" .
                    $templates->get('showthread_quickreply_options_close') .
                    "\";"
            );
        }
        ...
        if (is_moderator($thread['fid'], 'canstickunstickthreads')) {
            eval(
                "\$closeoption .= \"" .
                    $templates->get('showthread_quickreply_options_stick') .
                    "\";"
            );
        }
    ...
    if (
        ($mybb->user['uid'] == 0 &&
            (($session->is_spider == true &&
                $mybb->settings['threadviews_countspiders'] == 1) ||
                ($session->is_spider == false &&
                    $mybb->settings['threadviews_countguests'] == 1))) ||
        ($mybb->user['uid'] != 0 &&
            ($mybb->settings['threadviews_countthreadauthor'] == 1 ||
        ...
        if ($mybb->settings['delayedthreadviews'] == 1) {
            $db->shutdown_query(
                'INSERT INTO ' .
                    TABLE_PREFIX .
                    "threadviews (tid) VALUES('{$tid}')"
            );
        } else {
            $db->shutdown_query(
                'UPDATE ' .
                    TABLE_PREFIX .
                    "threads SET views=views+1 WHERE tid='{$tid}'"
            );
        }
        ++$thread['views'];
    }
    ...
    $rating = $ratethread = '';
    if (
        $mybb->settings['allowthreadratings'] != 0 &&
        $forum['allowtratings'] != 0
    ) {
        $rated = 0;
        ...
        if ($thread['numratings'] <= 0) {
            ...
            $thread['averagerating'] = (float) round(
                $thread['totalratings'] / $thread['numratings'],
                2
            );
        ...
        $ratingvotesav = $lang->sprintf(
            $lang->rating_average,
            $thread['numratings'],
            $thread['averagerating']
        );
        eval(
            "\$ratethread = \"" .
                $templates->get('showthread_ratethread') .
                "\";"
        );
    }
    ...
    if ($forumpermissions['cansearch'] != 0) {
        eval(
            "\$search_thread = \"" .
                $templates->get('showthread_search') .
                "\";"
        );
    }
    ...
    } elseif ($mybb->settings['threadusenetstyle'] == 1) {
    ...
    if ($mybb->get_input('mode') == 'threaded') {
        ...
        $query = $db->query(
			...
			LEFT JOIN " .
                TABLE_PREFIX .
                "reportedcontent r ON (r.id=p.pid AND r.type='post' AND r.reportstatus != 1)
			LEFT JOIN " .
                TABLE_PREFIX .
                "users u ON (u.uid=p.uid)
			LEFT JOIN " .
                TABLE_PREFIX .
                "userfields f ON (f.ufid=u.uid)
			LEFT JOIN " .
                TABLE_PREFIX .
                "users eu ON (eu.uid=p.edituid)
        ...
        eval(
            "\$threadexbox = \"" .
                $templates->get('showthread_threadedbox') .
                "\";"
        );
    ...
    // Linear display
        ...
        if (!empty($mybb->input['pid'])) {
            ...
            if (
                ...
                ($post['visible'] == -1 &&
                    !is_moderator($post['fid'], 'canviewdeleted') &&
                    $forumpermissions['canviewdeletionnotice'] == 0)
            ) {
                $footer .=
                    '<script type="text/javascript">$(function() { $.jGrowl(\'' .
                    $lang->error_invalidpost .
                    '\', {theme: \'jgrowl_error\'}); });</script>';
            } else {
                $query = $db->query(
                    "
					SELECT COUNT(p.dateline) AS count FROM " .
                        TABLE_PREFIX .
                        "posts p
					WHERE p.tid = '{$tid}'
        ...
        if ($visible_states != ['1']) {
            ...
            if (
                in_array('-1', $visible_states) &&
                in_array('0', $visible_states)
            ) {
        ...
        if ($mybb->seo_support == true) {
            if ($mybb->get_input('highlight')) {
                $highlight =
                    '?highlight=' . urlencode($mybb->get_input('highlight'));
            }
            ...
            if (!empty($mybb->input['highlight'])) {
                if (is_array($mybb->input['highlight'])) {
                    foreach ($mybb->input['highlight'] as $highlight_word) {
                        $highlight .=
                            '&amp;highlight[]=' . urlencode($highlight_word);
                    }
                } else {
                    $highlight =
                        '&amp;highlight=' .
                        urlencode($mybb->get_input('highlight'));
                }
            }
    ...
    if ($mybb->settings['showsimilarthreads'] != 0) {
        ...
        switch ($db->type) {
            case 'pgsql':
                $query = $db->query(
					...
					LEFT JOIN " .
                        TABLE_PREFIX .
                        "users u ON (u.uid = t.uid), plainto_tsquery ('" .
                        $db->escape_string($thread['subject']) .
                        "') AS query
            ...
            default:
                $query = $db->query(
					...
					WHERE t.fid='{$thread['fid']}' AND t.tid!='{$thread['tid']}' AND t.visible='1' AND t.closed NOT LIKE 'moved|%'{$own_perm} AND MATCH (t.subject) AGAINST ('" .
                        $db->escape_string($thread['subject']) .
                        "') >= '{$mybb->settings['similarityrating']}'
					ORDER BY t.lastpost DESC
        ...
        while ($similar_thread = $db->fetch_array($query)) {
            ...
            $trow = alt_trow();
            if (
                $similar_thread['icon'] > 0 &&
                $icon_cache[$similar_thread['icon']]
            ) {
                ...
                eval(
                    "\$icon = \"" .
                        $templates->get('forumdisplay_thread_icon') .
                        "\";"
                );
            } else {
                $icon = '&nbsp;';
            ...
            if (!$similar_thread['username']) {
                $similar_thread['username'] = $similar_thread[
                    'profilelink'
                ] = htmlspecialchars_uni($similar_thread['threadusername']);
            } else {
                $similar_thread['username'] = htmlspecialchars_uni(
                    $similar_thread['username']
                );
                $similar_thread['profilelink'] = build_profile_link(
                    $similar_thread['username'],
                    $similar_thread['uid']
                );
            }
            ...
            $similar_thread['subject'] = $parser->parse_badwords(
                $similar_thread['subject']
            );
            $similar_thread['subject'] = htmlspecialchars_uni(
                $similar_thread['subject']
            );
            $similar_thread['threadlink'] = get_thread_link(
                $similar_thread['tid']
            );
            $similar_thread['lastpostlink'] = get_thread_link(
                $similar_thread['tid'],
                0,
                'lastpost'
            );
            ...
            $similar_thread['replies'] = my_number_format(
                $similar_thread['replies']
            );
            $similar_thread['views'] = my_number_format(
                $similar_thread['views']
            );
            eval(
                "\$similarthreadbits .= \"" .
        ...
        if ($count) {
            eval(
                "\$similarthreads = \"" .
                    $templates->get('showthread_similarthreads') .
                    "\";"
            );
        }
    }
    ...
    if (
        ...
        if ($mybb->usergroup['canusesig'] && !$mybb->user['suspendsignature']) {
            eval(
                "\$option_signature = \"" .
                    $templates->get('showthread_quickreply_options_signature') .
                    "\";"
            );
        }

        if (
            isset($mybb->user['emailnotify']) &&
            $mybb->user['emailnotify'] == 1
        ) {
        ...
        if (!is_moderator($forum['fid'], 'canapproveunapproveposts')) {
            if ($forumpermissions['modposts'] == 1) {
                ...
                eval(
                    '$moderation_notice = "' .
                        $templates->get('global_moderation_notice') .
                        '";'
                );
            }
        ...
        eval(
            "\$quickreply = \"" .
                $templates->get('showthread_quickreply') .
                "\";"
        );
    }
    ...
    if ($ismod) {
        ...
        if (!empty($thread['notes'])) {
            ...
            if (strlen($thread['notes']) > 200) {
                eval(
                    "\$viewnotes = \"" .
                        $templates->get('showthread_threadnotes_viewnotes') .
                        "\";"
                );
                $thread['notes'] =
                    my_substr($thread['notes'], 0, 200) . "... {$viewnotes}";
            }
            ...
            eval(
                "\$threadnotesbox = \"" .
                    $templates->get('showthread_threadnotes') .
                    "\";"
            );
        }

        if (
            is_moderator($forum['fid'], 'canusecustomtools') &&
            (!empty($forum_stats[-1]['modtools']) ||
                !empty($forum_stats[$forum['fid']]['modtools']))
        ) {
            ...
            if (!empty($customposttools)) {
                eval(
                    "\$customposttools = \"" .
                        $templates->get('showthread_inlinemoderation_custom') .
                        "\";"
                );
            }
        }
        ...
        if (is_moderator($forum['fid'], 'cansoftdeleteposts')) {
            eval(
                "\$inlinemodsoftdelete = \"" .
                    $templates->get('showthread_inlinemoderation_softdelete') .
                    "\";"
            );
        }
        ...
        if (is_moderator($forum['fid'], 'canrestoreposts')) {
            eval(
                "\$inlinemodrestore = \"" .
                    $templates->get('showthread_inlinemoderation_restore') .
                    "\";"
            );
        }
        ...
        if (is_moderator($forum['fid'], 'candeleteposts')) {
            eval(
                "\$inlinemoddelete = \"" .
                    $templates->get('showthread_inlinemoderation_delete') .
                    "\";"
            );
        }
        ...
        if (is_moderator($forum['fid'], 'canmanagethreads')) {
            eval(
                "\$inlinemodmanage = \"" .
                    $templates->get('showthread_inlinemoderation_manage') .
                    "\";"
            );
        }
        ...
        if (is_moderator($forum['fid'], 'canapproveunapproveposts')) {
            eval(
                "\$inlinemodapprove = \"" .
                    $templates->get('showthread_inlinemoderation_approve') .
                    "\";"
            );
        }

        if (
            !empty($inlinemodsoftdelete) ||
            ...
            eval(
                "\$standardposttools = \"" .
                    $templates->get('showthread_inlinemoderation_standard') .
                    "\";"
            );
        }
        ...
        if (!empty($standardposttools) || !empty($customposttools)) {
            eval(
                "\$inlinemod = \"" .
                    $templates->get('showthread_inlinemoderation') .
                    "\";"
            );
        }
        ...
        if (!empty($customthreadtools)) {
            eval(
                "\$customthreadtools = \"" .
                    $templates->get('showthread_moderationoptions_custom') .
                    "\";"
            );
        }
        ...
        if (is_moderator($forum['fid'], 'canopenclosethreads')) {
            ...
            eval(
                "\$openclosethread = \"" .
                    $templates->get('showthread_moderationoptions_openclose') .
                    "\";"
            );
        }
        ...
        if (is_moderator($forum['fid'], 'candeletethreads')) {
            eval(
                "\$deletethread = \"" .
                    $templates->get('showthread_moderationoptions_delete') .
                    "\";"
            );
        }
        ...
        if (is_moderator($forum['fid'], 'canmanagethreads')) {
            ...
            eval(
                "\$managethread = \"" .
                    $templates->get('showthread_moderationoptions_manage') .
                    "\";"
            );
        }
        ...
        if ($pollbox && is_moderator($forum['fid'], 'canmanagepolls')) {
            eval(
                "\$adminpolloptions = \"" .
                    $templates->get('showthread_moderationoptions_deletepoll') .
                    "\";"
            );
        }
        ...
        if (
            is_moderator($forum['fid'], 'cansoftdeletethreads') &&
            $thread['visible'] != -1
        ) {
            eval(
                "\$softdeletethread = \"" .
                    $templates->get('showthread_moderationoptions_softdelete') .
                    "\";"
            );
        } elseif (
            is_moderator($forum['fid'], 'canrestorethreads') &&
            $thread['visible'] == -1
        ) {
            eval(
                "\$softdeletethread = \"" .
                    $templates->get('showthread_moderationoptions_restore') .
                    "\";"
            );
        }

        if (
            !empty($openclosethread) ||
            ...
            eval(
                "\$standardthreadtools = \"" .
                    $templates->get('showthread_moderationoptions_standard') .
                    "\";"
            );
        }
        ...
        if (!empty($standardthreadtools) || !empty($customthreadtools)) {
            eval(
                "\$moderationoptions = \"" .
                    $templates->get('showthread_moderationoptions') .
                    "\";"
            );
        }
    }
    ...
    eval(
        "\$printthread = \"" . $templates->get('showthread_printthread') . "\";"
    );
    ...
    if ($mybb->usergroup['cansendemail'] == 1) {
        eval(
            "\$sendthread = \"" .
                $templates->get('showthread_send_thread') .
                "\";"
        );
    }
    ...
    if (
        !$thread['poll'] &&
        ($thread['uid'] == $mybb->user['uid'] || $ismod == true) &&
        $forumpermissions['canpostpolls'] == 1 &&
        $forum['open'] != 0 &&
        $thread['closed'] != 1 &&
        ($ismod == true ||
            $thread['dateline'] >
                $time - $mybb->settings['polltimelimit'] * 60 * 60 ||
            $mybb->settings['polltimelimit'] == 0)
    ) {
    ...
    if ($mybb->user['uid']) {
        ...
        eval(
            "\$addremovesubscription = \"" .
                $templates->get('showthread_subscription') .
                "\";"
        );
    } else {
        $addremovesubscription = '';
    ...
    if ($mybb->settings['browsingthisthread'] != 0) {
        ...
        $query = $db->simple_select(
            'sessions',
            'COUNT(DISTINCT ip) AS guestcount',
            "uid = 0 AND time > $timecut AND location2 = $tid AND nopermission != 1"
        );
        ...
        $guestsonline = '';
        if ($guestcount) {
            $guestsonline = $lang->sprintf(
                $lang->users_browsing_thread_guests,
                $guestcount
            );
        }
        ...
        if ($inviscount && $mybb->usergroup['canviewwolinvis'] != 1) {
            $invisonline = $lang->sprintf(
                $lang->users_browsing_thread_invis,
                $inviscount
            );
        }
        ...
        eval(
            "\$usersbrowsing = \"" .
                $templates->get('showthread_usersbrowsing') .
                "\";"
        );
    }
    ...
    if (is_array($tree[$replyto])) {
        foreach ($tree[$replyto] as $key => $post) {
            ...
            if ($mybb->input['pid'] == $post['pid']) {
                eval(
                    "\$posts .= \"" .
                        $templates->get('showthread_threaded_bitactive') .
                        "\";"
                );
            } else {
                eval(
                    "\$posts .= \"" .
                        $templates->get('showthread_threaded_bit') .
                        "\";"
                );
            }
