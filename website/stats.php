...
$repliesperthread = my_number_format(
    round(($stats['numposts'] - $stats['numthreads']) / $stats['numthreads'], 2)
);
$postspermember = my_number_format(
    round($stats['numposts'] / $stats['numusers'], 2)
);
$threadspermember = my_number_format(
    round($stats['numthreads'] / $stats['numusers'], 2)
);

// Get number of days since board start (might need improvement)
$query = $db->simple_select('users', 'regdate', '', [
    'order_by' => 'regdate',
    'limit' => 1,
]);
...
if (!empty($most_replied)) {
    foreach ($most_replied as $key => $thread) {
        if (
            ...
            (!in_array($thread['fid'], $onlyusfids) ||
                ($mybb->user['uid'] && $thread['uid'] == $mybb->user['uid']))
        ) {
...
if (
    !$statistics ||
    $interval == 0 ||
    TIME_NOW - $interval > $statistics['time']
) {
...
$top_referrer = '';
if (
    $mybb->settings['statstopreferrer'] == 1 &&
    isset($statistics['top_referrer']['uid'])
) {
...
$havepostedpercent =
    my_number_format(round(($posters / $stats['numusers']) * 100, 2)) . '%';

$lang->todays_top_poster = $lang->sprintf(
    $lang->todays_top_poster,
    $topposter,
    my_number_format($topposterposts)
);
$lang->popular_forum = $lang->sprintf(
    $lang->popular_forum,
    $topforum,
    my_number_format($topforumposts),
    my_number_format($topforumthreads)
);
