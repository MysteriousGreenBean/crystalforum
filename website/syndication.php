...
$query = $db->simple_select(
    'forums',
    'name, fid, allowhtml, allowmycode, allowsmilies, allowimgcode, allowvideocode',
    '1=1 ' . $forumlist
);
$comma = ' - ';
...
$query = $db->simple_select(
    ...
    "visible='1' AND closed NOT LIKE 'moved|%' {$permsql} {$forumlist}",
...
if (!empty($firstposts)) {
    $firstpostlist =
        'pid IN(' . $db->escape_string(implode(',', $firstposts)) . ')';
    ...
    while ($post = $db->fetch_array($query)) {
        ...
        if (
            ...
            isset($attachments[$post['pid']]) &&
            is_array($attachments[$post['pid']])
        ) {
            foreach ($attachments[$post['pid']] as $attachment) {
                ...
                eval(
                    "\$attbit = \"" .
                        $templates->get('postbit_attachments_attachment') .
                        "\";"
                );
                if (
                    stripos(
