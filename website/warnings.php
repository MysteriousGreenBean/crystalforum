...
$templatelist =
    'warnings,warnings_warn_post,warnings_active_header,warnings_expired_header,warnings_warning,warnings_warn_existing,warnings_warn_type,warnings_warn_custom,warnings_warn_pm,warnings_view';
$templatelist .=
    ',warnings_view_post,warnings_view_user,warnings_view_revoke,warnings_view_revoked,warnings_warn_type_result,warnings_postlink,codebuttons,warnings_warn,warnings_warn_pm_anonymous';
$templatelist .=
    ',multipage,multipage_end,multipage_jump_page,multipage_nextpage,multipage_page,multipage_page_current,multipage_page_link_current,multipage_prevpage,multipage_start,warnings_no_warnings';
...
if ($mybb->input['action'] == 'do_warn' && $mybb->request_method == 'post') {
    ...
    if ($warningshandler->validate_warning()) {
        ...
        if (
            $mybb->get_input('send_pm', MyBB::INPUT_INT) == 1 &&
            ...
            if (
                $mybb->settings['allowanonwarningpms'] == 1 &&
                $mybb->get_input('pm_anonymous', MyBB::INPUT_INT)
            ) {
                $sender_uid = -1;
            ...
            if (!send_pm($pm, $sender_uid, true)) {
                $warningshandler->friendly_action .=
                    $lang->redirect_warned_pmerror;
            }
        }

        $plugins->run_hooks('warnings_do_warn_end');

        $lang->redirect_warned = $lang->sprintf(
            $lang->redirect_warned,
            htmlspecialchars_uni($user['username']),
            $warningshandler->new_warning_level,
            $warningshandler->friendly_action
        );
...
if ($mybb->input['action'] == 'warn') {
    ...
    if ($mybb->usergroup['maxwarningsday'] != 0) {
        ...
        $query = $db->simple_select(
            ...
            "issuedby='{$mybb->user['uid']}' AND dateline>'$timecut'"
        );
        $given_today = $db->fetch_field($query, 'given_today');
        if ($given_today >= $mybb->usergroup['maxwarningsday']) {
            error(
                $lang->sprintf(
                    $lang->warnings_reached_max_warnings_day,
                    $mybb->usergroup['maxwarningsday']
                )
            );
        }
    }
        ...
        while ($warning = $db->fetch_array($query)) {
            if ($warning['expired'] != $last_expired) {
                if ($warning['expired'] == 0) {
                    eval(
                        "\$warnings .= \"" .
                            $templates->get('warnings_active_header') .
                            "\";"
                    );
                } else {
                    eval(
                        "\$warnings .= \"" .
                            $templates->get('warnings_expired_header') .
                            "\";"
                    );
                }
            }
            ...
            eval(
                "\$warnings .= \"" . $templates->get('warnings_warning') . "\";"
            );
        }
        if ($warnings) {
            eval(
                "\$existing_warnings = \"" .
                    $templates->get('warnings_warn_existing') .
                    "\";"
            );
        }
    }
    ...
    if (!empty($warn_errors)) {
        ...
        if ($mybb->get_input('type', MyBB::INPUT_INT)) {
            $type_checked[$mybb->get_input('type', MyBB::INPUT_INT)] =
                "checked=\"checked\"";
        }
        ...
        $custom_reason = htmlspecialchars_uni(
            $mybb->get_input('custom_reason')
        );
        $custom_points = $mybb->get_input('custom_points', MyBB::INPUT_INT);
        $expires = $mybb->get_input('expires', MyBB::INPUT_INT);
        ...
        switch ($level['action']['type']) {
            case 1:
                if ($level['action']['length'] > 0) {
                    $ban_length = fetch_friendly_expiration(
                        $level['action']['length']
                    );
                ...
                $group_name =
                    $groupscache[$level['action']['usergroup']]['title'];
                $level['friendly_action'] = $lang->sprintf(
                    $lang->result_banned,
                    $group_name,
                    $period
                );
            ...
            case 2:
                if ($level['action']['length'] > 0) {
                    $period = fetch_friendly_expiration(
                        $level['action']['length']
                    );
    ...
    while ($type = $db->fetch_array($query)) {
        ...
        $new_warning_level = round(
            (($user['warningpoints'] + $type['points']) /
        ...
        if (!empty($new_level['friendly_action'])) {
            eval(
                "\$result = \"" .
                    $templates->get('warnings_warn_type_result') .
                    "\";"
            );
        }
    ...
    if ($mybb->settings['allowcustomwarnings'] != 0) {
        if (
            (empty($types) && empty($warn_errors)) ||
            $mybb->get_input('type') == 'custom'
        ) {
        ...
        eval(
            "\$custom_warning = \"" .
                $templates->get('warnings_warn_custom') .
                "\";"
        );
    }
    ...
    if (
        ...
        if ($mybb->settings['allowanonwarningpms'] == 1) {
            ...
            eval(
                '$anonymous_pm = "' .
                    $templates->get('warnings_warn_pm_anonymous') .
                    '";'
            );
        }
...
if ($mybb->input['action'] == 'do_revoke' && $mybb->request_method == 'post') {
    ...
    if (!trim($mybb->get_input('reason'))) {
        ...
        redirect(
            "warnings.php?action=view&wid={$warning['wid']}",
            $lang->redirect_warning_revoked
        );
    }
}
...
if ($mybb->input['action'] == 'view') {
    ...
    $query = $db->query(
		...
		WHERE w.wid='" .
            $mybb->get_input('wid', MyBB::INPUT_INT) .
            "'
	"
    );
    ...
    if (empty($user)) {
        $user = ['uid' => 0, 'username' => $lang->guest];
    }
    $user['username'] = htmlspecialchars_uni($user['username']);
    ...
    if (!empty($user['uid'])) {
        add_breadcrumb($lang->nav_profile, get_profile_link($user['uid']));
        add_breadcrumb(
            $lang->nav_warning_log,
            "warnings.php?uid={$user['uid']}"
        );
    } else {
    ...
    if ($warning['post_subject']) {
        ...
        eval(
            "\$warning_info = \"" .
                $templates->get('warnings_view_post') .
                "\";"
        );
    } else {
        eval(
            "\$warning_info = \"" .
                $templates->get('warnings_view_user') .
                "\";"
        );
    }
    ...
    if (!$warning['daterevoked']) {
        ...
        $revoked_user['username'] = htmlspecialchars_uni(
            $revoked_user['username']
        );
        $revoked_by = build_profile_link(
            $revoked_user['username'],
            $revoked_user['uid']
        );
        $revoke_reason = nl2br(htmlspecialchars_uni($warning['revokereason']));
        eval(
            "\$revoke = \"" . $templates->get('warnings_view_revoked') . "\";"
        );
    }
...
if (!$mybb->input['action']) {
    ...
    while ($warning = $db->fetch_array($query)) {
        ...
        if ($warning['post_subject']) {
            ...
            eval(
                "\$post_link = \"" .
                    $templates->get('warnings_postlink') .
                    "\";"
            );
        }
    ...
    if (!$warnings) {
        eval(
            "\$warnings = \"" . $templates->get('warnings_no_warnings') . "\";"
        );
    }
