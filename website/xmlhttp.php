...
if (
    isset($mybb->user['language']) &&
    $lang->language_exists($mybb->user['language'])
) {
...
if ($loadstyle != "def='1'") {
    $query = $db->simple_select(
        'themes',
        'name, tid, properties, allowedgroups',
        $loadstyle,
        ['limit' => 1]
    );
...
$templatelist =
    'postbit_editedby,xmlhttp_buddyselect_online,xmlhttp_buddyselect_offline,xmlhttp_buddyselect';
...
if (
    ...
    !in_array($mybb->input['action'], $closed_bypass)
) {
    // Show error
...
elseif (
    ...
    if (!is_moderator($forum['fid'], 'caneditposts')) {
        ...
        elseif (
            ...
            $post['dateline'] <
                TIME_NOW - $mybb->usergroup['edittimelimit'] * 60
        ) {
    ...
    exit();
} elseif ($mybb->input['action'] == 'edit_post') {
    ...
    if ($mybb->get_input('do') == 'get_post') {
        ...
        if (my_strtolower($charset) != 'utf-8') {
            if (function_exists('iconv')) {
                ...
                $editreason = @mb_convert_encoding(
                    $editreason,
                    $charset,
                    'UTF-8'
                );
        ...
        // No errors were found, we can call the update method.
            ...
            if (
                ...
                if ($thread['firstpost'] == $post['pid']) {
                    echo json_encode([
                        'moderation_thread' => $lang->thread_moderation,
                        'url' =>
                            $mybb->settings['bburl'] .
                            '/' .
                    ...
                    echo json_encode([
                        'moderation_post' => $lang->post_moderation,
                        'url' =>
                            $mybb->settings['bburl'] .
                            '/' .
                            get_thread_link($thread['tid']),
        ...
        if (
            ...
            $mybb->user['username'] = htmlspecialchars_uni(
                $mybb->user['username']
            );
            ...
            eval(
                "\$editedmsg = \"" . $templates->get('postbit_editedby') . "\";"
            );
        }
...
elseif ($mybb->input['action'] == 'get_multiquoted') {
    ...
    if (empty($mybb->input['load_all'])) {
        $from_tid =
            "p.tid != '" . $mybb->get_input('tid', MyBB::INPUT_INT) . "' AND ";
    } else {
        $from_tid = '';
    }
    ...
    while ($quoted_post = $db->fetch_array($query)) {
        if (
            (!is_moderator($quoted_post['fid'], 'canviewunapprove') &&
                $quoted_post['visible'] == 0) ||
            (!is_moderator($quoted_post['fid'], 'canviewdeleted') &&
                $quoted_post['visible'] == -1) ||
            (in_array($quoted_post['fid'], $onlyusfids) &&
                (!$mybb->user['uid'] ||
                    $quoted_post['thread_uid'] != $mybb->user['uid']))
        ) {
    ...
    exit();
} elseif ($mybb->input['action'] == 'refresh_captcha') {
    ...
    exit();
} elseif ($mybb->input['action'] == 'validate_captcha') {
    header("Content-type: application/json; charset={$charset}");
    $imagehash = $db->escape_string($mybb->get_input('imagehash'));
    $query = $db->simple_select(
        'captcha',
    ...
    if (
        my_strtolower($imagestring) ==
        my_strtolower($mybb->get_input('imagestring'))
    ) {
    ...
    $mybb->input['action'] == 'refresh_question' &&
    $mybb->settings['securityquestion']
) {
    header("Content-type: application/json; charset={$charset}");
    ...
    $mybb->input['action'] == 'validate_question' &&
    $mybb->settings['securityquestion']
) {
    header("Content-type: application/json; charset={$charset}");
    ...
    exit();
} elseif ($mybb->input['action'] == 'complex_password') {
    ...
    exit();
} elseif ($mybb->input['action'] == 'username_availability') {
    if (!verify_post_check($mybb->get_input('my_post_key'), true)) {
        xmlhttp_error($lang->invalid_post_code);
    }
    ...
    if ($user) {
        $lang->username_taken = $lang->sprintf(
            $lang->username_taken,
...
} elseif ($mybb->input['action'] == 'email_availability') {
    if (!verify_post_check($mybb->get_input('my_post_key'), true)) {
        xmlhttp_error($lang->invalid_post_code);
    }
...
} elseif ($mybb->input['action'] == 'get_buddyselect') {
    // Send our headers.
    ...
    if ($mybb->user['buddylist'] != '') {
        ...
        $query = $db->simple_select(
            'users',
            'uid, username, usergroup, displaygroup, lastactive, lastvisit, invisible',
        ...
        while ($buddy = $db->fetch_array($query)) {
            ...
            if (
                ...
                eval(
                    "\$online[] = \"" .
                        $templates->get('xmlhttp_buddyselect_online') .
                        "\";"
                );
            } else {
                eval(
                    "\$offline[] = \"" .
                        $templates->get('xmlhttp_buddyselect_offline') .
                        "\";"
                );
            }
        }
        ...
        eval(
            "\$buddy_select = \"" .
                $templates->get('xmlhttp_buddyselect') .
                "\";"
        );
        echo $buddy_select;
    } else {
...
} elseif ($mybb->input['action'] == 'get_referrals') {
    $lang->load('member');
    ...
    eval(
        "\$referrals = \"" .
            $templates->get('member_referrals_popup', 1, 0) .
            "\";"
    );
